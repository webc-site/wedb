...
- The integrity_crc32 seal is backfilled before publication and enforced on recovery from version 2; legacy formats below version 2 pass; decode_auto detects JSON (sonic-rs) vs bitcode by the first byte
...
- 完整性封签 integrity_crc32 自版本 2 起发布前回填、恢复强制校验；version < 2 遗留格式放行；decode_auto 按首字节自动识别 JSON（sonic-rs）/ bitcode
