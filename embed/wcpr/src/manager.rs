...
impl<D: Device> CheckpointManager<D> {
  ...
  /// RecoverHybridLogCheckpoint / RecoverIndexCheckpoint——上游 d20d63993 将该跳过
  /// 路径的吞异常改为 LogWarning，使「坏检查点被跳过」可区分于「检查点集为空」；
  /// 本实现自始即以 `warn!` 记录被跳过的 Token 与原因，语义一致）。目录中不存在
  /// 任何 Token 时返回 `NoValidCheckpoint`。
  ///
