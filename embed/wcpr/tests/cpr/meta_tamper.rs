...
use wcpr::{
  CheckpointManager, CheckpointMeta, CheckpointType, Error, FORMAT_VERSION, meta_filename,
  meta_tmp_filename,
};
...
/// 改写指定检查点 meta 的任一字段后按 JSON 原样写回（封签不重算）
fn tamper_meta(ckpt_dir: &Path, token: u128, mutate: impl FnOnce(&mut CheckpointMeta)) -> Void {
  ...
  let mut meta = CheckpointMeta::decode_json(&fs::read(&path)?)?;
  mutate(&mut meta);
  fs::write(&path, meta.encode_json()?)?;
  Ok(())
}

/// 封签逐字段校验：篡改任意载荷字段（created_at）后恢复必须报 MetaChecksumMismatch，
/// 结构合法的 JSON 篡改无法逃逸逐字段摘要比对
#[test]
...
fn single_bit_flip_rejects_recovery() -> Void {
  ...
  rt.block_on(async {
    ...
    let mut meta = CheckpointMeta::decode_json(&pristine)?;
    meta.seal();
    fs::write(&meta_path, meta.encode_json()?)?;
