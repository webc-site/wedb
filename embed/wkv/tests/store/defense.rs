...
fn test_edge_truncated_address_defense_and_backward_scan_termination() -> Void {
  ...
  rt.block_on(async {
    ...
    assert_eq!(prev, addr_chain1);

    // 验证截断防御边界逻辑：若 prev < begin_address 则终止追溯
    assert!(prev < store.begin_address(), "前驱版本已被截断到历史无效区");
    let read_past_trunc = store.hlog.read_record(prev).await;
