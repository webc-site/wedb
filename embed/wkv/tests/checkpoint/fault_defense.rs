...
fn test_corrupted_files_defense() -> Void {
  ...
  rt.block_on(async {
    ...
    // 1. 测试 TokenMismatch：篡改元数据中的 token
    {
      ...
      let json = sonic_rs::to_vec_pretty(&meta_corrupted)?;
      write(&meta_file, json)?;
      ...
      // 恢复正确的元数据
      let json_valid = sonic_rs::to_vec_pretty(&meta)?;
      write(&meta_file, json_valid)?;
    }
    ...
    {
      let tmp_meta = ckpt_dir.join(format!("checkpoint_{}_broken.meta.tmp", token));
      let tmp_index = ckpt_dir.join(format!("index_{}_broken.ckpt.tmp", token));
...
fn test_adversarial_fault_injection() -> Void {
  ...
  rt.block_on(async {
    ...
    //    （语义字段篡改后重新封签，聚焦地址语义校验而非封签比对）
    {
      ...
      corrupt_meta.seal();
      write(&meta_file, sonic_rs::to_vec_pretty(&corrupt_meta)?)?;
...
fn test_meta_integrity_seal_defense() -> Void {
  ...
  rt.block_on(async {
    ...
    let old_file = ckpt_dir.join(format!("checkpoint_{}.meta", meta_old.token));
    let new_file = ckpt_dir.join(format!("checkpoint_{}.meta", meta_new.token));

    // 1. 篡改语义字段而未重封签：静默错误恢复类损坏被拦截（期望值/实际值互换可辨）
    {
      let mut tampered = meta_new.clone();
      tampered.store_meta.page_size = 32 * 1024; // 合法但错误的取值
      write(&new_file, sonic_rs::to_vec_pretty(&tampered)?)?;
    ...
    // 2. 封签字段自身被翻转：内容合法但封签不符，同样拦截
    {
      ...
      write(&new_file, sonic_rs::to_vec_pretty(&flipped)?)?;
    ...
    // 4. 篡改后重新封签（信息性字段变更，属发布时合法操作）：恢复成功且数据完整
    {
      ...
      benign.seal();
      write(&new_file, sonic_rs::to_vec_pretty(&benign)?)?;
