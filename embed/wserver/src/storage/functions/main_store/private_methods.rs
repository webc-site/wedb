...
use super::upsert_methods::{LogRecord, UpsertInfo, StringOutput};

impl MainSessionFunctions {
    /// garnet相对路径:garnet/libs/server/Storage/Functions/MainStore/PrivateMethods.cs:WriteLogUpsert
    pub fn write_log_upsert(&self, _key: &[u8], _input: &mut StringInput, _value: &[u8], _version: i64, _session_id: i64) {
    }

    /// garnet相对路径:garnet/libs/server/Storage/Functions/MainStore/PrivateMethods.cs:WriteLogRMW
    pub fn write_log_rmw(&self, _key: &[u8], _input: &mut StringInput, _version: i64, _session_id: i64) {
    }
    ...
    pub fn in_place_writer_for_log_record_value(&self, _log_record: &mut LogRecord, _input: &mut StringInput, _input_log_record: &LogRecord, _output: &mut StringOutput, _upsert_info: &mut UpsertInfo) -> bool {
        true
    }

    /// garnet相对路径:garnet/libs/server/Storage/Functions/MainStore/PrivateMethods.cs:InPlaceWriterForSpanValue
    pub fn in_place_writer_for_span_value(&self, _log_record: &mut LogRecord, _input: &mut StringInput, _src_value: &[u8], _output: &mut StringOutput, _upsert_info: &mut UpsertInfo) -> bool {
        true
    }
}
