use mlua::{Lua, Result, Value, Variadic, Table};
...
impl LuaRunner {
    pub fn new() -> Result<Self> {
        ...
        let call_func = lua.create_function(|lua, args: Variadic<String>| {
            if args.is_empty() {
                return Err(mlua::Error::RuntimeError("Please specify at least one argument for redis.call()".to_string()));
            }
        ...
        let pcall_func = lua.create_function(|lua, args: Variadic<String>| {
            if args.is_empty() {
                return Err(mlua::Error::RuntimeError("Please specify at least one argument for redis.pcall()".to_string()));
            }
            Ok(Value::String(lua.create_string("OK")?))
        })?;
