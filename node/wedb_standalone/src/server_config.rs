...
use crate::{
  config::{runtime_server_config::RuntimeServerConfig, server_config_type::ServerConfigType},
  resp::{
    cmd_strings as cs,
    cmd_strings::{
  ...
  servers::garnet_server_options::{previous_power_of_2, try_parse_size_bytes},
};
...
mod tests {
  ...
  fn config_get_runtime_params_and_star() {
    ...
    // 零参 → wrong args
    let mut out = Vec::new();
    let _ = s.network_config_get(&[], &rc, &mut out).unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_get(&[b"maxmemory"], &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"*0\r\n");
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_get(&[b"slave-read-only"], &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"*2\r\n$15\r\nslave-read-only\r\n$2\r\nno\r\n");
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_get(&[b"cluster-node-timeout"], &rc, &mut out)
      .unwrap();
    ...
    // SET 后 GET 反映新值
    let _ = rc.try_set(ServerConfigType::ClusterNodeTimeout, "120");
    let mut out = Vec::new();
    let _ = s
      .network_config_get(&[b"cluster-timeout"], &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"*2\r\n$20\r\ncluster-node-timeout\r\n$3\r\n120\r\n");

    // "*" 输出含只读回落参数与运行时参数
    let mut out = Vec::new();
    let _ = s.network_config_get(&[b"*"], &rc, &mut out).unwrap();
  ...
  fn config_set_validation_and_disabled_subsystems() {
    ...
    // 奇数参数
    let mut out = Vec::new();
    let _ = s.network_config_set(&[b"a"], &rc, &mut out).unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"maxmemory", b"1g"], &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"cert-file-name", b"a.pem"], &rc, &mut out)
      .unwrap();
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"cluster-password", b"p"], &rc, &mut out)
      .unwrap();
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"memory", b"abc"], &rc, &mut out)
      .unwrap();
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"memory", b"1g"], &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"READCACHE-MEMORY", b"1g"], &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"index", b"1000"], &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"index", b"1024"], &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
  ...
  fn config_set_dynamic_runtime_values() {
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"slowlog-log-slower-than", b"5000"], &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    assert_eq!(
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"compaction-force-delete", b"yes"], &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"compaction-type", b"Lookup"], &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"compaction-force-delete", b"maybe"], &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    let _ = s
      .network_config_set(&[b"databases", b"8"], &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
  ...
  fn config_rewrite_ok_without_cluster() {
    ...
    let mut out = Vec::new();
    let _ = s.network_config_rewrite(&[], &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");

    // 带参 → wrong args
    let mut out = Vec::new();
    let _ = s.network_config_rewrite(&[b"extra"], &mut out).unwrap();
    assert_eq!(
      out,
