...
use wedb_standalone::{
  lua::{
    ...
    lua_runner::{LuaRunner, RespObject},
    scripting_api::ScriptingApi,
    session_script_cache::SessionScriptCache,
    storage_scripting_api::StorageScriptingApi,
  },
...
impl<'a> Harness<'a> {
  ...
  where
    F: FnOnce(&mut LuaSessionContext) -> bool,
  {
...
/// Lua 未启用形态。
#[test]
fn test_lua_disabled_error() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (_dir, store) = open_store("lua-disabled.db")?;
    let session = store.new_session()?;
    let storage = StorageSession::new(session.enter_batch());
    let mut api = StorageScriptingApi::new(&storage, None);
    ...
    assert!(!LuaCommands::check_lua_enabled(&mut ctx));
    assert_eq!(
      ctx.out,
      b"-ERR This instance has Lua scripting support disabled\r\n"
    );
    // TryEVAL 在禁用时短路（清空后单次写出）
    let disabled_len = ctx.out.len();
    ctx.out.clear();
    assert!(LuaCommands::try_eval(&mut ctx));
    assert_eq!(ctx.out.len(), disabled_len);
    OK
  })
}
...
/// 哑会话形态（respServerSession == null 对标）：redis.call 返回 nil。
#[test]
fn test_eval_no_session() {
  let mut runner = LuaRunner::new(
    LuaLoggingMode::Silent,
    None,
    Default::default(),
    b"return redis.call('GET', 'k')".to_vec(),
    false,
    "0.0.0.0",
  )
  .unwrap();
