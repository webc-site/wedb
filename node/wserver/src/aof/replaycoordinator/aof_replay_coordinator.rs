...
pub enum TxnAction {
  ...
  Handled,
  /// TxnCommit：返回提交会话 id，调用方取组并重放。
  Commit { session_id: i32 },
  /// 非事务条目：交还调用方按常规分派。
  None,
}
...
impl AofReplayCoordinator {
  ...
  pub fn new(virtual_sublog_count: usize, multi_log_enabled: bool) -> Self {
    Self {
      contexts: (0..virtual_sublog_count.max(1))
        ...
        .collect(),
      multi_log_enabled,
      leader_barriers: Mutex::new(HashMap::new()),
      ...
      .contains_key(&session_id)
    {
      match op_type {
        ...
        AofEntryType::TxnAbort => {
          self.clear_session_txn(virtual_sublog_idx, session_id);
        ...
        AofEntryType::TxnCommit => {
          let in_fuzzy = self.context(virtual_sublog_idx).in_fuzzy_region;
          let group = self.take_transaction_group(virtual_sublog_idx, session_id);
          if in_fuzzy {
            // 模糊区：登记提交标记 + 整组入队延后重放
            if let Some(group) = group {
              self
                .context(virtual_sublog_idx)
                .add_to_fuzzy_region_buffer(group, entry.to_vec());
            }
          } else {
            // 立即提交：组经提交出口交处理器重放
            if let Some(group) = group {
        ...
        _ => {
          self.push_transaction_operation(virtual_sublog_idx, session_id, entry);
          TxnAction::Handled
        }
      }
    } else {
      match op_type {
        AofEntryType::TxnStart => {
          ...
          self.context(virtual_sublog_idx).add_transaction_group(
            session_id,
            virtual_sublog_idx,
            participant_count,
  ...
  /// 组内操作追加。
  fn push_transaction_operation(&self, virtual_sublog_idx: usize, session_id: i32, entry: &[u8]) {
