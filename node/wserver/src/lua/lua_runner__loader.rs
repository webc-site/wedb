...
pub struct LuaRunner_Loader;

impl LuaRunner_Loader {
  ...
  pub fn prepare_loader_block_bytes(allowed_functions: &HashSet<String>) -> &'static str {
    ...
    for (leading, _trailing) in &partial {
...
mod tests {
  ...
  use super::LuaRunner_Loader;

  #[test]
  fn default_loader_block_exports_everything() {
    let empty = HashSet::default();
    let block = LuaRunner_Loader::prepare_loader_block_bytes(&empty);
  ...
  fn subset_loader_block_exports_only_allowed() {
    let allowed: HashSet<String> = ["redis", "os.clock"]
      ...
      .map(String::from)
      .collect();
    let block = LuaRunner_Loader::prepare_loader_block_bytes(&allowed);
    assert!(block.contains("    redis=redis;"));
    // os.clock 为局部导出形态。
  ...
  fn compile_source_passes_valid_and_keeps_invalid() {
    ...
    assert_eq!(LuaRunner_Loader::compile_source(valid), valid.to_vec());
    // 不可编译 → 按失败路径原样返回。
    let invalid = b"return ]]";
    assert_eq!(LuaRunner_Loader::compile_source(invalid), invalid.to_vec());
  }
}
