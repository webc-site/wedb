...
//!
//! 线格式：1 字节类型标记 + 各对象自有载荷。有序集合走
//! [`SortedSetObject`] 的 C# BinaryWriter 兼容格式；列表/集合/哈希
//! 经 wobject 载荷承载。自定义类型（CustomDeserialize）以
...
use crate::{objects::sortedset::sorted_set_object::SortedSetObject, types::GarnetObjectType};
...
mod tests {
  ...
  fn zset_round_trip_with_type_prefix() {
    ...
    // 类型标记字节在前
    assert_eq!(bytes[0], GarnetObjectType::SortedSet as u8);

    let restored = GarnetObjectSerializer::deserialize_bytes(&bytes)
      .unwrap()
      .unwrap();
    match restored {
      GarnetObjectValue::SortedSet(r) => {
        assert_eq!(r.count(), 2);
        assert_eq!(r.try_get_score(b"b"), Some(2.0));
        assert!(r.equals(&zset));
      }
      _other => panic!("unexpected variant"),
    }
  }

  #[test]
  fn null_and_hash_round_trip() {
    let bytes = GarnetObjectSerializer::serialize_bytes(&GarnetObjectValue::Null).unwrap();
    assert_eq!(bytes, [GarnetObjectType::Null as u8]);
    assert!(
      GarnetObjectSerializer::deserialize_bytes(&bytes)
        .unwrap()
        .is_none()
    );

    let hash = HashObject::new();
    hash.hash.pin().insert(b"k".to_vec(), b"v".to_vec());
    let bytes = GarnetObjectSerializer::serialize_bytes(&GarnetObjectValue::Hash(hash)).unwrap();
    match GarnetObjectSerializer::deserialize_bytes(&bytes)
      .unwrap()
      .unwrap()
    {
      GarnetObjectValue::Hash(h) => {
        let pin = h.hash.pin();
        assert_eq!(
          pin.get(&b"k".to_vec()).map(|v| v.as_slice()),
          Some(b"v".as_slice())
        );
      }
      _other => panic!("unexpected variant"),
  ...
  #[test]
  fn custom_band_and_reserved_marker() {
    // 自定义段透传
    let custom = GarnetObjectValue::Custom {
      type_id: 0x20,
      payload: vec![1, 2, 3],
    };
    let bytes = GarnetObjectSerializer::serialize_bytes(&custom).unwrap();
    match GarnetObjectSerializer::deserialize_bytes(&bytes)
      .unwrap()
      .unwrap()
    {
    ...
    // 内建保留段快速失败
    assert!(GarnetObjectSerializer::custom_deserialize(0x05, &[]).is_err());

    // 保留格式标记（0xFC..0xFF）拒绝
    let mut reserved = vec![0xFC_u8];
    reserved.extend_from_slice(b"whatever");
    assert!(GarnetObjectSerializer::deserialize_bytes(&reserved).is_err());
  }
}
