...
impl GarnetClient {
    ...
    pub async fn execute_for_string_result_async(&self, command: &[&str]) -> Result<String> {
        ...
        tx.clone().send(CommandItem::Command { cmd, resp_tx }).await.map_err(|_| Error::Other("Network loop died".into()))?;
    ...
    pub async fn execute_for_string_array_result_async(&self, command: &[&str]) -> Result<Vec<String>> {
        let (resp_tx, resp_rx) = oneshot::oneshot();
        let cmd = command.iter().map(|s| s.to_string()).collect();
        let tx = self.tx.as_ref().ok_or_else(|| Error::Other("Not connected".into()))?;
        tx.clone().send(CommandItem::CommandForArray { cmd, resp_tx }).await.map_err(|_| Error::Other("Network loop died".into()))?;
        resp_rx.await.map_err(|_| Error::Other("Response channel closed".into()))?
    }
