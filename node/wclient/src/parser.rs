...
impl RespReadResponseUtils {
    pub fn try_read_simple_string(ptr: &mut &[u8]) -> Result<Option<String>> {
        if ptr.is_empty() { return Ok(None); }
        if ptr[0] != b'+' { return Err(Error::Other(format!("Unexpected token {}", ptr[0] as char))); }
        if let Some(r_idx) = Self::find_crlf(&ptr[1..]) {
            let res = str::from_utf8(&ptr[1..1 + r_idx]).map_err(|e| Error::Other(e.to_string()))?.to_string();
            *ptr = &ptr[1 + r_idx + 2..];
            Ok(Some(res))
        } else { Ok(None) }
    }
    ...
    pub fn try_read_integer(ptr: &mut &[u8]) -> Result<Option<i64>> {
        ...
        let val: i64 = s.parse().map_err(|_| Error::Other("Invalid integer".into()))?;
        Ok(Some(val))
    }
    ...
    pub fn try_read_string_with_length_header(ptr: &mut &[u8]) -> Result<Option<Option<String>>> {
        if ptr.is_empty() { return Ok(None); }
        if ptr[0] != b'$' { return Err(Error::Other(format!("Unexpected token {}", ptr[0] as char))); }
        let r_idx = if let Some(i) = Self::find_crlf(&ptr[1..]) { i } else { return Ok(None); };
        
        let len_str = str::from_utf8(&ptr[1..1 + r_idx]).map_err(|e| Error::Other(e.to_string()))?;
        let len: isize = len_str.parse().map_err(|_| Error::Other("Invalid length".into()))?;
        ...
        if ptr.len() < content_start + len + 2 { return Ok(None); }
        
        if &ptr[content_start + len..content_start + len + 2] != b"\r\n" {
            return Err(Error::Other("Missing CRLF after bulk string".into()));
        }
        
        let res = str::from_utf8(&ptr[content_start..content_start + len]).map_err(|e| Error::Other(e.to_string()))?.to_string();
        *ptr = &ptr[content_start + len + 2..];
        Ok(Some(Some(res)))
    }

    pub fn try_read_string_array_with_length_header(ptr: &mut &[u8]) -> Result<Option<Option<Vec<String>>>> {
        if ptr.is_empty() { return Ok(None); }
        if ptr[0] != b'*' { return Err(Error::Other(format!("Unexpected token {}", ptr[0] as char))); }
        let r_idx = if let Some(i) = Self::find_crlf(&ptr[1..]) { i } else { return Ok(None); };
        
        let len_str = str::from_utf8(&ptr[1..1 + r_idx]).map_err(|e| Error::Other(e.to_string()))?;
        let len: isize = len_str.parse().map_err(|_| Error::Other("Invalid array length".into()))?;
        
        *ptr = &ptr[1 + r_idx + 2..];
        
        if len < 0 { return Ok(Some(None)); }
