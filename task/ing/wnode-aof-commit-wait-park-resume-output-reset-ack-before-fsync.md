甄别结论：通过（fix 席 r27，2026-09-27，定级 P2 维持）。逐锚现码双侧复跑属实：rust 侧 wedb/wnode/src/resp/parser/resp_command.rs:480-489 复位判据 pending_output_len()==0、调用点 :273-275（aof_commit_mode_gate 门控）亲验；pump.rs:172-174 pending_output_len 即 output.len()、:198-206 take_output_into 冲出后 output.clear()、:139-149 resolve_blocked_wait_into、:156-168 resolve_slow_wait_into 皆先冲会话缓冲；drive.rs:205 轮首 resp_pooled.clear、:208 drain_pubsub_into（审核补出的第六出口，实测在场）、:266 停车臂 flush_output_into、:371 出网臂单点读会话字段、:395 write_all 亲验；resp_session_consumer.rs:158-172 轮尾与违规批均经 take_output_into；txn.rs:30-32 批首 clear；wconf/src/node_options.rs:577-579 --aof-commit-wait default_value_t=false（默认档零影响）。C# 侧 garnet/libs/server/Resp/RespServerSession.cs:1440-1465 Send 唯一致命出口内 if (waitForAofBlocking) 先 WaitForCommitAsync 后 SendResponse，:1468 起 DebugSend 系 debug-only 非反证；RespCommand.cs:1267-1283 复位判据 dcurr==GetResponseObjectHead（含未出网字节）；StoreWrapper.cs:504-509；ListCommands.cs:283-284 网络线程 BlockingWait 同步收割。查重：doc/zh/deviations.md 对 wait_for_aof_blocking/pending_output_len/take_output_into/aof-commit-wait 零在册命中；task/reject/wnode-blocked-wait-pubsub-mailbox-stall 系挂起窗口邮箱排空他轴，不构成并案。分叉链实测复核：内层重入消费轮（drive.rs:211 起）在写出段之前再解析流水线后续命令，此刻 session.output 已被 take_output_into 清空，handle_aof_commit_mode 见 PING 即复位标记，:371 读到 false 跳过等待，resp_pooled 内积存应答未经 fsync 直发——缺陷成立。执行按票面审核裁定方案 1-4 落地，沙箱席 b01e。
