...
- 集合类型对标 C# 内存对象（garnet/libs/server/Objects/ 下 Hash/Set/List/ZSet 均为内存对象）：Hash/Set/List/ZSet 一律 wcol 内存对象信封存储（KeyTag::ObjectEnvelope + 内层 GarnetObjectType 标签区分），不做按大小分层；例外 RangeIndex 走独立持久化 wbftree（对齐 C# `RangeIndexManager` / `RangeIndexStub`，单树单文件生命周期，提供 ri_set, ri_get_callback, ri_del, ri_exists, ri_len, ri_scan, ri_range）
- 二进制 Key 刚性防穿透：
...
- 严格删空生命周期与原子墓碑：
  ...
  - O(1) 版本号栅栏逻辑秒删：DEL key 时推进版本号 / 写幽灵 meta 墓碑元记录（size=0），物理垃圾交由后台 Compaction 异步回收
- O(1) 复杂度计数规约：
  - HLEN / SCARD / ZCARD / LLEN：直读 wcol 内存对象计数 O(1)；RI.LEN 直读 MetaValue.size，严禁扫树
  - 全区间 RI.COUNT：短路直读 MetaValue.size；非全区间传 ScanReturnField::Key 纯键扫描
- 类型枚举：
