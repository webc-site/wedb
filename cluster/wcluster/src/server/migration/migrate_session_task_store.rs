...
impl MigrateSessionTaskStore {
    ...
    pub fn try_add_migrate_session(
        ...
        source_node_id: &str, target_address: &str, target_port: i32, target_node_id: &str,
        username: &str, passwd: &str, copy_option: bool, replace_option: bool, timeout: i32,
        slots: HashSet<i32>, sketch: Sketch, transfer_option: TransferOption,
    ) -> Option<Arc<MigrateSession>> {
        let m_session = Arc::new(MigrateSession::new(
            cluster_provider, source_node_id, target_address, target_port, target_node_id,
            username, passwd, copy_option, replace_option, timeout, slots.clone(), sketch, transfer_option
        ));
