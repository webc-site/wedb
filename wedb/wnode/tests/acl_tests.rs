...
use wacl::{AccessControlList, AclParser, GarnetAclAuthenticator, User, acl_password_check};
...
fn delete_default_user_namespaced() {
  ...
  let mut out = Vec::new();
  session
    .network_acl_del_user(&ctx_in_ns(&auth, 0), &store, &[b"default"], &mut out)
    .unwrap();
  assert_eq!(
    out,
  ...
  session
    ...
    .unwrap();
  assert_eq!(out, b"+OK\r\n");
  assert!(store.read(1, b"default").unwrap().is_some());

  // 租户可删自身 default：确有删除 :1，墓碑后点查落空
  let mut out = Vec::new();
  session
    .network_acl_del_user(&ctx_in_ns(&auth, 1), &store, &[b"default"], &mut out)
    .unwrap();
  assert_eq!(out, b":1\r\n");
  assert!(
    store.read(1, b"default").unwrap().is_none(),
    "租户 default 删除后点查落空"
  );
...
fn acl_cat_lists_all_categories() {
  let acl = Arc::new(AccessControlList::new("").unwrap());
  let mut auth = GarnetAclAuthenticator::new(Arc::clone(&acl));
  assert!(auth.authenticate(b"", b"x", acl_password_check));
  let ctx = ctx_for(&auth, None);
  let session = RespServerSession::default();
