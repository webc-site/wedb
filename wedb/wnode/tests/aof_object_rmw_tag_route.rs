...
/// 不得造半截幻影信封对象
#[test]
fn object_store_rmw_meta_tag_missing_stub_skips_not_envelope() {
...
/// 演化失配）：留痕跳过，绝不落信封通道
#[test]
fn object_store_rmw_unexpected_tag_skips_not_envelope() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let dir = tempfile::tempdir().unwrap();
...
#[test]
fn object_store_rmw_envelope_tag_keeps_envelope_channel() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let dir = tempfile::tempdir().unwrap();
