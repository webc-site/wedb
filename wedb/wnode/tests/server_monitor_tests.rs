...
#[compio::test]
async fn monitor_sampling_loop_rolls_metrics() -> aok::Result<()> {
...
#[compio::test]
async fn session_dispose_merges_into_monitor_history() -> aok::Result<()> {
...
#[compio::test]
async fn session_latency_metrics_aggregation_and_resp_commands() -> aok::Result<()> {
  ...
  let registry = Arc::new(ConsumerRegistry::new());
  registry.register(100, "127.0.0.1:50001".into(), "127.0.0.1:6379".into());
  ...
  // 会话延迟表不镜像给注册表条目：属主独占直写，样本由本任务在版本翻转点
  // 按引用直并全局延迟表（C# ActiveConsumers 直查臂的 rust 归属转移）
  ...
  // 2. 驱动采样轮推进迭代时钟；上一版本双缓冲槽由会话在下一批命令的
  // 版本翻转点按引用并入全局延迟表（属主线程归并，采样轮不再二转）
  run_iterations(&monitor, &registry, 1).await;
...
#[compio::test]
async fn resetstat_stats_branch_fires_gossip_and_reviv_arms() -> aok::Result<()> {
  let monitor = Arc::new(GarnetServerMonitor::new(1, true, false, false));
  let registry = Arc::new(ConsumerRegistry::new());
...
/// 条目镜像
#[compio::test]
async fn monitor_sample_carries_full_session_metrics_via_handle() -> aok::Result<()> {
  let monitor = Arc::new(GarnetServerMonitor::new(1, true, false, false));
  let registry = Arc::new(ConsumerRegistry::new());
...
/// INFO RESET STATS 复位即时性：挂接句柄的空闲连接（永不进下一批镜像同步）
/// 9 字段在复位轮立即归零、下一采样轮不回灌（C# CleanupGlobalStats
/// GarnetServerMonitor.cs:196-207 对全部活跃消费者原地 GetSessionMetrics.Reset()
/// 的跨线程直清承接；旧握手位形态下句柄复位依赖属主消费，空闲连接 9 字段
/// 滞留旧值被下一轮采样回灌，RESETSTAT 失效）
#[compio::test]
async fn resetstat_zeroes_idle_session_handle_without_refill() -> aok::Result<()> {
  let monitor = Arc::new(GarnetServerMonitor::new(1, true, false, false));
  let registry = Arc::new(ConsumerRegistry::new());
  let entry = registry.register(7, "127.0.0.1:50007".into(), "127.0.0.1:6379".into());

  // 会话挂接句柄并累加 9 字段（不写条目镜像的复位敏感面），随后转入空闲：
  // 本测试无属主连接任务消费镜像同步，等价于空闲连接永不进批的旧缺陷场景
  let handle = Arc::new(SessionMetricsHandle::default());
  entry.attach_session_metrics(Some(Arc::clone(&handle)));
  handle.incr_total_found(11);
  handle.incr_total_notfound(3);
  handle.incr_total_pending(2);
  handle.add_total_write_commands_processed(5);
  handle.add_total_read_commands_processed(6);
  ...
  // 复位前采样轮：9 字段旧值并入全局（复位基线在位）
  run_iterations(&monitor, &registry, 1).await;
  let gsm = monitor
    .snapshot()
    .expect("采样轮装配全局指标快照")
    .global_session_metrics;
  assert_eq!(gsm.total_found, 11);
  assert_eq!(gsm.total_number_resp_server_session_exceptions, 4);

  // INFO RESET STATS 复位轮：全局与历史清零 + reset_active_sessions 跨线程
  // 直清挂接句柄（C# CleanupGlobalStats 对 ActiveConsumers 原地 Reset）
  monitor.set_info_reset_flag(InfoMetricsType::Stats);
  run_iterations(&monitor, &registry, 1).await;

  // 空闲连接句柄 9 字段立即归零（条目镜像快照即监视器读出口）
  let m = registry.monitor_sample().sessions[0].metrics;
  assert_eq!(m.total_pending, 0);
  assert_eq!(m.total_found, 0);
  assert_eq!(m.total_notfound, 0);
  assert_eq!(m.total_cluster_commands_processed, 0);
  assert_eq!(m.total_write_commands_processed, 0);
  assert_eq!(m.total_read_commands_processed, 0);
  assert_eq!(m.total_transactions_commands_received, 0);
  assert_eq!(m.total_transaction_commands_execution_failed, 0);
  assert_eq!(m.total_number_resp_server_session_exceptions, 0);
  ...
  let gsm = monitor
    ...
    .expect("复位轮装配全局指标快照")
    .global_session_metrics;
  assert_eq!(gsm.total_pending, 0);
  assert_eq!(gsm.total_found, 0);
  assert_eq!(gsm.total_notfound, 0);
  assert_eq!(gsm.total_cluster_commands_processed, 0);
  assert_eq!(gsm.total_write_commands_processed, 0);
  assert_eq!(gsm.total_read_commands_processed, 0);
  assert_eq!(gsm.total_transactions_commands_received, 0);
  assert_eq!(gsm.total_transaction_commands_execution_failed, 0);
  assert_eq!(gsm.total_number_resp_server_session_exceptions, 0);

  // 下一采样轮（会话保持空闲）：无旧值回灌，RESETSTAT 基线成立
  run_iterations(&monitor, &registry, 1).await;
  let gsm = monitor
    .snapshot()
    .expect("复位后采样轮装配全局指标快照")
    .global_session_metrics;
  assert_eq!(
    (
      gsm.total_pending,
      gsm.total_found,
      gsm.total_notfound,
      gsm.total_cluster_commands_processed,
      gsm.total_write_commands_processed,
      gsm.total_read_commands_processed,
      gsm.total_transactions_commands_received,
      gsm.total_transaction_commands_execution_failed,
      gsm.total_number_resp_server_session_exceptions
    ),
    (0, 0, 0, 0, 0, 0, 0, 0, 0),
    "空闲连接句柄已直清，下一采样轮不得回灌复位前存量"
  );
  Ok(())
}
