...
/// 向量管理器（生产装配同形态：回调无状态，会话按执行域绑定；直调处理项经
...
fn interrupted_mark_delete_then_aof_replay_no_resurrect() {
  ...
  rt.block_on(async {
    ...
    vadd_two(&mut consumer, b"vs_k1").await;
    ...
    // RESP DEL：登记表摘除
    assert_eq!(
      pump(&mut consumer, &frame(&[b"DEL", b"vs_k1"])).await,
      b":1\r\n"
    );
    assert!(
    ...
    vm.process_request_cleanup(old_ctx).await;
    ...
    // 清理迭代恢复：旧上下文归还空闲池
    vm.process_cleanup(old_ctx).await;
...
fn interrupted_cleanup_phase_context_returned() {
  ...
  rt.block_on(async {
    ...
    vadd_two(&mut consumer, b"vs_k2").await;
    let old_ctx = context_of(
      &vm
    ...
    pump(&mut consumer, &frame(&[b"DEL", b"vs_k2"])).await;
    vm.process_request_cleanup(old_ctx).await;
    assert!(is_cleaning_up(&vm, old_ctx));

    // 清理中上下文不可被新分配拿走（next_vector_set_context 跳过 cleaning_up）
    let fresh = vm.next_vector_set_context(0).await.unwrap();
...
fn suppress_cleanup_delete_ignored_during_rename_window() {
  ...
  rt.block_on(async {
    ...
    vadd_two(&mut consumer, b"vs_k3").await;
...
fn replay_rebuild_then_delete_stays_deleted() {
  ...
  rt.block_on(async {
    ...
    assert!(
      vm.service.get_attribute(ctx, b"el_x").await.as_deref() == Some(b"attr".as_slice()),
      "重放属性应恢复"
    );

    // RESP DEL + 标记删除（清理迭代中断：盘上元素扫描未跑）
    assert_eq!(
      pump(&mut consumer, &frame(&[b"DEL", b"vs_k4"])).await,
      b":1\r\n"
    );
    vm.process_request_cleanup(ctx).await;
    assert!(
