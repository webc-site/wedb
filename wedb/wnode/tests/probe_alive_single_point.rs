...
/// VADD 写族挂起化（SlowWait 停车）后泵等价驱动：同步段 pump 返回空，
/// 挂起体取走 await 闭环后应答按序并入（对标网络泵 take_slow_wait →
/// resolve；与本文件 pump 的降级判定语义解耦——EXISTS 探针的挂起槽
/// 判定面 [`fast_exists`] 不经此处）
async fn vadd(c: &mut RespSessionConsumer, key: &[u8]) {
  let vec3: Vec<u8> = [1.0f32, 2.0, 3.0]
    .iter()
    .flat_map(|v| v.to_le_bytes())
    .collect();
  let mut out = pump(c, &[b"VADD", key, b"FP32", &vec3, b"elem1"]);
  let slow = c.take_slow_wait().expect("VADD 须登记写族挂起体");
  out.extend_from_slice(&slow.resolve().await);
  assert_eq!(out, b":1\r\n", "VADD 创建向量集失败");
}

/// 删向量集键（DEL 触发 delete_miss_hook 降级挂起 SlowWait 异步收口）
async fn del(c: &mut RespSessionConsumer, key: &[u8]) -> Vec<u8> {
  let mut out = pump(c, &[b"DEL", key]);
...
fn registry_fourth_state_agrees_across_sites_and_dbs() {
  ...
  // 全体探测驱动置于 runtime 上下文内（slow_exists 内层 block_on 同线程复用）
  rt.block_on(async {
    ...
    assert_eq!(del(&mut c, b"r:vs").await, b":1\r\n");
    assert!(
...
fn cold_evicted_keys_keep_same_verdict_through_slow_arm() {
  ...
  rt.block_on(async {
    ...
    vadd(&mut c, b"c:vs").await;

    // 冷化前：全部由同步快路径应答（无降级）
    let mut before = Vec::new();
    for k in keys {
      let out =
        fast_exists(&mut c, &[*k]).unwrap_or_else(|| panic!("{:?} 冷化前必须快路径应答", *k));
      before.push(out);
    }
    assert_eq!(
      before,
...
fn dead_meta_record_answers_missing_across_all_surfaces() {
  ...
  // 1 死元记录（Hash 型、size=0）直写 KeyTag::Meta，并造两版链：单版记录被
  // 幽灵清除臂删除时按 C# CanElide（Helpers.cs:83-90，prev < BeginAddress）整槽
  // 脱链，冷化后同步档会内存盲判缺失，本用例的「磁盘候选」判据失去输入态
