...
/// 恢复位点安全收敛端到端（对标 C# AllocatorBase.cs 的 RecoveryReset 把 tail/flushed
/// 收敛至提交点后 ClearPage 清尾）：已落盘但无 commit 帧见证的未提交记录绝不被
/// --recover 重启复活——SET a 经 commit_async 获得帧见证，SET b 经
/// commit_flush_only_async 纯刷盘落盘面（盘上无帧见证，停机链 commit_async 因
/// 提交水位已覆盖而短路返回，不补写见证）；第二代 tail 收敛至 a 帧尾，重放驱动
/// （until=tail）不得复活 b（修复前 tail=扫描终点，b 经重放转正）
#[test]
fn recover_uncommitted_record_not_resurrected_after_restart() {
  let rt = Runtime::new().expect("compio runtime");
  let dir = tempdir().expect("tempdir");
  let data_path = dir.path().join("node").join("uncommitted.db");
  // 手动提交档（commit_frequency_ms = -1）：杜绝常驻自动提交协程异步在队尾
  // 补写提交帧的干扰，未提交形态确定可构造
  let manual = || RuntimeServerOptions {
    commit_frequency_ms: -1,
    ..RuntimeServerOptions::default()
  };

  // ---- 第一代：SET a 带帧见证提交；SET b 纯刷盘无帧
  let provider = Arc::new(
    StorageSessionProvider::open_with_config_and_aof(
      test_store_config(),
      &data_path,
  ...
  let (server, addr) = start_server(Arc::clone(&provider));
  // 闭包内赋值须经初值承接（类型由后续 tail_address() 赋值推断为 AofAddress）
  let mut frame_end = Default::default();
  rt.block_on(async {
    let mut stream = TcpStream::connect(addr).await.expect("connect");
    send_cmd(&mut stream, &[b"SET", b"a", b"va"])
      .await
      .expect("set a");
    assert!(read_line_reply(&mut stream).await.starts_with(b"+OK"));
    provider
      ...
      .commit_async()
      .await;
    frame_end = provider.aof().expect("aof enabled").log().tail_address();

    // 未提交写：盘面字节已在（纯刷盘），但无 commit 帧在其后见证
    send_cmd(&mut stream, &[b"SET", b"b", b"vb"])
      .await
      .expect("set b");
    assert!(read_line_reply(&mut stream).await.starts_with(b"+OK"));
    provider
    ...
    assert!(
      provider
        ...
        .get(0)
        .unwrap_or(0)
        > frame_end.get(0).unwrap_or(0),
      "b 记录纯刷盘后物理日志尾须越过提交帧尾"
    );
    send_cmd(&mut stream, &[b"QUIT"]).await.expect("quit");
  });
  server.stop();
  ...
  assert_eq!(
    provider2.aof().expect("aof enabled").log().tail_address(),
    frame_end,
    "恢复尾位点须收敛至最后 commit 帧尾（而非含未提交孤儿的扫描终点）"
  );
  assert_eq!(
    provider2
      .recovered_aof_tail()
      .expect("--recover 形态须点亮恢复位点"),
    frame_end,
    "重放驱动上界（until=tail）不得覆盖未提交孤儿"
  );
  let (server2, addr2) = start_server(Arc::clone(&provider2));
  rt.block_on(async {
    let mut stream = TcpStream::connect(addr2).await.expect("reconnect");
    send_cmd(&mut stream, &[b"GET", b"a"]).await.expect("get a");
    assert_eq!(
      read_bulk_reply(&mut stream).await,
      Some(b"va".to_vec()),
      "带帧见证的已提交记录须正常恢复"
    );
    send_cmd(&mut stream, &[b"GET", b"b"]).await.expect("get b");
    assert_eq!(
      read_bulk_reply(&mut stream).await,
      None,
      "未提交（无帧见证）记录不得经重启复活"
