...
use wnode::{
  ...
  resp::{
    ...
    objects::object_store_utils::{SyncRmwCmd, SyncRmwHandlers, SyncRmwOutcome, run_sync_rmw},
...
/// 异步域单命令往返（融合轮询注入臂专用：底层穿透写入，验证写回复验）
async fn deliver(store: Arc<TestStore>, args: Vec<Vec<u8>>) -> Vec<u8> {
  ...
  if args[0] == b"DEL" {
    let deleted = sess.delete(&args[1]).await.unwrap();
    if deleted {
      b":1\r\n".to_vec()
    } else {
      b":0\r\n".to_vec()
    }
  } else if args[0] == b"SET" {
    let _ = sess.upsert(&args[1], &args[2]).await.unwrap();
    b"+OK\r\n".to_vec()
  } else {
    Vec::new()
  }
...
fn uncontended_sync_window_still_writes_back() {
  let (_dir, store) = open_test_store("reval-ctl-sync.db").unwrap();
  ...
  assert!(
    matches!(got, SyncRmwOutcome::Present(_)),
...
fn uncontended_async_window_still_writes_back() {
  let (_dir, store) = open_test_store("reval-ctl-async.db").unwrap();
...
fn sync_window_concurrent_del_is_never_resurrected() {
  let (_dir, store) = open_test_store("reval-sync-del.db").unwrap();
  ...
  let deleter = thread::spawn(move || {
    held_rx.recv().expect("RMW 已持窗装载");
    let sess = del_store.new_session().unwrap();
    let batch = sess.enter_batch();
    let deleted = batch
      .try_delete_sync_unprotected(&del_key)
      .unwrap()
      .unwrap_or(false);
  ...
  assert!(
    matches!(got, SyncRmwOutcome::Degrade),
...
fn sync_window_concurrent_set_keeps_single_domain() {
  let (_dir, store) = open_test_store("reval-sync-set.db").unwrap();
  ...
  let setter = thread::spawn(move || {
    held_rx.recv().expect("RMW 已持窗装载");
    let sess = set_store.new_session().unwrap();
    let batch = sess.enter_batch();
    let _ = batch
      .try_upsert_sync_unprotected(&set_key, b"hello")
      .unwrap();
...
fn async_window_concurrent_del_is_never_resurrected() {
  let (_dir, store) = open_test_store("reval-async-del.db").unwrap();
...
fn async_window_concurrent_set_keeps_single_domain() {
  let (_dir, store) = open_test_store("reval-async-set.db").unwrap();
...
fn drive_rmw_with_intruder(
  ...
  rt.block_on(poll_fn(|cx| {
    ...
    rmw.as_mut().poll(cx).map(|out| (true, out))
  }))
}
