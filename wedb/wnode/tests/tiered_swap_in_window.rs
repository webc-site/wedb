...
#[compio::test]
async fn sweep_rebuild_window_concurrent_hset_not_lost() -> Void {
  ...
  let pmap: HashMap<&[u8], &[u8]> = pairs
    .iter()
    .map(|(k, v)| (k.as_slice(), v.as_slice()))
    .collect();
  for (i, f) in acked.iter().enumerate() {
    let want = format!("v{i}");
    let got = pmap.get(f.as_slice()).copied().unwrap_or_default();
...
#[compio::test]
async fn materialize_swap_in_window_concurrent_hset_not_lost() -> Void {
  ...
  let rmap: HashMap<&[u8], &[u8]> = rpairs
    .iter()
    .map(|(k, v)| (k.as_slice(), v.as_slice()))
    .collect();
  for (f, v) in &pairs {
    assert_eq!(
      rmap.get(f.as_slice()).copied(),
      Some(v.as_slice()),
      "副本缺字段 {f:?} 或值发散"
    );
  }
