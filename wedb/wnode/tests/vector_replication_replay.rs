...
fn vector_writes_replay_to_fresh_manager() {
  ...
  rt.block_on(async {
    ...
    assert_eq!(
      as_int(&session.network_vadd(SessionPrefixBuf::ROOT.as_slice(), &arg_refs(&unit), SLOT0)),
      1
    );
    assert_eq!(
      as_int(&session.network_vadd(
    ...
    assert_eq!(
      as_int(&session.network_vsetattr(SessionPrefixBuf::ROOT.as_slice(), &setattr_args)),
      1,
      "VSETATTR 应成功"
    );
    let vrem_args: [&[u8]; 2] = [b"a", b"e2"];
    assert_eq!(
      as_int(&session.network_vrem(SessionPrefixBuf::ROOT.as_slice(), &vrem_args)),
      1,
      "VREM 应成功"
    );
    ...
    assert_eq!(
      as_int(&check.network_vcard(SessionPrefixBuf::ROOT.as_slice(), &vcard_args)),
      2,
      "3 VADD - 1 VREM"
    );
    ...
    let embedding =
      as_double_array(&check.network_vemb(SessionPrefixBuf::ROOT.as_slice(), &vemb_args));
    assert_eq!(embedding, vec![1.0, 0.0, 0.0, 0.0], "e1 嵌入应精确还原");

    let vgetattr_args: [&[u8]; 2] = [b"a", b"e1"];
    match check.network_vgetattr(SessionPrefixBuf::ROOT.as_slice(), &vgetattr_args) {
    ...
    assert_eq!(
      as_int(&check.network_vdim(SessionPrefixBuf::ROOT.as_slice(), &vdim_args)),
      4,
      "维度还原"
    );
    ...
    assert!(
      as_double_array(&check.network_vemb(SessionPrefixBuf::ROOT.as_slice(), &vemb_gone))
        .is_empty(),
      "VREM 后 e2 应缺失"
    );
...
#[test]
fn migrated_empty_vector_set_rebuilds_after_replay() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
