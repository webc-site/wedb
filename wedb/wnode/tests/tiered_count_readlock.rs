...
fn ver(map: &WatchVersionMap, key: &[u8]) -> u64 {
  map.read_version(TxnKeyEntryComparison::key_hash(key) as u64)
}
...
#[test]
fn steady_watermark_concurrent_counts_read_fresh_and_keep_watch() -> Void {
...
#[test]
fn crossed_watermark_concurrent_counts_sweep_once_and_reply_exact() -> Void {
  Runtime::new().unwrap().block_on(async {
