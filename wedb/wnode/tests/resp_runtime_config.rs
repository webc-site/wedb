...
#[test]
fn config_repl_attach_timeout_accepts_zero_and_reads_infinite() {
  with(|h| {
    // 默认值 60
    let resp = feed(&mut h.a, &["CONFIG", "GET", "repl-attach-timeout"]);
    assert_eq!(resp, b"*2\r\n$19\r\nrepl-attach-timeout\r\n$2\r\n60\r\n");
    assert_eq!(
      h.runtime_config
        .get_time_span(ServerConfigType::ReplAttachTimeout),
      Some(Duration::from_secs(60))
    );

    // CONFIG SET 0 合法接受（min=0），get_time_span 解释为 None（无限超时）
    let resp = feed(&mut h.a, &["CONFIG", "SET", "repl-attach-timeout", "0"]);
    assert_eq!(resp, b"+OK\r\n");
    assert_eq!(
      h.runtime_config
        .get_time_span(ServerConfigType::ReplAttachTimeout),
    ...
    // 正数透传
    let resp = feed(&mut h.a, &["CONFIG", "SET", "repl-attach-timeout", "120"]);
    assert_eq!(resp, b"+OK\r\n");
    assert_eq!(
      h.runtime_config
        .get_time_span(ServerConfigType::ReplAttachTimeout),
    ...
    // 负数越界拒绝
    let resp = feed(&mut h.a, &["CONFIG", "SET", "repl-attach-timeout", "-1"]);
    assert_eq!(
      resp,
      b"-ERR Value for 'repl-attach-timeout' is out of range (0..2147483647).\r\n"
    );
  });
}
