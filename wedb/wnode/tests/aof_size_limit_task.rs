...
use wconf::{RuntimeServerOptions, ServerConfigType};
...
/// 轮空观察窗口毫秒数（>1 个检查周期，确认任务驱动不触发）
const SKIP_WATCH_MS: u64 = 1100;
/// 「不超限不取闸」轮询节拍毫秒
...
///
/// 两路检查点（size-limit 周期任务臂 vs SAVE 的 take_checkpoint 收口入口）
/// 必须经同一 checkpoint_paused CAS 锁位互斥：若无互斥，旧检查点路径后执行的
...
fn aof_size_limit_task_concurrent_with_save_recovers_consistently() {
  ...
  rt.block_on(async {
    ...
    // 手动 SAVE 任务：与 SAVE 命令同一互斥路径（take_checkpoint 收口入口，
    // 占闸失败即 false 轮空）高频触发，与 size-limit 周期任务正面竞争
...
fn aof_size_limit_task_skips_replica_and_fires_after_promotion() {
  ...
  rt.block_on(async {
    ...
    // 副本轮空 >1 轮：无检查点、AOF 未截断、LastSave 未回填
    sleep(Duration::from_millis(SKIP_WATCH_MS)).await;
    assert_no_checkpoint(&provider);
    assert_eq!(
...
fn aof_size_limit_task_ignores_aof_below_limit() {
  ...
  rt.block_on(async {
    ...
    // 覆盖 >1 个检查周期的高频轮询：闸门恒未置位
    let paused = &provider.database_manager.db.checkpoint_paused;
    for _ in 0..SKIP_WATCH_MS / GATE_POLL_MS {
      assert!(
        !paused.load(Relaxed),
