...
fn bitmap_family_rejects_object_key_and_preserves_set() {
  ...
  rt.block_on(async {
    ...
    for args in [
      ...
      assert_eq!(raw, WRONGTYPE, "{:?}", String::from_utf8_lossy(&raw));
    }
    assert_eq!(line_reply(&cmd(&mut s, &[b"TYPE", b"bt"]).await), "+set");
    ...
    assert_eq!(raw, WRONGTYPE, "BITOP 源键对象键须整体 WRONGTYPE");
...
fn string_key_etag_and_bitmap_semantics_preserved() {
  ...
  rt.block_on(async {
    ...
    assert_eq!(raw, WRONGTYPE, "GETWITHETAG 对象键须回 WRONGTYPE");
    let raw = cmd(&mut s, &[b"GETIFNOTMATCH", b"h", b"0"]).await;
    assert_eq!(raw, WRONGTYPE, "GETIFNOTMATCH 对象键须回 WRONGTYPE");
