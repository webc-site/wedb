...
async fn vadd(consumer: &mut RespSessionConsumer, key: &[u8], element: &[u8], values: &[u8]) {
  let out = pump(
    consumer,
...
/// VCARD 经 RESP 命令面
async fn vcard(consumer: &mut RespSessionConsumer, key: &[u8]) -> i64 {
  let mut out = pump(consumer, &frame(&[b"VCARD", key])).await;
...
fn rename_to_absent_key_migrates_registry() {
  ...
  rt.block_on(async {
    ...
    vadd(&mut consumer, b"vs_src", b"foo", &values).await;
    let old_index = vm
    ...
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAME", b"vs_src", b"vs_dst"])).await,
      b"+OK\r\n"
    );
    ...
    assert_eq!(vcard(&mut consumer, b"vs_dst").await, 1);
    let vectors = RespServerSessionVectors::new(Arc::clone(&vm));
    let emb = vectors
      .network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"vs_dst", b"foo"])
      .await;
    match emb {
...
fn rename_onto_string_and_vector_dest_cleans_displaced() {
  ...
  rt.block_on(async {
    ...
    // ── 场景 2：新名 = 字符串 ──
    vadd(&mut consumer, b"src", b"foo", &values).await;
    assert_eq!(
      pump(&mut consumer, &frame(&[b"SET", b"dst", b"fizzbuzz"])).await,
      b"+OK\r\n"
    );
    ...
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAME", b"src", b"dst"])).await,
      b"+OK\r\n"
    );
    ...
    assert_eq!(vcard(&mut consumer, b"dst").await, 1, "向量语义经新名命中");
    let get_out = pump(&mut consumer, &frame(&[b"GET", b"dst"])).await;
    assert!(
    ...
    vadd(&mut consumer, b"src2", b"el_a", &values).await;
    ...
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAME", b"src2", b"dst2"])).await,
      b"+OK\r\n"
    );
    ...
    assert_eq!(vcard(&mut consumer, b"dst2").await, 1, "仅源集合元素存活");
    let vectors = RespServerSessionVectors::new(Arc::clone(&vm));
    match vectors
      .network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"dst2", b"el_b"])
      .await
    {
...
fn renamenx_and_missing_key_semantics() {
  ...
  rt.block_on(async {
    ...
    vadd(&mut consumer, b"nx_a", b"foo", &values).await;
    vadd(&mut consumer, b"nx_b", b"foo", &values).await;

    // 新名为向量集（登记表命中）→ 0，两键均不动
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAMENX", b"nx_a", b"nx_b"])).await,
      b":0\r\n"
    );
    assert!(
    ...
    let err = pump(&mut consumer, &frame(&[b"RENAME", b"no_such", b"any"])).await;
    assert!(err.starts_with(b"-ERR"), "缺失旧名应报错：{err:?}");

    // 新名空缺 → 1，登记项迁移
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAMENX", b"nx_a", b"nx_c"])).await,
      b":1\r\n"
    );
    assert!(
      vm.read_stored_index(SessionPrefixBuf::ROOT.as_slice(), b"nx_a")
    ...
    // 同键名恒成功（RENAME → +OK）
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAME", b"nx_c", b"nx_c"])).await,
      b"+OK\r\n"
    );
...
fn renamed_vector_set_replays_from_aof() {
  ...
  rt.block_on(async {
    ...
    vadd(&mut consumer, b"aof_src", b"foo", &values).await;
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAME", b"aof_src", b"aof_dst"])).await,
      b"+OK\r\n"
    );
    aof.log().commit();

    // ── AOF 条目形状断言（1 VADD + 1 RENAME）──
    // 计数口径与下方 rename_onto_vector_set_replay_cleans_displaced 同：
    // commit() 随批尾写入的 24B commit 元数据帧（waof/src/wal/commit.rs，
    // 对标 C# TsavoriteLog.cs:TryEnqueueCommitRecord）经重放消费面同一单点
    // 判据 is_commit_frame 滤除，逻辑数据条目恰 2（①VADD 条目 key=旧名
    // aof_src ②RENAME 条目 key=新名 aof_dst），帧落账时机随提交协程调度
    // 而变，滤除后计数恒确定。
    ...
    // ── 重放端：全新 vm（重启语义）恢复 ──
    let recovered = vector_manager_of(&store);
    // 重放段与重启后直调检查的会话绑定（测试单任务段，持至本收口段结束）
    let _replay_domain = recovered
      .bind_dedicated_session()
      .expect("专用向量会话工厂应已注入");
    ...
    match check
      .network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"aof_dst", b"foo"])
      .await
    {
      VectorReply::Array(items) => {
        let dims: Vec<f64> = items
...
fn rename_window_suppress_cleanup_follows_rename() {
  ...
  rt.block_on(async {
    ...
    vadd(&mut consumer, b"win_a", b"el_a", &values).await;
    vadd(
      &mut consumer,
      b"win_a",
      b"el_b",
    ...
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAME", b"win_a", b"win_b"])).await,
      b"+OK\r\n"
    );
    ...
    assert_eq!(vcard(&mut consumer, b"win_b").await, 2, "迁移后元素存活");

    // 新名删除照常触发清理（窗口已随迁移闭环）：标记 → 迭代 → 归还
    assert_eq!(
      pump(&mut consumer, &frame(&[b"DEL", b"win_b"])).await,
      b":1\r\n"
    );
    assert!(
    ...
    vm.process_request_cleanup(ctx).await;
    let (ci, cv) =
...
fn rename_onto_vector_set_replay_cleans_displaced() {
  ...
  rt.block_on(async {
    ...
    vadd(&mut consumer, b"rp_src", b"foo", &values).await;
    vadd(
      &mut consumer,
      b"rp_dst",
      b"el_b",
    ...
    assert_eq!(
      pump(&mut consumer, &frame(&[b"RENAME", b"rp_src", b"rp_dst"])).await,
      b"+OK\r\n"
    );
    aof.log().commit();
    ...
    //
    // 计数口径 = 逻辑数据条目而非物理帧（与姊妹票 bbd6d397 同口径）：
    // 逐条构成（实测物理帧 4、逻辑条目恰 3）：①StoreRMW VADD 条目
    // （key=rp_src 物理键）②commit() 常驻提交协程随批尾写入的 24B commit
    // 元数据帧（MAGIC+begin+cookie，addr 紧随首条 VADD，payload 起始
    // 0xff'4d'4f'43'54'49'4d'4d 取证实测，waof/src/wal/commit.rs，对标 C#
    // TsavoriteLog.cs:TryEnqueueCommitRecord）③StoreRMW VADD 条目
    // （key=rp_dst）④StoreRMW RENAME 条目（条目键=新名 rp_dst）。
    // 提交帧是恢复收敛提交边界的元数据、非数据条目，按重放消费面
    // （aof_processor / recover_replay_task）同一单点判据 is_commit_frame
    // 滤除后计数恒确定（帧落账时机随常驻提交协程调度而变）。
    let mut records = Vec::new();
    aof.log().scan_single_with(0, 0, i64::MAX, |rec| {
      if !waof::is_commit_frame(&rec.payload) {
        records.push(rec.clone());
