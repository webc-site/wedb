...
fn set_expiry_accepts_int64_value() {
  const LARGE_MS: i64 = 4_294_967_296;
  const LARGE_SEC: i64 = 3_000_000_000;

  with_batch(|s, batch| {
    let mut out = Vec::new();
    let ms_str = LARGE_MS.to_string();
    let sec_str = LARGE_SEC.to_string();

    // SET set-px value PX LargeMilliseconds
    s.network_setexnx(
      &[b"set-px", b"value", b"PX", ms_str.as_bytes()],
      batch,
      &mut out,
    )
    .unwrap();
    assert_eq!(out, b"+OK\r\n");
    let ttl = ttl_of_sync(batch, b"set-px")
      .unwrap()
      .value()
      .unwrap()
      .unwrap();
    assert!(ttl > now_ticks() + (LARGE_MS - 10_000) * TICKS_PER_MILLISECOND);

    // SET set-ex value EX LargeSeconds
    out.clear();
    s.network_setexnx(
      &[b"set-ex", b"value", b"EX", sec_str.as_bytes()],
      batch,
      &mut out,
    )
    .unwrap();
    assert_eq!(out, b"+OK\r\n");
    let ttl = ttl_of_sync(batch, b"set-ex")
      .unwrap()
      .value()
      .unwrap()
      .unwrap();
    assert!(ttl > now_ticks() + (LARGE_SEC - 10) * TICKS_PER_SECOND);

    // SETEX setex LargeSeconds value
    out.clear();
    s.network_setex(&[b"setex", sec_str.as_bytes(), b"value"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    let ttl = ttl_of_sync(batch, b"setex")
      .unwrap()
      .value()
      .unwrap()
      .unwrap();
    ...
    // PSETEX psetex LargeMilliseconds value
    out.clear();
    s.network_psetex(&[b"psetex", ms_str.as_bytes(), b"value"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    let ttl = ttl_of_sync(batch, b"psetex")
      .unwrap()
      .value()
      .unwrap()
      .unwrap();
    ...
    // SET set-nx value PX LargeMilliseconds NX
    out.clear();
    s.network_setexnx(
      &[b"set-nx", b"value", b"PX", ms_str.as_bytes(), b"NX"],
      batch,
      &mut out,
    )
    .unwrap();
    assert_eq!(out, b"+OK\r\n");
    let ttl = ttl_of_sync(batch, b"set-nx")
      .unwrap()
      .value()
      .unwrap()
      .unwrap();
    ...
    // SET set-xx-get new-value PX LargeMilliseconds XX GET
    out.clear();
    s.network_set(&[b"set-xx-get", b"old-value"], batch, &mut out)
      .unwrap();
    out.clear();
    s.network_setexnx(
      &[
    ...
    .unwrap();
    assert_eq!(out, b"$9\r\nold-value\r\n");
    let ttl = ttl_of_sync(batch, b"set-xx-get")
      .unwrap()
      .value()
      .unwrap()
      .unwrap();
...
fn set_expiry_out_of_range_is_rejected_without_killing_session() {
  ...
  const EXPECTED_ERR: &[u8] = b"-ERR invalid expire time in 'set' command\r\n";

  with_batch(|s, batch| {
    let mut out = Vec::new();
    s.network_set(&[KEY, b"original"], batch, &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");
    ...
    let mut assert_invalid = |cmd: &[&[u8]], is_setex: bool| {
      let mut out = Vec::new();
      if is_setex {
        s.network_setex(cmd, batch, &mut out).unwrap();
      } else {
        s.network_setexnx(cmd, batch, &mut out).unwrap();
      }
      assert_eq!(out.as_slice(), EXPECTED_ERR);
      let mut get_out = Vec::new();
      s.network_get(&[KEY], batch, &mut get_out).unwrap();
      assert_eq!(get_out, b"$8\r\noriginal\r\n");
    };

    // SET KEY replacement EX overflowSeconds
    assert_invalid(
      &[KEY, b"replacement", b"EX", overflow_sec_str.as_bytes()],
      false,
    );

    // SET KEY replacement PX i64::MAX
    assert_invalid(&[KEY, b"replacement", b"PX", max_i64_str.as_bytes()], false);

    // SETEX KEY overflowSeconds replacement
    assert_invalid(&[KEY, overflow_sec_str.as_bytes(), b"replacement"], true);

    // PSETEX KEY i64::MAX replacement
    out.clear();
    s.network_psetex(
...
/// 一次入账；WrongType 两侧同不计数（对标 C# MainStoreOps.cs:GET 的
/// status.Found 判定与批量 GET 循环逐键累加口径）
#[test]
