...
/// RUNTXP 过程解析与执行端到端测试
/// （C# TransactionProcTest1 的端到端变体；映射注释在 transaction_tests.rs 权威处）
///
/// 过程体三段式恒经存储执行域驱动（C# RespServerSession 构造必带
/// storeWrapper，TransactionManager.cs:188-190 三 api 由存储域装配），故此处
/// 须与生产装配同径挂上 `StoreGarnetApi`；缺域时 `try_transaction_proc`
/// 走 rust 自有装配缺口哨兵，非 C# 应答。
#[test]
fn runtxp_resolves_registered_proc_end_to_end() {
  ...
  let (_dir, store) = wtest_base::open_test_store("runtxp-proc").unwrap();
  s.set_garnet_api(Arc::new(StoreGarnetApi::new(store.new_session().unwrap())));

  // 模拟消息消费闭环
  let feed_consume = |session: &mut RespServerSession, bytes: &[u8]| -> Option<usize> {
    session.recv_buffer.extend_from_slice(bytes);
    session.try_consume_messages()
  };

  // RUNTXP <id>：编译期静态表 DEFAULT 空事务过程 → 提交回 +OK
