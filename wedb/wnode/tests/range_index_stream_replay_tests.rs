...
#[compio::test]
async fn partial_stream_is_pending_then_cleaned_up() {
...
#[compio::test]
async fn retry_first_chunk_resets_stale_partial_reassembly() {
  let dir = tempdir().unwrap();
  let engine = Arc::new(
    Engine::new(dir.path().join("ri"), dir.path().join("cpr")).expect("构造 RangeIndexManager"),
  );
...
#[compio::test]
async fn malformed_chunk_is_dropped() {
  let dir = tempdir().unwrap();
  let engine = Arc::new(
    Engine::new(dir.path().join("ri"), dir.path().join("cpr")).expect("构造 RangeIndexManager"),
  );
...
#[compio::test]
async fn final_flag_on_incomplete_stream_drops_reassembly() {
  let dir = tempdir().unwrap();
  let engine = Arc::new(
    Engine::new(dir.path().join("ri"), dir.path().join("cpr")).expect("构造 RangeIndexManager"),
  );
...
#[compio::test]
async fn publish_failure_on_complete_stream_throws() {
  let dir = tempdir().unwrap();
  let engine = Arc::new(
    Engine::new(dir.path().join("ri"), dir.path().join("cpr")).expect("构造 RangeIndexManager"),
  );
...
/// 超时或管理器析构时自动回收未完成流重组
#[compio::test]
async fn timeout_and_manager_drop_cleans_incomplete_streams() {
  let dir = tempdir().unwrap();
...
#[compio::test]
async fn aof_processor_drop_cleans_range_index_reassembly() {
  let dir = tempdir().unwrap();
  let engine = Arc::new(
    Engine::new(dir.path().join("ri"), dir.path().join("cpr")).expect("构造 RangeIndexManager"),
  );
