...
/// RI 命令链 AOF 形状：RICREATE/RISET/RIDEL 逐条 StoreRMW + Deterministic 标志
/// （C# RISetAndGetBasicTest 是 RESP 面 set/get 回环且锚在
/// range_index_tests.rs:ri_set_and_get_basic_test，本测试测的是落日志形状，
/// 为 rust 侧补充，不挂 C# 测试锚）
#[compio::test]
async fn ri_ops_apply_then_log_and_replay() -> Void {
...
/// RI.DEL 缺失字段仍落 StoreRMW 日志（C# RIDelFieldTest 是删除已存在字段、
/// GET 归 null 的 RESP 面且锚在 range_index_tests.rs:ri_del_field_test，
/// 本测试测的是缺失字段臂的日志形状，为 rust 侧补充，不挂 C# 测试锚）
#[compio::test]
async fn ri_del_missing_field_still_logs() -> Void {
...
/// RI.SET 直入 Disk(B+ 树) 后端后经 range_index_get 读回（C# RISetAndGetBasicTest
/// 是 MEMORY 后端 RESP 面回环且锚在 range_index_tests.rs:ri_set_and_get_basic_test，
/// 本测试测 Disk 后端落盘面，为 rust 侧补充，不挂 C# 测试锚）
#[compio::test]
async fn ri_set_reaches_bftree() -> Void {
...
#[compio::test]
async fn ri_aof_replay_converges_replica() -> Void {
...
#[compio::test]
async fn ttl_purge_single_deterministic_entry() -> Void {
  ...
  // expire_at 会话入口恒等直通（§143：粗化唯命令边界一处，对标 C# 存储侧
  // word 形恒等装载 UnifiedStore/RMWMethods.cs:216、:228）：输入 1 tick
  // 原值随 arg1 携带，重放端恒删不评估该值
  assert_eq!(purge_input.arg1, 1, "expire_at 直通后原值随 arg1 携带");
