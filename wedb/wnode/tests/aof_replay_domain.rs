...
use wconf::{MAX_DATABASES_MAX, RuntimeServerOptions};
...
/// 回放整段日志到目标 store（`store_version` 即重放端版本基线：0 = 全量重放不做
/// 代际跳过，取检查点版本号 = 「基线 + AOF 尾部增量」形态，早于基线的旧代条目一律
/// 经 `record_gate::should_skip_record` 跳过；目标 store 的 AOF 写端口暂停——重放
/// 写入不得镜像回写，与生产恢复会话同闸）
async fn replay_to(
...
/// 承载域才生出本判据。
#[test]
fn restart_baseline_replay_keeps_online_slot() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let Node {
      _dir,
    ...
    store.set_current_version(baseline);
    store
      .create_checkpoint_with_token(&cp_dir, CheckpointType::Snapshot, token)
      .await?;
    ...
    aof.set_vector_manager(Arc::clone(&replayed_vm));
    replay_to(&rstore, &aof, baseline).await?;
