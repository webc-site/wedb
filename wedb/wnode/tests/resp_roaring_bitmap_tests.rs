...
/// 对标登记见 object_store_utils envelope_overflow 注），单发 helper 无人
/// 驱动即停摆零应答（票 wnode-collection-growth-parks-resp-session-zero-
/// response：第 2866 次执行、整帧零消费、零应答三特征），故就地驱动闭环。
/// 「整段消费」断言保留为反向注入判据：挂起后无人闭环仍会整帧残留，
/// 根因未闭则此处必红，不掩盖
async fn exec(consumer: &mut RespSessionConsumer, parts: &[&[u8]]) -> Vec<u8> {
  ...
  loop {
    ...
    // 无挂起须整段消化（残余非零即停摆特征）；有挂起则泵语义驱动至完成
...
fn get_bit_on_missing_key_returns_0() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(
      exec(&mut s, &[b"R.GETBIT", b"rb", b"12345"]).await,
      b":0\r\n"
    );
    // 缺键读不得建键（C# ClassicAssert.IsFalse(db.KeyExists)）
    assert_eq!(exec(&mut s, &[b"EXISTS", b"rb"]).await, b":0\r\n");
    aok::OK
  })
}
...
fn bit_count_on_missing_key_returns_0() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await, b":0\r\n");
    assert_eq!(exec(&mut s, &[b"EXISTS", b"rb"]).await, b":0\r\n");
    aok::OK
  })
}
...
fn bit_pos_on_missing_key_bit1_returns_minus1() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(exec(&mut s, &[b"R.BITPOS", b"rb", b"1"]).await, b":-1\r\n");
    assert_eq!(exec(&mut s, &[b"EXISTS", b"rb"]).await, b":0\r\n");
    aok::OK
  })
}
...
fn bit_pos_on_missing_key_bit0_returns_from_or_zero() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(exec(&mut s, &[b"R.BITPOS", b"rb", b"0"]).await, b":0\r\n");
    assert_eq!(
      exec(&mut s, &[b"R.BITPOS", b"rb", b"0", b"100"]).await,
      b":100\r\n"
    );
    aok::OK
  })
...
fn set_bit_get_bit_basics() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"42", b"1"]).await,
      b":0\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"42", b"1"]).await,
    ...
    assert_eq!(exec(&mut s, &[b"R.GETBIT", b"rb", b"42"]).await, b":1\r\n");
    assert_eq!(exec(&mut s, &[b"R.GETBIT", b"rb", b"41"]).await, b":0\r\n");
    // 清位
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"42", b"0"]).await,
      b":1\r\n"
    );
    assert_eq!(exec(&mut s, &[b"R.GETBIT", b"rb", b"42"]).await, b":0\r\n");
    aok::OK
  })
}
...
fn bit_count_basics() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await, b":10\r\n");
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"5", b"0"]).await,
      b":1\r\n"
    );
    assert_eq!(exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await, b":9\r\n");
    aok::OK
  })
}
...
fn bit_pos_basics() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(exec(&mut s, &[b"R.BITPOS", b"rb", b"1"]).await, b":100\r\n");
    assert_eq!(
      exec(&mut s, &[b"R.BITPOS", b"rb", b"1", b"150"]).await,
      b":200\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"R.BITPOS", b"rb", b"1", b"300"]).await,
      b":70000\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"R.BITPOS", b"rb", b"1", b"70001"]).await,
      b":-1\r\n"
    );
    // 全置位区间后的首个未置位
    assert_eq!(exec(&mut s, &[b"R.BITPOS", b"rb", b"0"]).await, b":0\r\n");
...
fn errors_bad_offset() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"notanumber", b"1"]).await,
      b"-ERR bit offset is not an unsigned 32-bit integer\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"-5", b"1"]).await,
      b"-ERR bit offset is not an unsigned 32-bit integer\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"5", b"2"]).await,
...
fn string_key_and_custom_object_key_are_separate() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    exec(&mut s, &[b"SET", b"rb", b"hello"]).await;
    assert_eq!(
      exec(&mut s, &[b"R.GETBIT", b"rb", b"10"]).await,
      err_frame(RESP_ERR_WRONG_TYPE)
    );
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"10", b"1"]).await,
      err_frame(RESP_ERR_WRONG_TYPE)
    );
    assert_eq!(exec(&mut s, &[b"GET", b"rb"]).await, b"$5\r\nhello\r\n");
    aok::OK
  })
}
...
fn errors_bit_pos_bad_bit() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(
      exec(&mut s, &[b"R.BITPOS", b"rb", b"2"]).await,
...
fn errors_wrong_arity() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    let r1 = exec(&mut s, &[b"R.SETBIT", b"rb", b"1"]).await;
    assert!(r1.starts_with(b"-ERR wrong number of arguments"), "{r1:?}");
    let r2 = exec(&mut s, &[b"R.BITCOUNT", b"rb", b"extra"]).await;
...
fn delete_and_recreate() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    exec(&mut s, &[b"R.SETBIT", b"rb", b"5", b"1"]).await;
    assert_eq!(exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await, b":1\r\n");
    assert_eq!(exec(&mut s, &[b"DEL", b"rb"]).await, b":1\r\n");
    assert_eq!(exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await, b":0\r\n");
    exec(&mut s, &[b"R.SETBIT", b"rb", b"9", b"1"]).await;
    assert_eq!(exec(&mut s, &[b"R.GETBIT", b"rb", b"5"]).await, b":0\r\n");
    assert_eq!(exec(&mut s, &[b"R.GETBIT", b"rb", b"9"]).await, b":1\r\n");
    aok::OK
  })
}
...
fn large_offsets_and_chunk_boundaries() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    for &off in &interesting {
      ...
      assert_eq!(
        exec(&mut s, &[b"R.GETBIT", b"rb", off.to_string().as_bytes()]).await,
        b":1\r\n"
      );
    }
    assert_eq!(
      exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await,
      format!(":{}\r\n", interesting.len()).as_bytes()
    );
    assert_eq!(exec(&mut s, &[b"R.BITPOS", b"rb", b"1"]).await, b":0\r\n");
    assert_eq!(
      exec(
        &mut s,
        &[
...
fn dense_bitmap_promotion_stays_correct() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await, b":5000\r\n");
    ...
    assert_eq!(exec(&mut s, &[b"R.BITCOUNT", b"rb"]).await, b":4096\r\n");
    assert_eq!(
      exec(&mut s, &[b"R.GETBIT", b"rb", b"4095"]).await,
      b":1\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"R.GETBIT", b"rb", b"4096"]).await,
      b":0\r\n"
    );
    aok::OK
  })
...
fn slow_path_custom_object_set_and_get() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    // 快路径直读校验
    assert_eq!(exec(&mut s, &[b"R.GETBIT", b"rb", b"100"]).await, b":1\r\n");
    ...
    exec(&mut s, &[b"SET", b"str_key", b"val"]).await;
    let wait = SlowWait::for_command(
      &api,
      RespCommand::Customobjcmd,
      vec![
    ...
    let reply = wait.resolve().await;
    assert_eq!(reply, b":0\r\n");
    assert_eq!(exec(&mut s, &[b"EXISTS", b"missing_key"]).await, b":0\r\n");
...
fn memory_usage_counts_bitmap_heap() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"1", b"1"]).await,
      b":0\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"1000", b"1"]).await,
      b":0\r\n"
    );
    let sparse = parse_resp_int(&exec(&mut s, &[b"MEMORY", b"USAGE", b"rb"]).await);
    assert!(sparse >= 94, "usage must include heap estimate: {sparse}");

    // 跨 65536 边界第二容器（两数组容器）：堆估算 120 = 94 + 10 容器 + 16 头 + 2 body
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"70000", b"1"]).await,
      b":0\r\n"
    );
    let crossed = parse_resp_int(&exec(&mut s, &[b"MEMORY", b"USAGE", b"rb"]).await);
    assert!(
      crossed > sparse,
...
fn type_exists_agree_on_roaring_key() -> aok::Result<()> {
  ...
  rt.block_on(async {
    ...
    assert_eq!(exec(&mut s, &[b"TYPE", b"rb"]).await, b"+none\r\n");
    assert_eq!(exec(&mut s, &[b"EXISTS", b"rb"]).await, b":0\r\n");

    // 建键后：TYPE 回注册名（禁回 none），EXISTS 同键 :1
    assert_eq!(
      exec(&mut s, &[b"R.SETBIT", b"rb", b"7", b"1"]).await,
      b":0\r\n"
    );
    assert_eq!(
      exec(&mut s, &[b"TYPE", b"rb"]).await,
      b"+GarnetRoaringBitmap\r\n"
    );
    assert_eq!(exec(&mut s, &[b"EXISTS", b"rb"]).await, b":1\r\n");
