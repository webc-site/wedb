...
/// TTL 族读写两侧对向量集键的收敛口径（登记表第四态只挂存在性观测面）：
/// EXISTS :1（第四态保留，存活观测单点）；TTL/PTTL/EXPIRETIME/PEXPIRETIME
/// : -2（读侧与写侧同源三域探针单答）；EXPIRE 族 :0、PERSIST :0（写侧原
/// 口径）——「读 -1 写 :0」的合成缺口收敛为单源。
/// 刻意收敛声明与对齐前提详见 `doc/zh/deviations.md` §75；MEMORY USAGE 为尺寸
/// 观测面（非 TTL 族），保留登记表正整数应答
#[test]
fn vector_set_ttl_family_converged_with_write_side() {
  let rt = Runtime::new().unwrap();
  // VADD 消费链 insert 需当前线程挂 compio runtime（thread-per-core），
  // 全体命令驱动置于 runtime 上下文内（roundtrip 内层 block_on 同线程复用）
  rt.block_on(async {
    ...
    // 存在性观测面：EXISTS 保留第四态（登记命中即存活）
    let exists = roundtrip(&rt, &mut c, &encode_frame(&[b"EXISTS", b"vs_ttl"]));
    assert_eq!(exists, b":1\r\n", "向量键 EXISTS 应回 1（第四态观测面）");

    // TTL 族读侧：与写侧同源收敛 → -2（修复前读侧误回 -1 与写侧 :0 自相矛盾）
    for cmd in [b"TTL".as_slice(), b"PTTL", b"EXPIRETIME", b"PEXPIRETIME"] {
      let out = roundtrip(&rt, &mut c, &encode_frame(&[cmd, b"vs_ttl"]));
      assert_eq!(
        out,
        b":-2\r\n",
        "向量键 {:?} 应回 -2（与写侧 EXPIRE :0 同源，不得回 -1）",
        String::from_utf8_lossy(cmd),
      );
    }

    // TTL 族写侧：EXPIRE 全形态 :0、PERSIST :0（三物理域皆缺的同源口径）
    for frame in [
      encode_frame(&[b"EXPIRE", b"vs_ttl", b"100"]),
      encode_frame(&[b"PEXPIRE", b"vs_ttl", b"100000"]),
      encode_frame(&[b"EXPIREAT", b"vs_ttl", b"4102444800"]),
      encode_frame(&[b"PERSIST", b"vs_ttl"]),
    ] {
      let out = roundtrip(&rt, &mut c, &frame);
      assert_eq!(
        out,
        b":0\r\n",
        "向量键 TTL 族写侧应答 :0: {:?}",
    ...
    // 写侧零副作用核验：EXPIRE :0 后键仍存活且无 TTL 残留半状态
    assert_eq!(
      roundtrip(&rt, &mut c, &encode_frame(&[b"EXISTS", b"vs_ttl"])),
      b":1\r\n"
    );
    assert_eq!(
      roundtrip(&rt, &mut c, &encode_frame(&[b"TTL", b"vs_ttl"])),
      b":-2\r\n"
    );

    // 尺寸观测面不随本收敛变化：MEMORY USAGE 保留登记表正整数应答
    let mu = roundtrip(
      &rt,
      &mut c,
    ...
    // 普通字符串键：TTL 族 -1、EXPIRE :1（三域探针路径不回归）
    assert_eq!(
    ...
    assert_eq!(
      roundtrip(&rt, &mut c, &encode_frame(&[b"EXPIRE", b"str_k", b"100"])),
      b":1\r\n",
      "字符串键 EXPIRE 应回 :1"
    );
  })
...
/// 向量键写入后确定性组合应答语义锁（doc/zh/deviations.md §75 / 工单 zcode-r40-sentinel）：
/// 断言向量键写入后，确定性组合应答锁：
/// 1. EXISTS 回 :1（登记表第四态存活判定）
/// 2. TTL 回 :-2（三物理域缺失刻意收敛）
/// 3. EXPIRE 回 :0（三物理域缺失写侧拒绝）
///
/// 杜绝后续单端误开闸（如读面改回 -1 或写面开闸回 :1）造成读写撕裂与主从复制状态分叉。
#[test]
fn vector_key_exists_ttl_expire_semantic_lock() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let (mut c, _vm, _store) = consumer();

    create_vectorset(&rt, &mut c, b"vkey_sem_lock");

    // 1. EXISTS 回 :1（第四态登记表存活判定）
    let exists = roundtrip(&rt, &mut c, &encode_frame(&[b"EXISTS", b"vkey_sem_lock"]));
    assert_eq!(exists, b":1\r\n", "向量键 EXISTS 必须确定性回 :1");

    // 2. TTL 回 :-2（三物理域缺失刻意收敛，doc/zh/deviations.md §75）
    let ttl = roundtrip(&rt, &mut c, &encode_frame(&[b"TTL", b"vkey_sem_lock"]));
    assert_eq!(ttl, b":-2\r\n", "向量键 TTL 必须确定性回 :-2");

    // 3. EXPIRE 回 :0（写面三域缺失拒绝，doc/zh/deviations.md §75）
    let expire = roundtrip(
      &rt,
      &mut c,
      &encode_frame(&[b"EXPIRE", b"vkey_sem_lock", b"60"]),
    );
    assert_eq!(expire, b":0\r\n", "向量键 EXPIRE 必须确定性回 :0");
  });
}
...
/// INFO keyspace 向量键计数（agy 轮5 条2）：登记表旁挂域在 INFO keyspace
/// 消费点合并，单向量键库 db0 行 keys 计入向量键（修复前 keyspace_stats 纯
/// 物理扫描漏计 → 整行剔除，同库 DBSIZE :1 / KEYS 列出而 INFO 无 db 行，
/// 四方应答不自洽；C# 向量元数据驻留主存统计天然计入）。向量键 TTL 族 b 案
/// 不支持，expires 栏恒 0
#[test]
fn info_keyspace_counts_vector_keys() {
  let rt = Runtime::new().unwrap();
  // VADD 消费链 insert 需当前线程挂 compio runtime（thread-per-core），
  // 全体命令驱动置于 runtime 上下文内（roundtrip 内层 block_on 同线程复用）
  rt.block_on(async {
    ...
    create_vectorset(&rt, &mut c, b"vec_info");
    let out = roundtrip(&rt, &mut c, &encode_frame(&[b"INFO", b"keyspace"]));
    assert!(
      out.windows(10).any(|w| w == b"db0:keys=1"),
      "INFO keyspace 应计入向量键（db0:keys=1），实际 {out:?}"
    );
    assert!(
      out.windows(20).any(|w| w == b"db0:keys=1,expires=0"),
      "向量键 TTL 族不支持，expires 栏不得计入向量键：{out:?}"
    );
  })
}
