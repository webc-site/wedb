...
fn hcollect_star_streams_across_batch_boundary() {
  ...
  // 即落物化矫正臂，到期字段不计入应答计数
  assert_eq!(
    roundtrip(&mut c, &resp_frame_str(&["HLEN", "h0"])),
    b":1\r\n"
  );
  assert_eq!(
    roundtrip(&mut c, &resp_frame_str(&["HLEN", "h249"])),
    b":1\r\n",
    "末批键须真实存在"
  );
  ...
  for i in 0..KEYS {
    let key = format!("h{i}");
    assert_eq!(
      roundtrip(&mut c, &resp_frame_str(&["HLEN", &key])),
      b":1\r\n",
      "{key}（第 {} 批）未被收集轮覆盖",
      i / 100 + 1
    );
...
fn envelope_field_ttl_length_purges_and_self_heals() {
  ...
  // 水位越线（部分成员过期）：物化矫正臂 purge 后回正确存活计数
  assert_eq!(
    roundtrip(&mut c, &resp_frame_str(&["HLEN", "fh"])),
    b":2\r\n",
    "HLEN 必须剔除到期字段计数"
  ...
  // 与点查同口径：到期字段视同不存在
  assert_eq!(
    roundtrip(&mut c, &resp_frame_str(&["HGET", "fh", "gone"])),
    b"$-1\r\n"
  );
  ...
  assert_eq!(
    roundtrip(&mut c, &resp_frame_str(&["EXISTS", "fh"])),
    b":0\r\n",
    "hash 全字段到期 HLEN 触发删空自愈"
  );
  assert_eq!(
    roundtrip(&mut c, &resp_frame_str(&["EXISTS", "fz"])),
    b":0\r\n",
