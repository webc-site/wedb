...
use wtest_base::resp_frame as encode_frame;
...
/// 泵等价消费（直填会话接收缓冲 → 唯一入口 → 应答取出）；返回 (消费后
/// 残余, 应答)：Some(0) = 完整消费，None = 协议违规。同步段挂起 SlowWait
/// 的命令（VADD/VSETATTR 写族挂起化）随即泵等价驱动至完成，应答按流水线
/// 顺序并入（对标网络泵 take_slow_wait → resolve）
async fn pump(consumer: &mut RespSessionConsumer, frame: &[u8]) -> (Option<usize>, Vec<u8>) {
...
fn drop_vector_set_while_resetting_store() {
  ...
  rt.block_on(async {
    ...
    for i in 0..VECTORS {
      ...
      let (consumed, out) = pump(&mut consumer, &req).await;
    ...
    // 存储与会话仍健康：命令通路正常
    let (_, out) = pump(&mut consumer, &encode_frame(&[b"VCARD", key])).await;
