...
fn open_env(
  ...
  open_env_paged(tag, 64 * 1024)
}

/// 指定环形页大小的执行域变体（大 value 同步写窗口，体积维升阶门回归用）
fn open_env_paged(
  tag: &str,
  ring_page: usize,
) -> (
  ...
  let config = StoreConfig::new(1024, ring_page, 16, 0.5).unwrap();
...
/// SMOVE 目标写回命中升阶门降级：慢路径重放保 member 不丢（对标 C# 存储侧
/// SetMove 两键事务的原子性语义；修复前快路径先落盘源集合移除，目标命中升阶门
/// 降级后重放装载不到 member，应答 :0 且 member 双侧皆无；修复后写回序先目标
/// 后源，目标降级时源零变异，重放自完整初态执行）
#[test]
fn smove_destination_promote_degrade_keeps_member() {
  // 1MB 环形页：65535 条短成员信封全程同步建键，仅 SMOVE 越门限降级
  let (rt, api, _store, _dir) = open_env_paged("obj-cold-smove.db", 1024 * 1024);
  let mut s = session_with(&api);
  ...
  assert_eq!(
    sync_exec(
      ...
      &mut s,
      RespCommand::Sadd,
      &[b"smv-src", b"moving", b"keep"]
    ),
    b":2\r\n"
  );
  // 目标预置 TIERED_PROMOTE_THRESHOLD - 1 条短成员：贴近条目数升阶门不越限
  let total = wcol::TIERED_PROMOTE_THRESHOLD - 1;
  for chunk_start in (0..total).step_by(16384) {
    let chunk_end = (chunk_start + 16383).min(total - 1);
    let mut args: Vec<Vec<u8>> = Vec::with_capacity(chunk_end - chunk_start + 2);
    args.push(b"smv-dst".to_vec());
    ...
    assert_eq!(
      sync_exec(&api, &mut s, RespCommand::Sadd, &arg_slices),
      added.as_bytes()
    );
  }

  // SMOVE 移入 moving：目标本地条目数达门限 → 快路径目标写回命中升阶门降级
  //（Ok(false) 零写入）；慢路径重放：源仍含 moving、目标回到初态，
  // take + insert 后目标经重灌升阶为分层树，源保留剩余成员，应答 :1
  //（修复前重放装载源已无 moving，应答 :0 且 moving 双侧皆无）
  assert_eq!(
    cold_exec(
      &api,
      &rt,
      &mut s,
      RespCommand::Smove,
      &[b"smv-src", b"smv-dst", b"moving"]
    ),
    b":1\r\n"
  );

  // moving 落目标（树态键装载降级，SISMEMBER 走慢路径树内直达）
  assert_eq!(
    cold_exec(
      &api,
      &rt,
  ...
  // 目标基数（树态 Meta O(1) 直读）：预置全量 + moving
  let expect = format!(":{}\r\n", wcol::TIERED_PROMOTE_THRESHOLD);
  assert_eq!(
    sync_exec(&api, &mut s, RespCommand::Scard, &[b"smv-dst"]),
    expect.as_bytes()
  );
  // 源保留剩余成员（信封热键）
  assert_eq!(
    sync_exec(&api, &mut s, RespCommand::Scard, &[b"smv-src"]),
    b":1\r\n"
  );
  assert_eq!(
    sync_exec(&api, &mut s, RespCommand::Sismember, &[b"smv-src", b"keep"]),
    b":1\r\n"
  );
}
