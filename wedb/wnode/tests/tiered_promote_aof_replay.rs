...
/// 单批灌入字段上限（防单命令超载且兼顾流水线分块）
const CHUNK_SIZE: usize = 16384;

/// 字段批量填充参数
#[derive(Clone, Copy)]
struct FillFieldsArgs<'a> {
  key: &'a [u8],
  prefix: &'a [u8],
  start: usize,
  end: usize,
...
fn fill_fields(api: &GarnetApi, rt: &Runtime, s: &mut RespServerSession, args: FillFieldsArgs<'_>) {
  let mut buf = ItoaBuffer::new();
  let mut chunk_start = args.start;

  // 跨 chunk 复用缓冲区，杜绝散碎堆分配
  let mut field_buf = Vec::with_capacity(CHUNK_SIZE * 32);
  let mut ranges = Vec::with_capacity(CHUNK_SIZE * 2);
  let fixed_val = (args.value_bytes > 0).then(|| vec![b'v'; args.value_bytes]);

  while chunk_start <= args.end {
    let chunk_end = (chunk_start + CHUNK_SIZE - 1).min(args.end);
    field_buf.clear();
    ranges.clear();

    let mut slices = Vec::with_capacity((chunk_end - chunk_start + 1) * 2 + 1);
    slices.push(args.key);

    for i in chunk_start..=chunk_end {
      let num_bytes = buf.format(i).as_bytes();

      let f_start = field_buf.len();
      field_buf.extend_from_slice(args.prefix);
