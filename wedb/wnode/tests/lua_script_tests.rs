...
/// 回归（next/zcode-r4-lua 问题 1）：RESP3 会话 EVAL 后协议版本不变。
...
/// 回归（next/zcode-r4-lua 问题 1）：脚本内 setresp(3) 只作用脚本窗口，
...
/// 回归（next/zcode-r4-lua 问题 2）：脚本内 redis.call('SELECT') 只作用脚本
...
/// 回归（next/zcode-r4-lua 问题 3）：GET 快路径 WRONGTYPE 错误文本透传
/// （不再降级静态错误串）——Lua 错误载荷与外层连接直连同键的错误
/// 帧文本逐字节一致。
#[test]
fn eval_get_wrongtype_error_text_passes_through() {
  let (_dir, store) = open_test_store("lua-wrongtype-passthrough.db").expect("open test store");
  let mut s = lua_session(&store);

  // 造 WRONGTYPE：列表键上 GET（外层直连帧为文本基准）
  let out = cmd(
    &mut s,
  ...
  let direct_text = String::from_utf8_lossy(&direct[1..direct.len() - 2]).into_owned();

  // 快路径 GET 错误中止脚本，错误文本与直连一致
  let out = cmd(
    &mut s,
    eval_parts("return redis.call('GET', KEYS[1])", &[b"lua:wt"], &[]),
  );
  let expected = format!("-ERR Lua encountered an error: {direct_text}\r\n");
