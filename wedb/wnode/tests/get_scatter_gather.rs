...
#[test]
fn test_network_get_cold_batch_expired_key_matches_mget() -> aok::Result<()> {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (dir, store_session, mut s) = session_with_input(b"");
...
#[test]
fn test_read_batch_ttl_gate_due_and_ascending_order() -> aok::Result<()> {
  let rt = Runtime::new()?;
  rt.block_on(async {
...
#[test]
fn test_network_get_sg_expired_key_returns_nil() -> aok::Result<()> {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (dir, store_session, mut s) = session_with_input(b"");
    let batch = store_session.enter_batch();
