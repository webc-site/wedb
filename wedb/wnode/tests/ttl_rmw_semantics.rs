...
/// 裸写内核，值未粗化——粗化唯发生在命令入口 network_expire 单点，§143）
...
/// SETBIT/BITFIELD 写臂增长场景恒保留 key 级 TTL 至越期真删（deviations.md
/// 第 18 条语义锁）：SET k v EX 1 后越界偏移写位触发值增长，TTL 不被清除；
/// 睡眠越期后 EXISTS 回 :0（保留的生效面）。不对齐 C# RMWMethods.cs
/// InPlaceUpdater SETBIT/BITFIELD 增长臂的 RemoveExpiration（:581/:595/
/// :621/:635，inline 槽位腾挪副作用），按 C# 改回清 TTL 即真回归
#[test]
fn bitmap_grow_preserves_ttl_until_expiry() {
  with_test_env(|s, batch| {
    let mut out = Vec::new();

    // SETBIT 增长臂：1 字节旧值上写位 100，触发写臂增长
    s.network_set(&[b"bt:k", b"v", b"EX", b"1"], batch, None, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    out.clear();
    s.network_string_set_bit(&[b"bt:k", b"100", b"1"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b":0\r\n");
    out.clear();
    let exp = ttl_of_sync(batch, b"bt:k")
      .unwrap()
      .value()
      .unwrap()
      .expect("SETBIT 增长后必须保留键级 TTL（deviations 第 18 条）");
    assert!(exp > now_ticks(), "增长写后 TTL 必须仍在未来");

    // BITFIELD 增长臂：SET u8 100 同型越界增长
    s.network_set(&[b"bf:k", b"v", b"EX", b"1"], batch, None, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    out.clear();
    s.string_bit_field(&[b"bf:k", b"SET", b"u8", b"100", b"1"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"*1\r\n:0\r\n");
    out.clear();
    let exp = ttl_of_sync(batch, b"bf:k")
      .unwrap()
      .value()
      .unwrap()
      .expect("BITFIELD 增长后必须保留键级 TTL（deviations 第 18 条）");
    assert!(exp > now_ticks(), "增长写后 TTL 必须仍在未来");

    // 睡眠越期（1 秒 TTL + 200ms 余量，hash_ttl 同款）：若误清 TTL 此处
    // EXISTS 将回 :1，即 C# InPlace 臂形态的真回归信号
    sleep(Duration::from_millis(1200));
    s.network_exists(&[b"bt:k"], batch, None, &mut out).unwrap();
    assert_eq!(out, b":0\r\n", "SETBIT 增长后 TTL 保留须到期真删");
    out.clear();
    s.network_exists(&[b"bf:k"], batch, None, &mut out).unwrap();
    assert_eq!(out, b":0\r\n", "BITFIELD 增长后 TTL 保留须到期真删");
  });
}
...
fn smove_preserves_dst_ttl() {
  with_test_env(|s, batch| {
    ...
    out.clear();
    s.set_move(&[b"mv:src", b"mv:dst", b"m"], batch, None, &mut out)
      .unwrap();
    assert_eq!(out, b":1\r\n");
...
fn set_semantics_clears_ttl() {
  with_test_env(|s, batch| {
    ...
    s.network_set(&[b"set:k", b"new"], batch, None, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    ...
    out.clear();
    s.network_getset(&[b"set:getset", b"new"], batch, None, &mut out)
      .unwrap();
    assert_eq!(out, b"$3\r\nold\r\n");
    ...
    out.clear();
    s.network_setnx(&[b"set:nx", b"new"], batch, None, &mut out)
      .unwrap();
    assert_eq!(out, b":0\r\n", "SETNX 对存活键失败");
    // 失败不写不清：TTL 原样
...
fn store_family_dest_clears_ttl() {
  with_test_env(|s, batch| {
    ...
    s.sorted_set_add(&[b"st:zr", b"9", b"old"], batch, &mut out)
      .unwrap();
    let _ = seed_ttl_on_key(batch, b"st:zr");
    out.clear();
    ...
    for dst in ["st:zi", "st:zu", "st:zd"] {
      s.sorted_set_add(&[dst.as_bytes(), b"9", b"old"], batch, &mut out)
        .unwrap();
      let _ = seed_ttl_on_key(batch, dst.as_bytes());
    }
    out.clear();
    ...
    s.sorted_set_add(&[b"st:gdst", b"9", b"old"], batch, &mut out)
      .unwrap();
    let _ = seed_ttl_on_key(batch, b"st:gdst");
    out.clear();
...
// ---- 粗化单源收口回归（键级 4-bit coarse 唯命令边界 network_expire 一处
// 施加，wkv expire_at 会话入口与存储内核 put_ttl_sync / put_ttl 恒等裸写
// 不判，§143。绝对域秒/毫秒→ticks 换算天然 16 对齐，
...
fn set_ex_getex_stored_ticks_not_coarsened() {
  with_test_env(|s, batch| {
    ...
    for key in [b"nocg:s0".as_slice(), b"nocg:s1", b"nocg:s2", b"nocg:s3"] {
      out.clear();
      s.network_set(&[key, b"v", b"EX", b"60"], batch, None, &mut out)
        .unwrap();
      assert_eq!(out, b"+OK\r\n");
      let stored = ttl_of_sync(batch, key)
        .unwrap()
