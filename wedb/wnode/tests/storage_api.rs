...
#[compio::test]
async fn test_ttl_scan_watch() -> aok::Void {
...
#[compio::test]
async fn test_slot_keys_count_get_differential() -> aok::Void {
  ...
  //（SET k v EX 1 后 COUNT 与 GET 均不计 k，对标 C# ExpirationWithOption.cs 语义）
...
#[compio::test]
async fn test_single_database_manager_pause_snapshot_contract() -> aok::Void {
...
fn test_hash_object_roundtrip() -> aok::Void {
  ...
  obj.hash.insert(Arc::from(b"f".to_vec()), b"v".to_vec());
...
fn test_storage_session_obj_save() -> aok::Void {
  ...
  rt.block_on(async {
    ...
    let mut obj = HashObject::new();
    obj.hash.insert(Arc::from(b"k1".to_vec()), b"v1".to_vec());
...
#[compio::test]
async fn test_scan_cursor_semantics() -> aok::Void {
...
#[compio::test]
async fn test_smove_and_empty_collection_cleanup() -> aok::Void {
  ...
  // 登记前缀取写会话同源根域（new_session 缺省 (ns0,db0) 虚域恒等 ROOT）
  let mut watcher = TxnWatchedKeysContainer::new(Arc::clone(&version_map));
  watcher.add_watch(SessionPrefixBuf::ROOT.as_slice(), b"wk");
...
#[compio::test]
async fn test_storage_session_advanced_contract_apis() -> aok::Void {
...
/// (0, 空)、截断后旧地址钳制重扫不误杀
#[compio::test]
async fn test_scan_cursor_validation() -> aok::Void {
