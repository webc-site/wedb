...
fn vadd_parse_and_defaults() {
  ...
  // 参数不足
  let r = sess.network_vadd(SessionPrefixBuf::ROOT.as_slice(), &[b"k", b"FP32"], SLOT0);
  ...
  let index = sess
    .read_index(SessionPrefixBuf::ROOT.as_slice(), b"k")
    .unwrap();
  ...
  assert_eq!(r, VectorReply::Integer(1));
  let index = sess
    .read_index(SessionPrefixBuf::ROOT.as_slice(), b"k2")
    .unwrap();
  ...
  assert_eq!(r, VectorReply::Integer(1));
  let index = sess
    .read_index(SessionPrefixBuf::ROOT.as_slice(), b"kr")
    .unwrap();
  ...
  for dup in dup_sets {
    let r = sess.network_vadd(SessionPrefixBuf::ROOT.as_slice(), &dup, SLOT0);
    assert!(matches!(r, VectorReply::Error(_)), "重复选项应报错: {r:?}");
  }
  assert_eq!(
    err_text(sess.network_vadd(
      SessionPrefixBuf::ROOT.as_slice(),
...
fn vsim_options_and_output() {
  ...
  assert_eq!(
    err_text(sess.network_vsim(SessionPrefixBuf::ROOT.as_slice(), &[b"vs", b"WHAT", b"x"])),
  ...
  assert_eq!(
    sess.network_vsim(SessionPrefixBuf::ROOT.as_slice(), &[b"nope", b"ELE", b"x"]),
...
fn auxiliary_commands() {
  ...
  assert_eq!(
    sess.network_vcard(SessionPrefixBuf::ROOT.as_slice(), &[b"aux"]),
    VectorReply::Integer(2)
  );
  assert_eq!(
    sess.network_vdim(SessionPrefixBuf::ROOT.as_slice(), &[b"aux"]),
    VectorReply::Integer(2)
  );
  assert_eq!(
    sess.network_vismember(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1"]),
    VectorReply::Integer(1)
  );
  assert_eq!(
    sess.network_vismember(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"zz"]),
    VectorReply::Integer(0)
  );
  assert_eq!(
    sess.network_vismember_impl(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1"], true),
    VectorReply::Boolean(true)
  );

  // VEMB / VEMB RAW（NoQuant → fp32 通道）
  let emb = sess.network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1"]);
  let mut encoded = Vec::new();
  ...
  let raw = sess.network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1", b"RAW"]);
  ...
  // VEMB 非法选项
  assert_eq!(
    err_text(sess.network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1", b"BAD"])),
    "Unexpected option to VEMB"
  );
  // VEMB 缺失元素 → 空数组（对齐 C# NOTFOUND）
  assert_eq!(
    sess.network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"ghost"]),
    VectorReply::Array(Vec::new())
  );

  // VGETATTR / VSETATTR（成功 1 / 缺失 0，非错误）
  assert_eq!(
    sess.network_vgetattr(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1"]),
    VectorReply::Bulk(Some(b"{\"tag\":\"x\"}".to_vec().into()))
  );
  assert_eq!(
    sess.network_vsetattr(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1", b"{}"]),
    VectorReply::Integer(1)
  );
  assert_eq!(
    sess.network_vsetattr(
      SessionPrefixBuf::ROOT.as_slice(),
      &[b"aux", b"ghost", b"{}"]
    ),
    VectorReply::Integer(0)
  ...
  // VLINKS（缺失元素 → null）
  assert!(matches!(
    sess.network_vlinks(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1"]),
    VectorReply::Array(_)
  ));
  assert_eq!(
    sess.network_vlinks(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"ghost"]),
    VectorReply::Bulk(None)
  );
  assert_eq!(
    err_text(sess.network_vlinks(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e1", b"BAD"])),
    "ERR Unexpected option"
  );

  // VRANDMEMBER
  assert!(matches!(
    sess.network_vrandmember(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"2"]),
    VectorReply::Array(_)
  ));
  assert_eq!(
    err_text(sess.network_vrandmember(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"x"])),
    "ERR expected integer count"
  );
  assert_eq!(
    sess.network_vrandmember(SessionPrefixBuf::ROOT.as_slice(), &[b"ghost", b"2"]),
    VectorReply::Array(Vec::new())
  );
  assert_eq!(
    sess.network_vrandmember(SessionPrefixBuf::ROOT.as_slice(), &[b"ghost"]),
    VectorReply::Bulk(None)
  );

  // VINFO（对齐 C# 布局：14 项 + 小写枚举名）
  let info = sess.network_vinfo(SessionPrefixBuf::ROOT.as_slice(), &[b"aux"]);
  let mut encoded = Vec::new();
  ...
  assert_eq!(
    sess.network_vinfo(SessionPrefixBuf::ROOT.as_slice(), &[b"ghost"]),
    VectorReply::NullArray
  );

  // VREM
  assert_eq!(
    sess.network_vrem(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e2"]),
    VectorReply::Integer(1)
  );
  assert_eq!(
    sess.network_vrem(SessionPrefixBuf::ROOT.as_slice(), &[b"aux", b"e2"]),
  ...
  assert_eq!(
    err_text(sess.network_vdim(SessionPrefixBuf::ROOT.as_slice(), &[b"ghost"])),
    "ERR Key not found"
  );
}
...
fn disabled_and_reply_encoding() {
  ...
  for r in [
    ...
    sess.network_vsim(SessionPrefixBuf::ROOT.as_slice(), &[b"k", b"ELE", b"e"]),
    sess.network_vcard(SessionPrefixBuf::ROOT.as_slice(), &[b"k"]),
    sess.network_vrem(SessionPrefixBuf::ROOT.as_slice(), &[b"k", b"e"]),
  ] {
    assert!(matches!(r, VectorReply::Error(_)), "未启用应拒绝");
  }

  // WRONGTYPE 守卫（无句点文案，对齐 AbortVectorSetWrongType；键不存在 →
  // None 放行。探针为 wkv 同步面，真存储会话承接）
  ...
  assert!(
    sess
      .abort_vector_set_wrong_type(b"somekey", &batch)
      .is_none()
  );
...
fn vadd_variable_length_element_ids() {
  ...
  for id in &ids {
    let emb = sess.network_vemb(SessionPrefixBuf::ROOT.as_slice(), &[b"foo", id.as_slice()]);
...
fn add_remove_and_query_lifecycle() {
  let manager = manager();
  let _ = manager.service.create_index(
    16,
    IndexConfig::new(
      2,
      0,
  ...
  bad_m.num_links = 4;
  let res = manager.try_add(SessionPrefixBuf::ROOT.as_slice(), b"set", &index, &bad_m);
  assert_eq!(
  ...
  // VREM
  assert_eq!(
    manager.try_remove(b"set", &index, b"elem-a"),
    VectorManagerResult::OK
  );
  assert_eq!(
    manager.try_remove(b"set", &index, b"elem-a"),
...
fn element_similarity_and_attributes_batch() {
  ...
  // VSETATTR
  assert!(manager.try_set_attribute(b"set", &index, b"y", b"{\"b\":2}"));
  assert!(!manager.try_set_attribute(b"set", &index, b"none", b"{}"));
}
...
fn sanitize_ingested_records() {
  ...
  assert!(!manager.recovered_indexes.pin().contains_key(&98));
}
...
fn vector_set_opacity() {
  ...
  assert_eq!(
    sess.network_vadd(SessionPrefixBuf::ROOT.as_slice(), &args, SLOT0),
    VectorReply::Integer(1)
  );

  // 向量集键在位（GET foo → WRONGTYPE 的不透明性由键类型承载）
  let index = sess
    .read_index(SessionPrefixBuf::ROOT.as_slice(), b"foo")
    .expect("foo 应为向量集键");
  assert!(sess.manager.is_member(&index.to_bytes(), elem));
}
