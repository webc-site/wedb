...
/// 混合段名请求（INFO server hlogscan）不降级慢路径：hlogscan 段按 wmetric
/// 缺省形态呈现 Empty（同步面不触达存储域扫描），server 段正常输出
#[test]
fn info_mixed_sections_stay_sync() {
  ...
  assert_eq!(consumed, Some(0), "混合段应同步完整消费");
  let text = from_utf8(&out).unwrap();
  assert!(text.contains("# Server\r\n"), "{text}");
  // wmetric 缺省形态：段头保留、不产生条目（同步面不触达存储域扫描，
  // 绝不虚报 Empty 以外的任何分布）
  assert!(text.contains("# MainStoreHLogScan_DB_0\r\n"), "{text}");
  assert!(
    !text.contains("MainStore_HLog_0"),
    "混合段 hlogscan 不出存储域条目: {text}"
  );
}
