...
fn session_with(env: &Env, resp_protocol_version: u8) -> RespServerSession {
...
fn promote_set(env: &Env, key: &[u8], count: usize) -> Vec<Vec<u8>> {
...
fn promote_hash(env: &Env, key: &[u8], count: usize) -> Vec<(Vec<u8>, Vec<u8>)> {
...
fn smembers_tiered_reply_is_byte_identical_to_reference() {
  let env = env("frame-head-smembers.db");
  for ver in VERSIONS {
    let mut s = session_with(&env, ver);
    for count in COUNTS {
...
fn hash_three_arms_replies_are_byte_identical_to_reference() {
  let env = env("frame-head-hash.db");
  for ver in VERSIONS {
    let mut s = session_with(&env, ver);
    for count in COUNTS {
...
fn swept_expiry_backfills_actual_live_count() {
  let env = env("frame-head-swept.db");
...
fn all_expired_sweep_frames_empty_head() {
  let env = env("frame-head-empty.db");
  let stale = now_ticks() - 1;
  let total = 100usize;
  for ver in VERSIONS {
    let key = format!("h{ver}");
...
fn watermark_crossed_scan_err_leaves_no_partial_frame() {
  let env = env("frame-head-scan-err.db");
  let stale = now_ticks() - 1;
...
/// 内存态与分层态回包一致（collection.md 第 5 节透明口径）
///
/// 两态成员迭代序本就不同（内存态走字典序、分层态走树内字节序），故断言面为
/// 帧头（条数同源）与实体**集合**逐字节全等，实体序不参与断言，避免把无序契约
/// 写成假断言。
#[test]
fn memory_and_tiered_replies_agree_on_head_and_member_set() {
  let env = env("frame-head-cross-state.db");
  let count = 120usize;
  let mut s = session_with(&env, 3);
