...
/// zcode-r26-servercmd:
/// 1. 恢复成功与 AOF 重放后回填 last_save_ms，杜绝重启后 LASTSAVE 恒为 0
/// 2. 按需检查点在 last_save_ms > entry_ms 时短路跳过，杜绝重复全量快照
#[test]
fn test_last_save_recovery_and_on_demand_skip() -> aok::Result<()> {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (store_dir, store) = open_test_store("last_save_recovery_and_on_demand")?;
    let cp_dir = store_dir.path().join("cp");
    ...
    // 初始状态 last_save_ms 为 0
    assert_eq!(single.last_save_ms(), 0);
    ...
    assert!(single.take_checkpoint(false).await?);
    let t1 = single.last_save_ms();
    assert!(t1 > 0, "拍摄检查点后 last_save_ms 应推进且大于 0");

    // 模拟全新重启：新建 GarnetDatabase，初始 last_save_ms 为 0
    let recovered_store = Arc::clone(&store);
    let db_restart = Arc::new(GarnetDatabase::new(
      0,
    ...
    assert_eq!(db_restart.last_save_ms(), 0);

    // 执行检查点恢复
    let single_restart = SingleDatabaseManager::new(cp_dir.clone(), Arc::clone(&db_restart));
    let rec = single_restart
      .base
      .recover_database_checkpoint_async(&db_restart, None)
      .await?;
    assert!(rec.is_some());
    assert!(
      db_restart.last_save_ms() > 0,
      "检查点恢复成功后必须回填 last_save_ms"
    );
    let recovered_last_save = db_restart.last_save_ms();

    // 验证按需检查点跳过判定：
    // 若 entry_ms 早于当前 last_save_ms（模拟其他任务已在期间拍过），则跳过拍摄返回 false
    let skipped = single_restart
      .take_on_demand_checkpoint_async(recovered_last_save - 1)
      .await?;
    assert!(!skipped, "期间已推进的按需检查点应跳过执行");
    assert_eq!(db_restart.last_save_ms(), recovered_last_save);

    // 若 entry_ms >= 当前 last_save_ms，则执行拍摄返回 true，last_save_ms 推进
    let taken = single_restart
      .take_on_demand_checkpoint_async(recovered_last_save)
      .await?;
    assert!(taken, "尚未推进的按需检查点应真正拍摄");
    assert!(
      db_restart.last_save_ms() >= recovered_last_save,
      "拍摄后 last_save_ms 应推进"
    );
