...
fn seed_ri_backend(rt: &Runtime, c: &mut RespSessionConsumer, key: &str, backend: &str) {
  assert_eq!(
    cmd(rt, c, &["RI.CREATE", key, backend, "MINRECORD", "8"]),
    b"+OK\r\n",
    "RI.CREATE 应成功"
  );
  assert_eq!(
...
fn seed_ri(rt: &Runtime, c: &mut RespSessionConsumer, key: &str) {
  seed_ri_backend(rt, c, key, "MEMORY");
}
...
fn ri_key_read_gate_folds_wrongtype_and_whitelist_passes() {
  ...
  // 读数据命令（GET / HGET / LINDEX 等）：统一拦截 WRONGTYPE
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["GET", "idx"])));
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["HGET", "idx", "field1"])));
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["LINDEX", "idx", "0"])));
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["SCARD", "idx"])));
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["ZCARD", "idx"])));
}

/// 方向 1 附：写命令拦截（SET / HSET / LPUSH 等试图覆写已有 RI 键时拦截 WRONGTYPE）
#[test]
fn string_write_on_range_index_key_is_wrongtype() {
  ...
  // 试图以普通写命令写 RI 键：WRONGTYPE 拦截，原 RI 键不受破坏
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["SET", "idx", "value"])));
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["HSET", "idx", "f", "v"])));
  assert!(is_wrongtype(&cmd(&rt, &mut c, &["LPUSH", "idx", "v"])));
...
/// 方向 1 附：缺失键读写命令不被 WRONGTYPE 拦截（自然回 nil / 正常新建）
#[test]
fn range_index_missing_key_reports_missing_index() {
  let rt = Runtime::new().unwrap();
  let (_dir, mut c) = consumer();

  // 读不存在的 RI 键：RI 引擎自身报错而非全局 WRONGTYPE
  let out = cmd(&rt, &mut c, &["RI.GET", "nonexistent", "field1"]);
  assert!(
    !is_wrongtype(&out),
    "缺失键不应被 WRONGTYPE 拦截: {}",
    text(&out)
...
fn ri_key_rename_not_wrongtyped() {
  ...
  seed_ri_backend(&rt, &mut c, "idx", "DISK");
...
#[test]
fn string_write_on_range_index_key_is_wrongtype_exhaustive() {
  let rt = Runtime::new().unwrap();
  let (_dir, mut c) = consumer();
...
#[test]
fn range_index_missing_key_reports_missing_index_exhaustive() {
  let rt = Runtime::new().unwrap();
  let (_dir, mut c) = consumer();
