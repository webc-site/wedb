...
//!    PENDING_LAT 计量槽出样本，并经与版本翻转点同一按引用结算口
//!    （`flush` 并入全局延迟表）后，`LATENCY HISTOGRAM` 无参回显里不再是空项；
//! 2. 关延迟监视的会话不建计量槽，同一读命令应答逐字节一致（槽不在位即
...
use wmetric::{GarnetServerMonitor, RespLatencyCommands};
use wnode::resp::{
...
fn open_env(
  ...
  // PENDING_LAT 槽的全局归并出口来自进程级监视器（C# 延迟表构造持
  // monitor 的同形前置）；首装者生效，并发用例复用同一实例
  let _ = Arc::new(GarnetServerMonitor::new(1, true, true, false)).install_global();
  ...
  // 执行域挂入会话即单点回挂 PENDING_LAT 计量槽（C# 构造 storageSession 时
  // 下传 LatencyMetrics 的对位时序）
...
/// PENDING_LAT 计量槽未结算样本数（起停表的观测面）
fn pending_samples(s: &RespServerSession) -> u64 {
  s.pending_latency
    .as_ref()
    .expect("延迟监视开启时必建 PENDING_LAT 计量槽")
    .pending_samples()
}

/// 开延迟监视：磁盘候选降级读（真正 pending）在 PENDING_LAT 出样本，
/// 结算归并后 LATENCY HISTOGRAM 无参回显含 PENDING_LAT
#[test]
fn pending_read_records_pending_latency() {
  ...
  // 按引用结算并入全局延迟表（与版本翻转点同一 flush 口）后走真实命令面回显
  s.pending_latency.as_ref().unwrap().flush();
  let global = GarnetServerMonitor::global()
    .and_then(|m| m.global_latency_metrics())
    .expect("open_env 已装配延迟监视");
  let mut out = Vec::new();
  RespLatencyCommands::network_latency_histogram(&[], Some(&global.lock()), &mut out);
