...
async fn wkv_persistence_and_recall_after_recreate() {
  ...
  assert_eq!(
    service
      .insert(context, b"a", &f32_bytes(&[0.0, 0.0]), b"")
      .await,
    DiskAnnInsertResult::True
  );
  assert_eq!(
    service
      .insert(context, b"b", &f32_bytes(&[1.0, 1.0]), b"")
      .await,
    DiskAnnInsertResult::True
  );
  ...
  service
    ...
    .unwrap();
  assert_eq!(service.card(context), 2);
  assert!(service.check_external_id_valid(context, b"a").await);
  assert!(service.check_external_id_valid(context, b"b").await);
...
/// 结果与冷读一轮完全一致。
#[compio::test]
async fn cold_batch_read_recall_after_flush_and_evict() {
  let dir = tempdir().unwrap();
  ...
  let adjacency_cached = (0..ELEMENTS).any(|iid| {
    let key = _vector_domain
      .session()
      .vector_key(context | term::NEIGHBOR_LIST, &iid.to_le_bytes());
    store
      .index
      .load()
