...
//! `wal/wal.log`，恢复「成功」而数据视图为空/错，且原物理件被写坏。
//!
...
fn standalone_and_cluster_share_one_data_file_and_recover() {
  ...
  let device_len_before = fs::metadata(&data_path).expect("device exists").len();
  assert!(device_len_before > 0, "第一代数据文件须非空落盘");
  drop(provider);

  // 第一代目录形态：仅一份统一命名的数据文件，绝无异名残留件
  let names_after_gen1 = entries(&node_dir);
  assert!(names_after_gen1.contains(&DATA_FILE.to_string()));
  assert!(
    !names_after_gen1
      .iter()
  ...
  let device_len_after = fs::metadata(&data_path).expect("device still exists").len();
  assert!(
