...
async fn drive(addr: &str, batches: &[&[u8]], expect: &[u8]) {
  let mut stream =
    TcpStream::connect(addr.parse::<std::net::SocketAddr>().unwrap()).await.unwrap();
...
fn pump_half_packet_reassembly() {
  ...
  Runtime::new().unwrap().block_on(drive(&addr, &[b"PIN", b"G\r\n"], b"+PONG\r\n"));
  server.stop();
}
...
fn pump_large_pipeline_flush() {
  ...
  Runtime::new().unwrap().block_on(drive(&addr, &[&req], &expect));
  server.stop();
}
