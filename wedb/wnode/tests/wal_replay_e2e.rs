...
#[test]
fn test_range_index_del_to_empty_replays_to_empty_replica() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
...
/// 非零 (ns, db) 会话的 RI 创建/写入/删除端到端回放：钉死「入账键伪造 (0,0)」
/// 缺陷（next/ri-store-event-nsdb-keytag）。主端在 (ns=5, db=3) 建 RI 并写/删字段，
/// 重放到全新副本后：副本默认 (0,0) 域必须无该索引（杜绝幽灵/跨租界穿透），
/// 切至 (5,3) 域方可见且字段完整——证明 AOF 记录携真实 ns/db 前缀寻址。
#[test]
fn test_range_index_replay_preserves_ns_db_domain() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
