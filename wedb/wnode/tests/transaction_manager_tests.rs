...
use wtxn::{LockType, TransactionManager, TxnAofLog, TxnLockTable, TxnState, WatchVersionMap};
...
fn test_log(sublogs: usize, replay_tasks: i32) -> Arc<dyn TxnAofLog> {
...
fn test_compute_sublog_access_vector_sharded() {
  ...
  // 登记加锁键 k1 与 k2
  mgr.save_key_entry_to_lock(root().as_slice(), b"k1", LockType::Exclusive);
  mgr.save_key_entry_to_lock(root().as_slice(), b"k2", LockType::Exclusive);
