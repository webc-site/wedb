...
//!   事务态让闩、持窗期内绝不被串读」；
//! - `readonly_bitfield_never_grabs_exclusive_latch` 断言同步快路径 BITFIELD 只按
//!   写子命令标志取窗（纯读不抢排他闩、写子命令按需取窗），对标 C# hasWriteCommands
//!   选 LockType.Shared/Exclusive。
...
fn concurrent_incr_loses_no_update() {
  let (_dir, store) = open_test_store("rmw-incr.db").unwrap();
...
fn concurrent_append_loses_no_segment() {
  let (_dir, store) = open_test_store("rmw-append.db").unwrap();
...
fn concurrent_hset_loses_no_field() {
  let (_dir, store) = open_test_store("rmw-hset.db").unwrap();
  let handles: Vec<_> = (0..THREADS)
    .map(|t| {
      let store = Arc::clone(&store);
      thread::spawn(move || {
...
fn concurrent_pfadd_matches_serial_reference() {
  let (_dir, store) = open_test_store("rmw-hll.db").unwrap();
  let handles: Vec<_> = (0..THREADS)
    .map(|t| {
      let store = Arc::clone(&store);
      thread::spawn(move || {
  ...
  let (_ref_dir, ref_store) = open_test_store("rmw-hll-ref.db").unwrap();
...
/// 桶号按窗口 scoped 口径寻桶（`whasher::scoped_hash` 前缀种子；测试会话为
/// 缺省根域）
fn distinct_bucket_keys(store: &Arc<WedbStore<SegmentedDevice>>) -> (Vec<u8>, Vec<u8>) {
  ...
  let first_idx = index.bucket_index_for_hash(whasher::scoped_hash(
    SessionPrefixBuf::ROOT.as_slice(),
    first,
  ));
  for i in 0..4096_u32 {
    let candidate = format!("rmw:latch:b{i}").into_bytes();
    if index.bucket_index_for_hash(whasher::scoped_hash(
      SessionPrefixBuf::ROOT.as_slice(),
      &candidate,
    )) != first_idx
    {
...
fn same_key_window_is_exclusive_and_neighbour_key_unaffected() {
  let (_dir, store) = open_test_store("rmw-latch.db").unwrap();
...
fn transactional_window_yields_latch_and_guard_restores() {
  let (_dir, store) = open_test_store("rmw-latch-txn.db").unwrap();
...
fn two_session_windows_never_cross_read_or_lose_update() {
  let (_dir, store) = open_test_store("rmw-latch-rmw.db").unwrap();
...
fn cold_slow_setrange_keeps_every_acked_range() {
  const N: usize = 4;
  let (_dir, store) = open_test_store("rmw-cold-sr.db").unwrap();
  let rt = Runtime::new().unwrap();
...
fn cold_slow_append_keeps_every_acked_segment() {
  const N: usize = 4;
  let (_dir, store) = open_test_store("rmw-cold-ap.db").unwrap();
  let rt = Runtime::new().unwrap();
...
fn cold_slow_setbit_keeps_every_acked_bit() {
  const N: usize = 4;
  let (_dir, store) = open_test_store("rmw-cold-setbit.db").unwrap();
  let rt = Runtime::new().unwrap();
...
///（窗仅在有写子命令时取、纯读及 BITFIELD_RO 不取窗的本票接线判据）
#[test]
fn cold_slow_bitfield_keeps_every_acked_write() {
  const N: usize = 4;
  let (_dir, store) = open_test_store("rmw-cold-bf.db").unwrap();
  let rt = Runtime::new().unwrap();
...
fn cold_slow_incr_acks_distinct_values_and_final_matches_ack_count() {
  const N: usize = 4;
  let (_dir, store) = open_test_store("rmw-cold-incr.db").unwrap();
  let rt = Runtime::new().unwrap();
...
/// 只读子命令绝不申请本键桶排他闩（本票修复的真实行为判据）
///
/// 缺陷（票 wnode-bitfield-read-only-unconditional-exclusive-rmw-latch）：C#
/// `StringBitFieldAction`（BitmapCommands.cs:596）依 `hasWriteCommands` 选锁
///（写子命令 `LockType.Exclusive`、纯读 `LockType.Shared`，单子命令直连不提升事务），
/// 收口前 rust 同步快路径 `string_bit_field_action` 用下划线丢弃 `_has_write_commands`、
/// 无条件 `try_rmw_window` 抢本键桶排他闩，令并发只读 BITFIELD/BITFIELD_RO 互抢排他闩
/// 串行化、并在自旋预算耗尽时误降级。
/// 本用例不赌调度器：以另一独立会话持同键桶排他闩（模拟并发争用实况），断言
/// - 纯读子命令（单/多 GET、BITFIELD_RO）经一次性快照读内联完成并回正确应答
///   （返回值 `true`）；收口前无条件抢窗必自旋耗尽降级（`false`），故本判据可捕获回归；
/// - 写子命令（SET）仍按需取窗：窗被占时降级 `false`（绝不无窗盲写），放窗后
///   内联原子写回 `true` 且新值落库。
#[test]
fn readonly_bitfield_never_grabs_exclusive_latch() {
  let (_dir, store) = open_test_store("bf-latch.db").unwrap();
  let key = b"bf:latch:key".to_vec();

  // 基值：字符串域单字节 200（暖态，命令走同步快路径 string_bit_field_action）
  {
    let sess = store.new_session().unwrap();
    let batch = sess.enter_batch();
    let window = batch.try_rmw_window(&key).expect("基值写回取窗");
    window_write(&window, &[200]);
  }

  // 对面独立会话持有同键桶排他闩，贯穿本用例全部只读断言期（非事务态自取闩）
  let guard_sess = store.new_session().unwrap();
  let guard_batch = guard_sess.enter_batch();
  let guard = guard_batch
    .try_rmw_window(&key)
    .expect("对面持本键桶排他闩");
  ...
  // 单子命令纯读：不抢排他闩，快照读内联完成
  assert!(
    s.string_bit_field(&[&key, b"GET", b"u8", b"0"], &batch, &mut out)
      .unwrap(),
    "只读 BITFIELD 在他会话持同键排他窗时不得误降级（收口前无条件抢窗即 false）"
  );
  assert_eq!(out, b"*1\r\n:200\r\n");

  // BITFIELD_RO 纯读：同样不抢排他闩
  out.clear();
  assert!(
    s.string_bit_field_read_only(&[&key, b"GET", b"u8", b"0"], &batch, &mut out)
      .unwrap(),
    "BITFIELD_RO 在他会话持同键排他窗时不得误降级"
  );
  assert_eq!(out, b"*1\r\n:200\r\n");

  // 多子命令纯读（两段 GET 落在同一字节内，值确定）：仍不抢排他闩
  out.clear();
  assert!(
    s.string_bit_field(
      &[&key, b"GET", b"u4", b"0", b"GET", b"u4", b"4"],
      &batch,
      &mut out
    )
    .unwrap(),
    "多 GET 子命令的只读 BITFIELD 不得抢排他闩"
  );
  // 200 = 0b1100_1000：高 4 位 12、低 4 位 8
  assert_eq!(out, b"*2\r\n:12\r\n:8\r\n");

  // 写子命令（SET）：仍按需取排他窗，窗被占 → 降级 false（不得盲写）
  out.clear();
  assert!(
    !s.string_bit_field(&[&key, b"SET", b"u8", b"0", b"7"], &batch, &mut out)
      .unwrap(),
    "写子命令须按需取排他窗：窗被占时应降级，不得无窗盲写"
  );
  ...
  assert_eq!(out, b"*1\r\n:200\r\n", "SET 回旧值 200");
  assert_eq!(batch_value(&batch, &key), vec![7], "写回原子生效、新值落库");
}
