...
fn assert_null_forms(resp3: bool) {
  ...
  with_batch(|s, batch| {
    ...
    let mut out = Vec::new();
    s.network_set(&[b"exists_key", b"v1"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    let mut out = Vec::new();
    s.network_set(&[b"exists_key", b"v2", b"NX"], batch, &mut out)
      .unwrap();
    assert_eq!(out, nil, "SET NX 条件失败 @resp{ver}");
    let mut out = Vec::new();
    s.network_set(&[b"absent_key", b"v1", b"XX"], batch, &mut out)
      .unwrap();
    ...
    let mut out = Vec::new();
    s.network_set(&[b"bf_key", b"abc"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    let mut out = Vec::new();
