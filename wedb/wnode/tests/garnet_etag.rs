...
fn get_with_etag_on_non_etag_data_returns_0() {
  with_batch(|s, batch| {
    s.network_set(&[b"h", b"k"], batch, None, &mut Vec::new())
      .unwrap();
...
fn set_if_match_returns_new_value_and_etag_when_etag_matches() {
  with_batch(|s, batch| {
    ...
    s.network_del(&[b"florida"], batch, &mut Vec::new())
      .unwrap();
    s.network_set(&[b"florida", b"one"], batch, None, &mut Vec::new())
      .unwrap();
    let out = setif((s, batch), false, &[b"florida", b"lastOne", b"1738"]);
    assert_eq!(out, b"*2\r\n:0\r\n$3\r\none\r\n");
  });
...
fn set_if_greater_works_without_initial_etag() {
  with_batch(|s, batch| {
    s.network_set(&[b"meow-key", b"m"], batch, None, &mut Vec::new())
      .unwrap();
...
fn del_if_greater_works() {
  with_batch(|s, batch| {
    ...
    s.network_set(&[b"plain", b"m"], batch, None, &mut Vec::new())
      .unwrap();
...
fn set_with_etag_on_non_etag_data_initializes_to_1() {
  with_batch(|s, batch| {
    s.network_set(&[b"rizz", b"used"], batch, None, &mut Vec::new())
      .unwrap();
    assert_eq!(setwithetag((s, batch), &[b"rizz", b"buzz"]), 1);

    // DEL 后重建：etag 归零重来（级联清理不留残值）
    s.network_del(&[b"rizz"], batch, &mut Vec::new()).unwrap();
    s.network_set(&[b"rizz", b"my"], batch, None, &mut Vec::new())
      .unwrap();
...
fn plain_set_overwrite_keeps_etag() {
  with_batch(|s, batch| {
    ...
    // 普通 SET 覆写：etag 不动
    s.network_set(&[b"k", b"v2"], batch, None, &mut Vec::new())
      .unwrap();
...
fn rename_without_etag_clears_stale_etag_on_new_key() {
  with_batch(|s, batch| {
    ...
    // 旧键为无 etag 的普通字符串
    s.network_set(&[b"src", b"v1"], batch, None, &mut Vec::new())
      .unwrap();
