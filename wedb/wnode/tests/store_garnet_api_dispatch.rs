...
fn store_garnet_api_vector_dispatch() {
  ...
  Runtime::new().unwrap().block_on(async {
    ...
    resp_session.set_garnet_api(Arc::clone(&api));

    // 1. 执行 VADD（写族挂起化：exec 同步段登记 SlowWait 停车，本处模拟
    // 网络泵 take_slow_wait → resolve 驱动后取应答字节）
    let v1 = [0u8; 8];
    api.exec(
      &mut resp_session,
      RespCommand::Vadd,
    ...
    assert_eq!(resp_session.output, b"");
    let slow = resp_session.take_slow_wait().expect("VADD 须登记挂起体");
    assert_eq!(slow.resolve().await, b":1\r\n");

    // 2. 执行 VCARD（三臂同步段挂起化：同步段零输出，SlowWait 承接，
    // resolve 交付应答——与 vector_read_lock_arms_first_touch_parity 同口径）
    resp_session.output.clear();
    api.exec(&mut resp_session, RespCommand::Vcard, &[b"mykey"]);
    assert_eq!(resp_session.output, b"");
    let slow = resp_session.take_slow_wait().expect("VCARD 须登记挂起体");
    assert_eq!(slow.resolve().await, b":1\r\n");
    ...
    assert_eq!(resp_session.output, b"");
    let slow = resp_session.take_slow_wait().expect("VDIM 须登记挂起体");
    assert_eq!(slow.resolve().await, b":2\r\n");
  })
}
