...
use waof::{NO_COOKIE, WalLog, WalRecord};
...
impl<D: Device> WaofSublog<D> {
  ...
  /// 全量扫描（[`Self::scan_async_with`] 的 Vec 物化形态；副本回放消费）。
  pub async fn scan_async(&self, begin_address: i64, end_address: i64) -> Vec<WalRecord> {
    let mut records = Vec::new();
    let _ = self
      .scan_async_with::<(), _, _>(begin_address, end_address, |rec| {
        records.push(rec);
        async { Ok(true) }
      })
      .await;
