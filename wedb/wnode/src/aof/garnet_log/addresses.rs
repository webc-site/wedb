...
impl GarnetLog {
  ...
  ///
  /// 恢复时以各子日志提交 cookie 收敛恢复上限（C# 以 out 参返回，
  /// cookie 缺失返回 false；rust 以 Option 承接同语义）。
  ...
  /// libs/server/AOF/GarnetLog.cs:Scan
  ///
  /// Scan 的全量设备面迭代器直取（零闭包形态；副本重放热路径扁平
  /// 直驱；C# TsavoriteLogScanIterator 消费面由
  /// [`WalScanIterator`] 对位承接；闭包形态见
  /// [`Self::scan_single_async_with`]）。
  #[inline]
