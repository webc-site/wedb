...
use super::{
  AofSettings, aof_backpressure::AofBackpressure, sharded_log::ShardedLog, single_log::SingleLog,
...
impl GarnetLog {
  ...
  pub fn new(
    ...
    // 装配期尺寸体检单点（对标 C# 子日志设置装配前的组合校验）：三尺寸旋钮组合
    // 非法即在此拒启，不留运行期；投影出的物理尺寸由物理日志构造口装载
    //（见 AofSettings::wal_config），本口不重复计算。启动期其余组合校验
    //（如提交频率与等待提交档位）在同一入口接续，禁第二套体检函数。
    AofSettings::from_options(server_options)?;
