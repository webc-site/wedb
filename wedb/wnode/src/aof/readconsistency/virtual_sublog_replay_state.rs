...
/// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:ReadSessionWaiter
///
/// 可复用等待节点（对标 C# ReadSessionWaiter：ManualResetEventSlim 形态）。
/// 具备异步（compio reactor 安全）与同步双模式等待能力。
pub struct ReadSessionWaiter {
  /// 目标序列号（wait 装载）。
  target: AtomicI64,
  /// 唤醒事件（支持异步 listener.await 与同步 wait_timeout）。
  event: Event,
  /// 权威已通知标志（对标 ManualResetEventSlim.IsSet）。
  signaled: AtomicBool,
}

impl Default for ReadSessionWaiter {
  fn default() -> Self {
    Self {
...
impl ReadSessionWaiter {
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:Reset
  ///
  /// 装载目标并清除陈旧信号（C# ReadSessionWaiter.Reset：清链表指针 + 复位
  /// 唤醒事件；rust 节点离链单属主访问，指针域由所有权消除）。
  pub fn reset(&self, target: i64) {
    self.target.store(target, Ordering::Release);
    self.signaled.store(false, Ordering::Release);
  }

  /// 唤醒通知（回放推进时触发；对标 C# node.Signal.Set()）。
  #[inline]
  pub fn signal(&self) {
  ...
  /// 阻塞至 signaled 或超时；返回 false 表示超时（同步线程使用）。
  pub fn wait(&self, timeout: Duration) -> bool {
    ...
    listener.wait_timeout(timeout).is_some() || self.is_signaled()
  }
}

/// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:VirtualSublogReplayState
///
/// 虚拟子日志回放状态。
...
impl VirtualSublogReplayState {
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:Max
  ///
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:GetSketchSlot
  ///
  /// 草图槽下标（hash 高 32 位掩码）。
  #[inline]
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:GetFrontierSequenceNumber
  ///
  /// 前沿序列号：max(草图谱值, 子日志前沿)。
  #[inline]
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:GetKeySequenceNumber
  ///
  /// key 序列号（草图谱值直读）。
  #[inline]
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:PrefetchKeySequenceNumber
  ///
  /// 草图谱预取（C# Sse.Prefetch0）：读侧提前预热草图槽所在
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:UpdateMaxSequenceNumber
  ///
  /// 推进子日志最大序列号（单调）。
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:UpdateKeySequenceNumber
  ///
  /// 推进 key 序列号草图（单调）。
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:SignalWaiters
  ///
  /// 唤醒目标已达成的等待者（自队首起摘除）。
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:WaitForSequenceNumber
  ///
  /// 等待子日志前沿越过会话最大序列号。
  ///
  /// 自旋快路径 → 装载可复用节点入队 → 同步等待；超时返回 false
  ...
  pub fn wait_for_sequence_number(
    ...
    let ok = listener.wait_timeout(timeout).is_some() || waiter.is_signaled();
    if !ok {
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:InsertWaiter
  ///
  /// 升序插入等待节点（须持锁）。
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:RemoveWaiter
  ///
  /// 摘除节点（公开形态，供超时/取消方调用）。
  ...
  /// libs/server/AOF/ReadConsistency/VirtualSublogReplayState.cs:UpdateMinWaiterTarget
  ///
  /// 刷新最小等待目标（须持锁）。
