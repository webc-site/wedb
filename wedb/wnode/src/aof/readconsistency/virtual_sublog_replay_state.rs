...
impl ReadSessionWaiter {
  ...
  /// 异步等待至 signaled 或超时（compio 反应堆安全，不阻塞 worker 线程）。
  pub async fn wait_async(&self, timeout: Duration) -> bool {
    if self.is_signaled() {
      return true;
    }
    let listener = self.listen();
    ...
    match compio_timeout(timeout, listener).await {
      Ok(()) => true,
      Err(_) => self.is_signaled(),
    }
  }
}
...
impl VirtualSublogReplayState {
  ...
  /// 异步等待子日志前沿越过会话最大序列号（compio 反应堆安全，不挂起 worker 线程）。
  pub async fn wait_for_sequence_number_async(
    &self,
    maximum_session_sequence_number: i64,
    waiter: &Arc<ReadSessionWaiter>,
    timeout: Duration,
...
mod tests {
  ...
  #[test]
  fn waiter_async_signaled_and_timeout() {
    Runtime::new().unwrap().block_on(async {
      ...
      assert!(
        state
          .wait_for_sequence_number_async(4, &waiter, Duration::from_millis(1))
          .await
      );

      // 目标高于前沿但超时：返回 false 且安全清理
      assert!(
        !state
          .wait_for_sequence_number_async(100, &waiter, Duration::from_millis(10))
          .await
      );
      assert!(state.waiters.lock().is_empty());
    });
