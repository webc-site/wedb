...
mod tests {
  use super::*;
  use crate::aof::garnet_log::{GarnetLog, InMemorySublog};
  ...
  fn consistency_aof() -> Arc<GarnetAppendOnlyFile> {
    ...
    let backends = (0..2).map(|_| Arc::new(Sublog::Mem(InMemorySublog::new()))).collect();
  ...
  fn add_hash_then_update_routes_by_hash() {
    ...
    tracker.add_hash(hash_a);
    tracker.add_hash(hash_b);
