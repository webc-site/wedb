...
use crate::aof::garnet_log::{GarnetLog, virtual_sublog_idx};

/// libs/server/AOF/ReadConsistency/ReadConsistencyManager.cs:ReadConsistencyManager
///
/// 读取一致性管理器。
pub struct ReadConsistencyManager {
  ...
  /// 读侧新鲜度等待超时（C# serverOptions.ReplicaSyncTimeout，
  /// ReadConsistencyManager.cs:84 同源传入）。
...
impl ReadConsistencyManager {
  /// libs/server/AOF/ReadConsistency/ReadConsistencyManager.cs:ReadConsistencyManager
  /// libs/server/AOF/ReadConsistency/ReadConsistencyManager.cs:BuildVirtualSublogReplayStates
  ///
  /// 构造（C# 主构造的显式参数形态：拓扑 + 漂移配置；read_timeout 对标 C#
  /// 构造自 serverOptions 取 ReplicaSyncTimeout 传入栅栏与会话超时）。
  /// C# BuildVirtualSublogReplayStates 静态构建体（逐虚拟子日志播种首个
  /// 漂移扫描窗口下界，禁用时播种 i64::MAX）折叠为构造内联段。
  pub fn new(
    current_version: i64,
    physical_sublog_count: usize,
    replay_task_count: usize,
    ...
    Self {
      ...
      read_timeout,
      replay_barrier: ReplayAlignBarrier::new(virtual_sublog_count, Some(read_timeout)),
    }
  }

  /// 读侧新鲜度等待超时（C# serverOptions.ReplicaSyncTimeout 投影）。
  #[inline]
  pub fn read_timeout(&self) -> Duration {
    self.read_timeout
  }

  /// libs/server/AOF/ReadConsistency/ReadConsistencyManager.cs:CurrentVersion
  ///
  /// 管理器版本。
  #[inline]
  ...
  /// C# serverOptions.AofVirtualSublogCount 驱动的 vsrs 规模（精确锚点见 garnet_append_only_file.rs）
  ///
  /// 虚拟子日志数。
  ...
  /// 经域内单点 [`virtual_sublog_idx`] 换算）。
  #[inline]
  pub fn virtual_sublog_idx_of_hash(&self, hash: i64) -> usize {
    let h = hash as u64;
    let p = self.physical_sublog_count as u64;
    let r = self.replay_task_count as u64;
    virtual_sublog_idx(
      (h % p) as usize,
      (h / p % r) as usize,
  ...
  pub fn get_physical_sublog_max(&self, physical_sublog_idx: usize) -> i64 {
    let start_idx = virtual_sublog_idx(physical_sublog_idx, 0, self.replay_task_count);
  ...
  /// AdvanceVirtualSublogTime 的全槽折叠入口（对位
  /// libs/cluster/Server/Replication/ReplicaOps/AOFReplay/ReplicaReplayTask.cs 脉冲支）
  ///
  /// 一颗脉冲覆盖本物理子日志的全部虚拟子日志：C# 侧广播由回放任务各自推自有
  /// 槽（ReplicaReplayTask 的 FullPageBasedBackgroundReplayAsync 脉冲支），本仓
  /// 回放折叠为单消费者，故由本入口按回放任务数一次性落全槽，逐槽仍走
  /// [`Self::advance_virtual_sublog_time`] 的到场登记，槽算式与
  /// [`Self::update_physical_sublog_max_sequence_number`] 同一单点。
  pub fn advance_physical_sublog_time(&self, physical_sublog_idx: usize, sequence_number: i64) {
    let start_idx = virtual_sublog_idx(physical_sublog_idx, 0, self.replay_task_count);
    for rt in 0..self.replay_task_count {
      self.advance_virtual_sublog_time(start_idx + rt, sequence_number);
    }
  }
  ...
  /// 超时上抛 [`wkv::Error::ConsistentReadTimeout`] 中止一致读（C#
  /// WaitForSequenceNumber 超时抛 TimeoutException 同义），不静默续读。
      ...
      && mssn >= replica_read_session_context.cached_sublog_max(virtual_sublog_idx)
    {
      ...
      if mssn >= sketch_max_value {
        ...
        if !vsr.wait_for_sequence_number(mssn, replica_read_session_context.waiter(), timeout) {
          return Err(Error::ConsistentReadTimeout);
        }
...
mod tests {
  ...
  fn manager() -> ReadConsistencyManager {
    ...
    ReadConsistencyManager::new(1, 2, 2, -1, 0, Duration::from_millis(10))
  }
  ...
  fn virtual_sublog_routing_matches_formula() {
    ...
    // 域共享内核换算公式（测试拓扑 replay_task_count = 2）：1 * 2 + 2 = 4
    assert_eq!(virtual_sublog_idx(1, 2, 2), 4);
  }
  ...
  fn batch_protocol_cross_sublog_wait_and_validate() {
    ...
    let ctx = ReplicaReadSessionContext::default();
    let got1 = m
      .pre_batch_key_consistent_read(k1, &ctx, Duration::from_millis(10))
      .unwrap();
    // 第二键所在子日志已回放超越会话序列号：等待后通过
    m.update_virtual_sublog_key_sequence_number(m.virtual_sublog_idx_of_hash(h2), h2, 8);
    let got2 = m
      .pre_batch_key_consistent_read(k2, &ctx, Duration::from_millis(500))
      .unwrap();
  ...
  fn drift_bounding_opens_barrier_round() {
    // 阈值 5：漂移 > 5 即开轮
    let m = ReadConsistencyManager::new(1, 2, 1, 5, 0, Duration::from_millis(10));
  ...
  fn advance_virtual_sublog_time_signals_barrier() {
    let m = ReadConsistencyManager::new(1, 1, 2, 5, 0, Duration::from_millis(10));
