    ...
    while !data.is_empty() {
      let (buffer, capacity): (&mut Vec<u8>, usize) = match self.current_component {
        ...
        Component::Value if self.is_object_value => {
          ...
          let take = declared
            .saturating_sub(self.value_bytes_len)
            .min(data.len());
...
impl AofChunkedRecordReader {
  ...
  pub fn read_chunk(&mut self, entry: &[u8]) -> Option<ChunkedAccumulator> {
    ...
    let data = entry.get(data_offset..)?;
    ...
    if acc.all_components_filled() && acc.verify().is_ok() {
      let mut acc = self.in_progress.remove(&object_id)?;
...
mod tests {
  ...
  use crate::aof::{
    ...
    test_support::test_backends_with_config,
  };
  ...
  fn chunked_upsert_entry() -> Vec<Vec<u8>> {
    let options = RuntimeServerOptions::default();
    let backends = test_backends_with_config("chunk_reader", 1, small_page_config());
    ...
    log.scan_single_with(0, 0, i64::MAX, |r| {
      // 提交元数据帧非 AOF 条目，单点判据滤除（对标 C#
      // TsavoriteLogScanIterator.cs:GetNext 对 isCommitRecord 的跳过；与产品
      // 重放口 AofProcessor::process_aof_record_internal 的消费判据同源）。
      // auto_commit 下常驻提交协程异步在队尾补写提交帧，与本扫描合法并发，
      // 不过滤则尾随帧被误喂分块读取器，末条不闭合（r40 偶发红根因）。
  ...
  fn read_chunk_reassembles_from_cycle1_writer() {
    let records = chunked_upsert_entry();
  ...
  fn duplicate_chunk_after_completion_is_corrupt() {
    ...
    assert!(last.is_some());
    // 首块重放：已完成记录已自进行中映射移除，同 objectId 按新首块建簿后
    // 永无续块闭合（重复块为损坏，返回 None 且不外溢重组结果）
    assert!(reader.read_chunk(&records[0]).is_none());
  }
}
