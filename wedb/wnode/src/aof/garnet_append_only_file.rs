...
use wdatabase::{DatabaseAof, GarnetDatabase};
use wkv::Error;
...
use crate::{resp::vector::vector_manager::VectorManager, storage::StorageSession};
...
/// 对应 libs/server/AOF/GarnetAppendOnlyFile.cs:ReplayDatabaseAof 副本同步重放地址决策参数
...
impl GarnetAppendOnlyFile {
  ...
  /// libs/server/AOF/GarnetAppendOnlyFile.cs:HeaderSize
  ///
  /// 页头开销字节数。
  ...
  /// libs/server/AOF/GarnetAppendOnlyFile.cs:VirtualSublogCount
  ///
  /// 虚拟子日志总数。
  ...
  /// libs/server/AOF/GarnetAppendOnlyFile.cs:MaxAofAddress
  ///
  ...
  /// 异步等待全拓扑 AOF 提交落盘（0 表示等待当前尾地址；无锁事件挂起）
  #[inline]
  pub async fn wait_for_commit_async(&self, until_address: i64) {
    self.log().wait_for_commit_all_async(until_address).await;
  }
  ...
  /// libs/server/AOF/GarnetAppendOnlyFile.cs:TruncateUntil
  ///
  /// 异步截断至指定 AOF 地址
  #[inline]
  ...
  /// libs/server/AOF/GarnetAppendOnlyFile.cs:Recover
  ///
  /// 异步恢复
  #[inline]
  ...
  /// libs/server/AOF/GarnetAppendOnlyFile.cs:ReplayDatabaseAof
  ///
  /// 重放 AOF 条目至指定地址
  pub async fn replay_database_aof<D: wdev::Device>(
    self: Arc<Self>,
    db: &GarnetDatabase<D, Self>,
    until: u64,
  ) -> wkv::Result<u64> {
...
impl<D: wdev::Device> DatabaseAof<D> for GarnetAppendOnlyFile {
  ...
  fn wait_for_commit_async(&self, until_address: i64) -> impl Future<Output = ()> + '_ {
    self.wait_for_commit_async(until_address)
  }

  #[inline]
  fn safe_flush_address(&self) -> Option<u64> {
    self.safe_flush_address()
  }
  ...
  fn replay_database_aof<'a>(
    ...
    Self::replay_database_aof(self, db, until)
  }
}
...
mod tests {
  ...
  use super::*;
  use crate::aof::{
    garnet_log::{InMemorySublog, RecordShape},
    sublog::Sublog,
  };
  ...
  fn sync_replay_address_decision() {
    let aof = aof_with(1, 1);
    // 主侧写入并提交：地址 1..tail
    let address = aof.log().enqueue(&RecordShape {
      op_type: AofEntryType::StoreUpsert,
      version: 2,
      session_id: 1,
      key: b"k",
