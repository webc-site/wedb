...
use super::{
  ...
  garnet_log::{AofWriteContext, GarnetLog, RecordShape},
...
impl GarnetAppendOnlyFile {
  ...
  /// C# GarnetLog TruncateUntil 转发（精确锚点见 garnet_log/addresses.rs）
  ///
  /// 异步截断至指定 AOF 地址
  #[inline]
  ...
  /// 重置日志（位点归零）
  ///
  /// C# ResetDatabase 直调 `AppendOnlyFile.Log.Reset()`；真身路由至
  /// [`GarnetLog::reset_async`]
  #[inline]
  ...
  /// C# GarnetLog RecoverAsync 转发（精确锚点见 garnet_log/commit.rs）
  ///
  /// 异步恢复
  #[inline]
