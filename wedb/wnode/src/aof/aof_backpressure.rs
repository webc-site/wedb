...
pub struct AofBackpressure {
  ...
  /// 关停标志：置位后所有等待方立即放行。
  disposed: AtomicBool,
  /// 拥有日志（尾地址反查），构造后经 set_log / set_weak_log 注入。
  log: Mutex<Option<LogTailHandle>>,
}
...
impl AofBackpressure {
  ...
  fn query_tail_address(&self, sublog_idx: usize) -> Option<i64> {
    ...
    guard.as_ref()?.get_tail_address(sublog_idx)
  }
  ...
  pub fn any_stalled(&self) -> bool {
    ...
    for (i, watermark) in self.shipped_watermark.iter().enumerate() {
      let tail = self.query_tail_address(i).unwrap_or(0);
  ...
  ///
  /// 注入拥有日志的反查引用（强引用，仅构造期调用）。
  pub fn set_log(&self, log: Arc<GarnetLog>) {
    *self.log.lock() = Some(LogTailHandle::GarnetStrong(log));
  }

  /// 注入拥有日志的弱引用反查（对齐 C# raw field 语义，避免 Arc 环引用）。
  pub fn set_weak_log(&self, log: Weak<GarnetLog>) {
    *self.log.lock() = Some(LogTailHandle::GarnetWeak(log));
  }

  /// 注入原子计数器（用于单测与测试桩）。
  pub fn set_counter_log(&self, counter: Arc<AtomicU64>) {
    *self.log.lock() = Some(LogTailHandle::Counter(counter));
  }
