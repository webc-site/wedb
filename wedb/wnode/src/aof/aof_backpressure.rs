...
//! 使用 parking_lot::RwLock 消除查询 tail 时的排队互斥，
//! 使用 event_listener 无锁事件驱动异步挂起与快速唤醒，不阻塞 compio worker 线程。
...
/// 子日志尾地址句柄（支持强引用与弱引用，防止与 GarnetLog 产生引用环，避免虚表开销）。
pub enum LogTailHandle {
  GarnetStrong(Arc<GarnetLog>),
...
impl LogTailHandle {
  ...
  pub fn get_tail_address(&self, sublog_idx: usize) -> Option<i64> {
    match self {
      Self::GarnetStrong(arc) => Some(arc.get_tail_address(sublog_idx)),
...
pub struct AofBackpressure {
  ...
  log: RwLock<Option<LogTailHandle>>,
...
impl AofBackpressure {
  ...
  fn query_tail_address(&self, sublog_idx: usize) -> Option<i64> {
    self.log.read().as_ref()?.get_tail_address(sublog_idx)
  }
  ...
  pub fn any_stalled(&self) -> bool {
    ...
    let budget = self.per_sublog_budget.load(Ordering::Relaxed);
    let log_guard = self.log.read();
    let handle = log_guard.as_ref();
  ...
  #[inline]
  pub fn set_log(&self, log: Arc<GarnetLog>) {
    *self.log.write() = Some(LogTailHandle::GarnetStrong(log));
  }

  #[inline]
  pub fn set_weak_log(&self, log: Weak<GarnetLog>) {
    *self.log.write() = Some(LogTailHandle::GarnetWeak(log));
  }

  #[inline]
  pub fn set_counter_log(&self, counter: Arc<AtomicU64>) {
    *self.log.write() = Some(LogTailHandle::Counter(counter));
  }
