...
use waof::AofHeader;
use wcustom::{CustomTransactionProcedure, SharedCustomCommandManager, TxnProcFactory};
...
/// 注册表驱动的存储过程重放执行器（C# `RunCustomTxnProcAtReplica` 的无会话投影：
...
pub struct StoredProcRegistryReplayer {
  ...
  registry: SharedCustomCommandManager,
}

impl StoredProcRegistryReplayer {
  /// 以注册表构造执行器。
  pub fn new(registry: SharedCustomCommandManager) -> Self {
    Self { registry }
  }
...
mod tests {
  ...
  fn replayer_with_set_proc() -> StoredProcRegistryReplayer {
    let mut manager = CustomCommandManager::new();
    let id = manager
      .register_transaction("SETX", Some(set_proc_factory), None, None)
      .unwrap();
    assert_eq!(id, 0);
    StoredProcRegistryReplayer::new(Arc::new(RwLock::new(manager)))
  }
  ...
  fn replay_executes_proc_and_collects_key_hashes() {
    *LAST_SET_KV.lock() = None;
    let replayer = replayer_with_set_proc();
    ...
    assert_eq!(recorded.0, b"k1");
    assert_eq!(recorded.1, b"v1");
