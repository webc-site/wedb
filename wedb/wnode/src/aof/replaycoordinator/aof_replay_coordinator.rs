...
impl LeaderBarrier {
  ...
  /// 返回 true 表示当前为 Leader（首个到场者，已等待全员到齐或超时收敛）；
  /// 返回 false 表示当前为 Follower（已等待 Leader 释放或超时收敛）。
  ///
  /// 等待有界：超过 `timeout` 仍未会合/未放行即停止等待就地续跑——C#
  /// `ManualResetEventSlim.Wait(TimeSpan)` 超时只返回 false 不抛异常，首到场者
  /// 仍以 Leader 身份返回独占执行、Follower 照常放行续跑
  ///（libs/common/Synchronization/LeaderBarrier.cs:30-65）；缺席参与者由错误臂
  /// 取消链路收口（错误经 error_slot 于 join 后上抛），恢复绝不因栅栏缺员
  /// 永久悬挂。
  pub fn try_signal_or_wait(&self, timeout: Duration) -> bool {
    ...
    if is_first {
      while !state.all_arrived && !state.released {
        if self.release_first.wait_for(&mut state, timeout).timed_out() {
          break;
        }
      }
      true
      ...
      while !state.released {
        if self.release_all.wait_for(&mut state, timeout).timed_out() {
          break;
        }
      }
      false
...
pub enum TxnAction {
  ...
  /// 非事务条目：交还调用方按常规分派。
  None,
  /// 非法事务流拒绝（活动组在位撞嵌套 TxnStart / 组内 StoredProcedure）：
  /// 携拒绝文案交消费口转 AofReplayError 上抛，恢复即中止——C# 同位两臂
  /// throw GarnetException（AofReplayCoordinator.cs:146-147/:169-170），
  /// 绝不静默吞并保留残组带病续放。
  Reject(&'static str),
}

/// 活动组在位撞嵌套 TxnStart 的拒绝文案（C# 同位逐字：
/// AofReplayCoordinator.cs:146 "No nested transactions expected"）。
const REJECT_NESTED_TXN_START: &str = "No nested transactions expected";

/// 活动组在位撞组内 StoredProcedure 的拒绝文案（复用组外 SP 消费基线
/// replay_stored_proc「已移除自定义事务过程支持」同源文案 + C#
/// "within transaction" 语境，AofReplayCoordinator.cs:169）。
const REJECT_STORED_PROC_IN_TXN: &str = "已移除自定义事务过程支持，无法回放事务组内存储过程条目";
...
pub struct AofReplayCoordinator {
  ...
  /// 栅栏会合有界时限（C# serverOptions.ReplicaSyncTimeout 的同源投影：
  /// 构造期取 GarnetAppendOnlyFile::read_timeout 单一配置位，不建第二来源）。
  replica_sync_timeout: Duration,
}
...
impl AofReplayCoordinator {
  ...
  /// `replica_sync_timeout` 对标 C# serverOptions.ReplicaSyncTimeout（栅栏
  /// TrySignalOrWait 的会合时限，装配点自 GarnetAppendOnlyFile::read_timeout
  /// 同源传入）。
  pub fn new(
  ...
  ///
  /// 事务条目消化：
  /// 1. 会话已有活动组 → TxnStart 嵌套与组内 StoredProcedure 以
  ///    TxnAction::Reject 拒绝（消费口转 AofReplayError 上抛，C# 同位两臂
  ///    throw GarnetException，恢复即中止）/ TxnAbort 清组 / TxnCommit
  ///    提交或入模糊区 / 其余入组；
  ...
  pub fn add_or_replay_transaction_operation(
    ...
    if ctx.active_txns.contains_key(&session_id) {
      match op_type {
        // C# AofReplayCoordinator.cs:146-147 throw "No nested transactions
        // expected"——孤儿 TxnStart 后接新 TxnStart 的坏流撞点，恢复即中止
        AofEntryType::TxnStart => TxnAction::Reject(REJECT_NESTED_TXN_START),
        ...
        // C# AofReplayCoordinator.cs:169-170 throw "Unexpected AOF header
        // operation type .. within transaction"——文案对位组外 SP 消费基线
        // replay_stored_proc 恒 Err + within transaction 语境
        AofEntryType::StoredProcedure => TxnAction::Reject(REJECT_STORED_PROC_IN_TXN),
        _ => {
  ...
  /// TrySignalOrWait 段）；返回的 BarrierJoin 承接 Leader 清理凭证。等待以
  /// `replica_sync_timeout` 为界（对位 C# TrySignalOrWait(out _, ReplicaSyncTimeout)
  /// 的有限会合），超时就地续跑、缺席者由取消链路收口，不产生错误。
  fn join_barrier(
    &self,
    barrier_id: i32,
    sequence_number: i64,
    ...
    let is_leader = barrier.try_signal_or_wait(self.replica_sync_timeout);
    BarrierJoin {
      coord: self,
      key,
      barrier,
...
mod tests {
  ...
  /// 测试栅栏会合时限（生产对位 ReplicaSyncTimeout；会合双臂毫秒级到场，
  /// 时限仅为语义在场——两参与者必先于超时会合，不构成实际等待）
  const TEST_SYNC_TIMEOUT: Duration = Duration::from_secs(60);

  #[test]
  fn test_barrier_lifecycle() {
    let coord = AofReplayCoordinator::new(1, false, TEST_SYNC_TIMEOUT);
  ...
  fn test_multi_participant_barrier() {
    let coord = Arc::new(AofReplayCoordinator::new(2, true, TEST_SYNC_TIMEOUT));
