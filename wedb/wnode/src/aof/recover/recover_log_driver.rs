...
/// 页内条目切片描述（用于页内并行回放任务分派）
#[derive(Debug, Clone, Copy)]
pub struct PageReplaySlice<'a> {
  pub entries: &'a [&'a [u8]],
...
impl RecoverLogDriver {
  ...
  /// 单子日志的虚拟子日志数（路由换算）。
  fn virtual_sublog_per_sublog(&self, aof: &GarnetAppendOnlyFile) -> usize {
