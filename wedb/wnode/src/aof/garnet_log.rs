...
use waof::{
  AofAddress, AofChunkHeader, AofHeader, AofHeaderType, AofShardedHeader,
  AofShardedLogTransactionHeader, AofSingleLogTransactionHeader, SequenceNumberGenerator,
};
use wbase::entry_type::{AofEntryType, REPLAY_TASK_ACCESS_VECTOR_BYTES};
...
impl Default for InMemorySublog {
  fn default() -> Self {
    Self::new()
  }
}
...
impl SublogBackend for InMemorySublog {
  ...
  fn enqueue_parts(&self, parts: &[&[u8]]) -> i64 {
    ...
    records.push(LogRecord { address, payload });
    address
  }
  ...
  fn shift_begin_address(&self, new_begin: i64) {
    // 对标 C# TsavoriteLog.TruncateUntil 的 MonotonicUpdate：begin 只进不退
    self.begin.fetch_max(new_begin, Ordering::Release);
  }
  ...
  async fn wait_for_commit_async(&self, until_address: i64) {
    ...
    if self.committed_until_address() >= target {
      return;
    }
    for _ in 0..16 {
      if self.committed_until_address() >= target {
...
impl GarnetLog {
  ...
  pub async fn commit_async(&self) {
    if let Some(single) = &self.single_log {
      ...
      if let Some(evt) = self.commit_events.first() {
        evt.notify(usize::MAX);
      }
      return;
    }
    let cookie = self.next_sequence_number();
    if let Some(sharded) = &self.sharded_log {
      for (i, sublog) in sharded.sublog.iter().enumerate() {
        sublog.commit_flush_async(cookie).await;
        if let Some(evt) = self.commit_events.get(i) {
          evt.notify(usize::MAX);
        }
      }
    }
  ...
  /// 采用 spin 16 次 -> yield 48 次 -> 事件 listener 阻塞等待超时阶梯退避，杜绝忙循环空转。
  pub fn wait_for_commit(&self, sublog_idx: usize, address: i64) {
    ...
    let event = self.commit_events.get(sublog_idx);
  ...
  pub async fn wait_for_commit_all_async(&self, until_address: i64) {
    if self.using_single_physical_log {
      ...
      for i in 0..self.physical_sublog_count {
        self.wait_for_commit_async(i, until_address).await;
      }
    }
  }
  ...
  /// 异步背压等待（按键哈希映射到子日志，通过无锁事件异步挂起，不阻塞 OS 线程）
  #[inline]
  pub async fn backpressure_wait_key_async(&self, key: &[u8]) {
    let Some(backpressure) = &self.backpressure else {
      return;
    };
    let sublog_idx = self.get_physical_sublog_idx(Self::hash(key));
  ...
  /// 异步背压等待（直接使用键哈希）
  #[inline]
  pub async fn backpressure_wait_key_hash_async(&self, key_hash: i64) {
    let Some(backpressure) = &self.backpressure else {
      return;
    };
    let sublog_idx = self.get_physical_sublog_idx(key_hash);
  ...
  fn enqueue_with_header(&self, record: &RecordShape<'_>) -> i64 {
    ...
    let address = self.get_sub_log(physical_sublog_idx).enqueue_parts(&parts);
    if self.auto_commit {
      self.commit();
    }
    address
  ...
  pub fn enqueue_span_chunked(&self, chunk: &ChunkedShape<'_>) -> i64 {
    ...
    parts.push(key);
    let mut address = self.get_sub_log(physical_sublog_idx).enqueue_parts(&parts);
...
///（物理判别值单源 [`wbase::entry_type::AofEntryType`]，在 garnet 中的相对路径:
...
mod tests {
  ...
  use waof::{AofAddress, SequenceNumberGenerator};
  ...
  #[test]
  fn test_backpressure_wait_async_entries() {
    let options = RuntimeServerOptions {
    ...
    let backends: Vec<Arc<Sublog>> = (0..2)
      .map(|_| Arc::new(Sublog::Mem(InMemorySublog::new())))
      .collect();
    let seq_num_gen = Some(Arc::new(SequenceNumberGenerator::new(0)));
    let log = GarnetLog::new(&options, backends, seq_num_gen);

    Runtime::new().unwrap().block_on(async {
      log.backpressure_wait_key_async(b"foo").await;
      log.backpressure_wait_key_hash_async(12345).await;
