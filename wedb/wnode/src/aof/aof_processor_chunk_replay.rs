...
pub async fn replay_chunk<D: Device>(
  ...
  match acc.op_type {
    AofEntryType::StoreUpsert => {
      AofProcessor::store_upsert(target.session, tag, key, acc.value_span()).await
    }
    AofEntryType::StoreRMW => {
      processor
        .store_rmw(target.session, key, acc.input_span())
        .await
    }
    AofEntryType::StoreDelete => AofProcessor::store_delete(target.session, tag, key).await,
    AofEntryType::ObjectStoreUpsert => {
      let value = acc.object_value_bytes();
      object_store_upsert(target.session, key, &value).await
    }
    AofEntryType::ObjectStoreRMW => object_store_rmw(target.session, key, acc.input_span()).await,
    AofEntryType::ObjectStoreDelete => object_store_delete(target.session, key).await,
    AofEntryType::UnifiedStoreStringUpsert => {
      unified_store_string_upsert(target.session, key, acc.value_span()).await
    }
    ...
    AofEntryType::UnifiedStoreDelete => AofProcessor::store_delete(target.session, tag, key).await,
