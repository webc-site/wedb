...
use wcol::{
  HashObject, IGarnetObject, ListObject, ObjectOutput, SetObject, SortedSetObject,
...
///
/// C# UnifiedStoreObjectUpsert（统一存上下文对象覆载）在 wkv 单一存储面下
/// 与对象存 upsert 同形，统一存重放臂（replay_op / replay_chunk 的
/// UnifiedStoreObjectUpsert 支）直调本口
...
/// objectContext 多态的对象应用段，rust 侧以 [`IGarnetObject`] 静态分发：
/// 信封域现载荷 → [`GarnetObjectPayload::from_blob`] → [`IGarnetObject::operate`] →
...
async fn replay_object_channel<T: GarnetObjectPayload + IGarnetObject, D: Device>(
  session: &StorageSession<'_, D>,
  key: &[u8],
  sub_id: u8,
  args: &[&[u8]],
  ...
  //（trait operate 的 u16 臂内部按 sub_id as u8 收窄进固有通道）
  obj.operate(
    u16::from(sub_id),
    args,
    arg1,
  ...
  if GarnetObjectPayload::is_empty(&obj) {
    session
      .delete_string(key)
      .await
      .map_err(AofReplayError::Store)?;
