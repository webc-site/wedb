...
impl<D: Device> DatabaseManagerBase<D> {
  ...
  /// 收口入口或 AOF 限长任务的显式对进入）；wcpr 进程闸 `lock_ckpt_gate` 只串行
  /// 第 4 步 `create_checkpoint_with_token` 内部快照段，罩不住其外的版本推进
  /// （第 2 步）、移位标记入账（第 2 步与尾前步）、AOF 截断（第 5 步）与回收
  /// （第 7 步）——两检查点的这些段互斥由上层暂停闸唯一保证。
