...
impl LoggingBuilder {
  ...
  /// loggerFactory 段：控制台（DisableConsoleLogger 未设）+ 可选落文件
  /// （serverSettings.FileLogger，刷盘间隔取 C# 省略形参的
  /// [`DEFAULT_LOG_FLUSH_INTERVAL`]）+ 最低级别（serverSettings.LogLevel））。
  ///
  /// rust 侧无 serverSettings.DisableConsoleLogger 配置面，控制台恒开。
  ...
  /// 不存在绕过它的第二套安装口。
  pub fn flush_into(self, forward: &MemoryForwardLogger) {
...
mod tests {
  ...
  fn builder_from_node_projects_logger_args() {
    ...
    // 未配置 file_logger：仅控制台目标，级别缺省 Information（C# 默认）
    let logging = LoggingBuilder::from_node(&NodeArgs::default());
    assert!(logging.files.is_empty());
    assert_eq!(logging.minimum_level, LevelFilter::Info);
  }
