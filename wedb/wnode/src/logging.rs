...
impl LogFormatter {
  ...
  pub fn format_time(time: Timestamp) -> String {
    format!("{}.{:04}", time.strftime("%H:%M:%S"), fraction_fourths(time))
  }
}
...
fn format_record(level: Level, category: &str, args: &std::fmt::Arguments<'_>) -> String {
  format!(
    "[{}] ({}) <{category}> {args}",
...
impl FileLoggerOutput {
  ...
  /// 实现同样未消费该参数，逐行同步刷盘）。
  pub fn new(filename: impl AsRef<Path>, flush_interval: i32) -> std::io::Result<Self> {
...
impl FileLoggerProvider {
  ...
  pub fn log_record(&self, level: Level, category: &str, args: &std::fmt::Arguments<'_>) {
    self.logger_output.write_line(&format_record(level, category, args));
  }
}
...
pub struct MemoryLogger {
  ...
  memory_log: Mutex<Vec<(Level, String)>>,
}

impl MemoryLogger {
  /// C# MemoryLogger.FlushLogger(ILogger dstLogger)
  pub fn flush_logger(&self, dst_logger: &dyn Log) {
...
pub struct MemoryForwardLogger {
  memory: MemoryLogger,
  destination: RwLock<Option<Arc<dyn Log>>>,
}
...
impl MemoryForwardLogger {
  ...
  pub fn promote(&self, destination: Arc<dyn Log>) {
    *self.destination.write() = Some(Arc::clone(&destination));
    self.memory.flush_logger(destination.as_ref());
  }
...
impl LoggingBuilder {
  ...
  pub fn install(self) -> Result<(), log::SetLoggerError> {
    let destinations: Vec<Arc<dyn Log>> = (!self.disable_console)
      .then(|| Arc::new(ConsoleLogger { minimum_level: self.minimum_level }) as Arc<dyn Log>)
      .into_iter()
      .chain(self.files.into_iter().filter_map(|(path, interval)| {
        // 文件打开失败（目录缺失 / 权限）按 C# 异常装配面收敛为跳过该目标
        FileLoggerOutput::new(&path, interval)
          .ok()
          .map(|output| Arc::new(FileLoggerProvider::new(Arc::new(output))) as Arc<dyn Log>)
      }))
      .collect();
...
mod tests {
  ...
  fn file_output_appends_and_formats() {
    let dir = std::env::temp_dir().join(format!("wnode-log-test-{}", std::process::id()));
  ...
  fn forward_logger_buffers_until_promoted() {
    ...
    let dst = Arc::new(MemoryLogger::default());
    forward.promote(Arc::clone(&dst) as Arc<dyn Log>);
    assert!(forward.promoted());
