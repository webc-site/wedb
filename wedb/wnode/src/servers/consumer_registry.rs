...
impl ConsumerEntry {
  ...
  ///
  /// 消费点：[`ConsumerRegistry::monitor_sample`] 逐会话采样与
  /// [`ConsumerRegistry::monitor_iteration_inputs`] 全量/单类延迟复位两臂
...
pub struct ConsumerRegistry {
  ...
  /// 与 entries.len()（枚举在途量）同宿主单点，承接 accept 容量门；无生产
  /// 读取面——排空以 [`ConsumerRegistry::active_consumers`] 空判定承接
  ///（C# DisposeActiveHandlers 轮询字段的对偶），观测仅测试域字段直读
  ...
  pub fn monitor_sample(&self) -> ServerSample {
    ...
    let sessions: Vec<_> = pin
      ...
      .map(|entry| {
        ...
        // ActiveConsumers 直读 GetSessionMetrics 全量的承接）；复位敏感的
        // 网络字节与命令数三计数仍读条目镜像（INFO RESET STATS 经
        // reset_session_stats 即时清零立即生效，句柄复位滞后一拍）
