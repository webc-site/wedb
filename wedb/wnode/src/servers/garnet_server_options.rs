...
pub use super::server_options::MIN_PAGE_SIZE_BYTES;
pub use crate::{
...
impl GarnetServerOptions {
  ...
  pub fn checkpoint_base_directory(&self) -> String {
    self.checkpoint_base_directory_str().to_string()
  }
  ...
  /// AOF 基目录借用切片（零分配）。
  #[inline]
  pub fn append_only_file_base_directory_str(&self) -> &str {
    self.checkpoint_dir.as_deref().unwrap_or_default()
  }

  /// AOF 基目录（对齐 AppendOnlyFileBaseDirectory：CheckpointDir ?? ""）。
  #[inline]
  pub fn append_only_file_base_directory(&self) -> String {
    self.append_only_file_base_directory_str().to_string()
  }
  ...
  pub fn get_append_only_file_directory(&self, db_id: i32) -> PathBuf {
    Path::new(self.append_only_file_base_directory_str())
  ...
  pub fn get_aof_settings(&self, db_id: i32) -> Result<Vec<AofLogSettings>, OptionsError> {
    ...
    let mut log_buf = itoa::Buffer::new();
    Ok(
      (0..sublogs)
        .map(|i| {
          let log_file = if sublogs == 1 {
            ...
            let mut s = String::with_capacity(4 + 11 + 4);
            s.push_str("aof.");
            s.push_str(log_buf.format(i));
            s.push_str(".log");
            s
          };
          AofLogSettings {
