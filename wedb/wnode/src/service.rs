  ...
  match event {
    ...
    StoreEvent::TieredCollectionWrite(notif) => {
      ...
      enqueue_slices(
        ...
        AofEntryType::ObjectStoreRMW,
        ver,
        &physical_key(notif.ns, notif.db, KeyTag::Meta, notif.key),
        &[],
        &input,
      )?;
    }
...
fn node_components(
  ...
  let vector_callbacks = Callbacks::new(Arc::new(WedbVectorStoreCallbacks::new(Arc::clone(
    &vector_session,
  ))));
  ...
  // 登记旁路记录写透钩子（同一向量存储会话；登记表与上下文元数据随检查点
  // 持久，重启恢复回建——C# 索引/元数据记录驻主存的持久语义承接）
  vector_manager.attach_registry_store(Arc::new(WedbRegistryPersistence::new(vector_session)));
...
pub fn open_node_with_config(
  ...
  let (broker, vector_manager) = node_components(&store, false)?;
  ...
  F: Fn(u64, StoreGarnetApi<SegmentedDevice>) -> Option<RespSessionConsumer>,
{
  ...
  fn from_parts(
    ...
    vector_manager.set_watch_bump(version_map_watch_hook(Arc::clone(&watch_version_map)));
  ...
  pub async fn open_recovered_with_config(
    ...
    let (broker, vector_manager) = node_components(&store, vector_preview)?;
  ...
  pub async fn open_recovered_with_config_and_aof(
    ...
    // 重放落盘）；预览开关构造期定值（C# IsEnabled 对位，恢复回建同真值）
    let (broker, vector_manager) = node_components(&store, vector_preview)?;
    let aof = Arc::clone(node.aof());
