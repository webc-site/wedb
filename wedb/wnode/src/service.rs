...
pub type EngineSwapHook = Arc<dyn Fn(&SharedStore<SegmentedDevice>) + Send + Sync>;
...
/// 弱引用捕获，宿主释放后自然退出。任务体经 wbase [`supervise_task`] 顶层监督
///（单点一次成型）：panic 臂落 log::error + 监督快照计数后复位 `started`，
/// 由 CONFIG SET / 角色恢复重拉通路自然复活。
pub fn spawn_aof_size_limit_task(
  ...
  spawn(async move {
    if supervise_task(AOF_SIZE_LIMIT_TASK, async move {
      loop {
...
pub fn spawn_index_auto_grow_task(
  ...
  drop(database_manager);
  spawn(async move {
    if supervise_task(INDEX_AUTO_GROW_TASK, async move {
      loop {
...
fn node_components(
  ...
  vector_manager.attach_registry_store(Arc::new(WedbRegistryPersistence::<SegmentedDevice>::new()));
...
fn checkpoint_dir_of(base_dir: &Path, data_path: &Path) -> PathBuf {
  ...
  F: Fn(u64, StoreGarnetApi<SegmentedDevice>) -> Option<RespSessionConsumer>,
{
  ...
  pub fn open_with_config_and_aof(
    ...
    // 先于下方 GarnetDatabase/SingleDatabaseManager 消费链覆写嵌入式回落值
    provider.checkpoint_dir = checkpoint_dir_of(
      Path::new(&options.checkpoint_base_directory),
      data_path.as_ref(),
    );
    let aof_settings = AofSettings::from_options(&options)?;
    let (_, wal) = open_wal(data_path.as_ref(), wal_dir, &aof_settings)?;
    // 写监听端口注册 + 服务级会话（对标 C# EnableAOF 构造段）；调用方
    // options 全量透传门面，不再经合成 NodeArgs 二次投影
    let node = NodeService::with_options(&options, Arc::clone(&provider.store), Arc::clone(&wal))
    ...
    // 停机收口角色分派同源注入（dispose_async 副本纯刷盘不写本地帧）
    aof.attach_primary_tasks(Arc::clone(&provider.primary_tasks));
    // FLUSH 族登记表域回收联动注入
    provider
      .database_manager
    ...
    provider = provider.with_runtime_server_options(options);
    Ok(provider)
  }
  ...
  /// RecoverCheckpointAsync → RecoverAOFAsync → ReplayAOF；StoreWrapper 层
  ...
  pub async fn open_from_args_with_config(
    ...
    let provider = match (node.recover, node.aof) {
      ...
      (false, false) => {
        ...
        // checkpoint_base_dir() 投影的 runtime_server_options 同源）
        let mut provider = Self::open_with_config(config, data_path, decorate)?;
        provider.checkpoint_dir = checkpoint_dir_of(&node.checkpoint_base_dir(), data_path);
        provider
      }
    }
  ...
  pub fn with_quantization_task_count(self, raw: usize) -> Self {
  ...
  /// 在同一 database/store 对象上重建），functionsState.watchVersionMap 与
