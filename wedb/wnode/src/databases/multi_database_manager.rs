...
use crate::{aof::GarnetAppendOnlyFile, storage::functions::functions_state::FunctionsState};
...
pub struct MultiDatabaseManager<D: Device> {
  ...
  aof_factory: Mutex<Option<Arc<GarnetAppendOnlyFile>>>,
...
impl<D: Device> MultiDatabaseManager<D> {
  ...
  pub fn enable_aof(&self, aof: Arc<GarnetAppendOnlyFile>) {
  ...
  /// 新库 AOF 域句柄（全库共享同一实例）
  fn create_aof(&self) -> Option<Arc<GarnetAppendOnlyFile>> {
  ...
  async fn collect_db_kv(&self, db_id: i64) -> Vec<(Vec<u8>, Vec<u8>)> {
    if self.get_db_by_id(db_id).is_none() {
      return Vec::new();
    }
    let prefix = wval::SessionPrefixBuf::new(0, db_id.max(0) as u64);
      ...
      .scan(
        ...
        |_addr, rec| {
          ...
          if let Some(rest) = key.strip_prefix(prefix_slice)
            && !rec.is_tombstone()
            && rest.len() > 1
            && rest[0] == wval::KeyTag::String.as_u8()
          {
            map.insert(rest[1..].to_vec(), rec.value().to_vec());
          }
          Ok(true)
        },
      )
  ...
  async fn delete_db_keys(&self, db_id: i64) {
    if let Some(db) = self.get_db_by_id(db_id)
      && let Err(err) = self.base.reset_database(&db).await
    {
      log::warn!("清库删除键失败: {err}");
    }
  }
  ...
  /// 打开（或复用）检查点目录下已存在的库存储（保留 _db_id 与 _checkpoint_path 参数对标 C# 签名）
  pub fn open_database_store(
    &self,
    _db_id: i64,
    _checkpoint_path: &Path,
  ) -> wkv::Result<Arc<WedbStore<D>>> {
...
impl<D: Device> IDatabaseManager<D> for MultiDatabaseManager<D> {
  ...
  fn wait_for_commit_to_aof_async(&self, db_id: i64) -> wkv::Result<bool> {
    ...
    let ok = pin
      .iter()
      .filter(|(id, _)| db_id < 0 || **id == db_id)
      .all(|(_, db)| db.aof.as_ref().is_none_or(|aof| aof.wait_for_commit()));
    Ok(ok)
  }
