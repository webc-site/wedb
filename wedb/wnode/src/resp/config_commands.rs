...
use wresp::{
  ...
  cmd_strings::{
    ...
    GENERIC_ERR_INCORRECT_SIZE_FORMAT, GENERIC_ERR_INDEX_SIZE_AUTO_GROW,
    GENERIC_ERR_INDEX_SIZE_GROW_FAILED, GENERIC_ERR_INDEX_SIZE_POWER_OF_TWO,
    GENERIC_ERR_INDEX_SIZE_SMALLER_THAN_CURRENT,
...
/// libs/common/AsciiUtils.cs:EqualsLowerCaseSpanIgnoringCase
///
/// `lower` 须为全小写常量：先整串全等短路；否则逐位比较——常量位小写字母时
/// 键位须等值或为其大写形（+32 折叠），常量位非字母时按 `allow_non_alphabetic`
/// 分口径：false 恒拒（readcache-memory 等含 '-' 常量仅剩精确小写全等一形命中）、
/// true 须键位等值
fn eq_lower_ignore_case(key: &[u8], lower: &[u8], allow_non_alphabetic: bool) -> bool {
  if key == lower {
    return true;
  }
  if key.len() != lower.len() {
    return false;
  }
  key.iter().zip(lower).all(|(&k, &c)| {
    if c.is_ascii_lowercase() {
      k == c || k == c - 32
    } else {
      allow_non_alphabetic && k == c
    }
  })
}

/// CONFIG SET 的七个专用槽键归类（对标 C# 常量键大小写不敏感命中臂）
#[derive(Clone, Copy)]
enum SpecialKey {
  MainLogMemory,
  ReadCacheMemory,
...
/// CONFIG SET 特殊键别名静态表：归类标签 + C# 常量名 + `allowNonAlphabeticChars`
/// 口径（memory/index 纯字母恒等；readcache 含 '-' 仅剩精确小写全等命中传 false，
/// cert-/cluster- 四臂传 true）。编译期常量数组，各名互异至多一形命中。
const CONFIG_SET_ALIASES: &[(SpecialKey, &[u8], bool)] = &[
  (SpecialKey::MainLogMemory, MAIN_LOG_MEMORY, false),
  (SpecialKey::ReadCacheMemory, READ_CACHE_MEMORY, false),
  (SpecialKey::Index, INDEX, false),
  (SpecialKey::CertFileName, CERT_FILE_NAME, true),
  (SpecialKey::CertPassword, CERT_PASSWORD, true),
  (SpecialKey::ClusterUsername, CLUSTER_USERNAME, true),
  (SpecialKey::ClusterPassword, CLUSTER_PASSWORD, true),
];

/// 参数键 → 特殊键归类（原七路 `eq_lower_ignore_case` else-if 链的单点收敛）
fn classify_alias(key: &[u8]) -> Option<SpecialKey> {
  CONFIG_SET_ALIASES
    .iter()
    .find_map(|&(sk, lower, allow)| eq_lower_ignore_case(key, lower, allow).then_some(sk))
}
...
pub struct ConfigSetHost<'a> {
  ...
  /// .AdjustedIndexMaxCacheLines > 0 的会话侧可达面：`--index-max-size` 配置
  /// 即常驻拉起 IndexAutoGrowTask，CONFIG SET index 须据此短路拒绝，杜绝人工
  /// 扩容与自动扩容并发碰撞 grow_index_blocking 双写竞态）
  pub index_auto_grow_active: bool,
  /// TLS 证书热加载共享句柄（C# storeWrapper.serverOptions.TlsOptions 的
  /// 会话侧可达面；None = 未装配 TLS，证书对按 "ERR TLS is disabled." 拒绝）
  #[cfg(feature = "tls")]
  pub tls_config: Option<&'a ServerTlsConfig>,
}
...
impl ServerConfig {
  ...
  pub fn get_config(parameter: &[u8]) -> ServerConfigType {
    // C# 原地大写后比较；此处直接大小写不敏感等价（星号无大小写差异）
    if parameter == b"*" {
...
impl ServerConfig {
    ...
    D: Device + 'static,
  {
    ...
    // C# certFileName/certPassword 捕获 Encoding.ASCII.GetString 的值，
    // 键重复出项时末次覆盖（else-if 链逐对覆写同式）
    let mut cert_file_name: Option<String> = None;
    let mut cert_password: Option<String> = None;
    ...
    while c < parse_state.len() {
      ...
      // 特殊键单点归类（大小写折叠别名静态表）；未命中回落运行时配置表 /
      // 未知键（C# unknownOption 首未知键短路同式）
      match classify_alias(key) {
        Some(SpecialKey::MainLogMemory) => main_log_memory_size = Some(value.to_vec()),
        Some(SpecialKey::ReadCacheMemory) => read_cache_memory_size = Some(value.to_vec()),
        Some(SpecialKey::Index) => index = Some(value.to_vec()),
        Some(SpecialKey::CertFileName) => cert_file_name = Some(string_lossy(value)),
        Some(SpecialKey::CertPassword) => cert_password = Some(string_lossy(value)),
        Some(SpecialKey::ClusterUsername) => cluster_username = Some(string_lossy(value)),
        Some(SpecialKey::ClusterPassword) => cluster_password = Some(string_lossy(value)),
        None => {
          if let Some(config_type) = RuntimeServerConfig::try_get_type(key) {
    ...
    if unknown_option {
      ...
      // TLS 证书在线重载（C# ServerConfig.cs:173-182）：TlsOptions 在场则
      // UpdateCertFile 实时重载（certErrorMessage 原样 AppendError 累积，
      // 成功回 +OK），缺席 append "ERR TLS is disabled."
      #[cfg(feature = "tls")]
      if cert_file_name.is_some() || cert_password.is_some() {
        match host.tls_config {
          Some(tls) => {
            let cert_path = cert_file_name.as_deref().map(Path::new);
            let key_path = cert_password.as_deref().map(Path::new);
            if let Err(err) = tls.update_cert_file(cert_path, key_path) {
              error_msg.append(&err.to_string());
            }
          }
          None => error_msg.append("ERR TLS is disabled."),
        }
      }
      // 无 tls 特性构建：证书对无条件拒绝（GarnetTlsOptions 不存在 =
      // C# TlsOptions == null 同判）
      #[cfg(not(feature = "tls"))]
      if cert_file_name.is_some() || cert_password.is_some() {
      ...
      if let Some(index_size) = &index {
        self.handle_index_size_change(
          index_size,
          store,
          host.index_auto_grow_active,
          &mut error_msg,
        );
      }
  ...
  fn handle_memory_size_change(
    ...
    // 读/主日志两臂仅错误模板与选项名成对差异，单点取模板后一次写出
    let (template, option) = if is_read_cache {
      (
    ...
    error_msg.append_with_template(template, option);
  }
  ...
  /// 2 的幂校验后、读当前桶数前置 auto-grow 运行门（C# ServerConfig.cs:280
  /// `AdjustedIndexMaxCacheLines > 0` → GenericErrIndexSizeAutoGrow）：`--index-max-size`
  /// 配置即常驻拉起 IndexAutoGrowTask（service.rs spawn_index_auto_grow_task，
  /// grow_index_blocking 逐轮翻倍），人工 CONFIG SET index 与之并发同入
  /// grow_index_blocking 即双写竞态、破坏扩容单调性，故 auto-grow 在跑时短路拒绝。
  fn handle_index_size_change<D: Device + 'static>(
    &mut self,
    index_size: &[u8],
    store: Option<&Arc<WedbStore<D>>>,
    index_auto_grow_active: bool,
    error_msg: &mut ErrorMsgBuilder,
  ) {
    ...
    // C# 同序：auto-grow 运行门先于读当前索引尺寸（ServerConfig.cs:279-284），
    // 无论目标尺寸增减，自动扩容在跑一律拒人工改
    if index_auto_grow_active {
      error_msg.append_with_template(GENERIC_ERR_INDEX_SIZE_AUTO_GROW, INDEX);
      return;
    }
