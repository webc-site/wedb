...
use crate::config_owner::apply_config_reconcile;
    ...
    D: Device + 'static,
  {
    ...
    if unknown_option {
      ...
      for (config_type, value) in dynamic_sets {
        match runtime_config.try_set(config_type, &string_lossy(&value)) {
...
mod tests {
  ...
  fn config_set_validation_and_disabled_subsystems() {
    ...
    // 奇数参数
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"a"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    // 未知键 → 首例进错误
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"maxmemory", b"1g"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    // TLS 禁用
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"cert-file-name", b"a.pem"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"-ERR TLS is disabled.\r\n");

    // 集群禁用
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"cluster-password", b"p"], None, &rc, &mut out)
      .unwrap();
    ...
    // 尺寸格式非法
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"memory", b"abc"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"-ERR Incorrect size format in (option: 'memory')\r\n");

    // tracker 未运行（含格式合法场景）
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"memory", b"1g"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"READCACHE-MEMORY", b"1g"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    // index 非 2 的幂
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"index", b"1000"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
    ...
    // index 2 的幂 → 增长通道缺席错误
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"index", b"1024"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
  ...
  fn config_set_dynamic_runtime_values() {
    ...
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(
      &[b"compaction-force-delete", b"yes"],
      None,
      &rc,
      &mut out,
    )
    ...
    // 枚举值
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"compaction-type", b"Lookup"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(out, b"+OK\r\n");
    ...
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(
      &[b"compaction-force-delete", b"maybe"],
      None,
      &rc,
      &mut out,
    )
    ...
    // 只读参数拒绝
    let mut out = Vec::new();
    s.network_config_set::<SegmentedDevice>(&[b"databases", b"8"], None, &rc, &mut out)
      .unwrap();
    assert_eq!(
      out,
