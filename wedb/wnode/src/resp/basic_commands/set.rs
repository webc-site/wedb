...
use super::ttl::expiry_ticks_from_now;
use crate::{
...
impl RespServerSession {
  ...
  /// SETRANGE 分支 "not changing the presence of ETag or Expiration"）
  ...
  pub fn network_set_conditional<'a, D: wdev::Device>(
    ...
    // 双域读判定对象键 WRONGTYPE
    let old = match read_user_sync(store, key, |v| v.to_vec()) {
  ...
  ///
  /// RMW 语义写回（保留既有 key 级 TTL，对标 C# MainStore/RMWMethods.cs
  /// APPEND 分支 "not changing the presence of ETag or Expiration"）
