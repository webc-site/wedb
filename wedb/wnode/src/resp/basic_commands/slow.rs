    ...
    C::Getex => {
        ...
        .map_err(|_| ())?
      {
        UserReadAsync::Hit(val) => {
          ...
          match expiry {
            GetexExpiry::None => {}
            // PERSIST：移除键级 TTL（persist_key 包装单点：applied > 0 推进
            // WATCH 版本，键已确认存活；与 EXPIRE/PERSIST 慢路径及 AOF 重放
            // 共用同一推进尾巴，裸调 batch.persist 缺席推进禁再直调）
            GetexExpiry::Persist => {
              storage.persist_key(key).await.map_err(|_| ())?;
            }
            // 绝对过期刻度（expire_at_ticks 包装单点：applied > 0 推进 WATCH
            // 版本；过去时间戳立即删除与 wkv 判定表同源，头部 16 对齐粗化
            // 幂等恒等）
            GetexExpiry::At(ticks) => {
              storage.expire_at_ticks(key, ticks).await.map_err(|_| ())?;
            }
          }
