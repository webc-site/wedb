    ...
    if param.eq_ignore_ascii_case(b"MATCH") {
      ...
      filter.type_filter = match args[token_idx] {
        t if t.eq_ignore_ascii_case(b"zset") => {
          Some(ScanTypeFilter::Object(wval::GarnetObjectType::SortedSet))
        }
        t if t.eq_ignore_ascii_case(b"list") => {
          Some(ScanTypeFilter::Object(wval::GarnetObjectType::List))
        }
        t if t.eq_ignore_ascii_case(b"set") => {
          Some(ScanTypeFilter::Object(wval::GarnetObjectType::Set))
        }
        t if t.eq_ignore_ascii_case(b"hash") => {
          Some(ScanTypeFilter::Object(wval::GarnetObjectType::Hash))
        }
