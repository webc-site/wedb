...
impl RespServerSession {
  ...
  fn do_network_mget<P: RespProtocol, D: wdev::Device>(
    ...
    for key in parse_state {
      ...
      // 对非字符串键同答 nil，不报错）
      match read_user_sync_with_prefix(store, prefix_slice, key, |v| writer.write_bulk_string(v)) {
        Ok(UserRead::Hit(())) => {}
        Ok(UserRead::WrongType | UserRead::Missing) => writer.write_null(),
  ...
  pub fn network_mset<'a, D: wdev::Device>(
    ...
    match store.try_upsert_batch_sync(pairs) {
      Ok(Ok(())) => {}
      Ok(Err(_)) => return Ok(false),
      Err(_) => {
        output.write_resp_error(RESP_ERR_GENERIC);
  ...
  /// 集群门禁按 doc/zh/db.md SWAPDB 条款改库槽位归属判定（偏离 C# 一刀切，
  /// 槽位真值源 `Slot = Mixer(namespace, db)` 单点 wbase::hash_slot::slot_of）：
  /// 校验序相应调整为 arity → 两个整数 → 上界 → 归属门禁 → 同库短路——
  /// 归属判定需库号参与，门槛必然后置于下标解析。两库槽位均由本地节点掌管
  /// 才放行（同库交换亦须本节点持有该槽），分属不同节点或本节点不持有即拦截
  /// 回 RESP_ERR_GENERIC_SWAPDB_CLUSTER_MODE。真实换库为 wkv O(1) 虚库 ID
  ...
  pub fn network_swapdb(
    ...
    // 当前本地节点掌管，分属不同物理节点才拦截）
    if let Some(provider) = self.cluster_provider.as_ref()
      && provider.is_cluster_enabled()
    {
      let ns = self.namespace;
      if !(provider.is_slot_local(slot_of(ns, idx1)) && provider.is_slot_local(slot_of(ns, idx2))) {
  ...
  pub fn network_lcs<'a, D: wdev::Device>(
    ...
    // 报 WRONGTYPE
    let read_string_val = |key| match read_user_sync(store, key, |v| v.to_vec()) {
      Ok(UserRead::Hit(v)) => Ok(Some(v)),
      Ok(UserRead::WrongType) => Err(true),
      Ok(UserRead::Missing) | Err(_) => Ok(None),
      Ok(UserRead::Deferred) => Err(false),
    };
...
pub(crate) mod slow {
  ...
  async fn mget_inner<P: RespProtocol>(
    ...
    for key in refs {
      // String 域读：命中写值；缺失（含对象键与 TTL 过期键）写 nil
      //（Redis MGET 对非字符串键同答 nil 不报错，对位 C# 主存单域读）
      match storage.read_string(key).await {
        Ok(Some(v)) => writer.write_bulk_string(&v),
  ...
  pub(crate) async fn mset(
    ...
    for [key, val] in refs.as_chunks::<2>().0 {
      // SET 语义自动清新键残留 TTL（与快路径 try_upsert_sync 同口径）
