...
use itoa::{self, Buffer, Buffer as ItoaBuffer};
use wbase::{
  convert::{TICKS_PER_MILLISECOND, TICKS_PER_SECOND, UNIX_EPOCH_TICKS},
  num::{strict_f64, strict_i32, strict_i64},
  time::now_ticks,
};
use wcol::object_store_utils::object_heap_estimate;
use wresp::{
  RespSliceExt, RespVecExt, check_arg_count, cmd_strings as cs,
  cmd_strings::{
    RESP_ERR_GENERIC, RESP_ERR_WRONG_TYPE, abort_with_error_message, abort_with_unknown_subcommand,
    abort_with_unsupported_option, write_raw,
  },
  key_spec::KeySpecificationFlags,
  sanitize_error_str, unpack_args,
};
use wval::{GarnetObjectType, KeyTag};
use zmij::{self, Buffer as ZmijBuffer};

use super::{config_commands::ServerConfig, resp_server_session::RespServerSession};
use crate::{
  ...
  storage::session::common::ttl_sync::{
    del_ttl_sync, probe_alive, put_ttl_sync, read_adjudicated_envelope_sync,
    read_adjudicated_tag_with_size, read_adjudicated_user_sync, ttl_of_sync,
  },
};

/// 读取数值型键时的解析错误分流（WRONGTYPE 判定已由双域读 helper 承担）
...
impl RespServerSession {
  ...
  pub fn network_get<'a, D: wdev::Device>(
    ...
    match read_adjudicated_user_sync(store, key, |v| {
  ...
  pub fn network_set<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "SET", [key, value]);
  ...
  pub fn network_getset<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "GETSET", [key, val]);
  ...
  pub fn network_set_range<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "SETRANGE", [key, offset_raw, val]);
    ...
    match read_adjudicated_user_sync(store, key, |v| {
      ...
      Ok(Some(Some(Ok(existing)))) => match store.try_upsert_sync(key, &existing) {
  ...
  pub fn network_get_range<'a, D: wdev::Device>(
    ...
    let Some(mut end) = strict_i32(end_raw).map(i64::from) else {
      abort_with_error_message(output, ERR_NOT_INTEGER);
      return Ok(true);
    };

    match read_adjudicated_user_sync(store, key, |val| {
  ...
  /// libs/server/Resp/BasicCommands.cs:NetworkPSETEX
  ///
  ...
  fn network_setex_impl<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, cmd_name, [key, expiry_raw, val]);
  ...
  pub fn network_setnx<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "SETNX", [key, val]);
  ...
  pub fn network_set_conditional<'a, D: wdev::Device>(
    ...
    // 双域读判定对象键 WRONGTYPE
    let old = match read_adjudicated_user_sync(store, key, |v| v.to_vec()) {
      Ok(Some(Some(Ok(found)))) => Some(found),
      Ok(Some(Some(Err(())))) => {
        output.write_resp_error(RESP_ERR_WRONG_TYPE);
        return Ok(true);
      }
  ...
  pub fn network_increment<'a, D: wdev::Device>(
    ...
    let (key, delta) = if cmd.has_by() {
      unpack_args!(parse_state, output, cmd.as_str(), [key, by_raw]);
      ...
      unpack_args!(parse_state, output, cmd.as_str(), [key]);
      (key, cmd.sign())
    };

    // 解析在读取闭包内完成：免整值堆分配（C# IsValidNumber 失败即 StringOutput 错误）
    let val = match read_adjudicated_user_sync(store, key, |v| {
      v.as_str_safe()
        .parse::<i64>()
        .map_err(|_| ValueReadError::InvalidFormat)
    }) {
      Ok(Some(Some(Ok(Ok(v))))) => v,
      // 值非整数 → not-integer
      Ok(Some(Some(Ok(Err(ValueReadError::InvalidFormat))))) => {
        abort_with_error_message(output, ERR_NOT_INTEGER);
        return Ok(true);
      }
      // 对象键（信封域命中）/ 不可达 WrongType 变体 → WRONGTYPE
      // 对象键（信封域命中）→ WRONGTYPE
      Ok(Some(Some(Err(())))) => {
        output.write_resp_error(RESP_ERR_WRONG_TYPE);
        return Ok(true);
  ...
  pub fn network_increment_by_float<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "INCRBYFLOAT", [key, incr_by_raw]);
    let Some(incr_by) = try_parse_double(incr_by_raw) else {
    ...
    let val = match read_adjudicated_user_sync(store, key, |v| {
      try_parse_double(v).ok_or(ValueReadError::InvalidFormat)
    }) {
      Ok(Some(Some(Ok(Ok(v))))) => v,
    ...
    let formatted = wresp::format_double(next, &mut buf);
  ...
  pub fn network_append<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "APPEND", [key, val]);

    match read_adjudicated_user_sync(store, key, |v| v.to_vec()) {
      Ok(Some(Some(Ok(mut existing)))) => {
  ...
  ///
  /// 集群切面缺席（standalone，C# EnableCluster == false）恒放行；内部写
  /// 会话（AOF 回放 / 复制推流 / 迁移）豁免——rust 回放经 aof_processor
  /// 内核直达存储域、不经 RESP 分派，本门仅拦客户端会话，置位语义与
  /// C# AofProcessor 回放会话保持一致
  fn flush_replica_read_only_gate(&self, output: &mut Vec<u8>) -> bool {
    let Some(cluster) = &self.cluster_session else {
      return false;
    };
    if cluster.is_replica() && !cluster.is_internal_write_session() {
  ...
  pub fn network_strlen<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "STRLEN", [key]);

    match read_adjudicated_user_sync(store, key, |v| v.len()) {
      Ok(Some(Some(Ok(len)))) => output.write_resp_int(len as i64),
      Ok(Some(Some(Err(())))) => {
        output.write_resp_error(RESP_ERR_WRONG_TYPE);
      }
      Ok(Some(None)) => output.write_resp_int(0),
      Ok(None) => return Ok(false),
      Err(_) => output.write_resp_error(RESP_ERR_GENERIC),
    }
    Ok(true)
  ...
  pub fn write_command_response(&mut self, output: &mut Vec<u8>) -> wresp::Result<bool> {
    if let Some(infos) = super::resp_commands_info::try_get_resp_commands_info(true) {
      if self.resp_protocol_version >= 3 {
        let mut writer = wresp::RespWriter::<_, wresp::Resp3>::new_ref_p(output);
        writer.write_array_length(infos.len());
        for info in infos.values() {
          info.to_resp_format(&mut writer);
        }
      } else {
        let mut writer = wresp::RespWriter::<_, wresp::Resp2>::new_ref(output);
        writer.write_array_length(infos.len());
        for info in infos.values() {
          info.to_resp_format(&mut writer);
        }
  ...
  pub fn network_command_count<'a, D: wdev::Device>(
    ...
    check_arg_count!(parse_state, empty, output, "COMMAND|COUNT");
  ...
  pub fn network_command_docs<'a, D: wdev::Device>(
    ...
    if self.resp_protocol_version >= 3 {
      Self::write_command_docs_p::<wresp::Resp3>(count, parse_state, output);
    } else {
      Self::write_command_docs_p::<wresp::Resp2>(count, parse_state, output);
    }
    Ok(true)
  }
  fn write_command_docs_p<P: wresp::RespProtocol>(
    count: usize,
    parse_state: &[&[u8]],
    output: &mut Vec<u8>,
  ) {
    let mut writer = wresp::RespWriter::<_, P>::new_ref_p(output);
    if count == 0 {
  ...
  pub fn network_command_info<'a, D: wdev::Device>(
    ...
    if self.resp_protocol_version >= 3 {
      Self::write_command_info_p::<wresp::Resp3>(count, parse_state, output);
    } else {
      Self::write_command_info_p::<wresp::Resp2>(count, parse_state, output);
    }
    Ok(true)
  }
  fn write_command_info_p<P: wresp::RespProtocol>(
    count: usize,
    parse_state: &[&[u8]],
    output: &mut Vec<u8>,
  ) {
  ...
  fn prepare_command_keys_context<'b>(
    ...
    check_arg_count!(parse_state, !empty, output, cmd_name_for_err, return None);
  ...
  pub fn network_command_getkeysandflags<'a, D: wdev::Device>(
    ...
    for (key, flags_byte) in pairs {
      ...
      output.write_resp_bulk_string(key);
      let flags = KeySpecificationFlags(flags_byte as u16);
  ...
  pub fn network_echo(
    ...
    unpack_args!(parse_state, output, "ECHO", [msg]);
    output.write_resp_bulk_string(msg);
    Ok(true)
  }
  ...
  ///
  /// CONFIG SET 分派承接（实现位于 wconf::ServerConfig，同上）
  pub fn network_config_set<D: wdev::Device + 'static>(
    ...
    let mut config_server = ServerConfig;
    config_server.network_config_set(parse_state, store, self.runtime_config(), output)
  }

  /// libs/server/Resp/AdminCommands.cs:NetworkCONFIG_REWRITE
  ///
  /// CONFIG REWRITE 分派承接（实现位于 wconf::ServerConfig，同上）
  ...
  pub fn network_memory_usage<'a, D: wdev::Device>(
    ...
    // heap_memory_size 记账（wcol 相对记账，单点见 wcol object_heap_estimate）
    ...
    match read_adjudicated_tag_with_size(store, key, KeyTag::ObjectEnvelope, |raw, size| {
      size as i64 + object_heap_estimate(raw)
    }) {
      Ok(Some(Some(total))) => {
        output.write_resp_int(total);
      }
      // 两域皆缺：缺键 nil（C# status != OK → WriteNull）
  ...
  pub fn network_object<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, sub_cmd.as_str(), [key]);
    ...
    // C# ReadMethods.cs:67 `_ => CmdStrings.hashtable`）；
    // 过期键与缺失键一致回 nil（$-1\r\n）
    let probe = read_adjudicated_user_sync(store, key, |v| v.len());
    let (encoding, degrade) = match probe {
      Ok(Some(Some(Ok(_)))) => (Some(ENCODING_RAW), false),
      Ok(Some(Some(Err(())))) => match read_adjudicated_envelope_sync(store, key, |raw| {
        raw
          .first()
          .copied()
        ...
        Ok(Some(Some(enc))) => (Some(enc), false),
        _ => (None, true),
      },
    ...
    if degrade {
      ...
      if matches!(ttl_of_sync(store, key), Ok(Some(Some(exp))) if exp < now_ticks()) {
  ...
  pub fn network_objecthelp<'a, D: wdev::Device>(
    ...
    check_arg_count!(parse_state, empty, output, "object|help");
  ...
  pub(crate) fn apply_async_param(
    ...
    unpack_args!(parse_state, output, "ASYNC", [param]);

    if param.eq_ignore_ascii_case(b"ON") {
      self.use_async = true;
    } else if param.eq_ignore_ascii_case(b"OFF") {
  ...
  pub fn process_hello_command<'a, D: wdev::Device>(
    ...
    // WRONGPASS（state 版本承接，认证失败已写错误应答）
  ...
  /// ShiftBeginAddress，rust 清库须逐键跨 await，经 SlowWait 闭环后回 +OK，
