...
impl RespServerSession {
  ...
  pub fn network_setnx<'a, D: wdev::Device>(
    ...
    // 存在性探测：读闭包丢弃值本体，免整值堆拷贝
    match store.try_read_sync(key, |_| ()) {
      Ok(Some(Some(()))) => output.write_resp_int(0),
  ...
  pub fn network_increment<'a, D: wdev::Device>(
    ...
    let val = match store.try_read_sync(key, |v| v.as_str_safe().parse::<i64>().map_err(|_| ())) {
      Ok(Some(Some(Ok(v)))) => v,
      Ok(Some(Some(Err(())))) => {
        abort_with_error_message(output, ERR_NOT_INTEGER);
        return Ok(true);
  ...
  pub fn network_increment_by_float<'a, D: wdev::Device>(
    ...
    let val = match store.try_read_sync(key, |v| try_parse_double(v).ok_or(())) {
      Ok(Some(Some(Ok(v)))) => v,
      Ok(Some(Some(Err(())))) => {
        abort_with_error_message(output, cs::RESP_ERR_NOT_VALID_FLOAT);
        return Ok(true);
  ...
  pub fn network_config_set(
    ...
    let mut config_server = ServerConfig;
    config_server.network_config_set(parse_state, self.runtime_config(), output)
  }
  ...
  pub fn process_hello_command<'a, D: wdev::Device>(
    ...
    self.process_hello_command_state(resp_protocol_version, username, client_name, output);
    Ok(true)
  }
