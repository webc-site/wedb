...
//! [`SlowWait::resolve`]（compio 挂起不占线程；与终止广播 select 竞速，
//! KILL/注销胜出即丢弃执行体断连收口——C# RespServerSession.Dispose 的
//! asyncWaiterCancel 语义）→ 应答字节按流水线顺序写回 → 继续消费。存储
//! 执行域命令经 [`SlowWait::for_command`] 构造（单次实现，一处定义，多命令
//! 复用，杜绝逐命令特设分支）；集群切面等其他注入方经 [`SlowWait::new`]
//! 携带各自的异步闭环。
...
/// 慢路径应答 future（类型擦除壳：单指针 + poll/drop 函数指针，承接跨
/// Send 边界的线程绑定执行域 future）
...
impl SlowWait {
  ...
  /// 句柄克隆保 Arc 存活（move 进 future 保 'static），future 借用的
  /// 执行域在 await 期间有效。`resp_version` 随调度点快照会话真实协议
  /// 版本，作版本源单点流入慢路径
  pub fn for_command(
    api: &GarnetApi,
    cmd: RespCommand,
    ...
    Self {
      fut: Some(Arc::clone(api).exec_slow(cmd, args, resp_version)),
    }
  }
