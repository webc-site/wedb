...
impl<S: StoreCallbacks> VectorManager<S> {
  ...
  /// 向量登记表变更的用户键 WATCH 版本推进单点（写漏斗内部消费；未装配
  /// 零开销旁路）。键口径为用户键裸字节，与 wtxn WATCH 登记同表同哈希。
  /// 族内可见（pub(crate)）：AOF 重放臂（vector_manager_replication）迁移
  /// 成功后同样经此推进，一处定义禁第二推进形态
  #[inline]
  pub(crate) fn bump_watch(&self, user_key: &[u8]) {
  ...
  #[inline]
  pub(crate) fn persist_registry_index(&self, rk: &[u8], bytes: &[u8; INDEX_SIZE_BYTES]) {
    if let Some(store) = self.registry_store.get() {
      store.put(index_registry_physical_key(rk).as_slice(), bytes);
    }
  }

  /// 索引登记摘除写透（`remove_stored_index` 单点消费；未注入零开销旁路）。
  #[inline]
  pub(crate) fn evict_registry_index(&self, rk: &[u8]) {
    if let Some(store) = self.registry_store.get() {
      store.remove(index_registry_physical_key(rk).as_slice());
    }
  }

  /// 上下文元数据落盘写透（`persist_context_metadata` 单点消费；未注入
  /// 零开销旁路）。
  #[inline]
  pub(crate) fn persist_registry_metadata(&self, index: i32, bytes: &[u8; CONTEXT_METADATA_SIZE]) {
    if let Some(store) = self.registry_store.get() {
      store.put(metadata_registry_physical_key(index).as_slice(), bytes);
    }
  }
...
impl<S: StoreCallbacks> VectorManager<S> {
  ...
  /// 口径，键口径 = 用户键裸字节）
  pub fn try_remove(&self, key: &[u8], index_value: &[u8], element: &[u8]) -> VectorManagerResult {
    self.assert_have_storage_session();
  ...
  pub fn rename_vector_set(&self, prefix: &[u8], old_key: &[u8], new_key: &[u8]) -> bool {
    ...
    let migrated = self.rename_vector_set_of(rk_old.as_slice(), rk_new.as_slice());
    if migrated {
  ...
  pub(crate) fn rename_vector_set_of(&self, rk_old: &[u8], rk_new: &[u8]) -> bool {
    ...
    // 对齐）
    self.bump_watch(registry_user_key(rk_old));
    true
  }
    ...
    for i in 0..metas.len() {
      ...
      for j in 0..64u64 {
          ...
          && !metas[i].is_cleaning_up(allow_zero, context_value)
        {
          ...
          self.dirty_context_metadatas.lock().insert(i);
          contexts_to_cleanup.push(context);
        }
      }
    }
