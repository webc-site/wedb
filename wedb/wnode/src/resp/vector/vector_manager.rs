...
use wvector::{
  ...
  prepare_vector_data, try_compile,
};
...
impl<S: StoreCallbacks> VectorManager<S> {
  ...
  pub fn request_drop_in_memory_index(&self, key: &[u8], value: &[u8]) {
    ...
    if index.index_ptr != 0 {
      if !self.requested_drops.try_add(key.to_vec(), index.context) {
...
impl<'a> Iterator for LengthPrefixedIter<'a> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    let len = i32::from_le_bytes(self.rest[..4].try_into().unwrap_or([0; 4])).max(0) as usize;
    let total = 4 + len;
    if self.rest.len() < total {
      return None;
    }
    let item = &self.rest[4..total];
    self.rest = &self.rest[total..];
    Some(item)
  }
}

/// i32 长度前缀流的切片迭代（零分配迭代器）。
#[inline]
pub fn unpack_length_prefixed_iter(bytes: &[u8]) -> LengthPrefixedIter<'_> {
  LengthPrefixedIter::new(bytes)
}

/// i32 长度前缀流的切片收集。
pub fn unpack_length_prefixed(bytes: &[u8]) -> Vec<&[u8]> {
  unpack_length_prefixed_iter(bytes).collect()
}
