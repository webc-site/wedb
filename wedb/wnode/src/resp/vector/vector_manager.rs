...
use super::{
  ...
  vector_manager_locking::VectorSetLocks,
  vector_manager_quantization::{QuantizationChannel, QuantizationState, QuantizationStep},
  vector_manager_replication::ReplicationRuntime,
  vector_types::{
...
impl VectorManager {
  ...
  pub fn value_similarity(
    ...
    let mut filter_state = match compile_filter_state(opts.filter) {
      Ok(state) => state,
      Err(_) => {
    ...
    let mut predicate = |external_id: &[u8]| -> bool {
      match &mut filter_state {
        None => true,
        Some((program, state)) => {
          self.evaluate_candidate_filter(index.context, external_id, program, opts.filter, state)
        }
      }
    };
  ...
  pub fn element_similarity(
    ...
    let mut filter_state = compile_filter_state(opts.filter).map_err(|_| VectorOpError {
      result: VectorManagerResult::BadParams,
  ...
  fn build_similarity_output(
    ...
    if !filter.is_empty() {
      ...
      let _ = super::vector_manager_filter::apply_post_filter(
        filter,
        found,
        &view,
...
fn compile_filter_state(
  ...
  Option<(
    ...
    super::vector_manager_filter::CandidateFilterState,
  )>,
  (),
> {
  if filter.is_empty() {
  ...
  match super::expr_compiler::try_compile(filter) {
    Ok(program) => {
      let state = super::vector_manager_filter::CandidateFilterState::new(&program, filter);
