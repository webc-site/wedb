...
impl<S: StoreCallbacks> VectorManager<S> {
  ...
  pub fn read_vector_index_core<'a>(
    ...
    loop {
      ...
      if self.recreate_index_locked(key, &index).is_err() {
  ...
  fn create_or_recreate_under_exclusive(
    ...
    match self.stored_index_of(key) {
      Some(bytes) => match Index::from_bytes(&bytes) {
        ...
        Some(index) => self.recreate_index_locked(key, &index),
        None => match create {
  ...
  fn recreate_index_locked(&self, key: &[u8], index: &Index) -> Result<Index, VectorManagerResult> {
    if let Err(err) =
      self
        .service
  ...
  fn create_index_locked(
    ...
    self
      ...
      .map_err(|err| {
        log::error!("create_index_locked: 原生索引构建失败: {err:?}");
  ...
  /// 登记表复合键直写单点（内部轴键面；同步写透 KeyTag::VectorRegistry
  ...
  pub(crate) fn put_stored_index(&self, rk: &[u8], bytes: &[u8; INDEX_SIZE_BYTES]) {
    ...
    self.persist_registry_index(rk, bytes);
  }
