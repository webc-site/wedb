  ...
  pub fn encode_resp2(&self, out: &mut Vec<u8>) {
    match self {
      ...
      VectorReply::Integer(i) => {
        let mut ibuf = ItoaBuffer::new();
        out.push(b':');
        out.extend_from_slice(ibuf.format(*i).as_bytes());
        out.extend_from_slice(b"\r\n");
      }
      VectorReply::Bulk(v) => match v {
        Some(v) => {
          let mut ibuf = ItoaBuffer::new();
          out.push(b'$');
          out.extend_from_slice(ibuf.format(v.len()).as_bytes());
          out.extend_from_slice(b"\r\n");
      ...
      VectorReply::Map(pairs) => {
        let mut ibuf = ItoaBuffer::new();
        out.push(b'*');
        out.extend_from_slice(ibuf.format(pairs.len() * 2).as_bytes());
        out.extend_from_slice(b"\r\n");
        for (k, v) in pairs {
          k.encode_resp2(out);
          v.encode_resp2(out);
      ...
      VectorReply::Double(d) => {
        ...
        let s = format_double(*d, &mut buf);
        let mut ibuf = ItoaBuffer::new();
        out.push(b'$');
        out.extend_from_slice(ibuf.format(s.len()).as_bytes());
        out.extend_from_slice(b"\r\n");
  ...
  pub fn encode_resp3(&self, out: &mut Vec<u8>) {
    match self {
      VectorReply::Double(d) => {
        let mut buf = Buffer::new();
        out.push(b',');
        out.extend_from_slice(format_double(*d, &mut buf).as_bytes());
        out.extend_from_slice(b"\r\n");
      }
      VectorReply::Boolean(b) => {
  ...
  pub fn network_vadd_impl(&self, args: &[&[u8]], resp3: bool) -> VectorReply {
    ...
    match self.manager.try_add(key, &stored, &add_args) {
      Ok(VectorManagerResult::OK) => {
        ...
        self
          .manager
          .replicate_vector_set_add(key, dims, build_ef, &add_args);
  ...
  pub fn network_vrem(&self, args: &[&[u8]]) -> VectorReply {
    ...
    match removed {
      VectorManagerResult::OK => {
        ...
        self.manager.replicate_vector_set_remove(args[0], args[1]);
        VectorReply::Integer(1)
      }
  ...
  pub fn network_vsetattr_impl(&self, args: &[&[u8]], resp3: bool) -> VectorReply {
    ...
    let found = self.read_index(args[0]).is_some_and(|index| {
      ...
      if ok {
        ...
        self
          .manager
          .replicate_vector_set_set_attribute(args[0], args[1], args[2]);
      }
      ok
    });
    match (found, resp3) {
