...
impl<S: StoreCallbacks> RespServerSessionVectors<S> {
  ...
  pub fn network_vsim_impl(&self, prefix: &[u8], args: &[&[u8]], resp3: bool) -> VectorReply {
    ...
    let (stored, _index_guard) = match self.manager.read_vector_index(prefix, key) {
  ...
  pub fn network_vemb(&self, prefix: &[u8], args: &[&[u8]]) -> VectorReply {
    ...
    let (stored, _index_guard) = match self.manager.read_vector_index(prefix, args[0]) {
      (Some(index), guard) => (index.to_bytes(), guard),
      (None, _) => return VectorReply::Array(Vec::new()),
    };
  ...
  pub fn network_vrem(&self, prefix: &[u8], args: &[&[u8]]) -> VectorReply {
    ...
    let removed = self.manager.try_remove(args[0], &index.to_bytes(), args[1]);
    match removed {
  ...
  pub fn network_vsetattr_impl(&self, prefix: &[u8], args: &[&[u8]], resp3: bool) -> VectorReply {
    ...
    let found = self.read_index(prefix, args[0]).is_some_and(|index| {
      let ok = self
        .manager
        .try_set_attribute(args[0], &index.to_bytes(), args[1], args[2]);
      if ok {
