...
impl<S: StoreCallbacks> VectorManager<S> {
  ...
  pub async fn export_migration_elements(&self, index_value: &[u8]) -> Vec<MigratedElement> {
  ...
  pub async fn import_migrated_index(
    ...
    index.index_ptr = 1;
    if !self
      .write_stored_index(prefix, key, &index.to_bytes())
      .await
    {
      return Err(ERR_VECTOR_SERVICE_RESPONSE);
    }
  ...
  ///
  /// 锁协议与 [`VectorManager::delete_vector_set`] 同源：条带独占写锁排空
  /// 源端在途共享读（迁移纪元静止等待仅门控会话键访问，不冻结向量检索）
  /// 后摘除，锁轴与登记表同为复合键域。
  pub async fn delete_migrated_vector_set_of(&self, rk: &[u8], index_value: &[u8]) {
    if self
      .stored_index_of(rk)
      .as_ref()
