...
pub struct WedbVectorStoreCallbacks<D: Device + 'static> {
  pub session: Arc<StoreSession<D>>,
}
...
impl<D: Device + 'static> WedbVectorStoreCallbacks<D> {
  ...
  /// 单键读三态甄别（[`StoreCallbacks::read`] 的 NotFound/失败都报 false，
  /// 对齐 C# ReadSizeUnknown `!status.Found` 同一口径；但 [`Self::rmw`] 的
  /// 读改写必须区分二者——把 IO 失败当空值会零覆写存量向量）
  fn read_outcome<F>(&self, context: u64, key: &[u8], mut f: F) -> ReadOutcome
  where
    F: FnMut(&[u8]),
  {
    ...
    match self.session.try_read_raw_in_memory(&phys_key, |val| f(val)) {
      ...
      _ => {
        ...
        match blocking_wait(self.session.read_raw_with(&phys_key, |val| {
          called = true;
          f(val);
        })) {
...
impl<D: Device + 'static> StoreCallbacks for WedbVectorStoreCallbacks<D> {
    ...
    F: FnMut(u32, &[u8]),
  {
    ...
    match blocking_wait(self.session.read_batch_raw_with(&cold_keys, |i, val| {
      if let Some(v) = val {
        f(cold_idx[i], v);
      }
    })) {
  ...
  where
    F: FnMut(&[u8]),
  {
    matches!(self.read_outcome(context, key, f), ReadOutcome::Hit)
  }
    ...
    F: FnMut(&mut [u8]),
  {
    ...
    if write_len == 0 {
      return !matches!(self.read_outcome(context, key, |_| {}), ReadOutcome::Failed);
    }

    let mut buf = vec![0u8; write_len];
    let outcome = self.read_outcome(context, key, |curr| {
  ...
  /// 内联过滤（对标 C# 经 FilterCallbackUnmanaged 委托 EvaluateCandidateFilter；
  /// 本实现未接过滤表达式，恒放行）
  ///
