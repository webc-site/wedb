...
impl VectorAofSink {
  ...
  fn enqueue(&self, cmd: RespCommand, log_arg: i64, prefix: &[u8], key: &[u8], args: &[&[u8]]) {
    ...
    if let Err(e) =
      aof.enqueue_rmw_slices(AofEntryType::StoreRMW, ver, physical_key.as_slice(), &input)
    {
...
impl<S: StoreCallbacks> VectorManager<S> {
  ...
  pub fn replicate_vector_set_remove(&self, prefix: &[u8], key: &[u8], element: &[u8]) {
    let Some(sink) = self.aof_sink.read().clone() else {
      return;
    };
    let args = [element];
  ...
  pub fn replay_vector_set_add(
    ...
    match self.try_add(prefix, key, &index.to_bytes(), &add_args) {
  ...
  /// [`Self::replay_vector_set_rename`] 迁移登记表项。
  pub fn replicate_vector_set_rename(&self, prefix: &[u8], old_key: &[u8], new_key: &[u8]) {
    let Some(sink) = self.aof_sink.read().clone() else {
      return;
    };
  ...
  /// 未装配零开销旁路
  pub fn replay_vector_set_rename(
    &self,
    prefix: &[u8],
    new_key: &[u8],
    input: &ReplayInput,
    ...
    let Some(index_value) = self.stored_index_of(rk_old) else {
      return Err("Should never fail to find original key during RENAME replay".to_string());
    };
    // 覆写前清退被顶的既有登记：主端 delete_vector_set(new_key) 不入 AOF，
    // 重放端须自清（request_deletion 登记清理 + 丢弃被顶 HNSW 上下文），
    // 否则 VADD 条目重建而来的上下文每次 rename onto 泄漏一个
    if let Some(displaced) = self.stored_index_of(rk_new) {
      self.request_deletion(&displaced);
    }
    self.put_stored_index(rk_new, &index_value);
    self.remove_stored_index(rk_old);
    // 双键 WATCH 恰一推（见头注）：先旧键（DELETE(old) 语义位）后新键
    //（SET(newKey) 语义位），与主端迁移序一致
    self.bump_watch(old_key);
    self.bump_watch(new_key);
    Ok(())
  }
  ...
  /// numLinks, distanceMetric] 六参紧凑形态）。索引记录驻留域内登记表不随
  /// 检查点持久化，缺此条目则空向量集迁移后目标端重启整键消失（源端已删，
  /// 无从重迁）
  pub fn replicate_vector_set_index(&self, prefix: &[u8], key: &[u8], index: &Index) {
    let Some(sink) = self.aof_sink.read().clone() else {
      return;
    };
    let entry_args = [
  ...
  pub fn replay_vector_set_index(
    &self,
    prefix: &[u8],
    key: &[u8],
    slot: u16,
  ...
  pub fn replay_vector_set_remove(
    &self,
    prefix: &[u8],
    key: &[u8],
    input: &ReplayInput,
    ...
    let (index, _lock) = self.read_vector_index(prefix, key);
    let index =
      index.ok_or_else(|| "Failed to read Vector Set index during AOF replay".to_string())?;
    match self.try_remove(key, &index.to_bytes(), element) {
  ...
  pub fn replay_vector_set_set_attribute(
    ...
    let index =
      index.ok_or_else(|| "Failed to read Vector Set index during AOF replay".to_string())?;
    if self.try_set_attribute(key, &index.to_bytes(), element, attribute) {
      Ok(())
    } else {
      Err(
