...
impl ContextMetadata {
  ...
  pub fn update_hash_slot(&mut self, allow_zero: bool, context: u16, slot: u16) {
    debug_assert!(
      allow_zero || context != 0,
      "Zero context not permitted here"
    );
...
impl VectorManager {
  ...
  pub fn update_hash_slot(&self, old_value: &[u8], new_slot: u16) {
    let Some(index) = super::vector_manager_index::Index::from_bytes(old_value) else {
      return;
    };
...
mod tests {
  ...
  fn manager_context_allocation_flow() {
    ...
    let idx = super::super::vector_manager_index::Index {
      context: ctx,
      ..Default::default()
    };
