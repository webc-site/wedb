...
impl<S: StoreCallbacks> VectorManager<S> {
  ...
  pub fn create_index_in(
    ...
    let _ = self
      .service
      .create_index(context, index.index_config(), self.callbacks.clone());
  ...
  pub fn mark_suppress_cleanup(&self, key: &[u8]) {
    let mut flags = VectorSetFlags::NONE;
    ...
    flags = flags.union(VectorSetFlags::SUPPRESS_CLEANUP);
  ...
  pub fn clear_suppress_cleanup(&self, key: &[u8]) {
    self.set_flags(key, VectorSetFlags::NONE);
  }

  /// libs/server/Resp/Vector/VectorManager.Index.cs:SetFlags
  ///
  /// 写入标志位（经登记表 RMW 承接，假定并发修改已被锁阻止）。
  pub fn set_flags(&self, key: &[u8], flags: VectorSetFlags) {
  ...
  ///
  /// 原地更新记录中的 Flags 字段。
  pub fn set_index_flags(index_value: &mut [u8], flags: VectorSetFlags) {
    debug_assert_eq!(index_value.len(), INDEX_SIZE, "索引记录尺寸不符");
