...
use crate::{
  resp::{
    objects::object_store_utils::{obj_load_sync_degrades, obj_writeback_recheck_sync},
...
/// 命令整体不挂经纪、路由慢路径异步臂——冷键装载可出件即出件，未取到则
/// 经经纪等待面闭环（slow.rs `BlockWaitFace`，C# BlockingWait 键态解耦
/// 语义的慢路径承接；判定单源见 obj_load_sync_degrades）
