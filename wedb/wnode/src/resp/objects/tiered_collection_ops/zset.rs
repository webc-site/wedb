...
use wcol::{
  ...
  zset::{
    ...
    sorted_set_object_impl::{
      RangeArgError, SpecialRanges, parse_range_options, sorted_set_add_get_options,
...
use super::common::{
  TieredCollectionArgs, TieredCtx, emit_tiered_mirror, finish_tiered_arm, save_tiered_meta,
  scan_all_from_head, score_of_payload, tiered_count, tiered_guard, tiered_precheck,
  tiered_write_mirror, tree_member_state, tree_put_ok, tree_put_rejected,
};
  ...
  let result = 'arm: {
    match op {
      ...
      SortedSetOperation::Zincrby => {
        ...
        // 直存增量原值（对位 C# SortedSetObjectImpl.cs:SortedSetIncrement 缺席
...
impl ZScoreBounds {
  ...
  fn parse(min_span: &[u8], max_span: &[u8]) -> Option<Self> {
