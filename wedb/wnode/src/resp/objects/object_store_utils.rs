...
/// 同步装载对象四态收尾单源（list/hash/set/zset 族共用）：
/// 降级转 `Ok(false)` 异步重放、WRONGTYPE 错误帧已落即 `Ok(true)` 闭环、
/// 缺失态由调用方执行收尾表达式/块、命中即展开为对象。
#[macro_export]
macro_rules! obj_load_or_bail {
  ($load_fn:ident, $store:expr, $key:expr, $output:expr, $missing:expr) => {
    match $load_fn($store, $key, $output) {
      $crate::resp::objects::object_store_utils::ObjLoad::Degrade => return Ok(false),
      $crate::resp::objects::object_store_utils::ObjLoad::WrongType => return Ok(true),
      $crate::resp::objects::object_store_utils::ObjLoad::Missing => $missing,
      $crate::resp::objects::object_store_utils::ObjLoad::Present(obj) => obj,
    }
  };
  ($load:expr, $missing:expr) => {
    match $load {
      $crate::resp::objects::object_store_utils::ObjLoad::Degrade => return Ok(false),
      $crate::resp::objects::object_store_utils::ObjLoad::WrongType => return Ok(true),
      $crate::resp::objects::object_store_utils::ObjLoad::Missing => $missing,
...
#[macro_export]
macro_rules! obj_windowed_load {
  ($load_fn:ident, $store:expr, $key:expr, $output:expr, mut $name:ident, $missing:expr) => {
    let Some(_window) = $store.try_rmw_window($key) else {
      return Ok(false);
    };
    let mut $name = $crate::obj_load_or_bail!($load_fn, $store, $key, $output, $missing);
  };
}

/// 多键装载三态收尾单源（set / zset 族共用）：
/// 命中即取对象集，`Ok(None)` 磁盘候选降级转 `Ok(false)` 异步重放、
/// `Err(())` WRONGTYPE 错误帧已落转 `Ok(true)` 闭环。
#[macro_export]
macro_rules! load_many_or_bail {
  ($store:expr, $keys:expr, $output:expr) => {
    match load_many($store, $keys, $output) {
      Ok(Some(objs)) => objs,
...
use wbase::{
  convert::compute_expiration_ticks as base_compute_expiration_ticks, num::strict_i32,
  time::now_ticks,
};
pub(crate) use wcol::object_payload::{
  GarnetObjectPayload, ObjLoad, count_of_blob, expiry_watermark_of_blob,
};
use wcol::{
  object_payload::{obj_decode_custom, obj_encode_custom_into, object_heap_estimate},
  types::garnet_object::IGarnetObject,
};
use wdev::Device;
use wkv::BatchStoreSession;
use wresp::{
  check_args::{check_arg_count, parse_i32_arg, parse_i64_arg},
  cmd_strings::{
...
pub use super::rmw_helpers::{
  RespRmwDone, SyncRmwCmd, SyncRmwHandlers, SyncRmwOutcome, run_sync_rmw,
};
pub(crate) use super::rmw_helpers::{
  SealedLoad, SyncStoreWindow, load_sealed_tri, load_typed_sealed, obj_writeback_recheck_async,
  obj_writeback_recheck_sync, obj_writeback_tiered, rmw_window_pair_async, run_async_rmw,
  run_operate, slow_load_eval, store_dest_cold_common, store_writeback_clear_ttl,
  try_sync_rmw_window_pair, try_tiered_arm, write_rmw_aof_fail_frame, write_rmw_reply,
};
use crate::{
  resp::{custom_objects::custom_object_entry, resp_server_session::RespServerSession},
  storage::session::{
    common::{
      TagRead,
...
impl ElementHeaderKind {
  ...
  /// GenericErrMustMatchNoOfArgs 的 `{0}` 实参
  #[inline]
...
pub(crate) fn parse_elements_header(
  parse_state: &[&[u8]],
  curr_idx: usize,
  ...
  {
    cs::abort_with_args(output, cs::GENERIC_ERR_MANDATORY_MISSING, &[kind.token()]);
    return None;
  }
  ...
  let Some(num_elements) = parse_state.get(num_idx).and_then(|raw| strict_i32(raw)) else {
    cs::abort_with_args(
      output,
      cs::GENERIC_PARAM_SHOULD_BE_GREATER_THAN_ZERO,
      &[kind.num_param()],
    );
    return None;
  };
  ...
  if parse_state.len() as i64 != elements_start as i64 + i64::from(num_elements) {
    cs::abort_with_args(
      output,
      cs::GENERIC_ERR_MUST_MATCH_NO_OF_ARGS,
      &[kind.num_param()],
    );
    return None;
  }
...
pub(crate) fn compute_expiration_ticks(
  ...
  base_compute_expiration_ticks(now_ticks(), expiration, is_milliseconds, is_timestamp)
}
...
/// ticks 换算与 word 打包
pub(crate) fn parse_expire_elements_args<'a>(
  cmd_name: &'static str,
  parse_state: &'a [&'a [u8]],
  kind: ElementHeaderKind,
  is_milliseconds: bool,
...
pub(crate) fn parse_elements_only_args<'a>(
  cmd_name: &'static str,
  parse_state: &'a [&'a [u8]],
  kind: ElementHeaderKind,
  output: &mut Vec<u8>,
...
/// 的 SortedSetRandomMember 同构）：arity 1..=3 → count i32（宽度/前导零口径与
/// 错误帧见 [`strict_i32_or_err`]，负数合法表示从尾部取）→ 第三词元大小写门
/// （仅 len==3 校验，语法错误帧）→ count 钳至有符号 30 位（预留 2 位元数据位，
/// C# Math.Min 同款）
pub(crate) fn parse_random_member_args(
  cmd_name: &'static str,
  parse_state: &[&[u8]],
  with_token: &[u8],
  output: &mut Vec<u8>,
...
/// numkeys i32（宽度/前导零口径与错误帧见 [`strict_i32_or_err`]）→ numkeys ≥ 1 →
/// 键段足量 → LIMIT 形态恰多 2 参（token 大小写门）→ limit i32 → 负值拒
pub(crate) fn parse_intersect_card_args<'a>(
  ...
  let num_keys = parse_i32_arg(parse_state[0], output)?;
  if num_keys < 1 {
  ...
  if parse_state.len() > idx {
    ...
    let v = parse_i32_arg(parse_state[idx + 1], output)?;
    if v < 0 {
...
/// Meta 闸步进核（装载族与长度族 sync/async 四处共用）：`want_size` 开关
/// [`meta_gate`] 水位快路径，水位命中续步由 `on_size` 裁决
#[inline]
fn step_meta_gate<T>(
  ...
  want_size: bool,
  output: &mut Vec<u8>,
  on_size: impl FnOnce(usize) -> MetaStep<T>,
) -> MetaStep<T> {
  match meta_gate(read, tag, want_size) {
    MetaGate::SizeShortCircuit(size) => on_size(size),
...
/// （want_size=false 恒不产出水位臂；即便产出，装载仍须真实对象而非计数）
#[inline]
fn step_load_meta<T>(
  read: TagRead<Option<MetaValue>>,
  tag: u8,
  ...
  step_meta_gate(read, tag, false, output, |_| MetaStep::Envelope)
}
...
fn step_len_meta(
  ...
  step_meta_gate(read, tag, true, output, |size| {
    MetaStep::Done(ObjLoad::Present(size))
  })
}
...
/// 载荷不足头部（计数域 <4B / 水位域类型 <12B）显式判 Corrupt，经
/// [`step_envelope`] 既有 corrupt_payload_reject 臂与装载族单一漏斗收敛——
/// 严禁 unwrap_or 兜底出假计数（静默 :0 / 永快道水位与装载族错误帧双标准，
/// 且违背 object_payload「畸形载荷显式失败」契约）
#[inline]
fn env_decode_head(raw: &[u8], tag: GarnetObjectType) -> Result<ObjHead, EnvDecodeErr> {
  ...
  let count = count_of_blob(payload).ok_or(EnvDecodeErr::Corrupt)?;
  let watermark = expiry_watermark_of_blob(tag, payload).ok_or(EnvDecodeErr::Corrupt)?;
  Ok(ObjHead { count, watermark })
}
...
pub(crate) fn obj_load_typed_sync<T, D: Device>(
  store: &BatchStoreSession<'_, D>,
  key: &[u8],
...
pub(crate) fn obj_load_sync_degrades<D: Device>(
  store: &BatchStoreSession<'_, D>,
  key: &[u8],
  tag: GarnetObjectType,
) -> bool {
...
/// 本函数与 [`obj_load_custom`] 各自只发起读并归一化为 [`TagRead`]
///
/// 每步域读恒先过 wkv 域内 TTL 单点门：到期未清退的判死异构键（string 影子/
/// RI）恒判缺失而非 WRONGTYPE——对 C# ObjectStore Reader 门序为刻意采序
/// （对齐真 Redis「过期即缺席」，deviations.md §150），严禁按 C# 回改
pub(crate) fn obj_load_custom_sync<T, D: Device>(
  store: &BatchStoreSession<'_, D>,
  key: &[u8],
  tag: u8,
  output: &mut Vec<u8>,
...
pub async fn obj_load_typed<T, D: Device>(
  storage: &StorageSession<'_, D>,
  key: &[u8],
  tag: GarnetObjectType,
  output: &mut Vec<u8>,
  ...
  obj_load_custom(storage, key, tag as u8, output, deserialize).await
}

/// 自定义对象异步装载（支持 u8 扩展标签）—— [`obj_load_custom_sync`] 的异步对位，
/// 决策核共用，差异仅在读原语（磁盘候选在存储层异步内核内闭环，故无降级态）
/// 与 IO 错误如实上抛
pub async fn obj_load_custom<T, D: Device>(
  storage: &StorageSession<'_, D>,
  key: &[u8],
  tag: u8,
  output: &mut Vec<u8>,
...
#[inline]
pub(crate) fn obj_length_sync<D: Device>(
  store: &BatchStoreSession<'_, D>,
  key: &[u8],
  tag: GarnetObjectType,
  output: &mut Vec<u8>,
...
/// 集合长度应答单源输出（LLEN / SCARD / HLEN / ZCARD 快路径共用）
#[inline]
pub(crate) fn reply_obj_length(
  load: ObjLoad<usize>,
  output: &mut Vec<u8>,
  on_degrade: impl FnOnce(&mut Vec<u8>) -> wresp::Result<bool>,
) -> wresp::Result<bool> {
  match load {
    ObjLoad::Degrade => on_degrade(output),
...
pub(crate) fn envelope_heap_estimate(raw: &[u8]) -> i64 {
  ...
  // 扩展段（标签自 wval::CustomObjectType 分配单点，标准段外）：命中判定收口
  // [`crate::resp::custom_objects::custom_object_entry`] 编译期静态单表单源，本处零手拼臂
  raw
    .split_first()
    .and_then(|(&tag, payload)| {
      custom_object_entry(tag).map(|entry| (entry.heap_estimate)(payload))
    })
    .unwrap_or(0)
}
...
/// 并发 DEL/SET 被尾段盲写顶掉
///
/// 值路径走 wkv 单次成形快写（`try_upsert_envelope_sync_fill`）：信封
/// `[1B 标签][payload]` 在记录槽位一次落笔，无中间整值 `Vec`（增量条目另行
/// 显式入账，本口无 AOF 镜像诉求）
pub(super) fn obj_save_sync<D: Device>(
  ...
  if !rmw_ttl_rebuild_sync(store, key)?.closed() {
    return Ok(false);
  }
  Ok(
    store
      .try_upsert_envelope_sync_fill(key, tag as u8, payload)?
      .is_ok(),
  )
}
...
/// 死锁面），由本复验承接交错裁决
pub(crate) fn obj_save_recheck_sync<D: Device>(
  store: &BatchStoreSession<'_, D>,
  key: &[u8],
...
/// 三域内存态单趟快照定音（命中域 / 全域确认不存在，零 I/O）；三域有磁盘候选
/// 时先交 [`StorageSession::probe_alive_domain_with_prefix`] 读通裁决（过期记录
/// 在异步读内核内惰性清退后视同缺失）兼作让核点——放行窗口期对面 DEL/SET 物理
/// 键写臂完整落地——再以第二次内存快照终判。
///
/// 终判绝不采信早于终判的在途读通结果（缺陷背景，票
/// wnode-envelope-correct-arm-recheck-fails-block-resurrect）：装载视图与磁盘
/// 候选同源（同一记录），信封矫正臂冷键装载后候选未变，读通返回的仍是装载
/// 所见旧视图——对面 DEL 墓碑 / SET 清退在读通在途期间落地即被判前状态吞没，
/// 复验判 `unchanged=true` 盲写复活已 ACK 删除的键 / 与新写字符串双域并存。
/// 故候选态的「域未变」只能以内存观测定音：写只追加不改，窗口期落地的墓碑 /
/// 覆写必在内存留痕（命中或墓碑）；读通闭环后内存重观测仍见候选，即「自装载
/// 以来该域无任何内存写落地」，磁盘候选就是装载读通所见记录本身，域归属未变
pub async fn obj_save_recheck_async<D: Device>(
  ...
  // 一判：判定时刻内存快照（零 I/O，可定音即终判，与同步档同判据同序）
  if let Some(current) = probe_alive_domain_with_prefix(&storage.batch, prefix.as_slice(), key)? {
    return Ok(obj_domain_unchanged(loaded, current));
  }
  // 让核裁决：候选域读通闭环（过期惰性清退），放行窗口期对面物理键写臂落地；
  // 读通判异的域归属即刻拒绝，不待终判
  let adjudicated = storage
    .probe_alive_domain_with_prefix(prefix.as_slice(), key)
    .await?;
  if !obj_domain_unchanged(loaded, adjudicated) {
    return Ok(false);
  }
  // 二判（终判）：读通 I/O 闭环后的内存重观测——窗口期落地的墓碑/覆写以此为准
  if let Some(current) = probe_alive_domain_with_prefix(&storage.batch, prefix.as_slice(), key)? {
    return Ok(obj_domain_unchanged(loaded, current));
  }
  // 读通后候选仍为最新事实：装载后无任何内存写落地（写只追加，新写必在内存），
  // 磁盘候选即装载读通所见记录本身 → 域归属未变，放行写回
  Ok(true)
}
...
/// 值路径走 wkv 单次成形快写：信封 `[1B 标签][payload]` 在记录槽位一次落笔
/// （无中间整值 `Vec`），物理键单次编码复用至镜像；AOF 镜像与记录写共享
/// 同一已编码字节（写成功地址零拷贝借值，对齐 C# WriteLogUpsert 从
/// srcLogRecord 取值入账），杜绝镜像再拷贝
pub(super) fn obj_save_custom_notified<D: Device>(
  ...
  let rec_k = store.session_tag_key(KeyTag::ObjectEnvelope, key);
  let addr = store.try_upsert_envelope_sync_fill_with_prefix(key, &rec_k, tag, payload)?;
  match addr {
    StdResult::Ok(addr) => {
      // 极端驻留缺口（同步成功臂地址理论上恒内存驻留）防御物化补投
      let mirrored = store.with_record_value(addr, |val| {
        store.notify_envelope_upsert(rec_k.as_slice(), val)
      });
      let result = mirrored.unwrap_or_else(|| {
        let mut val = Vec::with_capacity(payload.len() + 1);
        obj_encode_custom_into(tag, payload, &mut val);
        store.notify_envelope_upsert(rec_k.as_slice(), &val)
      });
      // AOF 入队失败沿既有 Result 链上抛拒绝本命令（error.rs AofEnqueue 契约，
      // 与 storage_session.rs obj_save 异步臂 `?` 上抛同款单机制对偶；票
      // wnode-objrmw-aof-enqueue-swallow-matrix，同族先例 r167c-aoffail）：
      // 信封写已生效不回滚（发散显式可见），严禁吞错冒答 Ok(true) 假成功
      result.map_err(|e| {
        log::error!("对象信封写 AOF 入队失败，命令按 AofEnqueue 契约拒绝: {e}");
        e
      })?;
      Ok(true)
    }
    // 环形页翻转 / 磁盘候选：降级异步（调用方整体转异步重放）
    StdResult::Err(_) => Ok(false),
  }
}
...
/// 写与全量入账，无双份 AOF。
///
/// 收口面申报（票 wnode-store-dest-cold-upgrade-bypass 闭环）：本函数是同步档
/// 写回臂的单点收口；STORE 族冷漏斗 [`store_dest_cold_common`] 为异步档第二
/// 收口点——同为持窗写临界区落笔，判链与本函数同款（`should_promote` +
/// `envelope_overflow` 同序、同 wcol 单源谓词），命中就地经
/// `dest_cold_promote_arm` 走 bftree+Meta 换入升阶（apply_rmw_post_operate
/// 同型漏斗），升阶不可执行 `Err(())` 转拒写/重试通道，冷臂不再直写信封。
/// 除此两收口点外任何写回臂禁绕过判链直写信封
pub(crate) fn obj_save_or_gc<T: IGarnetObject, D: Device>(
  store: &BatchStoreSession<'_, D>,
  key: &[u8],
  tag: GarnetObjectType,
  obj: &T,
