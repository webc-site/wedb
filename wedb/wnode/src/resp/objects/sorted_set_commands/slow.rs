...
use crate::{
  resp::objects::{
    object_store_utils::{
      ElementHeaderKind, GarnetObjectPayload, IntersectCardKind, SealedLoad, SyncRmwCmd,
      SyncRmwHandlers, load_typed_sealed, obj_writeback_tiered, parse_elements_only_args,
    ...
    tiered_collection_ops::{TieredCollectionArgs, exec_tiered_zset},
  },
...
/// `Ok(Some(None))` = MISSING（调用方定短路应答）；物化降级装配单点转引
...
async fn load_typed(
  ...
  key: &[u8],
  output: &mut Vec<u8>,
) -> Result<Option<Option<(SortedSetObject, Option<SwapInWindowGuard>)>>, ()> {
  Ok(
  ...
  match cmd {
    ...
    RespCommand::Bzpopmin | RespCommand::Bzpopmax => {
      ...
      for k in &refs[..refs.len().saturating_sub(1)] {
        ...
        if zset_save_or_gc_local(storage, &obj, k, swap_in_window.is_some())
          .await
          .is_err()
        {
          return Err(());
...
async fn zset_pop_first_nonempty_cold(
  ...
  for key in keys {
    ...
    zset_save_or_gc_local(storage, &obj, key, swap_in_window.is_some()).await?;
