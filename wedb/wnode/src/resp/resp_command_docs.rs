...
impl RespCommandDocs {
  ...
  pub fn to_resp_format<B: RespBuffer, P: RespProtocol>(&self, writer: &mut RespWriter<B, P>) {
    let arg_count = 1 // group
      ...
      + usize::from(!self.sub_commands.is_empty());
...
impl IRespSerializable for RespCommandDocs {
...
/// libs/server/Resp/RespCommandDocs.cs:RespCommandDocsTables
///
/// 全量文档索引（C# 静态字段族）
...
mod tests {
  ...
  fn tables_initialize_and_lookup() {
    ...
    assert_eq!(all.len(), 256, "文档根命令数快照");
    // 内部命令不设文档，外部面与全量面等大
    assert_eq!(external.len(), 256, "外部文档根命令数快照");
  ...
  fn doc_flags_and_to_resp_format() {
    ...
    let get = try_get_resp_command_docs("get", false, false).unwrap();
    let mut w = RespMemoryWriter::<wresp::Resp3>::new_p();
