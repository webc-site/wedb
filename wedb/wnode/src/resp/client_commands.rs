...
use wbase::{num::strict_i64, time::now_ms_i64};
...
impl RespServerSession {
  ...
  pub fn network_clientlist(&mut self, output: &mut Vec<u8>) -> wresp::Result<bool> {
    ...
    let filter = if args.is_empty() {
      ...
      match try_get_client_type(&self.parse_state, &self.recv_buffer, 1) {
  ...
  fn parse_kill_filters(&self, args: &[&[u8]], output: &mut Vec<u8>) -> Option<ClientKillFilters> {
    ...
    while arg_ix < args.len() {
      ...
      if filter.eq_ignore_ascii_case(FILTER_ID) {
        ...
        let client_type =
          match try_get_client_type(&self.parse_state, &self.recv_buffer, arg_ix + 1) {
  ...
  pub fn network_clientsetinfo(
    ...
    let is_lib_name = option.eq_ignore_ascii_case(b"LIB-NAME");
    let is_lib_ver = option.eq_ignore_ascii_case(b"LIB-VER");
    if !is_lib_name && !is_lib_ver {
      abort_with_error_message(output, cs::RESP_ERR_GENERIC_SYNTAX_ERROR);
      return Ok(true);
    }
