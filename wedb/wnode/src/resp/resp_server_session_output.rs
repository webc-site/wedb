...
//!
//! 统一代理至 [`wresp::RespWriter`] 泛型单态化写出机制，消除重复拼装与热路径协议分支。

use wresp::{Resp2, Resp3, RespProtocol, RespWriter};
...
impl RespServerSession {
  ...
  /// 获取指定协议标记的泛型写出器
  #[inline(always)]
  pub fn writer_p<P: RespProtocol>(&mut self) -> RespWriter<&mut Vec<u8>, P> {
    RespWriter::new_ref_p(&mut self.output)
  }
  ...
  pub fn write_direct_large_resp_string(&mut self, message: &[u8]) {
    self.writer2().write_direct_large_resp_string(message);
  }
  ...
  pub fn write_map_length(&mut self, count: usize) {
    if self.resp_protocol_version >= 3 {
      self.writer3().write_map_len(count);
    } else {
      self.writer2().write_map_len(count);
    }
  }
  ...
  pub fn write_null_array(&mut self) {
    if self.resp_protocol_version >= 3 {
      self.writer3().write_null_array();
    } else {
      self.writer2().write_null_array();
    }
  }
  ...
  pub fn write_push_length(&mut self, count: usize) {
    if self.resp_protocol_version >= 3 {
      self.writer3().write_push_len(count);
    } else {
      self.writer2().write_push_len(count);
    }
  }
  ...
  pub fn write_set_length(&mut self, count: usize) {
    if self.resp_protocol_version >= 3 {
      self.writer3().write_set_len(count);
    } else {
      self.writer2().write_set_len(count);
    }
  }
