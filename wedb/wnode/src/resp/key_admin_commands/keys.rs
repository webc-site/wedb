...
use wbase::{
  convert::{
    ...
    expire_at_milliseconds_to_ticks, expire_at_seconds_to_ticks, milliseconds_from_diff_ticks,
...
impl RespServerSession {
  ...
  pub fn network_getdel<'a, D: wdev::Device>(
    ...
    match read_user_sync(store, key, None, |v| v.to_vec()) {
      Ok(UserRead::Hit(val)) => {
        ...
        let value = match read {
          ...
          ExpiryRead::At(exp) => {
            if command == TtlCmd::Pttl {
              ...
              seconds_from_diff_ticks(exp, now_ticks())
            }
          }
        };
...
fn rename_vector_set_sync<'a, D: wdev::Device>(
  ...
  // RENAMENX：新键存活（三域 + 登记表第四态）→ 0，不动旧键。折叠式与
  // rename_sync 主分支、慢路径同一单点 probe_alive_with_registry，臂内不再
  // 分写「三域探针 + 手抄登记表命中」两段（第四态与三域同判、无类型门）
  if nx {
    match probe_alive_with_registry(store, prefix.as_slice(), new_key, Some(vm)) {
      Ok(Some(true)) => {
        output.write_resp_int(0);
        return Ok(true);
      }
...
/// C# 向量键 TTL 全族一致的对齐须登记表过期刻度 + AOF/复制重放端配套改造
/// 后统一放行（probe_alive_with_registry 头注），本期明确不支持
