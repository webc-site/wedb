...
async fn ttl_read_ms(
  ...
  {
    // 存活向量键无过期记录：回 -1（对标 C# HandleTtl
    // `HasExpiration ? Expiration : -1`，快路径 probe_alive_with_registry 同口径）
    return Ok(-1);
  }
  Ok(ms)
}
...
async fn rename_slow(
      ...
      .map_err(|_| ())?
    {
        ...
        .map_err(|_| ())?
      {
        ...
        None => {
          ...
          // 的 rust 对偶）；未登记 → NOSUCHKEY
          if vector.is_some_and(|vm| {
            vm.read_stored_index(storage.batch.session_prefix().as_slice(), old_key)
              .is_some()
          }) {
            // RENAMENX 先判新键存活（wkv 双域 + 登记表；对齐
            // rename_vector_set_sync 的分支内 NX 序）
            if nx {
              let new_alive = storage
                .probe_alive_domain_async(new_key)
                .await
                .map_err(|_| ())?
                .is_some()
                || vector.is_some_and(|vm| {
                  vm.read_stored_index(storage.batch.session_prefix().as_slice(), new_key)
                    .is_some()
                });
              if new_alive {
  ...
  // RENAMENX：新键存活（三域 + 向量登记表第四态）→ 0，不动旧键
  if nx {
    let new_alive = storage
      .probe_alive_domain_async(new_key)
      .await
  ...
  if let Some(vm) = vector {
    vm.delete_vector_set(storage.batch.session_prefix().as_slice(), new_key);
  }

  match domain {
    RenameDomain::Meta => {
      // RI / 升阶键：整树 CPR 快照至新键数据文件 + 新键元记录（集合类型与
      // 成员 TTL 水位透传旧元记录）；随后 key 级 TTL 随迁，迁移尾部
      //（ETag 同步 + 旧键删除，经 handle_bftree_drain_and_delete 注销旧树
      // 与旧元记录）与 String / 对象域共用
      storage
        .batch
...
pub(crate) async fn key_admin_slow(
  ...
  match cmd {
    C::Exists => {
      // 多键计数；三域存活 + 向量登记表第四态（对标快路径
      // probe_alive_with_registry 逐键裁决）
    ...
    C::Ttl | C::Pttl => {
      ...
      // seconds_from_diff_utc_now_ticks 的截断嵌套恒等（.NET ticks 下
      // floor(floor(x/T_MS)/1000) == floor(x/T_S)）
      let value = ttl_read_ms(storage, key, false, vector).await?;
    ...
    C::Expiretime | C::Pexpiretime => {
      ...
      let value = ttl_read_ms(storage, key, true, vector).await?;
    ...
    C::Expire | C::Pexpire | C::Expireat | C::Pexpireat => {
      ...
      // wkv 异步判定表（快路径 expire_apply_sync 的镜像单点）：-2 键缺失 /
      // 0 条件不满足 → :0；1 已设置 / 2 过去时间戳已物理删除 → :1（含
      // Redis 7.4 立即删除语义）；头部粗化对命令边界粗化值幂等恒等
      let applied = storage
        .batch
        .expire_at(key, expire_at_ticks, opt)
        .await
        .map_err(|_| ())?;
      write_raw(
        output,
    ...
    C::Persist => {
      ...
      let removed = storage.batch.persist(key).await.map_err(|_| ())?;
    ...
    C::Getdel => {
        ...
        .map_err(|_| ())?
      {
        ...
        UserReadAsync::Missing => {
          output.write_resp_null_ver(storage.resp_protocol_version());
        }
      }
      Ok(())
    }
    ...
    C::Restore => {
      ...
      // SET_Conditional(SETEXNX)：仅键不存在时写入（NX 语义；登记命中即
      // BUSYKEY，C# 前置存在判定）
      if vector.is_some_and(|vm| {
        vm.read_stored_index(storage.batch.session_prefix().as_slice(), key)
          .is_some()
