...
impl RespServerSession {
  ...
  pub fn attempt_skip_line(&mut self) -> bool {
    ...
    {
      // 仅整行命中臂记日志（对标 C# RespCommand.cs:1050 LogWarning 逐字全等，
      // 文案本无上下文参不强加）；:426 半行 false 臂禁记——复刻 C# :1047-1049
      // 注释口径：投机窥测（如 NetworkGET_SG）越过批次尾不误报
      log::warn!("Received malformed input message. Trying to skip line.");
