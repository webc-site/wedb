...
impl RespServerSession {
  ...
  /// 保留形参以匹配 TryParseCustomCommand 统一接口签名
  pub fn try_parse_custom_command(&self, _command: &[u8]) -> Option<RespCommand> {
    None
  }
  ...
  pub fn array_parse_command(
    ...
    // 断连，此处以 parse_violation 表达；缓冲不足时按不完整等待
    let mut ptr = start + 1;
    if ptr < self.bytes_read && self.recv_buffer[ptr] == b'-' {
      // C#：'-'+不足 4 字节 → 不完整等待；否则 '-1\r\n' 之外的负长度一律
      // ThrowInvalidStringLength 断连
    ...
    if &self.recv_buffer[ptr..ptr + 2] != b"\r\n" {
      // 终止符不符：C# ThrowUnexpectedToken 断连
      self.parse_violation = true;
      return None;
    }
    ptr += 2;
  ...
  pub fn hash_lookup_command(
    ...
    *count -= 1;
    ...
    match primary {
      None => {
        // 非内建命令 → 自定义命令（冷路径，零拷贝切片）
        if let Some(custom_cmd) =
          self.try_parse_custom_command(&self.recv_buffer[start..start + len])
        {
