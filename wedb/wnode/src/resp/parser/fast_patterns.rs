...
/// 组装单个长度档组（消四份同构字面量）
const fn group<const N: usize>(
  mask: Option<[u8; 16]>,
  candidates: &'static [[u8; 16]; N],
  base: usize,
) -> MaskedGroup<'static> {
  MaskedGroup {
    mask,
...
pub(crate) static FAST_PATTERN_GROUPS: &[MaskedGroup<'static>] = &[
  group(Some(MASK_13), &PADDED_13, RUN13_START),
  group(Some(MASK_14), &PADDED_14, RUN14_START),
  group(Some(MASK_15), &PADDED_15, RUN15_START),
  group(None, &PADDED_16, RUN16_START),
];
