...
use wbitmap::{
  BitFieldCmdArgs, BitFieldOverflow, BitFieldSecondaryCommand, BitmapOperation, bit_count_driver,
  bit_field_execute, bit_pos_driver, invoke_bit_operation_unsafe, new_block_alloc_length_from_type,
...
use wresp::{
  ...
  cmd_strings::{RESP_ERR_GENERIC, RESP_ERR_WRONG_NUMBER_OF_ARGUMENTS, abort_with_error_message},
};
...
/// 位图键双域判型读结果（镜像 ETag 族 `ReadOutcome` 的三态折叠）
enum BitmapRead<R> {
  /// String 域命中（值内容任意）
  Hit(R),
  /// 键缺失
  Missing,
  /// 集合对象键（C# ValueIsObject → WRONGTYPE）
  WrongType,
  /// 磁盘候选 / TTL 过期待物理清除，须降级异步
  Deferred,
}

/// 位图键双域判型读（一处定义，位图族全部入口共用）
///
/// 对标 C# 主存读写首探 `DataHeader.ValueIsObject` 的 WRONGTYPE 拦截
/// （RMWMethods.cs:InPlaceUpdater / ReadMethods.cs:Reader）：先探 String 域，
/// 未命中再探 ObjectEnvelope 域——信封命中即集合对象键，调用方答
/// WRONGTYPE；两域皆缺为键缺失。`Err(())` 为存储错误
fn bitmap_read_user_sync<R, D: wdev::Device>(
  store: &wkv::BatchStoreSession<'_, D>,
  key: &[u8],
  f: impl FnOnce(&[u8]) -> R,
) -> Result<BitmapRead<R>, ()> {
  match read_adjudicated_user_sync(store, key, f) {
    Ok(Some(Some(Ok(v)))) => Ok(BitmapRead::Hit(v)),
    Ok(Some(Some(Err(())))) => Ok(BitmapRead::WrongType),
    Ok(Some(None)) => Ok(BitmapRead::Missing),
    Ok(None) => Ok(BitmapRead::Deferred),
    Err(_) => Err(()),
  }
}
...
impl RespServerSession {
  ...
  pub fn network_string_set_bit<'a, D: wdev::Device>(
    ...
    let mut val = match bitmap_read_user_sync(store, key, |v| v.to_vec()) {
      Ok(BitmapRead::Hit(v)) => v,
      Ok(BitmapRead::Missing) => Vec::new(),
      Ok(BitmapRead::WrongType) => {
        output.write_resp_error(cs::RESP_ERR_WRONG_TYPE);
        return Ok(true);
      }
      // 磁盘候选：降级
      Ok(BitmapRead::Deferred) => return Ok(false),
      Err(()) => {
  ...
  pub fn network_string_get_bit<'a, D: wdev::Device>(
    ...
    let byte_idx = (offset / 8) as usize;
    let bit_idx = 7 - (offset % 8) as u32;
    let bit = match bitmap_read_user_sync(store, key, |val| {
      if byte_idx < val.len() {
      ...
      Ok(BitmapRead::Hit(bit)) => bit,
      // C# NOTFOUND → :0
      Ok(BitmapRead::Missing) => 0,
  ...
  pub fn network_string_bit_count<'a, D: wdev::Device>(
    ...
    let total = match bitmap_read_user_sync(store, key, |val| {
      bit_count_driver(start, end, offset_type, val, val.len() as i64)
    }) {
      Ok(BitmapRead::Hit(total)) => total,
      // C# NOTFOUND → :0
      Ok(BitmapRead::Missing) => 0,
      // C# Read ValueIsObject → WrongType
      Ok(BitmapRead::WrongType) => {
  ...
  pub fn network_string_bit_position<'a, D: wdev::Device>(
    ...
    // 闭包内直接查找：跳过全值拷贝，零分配
    match bitmap_read_user_sync(store, key, |val| {
      bit_pos_driver(
        val,
        val.len() as i64,
        start_offset,
      ...
      Ok(BitmapRead::Hit(pos)) => output.write_resp_int(pos),
      // C# NOTFOUND：找 0 回 0，找 1 回 -1
      Ok(BitmapRead::Missing) => {
  ...
  pub fn network_string_bit_operation<'a, D: wdev::Device>(
    ...
    // 读源键，缺失键跳过（C# NOTFOUND continue）；任一源键为对象键即整体
    // WRONGTYPE 短路（C# StringBitOperation：目的键尚未写入，无副作用）
    let mut srcs: Vec<Vec<u8>> = Vec::with_capacity(count - 1);
    for src_key in &parse_state[1..] {
      match bitmap_read_user_sync(store, src_key, |v| v.to_vec()) {
        Ok(BitmapRead::Hit(v)) => srcs.push(v),
        Ok(BitmapRead::Missing) => {}
        Ok(BitmapRead::WrongType) => {
          output.write_resp_error(cs::RESP_ERR_WRONG_TYPE);
          return Ok(true);
    ...
    let result = if srcs.is_empty() {
      ...
      // 单次遍历同时取最短/最长源长度（源 ≤63 个，免三轮扫描）
      let (shortest, longest) = srcs.iter().fold((usize::MAX, 0), |(min, max), s| {
        (min.min(s.len()), max.max(s.len()))
      });
  ...
  pub fn string_bit_field_action<'a, D: wdev::Device>(
    ...
    // 整条命令只回错误帧
    let mut value: Option<Vec<u8>> = match bitmap_read_user_sync(store, key, |v| v.to_vec()) {
      Ok(BitmapRead::Hit(v)) => Some(v),
      Ok(BitmapRead::Missing) => None,
      Ok(BitmapRead::WrongType) => {
        output.write_resp_error(cs::RESP_ERR_WRONG_TYPE);
        return Ok(true);
      }
