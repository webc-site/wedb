...
impl RespServerSession {
  ...
  pub fn network_string_set_bit<'a, D: wdev::Device>(
    ...
    let mut val = match read_user_sync(store, key, None, |v| {
  ...
  pub fn network_string_get_bit<'a, D: wdev::Device>(
    ...
    let byte_idx = (offset / 8) as usize;
    let bit_idx = 7 - (offset % 8) as u32;
    let bit = match read_user_sync(store, key, None, |val| {
      if byte_idx < val.len() {
  ...
  pub fn network_string_bit_count<'a, D: wdev::Device>(
    ...
    let total = match read_user_sync(store, key, None, |val| {
  ...
  pub fn network_string_bit_position<'a, D: wdev::Device>(
    ...
    // 闭包内直接查找：跳过全值拷贝，零分配
    match read_user_sync(store, key, None, |val| {
      bit_pos_driver(
        val,
        val.len() as i64,
        start_offset,
  ...
  pub fn network_string_bit_operation<'a, D: wdev::Device>(
    ...
    for src_key in &parse_state[1..] {
      ...
      match read_user_sync(store, src_key, None, |v| acc.fold(v)) {
  ...
  pub fn string_bit_field_action<'a, D: wdev::Device>(
    ...
    // 整条命令只回错误帧
    let mut value: Option<Vec<u8>> = match read_user_sync(store, key, None, |v| v.to_vec()) {
