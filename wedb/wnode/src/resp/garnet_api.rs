...
pub struct CheckpointCtx {
  ...
  /// AOF 门面（SAVE/BGSAVE 经管理器内核做安全截断；AOF 未启用时 None）
...
pub struct StoreGarnetApi<D: Device> {
  ...
  /// 自定义命令注册表（C# storeWrapper.customCommandManager 的执行域
...
impl<D: Device> StoreGarnetApi<D> {
  ...
  /// 关联自定义命令注册表（自定义对象命令执行/慢路径重放经此解析）
  pub fn with_custom_command_manager(
    mut self,
    manager: wcustom::SharedCustomCommandManager,
  ) -> Self {
    self.custom_commands = Some(manager);
    self
  }
}
...
impl<D: Device> GarnetApiFace for StoreGarnetApi<D> {
  ...
  async fn exec_slow(&self, cmd: RespCommand, args: Vec<Vec<u8>>) -> Vec<u8> {
    ...
    // 检查点族须在会话纪元保护区外发起（检查点 fail-fast 契约：批处理纪元
    ...
    match cmd {
      // ---- GET 慢路径承接（同步段 TTL 到期裁决降级至此：异步 read_string
      // 含惰性过期物理清除（purge 后视同不存在），对标 C# InternalRead 的
      // ReadCache/磁盘冷读异步完成路径）
      C::Get => {
      ...
      // 逐键完整删除；FLUSHDB 清会话当前库——C# FlushDatabase(activeDbId)，
      ...
      C::Flushdb | C::Flushall => {
        ...
        let flushed = if cmd == C::Flushdb {
          // 当前库前缀扫描 + 逐键完整删除（DatabaseManagerBase.cs:FlushDatabase
          // 的共享存储面等价）
          storage.delete_all_user_keys().await
        } else {
          // 全域扫描 + 逐键完整删除（MultiDatabaseManager.cs:FlushAllDatabases
          // 的共享存储面等价）
      ...
      C::Swapdb => {
        ...
        if idx1 == idx2 {
          ...
          // C# TrySwapDatabases 活跃会话门控：RespServerSession 数 > 1 时
          // 拒绝换库（交换窗口无并发写，防搬移竞态丢数据）
          write_error_raw(&mut output, RESP_ERR_SWAPDB_UNSUPPORTED);
        } else {
          // wkv 全 tag 搬移内核（字符串 / 对象信封 / Meta + 随键 TTL/ETag；
          // 物理写经写端口镜像 AOF，--recover 与副本重放后与主端收敛）
          let swapped = storage
            .batch
            .session
      ...
      // 的异步承接：同步段磁盘候选降级重放。快照尾参为命令名，经名回查
      // 注册表后按同款四接口执行）
      C::Customobjcmd => {
        ...
        let Some((obj_cmd, tag)) = self.resolve_custom_object_command(name) else {
...
impl<D: Device> StoreGarnetApi<D> {
  ...
  /// NetworkLASTSAVE；rust 映射为 WedbStore::create_checkpoint（经 wkv 检查点通道）；
  /// LASTSAVE 为纯读取）
  ///
  ...
  async fn checkpoint_command_slow(&self, cmd: RespCommand) -> Vec<u8> {
    ...
    match cmd {
      ...
      C::Save | C::Bgsave => {
        ...
        // + AOF 安全截断（快照后增量条目携带新版本，恢复重放不丢）。
        // rust 以 wdatabase 管理器内核承接（裸 WedbStore::create_checkpoint
        // 不推进版本，SAVE 后增量在 --recover 重放时会被版本基线误跳过）
        let db = Arc::new(GarnetDatabase::with_garnet_aof(
          0,
          Arc::clone(&self.session.store),
          Arc::clone(&self.session.store.device),
          ctx.dir.clone(),
          ctx.aof.clone(),
        ));
        let mgr = SingleDatabaseManager::new(ctx.dir.clone(), db);
        if matches!(cmd, C::Bgsave) {
          ...
          if !mgr.try_pause_checkpoints() {
            write_error_raw(&mut output, RESP_ERR_CHECKPOINT_ALREADY_IN_PROGRESS);
            return output;
          }
          let last_save_ms = Arc::clone(&ctx.last_save_ms);
          spawn(async move {
            match mgr.take_checkpoint(false).await {
              Ok(true) => last_save_ms.store(now_ms() as i64, Ordering::Release),
  ...
  /// 按名解析自定义对象命令（含信封标签；慢路径重放承接，
  /// C# CustomCommandManagerSession.Match(CustomObjectCommand)）
  fn resolve_custom_object_command(
    &self,
    name: &[u8],
  ) -> Option<(wcustom::CustomObjectCommand, u8)> {
    let mgr = self.custom_commands.as_ref()?.read();
    let cmd = mgr.match_object_command(name)?;
    let tag = mgr.object_type_tag(cmd.ext_id);
    Some((cmd.clone(), tag))
  }

  /// 自定义对象命令异步执行（CustomRespCommands.cs:TryCustomObjectCommand
  /// 的慢路径承接：与同步段 objects/custom_object_commands.rs 同款四接口
  /// 分派；`cmd_refs` 为 [key, args...]，Err = 存储 IO 失败由调用方降噪）
  async fn custom_object_slow(
    &self,
    storage: &StorageSession<'_, D>,
    obj_cmd: &wcustom::CustomObjectCommand,
    tag: u8,
    fns: &wcustom::CustomObjectFns,
    cmd_refs: &[&[u8]],
    output: &mut Vec<u8>,
  ) -> Result<(), ()> {
    ...
    // 读取走 ObjectEnvelope 物理域；信封域未命中时反探 String 域——命中即
    // 用户字符串键（WRONGTYPE，含字符串键的统一存口径），两域皆缺为缺键
    let read = storage
      .read_tag_with(key, KeyTag::ObjectEnvelope, |raw| {
        obj_decode(raw, tag).map(|p| p.to_vec())
      })
      .await
      .map_err(|_| ())?;
    let read = match read {
      None => {
        let str_hit = storage
          ...
          .await
          .map_err(|_| ())?;
        if str_hit.is_some() { Some(None) } else { None }
      }
      Some(r) => Some(r),
    };
    match read {
      // 缺键：RMW 走 NeedInitialUpdate 防空墓碑 + 新建初值；Read 走 NotFound
      None => {
        if obj_cmd.command_type == CommandType::ReadModifyWrite {
      ...
      // 键存在但信封标签不符（含字符串键）：WRONGTYPE
      Some(None) => write_error_raw(output, RESP_ERR_WRONG_TYPE),
      Some(Some(raw)) => {
        let mut payload = raw;
        if obj_cmd.command_type == CommandType::ReadModifyWrite {
          if !(fns.updater)(&mut payload, args, output) {
