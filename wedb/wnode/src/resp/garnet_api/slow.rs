...
use wresp::{
  cmd_strings::{
    ...
    RESP_ERR_GENERIC_SYNTAX_ERROR, RESP_ERR_GENERIC_UNK_CMD, RESP_ERR_GENERIC_VALUE_IS_NOT_INTEGER,
    RESP_ERR_HCOLLECT_ALREADY_IN_PROGRESS, RESP_ERR_SLOW_PATH_STORAGE, RESP_ERR_SWAPDB_UNSUPPORTED,
...
use crate::{
  ...
  storage::session::{common::ttl_sync::probe_alive_with_prefix, storage_session::StorageSession},
  types::GarnetStatus,
};
...
impl<D: Device> StoreGarnetApi<D> {
  pub(crate) async fn exec_slow_impl(
    ...
    match cmd {
      ...
      // rust 对偶：前缀外提 + 判定与批量写入折叠单窗口 + 中途失败尽力回滚，
      // 全有或全无，应答与最终存储状态恒一致）
      C::Msetnx => {
        ...
        if !resume {
          for key in pairs.iter().step_by(2) {
            // 同步三域探针先行（快路径 network_msetnx 同款，String /
            // ObjectEnvelope / Meta：升阶键只余 Meta 元记录一个身份，漏探
            // 会误判不存在后写出双域键，对标 C# unified 域 EXISTS）；Deferred
            //（磁盘候选 / TTL 待裁决）才逐键异步三域闭环（TTL 过期键经异步读
            // 惰性清除后视同缺失）。逐键 await 窗口内的并发写入与 C# 锁序
            // 差异属顺序未定义，非语义破坏
            match probe_alive_with_prefix(&storage.batch, prefix_slice, key) {
              ...
              Ok(Some(false)) => {}
              Ok(None) => match storage.probe_alive_domain_async(key).await {
                Ok(Some(_)) => {
                  output.write_resp_int(0);
                  return output;
                }
        ...
        // 补写模式重写快路径前缀键同值幂等）。任一键 Err 尽力回滚全部涉及
        // 键——判定已保证写入前全键不存在，删除即恢复原状（含快路径 resume
        // 前缀键，命令级全有或全无；回滚自身失败仍报错，错误同源属尽力极限）
      ...
      C::Info => {
        ...
        // 至少持有一个键的库，零键库行与虚库号越界者一并剔除）。向量键登记表
        ...
        if text.is_empty() {
          // C# InfoCommand.NetworkINFO 空段回 CmdStrings.RESP_EMPTY（
          // 协议恒定的空批量串，非 nil）
      ...
      C::Expdelscan => {
        let db_id = match refs.first() {
          ...
          Some(arg) => match parse_db_index(arg) {
            ...
            Err(_) => {
              write_error_raw(&mut output, RESP_ERR_GENERIC_VALUE_IS_NOT_INTEGER);
              return output;
            }
          },
        };
      ...
      C::Mget => {
        if array_cmds::slow::mget(&storage, &refs, storage.resp_version, &mut output)
          .await
          .is_err()
        {
          write_error_raw(&mut output, RESP_ERR_SLOW_PATH_STORAGE);
