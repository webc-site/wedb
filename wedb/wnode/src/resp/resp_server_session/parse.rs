...
impl RespServerSession {
  ...
  ///
  /// 命令名侧无长度上限臂（C# GetCommand（RespServerSession.cs:1243-1275）
  /// 头消费后仅查值收齐与终止符两臂，亲验无帽），等待语义仅由「值收齐
  /// 检查」与「终止符检查」两臂承担；超 int 值域声明由
  /// [`try_read_unsigned_length_header`] 违例臂断连。512MiB 上限
  /// （`wresp::read::MAX_ARGUMENT_LENGTH_BYTES`）仅在会话参数侧
  /// （`wresp::session_parse_state`）与客户侧 reader 生效，本路径不设
  /// 第二机制
