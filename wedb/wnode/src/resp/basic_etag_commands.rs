...
use wresp::{
  ...
  cmd_strings::{RESP_ERR_GENERIC, RESP_ERR_WRONG_TYPE, abort_with_error_message},
...
use crate::{
  ...
  storage::session::common::{
    ...
    ttl_sync::{read_adjudicated_user_sync, ttl_of_sync},
  },
};

/// libs/server/Resp/CmdStrings.cs:RESP_ERR_INVALID_ETAG
const ERR_INVALID_ETAG: &str = cs::RESP_ERR_INVALID_ETAG;
...
#[inline]
fn write_etag_val_array(output: &mut Vec<u8>, etag: i64, value: Option<&[u8]>) {
...
fn read_value_and_etag<'a, D: wdev::Device>(
  ...
  match read_adjudicated_user_sync(store, key, |v| v.to_vec()) {
    Ok(Some(Some(Ok(val)))) => match etag_of_sync(store, key) {
      Ok(Some(etag)) => ReadOutcome::Hit(val, etag),
      Ok(None) => ReadOutcome::Deferred,
      Err(_) => ReadOutcome::Failed,
    },
    Ok(Some(Some(Err(())))) => ReadOutcome::WrongType,
    Ok(Some(None)) => ReadOutcome::NotFound,
    Ok(None) => ReadOutcome::Deferred,
    Err(_) => ReadOutcome::Failed,
  }
}
...
impl RespServerSession {
  ...
  pub fn network_getwithetag<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "GETWITHETAG", [key]);
    match read_value_and_etag(store, key) {
      ReadOutcome::Hit(val, etag) => write_etag_val_array(output, etag, Some(&val)),
      ReadOutcome::NotFound => output.write_resp_null(),
  ...
  pub fn network_getifnotmatch<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "GETIFNOTMATCH", [key, etag_raw]);
    ...
    match read_value_and_etag(store, key) {
      ReadOutcome::Hit(val, current) => {
        if current == given_etag {
          write_etag_val_array(output, current, None);
        } else {
          write_etag_val_array(output, current, Some(&val));
        }
      }
      ReadOutcome::NotFound => output.write_resp_null(),
      ReadOutcome::WrongType => output.write_resp_error(RESP_ERR_WRONG_TYPE),
  ...
  pub fn network_delifgreater<'a, D: wdev::Device>(
    ...
    unpack_args!(parse_state, output, "DELIFGREATER", [key, etag_raw]);
    let Some(given_etag) = strict_i64(etag_raw).filter(|&e| e >= 0) else {
      abort_with_error_message(output, ERR_INVALID_ETAG);
      return Ok(true);
    };

    match read_adjudicated_user_sync(store, key, |_| ()) {
      // 数据 / TTL 记录有磁盘候选，或键已过期待物理清除：降级异步
      Ok(None) => return Ok(false),
      // 对象键：判型先于 etag 比较，一律不删（键存活）
      Ok(Some(Some(Err(())))) => output.write_resp_int(0),
      // 键缺失：无删除目标
      Ok(Some(None)) => output.write_resp_int(0),
      // String 键存活：按 etag 条件判定删除
      Ok(Some(Some(Ok(())))) => match etag_of_sync(store, key) {
  ...
  pub fn network_setwithetag<'a, D: wdev::Device>(
    ...
    if matches!(
      read_adjudicated_user_sync(store, key, |_| ()),
      Ok(Some(Some(Err(()))))
    ) {
    ...
    match read_value_and_etag(store, key) {
      ReadOutcome::Hit(old_val, existing) => {
        if !cmd.hit(given_etag, existing) {
          ...
          write_etag_val_array(output, existing, value);
          return Ok(true);
        }
        ...
        match apply_etag_write(store, &args, new_etag, output) {
          Ok(true) => write_etag_val_array(output, new_etag, None),
          Ok(false) => return Ok(false),
          Err(()) => {}
        }
      }
