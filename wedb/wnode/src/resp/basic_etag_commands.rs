...
use wresp::{RespVecExt, cmd_strings};
...
const ERR_NOT_INTEGER: &str = cmd_strings::RESP_ERR_GENERIC_VALUE_IS_NOT_INTEGER;
...
impl RespServerSession {
  ...
  pub fn network_getifnotmatch<'a, D: wdev::Device>(
    ...
    let Some(given_etag) = str::from_utf8(parse_state[1])
      ...
      cmd_strings::write_error_raw(output, ERR_NOT_INTEGER);
      return Ok(true);
    };
  ...
  pub fn network_delifgreater<'a, D: wdev::Device>(
    ...
    if given_etag < 0 {
      cmd_strings::write_error_raw(output, ERR_INVALID_ETAG);
      return Ok(true);
    }
