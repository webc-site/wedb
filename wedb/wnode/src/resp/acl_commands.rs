...
use super::{
  acl_store::AclStore,
  custom_objects,
  resp_server_session::{RespServerSession, collect_arg_views},
};
...
impl RespServerSession {
  ...
  pub fn network_acl_set_user<D: Device>(
    ...
    if foreign_namespace_denied(ctx.caller_namespace, raw_name, target_ns) {
...
impl RespServerSession {
  ...
  pub fn process_acl_commands<D: Device>(
    ...
    // 视图借用随 args 析构收尾：SmallVec 的堆外溢释放点即接收缓冲借用终点，
    // 必须先于下方的会话字段写回与句柄换装
    drop(args);
    self.output = output;
