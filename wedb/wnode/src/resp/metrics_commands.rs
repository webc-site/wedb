...
impl RespServerSession {
  ...
  pub fn process_metrics_commands(&mut self, cmd: RespCommand) -> Option<bool> {
    ...
    let result: Result<(), String> = match cmd {
      ...
      RespCommand::LatencyReset => {
        let metrics = self.global_latency_metrics.as_ref();
  ...
  pub fn handle_slow_log(&mut self, cmd: RespCommand) {
    ...
    let slow_log_threshold = threshold_us * TICKS_PER_MICROSECOND as i64;
    let elapsed = now_ticks - self.slow_log_start_ticks;
