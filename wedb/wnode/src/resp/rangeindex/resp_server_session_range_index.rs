...
use wbftree::{RangeIndexStub, ScanReturnField, StorageBackendType, TreeTuning};
use wdev::Device;
...
/// RI.SCAN/RANGE 应答数组头的乐观预留字节数（`*NN\r\n` 2 位计数；≤99 条记录
/// 回填零移动，与 C# WriteScanToOutput 的 ReservedHeaderSize 同值）
const SCAN_HEADER_RESERVED: usize = 5;
...
/// 回填 RI.SCAN/RANGE 应答数组头 `*N\r\n`（1:1 对标
/// libs/server/Storage/Session/MainStore/RangeIndexOps.cs:BackfillArrayHeader）
///
/// 流式回调在预留位之后直写记录体，实际计数位宽与预留不等长时以 memmove
/// 同型（`copy_within`）移动记录体消除间隙
fn backfill_scan_array_header(output: &mut Vec<u8>, base: usize, count: usize) {
  let mut num = Buffer::new();
  let digits = num.format(count);
  let actual = 1 + digits.len() + 2;
  if actual < SCAN_HEADER_RESERVED {
    output.copy_within(base + SCAN_HEADER_RESERVED.., base + actual);
    output.truncate(output.len() - (SCAN_HEADER_RESERVED - actual));
  } else if actual > SCAN_HEADER_RESERVED {
    let old_len = output.len();
    output.resize(old_len + (actual - SCAN_HEADER_RESERVED), 0);
    output.copy_within(base + SCAN_HEADER_RESERVED..old_len, base + actual);
  }
  output[base] = b'*';
  output[base + 1..base + 1 + digits.len()].copy_from_slice(digits.as_bytes());
  output[base + actual - 2..base + actual].copy_from_slice(b"\r\n");
}
...
pub async fn network_riscan<D: Device, R>(
  ...
  let base = output.len();
  output.resize(base + SCAN_HEADER_RESERVED, 0);
  ...
  match scanned {
    Ok(_) => backfill_scan_array_header(output, base, records),
    Err(RangeIndexError::WrongType) => {
    ...
    Err(e) => {
      ...
      output.truncate(base);
      abort_with_error_message(output, &e.to_string());
    }
  }
  Ok(true)
}
...
/// 因此这里不存在第二套计数入口。
pub async fn network_ricount<D: Device, R>(
  parse_state: &[&[u8]],
  ri: Option<R>,
  session: &StoreSession<D>,
  output: &mut Vec<u8>,
  ...
  check_arg_count!(parse_state, 1, output, "RI.COUNT");

  match session.range_index_count(parse_state[0]).await {
    Ok(count) => {
...
impl RespServerSession {
  ...
  pub async fn network_riget<D: Device, R>(
    ...
    network_riget(parse_state, ri, session, self.resp_protocol_version, output).await
  }
  ...
  pub async fn network_ricount<D: Device, R>(
    ...
    network_ricount(parse_state, ri, session, output).await
  }
}
