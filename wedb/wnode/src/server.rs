...
impl<A: ServerArgs, C: ClusterProvider + Clone> ServerBootstrap<A, C> {
  ...
  {
    self.run_async(move |args, cluster| ready(session_provider_factory(&args, &cluster)))
  }
  ...
  /// 装配回调承接同一时序：恢复在 [`GarnetServer::run_until_shutdown`]
  ///（即端点 accept 启动）之前 await 闭环。
  ///
    ...
    Fut: Future<Output = crate::Result<Arc<P>>>,
  {
    ...
    // 3. 驱动 compio 全异步运行时启动服务并阻塞监听系统停机信号。
    //    会话提供者组装回调在运行时内执行——宿主装配存储引擎时可安全
    //    拉起 GC 等后台任务（对标 C# GarnetServer.InitializeServer 在
    //    服务器生命周期内完成存储打开）
    let rt = Runtime::new()?;
...
pub fn run_node<P: SessionProviderFace + 'static>(
  ...
  let bootstrap = ServerBootstrap::new(conf.clone()).banner(banner);

  #[cfg(feature = "tls")]
  let bootstrap = match (&conf.tls_cert, &conf.tls_key) {
    (Some(cert), Some(key)) => {
      let tls_config = ServerTlsConfig::from_pem_files(cert, key)?;
      bootstrap.with_tls_config(Some(tls_config))
    }
    (Some(_), None) | (None, Some(_)) => {
      return Err(crate::Error::Io(io::Error::new(
        io::ErrorKind::InvalidInput,
        "tls_cert 与 tls_key 必须同时提供",
      )));
    }
    (None, None) => bootstrap,
  };

  bootstrap.run_with_provider(session_provider)
}
  ...
  while !ctx.coordinator.is_stopped() {
    ...
    match accept_res {
      ...
      Ok(Err(e)) => {
        ...
        error!("Worker-{} 接收连接失败: {e}", ctx.core_id);
      }
      Err(Cancelled) => {
