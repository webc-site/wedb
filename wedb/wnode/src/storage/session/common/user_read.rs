...
impl<R> UserRead<R> {
  ...
  /// 快路径读命中/未命中入账（对位 C# MainStoreOps.cs:GET 的 `status.Found` →
  /// incr_session_found / else → incr_session_notfound 判定：Hit→found、
  /// Missing→notfound；WrongType 两侧同不计数，Deferred 交慢路径既有单点
  /// `StorageSession::record_read_outcome`）。句柄 None = 采样关闭或
  /// RMW 前置读（C# RMW 路径不入账），空条件跳过
  #[inline]
  pub fn record_outcome(&self, metrics: Option<&SessionMetricsHandle>) {
    let Some(metrics) = metrics else { return };
    match self {
      Self::Hit(_) => metrics.incr_total_found(1),
      Self::Missing => metrics.incr_total_notfound(1),
      Self::WrongType | Self::Deferred => {}
    }
  }
}
...
///
/// `metrics` 为会话指标共享句柄（GET/GET_SG/MGET/GETRANGE/STRLEN/GETEX 读
/// 命令臂传接）：漏斗尾一次收口命中/未命中入账（对位 C# MainStoreOps.cs:30/:39
/// GET 族 `status.Found` 判定，批量臂逐键经本漏斗即逐键入账与 C# 同口径）；
/// RMW 前置读传 None 不计，Deferred 交慢路径既有单点
#[inline]
...
pub fn read_tag_sync<R, D: Device>(
  ...
  let res = session
    .with_session_consistent_read(key, tag, || read_adjudicated_tag_sync(session, key, tag, f))?;
