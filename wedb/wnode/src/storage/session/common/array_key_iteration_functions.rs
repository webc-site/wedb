...
impl<'a, D: Device> StorageSession<'a, D> {
  ...
  /// 列出命中给定集群槽位的活用户键（至多 `key_count` 个，扫描序边扫边收集、
  /// 满 N 即停早退；活键判定与 [`Self::count_keys_in_slot`] 共用
  /// [`Self::live_key_at`] 单点）
  ///
  ...
  ///（对标 C# 逐记录 Add、`keys.Count < maxKeyCount` 早停：取扫描序前 N 个
  /// 活键，非旧实现的哈希序 take——分叉就此消除，且早停省去全库物化）
  ///
