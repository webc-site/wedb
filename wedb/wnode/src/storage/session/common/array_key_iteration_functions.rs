...
use wbase::{glob::glob_match_nocase, hash_slot::hash_slot as cluster_slot, time::now_ticks};
use wdev::Device;
use wkv::{Error, TtlProbe};
...
/// 物理键标签：集合对象信封记录
pub(crate) const TAG_ENVELOPE: u8 = KeyTag::ObjectEnvelope.as_u8();
...
impl<'a, D: Device, CR: wkv::ConsistentReadFunctions> StorageSession<'a, D, CR> {
      ...
      .scan(
        ...
        |_addr, rec| {
          ...
          match rest.split_first() {
            Some((&t, user_key)) if t == TAG_STRING || t == TAG_META || t == TAG_ENVELOPE => {
        ...
        |_addr, rec| {
          ...
          if let Some(rest) = key.strip_prefix(prefix_slice)
            && matches!(
              rest.split_first(),
              Some((&t, user_key))
                if (t == TAG_STRING || t == TAG_META || t == TAG_ENVELOPE)
      ...
      .scan(
        ...
        |_addr, rec| {
          ...
          if let Some(rest) = key.strip_prefix(prefix_slice)
            ...
            && (t == TAG_STRING || t == TAG_META || t == TAG_ENVELOPE)
            && cluster_slot(user_key) == slot
          {
  ...
  pub(crate) async fn string_snapshot(&self) -> wkv::Result<Vec<(Vec<u8>, Vec<u8>)>> {
    ...
    let mut entries: Vec<(Vec<u8>, Vec<u8>)> = map
      .drain()
      .filter_map(|(k, v)| {
        if !matches!(self.batch.probe_ttl(&k, now), TtlProbe::Due) {
          v.map(|val| (k, val))
        } else {
          None
        }
  ...
  pub(crate) async fn string_keys_snapshot(&self) -> wkv::Result<Vec<Vec<u8>>> {
    ...
    let mut keys: Vec<Vec<u8>> = map
      .into_iter()
      .filter(|(k, alive)| *alive && !matches!(self.batch.probe_ttl(k, now), TtlProbe::Due))
      .map(|(k, _)| k)
      .collect();
    keys.sort_unstable();
    Ok(keys)
