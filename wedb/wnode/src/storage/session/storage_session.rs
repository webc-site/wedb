...
impl<'a, D: Device> StorageSession<'a, D, ()> {
  ...
  /// 构造只读扫描会话：版本表为独立实例（无 WATCH 共享面）
  ///
  /// 适用于不登记/推进 WATCH 的慢路径只读扫描（SCAN/KEYS/DBSIZE、
  /// CLUSTER RESET 的 HasKeysInSlots 判定）；写入路径绝不可用（版本
  /// 推进会失联到独立表）
  pub fn new_readonly(batch: BatchStoreSession<'a, D>) -> Self {
    Self::new_with_read_session(
      batch,
...
impl<'a, D: Device, CR: ConsistentReadFunctions> StorageSession<'a, D, CR> {
  ...
  /// 对标 C# `srcLogRecord.AllocatedSize`）；慢路径 readonly 会话不登记 WATCH
  ///（无一致读上下文），直接走批处理双路
  ...
  /// 写指定标签物理键值（SET 语义：同步快路径优先，环形缓冲翻转 / TTL 清除异步闭环）
  ///
  ...
  /// 异步段对象写回的唯一漏斗（obj_save / 自定义对象命令慢路径 / RENAME 经此），
