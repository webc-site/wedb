...
use crate::{
  ...
  storage::session::common::{
    UserReadAsync,
    ttl_sync::{
      meta_collection_type_of, probe_alive_with_registry, probe_alive_with_registry_async,
    },
  },
  types::GarnetStatus,
};
...
impl<'a, D: Device> StorageSession<'a, D> {
  ...
  /// 既有 [`Self::read_tag_with`] 闭环（wkv 异步内核自带前缀解析，冷路径成本与
  /// 本口接入前一致，读命中入账亦在该口内完成，本口不重复记账）
    ...
    let opt =
      match self
        ...
        .with_session_consistent_read_with_prefix(prefix, key, tag, || {
          self
            .batch
            .try_read_tag_sync_unprotected_with_prefix(prefix, key, tag, &f)
        })?? {
        // 磁盘候选：整体降级异步读口（记账在该口内完成）
        StoreResult::RecordOnDisk => return self.read_tag_with(key, tag, &f).await,
        res => res.value(),
      };
    self.record_read_outcome(opt.is_some());
    Ok(opt)
