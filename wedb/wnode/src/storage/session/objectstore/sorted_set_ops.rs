...
use wcol::{
  parse_utils::try_parse_with_infinity, types::object_output::ObjectOutput,
  zset::sorted_set_object::SortedSetObject,
};
use wdev::Device;
// 聚合方式（ZUNION/ZINTER 权重合并语义，真值单点 wresp::SortedSetAggregateType）
...
impl<'a, D: Device, CR: wkv::ConsistentReadFunctions> StorageSession<'a, D, CR> {
  ...
  /// ZCOLLECT 单键收集 RMW：清除过期成员并回写，空对象整键回收
  ///
  /// C# RMWObjectStoreOperation(ZCOLLECT) 全语义：
  /// SortedSetObject.DeleteExpiredItems + 序列化回写；清空后的空集合按
  /// 对象生命周期整键回收。返回 (状态, 剩余成员数)——ZCOLLECT `*` 全库
  /// 批扫逐键调用本入口。
  ///
  /// libs/server/Storage/Session/ObjectStore/SortedSetOps.cs:SortedSetCollect
  pub async fn sorted_set_collect(&self, key: &[u8]) -> wkv::Result<(GarnetStatus, usize)> {
    match self.typed_load::<SortedSetObject>(key).await? {
      Err(s) => Ok((s, 0)),
      Ok(None) => Ok((GarnetStatus::NotFound, 0)),
      Ok(Some(mut obj)) => {
        obj.sorted_set_collect(&mut ObjectOutput::new());
        let remaining = obj.count();
        if obj.is_empty() {
          self.batch.delete(key).await?;
          return Ok((GarnetStatus::Ok, 0));
        }
        let payload = obj.serialize_to_vec();
        self
          .obj_save(key, SortedSetObject::OBJECT_TAG, &payload)
          .await?;
        Ok((GarnetStatus::Ok, remaining))
      }
    }
  }
