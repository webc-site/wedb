...
use itoa::Buffer;
use wbase::num::{strict_f64, try_parse_i64};
...
impl<'a, D: Device, CR: wkv::ConsistentReadFunctions> StorageSession<'a, D, CR> {
  ...
  /// HCOLLECT 单键收集 RMW：清除过期字段并回写，空对象整键回收
  ///
  /// C# RMWObjectStoreOperation(HCOLLECT) 全语义：HashObject.DeleteExpiredItems
  /// + 序列化回写；清空后的空哈希按对象生命周期整键回收。返回 (状态, 剩余字段数)——HCOLLECT `*` 全库批扫（C# ObjectCollect）逐键调用本入口。
  ///
  /// libs/server/Storage/Session/ObjectStore/HashOps.cs:HashCollect
  pub async fn hash_collect(&self, key: &[u8]) -> wkv::Result<(GarnetStatus, usize)> {
    match self.typed_load::<HashObject>(key).await? {
      Err(s) => Ok((s, 0)),
      Ok(None) => Ok((GarnetStatus::NotFound, 0)),
      Ok(Some(mut obj)) => {
        obj.hash_collect(&mut ObjectOutput::new());
        ...
        self.obj_save(key, HashObject::OBJECT_TAG, &payload).await?;
  ...
  pub async fn hash_scan(
    ...
    let extract = |payload: &[u8]| -> Option<Vec<Vec<u8>>> {
      Some(HashObject::deserialize_from_slice(payload).ok()?.get_keys())
    };
    self
      .object_scan(
        key,
