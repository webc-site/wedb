...
impl<'a, D: Device, CR: wkv::ConsistentReadFunctions> StorageSession<'a, D, CR> {
  ...
  pub(crate) async fn increment(&self, key: &[u8], delta: i64) -> wkv::Result<(GarnetStatus, i64)> {
    ...
    let (current, delta_result) = match parsed {
      None => {
        self
          .upsert_string(key, ItoaBuffer::new().format(delta).as_bytes())
          .await?;
    ...
    self
      .upsert_string(key, ItoaBuffer::new().format(updated).as_bytes())
      .await?;
  ...
  pub async fn increment_by_float(
    ...
    let updated = match parsed {
      None => {
        self
          .upsert_string(key, format_double(delta, &mut buf).as_bytes())
          .await?;
    ...
    self
      .upsert_string(key, format_double(updated, &mut buf).as_bytes())
      .await?;
