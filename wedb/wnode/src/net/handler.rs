...
use compio::{
  buf::{BufResult, IoBuf, IoBufMut, ReserveError, SetLen},
...
impl<C: MessageConsumerFace> NetworkHandler<C> {
  ...
  ///   批次级整量拷贝进会话，游标驻留泵。
  ///
  /// 两种读取形态均挂取消令牌：有注册条目时（CLIENT KILL / 注销）哨兵
  /// 打断挂起读，C# 直关套接字的等价物。
  async fn drive_loop<P: SessionProviderFace<Consumer = C>>(
    ...
    loop {
      ...
      if scratch_mode {
        ...
        let read_res = match self.kill_token.clone() {
          Some(token) => stream
            ...
            .fail_fast()
            .await,
          None => Ok(stream.read(RecvAppend(scratch)).await),
        };
        let BufResult(read_res, wrapped) = match read_res {
          Ok(pair) => pair,
          Err(Cancelled) => break,
        };
        scratch = wrapped.0;
        ...
        // 有注册条目时挂取消令牌（CLIENT KILL / 注销经哨兵打断挂起读）
        let read_res = match self.kill_token.clone() {
          Some(token) => stream.read(RecvAppend(raw_buf)).with_cancel(token).fail_fast().await,
          None => Ok(stream.read(RecvAppend(raw_buf)).await),
        };
        let BufResult(read_res, wrapped) = match read_res {
          Ok(pair) => pair,
          Err(Cancelled) => break,
      ...
      if scratch_mode {
        ...
        net_in = read_pos - batch_start;
      }

      // 镜像累加（监视器瞬时吞吐/ops/s 源；会话 dispose 时随条目注销换轨到
      // 历史归并，二者不双计）
      if let Some(entry) = &self.consumer_entry {
        entry.add_net_bytes(net_in as u64, resp_pooled.len() as u64);
        if let Some(session) = self.session.as_mut() {
