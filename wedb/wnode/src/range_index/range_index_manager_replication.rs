...
pub struct RangeIndexStreamArgs<'a> {
  ...
  /// 发布形态：分层重灌 / RENAME 覆写目标键换旧树为真（副本对既有索引以
  /// replace 换入重放，首升阶与用户 RI 迁移流为 false，同键已存在即拒发布）；
  /// 随流块 arg1 标志携载
  pub replace: bool,
  /// 发布后元记录的成员 TTL 水位（.NET Ticks；i64::MAX = 无成员挂 TTL）：随首块
  /// ReplayInput.arg2 携带（arg1 已被首/尾/replace 位占满），副本发布时落真水位，
  /// 杜绝 i64::MAX 假水位令副本分层集合计数门恒真、到期成员永不出账
  pub next_expiry: i64,
}
...
pub struct RangeIndexChunkArgs<'a> {
  ...
  pub replace: bool,
  /// 成员 TTL 水位（同 [`RangeIndexStreamArgs::next_expiry`]，随首块
  /// ReplayInput.arg2 携载，同键流块同值）
  pub next_expiry: i64,
}
...
/// 逐键 AOF 流重组状态：重组反序列化器 + 追踪活动 + 首块携带的发布判别类型与
/// 成员 TTL 水位（C# StreamReassemblyState；字段互斥访问经 parking_lot::Mutex，
/// 发布 await 前必须放锁）
struct StreamReassemblyState {
  ...
  obj_type: AtomicU8,
  /// 发布后元记录成员 TTL 水位（首块 ReplayInput.arg2 捕获；缺省 i64::MAX =
  /// 无成员挂 TTL，与假水位口径一致防误出账）
  next_expiry: AtomicI64,
}
...
impl StreamReassemblyState {
  fn new(temp_path: PathBuf) -> Result<Self, ReplicationError> {
    Ok(Self {
      ...
      next_expiry: AtomicI64::new(i64::MAX),
    })
  }
}
...
impl RangeIndexManagerReplication {
  ...
  /// 成员 TTL 水位随 ReplayInput.arg2 携载——arg1 已被标志位占满）
  ...
  /// 标志、arg2 解出成员 TTL 水位后交 [`Self::process_stream_chunk`]
  pub async fn handle_range_index_stream_replay<D: Device>(
    ...
    self
      .process_stream_chunk(
        ...
        RangeIndexChunkArgs {
          key,
          obj_type: GarnetObjectType::from_u8(input.obj_type).unwrap_or_default(),
          chunk,
          is_first,
          is_last,
          replace,
          next_expiry: input.arg2,
        },
      )
      .await
  }
  ...
  /// 时发布重组成的 BfTree。`replace` 随流块 arg1 标志携载：分层重灌 / RENAME
  /// 覆写流必须换入重放（旧树仍活着，固定 replace=false 会被 AlreadyExists 拒放）。
  /// 成员 TTL 水位随首块 ReplayInput.arg2 捕获，发布时落真水位（缺省 i64::MAX
  /// 与「无成员挂 TTL」同口径）。失败路径（喂块被拒 / 发布失败 / 末块后仍未完成）
  /// 一律清理重组状态后上抛
  pub async fn process_stream_chunk<D: Device>(
    ...
    // 首块捕获发布判别类型与成员 TTL 水位（同键流块同值）
    if is_first {
      state.obj_type.store(obj_type as u8, Ordering::Relaxed);
      state.next_expiry.store(next_expiry, Ordering::Relaxed);
    }
    ...
    if complete {
      ...
      // 首块携带的成员 TTL 水位随发布落盘（未捕获的旧流缺省 i64::MAX 与
      // 「无成员挂 TTL」同口径，绝不误伤计数校正）
      let next_expiry = state.next_expiry.load(Ordering::Relaxed);
