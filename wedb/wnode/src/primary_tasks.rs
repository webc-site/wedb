...
impl PrimaryTasks {
  ...
  /// 的 rust 形态：任务常驻 + 角色位门检（C# 副本不注册、角色切换时取消/
  /// 重启的等价承接，见 [`spawn_aof_commit_task`]）。
  pub fn try_start_commit_task(self: &Arc<Self>, aof: &Arc<GarnetAppendOnlyFile>, commit_ms: u64) {
    if commit_ms > 0 && !self.commit_started.swap(true, Ordering::Relaxed) {
      spawn_aof_commit_task(Arc::clone(self), Arc::clone(aof), commit_ms);
    }
  }
...
/// AOF 周期提交后台任务
///
/// libs/server/StoreWrapper.cs:CommitTaskAsync 的宿主驱动：按 commit_ms
///（最小 1ms）节拍驱动 [`GarnetAppendOnlyFile::commit_flush_async`]，
/// 定期将已追加的 AOF 记录物理刷盘并推进 committed_until_address。
///
/// 副本门控（对标 C# 「Replicas should never run the periodic commit task
/// because it can clobber replicated AOF addresses」）：rust 侧为正确性硬
/// 约束而不仅是对标——`WalLog::commit` 每批尾写 commit 元数据帧推进本地
/// tail，副本接收面以「本地 tail == 主端 currentAddress」严格衔接校验推流，
/// 周期提交写出的元数据帧会让下一帧校验恒 divergent 断流。故副本角色位
/// 挂起时仅 Delay 不提交（与 C# CommitTaskAsync 副本分支逐行同构）。
/// 弱引用捕获，宿主释放后自然退出。
pub fn spawn_aof_commit_task(
  tasks: Arc<PrimaryTasks>,
  ...
  let interval = Duration::from_millis(commit_ms);
  let weak_aof = Arc::downgrade(&aof);
  drop(aof);
  spawn(async move {
    loop {
      sleep(interval).await;
      match weak_aof.upgrade() {
        Some(aof) => {
...
fn spawn_object_collect_task(tasks: Arc<PrimaryTasks>) {
  spawn(async move {
    loop {
      // 进循环先读间隔与禁用位（对标 C# 任务体重读 RuntimeServerConfig）
      let freq_secs = {
      ...
      let Some(store) = tasks
        .object_collect_env
        .lock()
        .as_ref()
        .and_then(|e| e.store.upgrade())
      else {
