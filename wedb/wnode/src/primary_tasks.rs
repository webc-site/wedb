...
fn spawn_aof_commit_task(tasks: Arc<PrimaryTasks>) {
  spawn(async move {
    if supervise_task(AOF_COMMIT_TASK, aof_commit_loop(Arc::clone(&tasks)))
      .await
      .is_err()
    {
...
fn spawn_object_collect_task(tasks: Arc<PrimaryTasks>) {
  spawn(async move {
    if supervise_task(OBJECT_COLLECT_TASK, object_collect_loop(Arc::clone(&tasks)))
      .await
      .is_err()
    {
      // panic 臂：复位幂等启动位，CONFIG SET 调停与 resume 重拉通路自然复活
