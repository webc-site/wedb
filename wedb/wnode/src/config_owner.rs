...
///（C# 逐库快照遍历 `AppendOnlyFile?.backpressure?.SetBudget`；rust 共享单
/// AOF 面下快照遍历折叠为本入口的 AofSyncMaxLag 单臂直达背压闸门）
///
  ...
  D: Device + 'static,
{
  match msg {
    ...
    // TryStartCommitTask）。0 值切换已被 `RuntimeServerConfig` 拒绝，
    // 新值 <= 0 时运行中的任务下轮自退出，新值 > 0 时重拉周期提交任务。
    ...
    // 非正值已完整闭环支持无限超时（<=0 产 ms:0 消息，置入原子的 0 无限超时哨兵，
    // 对应 cluster_node_timeout() 的 None 分支）。
