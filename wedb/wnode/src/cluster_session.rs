...
pub trait ClusterSessionFace: Send + Sync {
  ...
  /// 同 C# clusterEnabled false 短路；`session_asking` 为 u8 剩余计数，
  /// 对标 C# byte SessionAsking——ASKING 置 2、逐命令递减、校验消费 > 0
  ///（RespServerSession.cs:736）；`wait_for_stable_slot` 对标 C# 同名形参
  ///（向量集写命令的槽位稳定等待面，事务 Prepare 段 C# 恒传 false）；
  /// `slot` 为会话库级槽位，MULTI/EXEC 期内 SELECT/SWAPDB 均被拒，整批共槽）
  ///
  /// 接口面逐键校验钩子：真实迭代校验在集群切面固有方法（映射注释在该层）
  ...
  ///
  /// 接口面默认空实现：真实写出在集群切面固有方法（映射注释在该层）
