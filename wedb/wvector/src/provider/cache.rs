...
#[derive(Clone)]
pub struct AdjList(pub AdjacencyList<u32>);
  ...
  pub(crate) fn set_neighbors(
    ...
    if !self.callbacks.rmw_iid(
      ...
      |data: &mut [u32]| {
        ...
        if iid == 0 {
          self.neighbor_cache.pin().insert(iid, neighbors.to_vec());
        }
      },
    ) {
      return Err(StoreError::Write.into());
  ...
  pub(crate) fn append_vector(
    ...
    if !self.callbacks.rmw_iid(
      ...
      |data: &mut [u32]| {
        ...
        if iid == 0 && self.neighbor_cache.pin().contains_key(&iid) {
          self.neighbor_cache.pin().insert(iid, data[..len].to_vec());
        }
      },
    ) {
      return Err(StoreError::Write.into());
