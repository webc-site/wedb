...
pub(crate) enum WedbProviderError {
  ...
  InvalidQuantizer,
  #[error("REDUCE dimension {0} must be <= vector dimensions {1}")]
  ReduceDims(usize, usize),
...
webc_diskann::convert_error!(WedbProviderError);
webc_diskann::always_escalate!(WedbProviderError);

impl From<StoreError> for webc_diskann::ANNError {
  #[inline]
