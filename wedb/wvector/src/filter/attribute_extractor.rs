...
/// 二进制属性值的类型判别符（对标 AttributeExtractor.cs 私有常量
/// BinTypeString/BinTypeNumber/BinTypeBoolTrue/BinTypeBoolFalse/BinTypeNull，
/// 按本仓「类型枚举一处定义，严禁散落裸 const u8」规范收敛为强类型枚举；
/// 线格式字节值与 C# 完全一致）。
...
impl TryFrom<u8> for BinAttrType {
  type Error = ();
...
//     [value_type: u8]              ← 见 BinAttrType（0=字符串 1=数字 2=true 3=false 4=null）
//     [value_len: u16 LE]
  ...
  loop {
    ...
    match json[pos] {
      ...
      b'"' => {
        ...
        output[out_pos] = BinAttrType::String as u8;
        out_pos += 1;
      ...
      c if is_digit(c) || c == b'-' || c == b'+' => {
        ...
        output[out_pos] = BinAttrType::Number as u8;
        out_pos += 1;
      ...
      b't' | b'f' | b'n' => {
        let (literal, ty) = match json[pos] {
          b't' => (&b"true"[..], BinAttrType::BoolTrue),
          b'f' => (&b"false"[..], BinAttrType::BoolFalse),
          _ => (&b"null"[..], BinAttrType::Null),
        };
        if !skip_literal(json, &mut pos, literal) {
          return -1;
        }
        output[out_pos] = ty as u8;
        out_pos += 1;
...
pub fn extract_fields_binary(
  ...
  for _ in 0..num_fields {
    ...
    if let Some(i) = match_index {
      // 未识别的类型字节保持默认词元（对齐 C# switch 无 default 分支后照常计数的语义）。
      if let Ok(ty) = BinAttrType::try_from(value_type) {
        match ty {
          BinAttrType::String => {
          ...
          BinAttrType::Number => {
            if value_len == 8 {
              // 上方已校验 pos + value_len <= binary.len()，8 字节切片必然在界内
              results[i] =
                ExprToken::new_num(f64::from_le_bytes(binary[pos..pos + 8].try_into().unwrap()));
            }
          }
          BinAttrType::BoolTrue => results[i] = ExprToken::new_num(1.0),
          BinAttrType::BoolFalse => results[i] = ExprToken::new_num(0.0),
          BinAttrType::Null => results[i] = ExprToken::new_null(),
        }
      }
...
mod tests {
  ...
  #[test]
  fn bin_attr_type_wire_values() {
    // 判别值必须与 C# BinType* 常量逐字节一致（AttributeExtractor.cs:428-432）
    assert_eq!(BinAttrType::String as u8, 0);
    assert_eq!(BinAttrType::Number as u8, 1);
    assert_eq!(BinAttrType::BoolTrue as u8, 2);
    assert_eq!(BinAttrType::BoolFalse as u8, 3);
    assert_eq!(BinAttrType::Null as u8, 4);
    // 0..=4 双向无损，5 及以上为非法类型字节
    for b in 0u8..=4 {
      let ty = BinAttrType::try_from(b).unwrap();
      assert_eq!(ty as u8, b);
    }
    assert_eq!(BinAttrType::try_from(5), Err(()));
    assert_eq!(BinAttrType::try_from(u8::MAX), Err(()));
  }

  #[test]
  fn binary_unknown_type_byte_keeps_token_none_but_counts() {
    // 手工按线格式构造未知类型字节的属性（对标 C# switch 无 default：
    // 未识别类型不写词元，但 found 照常计数）
    let bin: &[u8] = &[BINARY_MARKER, 1, 1, b'k', 9, 0, 0];
    let ranges = [(0i32, 1i32)];
    let mut results = [ExprToken::default()];
    assert_eq!(extract_fields_binary(bin, b"k", &ranges, &mut results), 1);
