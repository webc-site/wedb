...
use super::expression::{ExprProgram, ExprToken, ExprTokenType};
    ...
    if let Some(i) = match_index {
      match value_type {
        ...
        BIN_TYPE_NUMBER if value_len == 8 => {
          if let Ok(bytes) = <[u8; 8]>::try_from(&binary[pos..pos + 8]) {
            results[i] = ExprToken::new_num(f64::from_le_bytes(bytes));
          }
        }
