//! Shunting-Yard 过滤表达式编译器（对标 diskann-garnet/ExprCompiler.cs）
//!
...
use super::{
  ...
  expression::{ExprProgram, ExprToken, ExprTokenType, OpCode, get_arity, get_precedence},
};
...
///
/// libs/server/Resp/Vector/ExprCompiler.cs:TryMatchOp 的承接：C# 局部函数
/// 逐操作符「匹配则保留更长」的最长匹配循环，合并为本函数 MATCHES 表上的
/// 同序贪心扫描（`||`/`or` 先于 `|` 语义位由多字节优先保证）。
///
