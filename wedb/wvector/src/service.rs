...
use crate::{
  ...
  store::{Callbacks, Context, StoreCallbacks, VectorSetId},
...
impl SearchOutput {
  ...
  pub fn iter(&self) -> impl Iterator<Item = (&[u8], f32)> {
    fn ids(bytes: &[u8]) -> impl Iterator<Item = &[u8]> {
      let mut rest = bytes;
      from_fn(move || {
        if rest.len() < 4 {
          return None;
    ...
    ids(&self.ids).zip(self.distances.iter().copied())
  }
...
impl<S: StoreCallbacks> DiskANNService<S> {
  ...
  /// 向量维度查询。
  pub fn dims_of(&self, context: u64) -> Option<usize> {
    Some(self.indexes.pin().get(&context)?.dims)
  }
  ...
  /// 两元素全精度距离（ElementSimilarity 的精确补充通道）。
  pub fn distance_between(&self, context: u64, a: &[u8], b: &[u8]) -> Option<f32> {
    let index = self.indexes.pin().get(&context).cloned()?;
    let ctx = Context::new(context);
    let ia = index.inner.internal_id_of(&ctx, &VectorSetId::from(a))?;
    let ib = index.inner.internal_id_of(&ctx, &VectorSetId::from(b))?;
    let va = index.inner.full_vector_of_iid(&ctx, ia)?;
    let vb = index.inner.full_vector_of_iid(&ctx, ib)?;
    Some(index.inner.distance(&ctx, &va, &vb))
  }
}
