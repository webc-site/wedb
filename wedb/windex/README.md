...
- `table`：`HashIndex` 索引表 + `HashBuckets`（DirectVirtualMemory Demand-Zero 映射，≥2MB 自动对齐并开 MADV_HUGEPAGE）+ 多键加锁与 L1 预取
...
- `OverflowPool`：allocate / free / get / has_free / allocated_count（`CHUNK_SIZE = 1024`、`MAX_CHUNKS = 4096`）
- `HashIndex`：insert / find_tag / lookup，查重插入 find_tag_or_insert / find_or_create_tag(\_with_min_addr)（死槽清退），批量读两级预取内核 prefetch_batch_probes（12 键窗口、PrefetchProbe 哈希与首地址同源），update_address / delete（RCU CAS / 原子置零），多键加锁 acquire_keys_lock_exclusive
