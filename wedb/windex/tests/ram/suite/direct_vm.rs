...
/// 直接虚拟内存分配：demand-zero 全零、对齐、可读写、可释放
///
...
fn direct_vm_allocate_is_zeroed_aligned_writable() -> Void {
  ...
  info!("对标 DirectVmAllocateIsZeroedAlignedWritable：1MB/4KB 映射、全零、读写、Free");
...
/// as_aligned_slice 与 as_aligned_mut_slice 空块、全长读写与越界安全
#[test]
fn direct_vm_as_aligned_slice_safety() -> Void {
  let _guard = TRACKER_LOCK.lock();
  info!("验证 as_aligned_slice 与 as_aligned_mut_slice 的空块防护与读写正确性");

  // 1. 空块安全返回空切片
  let mut empty = DirectVmBlock::empty();
  assert!(empty.as_aligned_slice().is_empty());
  assert!(empty.as_aligned_mut_slice().is_empty());

  // 2. 有效分配块的切片读写
  let size = 4096;
  let mut block = DirectVirtualMemory::allocate(size, 4096)?;
  assert!(block.avail_len() >= size);

  let slice = block.as_aligned_slice();
  assert_eq!(slice.len(), block.avail_len());
  assert!(slice.iter().all(|&b| b == 0));

  let mut_slice = block.as_aligned_mut_slice();
  mut_slice[0] = 0x12;
  mut_slice[size - 1] = 0x34;

  assert_eq!(block.as_aligned_slice()[0], 0x12);
  assert_eq!(block.as_aligned_slice()[size - 1], 0x34);

  // 3. 显式 Free 后再次调用必须安全返回空切片
  DirectVirtualMemory::free(&mut block);
  assert!(block.as_aligned_slice().is_empty());
  assert!(block.as_aligned_mut_slice().is_empty());
