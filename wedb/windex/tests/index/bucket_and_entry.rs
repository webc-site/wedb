//! 自研依据: 桶/条目位打包（C# 对应 HashBucket 布局 libs/storage/Tsavorite/cs/src/core/Index/HashBucket + test/NativeHashIndexTests.cs）
