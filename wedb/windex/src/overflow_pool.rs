...
impl OverflowPool {
  ...
  /// 清空溢出池（释放全部已分配块并重置分配计数与空闲栈）
  pub fn clear(&self) {
    for chunk in self.chunks.iter() {
      let ptr = chunk.swap(null_mut(), Ordering::AcqRel);
      if !ptr.is_null() {
        // SAFETY: ptr 由 ensure_chunk 的 Box::into_raw 发布，swap 独占置空后无并发访问，
        // 布局为 [HashBucket; CHUNK_SIZE]，还原 Box 释放。
        unsafe {
