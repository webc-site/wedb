...
impl OverflowPool {
  ...
  /// libs/storage/Tsavorite/cs/src/core/Allocator/MallocFixedPageSize.cs:Allocate
  ...
  ///
  /// C# 索引→内存访问原语的统一落点（GetPhysicalAddress 的指针换算随借用引用消除）：
  /// libs/storage/Tsavorite/cs/src/core/Allocator/MallocFixedPageSize.cs:Get
  /// libs/storage/Tsavorite/cs/src/core/Allocator/MallocFixedPageSize.cs:GetPhysicalAddress
  ///
...
impl Drop for OverflowPool {
  /// 池析构释放全部已分配内存块（RAII 承担 C# Dispose）
  /// libs/storage/Tsavorite/cs/src/core/Allocator/MallocFixedPageSize.cs:Dispose
  fn drop(&mut self) {
