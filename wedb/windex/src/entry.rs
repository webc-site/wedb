...
/// 哈希桶条目（64位紧凑无锁结构）
///
/// 对标 C# Tsavorite 同名的 `HashBucketEntry` 结构（声明于
/// libs/storage/Tsavorite/cs/src/core/Index/Tsavorite/HashBucketEntry.cs），
/// 其 tag/tentative 位段与地址位段一并声明在本类型自身内，与 C# 同拓扑。
///
/// 内存排布（小端 64 位整型）：
...
impl HashBucketEntry {
  ...
  /// 指纹位长度（15 位 = 63 - 地址位宽，对标 C# HashBucketEntry.kTagSize）
  pub const TAG_BITS: u32 = 63 - Self::ADDRESS_BITS;
  /// 指纹在桶条目中的偏移量（48 位，对标 C# HashBucketEntry.kTagShift）
  pub const TAG_SHIFT: u32 = 63 - Self::TAG_BITS;
  /// 指纹有效值掩码（0x7FFF，对标 C# HashBucketEntry.kTagMask）
  pub const TAG_MASK: u64 = (1u64 << Self::TAG_BITS) - 1;
  /// 指纹在 64 位整型中的位置掩码（0x7FFF_0000_0000_0000，对标 C# HashBucketEntry.kTagPositionMask）
  pub const TAG_POS_MASK: u64 = Self::TAG_MASK << Self::TAG_SHIFT;

  /// 试探性标记位偏移量（第 63 位，对标 C# HashBucketEntry.kTentativeBitShift）
  pub const TENTATIVE_SHIFT: u32 = 63;
  /// 试探性标记位掩码（0x8000_0000_0000_0000，对标 C# HashBucketEntry.kTentativeBitMask）
  pub const TENTATIVE_MASK: u64 = 1u64 << Self::TENTATIVE_SHIFT;

  /// 哈希计算提取 Tag 的右移位数（64 - 15 = 49，对标 C# HashBucketEntry.kHashTagShift）
  ...
  /// 构造新的条目
  ///
  /// C# 可变设定器同挂此处（构造即整体置位，rust 条目为不可变值语义）：
  /// libs/storage/Tsavorite/cs/src/core/Index/Tsavorite/HashBucketEntry.cs:Set
  #[inline]
  ...
  ///
  /// 在 garnet 中的相对路径:libs/storage/Tsavorite/cs/src/core/Index/Tsavorite/HashBucketEntry.cs:GetTag
  #[inline]
