...
impl HashIndex {
  ...
  pub fn find_or_create_tag_by_hash_with_min_addr(
    ...
    // 链首主桶引用全程不变（对标 C# TsavoriteBase.cs:FindOrCreateTag 的
    // `hei.firstBucket = hei.bucket = table + bucketIndex` 装载：跨入溢出桶仅推进
    // bucket，句柄锁仲裁锚点始终指向主桶）
    ...
    loop {
      ...
      // 数据槽按序探查（末槽为溢出指针，不入扫描面，对标 C# h < EntriesPerBucket 常量循环）
      for slot in 0..DATA_ENTRIES {
        if let Some(hei) = check_slot(
          first_bucket,
          walker.curr,
          slot,
          tag,
  ...
  ///
  /// 记录脱钩调用侧：HashEntryInfo.cs 的 TryElide 法定锚由本 crate entry_info.rs try_set_invalid 单点持有
  /// （CAS 置 kInvalidAddress 摘除链上槽位，线性化点与失败回 false 口径一致）
