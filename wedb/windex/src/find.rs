...
impl HashIndex {
  ...
  pub fn find_tag_entry_by_hash_with_min_addr(
    ...
    // 链首主桶引用全程不变（对标 C# TsavoriteBase.cs:FindTag 的 firstBucket 装载：
    // 跨入溢出桶仅推进 bucket，句柄锁仲裁锚点始终指向 table[bucketIndex]）
    let first_bucket = self.get_bucket(bucket_idx);
    let mut walker = ChainWalker::new(first_bucket);
    loop {
      #[inline(always)]
      fn check_slot<'a>(
        first_bucket: &'a HashBucket,
        bucket: &'a HashBucket,
        slot: usize,
        tag: u16,
        min_valid_addr: u64,
      ...
      if let Some(hei) = check_slot(first_bucket, walker.curr, 0, tag, min_valid_addr) {
        return Some(hei);
      }
      if let Some(hei) = check_slot(first_bucket, walker.curr, 1, tag, min_valid_addr) {
        return Some(hei);
      }
      if let Some(hei) = check_slot(first_bucket, walker.curr, 2, tag, min_valid_addr) {
        return Some(hei);
      }
      if let Some(hei) = check_slot(first_bucket, walker.curr, 3, tag, min_valid_addr) {
        return Some(hei);
      }
      if let Some(hei) = check_slot(first_bucket, walker.curr, 4, tag, min_valid_addr) {
        return Some(hei);
      }
      if let Some(hei) = check_slot(first_bucket, walker.curr, 5, tag, min_valid_addr) {
        return Some(hei);
      }
      if let Some(hei) = check_slot(first_bucket, walker.curr, 6, tag, min_valid_addr) {
        return Some(hei);
      }
  ...
  pub(crate) fn find_exact_entry_by_hash(
    ...
    let tag = HashBucketEntry::tag_from_hash(hash);
    let first_bucket = self.get_bucket((hash as usize) & self.mask);
    let mut walker = ChainWalker::new(first_bucket);
    loop {
