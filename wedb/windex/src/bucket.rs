...
impl HashBucket {
  ...
  ///
  /// 在 garnet 中的相对路径:libs/storage/Tsavorite/cs/src/core/Index/Tsavorite/HashBucket.cs:TryPromoteLatch
...
impl<'a> BucketSharedGuard<'a> {
  ...
  /// 尝试将共享锁升级为独占锁守卫，失败时保留原共享锁守卫（供锁升级测试与调试断言）
...
/// HashIndex 按键闩转发域：桶闩 CAS 体唯一位点在本件 HashBucket 原语上，
/// HashIndex 侧只余按键寻址（`bucket_for_key`）与一行转调，无第二份锁实现
impl HashIndex {
  ...
  ///
  /// [`Self::try_lock_key_exclusive`] 的哈希入参对位：调用方已经
  /// `whasher::scoped_hash(prefix, key)` 全仓单点算定归属域哈希（会话物理
  /// 前缀种子，与事务键锁 `wtxn::TxnKeyEntryComparison::scoped_key_hash`
  /// 同一构造口径）后经本入口寻桶取闩——桶下标截断与 [`Self::bucket_for_key`]
  /// 同式（`hash & mask`）。消费面为 wkv TTL 键闩（`ttl.rs`），与 wkv rmw
  /// 窗口、wtxn 事务键锁三面同键同桶互斥
