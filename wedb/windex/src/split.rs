...
///
/// `min_valid_addr`：日志 begin 地址门（传 0 关闭）。低于它的槽位是已被物理截断的
/// 死条目，原位跳过、不迁两侧。
///
/// 防膨胀有意偏差（登记于 doc/zh/deviations.md「分裂期按 begin 过滤死条目」）：
/// C# 分裂内核只门 HeadAddress，分裂期不滤死条目——低于 begin 的
/// 死记录随双写分支落入两侧子桶，靠查找路径惰性 CAS 清退兜底（rust 同款清退已落地
/// 于 find.rs 的 `classify_slot`）。本函数在此之上按 begin 额外过滤：死条目双写会
/// 挤占新表 7 数据槽并无谓诱发溢出桶分配。过滤不丢活数据：日志地址单调、prev 链
/// 恒指向更旧地址，凡仅经低于 begin 锚点可达的记录其地址必同样低于 begin（早已
/// 物理回收）；查找侧对 < begin 条目一律清退、绝不解引用，可达性不因滤除而改变。
  ...
  T: FnMut(u64, usize) -> Option<u64>,
{
  ...
  loop {
    for slot in walker.curr.entries.iter().take(crate::DATA_ENTRIES) {
      ...
      // begin 门：物理截断死条目跳过迁移（ReadCache 条目第 47 位指示位使其数值
      // 不落入截断区，口径与 classify_slot 一致显式豁免；偏差论证见函数级注释）
      if min_valid_addr > 0 && !entry.is_read_cache() && addr < min_valid_addr {
        continue;
      }
...
///
/// 迁移第 `chunk_idx` 个分块中的所有桶。`min_valid_addr` 为 begin 地址门的
/// 防膨胀偏差口径，逐条目判定见 [`split_single_bucket`] 函数级注释
/// （登记于 doc/zh/deviations.md，非 C# SplitChunk 原生门控）。
