...
mod tests {
  ...
  #[test]
  fn test_dummy_object() {
    let obj = DummyObjectFactory::create(42);
    assert_eq!(obj.obj_type, 42);
    let cloned = obj.clone_object();
    assert_eq!(cloned.obj_type, 42);
    let (items, cursor) = obj.scan(0, 10, None, true);
