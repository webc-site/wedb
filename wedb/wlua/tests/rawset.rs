...
use wlua::{LuaLoggingMode, LuaMemoryManagementMode, LuaRunner, RespObject, RunnerCreateOptions};
...
fn run_script(source: &[u8]) -> Result<RespObject> {
  let mut runner = LuaRunner::new(
    ...
    &RunnerCreateOptions {
      ...
      log_mode: Some(LuaLoggingMode::default()),
      allowed_functions: Some(HashSet::default()),
      redis_version: DEFAULT_REDIS_VERSION.to_string(),
      ..Default::default()
    },
  )
  .map_err(Error::msg)?;
