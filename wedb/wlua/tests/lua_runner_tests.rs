...
fn internal_functions_ignored_in_allowed_functions() {
  ...
  let mut all_runner = LuaRunner::with_options(
    &opts,
    b"return tostring(garnet_call)",
    false,
    "0.0.0.0",
    TxnLockTable::new(),
  )
...
fn issue_1165() {
  ...
  let mut runner =
    LuaRunner::with_options(&opts, b"return 1", false, "0.0.0.0", TxnLockTable::new()).unwrap();
  let mut out = Vec::new();
  runner.compile_for_runner(&mut out).unwrap();
...
fn all_memory_modes_table_operations() {
  for mode in [
    ...
    "#;
    let mut runner =
      LuaRunner::with_options(&opts, script, false, "0.0.0.0", TxnLockTable::new()).unwrap();
    let res = compile_and_run(&mut runner).unwrap();
