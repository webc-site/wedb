...
fn redis_log_disabled() {
  ...
  let mut runner =
    LuaRunner::with_options(&opts, b"redis.log(redis.LOG_WARNING, 'foo')", "0.0.0.0").unwrap();
  let err = compile_and_run(&mut runner).unwrap_err();
...
fn internal_functions_ignored_in_allowed_functions() {
  ...
  let mut all_runner =
    LuaRunner::with_options(&opts, b"return tostring(garnet_call)", "0.0.0.0").unwrap();
  let res = compile_and_run(&mut all_runner).unwrap();
...
fn issue_1165() {
  ...
  let mut runner = LuaRunner::with_options(&opts, b"return 1", "0.0.0.0").unwrap();
  let mut out = Vec::new();
  runner.compile_for_runner(&mut out).unwrap();
...
fn all_memory_modes_table_operations() {
  for mode in [
    ...
    let script = br#"
      ...
      return #t
    "#;
    let mut runner = LuaRunner::with_options(&opts, script, "0.0.0.0").unwrap();
    let res = compile_and_run(&mut runner).unwrap();
...
#[test]
fn bit_tohex_operations() {
  let mut runner = new_runner("return bit.tohex(0x12345678, 8)");
  assert_eq!(
    resp_as_str(&compile_and_run(&mut runner).unwrap()),
    "12345678"
  );

  let mut runner = new_runner("return bit.tohex(0x12345678, 4)");
  assert_eq!(resp_as_str(&compile_and_run(&mut runner).unwrap()), "5678");

  let mut runner = new_runner("return bit.tohex(0x1234abcd, -4)");
  assert_eq!(resp_as_str(&compile_and_run(&mut runner).unwrap()), "ABCD");

  let mut runner = new_runner("return bit.tohex(0x1234abcd)");
  assert_eq!(
    resp_as_str(&compile_and_run(&mut runner).unwrap()),
    "1234abcd"
  );
