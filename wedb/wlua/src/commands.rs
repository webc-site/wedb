...
use wresp::cmd_strings::{GENERIC_ERR_WRONG_NUM_ARGS, RESP_ERR_GENERIC_VALUE_IS_NOT_INTEGER};

#[cfg(test)]
use crate::sender::ScratchBufferNetworkSender;
use crate::{
  api::ScriptingApi,
  cache::{LuaScriptHandle, RunnerCreateOptions, SHA1_LEN, SessionScriptCache},
...
impl LuaCommands {
  ...
  pub fn try_eval<S: ScriptingApi>(ctx: &mut LuaSessionContext<'_, S>) -> bool {
    ...
    if let Some(new_handle) = created {
      ...
      _ = ctx.store_cache.try_add(on_stack_script_key, new_handle);
    }
  ...
  pub fn network_script_load<S: ScriptingApi>(ctx: &mut LuaSessionContext<'_, S>) -> bool {
    ...
    if let Some(new_handle) = created {
      ...
      _ = ctx.store_cache.try_add(digest, new_handle);
    }
  ...
  pub fn check_lua_enabled<S: ScriptingApi>(ctx: &mut LuaSessionContext<'_, S>) -> bool {
    if !ctx.lua_enabled {
      let mut resp = RespOut::session(ctx.out, 2);
  ...
  /// 共享截止槽 = now + timeout（SetCookie 形态），run 结束 disarm 清 0
  ///（SetCookie(0) 形态）；到期激活由 LuaTimeoutManager.tick 承接。
  pub fn try_execute_script<S: ScriptingApi>(
    ctx: &mut LuaSessionContext<'_, S>,
    count: usize,
    script_key: &ScriptHashKey,
  ) -> bool {
    ...
    if let Some((registration, timeout_millis)) = &timeout {
      ...
      registration.arm(now_ms() as i64, *timeout_millis);
    }
...
impl ScriptingApi for NoopScriptingApi {
  /// 满足 ScriptingApi trait 契约，空会话无操作保留参数
  fn dispatch_resp(&mut self, _cmd: &[u8], _sender: &mut ScratchBufferNetworkSender) {}
  ...
  /// 满足 ScriptingApi trait 契约，空会话无操作保留参数
  fn check_acl_permissions(&self, _cmd_name: &str) -> bool {
    true
  }
}
