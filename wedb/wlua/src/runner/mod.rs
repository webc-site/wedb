...
mod resp_convert;
pub use host::{
  HostFn, HostShared, ScriptSessionPtr, ScriptSessionRef, ScriptingApiVtable, callback_context,
  clear_callback_context, set_callback_context,
};
pub use resp_convert::{
  RespObject, RespOut, lua_wrapped_error_view, process_resp_response_view,
  process_single_resp_term_view,
};
...
impl LuaRunner {
  ...
  ///
  /// 散参入 [`crate::RunnerCreateOptions`] 聚合承接（含所属引擎实例的
  /// `lock_table` 句柄，对标 C# 构造期自 `storeWrapper` 下传 `store.LockTable`）。
  pub fn new(source: Vec<u8>, options: &RunnerCreateOptions) -> Result<Self, LuaRunnerInitError> {
    ...
    let mut runner = Self {
      ...
      source,
      txn_mode,
      host: Box::new(HostShared::new(
        log_mode,
        txn_mode,
        options.lock_table.clone(),
      )),
  ...
  ///
  /// `lock_table` = 脚本所属引擎实例锁表句柄（同 [`Self::new`]，
  /// 对标 C# `LuaRunner(LuaOptions, ...)` 仍自 `storeWrapper` 取锁表）。
  pub fn with_options(
    options: &LuaOptions,
    source: &[u8],
    txn_mode: bool,
