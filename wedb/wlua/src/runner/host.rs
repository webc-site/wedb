...
pub struct HostShared {
  ...
  pub(crate) session: Option<ScriptSessionPtr>,
...
impl HostShared {
  /// 构造
  pub fn new(log_mode: LuaLoggingMode) -> Self {
    Self {
...
fn set_callback_context(context: *mut HostShared) {
...
fn clear_callback_context(context: *mut HostShared) {
...
type VtableGetFn = unsafe fn(*mut (), &[u8]) -> Result<Option<Vec<u8>>, ScriptApiError>;
type VtableSetFn = unsafe fn(*mut (), &[u8], &[u8]) -> Result<(), ScriptApiError>;

/// 脚本命令面函数表（静态函数指针，彻底消除动态分发 ScriptingApi）
pub(crate) struct ScriptingApiVtable {
  ...
  acl_mount_stale: unsafe fn(*const ()) -> bool,
  take_script_yield: unsafe fn(*mut ()) -> Option<i32>,
}
...
impl ScriptingApiVtable {
  ...
  pub const fn of<S: ScriptingApi>() -> Self {
    Self {
      ...
      acl_mount_stale: |ptr| unsafe { (*ptr.cast::<S>()).acl_mount_stale() },
      take_script_yield: |ptr| unsafe { (*ptr.cast::<S>()).take_script_yield() },
    }
  }
}
...
#[derive(Clone, Copy)]
pub(crate) struct ScriptSessionPtr {
  ptr: *mut (),
...
/// 会话命令面借用视图
pub(crate) struct ScriptSessionRef<'a> {
  ptr: *mut (),
...
impl ScriptingApi for ScriptSessionRef<'_> {
  ...
  fn acl_mount_stale(&self) -> bool {
    unsafe { (self.vtable.acl_mount_stale)(self.ptr) }
  }

  fn take_script_yield(&mut self) -> Option<i32> {
    unsafe { (self.vtable.take_script_yield)(self.ptr) }
  }
}

/// 宿主函数通用形态：操作栈镜像 + 宿主上下文，返回栈上结果数。
pub(crate) type HostFn = fn(&mut LuaState, &mut HostShared) -> i32;
