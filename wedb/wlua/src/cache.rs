...
impl SessionScriptCache {
  ...
  pub fn try_load(&mut self, hash: &ScriptHashKey, script: &[u8]) -> bool {
    self.scripts.entry(*hash).or_insert_with(|| script.to_vec());
    true
  }
  ...
  pub fn try_load_runner(
    ...
    let compiled_source = LuaRunnerLoaderProxy::compile_source(source);

    let mut runner = LuaRunner::new(
      options.log_mode.unwrap_or_default(),
      options.mem_mode.unwrap_or(LuaMemoryManagementMode::Native),
...
struct LuaRunnerLoaderProxy;

impl LuaRunnerLoaderProxy {
  fn compile_source(source: &[u8]) -> Vec<u8> {
    LuaRunnerLoader::compile_source(source)
  }
}
...
mod tests {
  ...
  fn running_lifecycle_and_timeout_assembly() {
    ...
    // arm：截止 ≈ now + timeout（真实时钟，界检查）。
    let arm_at = now_ms() as i64;
    ...
    let deadlines = manager.active_deadlines();
    assert_eq!(deadlines.len(), 1);
    assert_eq!(deadlines[0], arm_at + 300);

    // disarm：截止清 0。
    registration.disarm();
    assert_eq!(manager.active_deadlines(), vec![0]);
