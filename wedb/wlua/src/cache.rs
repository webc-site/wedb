...
pub struct RunnerCreateOptions {
  ...
  /// 脚本所在引擎实例的锁表句柄（对标 C# `LuaRunner` 构造期的
  /// `storeWrapper.store.LockTable`；键集加锁经该表，跨实例互不干扰）
...
impl SessionScriptCache {
  ...
  pub fn try_load_runner(
    ...
    let mut runner = LuaRunner::new(compiled_source.clone(), options).ok()?;
...
mod tests {
  ...
  use super::{LuaScriptHandle, SessionScriptCache, TxnLockTable};
  use crate::{
    hash_key::ScriptHashKey,
    options::{LuaLoggingMode, LuaMemoryManagementMode},
