...
pub const ERR_TEXT_ONLY_MODE: &str = "cannot load binary chunk in a text-only mode";
/// 源代码含 NUL 截断防护错误文案。
pub const ERR_NULL_BYTE: &str = "unexpected character near '\\0'";

/// 超时截止槽（`0` = 空闲；`>0` = 已设截止，单调毫秒；
/// [`TIMEOUT_TRIGGERED`](crate::timeout::TIMEOUT_TRIGGERED) = 到期已激活）。
///
/// VM 中断回调（safepoint 同线程）与超时管理器 tick 线程共读；
/// 值写入：run 开始/结束（VM 线程）与到期激活哨兵（tick 线程 CAS）。
pub type Deadline = AtomicI64;
...
pub struct LuaState {
  ...
  /// 超时截止槽（曾设过钩子后恒为 Some；外部注入共享槽时与登记方共同保活）。
...
impl LuaState {
  ...
  /// 本方法直接经 `sys::lua_ref` FFI 单跳完成同一功能。引用由注册表持有
  /// 至 VM 关闭（整 VM Drop 即 lua_close 一次回收），无逐引用弃用面。
  ...
  pub fn known_string_to_buffer(&self, idx: i32) -> Option<Vec<u8>> {
    self.known_string_to_slice(idx).map(<[u8]>::to_vec)
  }

  /// 取 `idx` 字符串的零分配只读切片（非串返回 None）。
  /// 只要栈顶不变且值在栈上，指针持续有效。
  pub fn known_string_to_slice(&self, idx: i32) -> Option<&[u8]> {
    if self.type_name(idx) != Some("string") {
      return None;
    }
    let mut len = 0usize;
    // SAFETY：串值在栈上（GC 锚定），指针在调用帧期间有效。
    let text = unsafe { sys::lua_tolstring(self.l, idx as c_int, &mut len) };
    if text.is_null() {
      return None;
    }
    Some(unsafe { from_raw_parts(text.cast::<u8>(), len) })
  }
  ...
  /// libs/server/Lua/LuaStateWrapper.cs:TrySetHook / 超时换挂单口
  ///
  /// 会话脚本超时的槽由 `TimeoutRegistration` 持有：run 开始时 VM 线程
  /// 换挂（本方法）+ 写截止；tick 线程到期 CAS 激活。Arc 双持（本状态
  /// 与登记方）保证 userdata 指针在 VM 存活期内有效。
  ///
  /// C# 的 TrySetHook 自带「无槽则新建」分支；本仓截止槽一律由登记方持有
  /// （VM 中断需与 tick 线程共享），故换挂单口只接受共享槽，不留第二出口。
  pub fn hook_shared_deadline(&mut self, slot: Arc<Deadline>) {
    self.install_interrupt(slot);
  }

  /// 装中断回调并锚定截止槽。
  fn install_interrupt(&mut self, slot: Arc<Deadline>) {
    // SAFETY：callbacks 指向 VM 全局回调集（VM 存活期内有效）；
    // userdata 为 Luau 承诺不覆写的宿主槽，指向随 Arc 保活的截止槽
    //（self.deadline 持克隆，注入方持原件）。
    unsafe {
      let callbacks = sys::lua_callbacks(self.l);
      (*callbacks).userdata = Arc::as_ptr(&slot).cast_mut().cast();
  ...
  /// libs/server/Lua/NativeMethods.cs:GC（LuaGC.GCStep 臂）
  ///
  /// 增量 GC 显式步进（Luau LUA_GCSTEP=6，非标准 Lua 的 5；data 单位为 KB）。
  /// 脚本收尾调用：空闲期无分配驱动的 GC 辅助，步进推进死对象回收，
  /// 使配额分配器的 used_bytes 及时回落，避免后续请求误触上限（伪 OOM）。
  /// 返回本轮周期是否已完成（1 = 周期结束）。
  pub fn gc_step(&mut self, data: i32) -> i32 {
    // SAFETY：有效 VM；步进为纯维护操作，不动栈。
    unsafe { sys::lua_gc(self.l, sys::LUA_GCSTEP, data as c_int) }
  }
  ...
  /// 派生协程线程并压栈（`sys::lua_newthread` 薄封装）：线程值留在栈上，
  /// 由调用方持有栈位锚定（弹出即失锚，GC 可回收挂起协程）。
  pub fn new_thread(&mut self) -> *mut sys::lua_State {
    // SAFETY：有效 VM；返回线程指针随 VM 存活，栈位即锚。
    unsafe { sys::lua_newthread(self.l) }
  }

  /// 协程续跑（`sys::lua_resume` 薄封装）：`nargs` 个参数须已压入栈顶。
  /// `from = NULL`：每次续跑 C 调用深度归零（多次 yield/resume 循环无累计
  /// 误触 LUAI_MAXCCALLS）。lua_resume 不跨本帧长跳转——错误转状态码并把
  /// 错误对象压至栈底 1 位（与 pcall errfunc=0 的错误形态一致）。
  pub fn resume(&mut self, nargs: usize) -> c_int {
    // SAFETY：l 为非运行态线程；参数已压栈（调用方保证）。
    unsafe { sys::lua_resume(self.l, ptr::null_mut(), nargs as c_int) }
  }

  /// 线程状态快照（[`sys::LUA_OK`]/[`sys::LUA_YIELD`]/[`sys::LUA_ERRRUN`] 等）。
  pub fn status(&self) -> c_int {
    // SAFETY：仅读线程状态标量。
    unsafe { sys::lua_status(self.l) }
  }

  /// C 函数内挂起协程（`sys::lua_yield` 薄封装）：`nresults` 个让渡值须已
  /// 压入栈顶。返回 -1 须由 C 函数原样上返——VM 见负返回即退出执行循环，
  /// 再次 resume 时本 C 函数以 resume 前压入的参数值收帧返回。
  pub fn script_yield(&mut self, nresults: usize) -> c_int {
    // SAFETY：有效线程；让渡值已压栈（调用方保证），nresults <= top - base。
    unsafe { sys::lua_yield(self.l, nresults as c_int) }
  }

  /// 运行线程换岗：窗口期把状态层 VM 指针切到协程线程（[`Self::new_thread`]
  /// 产物），Drop（含 panic 展开）还原主线程指针。
  ///
  /// 语义依据：`lua_State` 的回调集（`global->cb`，含超时中断）、分配器、
  /// 注册表与全局表均为 **VM 级共享**，线程仅持独立数据栈与 CallInfo 链——
  /// 切换后回调机器、GC 锚定与超时中断语义不变，`self` 的一切栈操作改落
  /// 协程线程。窗口纪律：同一时刻至多一个守卫，窗口期内不得再派生守卫。
  pub fn swap_thread(&mut self, thread: *mut sys::lua_State) -> ThreadSwapGuard {
    ThreadSwapGuard {
      state: self,
      old_l: mem::replace(&mut self.l, thread),
    }
  }
...
/// 运行线程换岗守卫（[`LuaState::swap_thread`] 产物）：裸指针持有状态层，
/// 不延长借用——窗口期内调用方仍可经 `&mut LuaState` 正常操作（已落协程
/// 线程），Drop 还原主线程指针。
pub struct ThreadSwapGuard {
  /// 换岗的状态层（swap_thread 借出，窗口期内不移动）。
  state: *mut LuaState,
  /// 还原用的主线程指针。
  old_l: *mut sys::lua_State,
}

impl Drop for ThreadSwapGuard {
  fn drop(&mut self) {
    // SAFETY：state 由 swap_thread 借出；守卫活期内状态层未被移动
    // （&mut 独占借用纪律，见 swap_thread 窗口契约）。
    unsafe { (*self.state).l = self.old_l };
  }
}

/// 中断回调：safepoint 触发，截止槽已置哨兵即抛超时错误（可被脚本层 pcall
/// 截获一次，截获后下个 safepoint 续抛，直至上抛宿主）。
/// 满足 Luau 中断回调 C 签名规范
///
/// libs/server/Lua/LuaRunner.Functions.cs:RequestTimeout 的承接：C# 看门狗
/// 到期才 sethook（未激活期 VM 原生执行零时钟开销，LuaRunner.ResetTimeout
/// 平时全卸钩），单指令钩子抛错的合并体。luau 以常驻 safepoint 中断回调
/// 直达同一效果：本回调只做一次截止槽原子哨兵读（== TIMEOUT_TRIGGERED 即
/// `lua_error` 抛 TIMEOUT_ERROR），零时钟系统调用——计时判决权全在看门狗
/// tick（timeout.rs LuaTimeoutManager::tick 单调时钟 CAS），热路径与
/// C# 的「未激活零成本」形态对齐。
///
/// # Safety（回调契约）
/// - userdata 指向随 Arc 保活的截止槽（install_interrupt 注入）。
/// - lua_error 长跳转：本帧内除 POD 外无存活析构对象。
/// - `count`: 满足 Luau C ABI 中断钩子签名规范，-1 为 VM 指令/回边 safepoint，>=0 为 GC 步进。
///   压入错误信息串会触发 GC 步进回调，故仅在指令级 safepoint（count < 0）响应中断；
///   GC 步进重入由 count>=0 早退分支覆盖，压串触发的 GC 步不会递回指令级中断。
///   无防重入哨兵：超时错误在截止槽激活期内于每个指令 safepoint 持续重抛
///   （对齐 C# 每指令 sethook 重抛、脚本层 pcall 吞不断），错误被脚本层保护点
///   截获后下个 safepoint 即再抛，直至越过无保护帧上抛宿主。
unsafe extern "C" fn interrupt_trampoline(l: *mut sys::lua_State, count: c_int) {
  if count >= 0 {
    return;
  }
  // SAFETY：callbacks 指针有效；userdata 由 install_interrupt 置入。
  ...
  // SAFETY：slot 随 Arc 存活；tick 线程 CAS 写哨兵经 AcqRel 序、此处 Acquire 读。
  let deadline = unsafe { (*slot).load(Ordering::Acquire) };
  if deadline == TIMEOUT_TRIGGERED {
    // SAFETY：压串后长跳转至宿主 lua_pcall；跳越帧无析构对象。压串触发的
    // GC 步以 count>=0 分支早退，不递回指令级中断。
    unsafe {
