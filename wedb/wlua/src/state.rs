...
impl LuaState {
  ...
  ///
  /// 共享槽形态（[`Self::hook_shared_deadline`] 注入）下同槽写值：
  /// tick 线程与 VM 线程经原子序可见。
  pub fn try_set_hook(&mut self, deadline_monotonic_millis: Option<i64>) {
    match &self.deadline {
      Some(slot) => slot.store(deadline_monotonic_millis.unwrap_or(0), Ordering::Release),
      None => {
        let slot = Arc::new(Deadline::new(deadline_monotonic_millis.unwrap_or(0)));
