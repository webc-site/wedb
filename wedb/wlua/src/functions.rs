...
//! （见 LuaRunner::register）在真实栈上直调本文件函数，函数签名统一为
...
use crate::{
  ...
  runner::{
    ...
    process_resp_response_view, set_callback_context,
  },
  sender::ScratchBufferBuilder,
...
impl LuaRunnerFunctions {
  ...
  /// Rust 参数本就为字节，恒直返；物理删除未使用的 ScratchBufferBuilder 参数）。
  pub fn prepare_string(raw: &[u8]) -> &[u8] {
    raw
  }
  ...
  /// RunInTransaction 包络）。
  pub fn garnet_call_with_transaction(state: &mut LuaState, host: &mut HostShared) -> i32 {
  ...
  pub fn request_timeout_fn(state: &mut LuaState, host: &mut HostShared) -> i32 {
  ...
  pub fn log(state: &mut LuaState, host: &mut HostShared) -> i32 {
    ...
    for arg_ix in 2..=arg_count {
      let buff: Option<Vec<u8>> = match state.type_name(arg_ix) {
  ...
  pub fn c_json_encode(state: &mut LuaState, host: &mut HostShared) -> i32 {
    ...
    if ret == 1 {
      ...
      state.push_buffer(host.scratch.view_full_arg_slice());
    }
  ...
  pub fn acl_check_command(state: &mut LuaState, host: &mut HostShared) -> i32 {
    ...
    let success = if is_bit_op_parent {
      ...
      SUB_COMMANDS.iter().all(|cmd| session.check_acl_permissions(cmd))
    } else {
  ...
  fn try_fast_path_set(
    ...
    if !session.check_acl_permissions("SET") {
      return Some(lua_wrapped_error_view(
        state,
        1,
    ...
    if let Err(err) = session.set(&key, &value) {
      return Some(lua_wrapped_error_view(state, 1, err.as_bytes()));
    }
  ...
  fn try_fast_path_get(
    state: &mut LuaState,
    host: &mut HostShared,
    arg_count: i32,
    cmd: &[u8],
  ) -> Option<i32> {
    if !cmd.eq_ignore_ascii_case(b"GET") || arg_count != 2 {
      return None;
    }
    let Some(session) = host.session.as_mut() else {
    ...
    let mut session = session.get();
    if !session.check_acl_permissions("GET") {
      return Some(lua_wrapped_error_view(
        state,
        1,
        ConstantStrings::ERR_NO_PERM,
    ...
    match session.get(&key) {
      ...
      Err(err) => Some(lua_wrapped_error_view(state, 1, err.as_bytes())),
    }
  }
  ...
  fn dispatch_scripting_command_fallback(
    ...
    let mut session = session.get();
    session.dispatch_resp(host.scratch.view_full_arg_slice(), &mut host.sender);
    let resp_protocol_version = session.resp_protocol_version();

    let result =
      process_resp_response_view(state, resp_protocol_version, host.sender.get_response());
  ...
  pub fn process_command_from_scripting(state: &mut LuaState, host: &mut HostShared) -> i32 {
    ...
    if let Some(res) = Self::try_fast_path_set(state, host, arg_count, &cmd) {
      return res;
    }
    if let Some(res) = Self::try_fast_path_get(state, host, arg_count, &cmd) {
      return res;
    }

    // As fallback, we format a RESP request and dispatch it through the session.
    Self::dispatch_scripting_command_fallback(state, host, arg_count, &cmd)
  }
  ...
  pub fn garnet_call_no_session(state: &mut LuaState, host: &mut HostShared) -> i32 {
    Self::no_session_response(state, host)
  }

  /// libs/server/Lua/LuaRunner.Functions.cs:GarnetCallNoTransaction（trampoline 形态）
  pub fn garnet_call_no_transaction(state: &mut LuaState, host: &mut HostShared) -> i32 {
...
fn stack_bytes(state: &mut LuaState, index: i32) -> Option<Vec<u8>> {
...
#[cfg(test)]
mod tests {
  use super::{LuaRunnerFunctions, format_number_g, lua_number_to_bit_value};
  use crate::{LuaState, cache::SessionScriptCache, functions_struct as struct_codec};
