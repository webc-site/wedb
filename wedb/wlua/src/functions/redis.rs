...
use crate::{
  LuaState,
  api::{ScriptApiError, ScriptingApi},
...
impl LuaRunnerFunctions {
  ...
  fn frame_and_acl_check(
    ...
    match session.parse_resp_command_buffer(scratch) {
      Some(cmd) => {
        // 窗内改权收口（deviations §159）：挂载代数落后时本位图门给的是
        // 陈旧裁决（撤权后仍回 true，与 C# :2993 现读新句柄反向分叉），
        // 而刷新臂同步窗口内不可达——不回陈旧 bool，报同文案确定性错误
        // 中断脚本（本面纯判定零存储动作，与 dispatch_resp 环尾收口同型）
        if session.acl_mount_stale() {
          return Err(ScriptApiError::ACL_CHANGED_TEXT.as_bytes());
        }
        Ok(session.check_acl_permissions(cmd))
      }
  ...
  /// libs/server/Lua/LuaRunner.Functions.cs:PrepareString（string → UTF-8
  /// 字节 span，缓冲不足重建 ArgSlice）的承接为本函数 `string` 分支——
  /// rust Lua 串本就 UTF-8 字节，零重编码直写 `RespWriter`（拼装单点收敛
  /// 至 wresp::resp_memory_writer，恒 RESP2：请求面无 RESP3 形态）。
  ///
      ...
      match state.type_name(stack_ix) {
        ...
        Some("number") => {
          if let Some(num) = state.check_number(stack_ix) {
            // 整值 double 直写精确 i64 文本系刻意差异：C# 侧（标准 Lua 5.4，
            // KeraLua "lua54"）形参面一律经 lua_tolstring 转串，float 子类型
            // 出 "%.14g" 文本（123.0→"123.0"、2^62→"4.6116860184274e+18"、
            // -0.0→"-0.0"），双侧在 float 子类型全域逐字节分叉——Luau 单
            // double 型无 integer 子类型，子类型在 rust 侧不可观测，登记于
            // deviations §112（与第 1 条最短往返裁决同向），严禁按 C# 文本形态回改。
  ...
  fn try_fast_path_set(state: &mut LuaState, host: &mut HostShared, arg_count: i32) -> Option<i32> {
    ...
    // C# 走存储 API 直连：`_ = api.SET(key, value)` 显式丢弃 status 恒推
    // +OK（LuaRunner.Functions.cs:3214-3216），错误应答对脚本不可见。rust
    // 侧重入 RESP 会话会产错误帧，ErrorReply 形态按原型折叠为成功；唯
    // Protocol（应答不可解析，无 C# 对应态）与 AclChanged（窗内改权停车
    // 收口，deviations §159）上抛。与 fallback 臂（'-' 帧转错误上抛）的
    // 不对称系刻意设计，勿按 fallback 形态「统一」改回。
    match session.set(key, value) {
      Err(ScriptApiError::Protocol) => {
        ...
        return Some(lua_wrapped_error_view(
          state,
          1,
          ScriptApiError::PROTOCOL_TEXT,
        ));
      }
      Err(ScriptApiError::AclChanged) => {
        state.clear_stack();
        return Some(lua_wrapped_error_view(
          state,
          1,
          ScriptApiError::ACL_CHANGED_TEXT.as_bytes(),
        ));
      }
      Ok(()) | Err(ScriptApiError::ErrorReply) => {}
    }
  ...
  fn try_fast_path_get(state: &mut LuaState, host: &mut HostShared, arg_count: i32) -> Option<i32> {
    ...
    match res {
      ...
      // C# 存储 API 直连下非 OK status（NOTFOUND 与 WRONGTYPE 等一切，
      // LuaRunner.Functions.cs:3246-3259）一律 PushBoolean(false) 脚本继续；
      // rust 重入会话产的错误帧按该语义折叠。AclChanged（窗内改权停车
      // 收口，deviations §159）与 Protocol 同族上抛。与 fallback 臂（'-'
      // 帧转错误上抛）的不对称系刻意设计，勿按 fallback 形态「统一」改回。
      Ok(None) | Err(ScriptApiError::ErrorReply) => {
        // Redis is weird, but false instead of Nil is correct here
        state.push_boolean(false);
        Some(1)
      }
      Err(ScriptApiError::AclChanged) => Some(lua_wrapped_error_view(
        state,
        1,
        ScriptApiError::ACL_CHANGED_TEXT.as_bytes(),
      )),
      Err(ScriptApiError::Protocol) => Some(lua_wrapped_error_view(
        state,
        1,
        ScriptApiError::PROTOCOL_TEXT,
      )),
  ...
  fn dispatch_scripting_command_fallback(
    ...
    let result = if host.response.is_empty() {
      0
    } else {
      process_resp_response_view(state, resp_protocol_version, &host.response)
    };
  ...
  pub fn process_command_from_scripting(state: &mut LuaState, host: &mut HostShared) -> i32 {
    ...
    // 快路径优先，未命中落 RESP 分派；两路共用挂起让渡收尾（协程化挂起协议
    // 单点，见 [`Self::settle_script_call`]）。
    let result = Self::try_fast_path_set(state, host, arg_count)
      .or_else(|| Self::try_fast_path_get(state, host, arg_count))
      .unwrap_or_else(|| Self::dispatch_scripting_command_fallback(state, host, arg_count));
    Self::settle_script_call(state, host, result)
  }

  /// redis.call 收尾单点（协程化挂起协议）：命中会话置位的挂起让渡标记即
  /// 改挂协程——清回调帧栈、压让渡标记、`lua_yield` 挂起（返回 -1 经蹦床
  /// 原样上返，VM 见负返回即出执行循环；再次 resume 时本 C 函数以 resume
  /// 前压入的应答转换值收帧，redis.call 的脚本侧返回值由此回填）。
  /// C# 侧脚本内命令在回调栈上同步收割（BlockingWait 内联），rust 协程化
  /// 后挂起点唯一收敛于此。
  fn settle_script_call(state: &mut LuaState, host: &mut HostShared, result: i32) -> i32 {
    let Some(tag) = host
      .session
      .as_mut()
      .and_then(|session| session.get().take_script_yield())
    else {
      return result;
    };
    state.clear_stack();
    state.push_integer(i64::from(tag));
