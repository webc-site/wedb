...
impl LuaRunnerFunctions {
  ...
  ///
  /// redis.call 统一入口：C# 按事务形态分注册 GarnetCallWithTransaction /
  /// GarnetCallNoTransaction 两个 trampoline
  /// （在 garnet 中的相对路径:libs/server/Lua/LuaRunner.Functions.cs:GarnetCallWithTransaction、
  /// 在 garnet 中的相对路径:libs/server/Lua/LuaRunner.Functions.cs:GarnetCallNoTransaction，
  /// 分别走 transactionalGarnetApi / basicGarnetApi）；
  /// Rust 侧两形态合并承接，事务面由 host.txn_mode 与会话事务窗口决定。
