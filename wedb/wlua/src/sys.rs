...
/// 线程/调用状态码（lua.h `lua_Status` 枚举序）。
/// 保护调用成功状态。
pub const LUA_OK: c_int = 0;
/// 线程挂起（lua_yield 后 / 可 lua_resume 续跑）。
pub const LUA_YIELD: c_int = 1;

/// 增量 GC 显式步进（lua.h `lua_GCOp` 枚举序：Luau 的 LUA_GCISRUNNING 占 5，
/// 故 LUA_GCSTEP = 6，不同于标准 Lua 5.1/5.4 的 5）。
...
unsafe extern "C" {
  ...
  // ---- 协程（线程）----
  /// 派生协程线程并压栈（`lua.h:lua_newthread`；线程对象随栈位 GC 锚定）。
  pub fn lua_newthread(l: *mut lua_State) -> *mut lua_State;
  /// 续跑/启动协程（`ldo.cpp:lua_resume`）：`narg` 个参数须已在 `l` 栈顶。
  /// 不跨本帧长跳转——错误转状态码并压错误对象；返回 [`LUA_OK`] /
  /// [`LUA_YIELD`]（挂起，让渡值在 `L->base..top`）/ [`LUA_ERRRUN`] 等。
  pub fn lua_resume(l: *mut lua_State, from: *mut lua_State, narg: c_int) -> c_int;
  /// 从 C 函数挂起协程（`ldo.cpp:lua_yield`）：`nresults` 个值须已在 `l`
  /// 栈顶。**不抛错**——置 `LUA_YIELD` 状态后返回 -1，须由 C 函数原样
  /// `return lua_yield(...)` 交还 VM（LOP_CALL 见负返回即出执行循环）；
  /// 再次 `lua_resume` 时该 C 函数以 resume 前压入的 `narg` 个值作为自身
  /// 返回值收帧（cont == NULL 路径 `luau_poscall(L, firstArg)`）。
  pub fn lua_yield(l: *mut lua_State, nresults: c_int) -> c_int;
  /// 线程状态快照（[`LUA_OK`]/[`LUA_YIELD`]/[`LUA_ERRRUN`] 等）。
  ...
  // ---- GC ----
  /// GC 控制（对标 NativeMethods.cs:675 lua_gc 声明）；what 取 [`LUA_GCSTEP`] 等
  /// `lua_GCOp` 枚举值，data 为步进量（KB）。
