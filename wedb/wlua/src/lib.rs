...
pub use api::ScriptingApi;
pub use cache::{RunnerCreateOptions, SessionScriptCache};
...
pub use limited_allocator::{BlockRef, LuaLimitedManagedAllocator};
...
pub use timeout::{LuaTimeoutManager, TimeoutRegistration};
...
mod tests {
  ...
  use super::{Deadline, Error, LuaState};
  ...
  fn timeout_interrupt_raises_error() {
    let mut state = LuaState::new();
    // 生产换挂单口：共享截止槽置为已过期 → 下一 safepoint（循环回边）即中断。
    let slot = Arc::new(Deadline::new(now_ms() as i64));
    ...
    state.clear_stack();
    // 同槽清 0 撤销截止（tick 线程同款形态）后脚本可正常跑完。
