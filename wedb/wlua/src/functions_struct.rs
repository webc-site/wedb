...
//! （对标 libs/server/Lua/LuaRunner.Functions.Struct.cs 编解码层）。
//!
...
pub(crate) const fn default_options() -> FormatOptions {
  FormatOptions {
...
/// 解包结果：值序列 + 消费位置。
#[derive(Debug, Clone, PartialEq)]
pub(crate) struct StructUnpackOut {
  /// 解码值序列。
...
pub(crate) fn struct_size(format: &[u8]) -> Option<usize> {
  let mut header = default_options();
  let mut pos = 0usize;
