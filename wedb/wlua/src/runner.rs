...
//! Rust 以 [`wlua`] 的 C 蹦床（catch_unwind + thread-local 上下文）承接同
//! 语义。会话上下文经 thread-local [`HostShared`] 指针传递
...
impl<'a> RespOut<'a> {
  ...
  pub fn writer2(&mut self) -> wresp::RespWriter<&mut Vec<u8>, wresp::Resp2> {
    wresp::RespWriter::new_ref(self.buf)
  }

  /// 获取底层统一 RESP3 写出器
  #[inline(always)]
  pub fn writer3(&mut self) -> wresp::RespWriter<&mut Vec<u8>, wresp::Resp3> {
    wresp::RespWriter::new_ref_p(self.buf)
  }

  /// 获取指定协议标记的底层写出器
  #[inline(always)]
  pub fn writer_p<P: wresp::RespProtocol>(&mut self) -> wresp::RespWriter<&mut Vec<u8>, P> {
    wresp::RespWriter::new_ref_p(self.buf)
  }
  ...
  pub fn write_bulk_string(&mut self, value: &[u8]) {
    self.writer2().write_bulk_string(value);
  }
  ...
  /// 写 RESP simple string（`+<str>\r\n`）。
  #[inline]
  pub fn write_simple_string(&mut self, value: &[u8]) {
    self.writer2().write_simple_string_bytes(value);
  }

  /// 直写字节。
  #[inline]
  pub fn write_direct(&mut self, bytes: &[u8]) {
    self.writer2().write_direct(bytes);
  }

  /// RESP2 null（`$-1\r\n`）。
  #[inline]
  pub fn write_resp2_null(&mut self) {
    self.writer2().write_resp2_null();
  }

  /// RESP3 null（`_\r\n`）。
  #[inline]
  pub fn write_resp3_null(&mut self) {
    self.writer3().write_resp3_null();
  }

  /// 协议自适应 null（RESP3 为 `_\r\n`，RESP2 为 `$-1\r\n`）。
  #[inline]
  pub fn write_null(&mut self) {
    if self.protocol_version >= 3 {
      self.writer3().write_null();
    } else {
      self.writer2().write_null();
    }
  }
  ...
  pub fn write_array_len(&mut self, len: usize) {
    self.writer2().write_array_len(len);
  }
  ...
  pub fn write_map_len(&mut self, len: usize) {
    if self.protocol_version >= 3 {
      self.writer3().write_map_len(len);
    } else {
      self.writer2().write_map_len(len);
    }
  }
  ...
  pub fn write_set_len(&mut self, len: usize) {
    if self.protocol_version >= 3 {
      self.writer3().write_set_len(len);
    } else {
      self.writer2().write_set_len(len);
    }
  }
  ...
  /// 协议自适应 boolean（RESP3 为 `#t`/`#f`，RESP2 降级为 `:1`/`:0`）。
  #[inline]
...
pub struct HostShared {
  ...
  /// redis.call 响应接收面。
  pub sender: ScratchBufferNetworkSender,
...
impl HostShared {
  ...
  pub fn new(log_mode: LuaLoggingMode, txn_mode: bool) -> Self {
    Self {
      ...
      sender: ScratchBufferNetworkSender::new(),
      preamble_keys: None,
      preamble_argv: None,
...
pub struct ScriptingApiVtable {
  dispatch_resp: unsafe fn(*mut (), &[u8], &mut ScratchBufferNetworkSender),
  get: VtableGetFn,
  set: VtableSetFn,
  resp_protocol_version: unsafe fn(*const ()) -> u8,
  update_resp_protocol_version: unsafe fn(*mut (), u8),
  check_acl_permissions: unsafe fn(*const (), &str) -> bool,
...
impl ScriptingApiVtable {
  ...
  pub const fn of<S: ScriptingApi>() -> Self {
    Self {
      dispatch_resp: |ptr, req, sender| unsafe { (*ptr.cast::<S>()).dispatch_resp(req, sender) },
...
impl ScriptingApi for ScriptSessionRef<'_> {
  fn dispatch_resp(&mut self, request: &[u8], sender: &mut ScratchBufferNetworkSender) {
    unsafe { (self.vtable.dispatch_resp)(self.ptr, request, sender) }
  }
...
impl LuaRunner {
  ...
  pub fn new(
    log_mode: LuaLoggingMode,
    mem_mode: LuaMemoryManagementMode,
    ...
    Self::register(
      ...
      LuaRunnerFunctions::request_timeout_fn,
    )?;
    ...
    macro_rules! ref_global {
      ($field:ident, $name:expr, $err:expr) => {{
        ...
        runner.$field = runner.state.try_ref();
        if runner.$field <= 0 {
          return Err(LuaRunnerInitError($err.into()));
        }
      }};
  ...
  fn compile_common(&mut self, out: &mut Vec<u8>) {
    ...
    if call_res.is_ok() && self.state.get_top() == 2 && !self.state.to_boolean(1) {
      ...
      out.clear();
      wresp::RespWriter::new_ref(out).write_error(&err_str);
    }
  ...
  fn try_reset_parameters(&mut self, n_keys: usize, n_args: usize) -> Result<(), crate::Error> {
    if self.key_length > n_keys || self.argv_length > n_args {
      if !self.state.push_ref(self.reset_keys_and_argv_registry_index) {
        return Err(crate::Error::Misuse("reset_keys_and_argv ref missing"));
      }
  ...
  /// 写出 RESP simple string 并自栈顶弹出（Redis status_reply 对标）。
  pub fn try_write_simple_string(
    runner: &mut LuaRunner,
    resp: &mut RespOut,
    _err: &mut Option<&'static [u8]>,
  ) -> bool {
  ...
  pub fn try_write_resp3_boolean(
    ...
    resp.write_resp3_bool(runner.state.to_boolean(cur_top));
    runner.state.pop(1);
    true
  }
...
fn resp_out_error(out: &mut Vec<u8>, msg: &[u8]) {
  out.clear();
  wresp::RespWriter::new_ref(out).write_error_bytes(msg);
}
