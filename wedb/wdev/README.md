...
- `SegmentedDevice`: segmented-file device; the capacity / preallocate / read-only / delete-on-close knobs are injected only through `DeviceParams` at construction and immutable at runtime
...
- `SegmentedDevice`：段文件设备；容量上限 / 预分配 / 只读 / 关闭即删四旋钮只经 `DeviceParams` 构造注入，运行期不可变
