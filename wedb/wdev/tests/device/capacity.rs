...
/// 单文件无界模式 + 有界容量的硬上界防御（Rust 补齐语义，C# HandleCapacity 在
/// segmentSize = -1 时位移归约失效、容量形同虚设）：越界写入必须拒绝为 OutOfBounds，
/// 容量内写入不受影响。
#[test]
fn single_file_bounded_capacity_rejects_out_of_bounds_writes() -> Void {
  ...
  rt.block_on(async {
    let dir = tempdir()?;
    let mut device = SegmentedDevice::single_file(dir.path().join("cap_single.log"))?;
    let cap = 4096u64 * 2;
    device.set_capacity(Some(cap))?;
    ...
    // 巨偏移溢出同样拦截（offset + len 回绕不产生假越界通过）
