...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.hlog/LogRecoverReadOnlyTests.cs（设备恢复装载）
...
fn recovery_larger_existing_segment_detects_mismatch() -> Void {
  ...
  rt.block_on(async {
    ...
    {
      let device = SegmentedDevice::new(&db_path, big_seg, DEFAULT_SECTOR_SIZE)?;
    ...
    let device = SegmentedDevice::new(&db_path, small_seg, DEFAULT_SECTOR_SIZE)?;
...
/// 恢复扫描遇中部空洞时 fail-fast 显式上抛 SegmentGap，杜绝静默跳过已提交历史。
#[test]
fn recovery_middle_gap_fails_fast() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let seg_size: u64 = 64 * 1024;
    let db_path = dir.path().join("recover_middle_gap.log");

    // 写入段 0..=6
    {
      let device = SegmentedDevice::new(&db_path, seg_size, DEFAULT_SECTOR_SIZE)?;
      for seg_id in 0..=6u32 {
        let buf = AlignedBuf::from_slice(&[(seg_id as u8 + 1); 4096], 4096)?;
        let (res, _) = device.write_aligned((seg_id as u64) * seg_size, buf).await;
        assert_eq!(res?, 4096);
      }
      device.sync().await?;
    ...
    // 模拟中间崩溃或外部删文件导致中部空洞：物理删除段 5
    let device = SegmentedDevice::new(&db_path, seg_size, DEFAULT_SECTOR_SIZE)?;
    remove_file(device.segment_path(5))?;

    // 重开恢复：检测到段号空隙（缺失段 5），必须 fail-fast 报 SegmentGap { gap: 5 }
    let recovered = device.recover();
    assert!(
      matches!(recovered, Err(Error::SegmentGap { gap: 5 })),
      "遇中部空洞必须 fail-fast 报 SegmentGap {{ gap: 5 }}，实际为 {recovered:?}"
    );

    // 段 0..=4 物理文件依然保留，未被静默抛弃
    ...
    info!("恢复中部空洞快速失败检测通过 (recovery_middle_gap_fails_fast)");
    aok::Result::<()>::Ok(())
  })?;
...
/// erase_tail_after 降序删除孤儿段：消除中间崩溃产生中部空洞的可能，
/// 恢复后前缀连续数据完好。
#[test]
fn erase_tail_after_descending_order_and_recover() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let seg_size: u64 = 64 * 1024;
    let db_path = dir.path().join("erase_tail.log");

    // 写入段 0..=4
    let device = SegmentedDevice::new(&db_path, seg_size, DEFAULT_SECTOR_SIZE)?;
    for seg_id in 0..=4u32 {
      let buf = AlignedBuf::from_slice(&[(seg_id as u8 * 7 + 3); 4096], 4096)?;
      let (res, _) = device.write_aligned((seg_id as u64) * seg_size, buf).await;
      assert_eq!(res?, 4096);
    }
    device.sync().await?;
    assert_eq!(device.end_segment(), Some(4));

    // 从段 2 之后擦尾（提交点位于段 2 偏移 4096 处）
    device.erase_tail_after(2 * seg_size + 4096).await?;

    // 孤儿段 3、4 必须已被降序物理删除
    assert!(!device.segment_path(3).exists(), "孤儿段 3 必须已删除");
    assert!(!device.segment_path(4).exists(), "孤儿段 4 必须已删除");
    // 提交段及之前段完好保留
    assert!(device.segment_path(0).exists(), "段 0 必须保留");
    assert!(device.segment_path(1).exists(), "段 1 必须保留");
    assert!(device.segment_path(2).exists(), "段 2 必须保留");
    assert_eq!(device.end_segment(), Some(2));

    // 重开恢复：前缀连续，start=0，end=2
    {
      let reopened = SegmentedDevice::new(&db_path, seg_size, DEFAULT_SECTOR_SIZE)?;
      reopened.recover()?;
      assert_eq!(reopened.start_segment(), 0);
      assert_eq!(reopened.end_segment(), Some(2));
    }

    // 仿真崩溃序列：若擦尾按降序先删 4 后崩溃（保留 0..=3），恢复依然前缀连续无空隙
    {
      let crash_dir = tempdir()?;
      let crash_path = crash_dir.path().join("crash.log");
      let d = SegmentedDevice::new(&crash_path, seg_size, DEFAULT_SECTOR_SIZE)?;
      for seg_id in 0..=4u32 {
        let buf = AlignedBuf::from_slice(&[0x11; 4096], 4096)?;
        let (res, _) = d.write_aligned((seg_id as u64) * seg_size, buf).await;
        assert_eq!(res?, 4096);
      }
      d.sync().await?;
      // 降序第一步删段 4，模拟在此处断电崩溃
      remove_file(d.segment_path(4))?;
      // 此时剩余段 0..=3，前缀连续
      let crash_reopened = SegmentedDevice::new(&crash_path, seg_size, DEFAULT_SECTOR_SIZE)?;
      crash_reopened.recover()?;
      assert_eq!(crash_reopened.start_segment(), 0);
      assert_eq!(crash_reopened.end_segment(), Some(3));
    }

    info!("erase_tail_after 降序删除与前缀连续恢复校验通过");
    aok::Result::<()>::Ok(())
  })?;
