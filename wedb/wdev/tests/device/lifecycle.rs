...
//! sync 刷盘对标 C# `LocalStorageDevice` 句柄表全量刷新语义（全局覆盖契约，
//! 句柄按 (device_id, segment_id) 线程本地隔离，任一线程调用即覆盖在册段落盘）。
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.hlog/DeviceTests.cs（设备生命周期）
...
/// 对标 C# `LocalStorageDevice` 句柄表全量刷新语义：sync 刷盘后数据完整可回读；
...
fn sync_persists_data_and_handles_reopen_after_reset() -> Void {
  ...
  rt.block_on(async {
    ...
    let device = SegmentedDevice::new(dir.path().join("sync.log"), seg_size, DEFAULT_SECTOR_SIZE)?;
...
fn read_only_device_blocks_writes_while_allowing_reads() -> Void {
  ...
  rt.block_on(async {
    ...
    // 1. 先用可写设备写入基准数据
    {
      let device = SegmentedDevice::new(&log_path, seg_size, DEFAULT_SECTOR_SIZE)?;
      let buf = AlignedBuf::from_slice(&pattern, 4096)?;
      let (res, _) = device.write_aligned(0, buf).await;
      assert_eq!(res?, 4096);
    }
...
fn device_trait_static_dispatch() -> Void {
  async fn run_device_ops<D: Device>(device: &D, seg_size: u64) -> Result<(), wdev::Error> {
    ...
    assert_eq!(device.segment_size(), seg_size);
...
fn device_knobs_are_fixed_at_construction_and_visible_through_trait() -> Void {
  ...
  rt.block_on(async {
    ...
    fn knobs<D: Device + ?Sized>(d: &D) -> (Option<u64>, u64) {
      (d.capacity(), d.segment_size())
    }
    assert_eq!(knobs(&*device), (Some(cap), seg_size));
