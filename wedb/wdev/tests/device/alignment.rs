...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.hlog/DeviceTests.cs（扇区对齐）
...
fn unaligned_offset_read_async_throws() -> Void {
  ...
  rt.block_on(async {
    let dir = tempdir()?;
    let device = SegmentedDevice::new(dir.path().join("unaligned_read.log"), 1 << 30, 4096)?;
...
fn unaligned_offset_write_async_throws() -> Void {
  ...
  rt.block_on(async {
    let dir = tempdir()?;
    let device = SegmentedDevice::new(dir.path().join("unaligned_write.log"), 1 << 30, 4096)?;
...
fn unaligned_length_write_async_throws() -> Void {
  ...
  rt.block_on(async {
    let dir = tempdir()?;
    let device = SegmentedDevice::new(dir.path().join("unaligned_len.log"), 1 << 30, 4096)?;
...
fn unaligned_buffer_write_async_throws() -> Void {
  ...
  rt.block_on(async {
    let dir = tempdir()?;
    let device = SegmentedDevice::new(dir.path().join("unaligned_buf.log"), 1 << 30, 4096)?;
    ...
    for _ in 0..100 {
      ...
      // 读/写两口各用一份独立分配的未对齐缓冲（AlignedBuf 无 Clone：深拷贝
      // 臂无法以签名平滑表达分配失败，票 zcode-r55-alloc）
      let (res, _) = device.write_aligned(0, candidate).await;
      match res {
      ...
      let reader = AlignedBuf::new(4096, 512)?;
      if reader.is_aligned_to(4096) {
        // 该轮分配天然对齐，回循环另取（写拒绝已先行验证，重复幂等）
        continue;
      }
      let ptr_val = reader.as_ptr() as usize;
      let (res, _) = device.read_aligned(0, reader).await;
      match res {
        Err(Error::UnalignedBuffer { ptr, align }) => {
          assert_eq!(ptr, ptr_val);
          assert_eq!(align, 4096);
...
fn non_power_of_two_segment_size_throws() -> Void {
  ...
  rt.block_on(async {
    let dir = tempdir()?;
    assert!(matches!(
      SegmentedDevice::new(dir.path().join("seg_3mib.log"), 3 * 1024 * 1024, 4096),
...
fn segment_size_smaller_than_sector_throws() -> Void {
  ...
  rt.block_on(async {
    ...
    // 512 段尺寸 < 4096 扇区尺寸
    assert!(matches!(
      SegmentedDevice::new(dir.path().join("seg_small1.log"), 512, 4096),
      Err(Error::InvalidSegmentSize(512))
    ));
    // 256 段尺寸 < 512 扇区尺寸（C# 用例取值）
    assert!(matches!(
      SegmentedDevice::new(dir.path().join("seg_small2.log"), 256, 512),
...
fn zero_segment_size_throws() -> Void {
  ...
  rt.block_on(async {
    let dir = tempdir()?;
    assert!(matches!(
      SegmentedDevice::new(dir.path().join("seg_zero.log"), 0, 4096),
  ...
  rt.block_on(async {
    ...
    for sector in [512usize, 4096] {
      let device = SegmentedDevice::new(
        dir.path().join(format!("sec_{sector}.log")),
        1 << 30,
        sector,
      )?;
    ...
    for bad in [256usize, 500, 3000] {
      assert!(
        matches!(
          SegmentedDevice::new(dir.path().join(format!("sec_bad_{bad}.log")), 1 << 30, bad),
