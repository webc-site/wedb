...
impl Device for NullDevice {
  ...
  /// 对标 libs/storage/Tsavorite/cs/src/core/Device/NullDevice.cs:WriteAsync：
  /// 即时假成功确认全部逻辑长度（C# callback(0, numBytesToWrite)）
  /// 空设备无需实际物理写操作，保留 _offset 满足 Device trait 接口
  async fn write_aligned(&self, _offset: u64, buf: AlignedBuf) -> (Result<usize>, AlignedBuf) {
    (Ok(buf.len()), buf)
  }

  /// 对标 libs/storage/Tsavorite/cs/src/core/Device/NullDevice.cs:ReadAsync
  /// 空设备无需实际物理定位，保留 _offset 满足 Device trait 接口
  ...
  /// 缓冲 I/O 便捷读取（复用 [`NullDevice::read_aligned`]，假成功语义同其 C# 对照）
  #[inline]
  ...
  /// 空设备假段无实际文件，保留 _segment_id 满足 Device trait 接口
  #[inline]
  ...
  /// 的 MonotonicUpdate 单调推进语义；物理删除无操作，对标 NullDevice.cs:RemoveSegment）
