...
/// `address >> segmentSizeBits` 单点换段算术：
...
impl SegmentChunks {
  ...
  /// 内部位移与掩码运算依赖该前提。
  #[inline]
  pub const fn new(offset: u64, total_len: usize, segment_size: u64) -> Self {
    let (shift, mask) = (segment_shift(segment_size), segment_mask(segment_size));
    Self {
      curr_offset: offset,
      buf_pos: 0,
      total_len,
...
impl Iterator for SegmentChunks {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    let seg_remain = usize::try_from(self.segment_size - off_in_seg).unwrap_or(usize::MAX);
