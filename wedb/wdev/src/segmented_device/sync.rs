...
impl SegmentedDevice {
  ...
  ///
  /// - compio `File` 未启用 `sync` feature 时内含 `Rc`（实测 `!Send`，链路 `File →
  ///   AsyncFd → Attacher → SharedFd → Rc<Inner<File>>`），句柄无法跨线程共享；
  ///   本实现据此设计为"句柄永不过线程"：sync 只收集调用线程 TLS 内的本地句柄，
  ///   他线程写入过的段由调用线程按文件存在性就地补开（打开动作发生且仅发生在
  ///   调用线程自身的驱动上），随后对本线程句柄提交 fsync/fdatasync；
  ///   收集前先与设备失效戳对账（`SegmentedDevice::reconcile`），使 sync 顺带
  ///   成为陈旧句柄的回收点之一；
  ...
  async fn sync_internal(&self, datasync: bool) -> Result<()> {
    // 1. 与设备失效戳对账（就地关闭本线程持有的已截断段/被整表失效句柄），
    //    再收集本线程在册句柄
    ...
    let mut files: HashMap<u32, Rc<File>> = LOCAL_FILES.with(|map| {
      map
        .borrow()
        .iter()
        .filter(|&(&(dev_id, _), _)| dev_id == self.device_id)
        .map(|(&(_, sid), cached)| (sid, Rc::clone(&cached.1)))
        .collect()
    });

    // 2. 水位采样收敛单点：在收集本核心句柄后采样全局 start/end 段号，
    //    跨线程写入其 fetch_max 必先于写入完成，后发采样必然可见，消除漏刷窗口
    ...
    // 3. 若存在本核心未打开但全局在册已写入的段（如他核心写入后跨线程 sync），按需补开。
  ...
  fn debug_verify_synced(&self, pending: &[u32], synced: &HashMap<u32, Rc<File>>) {
    ...
    for &seg in pending {
      let is_dirty = (self.dirty_segs[seg as usize / 64].load(Ordering::Relaxed)
        & (1 << (seg as usize % 64)))
        != 0;
      if !is_dirty {
        // 已被 remove_segment 显式清除免责，跳过断言
        continue;
      }
