...
/// 失败上抛错误以阻断写入并触发新建段回滚；非 Unix 平台直接返回 `Ok(())`。
#[inline]
fn sync_dir(parent: &Path) -> Result<()> {
  Ok(crate::sync_dir(parent)?)
}
...
impl SegmentedDevice {
  ...
  /// 构造文件打开选项
  ///
  /// `create` 决定可写打开是否携带 O_CREAT：写路径 true（对标 C# 写句柄 `OpenOrCreate`），
  /// 读路径与 sync 补开路径 false（仅打开已存在段、以 O_RDWR 不带 O_CREAT——元数据
  /// 预检后、open 前段文件被并发删除（`remove_segment`/外部删除）时以 NotFound 失败
  /// 而非竞态窗口重建幽灵段；保留写权限使读写设备已存在段的 `Rc<File>` 可被后续写入复用）。
  /// `create_new` 为 true 时以 O_CREAT | O_EXCL 打开（新建段防竞态专属，败方转既有段打开）。
  #[inline]
  fn open_options(read_only: bool, create: bool, create_new: bool) -> OpenOptions {
  ...
  async fn open_file(
    ...
    if self.direct_io.load(Ordering::Relaxed) {
      ...
      let mut opts = Self::open_options(read_only, create, create_new);
    ...
    let file = Self::open_options(read_only, create, create_new)
      .open(path)
      .await?;
  ...
  /// OpenOrCreate 语义，写路径专用）；为 false 时仅打开磁盘上已存在的段文件，
  /// 缺失段返回 [`Error::SegmentNotFound`]（读路径与 sync 补开路径专用，杜绝从未
  /// 写入的空洞段因读/sync 被幽灵创建，对齐 `recover` 的幽灵段防御）。
  ///
  ...
  pub(super) async fn get_or_open_file(&self, segment_id: u32, create: bool) -> Result<Rc<File>> {
    ...
    // 写模式下先探测段文件是否缺失：新建段先以排他创建（create_new）打开，
    // 成功者作为真创建者承担父目录 fsync 屏障与失败时的回滚；若并发竞态返回
    // AlreadyExists，则消除创建者身份转既有段打开路径，杜绝双创建者误删活跃段
    let mut is_new_segment = if create {
    ...
    let mut file = None;
    if is_new_segment {
      // 预分配仅在物理新建段时生效（对标 C# preallocateFile 只作用于 CreateWriteHandle
      // 的新建流）：打开已存在段（含 create=false 的读路径，其 is_new_segment 恒为
      // false）绝不重复 set_len，杜绝读操作对磁盘文件尺寸的副作用与反复预分配
      ...
      match self
        .open_file(&path, self.read_only, prealloc, true, true)
        .await
      {
        Ok(f) => file = Some(f),
        Err(Error::Io(e)) if e.kind() == ErrorKind::AlreadyExists => {
          // 并发创建竞态败方：他线程已抢先创建段文件，转为既有段打开路径；
          // is_new_segment 置假，不刷目录屏障、不承担回滚（杜绝双创建者卸载对侧在册段）
    ...
    let file = match file {
      Some(f) => f,
      None => match self
        .open_file(&path, self.read_only, None, create, false)
        .await
      {
        Ok(f) => f,
    ...
    if is_new_segment && let Err(e) = sync_dir(self.parent_dir()) {
      drop(file);
