...
pub enum Error {
  ...
  /// 迁移载荷声名记录计数超载荷可证上界（协议违约：每条 frame 至少 1 字节
  /// kind，声名条数 > 剩余字节数即结构性非法，解析循环前显式拒绝而非循环内
  /// 截断）；预约容量随之钳在上界内，伪造巨值计数绝无按 u32 全量预分配的
  /// 协议面 DoS（分配失败 abort 全进程）。C# 对标 libs/cluster/Session/
  /// RespClusterMigrateCommands.cs:102-114 逐记录边读边校验、无预分配面
  #[error("migration record count {0} exceeds payload provable bound {1}")]
  RecordCountExceedsPayload(u32, usize),
