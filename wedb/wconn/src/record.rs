...
impl MigrationDomainContext {
  ...
  pub fn encode_frame(&self) -> [u8; 1 + Self::BODY_LEN] {
    let mut buf = [0u8; 1 + Self::BODY_LEN];
    buf[0] = MigrationRecordKind::DomainContext as u8;
    buf[1..9].copy_from_slice(&self.vns.to_le_bytes());
    buf[9..17].copy_from_slice(&self.vdb.to_le_bytes());
    buf[17..25].copy_from_slice(&self.ns.to_le_bytes());
    buf[25..33].copy_from_slice(&self.db.to_le_bytes());
    buf
  }
...
fn parse_typed_record<'a>(kind: u8, mut body: &'a [u8]) -> Result<(MigrationRecord<'a>, usize)> {
  ...
  let Some(kind_enum) = MigrationRecordKind::from_repr(kind) else {
  ...
  let record = match kind_enum {
    ...
    other => {
      return Err(Error::InvalidRecord(format!(
        "Unsupported migration record kind {:?}",
        other
      )));
    }
  };
...
pub fn encode_dbmeta_frame(key: &[u8], value: &[u8]) -> Vec<u8> {
  ...
  buf.push(MigrationRecordKind::DbMeta as u8);
...
fn write_vector_set_index_frame(buf: &mut Vec<u8>, key: &[u8], value: &[u8]) {
  buf.push(MigrationRecordKind::VectorSetIndex as u8);
  buf.extend_from_slice(&(key.len() as u32).to_le_bytes());
  buf.extend_from_slice(key);
  buf.extend_from_slice(&(value.len() as u32).to_le_bytes());
  buf.extend_from_slice(value);
