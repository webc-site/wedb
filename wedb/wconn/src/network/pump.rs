...
use wbase::{
  pool::{DEFAULT_MAX_ENTRIES_PER_LEVEL, LimitedFixedBufferPool},
  primed::PrimedVec,
};
...
pub const RECV_IDLE_PROBE: Duration = Duration::from_millis(250);
...
pub(crate) fn resolve_network_pool(
  ...
  pool.unwrap_or_else(|| LimitedFixedBufferPool::new(READ_CHUNK, DEFAULT_MAX_ENTRIES_PER_LEVEL))
}
...
pub async fn read_pump(
  ...
  // 持久消费游标（对标 C# ProcessReplies 的 readHead、兄弟服务端 read_head）：
  // 跨派发调用与读事件驻留，帧间只推进此游标不搬字节，残余段仅在尾部空间不足时前移
  ...
  loop {
    ...
    // 滞留错误应答感知：队列空时游标后残留完整 `-ERR` 行 = fire-and-forget
    // 记录帧（APPENDLOG）已被写出即忘，服务端拒收回写无人认领——判流失效
    // 断连（C# 连接异常等价，驱动主端剔除副本转入重同步，防 shipped_watermark
    // 静默推进扩大主从分歧）
    if queue.is_empty()
      && let Some(err) = orphan_error_reply(&read_buf[read_head..])
    {
    ...
    if read_fut.is_none() {
      ...
      // 池槽代际随缓冲借出注入（TLS 读空闲段清零每缓冲付一次的跨读记忆，
      // wbase::primed 契约；读毕回写槽）
      let mut chunk = PrimedVec::from_primed(
        pooled.take_buffer().expect("池化接收缓冲在位"),
        pooled.prime_key(),
      );
      // 覆盖写口径前置：泵槽 set_buffer 回槽不清长度，PrimedVec 读目标为
      // 空闲段、完成按追加推进——残段不归零则新读写在上一读尾后，`back[..n]`
      // 取到陈旧前缀（应答错位一拍）。归零后追加语义与覆盖语义合一，
      // clear 不动容量与代际，TLS 每缓冲一次的清零记忆不受影响
      chunk.clear();
    ...
    // 三路竞速唤醒（读请求全程存活）
    let wake = {
      let stalled = queue.is_empty() && read_buf.len() > read_head && !writer_gone;
    ...
    match wake {
      Wake::Read((s, BufResult(read_res, back))) => {
        stream = Some(s);
        read_fut = None;
        // 读得字节先并入累积缓冲，缓冲随即交回池句柄（覆盖语义：下一读
        // 自缓冲头写入，残余不驻留此块）；初始化代际回写槽跨读续用
        if let Ok(n) = &read_res {
          if read_head > 0 && read_buf.capacity() - read_buf.len() < READ_CHUNK {
            read_buf.copy_within(read_head.., 0);
            read_buf.truncate(read_buf.len() - read_head);
            read_head = 0;
          }
          read_buf.extend_from_slice(&back[..*n]);
        }
        pooled.set_prime_key(back.prime_key());
        pooled.set_buffer(back.into_inner());
        match read_res {
...
/// 读请求落定输出：流句柄与读结果（所有权外还；载体恒为池块记忆缓冲）
type ReadDone = (ReadHalf, BufResult<usize, PrimedVec>);
