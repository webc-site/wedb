...
use log::info;
use wconn::{api::SortedSetPairCollection, client::GarnetClient, session::GarnetClientSession};
...
fn test_garnet_client_and_session_apis() -> Void {
  ...
  rt.block_on(async {
    ...
    // 多键/带 delta 便捷形态（C# GarnetClientTests 消费的重载对位）
    assert_eq!(
      client.string_get_multi_async(&["k1", "k2"]).await?,
      vec!["hello".to_string(), "world".to_string()]
    );
    assert_eq!(client.key_delete_multi_async(&["k1", "k2", "k3"]).await?, 1);
    assert_eq!(client.string_increment_by_async("num", 7).await?, 42);
    assert_eq!(client.string_decrement_by_async("num", 7).await?, 41);
    ...
    assert_eq!(
      client
        .sorted_set_remove_multi_async("zset", &["m1", "m2"])
        .await?,
      1
    );
