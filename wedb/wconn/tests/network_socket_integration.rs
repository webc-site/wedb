...
use wconn::{
  Error,
  network::{OutStream, READ_CHUNK, read_pump, write_pump},
...
fn idle_pump_borrows_read_buffer_once() {
  ...
  Runtime::new().unwrap().block_on(async {
    ...
    // 假端点：收下连接后整窗静默（零字节空闲链路），确认挂读后即 drop 令读侧得到 EOF
    spawn(async move {
      let (sock, _peer) = listener.accept().await.unwrap();
      while !drop_signal_clone.load(Ordering::Acquire) {
    ...
    let (_in_flight_tx, in_flight_rx) = mpsc::bounded_async::<CommandItem>(IN_FLIGHT_CAPACITY);
    ...
    // 读泵稳态挂读已坐实，通知假端点关闭触发 EOF
    drop_signal.store(true, Ordering::Release);
...
#[compio::test]
async fn write_pump_byte_fragmentation_preserves_stream() {
  let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
  let addr = listener.local_addr().unwrap();
