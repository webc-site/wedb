...
impl RangeIndexManager {
  ...
  /// 不设上限，屏障持有者异常时读者被动挂起直至清屏。`id_key` 域语义见
  /// [`RangeIndexManager::key_id_of`] 身份契约
  pub async fn wait_for_tree_checkpoint_async(&self, id_key: &[u8]) -> Result<bool> {
    ...
    for entry in &entries {
      ...
      let res = if let Some(tree) = tree_opt {
        ...
        if data_path.exists() {
          ...
          // 工作文件缺失留痕跳过 (1:1 对标 C# LogWarning，
          // libs/server/Resp/RangeIndex/RangeIndexManager.cs:837)：无源可拷，
          // 本条目不计入快照数，绝不上抛阻断整轮检查点
          log::warn!(
            "SnapshotAllTreesForCheckpoint: data.bftree missing for pending {}",
            prefix.as_str()
          );
        }
        Ok(())
