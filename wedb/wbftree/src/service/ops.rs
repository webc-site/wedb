...
/// 扫描故障注入钩子（仅测试用，一处定义）：置真后**下一次**扫描初始化即返回
/// [`Error::Scan`]（一次性，消费即自动复位，生产路径恒假零开销）。供宿主
/// 验证「扫描 Err 必须上抛成错误帧、严禁折成空集/静默截断」的全链路
///（tiered 集合读臂经 wnode 慢路径存储错误漏斗闭环成 RESP 错误帧）
#[cfg(debug_assertions)]
pub static SCAN_FAIL_INJECT: AtomicBool = AtomicBool::new(false);
...
impl BfTreeService {
  ...
  ///
  /// 零锁直调：内核经 `with_tree` 无锁借用 &BfTree，
    ...
    F: FnMut(&[u8], &[u8]) -> bool,
  {
    ...
    // 故障注入门（测试钩子，一次性）：模拟底层 range_index 扫描失败面，
    // 与 make_iter 失败同型上抛，杜绝测试绕过本统一驱动
    #[cfg(debug_assertions)]
    if SCAN_FAIL_INJECT.swap(false, Ordering::AcqRel) {
      return Err(Error::Scan("扫描故障注入（测试钩子）".into()));
    }
