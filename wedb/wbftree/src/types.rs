...
impl From<ScanReturnField> for bf_tree::ScanReturnField {
  ...
  fn from(f: ScanReturnField) -> Self {
    match f {
      ScanReturnField::Key => bf_tree::ScanReturnField::Key,
      ScanReturnField::Value => bf_tree::ScanReturnField::Value,
      ScanReturnField::KeyAndValue => bf_tree::ScanReturnField::KeyAndValue,
    }
  }
}
...
impl From<bf_tree::StorageBackend> for StorageBackend {
  #[inline]
  fn from(b: bf_tree::StorageBackend) -> Self {
    match b {
      bf_tree::StorageBackend::Std => StorageBackend::Std,
      bf_tree::StorageBackend::Memory => StorageBackend::Memory,
    }
  }
}
...
impl BfTreeConfig {
  ...
  pub fn file_path(&mut self, path: impl AsRef<Path>) -> &mut Self {
    ...
    self.inner.storage_backend(StorageBackend::Std.into());
