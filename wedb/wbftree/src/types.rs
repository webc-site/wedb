...
impl TreeTuning {
  /// RI.CREATE 默认调参 (1:1 对标 Garnet libs/server/Resp/RangeIndex/
  /// RespServerSessionRangeIndex.cs:44-50 Defaults：16MiB 缓存 / min 64 /
  /// max 1024 / max key 128 / PAGESIZE 0 按 max_record_size 自动推导)。
  /// RI 建树默认调参的唯一单点，RESP 解析层与存储层兜底均引用此处
  pub const DEFAULT_RI: Self = Self {
  ...
  /// 集合升阶建树调参 (wkv promote_collection_to_bftree)
  ///
  /// min_record_size 取 bf-tree 引擎硬下限 2 (引擎 insert 对
  /// key+value < cb_min_record_size 返回 InvalidKV，配置下限断言为 2——
  /// bf-tree tree.rs "cb_min_record_size cannot be less than 2")：升阶条目
  /// 来自通用集合 (member/score、field/value 常短于 [`Self::DEFAULT_RI`]
  /// 的 64B 记录下限)，取 64 会整批装载失败；其余字段与 [`Self::DEFAULT_RI`]
  /// 同源
  pub const DEFAULT_RI_COLLECTION: Self = Self {
    min_record_size: 2,
    ..Self::DEFAULT_RI
  };
}
