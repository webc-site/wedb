//! 自研依据: bf-tree 点查/点写语义（C# 参照面 test/standalone/Garnet.test.rangeindex/RespRangeIndexTests.cs）
use aok::{OK, Result};
use wbftree::{
...
fn test_read_into_buffer_contract() -> Result<()> {
  let (_dir, _manager, tree) = managed_tree(
    "bftree_test_read_into_contract",
    StorageBackendType::Disk,
    TreeTuning {
      min_record_size: 4,
      max_record_size: 4095,
      ..TreeTuning::default()
    },
  )?;
  ...
  assert_eq!(tree.insert(b"big", &big_val), BfTreeInsertResult::Success);
