//! 自研依据: doc/zh/collection.md「双向转换触发逻辑」升阶/降阶管理器
use std::{
  fs,
  sync::{Arc, Mutex, Once, OnceLock, atomic::Ordering},
  thread,
  time::Duration,
};
...
fn test_pre_stage_missing_source_and_restore_error() -> Result<()> {
  ...
  // 磁盘后端无 data.bftree 且无刷盘快照 → get_or_open_tree 降级 NotFound
  // (1:1 对标 C# RestoreTree 的 File.Exists(workingPath) 假臂：Release 模式
  // LogWarning + return false，上层答 NOTFOUND，绝不以致命恢复错误上抛)
  ...
  assert!(matches!(
    manager.get_or_open_tree(key, &stub),
    Err(Error::NotFound)
  ));
...
/// 冷拷贝臂 warning 捕获日志器（仅缺失臂留痕断言使用；进程级单次安装，其余
/// 测试日志照常流入缓冲，断言按文案子串过滤不受并发干扰）
static WARN_CAPTURE: OnceLock<Mutex<Vec<String>>> = OnceLock::new();
static WARN_CAPTURE_INIT: Once = Once::new();
...
/// 测试检查点冷拷贝臂：工作文件缺失的 pending 条目留痕跳过不致命 (1:1 对标
/// C# RangeIndexManager.cs:837 LogWarning)，工作文件在场的冷条目照常拷贝计数
#[test]
fn test_snapshot_cold_entry_missing_data_file_skips_with_warning() -> Result<()> {
  WARN_CAPTURE_INIT.call_once(|| {
    let _ = log::set_boxed_logger(Box::new(WarnCapture));
    log::set_max_level(LevelFilter::Warn);
  });

  let env = ManagerEnvGuard::new("cold_missing");
  let manager = RangeIndexManager::new(&env.ri_root.path, &env.cpr_root.path)?;

  // 冷条目 A：pending（无在线树）且工作文件缺失
  let missing_key = b"cold_missing_key";
  let missing_id = RangeIndexManager::key_id_of(missing_key);
  let entry = TreeEntry::new(None, fast_hash(missing_key), missing_id);
  entry.snapshot_pending.store(true, Ordering::SeqCst);
  manager
    .live_indexes()
    .pin()
    .insert(missing_id, Arc::new(entry));

  // 冷条目 B：真实 Disk 树构造（工作文件在盘）后摘除在线实例成冷态
  let present_key = b"cold_present_key";
  let tree = manager.create_bftree(present_key, StorageBackendType::Disk, TUNE)?;
  assert_eq!(tree.insert(b"k1", b"v1"), BfTreeInsertResult::Success);
  let present_id = RangeIndexManager::key_id_of(present_key);
  let present_entry = manager
    .live_indexes()
    .pin()
    .get(&present_id)
    .cloned()
    .expect("Disk 树条目应在册");
  *present_entry.tree.write() = None;
  present_entry.snapshot_pending.store(true, Ordering::SeqCst);

  // 快照：缺失臂留痕跳过不计数不上抛，在场臂照常拷贝
  let count = manager.snapshot_all_trees_to_dir(&env.cpr_root.path, 77u128)?;
  assert_eq!(
    count, 1,
    "缺失工作文件的冷条目必须跳过不计数，在场冷条目必须拷贝"
  );
  assert!(
    RangeIndexManager::checkpoint_snapshot_path_in(&env.cpr_root.path, 77, present_id).exists(),
    "工作文件在场的冷条目必须照常拷贝"
  );
  assert!(
    !RangeIndexManager::checkpoint_snapshot_path_in(&env.cpr_root.path, 77, missing_id).exists(),
    "缺失工作文件的冷条目不得产出快照文件"
  );

  // 留痕契约：检查点日志含 warning（C# LogWarning 对位）
  let warned = WARN_CAPTURE
    .get_or_init(|| Mutex::new(Vec::new()))
    .lock()
    .expect("warn 捕获缓冲锁不得中毒")
    .iter()
    .any(|msg| msg.contains("data.bftree missing for pending"));
  assert!(warned, "冷拷贝臂 data.bftree 缺失必须记 warning 留痕");
  OK
}
