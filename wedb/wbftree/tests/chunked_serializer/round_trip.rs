//! 在 garnet 中的相对路径: test/standalone/Garnet.test/RangeIndexChunkedSerializerTests.cs:SingleChunkRoundTrip/MultiChunkRoundTrip
...
fn single_chunk_round_trip() -> Result<()> {
  let td = TestPathGuard::new("ri_test_single_chunk", true);
  let file_data = random_bytes(1024, 42);
  let file_path = td.join("small.bftree");
...
fn file_content_preserved_in_round_trip() -> Result<()> {
  let td = TestPathGuard::new("ri_test_content_pres", true);
  let file_data = random_bytes(DEFAULT_MIGRATION_CHUNK_SIZE * 2 + 500, 77);
  let file_path = td.join("content.bftree");
  fs::write(&file_path, &file_data)?;
...
fn stub_preserved_in_round_trip() -> Result<()> {
  let td = TestPathGuard::new("ri_test_stub_pres", true);
  let file_path = td.join("stubtest.bftree");
...
fn key_spanning_multiple_chunks_round_trip() -> Result<()> {
  let td = TestPathGuard::new("ri_test_key_multichunk", true);
  let file_path = td.join("tinyChunk.bftree");
...
fn small_buffer_round_trip() -> Result<()> {
  let td = TestPathGuard::new("ri_test_small_buf_rt", true);
  let file_data = random_bytes(50, 99);
  let file_path = td.join("smallbuf.bftree");
  fs::write(&file_path, &file_data)?;
...
fn file_data_exactly_fills_chunk_trailer_in_next_chunk() -> Result<()> {
  let td = TestPathGuard::new("ri_test_fill_chunk", true);
  let key = b"testkey";
...
fn file_data_one_byte_per_chunk() -> Result<()> {
  let td = TestPathGuard::new("ri_test_1b_per_chunk", true);
  let key = b"k";
