//! 在 garnet 中的相对路径: test/standalone/Garnet.test/RangeIndexChunkedSerializerTests.cs（分块边界）
use std::{
  fs::{self, File},
  io::{Seek, SeekFrom},
};
...
use wbftree::{
  ...
  RangeIndexChunkedSerializer, RangeIndexManager,
};
...
fn buffer_too_small_for_key_header_defers_to_next_chunk() -> Result<()> {
  let td = TestPathGuard::new("ri_test_buf_small_key", true);
  let file_path = td.join("keyheader.bftree");
...
fn buffer_too_small_for_file_header_defers_to_next_chunk() -> Result<()> {
  let td = TestPathGuard::new("ri_test_buf_small_file_hdr", true);
  let file_path = td.join("fileheader.bftree");
...
fn buffer_too_small_for_trailer_defers_to_next_chunk() -> Result<()> {
  let td = TestPathGuard::new("ri_test_buf_small_trailer", true);
  let file_data = random_bytes(64, 42);
  let file_path = td.join("trailer.bftree");
  fs::write(&file_path, &file_data)?;
...
fn exact_phase_boundary_transitions() -> Result<()> {
  let td = TestPathGuard::new("ri_test_exact_phase", true);
  let key = b"k";
  let stub = create_stub();
  ...
  let file_path = td.join("boundary.bftree");
  fs::write(&file_path, &file_data)?;
...
fn trailer_checksum_and_stub_content_verification() -> Result<()> {
  let td = TestPathGuard::new("ri_test_trailer_cs", true);
  let file_data = random_bytes(256, 44);
  let file_path = td.join("checksum.bftree");
  fs::write(&file_path, &file_data)?;
...
fn is_complete_transitions_correctly() -> Result<()> {
  let td = TestPathGuard::new("ri_test_is_complete", true);
  let file_data = random_bytes(DEFAULT_MIGRATION_CHUNK_SIZE + 100, 33);
  let file_path = td.join("complete.bftree");
  fs::write(&file_path, &file_data)?;
...
fn buffer_exhausted_at_file_data_phase_does_not_throw() -> Result<()> {
  let td = TestPathGuard::new("ri_test_buf_exhaust", true);
  let file_data = random_bytes(100, 42);
  let file_path = td.join("exhausted.bftree");
  fs::write(&file_path, &file_data)?;
...
fn empty_chunk_during_waiting_for_trailer() -> Result<()> {
  let td = TestPathGuard::new("ri_test_empty_trailer", true);
  let key = b"k";
...
fn empty_chunk_during_receiving_file_data() -> Result<()> {
  let td = TestPathGuard::new("ri_test_empty_receiving", true);
  let key = b"mykey";
...
fn empty_chunk_at_waiting_for_key_header() -> Result<()> {
  let td = TestPathGuard::new("ri_test_empty_key_hdr", true);
  let key = b"mykey";
  let file_data = [0x01, 0x02];
  let stub = create_stub();
  let payload = build_payload(key, &file_data, &stub);
...
fn empty_chunk_during_waiting_for_file_header() -> Result<()> {
  let td = TestPathGuard::new("ri_test_empty_file_hdr", true);
  let key = b"mykey";
  let file_data = [0x01, 0x02];
  let stub = create_stub();
  let payload = build_payload(key, &file_data, &stub);
...
fn key_split_across_two_chunks() -> Result<()> {
  let td = TestPathGuard::new("ri_test_key_split", true);
...
fn key_header_alone_in_chunk() -> Result<()> {
  let td = TestPathGuard::new("ri_test_key_hdr_alone", true);
  let key = b"mykey";
