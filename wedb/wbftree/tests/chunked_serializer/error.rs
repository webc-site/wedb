//! 在 garnet 中的相对路径: test/standalone/Garnet.test/RangeIndexChunkedSerializerTests.cs（损坏校验和/非法头）+ RangeIndexStreamReplayTests.cs（半流清退）
...
use whasher::compute_checksum;

use super::common::{
  TestPathGuard, build_payload, create_buffer, create_stub, random_bytes, serializer_move_next,
};

/// 空文件反序列化时安全拦截并进入错误状态
#[test]
fn empty_file_rejected_by_deserializer() -> Result<()> {
  let td = TestPathGuard::new("ri_test_empty_file", true);
  let key = b"emptykey";
  let file_data = b"";
  let stub = create_stub();
  let payload = build_payload(key, file_data, &stub);

  let manager = RangeIndexManager::new(&td.path, td.path.join("cpr")).unwrap();
...
fn corrupted_checksum_detected() -> Result<()> {
  let td = TestPathGuard::new("ri_test_corrupt_cs", true);
  let file_data = random_bytes(512, 99);
  let file_path = td.join("corrupt.bftree");
...
fn negative_file_size_is_error() -> Result<()> {
  let td = TestPathGuard::new("ri_test_neg_file_size", true);
...
fn split_file_header_is_error() -> Result<()> {
  let td = TestPathGuard::new("ri_test_split_hdr", true);
  let key = b"k";
...
fn trailer_too_small_is_error() -> Result<()> {
  let td = TestPathGuard::new("ri_test_trailer_small", true);
  let key = b"k";
...
fn file_open_failure_goes_to_error_state() -> Result<()> {
  let td = TestPathGuard::new("ri_test_bad_open", true);
  // 无法创建的非法目录路径
  let bad_path = td
    .path
...
fn too_small_header_is_error() -> Result<()> {
  let td = TestPathGuard::new("ri_test_hdr_small", true);
...
fn error_state_is_terminal() -> Result<()> {
  let td = TestPathGuard::new("ri_test_terminal_err", true);
  let mut payload = vec![0u8; 4 + 8];
  payload[..4].copy_from_slice(&0u32.to_le_bytes());
...
fn invalid_stub_size_is_error() -> Result<()> {
  let td = TestPathGuard::new("ri_test_bad_stub_size", true);
...
fn truncated_stub_is_error() -> Result<()> {
  let td = TestPathGuard::new("ri_test_trunc_stub", true);
  let key = b"truncstub";
  let file_data = [0xABu8];
  let hash = compute_checksum(&file_data);
...
fn over_long_trailer_is_error() -> Result<()> {
  let td = TestPathGuard::new("ri_test_overlong_trailer", true);
  let key = b"overlong";
  let file_data = [0xABu8];
  let hash = compute_checksum(&file_data);
...
fn move_next_after_done_throws() -> Result<()> {
  let td = TestPathGuard::new("ri_test_done_throws", true);
  let file_path = td.join("done.bftree");
...
fn truncated_file_throws_exception() -> Result<()> {
  let td = TestPathGuard::new("ri_test_trunc_file", true);
  let file_path = td.join("truncated.bftree");
...
fn double_dispose_is_idempotent() -> Result<()> {
  let td = TestPathGuard::new("ri_test_double_disp", true);
  let file_path = td.join("dispose.bftree");
...
fn dispose_cleans_temp_file() -> Result<()> {
  let td = TestPathGuard::new("ri_test_dispose_clean", true);
  let manager = RangeIndexManager::new(&td.path, td.path.join("cpr")).unwrap();
  let mut deserializer = RangeIndexChunkedDeserializer::new(manager.derive_temp_migration_path())?;
...
fn startup_cleans_up_migration_tmp_dir() -> Result<()> {
  let td = TestPathGuard::new("ri_test_startup_clean", true);
  let tmp_dir = td.join("migration-tmp");
...
fn zero_length_key_rejected_by_deserializer() -> Result<()> {
  let td = TestPathGuard::new("ri_test_zero_key", true);
  let key = b"";
...
fn zero_file_data_rejected_by_deserializer() -> Result<()> {
  let td = TestPathGuard::new("ri_test_zero_file_data", true);
  let key = b"k";
  let file_data = b"";
  let stub = create_stub();
  let payload = build_payload(key, file_data, &stub);
...
fn corrupted_file_data_fails_checksum_in_trailer() -> Result<()> {
  let td = TestPathGuard::new("ri_test_corrupt_trailer_cs", true);
  let key = b"k";
...
fn declared_size_smaller_than_actual_file_emits_truncated_prefix() -> Result<()> {
  let td = TestPathGuard::new("ri_test_decl_smaller", true);
  let full_file_data = random_bytes(500, 55);
  let file_path = td.join("shorter.bftree");
