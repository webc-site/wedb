//! 在 garnet 中的相对路径: test/standalone/Garnet.test/RangeIndexChunkedSerializerTests.cs（反序列化器）
...
use super::common::{ChunkDriver, TestPathGuard, assert_round_trip, create_stub, random_bytes};
...
fn round_trip_single_chunk() -> Result<()> {
  for driver in ALL_DRIVERS {
    let td = TestPathGuard::new("rt_single_chunk", true);
    assert_round_trip(
      driver,
      &td,
      b"mykey",
...
fn round_trip_multi_chunk() -> Result<()> {
  for driver in ALL_DRIVERS {
    let td = TestPathGuard::new("rt_multi_chunk", true);
    assert_round_trip(
      driver,
      &td,
      b"largekey",
...
fn round_trip_key_larger_than_chunk() -> Result<()> {
  for driver in ALL_DRIVERS {
    let td = TestPathGuard::new("rt_key_larger", true);
    assert_round_trip(
      driver,
      &td,
      &random_bytes(200, 66),
...
fn round_trip_small_chunk() -> Result<()> {
  for driver in ALL_DRIVERS {
    let td = TestPathGuard::new("rt_small_chunk", true);
    assert_round_trip(driver, &td, b"k", &random_bytes(500, 77), 64)?;
  }
  OK
}
...
fn round_trip_file_exactly_fills_first_chunk() -> Result<()> {
  for driver in ALL_DRIVERS {
    let td = TestPathGuard::new("rt_fill_first", true);
    assert_round_trip(driver, &td, b"k", &random_bytes(51, 88), 64)?;
  }
  OK
}
...
fn round_trip_exact_min_chunk_size() -> Result<()> {
  for driver in ALL_DRIVERS {
    let td = TestPathGuard::new("rt_min_chunk", true);
    assert_round_trip(driver, &td, b"k", &random_bytes(300, 31), MIN_CHUNK_SIZE)?;
  }
  OK
}
...
fn reader_truncated_file_throws() -> Result<()> {
  let td = TestPathGuard::new("ri_test_rdr_trunc", true);
  let src_path = td.join("reader-trunc.bftree");
...
fn reader_non_positive_chunk_size_throws() -> Result<()> {
  let td = TestPathGuard::new("ri_test_rdr_chunk_zero", true);
  let src_path = td.join("reader-chunksize.bftree");
...
fn reader_destination_below_minimum_throws() -> Result<()> {
  let td = TestPathGuard::new("ri_test_rdr_dest_min", true);
  let src_path = td.join("reader-destmin.bftree");
