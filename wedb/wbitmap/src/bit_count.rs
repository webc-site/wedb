...
use crate::manager::{process_negative_offset, reverse};
...
fn __scalar_popc(bitmap: &[u8], start: usize, end: usize) -> i64 {
  let mut count: u64 = 0;
...
fn __simd_popc_x256(bitmap: &[u8], start: usize, end: usize) -> i64 {
  let mut count: u64 = 0;
...
fn u64_read(bitmap: &[u8], idx: usize) -> u64 {
  ...
  unsafe {
    (bitmap.as_ptr().add(idx) as *const u64)
      .read_unaligned()
      .to_le()
  }
}
...
#[cfg(test)]
mod tests {
  use super::{__scalar_popc, __simd_popc_x256, bit_count_driver};
