...
mod tests {
  use super::{
    super::parse::{BitFieldOverflow, length_from_type, new_block_alloc_length_from_type},
    BitFieldCmdArgs, BitFieldSecondaryCommand, bit_field_execute, bit_field_execute_ro,
    check_bitfield_overflow, check_signed_bitfield_overflow, check_unsigned_bitfield_overflow,
    get_bitfield,
  };

  /// 分配尺寸单源对拍：`new_block_alloc_length_from_type` / `length_from_type`
  /// 与独立字节跨度式 `(offset + bit_count - 1) / 8 + 1` 恒等。
  ///
  /// 原 manager::new_block_alloc_length 双源已删（bar-r65-wbitmap-audit）；
  /// SETBIT 会话层增长已收敛 wbitmap::length_in_bytes（zcode-r21-wbitmap），
  /// 本断言锁定 BITFIELD 写路径与 SETBIT 增长同一字节跨度口径。
  #[test]
  ...
  fn get_set_roundtrip_matrix() {
    for (signed, bits) in [
      ...
      .unwrap();
      // 回读（生产单源 get_bitfield，与只读签名 bit_field_execute_ro 同核）
  ...
  /// 只读签名契约：GET 同值、非 GET 拒绝（C# GarnetException 对位）
  #[test]
  fn read_only_signature_contract() {
    ...
    // SET / INCRBY 非只读合法子命令：C# GarnetException → None
    for op in [
      BitFieldSecondaryCommand::Set,
      BitFieldSecondaryCommand::IncrBy,
    ] {
