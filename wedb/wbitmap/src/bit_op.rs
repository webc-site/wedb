...
//!
//! 会话层 BITOP 单源为 [`BitOpAccumulator`] 增量折叠；下方全量批处理版
//! （invoke_bit_operation_unsafe 及其算子链）crate 外零生产调用且零导出，
//! 仅作为折叠器等价性对拍的测试 oracle 存活（task/done/zcode-r21-wbitmap.md）。
//!
...
#[cfg(test)]
fn invoke_bit_operation_unsafe(
  op: BitmapOperation,
  srcs: &[&[u8]],
  dst: &mut [u8],
...
fn invoke_nary_bitwise_operation<O: BinaryOperator>(
  ...
  for (align, width) in [(512, 64), (256, 32), (128, 16)] {
    let batch = remaining & !(align - 1);
    if batch > 0 {
      vectorized_n::<O>(srcs, &mut cursors, dst, dst_pos, batch, width);
      dst_pos += batch;
      remaining -= batch;
    }
  }
