...
///
/// TryGetBitFieldOverflow 的匹配内核；SessionParseState 版薄适配见
/// session_parse_state_extensions 模块（唯一 C# 映射点）
...
/// 解析位域编码（u<位宽> / i<位宽>）
///
/// libs/server/Resp/Bitmap/BitmapCommands.cs:TryGetBitfieldEncoding
...
/// 位域 offset 解析（C# TryGetBitfieldOffset：`#<n>` 倍乘形式或裸位偏移，须 ≥ 0）
///
/// 返回（offset，是否倍乘）
