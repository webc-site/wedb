...
//!    参数 (ns, db, 用户键, expire_at_ticks) 正确，purge 后普通写镜像不受影响
//!    （抑制标志零残留）；
...
/// purge 端口记录 (ns, db, 用户键, expire_at_ticks)
type PurgeLog = Arc<Mutex<Vec<(u64, u64, Vec<u8>, i64)>>>;
...
fn mirror_listener(log: MirrorLog) -> wkv::WriteListenerFn {
  wkv::WriteListenerFn::new(move |key, val, tombstone| {
    log
      .lock()
      .unwrap()
...
fn purge_listener(log: PurgeLog) -> wkv::TtlPurgeListenerFn {
  wkv::TtlPurgeListenerFn::new(move |ns, db, key, expire_at_ticks| {
    log
      .lock()
      .unwrap()
      .push((ns, db, key.to_vec(), expire_at_ticks));
  })
}
  ...
  Runtime::new()?.block_on(async {
    ...
    assert_eq!(
      *purges.lock().unwrap(),
      ...
      "purge 端口必须恰好收到一次且 (ns, db, key, expire_at_ticks) 正确"
    );
