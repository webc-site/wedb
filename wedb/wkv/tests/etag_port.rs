  ...
  Runtime::new()?.block_on(async {
    ...
    let etags: Vec<_> = entries
      ...
      .filter_map(|e| match e {
        RecordedEvent::EtagWrite { ns, db, key, etag } => Some((*ns, *db, key.clone(), *etag)),
        _ => None,
      })
      .collect();
    assert_eq!(etags.len(), 2, "upsert 与原位改写各分发一次 EtagWrite");
    assert_eq!(etags[0], (3, 1, key.to_vec(), Some(6)));
    assert_eq!(etags[1], (3, 1, key.to_vec(), Some(7)));
...
fn etag_port_fires_tombstone_on_del_and_cascade() -> Void {
  Runtime::new()?.block_on(async {
    ...
    assert_eq!(etags[1], (0, 0, k1.to_vec(), None), "del_etag 墓碑");
    assert_eq!(etags[3], (0, 0, k2.to_vec(), None), "级联清退墓碑");
    OK
  })
}
