...
//!
//! 在 garnet 中的相对路径: libs/host/ServerSettingsManager.cs（MaxIndexSize/MaxMemorySize 装载校验）+ test/standalone/Garnet.test/GarnetServerConfigTests.cs
...
use wkv::{
  Error, INDEX_BUCKET_BYTES, INDEX_BUCKET_DATA_SLOTS, MAX_INDEX_SIZE, MIN_INDEX_SIZE, StoreConfig,
  WedbStore,
};
...
#[compio::test]
async fn from_components_accepts_equal_capacity() -> Void {
  let dir = tempdir()?;
  ...
  assert_eq!(store.config.index_size, store.index.load().size);
