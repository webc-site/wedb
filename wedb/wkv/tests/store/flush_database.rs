...
#[test]
fn test_flush_release_waits_safety_epoch_and_old_domain_recovers() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempfile::tempdir()?;
    let mut cfg = StoreConfig::new(1024, 64 * 1024, 16, 0.5)?;
    ...
    old.set_virtual_context(vns, old_vdb);
    assert_eq!(
      old.range_index_get(b"idx", b"field").await?,
      Some(b"value".to_vec()),
...
#[test]
fn test_tiered_same_name_cross_db_isolation() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempfile::tempdir()?;
...
#[test]
fn test_open_shared_reclaimer_drives_release() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let mut cfg = StoreConfig::new(1024, 64 * 1024, 16, 0.5)?;
