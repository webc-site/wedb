...
/// 解除后重试收敛（主端重试对齐契约）
#[test]
fn rename_snapshot_failure_keeps_dst_and_restores_source() -> Void {
...
fn fail_range_index_stream_sink(_: &(), event: StoreEvent<'_>) -> wkv::Result<()> {
  match event {
...
#[test]
fn rename_emit_failure_restores_source_and_keeps_dst() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
#[test]
fn rename_post_emit_failure_compensates_replica_and_releases_claim() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
/// 竞态）
#[test]
fn rename_concurrent_source_writes_rejected_not_lost() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
#[test]
fn rename_concurrent_dst_writes_rejected_not_lost() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
/// 路径回归）
#[test]
fn rename_absent_dst_claims_nothing_and_recovers() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
#[test]
fn rename_old_key_terminal_state_rejects_late_writes() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
fn fail_dst_env_tombstone_sink(_: &(), event: StoreEvent<'_>) -> wkv::Result<()> {
  match event {
    StoreEvent::Write {
      key,
      tombstone: true,
...
#[test]
fn rename_dst_clear_failure_releases_both_claims() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
#[test]
fn rename_sampling_failure_keeps_concurrent_claim_intact() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
#[test]
fn rename_claim_try_failure_never_deletes_others_claims() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
...
/// TTL 刻度比对即判点位置的直接判据。
#[test]
fn rename_window_string_writes_rejected_zero_side_effect() -> Void {
  Runtime::new()?.block_on(async {
    let dir = tempdir()?;
