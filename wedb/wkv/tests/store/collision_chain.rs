...
use crate::support::{HashIndexTestOps, read_cache_holds_key};
...
fn find_collision<D: Device>(session: &StoreSession<D>, anchor: &[u8], mask: u64) -> String {
  ...
  buf.extend_from_slice(b"collision-twin-");
...
#[compio::test]
async fn test_disk_collision_chain_read_and_delete() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_rc_collision_head_buried_delete() -> Void {
  let dir = tempdir()?;
  let device = Arc::new(SegmentedDevice::single_file(
  ...
  assert_eq!(session.read(&victim).await?, Some(victim_val.clone()));
  let phys_v = session.session_string_key(&victim);
  let victim_slot = store.index.load().find_tag(phys_v.as_slice()).unwrap();
  assert!(
  ...
  let phys_t = session.session_string_key(twin.as_bytes());
  assert!(
    is_read_cache(store.index.load().find_tag(phys_t.as_slice()).unwrap()),
...
/// Helpers 中 FindTagAndTryEphemeralXLock 到 TsavoriteBase.FindTag 的「只查不建」语义）：
...
#[compio::test]
async fn test_delete_miss_on_full_chain_never_allocates_overflow() -> Void {
  let dir = tempdir()?;
  let device = Arc::new(SegmentedDevice::single_file(
  ...
  let bucket_mask = index.mask as u64;
  ...
  let missing_tag = HashBucketEntry::tag_from_hash(missing_hash) as u64;
  ...
  for &(hash, addr) in &filler_hashes {
    index.insert_by_hash(hash, addr)?;
  }

  let before = index.overflow_pool.allocated_count();
  assert_eq!(
    before, 2,
  ...
  let live = (0u64..)
    ...
    .find(|k| {
      ...
      hash & bucket_mask == bucket_idx
        && !filler_tags.contains(&(HashBucketEntry::tag_from_hash(hash) as u64))
        && HashBucketEntry::tag_from_hash(hash) != missing_tag as u16
    })
    .expect("对照键搜索不可能耗尽");
...
/// Tag 碰撞链深层键的 ReadCache 读穿透回填与页驱逐链缝合（本票修复核心，严格对标
/// libs/storage/Tsavorite/cs/src/core/Index/Tsavorite/Implementation/TryCopyToReadCache.cs 的 TryCopyToReadCache
/// 的链首插入契约与 ReadCache.cs 的 ReadCacheEvictChain）：
/// 1. 碰撞对双键写满冷区后先后触发读穿透：深层键（其地址仅存于碰撞链头前驱的
///    prev_address、从不在哈希槽位中）必须同样成功回填 ReadCache——槽位精确匹配
///    旧契约下深层键 CAS 期望恒不成立，挂载 100% 失败、永久读穿透；
/// 2. 双键后续读取均直接命中 ReadCache 链；
/// 3. 冷读流量写满 ReadCache 环形窗口触发页驱逐：链式摘除缝合高位记录
///    prev_address，双键读取保持正确且无 RETRY_LATER 活锁。
#[compio::test]
async fn test_rc_tag_collision_deep_key_backfill_and_evict_chain() -> Void {
  let dir = tempdir()?;
  let device = Arc::new(SegmentedDevice::single_file(
    dir.path().join("rc_tag_collision.db"),
  )?);
  // 2 页环形窗口（8KB）：filler 冷读回填流量必然回绕驱逐早期 RC 记录
  let config = StoreConfig::new(64, DEFAULT_SECTOR_SIZE, 16, 0.5)?
  ...
  let mask = (store.index.load().mask as u64) | (0x7fffu64 << HashBucketEntry::HASH_TAG_SHIFT);
  let victim = b"rc-deep-victim".to_vec();
  let twin = find_collision(&session, &victim, mask);

  // 碰撞对双键写入并全量驱逐至磁盘冷区：twin 后写 CAS 抢占碰撞槽位头，
  // victim 记录被链入 prev 链深处
  session.upsert(&victim, b"victim-deep-value").await?;
  session.upsert(twin.as_bytes(), b"twin-deep-value").await?;
  store.flush_and_evict_all().await?;

  // 先回填链头键 twin，再触发深层键 victim 的读穿透：挂载时哈希槽位为 twin
  // 的 RC 记录，链首插入契约下 victim RC 的 prev_address 恒取槽位链头并 CAS
  // 抢占链头，整条旧链完整串接其后
  assert_eq!(
    session.read(twin.as_bytes()).await?,
    Some(b"twin-deep-value".to_vec())
  );
  assert_eq!(
    session.read(&victim).await?,
    Some(b"victim-deep-value".to_vec())
  );

  // 双键均在 ReadCache 链上可走查命中（深层键不因 Tag 碰撞被拒挂）
  let phys_v = session.session_string_key(&victim);
  let phys_t = session.session_string_key(twin.as_bytes());
  assert_eq!(
    read_cache_holds_key(&store, phys_v.as_slice()),
    Some(true),
    "碰撞链深层键 victim 必须成功回填 ReadCache"
  );
  assert_eq!(
    read_cache_holds_key(&store, phys_t.as_slice()),
    Some(true),
    "碰撞链头键 twin 必须保持 ReadCache 命中"
  );

  // filler 冷读回填写满环形窗口，强制 victim/twin 的 RC 记录被页驱逐：
  // 链式摘除必须缝合高位记录 prev_address，读者不再顺链触碰已驱逐页
  for i in 0..1024u32 {
    let k = format!("rc-evict-filler-{i:04}");
    session.upsert(k.as_bytes(), b"filler").await?;
  }
  store.flush_and_evict_all().await?;
  for i in 0..1024u32 {
    let k = format!("rc-evict-filler-{i:04}");
    assert_eq!(session.read(k.as_bytes()).await?, Some(b"filler".to_vec()));
  }

  // 驱逐后双键读取保持正确（链缝合正确性；未缝合将 RETRY_LATER 活锁挂死测试）
  assert_eq!(
    session.read(&victim).await?,
    Some(b"victim-deep-value".to_vec())
  );
