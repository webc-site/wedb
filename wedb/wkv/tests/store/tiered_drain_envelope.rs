...
//!
//! 自研依据: doc/zh/collection.md「懒降阶与删空自愈」分层排空信封
...
#[compio::test]
async fn test_drain_dual_state_key_tombstones_envelope() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_drain_migration_arm_preserves_envelope_and_ttl() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_delete_dual_state_key_unchanged() -> Void {
  let dir = tempdir()?;
