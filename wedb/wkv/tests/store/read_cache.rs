...
use crate::support::read_cache_holds_key;
...
#[compio::test]
async fn test_read_cache_rmw_atomic_detach_and_invalidate() -> Void {
  let dir = tempdir()?;
  ...
  let window = match batch.try_rmw_window(key) {
    Some(w) => w,
    None => batch.rmw_window(key).await?,
  };
  let old = batch.read(key).await?;
  let mut cur = old.unwrap();
  cur.extend_from_slice(b"_modified");
  let res = window.upsert_rmw(&cur).await?;
  assert_ne!(res, 0);
  drop(window);
...
#[compio::test]
async fn test_read_cache_upsert_atomic_detach_and_invalidate() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_read_cache_delete_atomic_detach_and_invalidate() -> Void {
  let dir = tempdir()?;
  let device = Arc::new(SegmentedDevice::single_file(
          ...
          for round in 0..10 {
            for i in 0..num_keys {
              ...
              if (w + round) % 2 == 0 {
                ...
                if let Some(win) = batch.try_rmw_window(&k) {
                  let _ = win.upsert_rmw(&v).await;
                }
              }
            }
          }
...
fn test_read_cache_wraparound_no_tearing() -> Void {
  ...
  let rc = Arc::new(ReadCache::new(1024, 4, true, Arc::new(LightEpoch::new(8)))?);
  ...
  for w in 0..WRITERS {
    ...
    handles.push(spawn(move || {
      ...
      for i in 0..APPENDS_PER_WRITER {
        ...
        // 对标生产 upsert 的 FindOrCreateTag 单槽契约：同 Tag 碰撞键共享链头
        // 槽位，新地址链首 CAS（hei.TryCAS），旧链经记录 prev_address 串接；
        // append 内部以链首插入协议挂载 RC 地址（对标 TryCopyToReadCache）
        let hash = HashIndex::hash_key(key.as_bytes());
        if let Ok(mut hei) = index.find_or_create_tag_by_hash_with_min_addr(hash, 0) {
          hei.try_cas(main_addr);
        }
        // 40B 记录（16 头 + 9 键 + 8 值对齐），非整除 1024B 页，强制页界分支与回绕清洗
        let appended = rc.append(key.as_bytes(), b"vvvvvvvv", &index, 0);
        // 回绕换页武装后于本线程出借期安全点泵入纪元延迟关闭（未武装零开销直返）
        rc.pump_close_barrier(&index, None);
        let Some(rc_addr) = appended else {
          continue;
        };
        if i % 64 == 0 {
  ...
  // 终态一致性：同 Tag 碰撞键共享链头槽位（链首插入契约），逐键走查取证——
  // 自 find_tag 槽位沿 RC 前驱链下探，必须命中本键窗口内 RC 记录（且其前驱链
  // 连续可达主日志段），或链落主日志段（本键已清洗摘链后的健康形态）；
  // 断链 / Gone / 落出主日志地址域均为撕裂或清洗截断
  for w in 0..WRITERS {
    for i in (0..APPENDS_PER_WRITER).step_by(64) {
      ...
      assert!(
        chain_holds_key_or_main(&rc, slot, key.as_bytes()),
        "{key} 链走查不可达（断链 / Gone / 落出主日志域）"
      );
    }
  }
...
/// 链走查取证：自槽位链头沿 RC 前驱链下探，命中本键窗口内 RC 记录时其前驱链
/// 必须连续可达主日志段（skip 不断链），否则链落主日志段（地址域 >= 1_000_000，
/// 即测试写入的全部 main_addr 下界）即为达
fn chain_holds_key_or_main(rc: &ReadCache, mut addr: u64, key: &[u8]) -> bool {
  loop {
    if !is_read_cache(addr) {
      return addr >= 1_000_000;
    }
    match rc.with_record(addr, |k, _| Some(k.to_vec())) {
      RcVisit::Found(k) if k == key => {
        return matches!(rc.skip_read_cache(addr), Some(m) if m >= 1_000_000);
      }
      // 碰撞键记录：继续沿其前驱下探
      RcVisit::Found(_) => {}
      // 已作废（closed）记录不比对键、携 prev 续链；链尾 0 落入主日志域判定
      RcVisit::Next(prev) => addr = prev,
      RcVisit::Gone => return false,
    }
    match rc.prev_address_of(addr) {
      Some(prev) => addr = prev,
      None => return false,
    }
  }
