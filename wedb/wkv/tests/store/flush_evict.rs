...
fn test_store_address_advancement_api() -> Void {
  ...
  rt.block_on(async {
    ...
    assert_eq!(store.safe_tail_address(), store.tail_address());
    assert!(store.safe_tail_address() > addr1);
    assert_eq!(store.begin_address(), store.hlog.begin_address());
    assert_eq!(store.head_address(), store.hlog.head_address());
    assert_eq!(store.safe_head_address(), store.head_address());
    ...
    store.flush_all().await?;
    assert!(store.flushed_until_address() >= store.tail_address());

    // 测试 shift_read_only_to_tail
    let tail_before = store.tail_address();
    let ro_tail = store.shift_read_only_to_tail();
    assert_eq!(ro_tail, tail_before);
