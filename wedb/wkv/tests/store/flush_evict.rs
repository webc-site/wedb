...
#[compio::test]
async fn test_flush_and_cold_read_evicted_pages() -> Void {
  // 页面大小采用扇区基准尺寸
...
#[compio::test]
async fn test_flush_and_evict_large_dataset_cold_reads() -> Void {
  let page_size = DEFAULT_SECTOR_SIZE;
...
/// 验证 WedbStore 的完整地址推进 API（对标 C# Tsavorite LogAccessor / TsavoriteLog）
#[compio::test]
async fn test_store_address_advancement_api() -> Void {
  let page_size = DEFAULT_SECTOR_SIZE;
  let env = open_store("addr_advance.db", config(1024, page_size, 16)?)?;
  let store = env.store;
  let session = store.new_session()?;
  ...
  assert!(store.tail_address() > page_size as u64);

  // 验证初始地址状态
  assert_eq!(store.tail_address(), store.hlog.tail_address());
  assert!(store.hlog.tail_address() > addr1);
  assert_eq!(store.begin_address(), store.hlog.begin_address());
  assert_eq!(store.head_address(), store.hlog.head_address());
  assert_eq!(store.hlog.safe_head_address(), store.head_address());
  assert_eq!(store.read_only_address(), store.hlog.read_only_address());
  assert_eq!(store.safe_read_only_address(), store.read_only_address());

  // 刷盘推进 flushed_until_address（涵盖第 0 页与第 1 页已写部分）
  store.flush_all().await?;
  assert!(store.hlog.flushed_until_address() >= store.tail_address());

  // 测试 ReadOnlyAddress 推进至 Tail（经保留的 shift_read_only_address 组合，
  // 含复活池 purge_below 联动，与 whlog 层 ShiftReadOnlyToTail 同构）
  let tail_before = store.tail_address();
  store.shift_read_only_address(tail_before);
  assert_eq!(store.read_only_address(), tail_before);

  // 测试 shift_head_address 将第 0 页驱逐（flushed_until >= page_size，驱逐安全合规）
  store.shift_head_address(page_size as u64);
  assert!(store.head_address() >= page_size as u64);

  // 测试 shift_begin_address
  store.shift_begin_address(page_size as u64).await?;
  assert_eq!(store.begin_address(), page_size as u64);

  // 测试 flush_and_evict_all
  store.flush_and_evict_all().await?;
  assert_eq!(store.read_only_address(), store.tail_address());
  assert_eq!(store.head_address(), store.tail_address());
