...
use log::info;
use wbase::{align::DEFAULT_SECTOR_SIZE, map::HashMap};
use whasher::fast_hash;
use windex::HashBucketEntry;
use wkv::StoreResult;

use crate::support::{HashIndexTestOps, config, open_store, pad};
...
/// 能够完整无遗漏地追溯所有历史版本。
#[compio::test]
async fn test_version_chain_backward_scan() -> Void {
  ...
  let rec_tombstone_addr = store.index.load().lookup_vec(&str_k)[0];
...
/// 4. 验证对已截断地址记录执行 `read()`：安全防御并返回 `RecordOnDisk`，同时自愈删除索引槽位。
...
#[compio::test]
async fn test_edge_truncated_address_defense_and_backward_scan_termination() -> Void {
...
/// 4. 彻底未写入的 Tag 极速判定不存在（NotFound）；
/// 5. 数据落盘驱逐后正确返回 None（RECORD_ON_DISK）并顺利通过异步回退加载。
#[compio::test]
async fn test_find_tag_probe_traceback_and_collision() -> Void {
  ...
  let gold_res = session.try_read_sync(key_gold, |v| v.to_vec())?;
  assert_eq!(
    gold_res,
    StoreResult::Success(val_gold.to_vec()),
    "首项快速探针必须纯同步零拷贝命中返回值"
  );
  ...
  let tomb_res = session.try_read_sync(tomb_key, |v| v.len())?;
  assert_eq!(
    tomb_res,
    StoreResult::NotFound,
    "首项探针检测到墓碑时必须直接返回 NotFound"
  );
  ...
  let mask = 64 - 1; // 64 桶
  let mut first_of_combo: HashMap<(usize, u16), Vec<u8>> = HashMap::default();
  let mut pair_of_combo: HashMap<(usize, u16), (Vec<u8>, Vec<u8>)> = HashMap::default();
  ...
  let (addr_b, _) = store.hlog.append(&phys_b, val_b, addr_a, false)?;
  assert!(store.index.load().update_address(&phys_a, addr_a, addr_b));

  let traceback_res = session.try_read_sync(&key_a, |v| v.to_vec())?;
  assert_eq!(
    traceback_res,
    StoreResult::Success(val_a.to_vec()),
    "TraceBackForKeyMatch 必须沿 prev_address 反向链表成功回溯匹配目标键"
  );

  let direct_b_res = session.try_read_sync(&key_b, |v| v.to_vec())?;
  assert_eq!(
    direct_b_res,
    StoreResult::Success(val_b.to_vec()),
    "首项记录与 key_b 直接匹配时立即命中"
  );

  // 3.2 测试同 Tag 第三键完全未写入的碰撞回退：
  // 索引槽位与回溯链上只有 key_a / key_b，第三把同 (桶, Tag) 碰撞键从未写入，
  // 反向链与候选槽位均无此键时，必须判定未找到并返回 NotFound
  let miss_collision_res = session.try_read_sync(&key_c_non_exist, |v| v.len())?;
  assert_eq!(
    miss_collision_res,
    StoreResult::NotFound,
    "Tag 碰撞但反向链和候选槽位均无此键时，必须判定未找到并返回 NotFound"
  );

  // 4. 完全不存在的 Tag（连哈希桶都没有匹配 Tag）
  let completely_missing = b"random_unregistered_key_9999";
  let no_tag_res = session.try_read_sync(completely_missing, |v| v.len())?;
  assert_eq!(
    no_tag_res,
    StoreResult::NotFound,
    "哈希表中无对应 Tag 时极速返回 NotFound"
  );
  ...
  let disk_probe_res = session.try_read_sync(disk_k, |v| v.to_vec())?;
  assert_eq!(
    disk_probe_res,
    StoreResult::RecordOnDisk,
    "页面已被驱逐至磁盘时，try_read_sync 必须返回 RecordOnDisk"
  );
...
/// 2. 验证 Session 的 `try_read_sync` 能够稳定命中并读取不可变区数据。
/// 3. 多线程并发读取：创建多个 Session（各持有独立 Epoch 参与者与 Guard），
///    8 个线程高并发调用 `try_read_sync` 与 `read_with` 回读历史数据。
...
fn test_epoch_protected_multithreaded_lock_free_reads() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    // 页面大小 DEFAULT_SECTOR_SIZE，总槽位 64，mutable_fraction=0.5
    let env = open_store("lock_free_store.db", config(4096, DEFAULT_SECTOR_SIZE, 64)?)?;
    let store = env.store;
    let session = store.new_session()?;
    ...
    // 2. 单线程验证 try_read_sync 命中
    for (k, v) in &keys {
      let read_res = session.try_read_sync(k, |val| val.to_vec())?;
      assert_eq!(read_res, StoreResult::Success(v.clone()));
    }
        ...
        while !stop.load(Ordering::Acquire) && loop_count < 100 {
          for (k, v) in keys_ref.iter() {
            let res = thread_session
              .try_read_sync(k, |val| val.to_vec())
              .expect("try_read_sync 成功");
            assert_eq!(
              res,
              StoreResult::Success(v.clone()),
    ...
    for (k, v) in &keys {
      let res = session.try_read_sync(k, |val| val.to_vec())?;
      assert_eq!(res, StoreResult::Success(v.clone()));
    }
