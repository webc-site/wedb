...
#[compio::test]
async fn test_basic_upsert_read_delete() -> Void {
...
#[compio::test]
async fn test_in_place_update_vs_copy_update() -> Void {
...
#[compio::test]
async fn test_edge_empty_key_value_crud() -> Void {
  ...
  assert_eq!(
    store.entry_count(),
    0,
    "单版本记录删除后对标 C# HandleRecordElision 脱钩清链，哈希槽位释放"
  );
...
#[compio::test]
async fn test_edge_oversized_records_and_boundary_handling() -> Void {
  let env = open_store("edge_oversized.db", config(1024, DEFAULT_SECTOR_SIZE, 16)?)?;
  let store = env.store;
  let session = store.new_session()?;
...
#[compio::test]
async fn test_try_read_batch_in_memory() -> Void {
