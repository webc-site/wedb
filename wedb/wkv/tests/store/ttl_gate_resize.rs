//! 自研依据: 扩容窗口 TTL 门初筛裁决矩阵（windex 增量扩容与 TTL 收敛协同，本仓扩容机制组件）
use std::sync::atomic::Ordering;

use aok::{OK, Void};
use compio::runtime::Runtime;
use windex::{HashIndex, SPLIT_COMPLETED, SPLIT_UNSTARTED, chunk_offset_for_hash};
use wkv::{TtlGate, store::ResizePhase};
use wtest_base::open_test_store;

use crate::support::{split_status_of, stage_resize};
...
fn check_resize_gate(publish: bool, stage_phase: ResizePhase) -> Void {
  Runtime::new()?.block_on(async {
    for probe_tag in [false, true] {
      ...
      assert_eq!(
        split_status_of(&store, &ttl_key),
        if publish {
          SPLIT_COMPLETED
        } else {
          SPLIT_UNSTARTED
        },
        "分块 {chunk} 协同状态应与装配窗口形态一致"
      );
