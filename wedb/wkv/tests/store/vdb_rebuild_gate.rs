...
#[test]
fn restart_rebuild_resolves_persisted_vid_and_keeps_watermark() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    ...
    let meta = store.create_checkpoint(&cpr_dir, CheckpointType::Snapshot).await?;
    drop(session);
    drop(store);
...
#[test]
fn rebuild_tail_generation_bump_forces_session_reparse() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
