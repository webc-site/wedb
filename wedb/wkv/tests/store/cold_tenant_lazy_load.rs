...
#[test]
fn test_cold_db_lazy_load_no_renumber() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
...
#[test]
fn test_route_idle_evict_and_reload() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let store = Arc::new(WedbStore::open(
...
/// 原样不换号，旧域数据对用户绝不消失
#[test]
fn test_evict_between_cold_check_and_resolve_no_renumber() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let store = Arc::new(WedbStore::open(
...
#[test]
fn test_gc_dead_sweep_expired_prefix() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let store = Arc::new(WedbStore::open(
...
#[test]
fn test_root_domain_immortal() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let store = Arc::new(WedbStore::open(
