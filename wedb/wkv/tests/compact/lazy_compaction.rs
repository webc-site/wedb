...
#[compio::test]
async fn lazy_compaction_frees_garbage_and_advances_begin() -> Void {
  ...
  // 可变区删除原位置设为墓碑（零追加），再统一落盘驱逐并截断只读区：紧缩窗口覆盖全部 100 条记录
  ...
  // 窗口内 = 100 条初版，其中 50 条在可变区原位置设为墓碑（InPlaceDeleter）；存活 50 条；原位置墓碑无旧版本取代，50 条墓碑计为判死丢弃
  assert_eq!(stats.scanned_records, TOTAL_RECORDS);
  assert_eq!(stats.live_copied, TOTAL_RECORDS - DELETE_COUNT);
  assert_eq!(stats.superseded, 0, "原位置墓碑无区间内旧版本被取代");
...
#[compio::test]
async fn lazy_compaction_bounded_seek_rolling_rounds() -> Void {
...
#[compio::test]
async fn lazy_compaction_short_circuit_empty() -> Void {
