...
use std::{convert::Infallible, fmt::Write, sync::Arc};
...
#[compio::test]
async fn spanbyte_compaction_test1_lookup() -> Void {
...
#[compio::test]
async fn spanbyte_compaction_test1_scan() -> Void {
...
#[compio::test]
async fn spanbyte_compaction_test2_multilevel_lookup() -> Void {
...
#[compio::test]
async fn spanbyte_compaction_test2_multilevel_scan() -> Void {
...
#[compio::test]
async fn spanbyte_compaction_test3_with_deletions_lookup() -> Void {
...
#[compio::test]
async fn spanbyte_compaction_test3_with_deletions_scan() -> Void {
...
/// 自定义谓词经 CompactionFunctions 泛型注入（对标 C# TCompactionFunctions 形态）
#[derive(Debug, Default, Clone, Copy)]
struct OddTailFilter;

impl wcompact::CompactionFunctions<WedbStore<SegmentedDevice>> for OddTailFilter {
  type Error = Infallible;

  async fn is_deleted(
    &self,
    _session: &StoreSession<SegmentedDevice>,
    _key: &[u8],
    val: &[u8],
    _now: i64,
  ) -> bool {
...
#[compio::test]
async fn spanbyte_compaction_custom_filter_test1_lookup() -> Void {
  ...
  let compactor = LogCompactor::new(Arc::clone(&store));
  let stats = compactor
    .compact_with_filter(compact_until, CompactionType::Lookup, &OddTailFilter)
    .await?;
...
#[compio::test]
async fn spanbyte_compaction_custom_filter_test1_scan() -> Void {
  ...
  let compactor = LogCompactor::new(Arc::clone(&store));
  let stats = compactor
    .compact_with_filter(compact_until, CompactionType::Scan, &OddTailFilter)
    .await?;
...
#[compio::test]
async fn spanbyte_compaction_custom_functions_test2_lookup() -> Void {
...
#[compio::test]
async fn spanbyte_compaction_custom_functions_test2_scan() -> Void {
...
/// 验证紧缩搬迁 RangeIndex 存根至尾部时自动清除 Flushed 标志（1:1 对标 GarnetRecordTriggers.cs:PostCopyToTail 自愈语义）
#[compio::test]
async fn spanbyte_compaction_post_copy_to_tail_range_index_stub_healing() -> Void {
  let (_dir, store, session) = create_test_store("spanbyte_ri_stub_healing.db")?;

  let tree_tuning = wbftree::TreeTuning {
  ...
  session
    .range_index_create(b"ri:compact:healed", StorageBackendType::Disk, tree_tuning)
    .await?;

  let val_payload = vec![b'x'; 64];
  session
    .range_index_set(b"ri:compact:healed", b"field_alpha", &val_payload)
    .await?;

  // 手动模拟冷存储状态：将存根置为 is_flushed = true 并覆写记录
  let (meta, mut stub) = session
    .load_range_index_stub(b"ri:compact:healed")
    .await?
    .unwrap();
  stub.set_flushed(true);
  assert!(stub.is_flushed());

  let meta_k = session.session_meta_key(b"ri:compact:healed");
  let mut val = [0u8; wval::META_VALUE_SIZE + wbftree::RANGE_INDEX_STUB_SIZE];
  val[..wval::META_VALUE_SIZE].copy_from_slice(&meta.to_bytes());
  stub.encode_into(&mut val[wval::META_VALUE_SIZE..]).unwrap();
  session.upsert_raw(&meta_k, &val).await?;

  // 验证置位成功
  let (_, flushed_stub) = session
    .load_range_index_stub(b"ri:compact:healed")
    .await?
    .unwrap();
  assert!(flushed_stub.is_flushed(), "存根应已被置为 Flushed");
  ...
  let stats = compactor
    .compact(compact_until, CompactionType::Lookup)
    .await?;
  assert!(stats.live_copied >= 1, "存根记录必须被搬迁");

  // 验证搬迁至尾部后，存根被自动清除 Flushed 标志（自愈）
  let (_, healed_stub) = session
    .load_range_index_stub(b"ri:compact:healed")
    .await?
    .unwrap();
  assert!(
    !healed_stub.is_flushed(),
    "紧缩搬迁至尾部后必须自动清除 Flushed 标志（对标 PostCopyToTail 规范）"
  );

  // 验证数据依然正确可查
  let res = session
    .range_index_get(b"ri:compact:healed", b"field_alpha")
    .await?;
