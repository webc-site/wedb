...
#[compio::test]
async fn more_log_compaction_delete_lookup() -> Void {
...
#[compio::test]
async fn more_log_compaction_delete_scan() -> Void {
...
#[compio::test]
async fn more_log_compaction_entire_log_to_tail() -> Void {
...
#[compio::test]
async fn more_log_compaction_multigeneration_continuous_updates() -> Void {
  ...
  // 第二代紧缩（Scan 模式）：可变区原位覆写与原位墓碑，条目数保持 3
  let stats2 = compactor.compact(gen2_cut, CompactionType::Scan).await?;
  assert_eq!(stats2.scanned_records, 3);
  assert_eq!(stats2.live_copied, 2);
  assert_eq!(stats2.superseded, 0, "原位更新与原位墓碑无区间内旧版本取代");
...
#[compio::test]
async fn more_log_compaction_multiversion_single_key_many_updates() -> Void {
  let (_dir, store, session) = create_test_store("more_log_single_key.db")?;

  let k = b"key:hotspot:chain";

  // 写入 10 个递增版本强制形成日志多版本链
  // （步长 8 字节：记录 8 字节对齐后隐式填充可被原位松弛更新复用，
  // 每版本至少增长 8 字节方能保证逐版本尾部追加、绝不原位塌缩）
  let mut val = String::with_capacity(96);
  for v in 1..=10usize {
    val.clear();
...
/// 对标 C# MoreLogCompactionTests DeleteCompactLookup 阶段扫描（精确锚点见本文件 23 行；提前终止为 rust 自有优化）
#[compio::test]
async fn more_log_compaction_scan_mode_stage2_early_termination() -> Void {
