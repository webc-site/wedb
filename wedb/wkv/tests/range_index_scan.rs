...
//! 3. range_index_get_with 闭包借用点查与缺失路径；
//! 4. 边界用例：count = 0、反向区间、Memory 模式报错。
//!
//! 在 garnet 中的相对路径: test/standalone/Garnet.test.rangeindex/RespRangeIndexTests.cs（区间索引扫描）
...
#[compio::test]
async fn test_range_index_scan_stream_basic() -> Void {
  let dir = tempdir()?;
  ...
  session
    .range_index_create(idx_key, StorageBackendType::Disk, TUNE)
    .await?;
  ...
  // 5. 点查闭包借用：命中时闭包收到值切片视图，缺失时收到 None
  let mut hit_val = [0u8; 8];
  let hit = session
    .range_index_get_with(idx_key, b"age_05", |v| {
      let slice = v.expect("age_05 应存在");
      hit_val[..slice.len()].copy_from_slice(slice);
      slice.len()
    })
    .await?;
  assert_eq!(hit, 6);
  assert_eq!(&hit_val[..6], b"val_05");

  let miss = session
    .range_index_get_with(idx_key, b"age_missing", |v| v.map(<[u8]>::to_vec))
    .await?;
  assert_eq!(miss, None);
...
#[compio::test]
async fn test_range_index_memory_mode_scan_rejected() -> Void {
  let dir = tempdir()?;
  ...
  session
    .range_index_create(b"mem_idx", StorageBackendType::Memory, TUNE)
    .await?;
