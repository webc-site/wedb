...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recovery/CheckpointManagerTests.cs
...
#[compio::test]
async fn test_purge_check() -> Void {
  let dir = tempdir()?;
  ...
  for i in 0..6 {
    ...
    let meta = store.create_checkpoint(&ckpt_dir, cp_type).await?;
  ...
  let err = WedbStore::recover(&ckpt_dir, remove_target, Arc::clone(&store.device)).await;
  assert!(matches!(err, Err(Error::MetaNotFound(_))));

  // 验证未被删除的 Token 依然完好且可恢复
  let survivor = tokens[0];
  let recovered_survivor =
    Arc::new(WedbStore::recover(&ckpt_dir, survivor, Arc::clone(&store.device)).await?);
...
#[compio::test]
async fn test_concurrent_sessions_after_recovery() -> Void {
  let dir = tempdir()?;
