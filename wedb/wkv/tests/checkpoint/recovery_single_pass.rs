...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recovery/ComponentRecoveryTests.cs（单趟组件恢复）
...
#[compio::test]
async fn single_pass_recovery_rebuilds_stub_vdb_and_fuzzy_window() -> Void {
  let dir = tempdir()?;
  ...
  // 首进程：三类素材各写一份，再做检查点
  {
    ...
    let addr_miss = {
      ...
      store.hlog.append(&phys_miss, b"rescued", 0, false)?.0
    };
