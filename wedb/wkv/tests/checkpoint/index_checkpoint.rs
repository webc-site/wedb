...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recovery/CheckpointManagerTests.cs + ComponentRecoveryTests.cs（索引检查点与组件恢复）
...
use wcpr::{CheckpointType, Error, read_index_checkpoint_truncated, write_index_checkpoint};
use wdev::SegmentedDevice;
use windex::{HashBucket, HashBucketEntry, HashIndex};
use wkv::{StoreConfig, WedbStore};

/// 无 ReadCache 宿主的解析端口：该端口仅被 `resolve_read_cache` 对**置有 ReadCache 位**
/// 的槽位调用（无 RC 位槽位零开销透传，不产生任何调用）。无读缓存宿主无从将易失 RC
/// 指针顺链解析为主日志真实地址，故一律返回 0（链尽/无主日志对应），令写端净化清零，
/// 复现折叠前 `Option::None` 归零语义
const NO_RC_RESOLVE: fn(&AtomicU64) -> u64 = |_slot| 0;

/// 模糊索引截断（Fuzzy Index Truncation）与 Token 子目录彻底回收：
/// 恢复阶段超出截断点的脏条目与试探态条目被单次流式还原直接截断清零；
/// purge 时 token 专属目录被完全回收，零孤儿目录残留。
#[compio::test]
async fn test_fuzzy_index_truncation_and_token_dir_cleanup() -> Void {
  let dir = tempdir()?;
  ...
  // 模拟 RangeIndex 子目录创建（如 b32/rangeindex）
  ...
  // 验证正常恢复成功
  {
    let rec = Arc::new(WedbStore::recover(&ckpt_dir, token, Arc::clone(&device)).await?);
...
#[compio::test]
async fn test_read_index_checkpoint_truncated_bit_operations() -> Void {
  let dir = tempdir()?;
  ...
  let meta = write_index_checkpoint(&index, 100, &ckpt_dir, token, &NO_RC_RESOLVE).await?;
  ...
  {
    let (rec_index_notrunc, _) = read_index_checkpoint_truncated(&index_path, token, None).await?;
...
#[compio::test]
async fn test_batch_buffer_arbitrary_buckets_and_crc32() -> Void {
  let dir = tempdir()?;
  ...
  for (case_idx, (num_buckets, overflow_count)) in configs.into_iter().enumerate() {
    ...
    let meta =
      write_index_checkpoint(&index, num_buckets * 2, &ckpt_dir, token, &NO_RC_RESOLVE).await?;
    assert_eq!(meta.size, num_buckets);
    assert_eq!(meta.overflow_count, overflow_count);

    let ckpt_path = ckpt_dir.join(wcpr::index_filename(token));
...
/// 5. 销毁原实例模拟断电宕机，新引擎调用 `WedbStore::recover` 执行崩溃恢复；
/// 6. 验证恢复流程自动触发 `run_recovery_pass` 单趟扫描内核，自愈 RangeIndex 存根状态（标记已从检查点恢复）；
...
#[compio::test]
async fn test_range_index_cpr_stub_healing_recovery() -> Void {
  let dir = tempdir()?;
  ...
  // 1. 初始化并写入普通 KV 与 RangeIndex 数据
  {
    ...
    session
      .range_index_create(b"accounts", StorageBackendType::Memory, TUNE)
      .await?;
    session
    ...
    let b32 = wcpr::token_to_base32(token);
    let token_ri_dir = ckpt_dir.join(b32.as_str()).join("rangeindex");
    assert!(
      token_ri_dir.exists(),
...
/// 1. 写入端传入解析单口时，指向读缓存的条目必须顺链回写为主日志真实地址（恢复后键仍可见）；
/// 2. 链尽形态（RC 专属记录无主日志对应，本用例宿主以端口返回 0 复现）条目被安全归零，
///    绝不将物理偏移误当主日志地址；
/// 3. 读取端防御：快照中残留的未解析 ReadCache 指针（如旧版本引擎产物）在恢复时被彻底清零。
#[compio::test]
async fn test_read_cache_pointer_snapshot_resolution() -> Void {
  let dir = tempdir()?;
  ...
  // 1. 写入端解析单口（模拟 store.skip_read_cache_with_wait 恒返地址契约：命中则
  //    剥离 RC 位回写主日志地址，链尽形态返回 0 交下游净化）
  let resolve = |slot: &AtomicU64| {
    let raw = slot.load(Ordering::Relaxed) & HashBucketEntry::ADDRESS_MASK;
    u64::from(raw == rc_addr) * 0x2000
  };
  write_index_checkpoint(&index, 3, &ckpt_dir, token, &resolve).await?;

  let (rec, _) =
    read_index_checkpoint_truncated(ckpt_dir.join(wcpr::index_filename(token)), token, None)
      .await?;
  ...
  let raw_path = ckpt_dir.join(wcpr::index_filename(raw_token));
...
/// 验证 write_index_checkpoint 纯异步持久化、跨 32KB 批处理边界与硬件 CRC32 校验
#[compio::test]
async fn test_write_index_checkpoint_multi_batch() -> Void {
  let dir = tempdir()?;
  ...
  let meta1 =
    write_index_checkpoint(&index, num_buckets + 1, &ckpt_dir, token1, &NO_RC_RESOLVE).await?;
  ...
  let meta2 =
    write_index_checkpoint(&index, num_buckets + 1, &ckpt_dir, token2, &NO_RC_RESOLVE).await?;
  assert_eq!(meta2.size, num_buckets);
  assert_eq!(meta2.overflow_count, 1);
  assert_eq!(meta2.entry_count, num_buckets + 1);

  // 校验持久化文件可完整回读并校验 CRC
  let ckpt1_path = ckpt_dir.join(wcpr::index_filename(token1));
  ...
  info!("write_index_checkpoint 跨 32KB 批处理与纯异步持久化测试通过");
  OK
}
/// 检查点 RI 快照与 hlog tail 刚性对齐回归锁（写者与快照赛跑）。
///
/// 旧实现 BfTree 外层屏障仅覆盖快照同步段：tail 捕获（PREPARE）到 RI 快照之间
/// 落入的树写混进快照，而其 meta.size 增量记录落 hlog `[tail, ∞)` 被恢复截断丢弃，
/// 恢复后 MetaValue.size 与树实际成员数双向漂移（幻影成员 / 僵尸元记录，
/// RI.COUNT O(1) 直读失真、删空自愈误判）。现实现 VersionShift 外层屏障自一致性
/// 截断点捕获之前跨 await 持有（写者异步挂起让出 reactor），快照与截断点严格
/// 一致前缀（对标 C# OnCheckpoint(VersionShift) → SetCheckpointBarrier 语义）。
/// 本测试以交错 set/del 写任务与检查点赛跑采样，恢复后逐键断言
/// MetaValue.size == 树实扫描计数，基线成员（截断点前持久状态）绝不丢失。
#[compio::test]
async fn test_range_index_checkpoint_tail_alignment_under_concurrent_writes() -> Void {
  let dir = tempdir()?;
  let db_path = dir.path().join("ri_tail_alignment.db");
  let ckpt_dir = dir.path().join("checkpoints");
  let ri_dir = dir.path().join("ri_data");
  create_dir_all(&ri_dir)?;
...

  {
    let session = store.new_session()?;
    session
      .range_index_create(b"acct", StorageBackendType::Disk, TUNE)
      .await?;
    // 基线成员：必然位于截断点之前的持久状态
    for i in 0..8u32 {
      session
        .range_index_set(b"acct", format!("base:{i:03}").as_bytes(), b"v")
        .await?;
    }
  }

  // 写者与检查点赛跑：交错 set/del 制造双向漂移窗口采样
  // （树侧加、记录截断 → 幻影；树侧删、减量截断 → 僵尸）
  const ROUNDS: u32 = 1024;
  let writer_store = Arc::clone(&store);
  let writer = spawn(async move {
    let session = writer_store.new_session()?;
    for r in 0..ROUNDS {
      let f = format!("racy:{r:03}");
      session
        .range_index_set(b"acct", f.as_bytes(), f.as_bytes())
        .await?;
      if r % 2 == 1 {
        let g = format!("racy:{:03}", r - 1);
        session.range_index_del(b"acct", g.as_bytes()).await?;
      }
      // 让出 executor：强制写者与检查点状态机在单线程运行时上交叠采样
  ...
  let meta = store
    .create_checkpoint(&ckpt_dir, CheckpointType::FoldOver)
    .await?;
  writer.await.expect("写者任务异常退出")?;
  let token = meta.token;

  // 崩溃恢复：RI 快照树内容与 hlog 截断点重建的元记录必须刚性一致
  drop(store);
  drop(device);
  let device = Arc::new(SegmentedDevice::single_file(&db_path)?);
  let recovered = Arc::new(WedbStore::recover(&ckpt_dir, token, device).await?);
  ...
  let meta_val = session
    .load_meta(b"acct")
    .await?
    .expect("恢复后元记录必须存活");
  let mut scanned = 0usize;
  session
    .range_index_scan_stream(b"acct", b"a", usize::MAX, ScanReturnField::Key, |_, _| {
      scanned += 1;
      true
    })
    .await?;
  assert_eq!(
    meta_val.size as usize, scanned,
    "恢复后 MetaValue.size ({}) 与树成员数 ({scanned}) 漂移：RI 快照与 hlog tail 未刚性对齐",
    meta_val.size
  );

  // 基线成员逐个可读（截断点前持久状态绝不丢失）
  for i in 0..8u32 {
    assert_eq!(
      session
        .range_index_get(b"acct", format!("base:{i:03}").as_bytes())
        .await?,
      Some(b"v".to_vec()),
      "基线成员 base:{i:03} 必须在恢复后存活"
    );
  }

  info!("检查点 RI 快照与 hlog tail 刚性对齐并发赛跑测试通过");
  OK
}
