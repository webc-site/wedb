...
#[test]
fn test_write_index_checkpoint_multi_batch() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    ...
    let meta1 =
      write_index_checkpoint(&index, num_buckets + 1, &ckpt_dir, token1, &|addr| addr).await?;
    ...
    let meta2 =
      write_index_checkpoint(&index, num_buckets + 1, &ckpt_dir, token2, &|addr| addr).await?;
...
#[test]
fn test_range_index_checkpoint_tail_alignment_under_concurrent_writes() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
