...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recovery/RecoveryTests.cs + SimpleRecoveryTest.cs + MiscTests.cs:ForceRCUAndRecover
...
#[compio::test]
async fn test_simple_recovery_foldover() -> Void {
  let dir = tempdir()?;
  ...
  // 1. 初始化源存储并灌入数据
  {
    ...
    for i in 0..num_ops {
      let k = String::from("user_click:") + &pad_str(Buffer::new().format(i), 5);
      let v = String::from("click_count_") + Buffer::new().format(i);
  ...
  // 2. 崩溃恢复并在新 store 上验证
  {
    ...
    for i in 0..num_ops {
      let k = String::from("user_click:") + &pad_str(Buffer::new().format(i), 5);
      let expected_v = String::from("click_count_") + Buffer::new().format(i);
...
/// 全量数据 1:1 回读一致，ReadOnlyAddress 与 FoldOver 同型对齐 TailAddress（快照
/// 基线全程只读），历史键原位覆写降级 RCU 尾部追加且内容即时一致。
///
/// 流程：
/// 1. 写入数据集并创建 Snapshot 快照；
/// 2. 崩溃恢复后，验证 ReadOnlyAddress 对齐 TailAddress（快照基线只读隔离）；
/// 3. 对恢复后的历史键执行等值覆写，验证降级为 RCU 尾部追加（TailAddress 前移）
///    且内容一致；
/// 4. 再次写入新键并落盘校验。
///
/// 快照基线只读的契约依据：C# SnapshotCheckpointSMTask 以独立快照文件设备隔离
/// 主日志运行写与快照字节（libs/storage/Tsavorite/cs/src/core/Index/Checkpointing/
/// SnapshotCheckpointSMTask.cs；Recovery.cs:465/:595 经 useSnapshotFile 分支读页），
/// in-place 恢复语义与独立快照文件是配套设计；本实现无独立快照文件（主日志即
/// 快照字节唯一载体），隔离性由 ro=tail 单语义等价达成——恢复实例的原位写若可
/// 触碰 [begin, tail) 快照区间，二次崩溃恢复装载污染页后 AOF 重放同一非幂等
/// 命令即静默双算。
#[compio::test]
async fn test_simple_recovery_snapshot() -> Void {
  let dir = tempdir()?;
...

  {
    ...
    for i in 0..num_ops {
      let k = String::from("device_metric:") + &pad_str(Buffer::new().format(i), 4);
      let v = String::from("cpu_load_") + Buffer::new().format(i);
      session.upsert(k.as_bytes(), v.as_bytes()).await?;
    }
...

  {
    ...
    // Snapshot 恢复语义：ReadOnlyAddress 与 FoldOver 同型对齐 TailAddress，
    // 快照基线全程只读（隔离性契约依据见本测试函数头文档——C# 以独立快照
    // 文件达成的隔离由 ro=tail 单语义等价承接）
    assert_eq!(
      recovered.read_only_address(),
      recovered.tail_address(),
      "Snapshot 恢复后 ReadOnlyAddress 必须对齐 TailAddress（快照基线只读）: ro={:#x}, tail={:#x}",
      recovered.read_only_address(),
      recovered.tail_address()
    );
    ...
    for i in 0..num_ops {
      let k = String::from("device_metric:") + &pad_str(Buffer::new().format(i), 4);
      let expected_v = String::from("cpu_load_") + Buffer::new().format(i);
      assert_eq!(
    ...
    // 对恢复后的历史键执行等值覆写：快照基线只读（ro=tail）下原位臂冻结，
    // 降级为 RCU 尾部追加——TailAddress 必然前移（新记录承接覆写效果），
    // 内容即时一致
    ...
    session
      ...
      .await?;
    assert!(
      recovered.tail_address() > tail_before,
      "快照基线只读下等值覆写必须降级 RCU 追加（TailAddress 前移）: tail {:#x} -> {:#x}",
      tail_before,
      recovered.tail_address()
    );
...
#[compio::test]
async fn test_should_recover_begin_address() -> Void {
  let dir = tempdir()?;
...

  {
    ...
    for i in 0..1000 {
      let k = String::from("account:") + &pad_str(Buffer::new().format(i), 4);
      let v = String::from("balance_") + Buffer::new().format(i);
      session.upsert(k.as_bytes(), v.as_bytes()).await?;
...
#[compio::test]
async fn test_read_and_update_info() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_full_checkpoint_periodic() -> Void {
  let dir = tempdir()?;
