...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recovery/RecoveryCheckTests.cs
...
#[compio::test]
async fn test_empty_store_checkpoint_and_recovery() -> Void {
  let dir = tempdir()?;
  ...
  // 从空快照恢复
  {
    ...
    let recovered2 = Arc::new(WedbStore::recover(&ckpt_dir, token2, device2).await?);
...
/// 新尾页槽位清零起步（不装载预热），历史数据安全回读，新写入从新页第 0 偏移无缝追加。
#[compio::test]
async fn test_page_boundary_aligned_checkpoint_recovery() -> Void {
  let dir = tempdir()?;
...

  {
    ...
    head_before = store.head_address();
  ...
  // 崩溃恢复
  {
    ...
    // head 保留 checkpoint 元数据值（[head, tail) 驻留窗口整段装载，对标 C#
    // DoPostRecovery 的 RecoveryReset 保留快照 headAddress 语义）
    assert_eq!(recovered.head_address(), head_before);

    // 回读全量历史数据（offset==0 的新尾页槽位清零，不参与装载）
    for i in 0..num_records {
      let k = format!("page_key_{i:07}");
...
/// WedbStore 以 `CheckpointWhileEpochProtected` 类型化错误拒绝保护区内的发起
/// （零副作用、零残留可恢复视图），退出保护区后同一调用正常完成且崩溃恢复 100% 正确。
#[compio::test]
async fn test_checkpoint_under_epoch_protected_caller() -> Void {
  let dir = tempdir()?;
