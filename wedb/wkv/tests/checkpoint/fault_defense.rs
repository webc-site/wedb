...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recovery/CheckpointManagerTests.cs + test/SimulatedFlakyDevice.cs（注入故障设备）
...
#[compio::test]
async fn test_corrupted_files_defense() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_adversarial_fault_injection() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_meta_integrity_seal_defense() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_fallback_cleanup_defense() -> Void {
