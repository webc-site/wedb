...
use wval::{GarnetObjectType, CompactHashCodec, META_VALUE_SIZE, MetaValue, StorageEncoding};
...
fn test_hexpire_at_return_codes_and_conditions() -> Void {
  Runtime::new()?.block_on(async {
    ...
    assert_eq!(
      CompactHashCodec::find(payload, b"a")
        .unwrap()
        .expire_at_ticks,
      Some(future),
      "字段 TTL 必须精确落载"
    ...
    assert_eq!(
      CompactHashCodec::find(payload, b"b")
        .unwrap()
        .expire_at_ticks,
      None,
      "未设置 TTL 的字段不受影响"
    ...
    // 1：XX——已有 TTL 的字段放行
    let later = future + TICKS_PER_SECOND * 10;
    assert_eq!(
      session
    ...
    let shorter = now_ticks() + TICKS_PER_SECOND * 30;
    assert_eq!(
      session
...
/// 测试 8: 大 hash 物理编码不支持字段级 TTL——返回 -3 与 -2「不存在」显式区分
///（C# Garnet 字段级过期全程在内存 HashObject 上无此分支，-3 为 wedb 双物理
/// 后端扩展码，见 wkv collection.rs hash_field_ttl 文档）
#[test]
fn test_hexpire_unsupported_encoding_returns_minus3() -> Void {
  Runtime::new()?.block_on(async {
    let (_dir, store) = open_store("enc3").await?;
    let session = store.new_session()?;
    let future = now_ticks() + TICKS_PER_SECOND * 60;

    // Flattened（whlog 打平）后端：集合存在但 -3，与不存在键的 -2 区分
    assert!(session.flattened_hset(b"ht:flat", b"a", b"1").await?);
    assert_eq!(
      session
        .hexpire_at(b"ht:flat", b"a", future, TtlOpt::NONE)
        .await?,
      -3
    );
    ...
    assert_eq!(
      session
        .hexpire_at(b"ht:missing", b"a", future, TtlOpt::NONE)
        .await?,
      -2
    );
