...
///
/// C# ExpiredKeysBase 的 IScanIteratorFunctions 五回调在 rust 闭包迭代器下
/// 折叠为本函数的参数线与循环体，逐枚举挂准（一处定义，禁止扫描点再长第二套）：
/// libs/server/Storage/Session/Common/ArrayKeyIterationFunctions.cs:Initialize
/// （storageSession/键注入 → `session`/`store`/`db_match` 参数线）；
/// libs/server/Storage/Session/Common/ArrayKeyIterationFunctions.cs:Reader
/// （逐记录判定 → `next_ref` 闭包体：墓碑/非 TTL/未到期 Skip，到期 Accept 收集）；
/// libs/server/Storage/Session/Common/ArrayKeyIterationFunctions.cs:OnStart
/// （totalCount/deletedCount 清零 → `scanned`/`picked` 零值起点）；
/// libs/server/Storage/Session/Common/ArrayKeyIterationFunctions.cs:OnStop
/// （扫描收尾 → 循环 break 后的游标/线头返回值）；
/// libs/server/Storage/Session/Common/ArrayKeyIterationFunctions.cs:OnException
/// （C# 吞异常续扫 → rust `Err` 上传播报，调用方分段容错重试，不留哑异常）
///
  ...
  loop {
    ...
    let next = scan
      .next_ref(|item| {
        ...
        // 直设落域显式携版本轨入账逻辑域（键前缀解出的是物理对，活域正查、
        // 死域孤域代位，见 version_domain_of 单点）
        let (lns, ldb) = store.vdb.version_domain_of(ns, db);
...
impl<D: Device> GcManager<D> {
  ...
  ///
  /// StorageSession 会话级驱动口（funcs 初始化 + ScanCursor 驱动 + 双计数回传）
  /// 的 rust 对位：libs/server/Storage/Session/Common/ArrayKeyIterationFunctions.cs:ExpiredKeyDeletionScan
  pub(super) async fn sweep_expired(
    ...
    for (ns, db, key) in &picked {
      // 逐键删除前落域：物理对直设，版本轨入账逻辑域随带（与收集臂同单点换算）
      let (lns, ldb) = store.vdb.version_domain_of(*ns, *db);
      session.set_virtual_context(*ns, *db, lns, ldb);
