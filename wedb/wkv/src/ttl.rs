...
/// TTL 记录值定长字节数（8 字节大端 i64 .NET Ticks，无需 bitcode）
...
/// TTL 记录值解码（8 字节大端 i64 .NET Ticks，非法长度按无 TTL 容错处理）
///
/// 模块内私有：crate 出口为 `wval::TtlCodec`（node 已直依 wval），wkv 不再导出
#[inline(always)]
const fn ttl_val(v: &[u8]) -> Option<i64> {
  TtlCodec::decode(v)
}
...
impl<D: Device> StoreSession<D> {
  ...
  pub(crate) async fn purge_expired(&self, user_key: &[u8], expire_at_ticks: i64) -> Result<()> {
    ...
    drop(_suppress);
    if !self.store.aof_listeners_paused.load(Relaxed)
      && let Some(listener) = self.store.ttl_purge_listener()
    {
      listener(
        self.namespace(),
        self.active_db(),
  ...
  ///
  /// `now` 为 .NET Ticks 判定基准（与 TTL 记录值同域）
  pub fn probe_ttl(&self, user_key: &[u8], now: i64) -> TtlProbe {
    let ttl_k = self.ttl_key(user_key);
  ...
  pub async fn check_expired(&self, user_key: &[u8]) -> Result<bool> {
    ...
    // 到期判定取严格小于（对标 C# LogRecordUtils.cs:20 读路径口径）
    if exp >= now_ticks() {
      return Ok(false);
    }
