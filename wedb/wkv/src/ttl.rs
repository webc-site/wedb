...
impl<D: Device> StoreSession<D> {
  ...
  pub fn ttl_key(&self, user_key: &[u8]) -> TaggedKeyBuf {
    ...
    Self::ttl_key_with_prefix(prefix.as_slice(), user_key)
  }

  /// 显式前缀编码 key 级 TTL 记录物理键（循环前缀外提内核，transpile SKILL 工程准则；
  /// rust 工程优化无 c# 对应）
  #[inline(always)]
  ...
  pub(crate) async fn ttl_record_of(&self, ttl_k: &[u8]) -> Result<Option<i64>> {
    Ok(self.read_raw_with(ttl_k, I64Codec::decode).await?.flatten())
  }
