...
use std::{collections::HashMap, mem::size_of, sync::atomic::Ordering};
...
/// 直方图容器（键 = 计数，值 = 桶数）
type Hist = HashMap<u32, u64, GxBuildHasher>;
...
/// 有效条目判定（与 [`WedbStore::entry_count`] 同式：主存地址 ≥ begin 或
/// 读缓存滑窗内 / 折算回主存 ≥ begin）
#[inline]
...
impl<D: Device> WedbStore<D> {
  ...
  /// 三个直方图），计数口径按 windex 真实结构收敛：有效条目判定与
  /// [`WedbStore::entry_count`] 同式，非零且判无效计入 below-begin 计数，
  /// 全零槽计入空闲槽。O(桶数) 诊断扫描，仅供显式 INFO STOREHASHTABLE
  /// 的慢路径执行域触发。
