...
use wdev::Device;
use wval::{KeyTag, MetaValue, NamespaceDbCodec, SessionPrefixBuf};
...
impl<D: Device> WedbStore<D> {
  ...
  pub async fn flush_database(self: &Arc<Self>, ns: u64, db_id: u64) -> Result<(u64, u64)> {
    ...
    if let Some(old_vdb) = old_vdb_opt {
      ...
      self.reclaim_bftree_keys(vns, old_vdb, keys);
    }
    ...
    match rec {
      ...
      DbMetaRecord::GcDeadNs { old_vns, .. } => {
        for (dvns, dvdb, keys) in self.take_bftree_domains_of_vns(old_vns) {
          self.reclaim_bftree_keys(dvns, dvdb, keys);
        }
      }
      _ => {}
    }
  ...
  ///
  /// 入口持 lock_dbmeta 串行闸覆盖 shift_begin_address -> range_clear ->
  /// vdb.reset -> clear_bftree_domains 全编排，与既有四持闸者同形复用单机制，
  /// 杜绝并发换号时死亡登记蒸发与旧号复用撞号；编排内无 lock_dbmeta 再入，
  /// 互不嵌套契约不破。
  pub async fn flush_all_databases(self: &Arc<Self>) -> Result<()> {
    let _lock = self.lock_dbmeta().await;
