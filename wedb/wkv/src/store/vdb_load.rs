...
impl<D: Device> WedbStore<D> {
  ...
  /// `MAX_DATABASES_MIN..=MAX_DATABASES_MAX`——故一切合法在册记录的 logic_db
  /// 恒落于此协议绝对上界内，按上界枚举即覆盖全部格。上界改动须与这两个门禁
  /// 同步，此处不另立第二套上界定义。
  ///
  ...
  pub async fn load_routes_of_vns(self: &Arc<Self>, vns: u64) -> Result<()> {
    ...
    session.set_context(0, 0);
    let out = async {
      let space = u64::try_from(wconf::MAX_DATABASES_MAX).expect("库号上界为定值正常量，升位无损");
