    ...
    for bucket in index.buckets.iter() {
      ...
      loop {
        for item in curr_bucket.entries.iter().take(HashBucket::DATA_ENTRIES) {
          ...
          // 单次解码（身份键 = 物理 Meta 键本身，域值与用户键一并解出，域隔离
          // 与树注册同源）：非 Meta 域记录跳过
          let Ok((vns, vdb, tag, user_key)) = NamespaceDbCodec::decode_tagged_key(key) else {
            continue;
          };
          if tag != KeyTag::Meta {
            continue;
          ...
          // 物理键即树身份键直传
          if self.range_index.register_pending(key) {
            count += 1;
          }

          // 恢复期登记换号回收旁表：域 (vns, vdb) 已随上方单次解码解出。登记与
...
struct RecoveryPassVisitor<'a, D: Device> {
  ...
  /// 取交集，只治愈确被索引引用的那一条
  stub_records: HashMap<u64, (Vec<u8>, Vec<u8>)>,
}
