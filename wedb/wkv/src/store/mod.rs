...
pub struct WedbStore<D: Device> {
  ...
  /// 用此 Store 级闸门防并发执行
  pub(crate) reclaim_inflight: AtomicBool,
...
impl SerialLock {
  ...
  pub(crate) async fn acquire(&self) -> SerialLockGuard<'_> {
    loop {
      if self
        .busy
...
impl<D: Device> WedbStore<D> {
  ...
  /// （flush_database / flush_all_databases / flush_namespace / swap_databases / apply_dbmeta_record）
  ...
  fn assemble(
    ...
    let read_cache = Arc::new(
      ...
      .unwrap_or_else(|e| {
        ...
        unsafe {
          ReadCache::new(DEFAULT_SECTOR_SIZE, 8, false, Arc::clone(&epoch)).unwrap_unchecked()
        }
      }),
    );
...
mod tests {
  ...
  //! 烧 CPU」行为面验收）。负控实测：把 `SerialLock::acquire` 改回
