...
impl<D: Device> StoreSession<D> {
  ...
  pub fn etag_key(&self, user_key: &[u8]) -> TaggedKeyBuf {
    ...
    Self::etag_key_with_prefix(prefix.as_slice(), user_key)
  }

  /// 显式前缀编码 key 级 ETag 记录物理键（循环前缀外提内核，transpile SKILL 工程准则；
  /// rust 工程优化无 c# 对应）
  #[inline(always)]
  pub fn etag_key_with_prefix(prefix: &[u8], user_key: &[u8]) -> TaggedKeyBuf {
    NamespaceDbCodec::encode_with_session_prefix(prefix, KeyTag::Etag, user_key)
  }
  ...
  pub async fn etag_of(&self, user_key: &[u8]) -> Result<Option<i64>> {
    ...
    Ok(
      self
        .read_raw_with(&etag_k, I64Codec::decode)
        .await?
        .flatten(),
    )
  }
  ...
  fn has_etag_key(&self, etag_k: &TaggedKeyBuf) -> Result<bool> {
    let _guard = self.enter_gated();
    Ok(self.store.index.load().find_tag(etag_k).is_some())
  }
