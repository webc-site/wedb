...
//! 紧缩按阈值判定经 `wcompact::LogCompactor` 执行；其单条记录死亡判定（四通道短路：
//! 墓碑 → 用户谓词 → 集合历史子键 → TTL 过期/孤儿，见 compact.rs
//! `is_expired_or_orphan_record`）在 TTL 通道以**最新 TTL 态**同口径判定数据记录与
//! TTL 记录两者——过期时一并判死，绝不出现「丢弃 TTL 而迁移数据」的永不过期键；
//! 本模块的后台扫描仍是过期清理的主通道，紧缩期 TTL 判死只是同一最新态判定的幂等复用。
//!
...
impl<D: Device> GcManager<D> {
  ...
  /// 是否应运行后台轮次（每轮重读；禁用或零间隔 = 关闭后台定时循环）
  fn enabled_by_config(&self) -> bool {
  ...
  /// `enabled = false` 或 `scan_interval_ms = 0` 时循环退出，由
  /// `WedbStore::start_gc` 按新配置重新拉起——等价于 C# ReconcilePrimaryTask
  /// 停任务 → 按新间隔重启的外部可观测行为。
      ...
      loop {
        ...
        let interval_ms = match weak.upgrade() {
          ...
          Some(m) => {
            ...
            let cfg = m.cfg.read();
            if !cfg.enabled || cfg.scan_interval_ms == 0 {
              return;
            }
            let ms = cfg.scan_interval_ms.max(MIN_SCAN_INTERVAL_MS);
            drop(cfg);
            drop(m);
            ms
          }
  ...
  pub async fn drive(&self) {
    loop {
      if self.cancel.load(Relaxed) || !self.enabled_by_config() {
        return;
      }
  ...
  async fn sweep_expired(
    ...
    // 过期判定基准：.NET Ticks（与 TTL 记录值同域；key 级与字段级收集共用同一 i64 基准）
    let now = now_ticks();
    ...
    // compact hash 字段级过期域与 key 级同为 i64 .NET Ticks，同域直传
    let fields_deleted = 0u64;
  ...
  /// 的既有分工对本链原样成立（`now` 为 .NET Ticks 过期判定基准，与 TTL 记录值同域）
