//! 自研依据: doc/zh/db.md 虚拟库路由（papaya+ArcSwap 零锁查询、秒级换号）
use std::{
  cmp::Reverse,
...
impl VirtualDbManager {
  ...
  pub fn reset(&self) {
    ...
    // 持闸后全部在册死域 tail_address 必低于本次取样 tail、截断线已覆盖，
    // 紧缩扫描域自不再触达；tail 取样后旧路在途迟到的残段由水位单调不再撞号 +
    // 宽限期机制承接，仅余有界小域滞留，不构成可见面。
    self.gc_dead.clear();
    self.idle_heap.lock().clear();

    // next_virtual_id 去回绕：保留当前分配水位不重置回 1，对齐盘上 0x05
    // 水位镜像「只升不降」语义（rebuild 经 fetch_max/bump_watermark 永不回退），
    // 杜绝旧号复用撞号与内存-盘上发散。
  ...
  /// 推进换号版本代数（FLUSHDB / SWAPDB / FLUSHALL / reset / 重建收尾写面的唯一 mutator；
  /// Release 与 [`crate::session::StoreSession::virtual_domain`]、
  /// [`crate::session::StoreSession::set_context`] 读面的 Acquire 配对，确保「见到新代数」
  /// 的会话必能见到推进代数前已发布的换指、ns_map 与 DbMeta 变更，杜绝以旧路由配新代数钉死幽灵段）
  #[inline]
  ...
  /// 枚举本节点全部常驻活跃虚拟域 `(vns, vdb)`（周期对象收集等后台维护任务的
  /// 物理域枚举单点，冷域未常驻本轮跳过，复用 registered_dbs 避免第二源）
  pub fn list_active_virtual_dbs(&self) -> Vec<(u64, u64)> {
    let ns_pin = self.ns_map.pin();
    let mut out = Vec::new();
    for (_, vns) in ns_pin.iter() {
      for (vdb, _) in self.registered_dbs(*vns) {
        if !self.is_dead_domain(*vns, vdb) {
          out.push((*vns, vdb));
        }
      }
    }
  ...
  /// 版本轨 bump 落账域单点（物理域 → 逻辑域，[`Self::logic_domain_of`] 的
  /// 直设写臂兜底形态）
  ///
  /// 双轨分置：版本轨种子=逻辑域（wkv `bump_watch_version` 经会话逻辑投影
  /// 取用）。直设物理域的后台/回放写臂（内置 GC 死域清扫、AOF 回放守卫、
  /// 周期收集、分层降阶）不落连接面、无在握逻辑上下文，其 bump 落账域经本
  /// 单点**一次性**换算后随 [`crate::StoreSession::set_virtual_context`] 显式
  /// 入账冻结，禁在 bump 解析链路内逐次反查（反查与并发换号竞态会重引固着
  /// 漂移）：
  /// - 在册活域：正查命中，返回其逻辑 `(ns, db)`，与在线面连接会话的逻辑
  ///   槽逐值同值，换号前后同槽承诺成立；
  /// - 换号退役死域：无活逻辑入口，返回物理对本值作**孤域代位**——bump 落
  ///   进活域 WATCH 不消费的孤域槽（该旧域内容已被换号判死，与「换号/flush
  ///   本身零触版本表」的裁净口径正合；与活逻辑域种子至多数值重合的碰撞
  ///   面只多 abort 不少 abort，属安全侧）。
  #[inline]
  pub fn version_domain_of(&self, vns: u64, vdb: u64) -> (u64, u64) {
    self.logic_domain_of(vns, vdb).unwrap_or((vns, vdb))
  }
  ...
  pub fn is_virtual_id_dead_and_expired(&self, vns: u64, vdb: u64, now: i64) -> bool {
    ...
    self.gc_dead.is_in_grace(vns, vdb, now)
  }
