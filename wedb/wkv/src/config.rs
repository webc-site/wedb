...
use whlog::{DEFAULT_MUTABLE_FRACTION, DEFAULT_NUM_PAGES, DEFAULT_PAGE_SIZE, HybridLogConfig};
...
/// 单轮主动过期扫描记录数上限
const DEFAULT_GC_MAX_SCAN_RECORDS: usize = 4096;
...
impl StoreConfig {
  ...
  pub fn from_memory_budget_with_keys(memory_bytes: u64, expected_keys: Option<u64>) -> Self {
    ...
    // 向下取 2 的幂，绝不超预算
    let max_index_size = prev_power_of_two(max_buckets);
    ...
    let index_size = if let Some(keys) = expected_keys {
      ...
      let default_buckets = ((default_index_bytes / INDEX_BUCKET_BYTES as u64) as usize)
        .clamp(MIN_INDEX_SIZE, MAX_INDEX_SIZE);
      prev_power_of_two(default_buckets)
        .min(max_index_size)
        .clamp(MIN_INDEX_SIZE, MAX_INDEX_SIZE)
    };
    ...
    let num_pages = if raw_pages >= MIN_NUM_PAGES {
      prev_power_of_two(raw_pages).clamp(MIN_NUM_PAGES, MAX_NUM_PAGES)
    } else {
      MIN_NUM_PAGES
    };
