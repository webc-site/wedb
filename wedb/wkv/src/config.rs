...
/// 默认哈希索引主桶数基线（65536）
pub(crate) const DEFAULT_INDEX_SIZE: usize = 65536;
...
pub(crate) const DEFAULT_REVIVIFIABLE_FRACTION: f64 = 1.0;

/// 默认最大并发纪元会话数基线（128）
pub(crate) const DEFAULT_MAX_SESSIONS: usize = 128;

/// 最小自适应物理内存预算（256 MB）
pub(crate) const MIN_MEMORY_BUDGET_BYTES: u64 = 256 * 1024 * 1024;

/// 升阶树页缓存全局总预算默认值（256 MiB）
///
/// 长驻升阶树页环与并发升阶 scratch 环共用的字节总闸（wbftree
/// RangeIndexManager.cache_budget 注入源）。256 MiB ≈ 16 棵 16MiB 默认调参树
/// 的常驻和，超限后升阶以 CacheBudgetExhausted 拒绝、集合回落信封态，杜绝
/// 「树数随自动升阶无上限」的内存耗尽面（doc/zh/collection.md 分层防 OOM 承诺）
pub const DEFAULT_TREE_CACHE_BUDGET_BYTES: usize = 256 * 1024 * 1024;

/// 默认最大自动自适应内存预算（32 GB，防止默认独占全部宿主内存）
pub(crate) const MAX_DEFAULT_MEMORY_BUDGET_BYTES: u64 = 32 * 1024 * 1024 * 1024;

/// 默认物理内存占比百分比（25%）
pub(crate) const DEFAULT_MEMORY_PERCENT: u64 = 25;
...
/// ReadCache 内存页数硬下界（2 页）
///
/// 在 garnet 中的相对路径:libs/storage/Tsavorite/cs/src/core/Allocator/AllocatorBase.cs:650-651
/// （`rcs.MemorySize < (1L << rcs.PageSizeBits) * 2` 即拒，正对本仓「预算 / 页容量
/// 推导页数」一条路）与 LogSettings.cs:28 `kMinPageCount = 2`。
///
/// 1 页环形并发协议自洽，但一次换页即整环作废（head == closed_until == 新 tail，
/// head/tail 之间无余页承接第二机会与在途滞留，对标 ReadCacheSettings.cs:32
/// SecondChanceFraction 的余页前提），有效容量塌至零余页。
///
/// 收敛唯一入口 [`StoreConfig::with_read_cache_pages`]（装配口
/// `wnode::service::apply_hlog_overrides` 与检查点恢复回灌
/// `store/cpr_host.rs::from_recovered` 皆经此口，调用方禁另写钳位成第二口径；
/// 历史 1 页检查点经回灌收敛为 2 页而非硬失败）
pub const MIN_READ_CACHE_NUM_PAGES: usize = 2;
...
/// 默认数据库级垃圾回收延时等待秒数（86400 秒 = 24 小时）
pub const DEFAULT_DB_GC_RECLAIM_DELAY_SECS: u64 = 86400;
...
pub struct GcConfig {
  ...
  /// （默认 32；0 = 永不紧缩。segment_size 取设备分段大小）
  pub compaction_max_segments: usize,
  /// 日志紧缩档位（类型一处定义于 `wbase::cfg::LogCompactionType`，对标 C#
  /// libs/server/LogCompactionType.cs:LogCompactionType；默认 None，对标 C#
  /// GarnetServerOptions.CompactionType / RuntimeServerConfig COMPACTION_TYPE
  /// 每轮现取；CONFIG SET compaction-type 经调停消息投影进本字段）：
...
impl Default for GcConfig {
  fn default() -> Self {
    Self {
      ...
      db_gc_reclaim_delay_secs: DEFAULT_DB_GC_RECLAIM_DELAY_SECS,
...
pub struct StoreConfig {
  ...
  /// ReadCache 内存页数（必须为不低于 [`MIN_READ_CACHE_NUM_PAGES`] 的 2 的幂；
  /// 默认 64 页，随 page_size 线性伸缩。仅 `enable_read_cache` 为真时才物化
  /// 整环内存，禁用态零预算占位）
  ...
  pub gc: GcConfig,
  /// 升阶树页缓存全局总预算（字节，0 = 不设限）
  ///
  /// 装配期单点注入 RangeIndexManager（`init_range_index`），此后预算额不变；
  /// 长驻树页环登记与并发升阶 scratch 建树共用本闸，超限升阶显式拒绝并回落
  /// 信封态（恢复/换入挂点正确性优先强制放行仅告警）
  pub tree_cache_budget_bytes: usize,
}
...
impl StoreConfig {
  ...
  /// `tree_cache_budget_bytes`），新增配置字段只在此处落点
  #[must_use]
  pub fn minimal() -> Self {
    Self {
      ...
      gc: GcConfig::default(),
      tree_cache_budget_bytes: DEFAULT_TREE_CACHE_BUDGET_BYTES,
    }
  }
  ...
  pub fn validate(&self) -> Result<()> {
    ...
    // 页数下界仅作不变式：正常路径一律经 with_read_cache_pages 收敛，此处永不触发
    if !self.read_cache_num_pages.is_power_of_two()
      || self.read_cache_num_pages < MIN_READ_CACHE_NUM_PAGES
    {
      let mut msg = String::from("read_cache_num_pages 必须为非零 2 的幂且不低于 ");
      let mut buf = Buffer::new();
      msg.push_str(buf.format(MIN_READ_CACHE_NUM_PAGES));
      msg.push_str("，当前为 ");
  ...
  /// 配置 ReadCache 内存页数（页数收敛唯一入口：低于 [`MIN_READ_CACHE_NUM_PAGES`]
  /// 的入参钳位至下界，其余非零 2 的幂约束由 `validate` 把守）
  pub fn with_read_cache_pages(mut self, num_pages: usize) -> Result<Self> {
    self.read_cache_num_pages = num_pages.max(MIN_READ_CACHE_NUM_PAGES);
    self.validate()?;
    Ok(self)
  }
  ...
  /// 配置升阶树页缓存全局总预算（字节，0 = 不设限）
  pub fn with_tree_cache_budget(mut self, bytes: usize) -> Self {
    self.tree_cache_budget_bytes = bytes;
    self
  }
