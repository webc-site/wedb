...
impl<'a, D: Device, F: ConsistentReadFunctions + ?Sized> ConsistentReadContext<'a, D, F> {
  ...
  pub async fn read_with<R>(
    ...
    let hash = self.session.consistent_read_hash(KeyTag::String, user_key);
  ...
  pub async fn read_tag_with<R>(
    ...
    let hash = self.session.consistent_read_hash(tag, user_key);
    self.functions.pre_single_key_consistent_read(hash)?;
    ...
    Func: FnMut(usize, Option<&[u8]>),
  {
    ...
    let heap_keys;
    let key_slices: &[&[u8]] = if record_keys.len() <= STACK_KEYS_LIMIT {
      for (slot, k) in stack_keys.iter_mut().zip(record_keys.iter()) {
        *slot = k.as_slice();
      }
      &stack_keys[..record_keys.len()]
    } else {
      heap_keys = record_keys.iter().map(|k| k.as_slice()).collect::<Vec<_>>();
      &heap_keys
    };
  ...
  fn test_consistent_read_lifecycle() {
    Runtime::new().unwrap().block_on(async {
      ...
      assert_eq!(
        ...
        record_key_hash(record_key.as_slice()),
        "一致读触发哈希须落在记录物理键域"
      );
      assert_ne!(
        fns.last_hash.load(Ordering::SeqCst),
        record_key_hash(b"hello"),
