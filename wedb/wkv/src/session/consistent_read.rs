...
/// 一致读键哈希（whasher 单一哈希域：读侧触发与回放草图同域，对标
/// libs/storage/Tsavorite/cs/src/core/ClientSession/ConsistentReadContext.cs:GetKeyHash）
#[inline]
pub fn key_hash(user_key: &[u8]) -> i64 {
  whasher::fast_hash(user_key) as i64
}
...
impl<'a, D: Device, F: ConsistentReadFunctions + ?Sized> ConsistentReadContext<'a, D, F> {
  ...
  #[inline]
  pub fn with_consistent_read<R>(&self, user_key: &[u8], f: impl FnOnce() -> R) -> Result<R> {
    single_key_around(self.functions, key_hash(user_key), f)
  }
  ...
  pub async fn read_with<R>(
    ...
    self
      .functions
      .pre_single_key_consistent_read(key_hash(user_key))?;
  ...
  pub async fn read_tag_with<R>(
    ...
    self
      .functions
      .pre_single_key_consistent_read(key_hash(user_key))?;
    let result = self.session.read_tag_with(user_key, tag, f).await;
    self.functions.post_single_key_consistent_read_callback();
    result
  }
  ...
  pub fn try_read_tag_sync_with_size<R>(
    ...
    self
      .functions
      .pre_single_key_consistent_read(key_hash(user_key))?;
    let result = self.session.try_read_tag_sync_with_size(user_key, tag, f);
    self.functions.post_single_key_consistent_read_callback();
    result
  }
  ...
  pub async fn read_tag_with_size<R>(
    ...
    self
      .functions
      .pre_single_key_consistent_read(key_hash(user_key))?;
    let result = self.session.read_tag_with_size(user_key, tag, f).await;
    self.functions.post_single_key_consistent_read_callback();
    result
  }
...
mod tests {
  ...
  fn test_consistent_read_lifecycle() {
    Runtime::new().unwrap().block_on(async {
      ...
      assert_eq!(fns.last_hash.load(Ordering::SeqCst), key_hash(b"hello"));
