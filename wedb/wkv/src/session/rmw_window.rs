...
//! ephemeral 闩，其与本窗口的交错面即该两基结构：两侧取闩纪律一律不改（把
//! DEL/SET 纳入本窗即信封写回自身嵌套记录闩的重入折算，双锁序死锁面），交叠
//! 裁决改由落笔前终态复验承接（见 wnode
//! `resp/objects/object_store_utils.rs:obj_save_recheck_sync` 与同名异步档，
//! 票 r6-del-rmw-writeback-revalidate）——复验不过即弃写，按命令语义走既有
//! 降级 / 存储忙信号，绝不容装载期旧视图尾段盲写。
