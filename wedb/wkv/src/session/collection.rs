...
impl<D: Device> StoreSession<D> {
  /// 查询键活跃升阶树的常驻页环容量（字节；冷态未打开 / 键非树态返回 0）
  ///
  /// 经 RangeIndexManager 的 live_indexes 查在线树并直读其 `cache_bytes()`
  /// （新建树取配置值、恢复树取快照配置头回读值，整块常驻口径）。唯一消费方
  /// 是 MEMORY USAGE 升阶臂（wnode 快慢双路径）：Meta 域命中时在元记录物理
  /// 尺寸之上并项树驻留页内存，杜绝「页环整块常驻却按非常驻漏报」的口径失实。
  /// C# 无对应——树页缓存无运行态披露面，本方法为 rust 分层自定义面的
  /// 自带义务（树数随自动升阶无上限，常驻页内存必须可观测）
  #[inline]
  pub fn live_tree_cache_bytes(&self, key: &[u8]) -> usize {
    let id_key = self.session_meta_key(key);
    self
      .store
      .range_index
      .get_tree(&id_key)
      .map_or(0, |tree| tree.cache_bytes())
  }
  ...
  ///
  /// 在 garnet 中的相对路径:libs/server/Storage/Functions/MainStore/DeleteMethods.cs:InitialDeleter
  ///
  /// 删除结果收口与同步内核 try_delete_sync_unprotected_with_prefix 完全
  /// 同一判据：双域未命中经 delete_miss_hook 缺席观测（GarnetRecordTriggers
  /// .cs:OnDispose 的 Deleted 语义）视同删除成功；WATCH 版本推进无条件
  /// 执行（缺席键墓碑同向计入 watchVersionMap.IncrementVersion）
  /// DEL/GETDEL 异步链共序单点：RENAME 迁移 claim 判点先于一切清退（DEL 链
  /// 与 SET 两臂同纪律，wkv 写内核单点）：Meta 在场探测 + migration_claimed
  /// 复合判定置于 del_ttl/del_etag 之前，claim 在册即显式回
  /// [`Error::MigrationBusy`]——禁「视同不存在」穿透（load_meta 的缺席穿透仅
  /// 服务探测面；DEL 是写入口，穿透即排空臂放弃后继续销毁信封域：懒降阶窗
  /// 内被拒 DEL 删掉降阶臂刚写回的新信封，drain 落 meta 墓碑后整键消失；
  /// RENAME 窗内被拒 DEL 剥旧键 TTL，破坏「失败即原态」承诺），被拒 DEL 零
  /// 副作用 → 随键 TTL/ETag 旁路记录级联清退（先清除随键 TTL 与 ETag：
  /// C# 删除记录连同记录尾可选 ETag/Expiration 字段一并消失，同时保证后续
  /// load_meta 的 TTL 守卫探测不到记录而不触发二次清除；无旁路记录键仅哈希
  /// 探针零额外写入）
  async fn claim_gate_and_strip_sidecars(&self, key: &[u8]) -> Result<()> {
    let meta_k = self.session_meta_key(key);
    // Meta 在场探针前先走分裂协同（对标 InternalDelete.cs:57-59 入口铁律），
    // 杜绝扩容期漏检在册 claim 键元记录致 RENAME 迁移冲突检测失效
  ...
  pub async fn delete(&self, key: &[u8]) -> Result<bool> {
    self.claim_gate_and_strip_sidecars(key).await?;

    // 快速检查对象元数据防复合对象删除
    // 99.999% 普通键：直接走 delete_raw 极速通道，零复合对象判断与零多余 load_meta 异步查询开销
    let mut deleted = if let Some(false) = self.check_object_meta_fast(key)? {
      ...
      if self.load_meta(key).await?.is_some() {
        ...
        if let Err(e) = self.handle_bftree_drain_and_delete(key, false).await {
      ...
      meta_del || key_del || env_del
    };
    // 双域（String/ObjectEnvelope）皆未命中：宿主值域外登记态缺席观测（对标
    // C# GarnetRecordTriggers.cs:OnDispose Deleted → VectorManager.RequestDeletion）。
    // 命中登记即视同删除成功，与同步内核 try_delete_sync_unprotected_with_prefix
    // 同一判据同一单点，杜绝同步删/异步删双轨分叉（同步内核遇钩子在场即降级
    // 本异步路由，钩子臂为真异步 `.await`，两臂判据同源不分叉）
    if !deleted
      && let Some(hook) = self.store.delete_miss_hook.get()
      && hook.call(self.session_prefix().as_slice(), key).await
    {
      deleted = true;
    }
    // WATCH 版本推进收口（无条件）：缺席键删除同样推进，与同步内核及 C#
    // DeleteMethods.InitialDeleter 严格一致，同步删与异步删不分叉
    self.bump_watch_version(key);
    Ok(deleted)
  }

  /// 取删字符串域记录并回传被摘值（Take String；GETDEL 读删一体慢路径闭环）
  ///
  /// 域判已由调用方带 TTL 裁决的探针收敛（Hit 即 String 域在场），本单点只做
  /// claim 判点前置 + 随键 TTL/ETag 级联 + String 域取删一体摘除 + WATCH 推进
  /// 收口；应答值 = 实际摘除记录的值（捕获与摘除同一临界区），键缺席回 `None`
  pub async fn take_string(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
    self.claim_gate_and_strip_sidecars(key).await?;
    let str_k = self.session_string_key(key);
    let taken = self.take_raw(&str_k).await?;
    self.bump_watch_version(key);
    Ok(taken)
  }
  ...
  pub fn check_object_meta_fast_unprotected_with_prefix(
    ...
    // 极速初筛探针前先走分裂协同（对标 InternalDelete/InternalUpsert 入口铁律，
    // 与 try_delete_sync_unprotected_with_prefix 的 Meta 在场探针同纪律）：
    // 扩容期对未迁移新桶采得 None 即假判「无复合对象」放行普通删，元记录与
    // 树文件脱链成幽灵，杜绝之
  ...
  /// 空树仍为存活索引故不计）物理清除元记录与随键 TTL/ETag 旁路记录后视同不存在，
  /// 杜绝幽灵空元记录引发的状态不一致与孤儿 ETag 泄漏。
    ...
    match self.read_raw_with(&meta_k, MetaValue::from_slice).await? {
      Some(meta) => {
        ...
        if !is_alive {
          ...
          // 幽灵空元记录清除须与 TTL 成对级联清 ETag 旁路记录，杜绝删空自愈
          // 遗留无主孤儿 ETag（对标 C# 记录连同尾随 HasETag 一体消亡）
          let _ = self.del_etag(user_key).await;
          return Ok(None);
        }
        Ok(Some(meta))
      }
  ...
  /// 2. 随键 TTL/ETag 旁路记录分流（keep_ttl 判别，见下）。
  ///
  /// keep_ttl 两态对标 C# 对象记录两形态的过期字段口径：
  /// - false = 删键臂（DEL/过期清除、SET 覆写清退、RMW 删空自愈、STORE 族目标
  ///   清退）：记录随键消亡，HasExpiration/HasETag 亦随之消失（RMWMethods.cs:111
  ///   CheckExpiry → ExpireAndStop 与 DeleteMethods 删除臂连尾随字段一并清除），
  ///   同步 del_ttl + del_etag 清两旁路记录，杜绝孤儿 TTL/ETag；
  ...
  ///   不触碰旁路。此臂绝不清 TTL/ETag，否则一次迁移即静默抹掉 EXPIRE 设置的键级
  ///   过期与条件写基线 etag，且清除会经 TtlWrite(expire_at=None) 镜像成 Persist
  ///   条目扩散到从库与 AOF 回放面。
  ///
  ...
  pub(crate) async fn drain_and_delete_collection_meta(
    ...
    // 元记录墓碑已落盘：后续旁路清理失败属「键已死后的失败」，经
    // [`Error::Swapped`] 分级包装上抛——树物理面已实际变更，分层臂 WATCH 判据
    // 据此保持置脏（推进真实反映变更）；与元记录墓碑失败（键未消亡、零变更，
    // 原样上抛须复位脏标记）在错误面判别
    if !keep_ttl {
      self.del_ttl(user_key).await.map_err(Error::swapped)?;
      // 删键臂随键级联清 ETag 旁路记录：与 del_ttl 成对，对标 C# 对象记录删空
      // ExpireAndStop 时记录连同尾随 HasExpiration/HasETag 可选字段一体消亡；
      // 漏清即遗留无主孤儿 ETag，破坏条件写语义并被紧缩误判存活回拷永生
      self.del_etag(user_key).await.map_err(Error::swapped)?;
    }
    Ok(())
  }
}
