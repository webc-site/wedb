...
type WatchFn = Arc<dyn Fn(&[u8]) + Send + Sync>;

/// WATCH 版本推进钩子（收用户键；宿主上下文类型由 std 闭包对象擦除，擦除形态
/// 见 [`WatchFn`]）
///
...
impl WatchHook {
  ...
  pub fn new<T: Send + Sync + 'static>(ctx: Arc<T>, handler: fn(&T, &[u8])) -> Self {
    Self(Arc::new(move |key: &[u8]| handler(&ctx, key)))
  }
...
type DeleteMissFn = Arc<dyn Fn(&[u8], &[u8]) -> bool + Send + Sync>;
...
impl<D: Device> StoreSession<D> {
  ...
  pub fn set_context(&self, ns: u64, db: u64) -> bool {
    ...
    // 冷检窗口测试留钩（一次性回调，生产恒 None 零负担）
    if let Some(hook) = TEST_COLD_WINDOW_HOOK.lock().take() {
      hook();
    }
...
impl<'a, D: Device> BatchStoreSession<'a, D> {
  ...
  pub fn notify_tiered_collection_write(
    ...
    self.store.notify_tiered_collection_write(notif)
  }
