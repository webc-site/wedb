...
impl<D: Device> StoreSession<D> {
  ...
  pub fn try_upsert_tag_sync_unprotected(
    ...
    if tag == KeyTag::String {
      ...
      if self.store.index.find_tag(&env_k).is_some()
    ...
    self.try_upsert_raw_sync_unprotected(&rec_k, val)
  }
  ...
  ///
  /// SET 语义：清除既有 key 级 TTL 记录（Redis 字符串写入命令一律移除 TTL）；
  ...
  pub async fn upsert_tag(&self, user_key: &[u8], tag: KeyTag, val: &[u8]) -> Result<u64> {
    ...
    let rec_k = self.session_tag_key(tag, user_key);
    self.upsert_raw(&rec_k, val).await
  }
    ...
    match self.check_object_meta_fast_unprotected(user_key)? {
      Some(false) => {
        ...
        match self.try_delete_raw_sync_unprotected(&str_k)? {
          ...
          Ok(false) => {
            let env_k = self.session_tag_key(KeyTag::ObjectEnvelope, user_key);
            self.try_delete_raw_sync_unprotected(&env_k)
          }
        }
      }
      _ => Ok(Err(u64::MAX)),
