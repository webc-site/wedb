...
impl<D: Device> StoreSession<D> {
  ...
  pub fn try_read_raw_in_memory_with_addr<R>(
    ...
    self.with_addr_reader(key, first_addr, f)
  }
  ...
  ///
  /// 带标签读内核（一处定义）：String 版 [`Self::try_read_in_memory_unprotected`]
  /// 与对象信封版共用本实现
  #[inline]
  pub fn try_read_tag_in_memory_unprotected<R>(
    ...
    let first_addr = self.store.index.find_tag(&rec_k);
    self.try_read_raw_in_memory_with_addr(&rec_k, first_addr, f)
  }
  ...
  pub fn try_read_tag_in_memory_with_size<R>(
    ...
    let rec_k = self.session_tag_key(tag, user_key);
    let first_addr = self.store.index.find_tag(&rec_k);
    self.with_addr_reader(&rec_k, first_addr, with_record_size(f))
  }
  ...
  pub fn try_read_in_memory_unprotected<R>(
    ...
    self.try_read_tag_in_memory_unprotected(user_key, KeyTag::String, f)
  }
  ...
  /// 在已有纪元保护下同步读取当前会话指定标签物理键（TTL 快门控 + 内存直读，彻底绕过 enter() 原子开销）
  ///
  /// 返回三态与 [`Self::try_read_sync_unprotected`] 一致；TTL 门控按用户键
  /// （KeyTag::Ttl 旁路记录）裁决，与数据记录标签无关
  #[inline]
  pub fn try_read_tag_sync_unprotected<R>(
    ...
    self.try_read_tag_in_memory_unprotected(user_key, tag, f)
  }

  /// 同步读指定标签物理键快路径并披露记录物理尺寸（TTL 快门控 + 内存直读）
  ///
  ...
  pub fn try_read_tag_sync_with_size<R>(
    ...
    self.try_read_tag_in_memory_with_size(user_key, tag, f)
  }
