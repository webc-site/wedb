...
use wval::{EtagCodec, KeyTag, NamespaceDbCodec, TtlCodec};
...
impl<D: Device> StoreSession<D> {
  ...
  pub(super) fn notify_write_listener(&self, key: &[u8], val: &[u8], tombstone: bool) {
    if !self.store.has_event_sink() || self.store.aof_listeners_paused.load(Ordering::Relaxed) {
      return;
    }
    ...
    if let Some(tag) = NamespaceDbCodec::decode_tag(key) {
      if tag == KeyTag::Ttl {
        if let Ok((ns, db, _, user_key)) = NamespaceDbCodec::decode_tagged_key(key) {
          let expire = (!tombstone).then(|| TtlCodec::decode(val)).flatten();
      ...
      {
        let etag = (!tombstone).then(|| EtagCodec::decode(val)).flatten();
