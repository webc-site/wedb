...
impl<D: Device> StoreSession<D> {
  ...
  ///
  /// # 回调次序契约
  /// 逐块串行推进，`on_item` 的 idx 恒全局升序、每键恰回调一次；`Err` 中止时
  /// 透传 [`Self::read_batch_raw_with`] 的整体丢弃契约（首个磁盘候选之前的项已
  /// 先行交付、无法回滚，调用方须整体丢弃本批部分结果）
  ///
  /// 按预取窗口常量分块，`TaggedKeyBuf`（Copy）在单个栈上数组内编码后逐块调用底层批量读，
  /// 任意批量规模全程零堆物化（仅磁盘候选收割冷路径按需分配）。
  /// 批量读单点对齐单键读 TTL 门控（对位 C# Reader 内 CheckExpiry，LogRecordUtils.cs），
  /// 保证过期键统一返回 None/nil 并驱动惰性清理。
    ...
    F: FnMut(usize, Option<&[u8]>),
  {
    ...
    let mut alive_mask = [true; PREFETCH_WINDOW];
    ...
    for (chunk_ix, chunk) in keys.chunks(PREFETCH_WINDOW).enumerate() {
      ...
      let mut needs_async_ttl = false;
      {
        let _guard = self.enter_gated();
        let now = now_ticks();
        for (i, (stack_k, k)) in stack_keys[..chunk_len]
          .iter_mut()
          .zip(chunk.iter())
          .enumerate()
        {
          let user_k = k.as_ref();
          *stack_k =
            NamespaceDbCodec::encode_with_session_prefix(prefix_slice, KeyTag::String, user_k);
          match self.ttl_gate_mem_at_with_prefix(prefix_slice, user_k, now)? {
            TtlGate::Pass => alive_mask[i] = true,
            TtlGate::Due | TtlGate::Degrade => {
              alive_mask[i] = false;
              needs_async_ttl = true;
            }
          }
        }
      }
      if needs_async_ttl {
        for (i, k) in chunk.iter().enumerate() {
          if !alive_mask[i] {
            alive_mask[i] = self.probe_alive(k.as_ref()).await?;
          }
        }
      }
      let mut f = |i: usize, v: Option<&[u8]>| {
        let val = if alive_mask[i] { v } else { None };
        on_item(chunk_base + i, val);
      };
      self
        .read_batch_raw_with(&stack_keys[..chunk_len], &mut f)
        .await?;
    }
    Ok(())
  }
    ...
    for (chunk_ix, chunk) in keys.chunks(PREFETCH_WINDOW).enumerate() {
      ...
      let chunk_len = chunk.len();
      {
          ...
          .enumerate()
        {
          ...
          *stack_k =
            NamespaceDbCodec::encode_with_session_prefix(prefix_slice, KeyTag::String, user_k);
          alive_mask[i] = match self.ttl_gate_mem_at_with_prefix(prefix_slice, user_k, now)? {
            TtlGate::Pass => true,
            TtlGate::Due | TtlGate::Degrade => false,
          };
        }
      }
      let mut f = |i: usize, v: Option<&[u8]>| {
      ...
      self.try_read_batch_raw_in_memory(&stack_keys[..chunk_len], &mut f)?;
    }
    Ok(())
  }
  ...
  /// MGET 线上协议（按回调序对位写响应、无 idx 参数）依赖此契约。
    ...
    K: AsRef<[u8]>,
  {
    ...
    index.prefetch_batch_probes(
      ...
      |hash| self.ensure_split_by_hash(hash),
      |addr| {
