...
impl<D: Device> StoreSession<D> {
  ...
  pub(in crate::session) fn try_upsert_raw_sync_unprotected(
    ...
    loop {
      ...
      // 优化5（严格对标 C# Helpers.FindOrCreateTagAndTryEphemeralXLock）：复活配置
      ...
      let prev_addr = if !hei.is_found() {
        ...
        if is_read_cache(addr) {
          ...
          // 优化3（对标 C# InternalUpsert.CreateNewRecordUpsert elideSourceRecord +
          // C# Helpers.CanElide）：命中记录即链首槽位（hei.Address == 源记录地址）且其
  ...
  pub(in crate::session) fn try_delete_raw_sync_unprotected(
    ...
    'retry: loop {
      ...
      // 优化5（严格对标 C# Helpers.FindTagAndTryEphemeralXLock）：复活配置开启时，
      ...
      while cur >= head_addr {
        ...
        match probe {
          ...
          Some(MemoryRecordProbe::Active { prev, size }) => {
            ...
            // 优化4（严格对标 C# Helpers.HandleRecordElision）：脱钩清链不再被
