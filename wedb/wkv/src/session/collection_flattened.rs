...
use wdev::Device;
use wval::{GarnetObjectType, CompactHashCodec, KeyTag, MetaValue, StorageEncoding};
...
impl<D: Device> StoreSession<D> {
  ...
  pub async fn flattened_hget_with<R>(
    ...
    if meta.collection_type != GarnetObjectType::Hash || meta.encoding() != StorageEncoding::Flattened
    {
      return Ok(None);
    }
    let sub_k = self.sub_key(KeyTag::Hash, meta.key_id, meta.version, field);
  ...
  async fn collect_flattened_hash_entries(
    ...
    // 字段名候选清单（日志内同名多版本去重）
    let mut fields: HashSet<Box<[u8]>> = HashSet::default();
    self
      .store
      .hlog
      .scan(
