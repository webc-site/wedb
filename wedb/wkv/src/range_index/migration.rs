//! 自研依据: 区间索引升阶迁移（doc/zh/collection.md RIPROMOTE 保序）
use std::{
  fs,
  path::{Path, PathBuf},
...
use wbftree::{
  BfTreeService, ERR_MEMORY_TREE_MIGRATION, RangeIndexManager, RangeIndexStub, StorageBackendType,
};
use wdev::Device;
...
impl<D: Device> StoreSession<D> {
  ...
  pub async fn publish_migrated_range_index(
    ...
    // 发布前置存在性门（libs/server/Resp/RangeIndex/RangeIndexManager.Migration.cs:KeyExists
    // 的承接）：C# GET 门「任何类型命中」一律 true——Found = String 键命中，
    // WrongType = 对象/RI/向量等类型化记录命中——保证迁移绝不覆写或绕开
    // 既有任意形态键。rust 分层域三态对位：String 域命中、存活非 RI 元记录、
    // 集合信封域命中同属「既有键」，纯迁移通道（!replace，C# 世界唯一形态）
    // 一律 AlreadyExists（会话面收敛 SkippedAlreadyExists）；replace = true 是
    // rust 分层扩展的重灌/RENAME 换入合法通道（目的名必为同源树域），不在此收窄。
    // 集合升阶流（obj_type = 原集合类型）例外放行信封命中：升阶写序不变量是
    // 「先发流后删信封」（promote_collection_to_bftree），副本按 AOF 序回放时
    // 升阶前历史条目已重建信封对象，信封在场即预期形态，主端 AOF 流块后随的
    // 信封墓碑条目自会清退——按 C# 门拒放会让副本发布整流跳过（树与元记录俱
    // 缺），后续稳态写镜像条目全部探空，主从永久发散
    if !replace {
      // String 域命中（C# GET Found 臂）
      if self.read(key).await?.is_some() {
        return Err(RangeIndexError::AlreadyExists);
      }
      // Meta 域：存活非 RI 元记录（C# GET WrongType 臂；RI 元记录命中即下方
      // 既有门，不在此重复判定）
      ...
      // 集合信封域命中（C# ValueIsObject 分支）：仅纯 RI 迁移流拒绝
      if obj_type == GarnetObjectType::RangeIndex {
        let env_k = self.session_tag_key(KeyTag::ObjectEnvelope, key);
        if self.contains_key_raw(&env_k).await? {
          return Err(RangeIndexError::AlreadyExists);
        }
      }
      // 既有 RI 元记录门：同名索引已驻留，未指 REPLACE 即拒绝
      if self.range_index_exists(key).await? {
        return Err(RangeIndexError::AlreadyExists);
      }
    }
  ...
  ///   live 并发重建记录混入）→ TTL 随迁（窗内读旧键 TTL 现势 → 旧键排空回收
  ///   单点 [`handle_bftree_drain_and_delete`](StoreSession::handle_bftree_drain_and_delete)
  ///   （信封域幂等墓碑 + 元记录持久墓碑 + 随键 TTL 清理 + 树销毁注销 +
  ///   RangeIndexDrop 入账，副本据此清场旧键）→ 随迁值落新键）→ 释放迁移
  ///   claim（双键成对）。
  ///
  ...
  /// | 臂 | 持有 claim | 释放口 |
  /// |---|---|---|
  /// | 前置早退（旧键元记录缺失/定长不足/解码败/Memory 后端门禁/检查点屏障败） | 无 | 无（claim 未登记，无泄可谈） |
  ...
  /// | 段五守卫读失败 / 判否 / 排空失败 / TTL 随迁读·落失败 | old+new | release_claims |
  ...
  pub async fn rename_range_index(&self, old_key: &[u8], new_key: &[u8]) -> Result<()> {
    ...
    let (old_meta, mut stub) = match meta_and_stub_of(&bytes) {
      Ok(res) => res,
      Err(Error::RangeIndex(range_index::RangeIndexError::WrongType)) => return Ok(()),
      Err(e) => return Err(e),
    };

    // Memory 后端树迁移门禁（C# SnapshotForMigration 存根重读后的同位判定
    // libs/server/Resp/RangeIndex/RangeIndexManager.Migration.cs:307-311）：RI.CREATE
    // MEMORY 经 RESP 可达，memory-only 树无盘工件可迁移——放行会在 ri_log_root
    // 产出盘工件、dst 存根标 Memory 而换入实例硬编码 Disk，重启后 get_or_open_tree
    // 按存根 Memory 建空内存树致数据蒸发，主从同型发散。前置早退臂：迁移 claim
    // 未登记、快照未产出，零补偿直接上抛（失败语义表「前置早退」行；CLUSTER
    // MIGRATE 同文案门禁在 wnode 快照编排层，两处共用 ERR_MEMORY_TREE_MIGRATION
    // 单点禁另起第二文案）
    if stub.storage_backend == StorageBackendType::Memory.to_u8() {
      return Err(RangeIndexError::Internal(ERR_MEMORY_TREE_MIGRATION.to_string()).into());
    }
    ...
    // dst 存活性采样读（claim 窗前置）：承接 C# RENAME 的 GET(newKey) 存活性探测
    // （libs/server/Storage/Session/UnifiedStore/UnifiedStoreOps.cs:298）。rust 设备
    // 读可败而 C# 事务锁为内存闩不可败，故采样读按头注「锁形取舍」前置于互斥
    // 登记窗外——失败臂零持有零补偿（释放配对表「dst 存活性采样读失败 | 无
    // （尚未 try_claim）」行：`?` 直接上抛，禁调 release，未持有即释放会误删并发
    // RENAME 的同源键 claim）。读值不参与 dst 登记判据（下方无条件成对登记，
    // 旧判据「dst 有存活记录才 claim」退役），仅保留失败面与 C# 探测序对位
    let _ = self.load_meta(new_key).await?;
    ...
    // 两段先后串行不嵌套）。
    //
    // dst 侧成对登记：无条件登记（旧判据「dst 有存活记录才 claim」退役）——
    // dst 常态（无存活记录）不登记时，段四 meta 落盘后、随迁落笔前对 dst 的
    // EXPIRE/PERSIST 放行写会被随迁值覆盖（应答 :1 而终态被吞，非可串行化）；
    // 无条件登记后 claim 窗即 C# RENAME 双键排他锁窗的完整对位（窗内并发写
    // 经 wkv expire_at/persist 与写原语各判点零副作用拒绝，客户端重试即线性化
    // 到迁移完成后），并顺带消除「本 RENAME 未持 dst claim 却经 release_claims
    // 误删并发 RENAME 同源键 claim」的释放面瑕疵。claim 表键为 128 位 key_id
    // （按物理 Meta 键哈希派生），两次 try_claim 之间零 await，杜绝「源键已
    // 封堵、dst 未封堵」的半登记窗。
    ...
    if !self.store.range_index.try_claim_migration(&dst_meta_k) {
    ...
    // TTL 随迁下沉本内核 claim 窗内（旧归调用方按窗前快照回填，退役）：
    // 排空前在窗内读旧键 TTL 现势——窗内 EXPIRE/PERSIST 双键已被
    // expire_at/persist 判点零副作用拒绝，快照读取至 claim 登记间隙的插入面
    // 亦折叠于此，读值即随迁终态（对标 C# RENAME 双键锁内 TryCopyFrom 连同
    // Expiration 拷入新记录）；排空清退旧键 TTL 后，随迁值落新键（窗内 dst
    // claim 封堵 EXPIRE(new)/PERSIST(new)，无覆盖竞争）。TtlWrite 镜像经
    // put_ttl/del_ttl 既有漏斗入账（Pexpireat/Persist 条目，时点移入窗内，
    // 条目形态不变）。读/落失败臂：release_claims 后上抛——dst 已换入而
    // TTL 缺失/旧键残留，错误面可见交重试清理（与段五既有失败语义同级）
    let live_ttl = match self.ttl_of(old_key).await {
      Ok(ttl) => ttl,
      Err(e) => {
        release_claims();
        return Err(e);
    ...
    let ttl_migrated = match live_ttl {
      Some(exp) => self.put_ttl(new_key, exp).await,
      None => self.del_ttl(new_key).await,
    };
    if let Err(e) = ttl_migrated {
      release_claims();
      return Err(e);
    }
    release_claims();
