...
use wcompact::{
  ...
  Error as WcompactError, LogCompactor,
};
use wdev::Device;
use whlog::HybridLog;
  ...
  async fn is_expired_or_orphan_record(
    ...
    match tag {
      KeyTag::Ttl => {
        ...
        // 宿主可能为集合对象信封记录（对象键随键 TTL 同样有效）
        let env_k = NamespaceDbCodec::replace_tag_at(key, tag_offset, KeyTag::ObjectEnvelope);
        if self.index.find_tag(&env_k).is_some() {
          return false;
        }
        let meta_k = NamespaceDbCodec::replace_tag_at(key, tag_offset, KeyTag::Meta);
        let env_k = NamespaceDbCodec::replace_tag_at(key, tag_offset, KeyTag::ObjectEnvelope);
        // 宿主既无 String 也无 Meta 也无对象信封：属于已被删除的主键留下的
        // 孤儿 TTL 记录，直接判死丢弃！
        self.index.find_tag(&meta_k).is_none() && self.index.find_tag(&env_k).is_none()
      }
      KeyTag::String | KeyTag::Meta | KeyTag::ObjectEnvelope => {
