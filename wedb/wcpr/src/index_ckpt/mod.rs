...
use std::{fs::remove_file, path::Path, sync::atomic::AtomicU64};
...
///    试探性标记；指向易失 ReadCache 的条目先经 `rc_resolve` 顺链解析为主日志真实地址，
///    解析触到换页驱逐过渡态由该端口就地等待清洗落定并重读槽位重探，见
///    [`CprStore::skip_read_cache_with_wait`](crate::CprStore::skip_read_cache_with_wait)）。
/// 3. 逐一写入溢出内存池中已分配的所有 64 字节桶数据（空洞以全零桶补齐；扫描期间
///    并发新分配、ID 超出头部采样值 overflow_count 的超界溢出指针截断归零，其模糊区
///    增量由恢复期日志重放重新链入）。
/// 4. 计算全量桶数据 CRC32 校验和并回写至头部，保证数据完整性校验。
/// 5. 刷盘并执行文件重命名，保证快照写入的完全原子性（避免中途断电留下半截文件）。
///
/// `rc_resolve` 对标 C# Tsavorite SkipReadCacheBucket 委托，契约同
/// [`CprStore::skip_read_cache_with_wait`](crate::CprStore::skip_read_cache_with_wait)：
/// 入参为待落盘的数据槽位本体，返回解析后的低 48 位主日志地址（0 = 链尽）；无
/// ReadCache 的宿主传恒等闭包 `&|slot| clean_address(slot.load(Acquire))` 即可
/// （对无 ReadCache 位槽位不产生任何调用开销）。
///
/// 失败清场：任一 I/O 阶段失败（如磁盘满 ENOSPC）时就地回收半截 `.tmp` 残留，
/// 不遗留垃圾文件（正常路径的原子性由「tmp 写 + fsync + rename」保证）。
pub async fn write_index_checkpoint<F: Fn(&AtomicU64) -> u64>(
  index: &HashIndex,
  entry_count: usize,
  ...
  let res = write_index_checkpoint_inner(index, entry_count, dir, token, rc_resolve).await;
  if res.is_err() {
...
async fn write_index_checkpoint_inner<F: Fn(&AtomicU64) -> u64>(
  index: &HashIndex,
  entry_count: usize,
  dir: &Path,
  token: u128,
