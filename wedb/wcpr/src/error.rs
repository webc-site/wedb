...
pub enum Error {
  ...
  /// 转发），确属跨层动态文案，见 wkv/src/store/cpr_host.rs 的 cpr_err 映射
  #[error("宿主存储引擎错误: {0}")]
  Host(String),
  ...
  /// 数据文件缺失或物理长度不足（伪空库防御）
  #[error("数据文件缺失或与检查点不配套 (期望长度 >= {expected} 字节，实际仅有 {actual} 字节)")]
  DeviceTooShort { expected: u64, actual: u64 },

  /// 恢复逻辑地址校验异常
  #[error("恢复地址校验异常: {0}")]
  InvalidRecoveryAddress(String),

  /// 元数据格式版本与当前引擎支持的版本不匹配
  #[error("不支持的 Checkpoint 元数据格式版本: {actual}（当前仅支持版本 {supported}）")]
