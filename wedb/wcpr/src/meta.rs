...
///
/// 在 garnet 中的相对路径:libs/storage/Tsavorite/cs/src/core/Index/CheckpointManagement/DefaultCheckpointNamingScheme.cs:Token
/// （C# 默认命名方案 `prefix_{guid}`；wedb 固定 Base32hex 无连字方案，Guid → u128）
#[inline]
...
///
/// 命名方案契约接口同挂此处（rust 无 trait 间接层，元数据文件名即契约单点）：
/// libs/storage/Tsavorite/cs/src/core/Index/CheckpointManagement/ICheckpointNamingScheme.cs:Token
#[inline]
...
/// 杜绝在途原位写撕裂检查点）；恢复阶段 ReadOnlyAddress 两型一律重建至
/// TailAddress（快照基线全程只读，原位能力由换页驱动随 head 推进自然重建），
/// 类型差异仅保留在元数据签发面（对标 C# CheckpointType 枚举的创建路径分叉：
/// FoldOverSMTask 折叠尾页 / SnapshotCheckpointSMTask 刷独立快照文件——本实现
/// 无独立快照文件，恢复隔离性由「快照基线只读」单语义等价达成）
#[derive(Debug, Clone, Copy, PartialEq, Eq, Encode, Decode)]
pub enum CheckpointType {
  /// 折叠模式（对标 C# Tsavorite CheckpointType.FoldOver）：
  /// 恢复时 ReadOnlyAddress 对齐 TailAddress，全部历史记录封印为只读，
  /// 后续更新全部走 RCU 追加（增量检查点友好，日志增长更快）
  FoldOver,
  /// 快照模式（对标 C# Tsavorite CheckpointType.Snapshot）：
  /// 恢复时 ReadOnlyAddress 同样对齐 TailAddress——C# 以独立快照文件设备
  /// （SnapshotCheckpointSMTask 的 snapshotFileDevice）隔离主日志运行写与
  /// 快照字节，本实现主日志即快照字节唯一载体，隔离性由快照基线只读单语义
  /// 等价达成：运行期原位写绝不触碰 [begin, tail) 检查点区间，二次崩溃恢复
  /// 从干净基线 + AOF 版本过滤重放，非幂等命令恰好一次
  Snapshot,
}
...
pub struct CheckpointMeta {
  ...
  /// 持久化——`Length == 1` 走 legacy 单标量分支，多子日志走
  /// `CurrentSafeAofAddress.Serialize(writer)` 全向量；wedb 统一取向量形态，
  /// 按物理子日志逐位保存覆盖边界，恢复侧逐子日志还原，绝不以子日志 0 标量
  /// 广播全维。None = 未启用 AOF，或快照已提交而 AOF 边界补写
  /// （publish_checkpoint_aof_address）尚未落盘时崩溃——Option 语义保证该
  /// 窗口内单机恢复正确性不受影响。纳入 integrity_crc32 封签覆盖。
  pub checkpoint_aof_address: Option<Vec<u64>>,
  /// 元数据格式版本号
...
impl CheckpointMeta {
  ...
  ///
  /// 在 garnet 中的相对路径:libs/storage/Tsavorite/cs/src/core/Index/CheckpointManagement/RecoveryInfo.cs:Checksum
  /// （C# 逐字段字符串校验和，wedb 为规范化二进制 CRC32 封签）
  ///
  ...
  /// 从 bitcode 二进制字节反序列化
  ///
  /// 在 garnet 中的相对路径:libs/storage/Tsavorite/cs/src/core/Index/CheckpointManagement/RecoveryInfo.cs:Initialize
  /// （C# Initialize(StreamReader) 逐行文本解析 + 校验和复核；wedb 为 bitcode
  /// 一步反序列化，封签复核交由恢复侧 integrity_digest 比对）
  #[inline]
  pub fn decode(bytes: &[u8]) -> Result<Self> {
    bitcode::decode(bytes).map_err(Error::from)
  }

  /// 读取侧统一校验核：decode → token 比对 → 版本门 → 完整性封签复核
  /// （恢复组件加载与 AOF 边界补写两路共用，拒绝面逐项同型同序；
  /// 对标 C# RecoveryInfo.Checksum 强制比对——介质位翻转虽可通过反序列化，
  /// 但无法通过逐字段摘要比对，在此拦截「静默错误恢复」类损坏）
  pub fn verify_sealed(bytes: &[u8], token: u128) -> Result<Self> {
