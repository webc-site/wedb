...
//! 最新检查点损坏时回退更早版本、token 不匹配与非法版本拒绝

use std::{fs, fs::OpenOptions, path::Path, sync::Arc};

use aok::{OK, Void};
use compio::runtime::Runtime;
use tempfile::tempdir;
use wcpr::{
  CheckpointMeta, CheckpointType, CkptGateState, Error, FORMAT_VERSION, HlogMeta, IndexMeta,
...
#[compio::test]
async fn tampered_meta_field_rejected_by_integrity_seal() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn single_bit_flip_rejects_recovery() -> Void {
  let dir = tempdir()?;
  let ckpt_dir = dir.path().join("checkpoints");
...
#[compio::test]
async fn corrupted_latest_falls_back_to_older_checkpoint() -> Void {
  let dir = tempdir()?;
  let ckpt_dir = dir.path().join("checkpoints");
...
/// AOF 边界补写：publish_checkpoint_aof_address 落盘字段、封签自洽、
/// 篡改补写值必被封签拦截、token 不匹配显式报错
/// （对标 GarnetCheckpointManager GetCookie 的 CurrentSafeAofAddress 持久化语义）
#[compio::test]
async fn publish_checkpoint_aof_address_seals_and_verifies() -> Void {
  let dir = tempdir()?;
  let ckpt_dir = dir.path().join("checkpoints");
  let store = MiniStore::open(dir.path().join("publish.db"))?;
  let gate = CkptGateState::default();
  let p = store.session()?;

  store.put(&p, b"pub:key", b"v").await?;
  let meta = wcpr::create_checkpoint(&store, &gate, &ckpt_dir, CheckpointType::FoldOver).await?;
  assert_eq!(
    meta.checkpoint_aof_address, None,
    "创建时 AOF 边界必须为 None"
  );

  // 补写 AOF 边界：字段落盘且封签自洽
  wcpr::publish_checkpoint_aof_address(&ckpt_dir, meta.token, &[0x4000]).await?;
  let sealed = CheckpointMeta::decode(&fs::read(ckpt_dir.join(meta_filename(meta.token)))?)?;
  assert_eq!(sealed.checkpoint_aof_address, Some(vec![0x4000]));
  assert_eq!(sealed.integrity_crc32, sealed.integrity_digest());

  // 恢复路径读到的元数据携带补写边界
  let device = Arc::new(SegmentedDevice::single_file(dir.path().join("publish.db"))?);
  ...
  // 篡改补写值（封签不重算）：必须被封签拦截
  tamper_meta(&ckpt_dir, meta.token, |m| {
    m.checkpoint_aof_address = Some(vec![0x9999]);
  })?;
  let device = Arc::new(SegmentedDevice::single_file(dir.path().join("publish.db"))?);
  let err = wcpr::recover::<_, MiniStore>(&ckpt_dir, meta.token, device)
    .await
    .err()
    .expect("补写字段篡改必须导致恢复失败");
  assert!(
    matches!(err, Error::MetaChecksumMismatch { .. }),
    "AOF 边界篡改必须被封签拦截: {err}"
  );

  // 补写防御：损坏的元数据补写前拒绝（杜绝重算封签洗白损坏字段）
  tamper_meta(&ckpt_dir, meta.token, |m| {
    m.created_at += 1;
  })?;
  let err = wcpr::publish_checkpoint_aof_address(&ckpt_dir, meta.token, &[0x5000])
    .await
    .expect_err("损坏元数据补写必须被封签拦截");
  assert!(
    matches!(err, Error::MetaChecksumMismatch { .. }),
    "损坏元数据补写必须被封签拦截: {err}"
  );
  ...
  let err = wcpr::publish_checkpoint_aof_address(&ckpt_dir, meta.token, &[0x5000])
    .await
    .expect_err("非法版本元数据补写必须被版本门控拦截");
  assert!(
    matches!(err, Error::UnsupportedMetaVersion { .. }),
    "非法版本元数据补写必须被版本门控拦截: {err}"
  );

  // token 不匹配：补写必须显式报错（异名 token 复制 meta 后 token 字段不符）
  let alien_token = meta.token.wrapping_add(1);
  fs::copy(
    ckpt_dir.join(meta_filename(meta.token)),
    ckpt_dir.join(meta_filename(alien_token)),
  )?;
  let err = wcpr::publish_checkpoint_aof_address(&ckpt_dir, alien_token, &[8])
    .await
    .expect_err("异名 token 补写必须失败");
  assert!(
    matches!(err, Error::TokenMismatch { .. }),
    "异名 token 补写必须报 TokenMismatch: {err}"
  );
  OK
}

/// token 不匹配与非法格式版本（超前/滞后/0）均被显式拒绝
#[compio::test]
async fn token_mismatch_and_invalid_version_rejected() -> Void {
  let dir = tempdir()?;
  let ckpt_dir = dir.path().join("checkpoints");
  ...
  // 非法格式版本：高于或低于当前版本的 meta 均被版本门控拒绝（无向后兼容语义，对标 C# RecoveryInfo）
  tamper_meta(&ckpt_dir, token, |m| {
    m.format_version = FORMAT_VERSION + 1;
    // 重封签使封签自洽，确保拒绝来自版本门控而非封签校验
    m.seal();
  })?;
  let err = wcpr::recover::<_, MiniStore>(&ckpt_dir, token, Arc::clone(&device))
    .await
    .err()
    .expect("超前版本恢复必须失败");
  assert!(
  ...
  tamper_meta(&ckpt_dir, token, |m| {
    m.format_version = FORMAT_VERSION.saturating_sub(1);
    m.seal();
  })?;
  let err = wcpr::recover::<_, MiniStore>(&ckpt_dir, token, Arc::clone(&device))
    .await
...
#[compio::test]
async fn device_too_short_rejected() -> Void {
  let dir = tempdir()?;
  let ckpt_dir = dir.path().join("checkpoints");
  ...
  for i in 0..10 {
    store.put(&p, format!("k{i}").as_bytes(), b"v").await?;
  }
  let meta = wcpr::create_checkpoint(&store, &gate, &ckpt_dir, CheckpointType::FoldOver).await?;

  let seg0_path = store.device.segment_path(0);
  drop(p);
  drop(store);

  let flushed = meta.hlog_meta.flushed_until_address;
  assert!(flushed > 64, "flushed_until 需大于初始地址");

  let f = OpenOptions::new().write(true).open(&seg0_path)?;
  f.set_len(flushed - 1)?;
  drop(f);

  let device = Arc::new(SegmentedDevice::single_file(&db_path)?);
  let err = wcpr::recover::<_, MiniStore>(&ckpt_dir, meta.token, device.clone())
    .await
    .err()
    .expect("短设备必须拒启");

  assert!(
    matches!(err, Error::DeviceTooShort { .. }),
    "期望 DeviceTooShort，实际为 {:?}",
    err
  );
  ...
  wcpr::recover::<_, MiniStore>(&ckpt_dir, meta.token, device2).await?;
