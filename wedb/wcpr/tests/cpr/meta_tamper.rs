...
#[test]
fn publish_checkpoint_aof_address_seals_and_verifies() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let ckpt_dir = dir.path().join("checkpoints");
    ...
    assert_eq!(sealed.checkpoint_aof_address, Some(0x4000));
    ...
    tamper_meta(&ckpt_dir, meta.token, |m| {
      m.checkpoint_aof_address = Some(0x9999);
    })?;
    let device = Arc::new(SegmentedDevice::single_file(dir.path().join("publish.db"))?);
    let err = wcpr::recover::<_, MiniStore>(&ckpt_dir, meta.token, device)
      .await
...
#[test]
fn token_mismatch_and_invalid_version_rejected() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let ckpt_dir = dir.path().join("checkpoints");
