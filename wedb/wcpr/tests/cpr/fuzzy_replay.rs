...
use wcpr::{CheckpointMeta, CheckpointType, CkptGateState, meta_filename};
...
#[compio::test]
async fn fuzzy_region_replay_restores_snapshot_missed_keys() -> Void {
  let dir = tempdir()?;
  ...
  assert!(
    addr_miss >= crafted.hlog_meta.begin_address && addr_miss < crafted.index_start_logical_address,
...
#[compio::test]
async fn fuzzy_window_out_of_range_is_rejected() -> Void {
  let dir = tempdir()?;
  let ckpt_dir = dir.path().join("checkpoints");
