...
//!
//! 自研依据: 并发检查点互斥（C# 对应 CheckpointManager 多检查点独占语义）
...
fn writers_and_checkpoints_run_concurrently() -> Void {
  ...
  rt.block_on(async {
    ...
    // 实例闸（宿主所有权形态，对标 C# GarnetDatabase.CheckpointingLock）：
    // 同一 store 上全部检查点入口共享同一闸门状态，Arc 携带跨任务复用
    let gate = Arc::new(CkptGateState::default());
    ...
    let first = wcpr::create_checkpoint(&store, &gate, &ckpt_dir, CheckpointType::FoldOver).await?;
