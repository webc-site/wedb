...
//!
//! 自研依据: RCU 标签一致性（C# 对应 MiscTests.cs:ForceRCUAndRecover 恢复面）
...
use super::support::{MiniStore, RC_VIRTUAL_BASE, pick_nonzero_tag_key};

/// 本用例键名前缀（与驱逐过渡态用例的键空间互不相交）
const TAG_MARK: &str = "rc:tag:";
...
fn install_broken_rc_entry(store: &MiniStore, p: &Participant, key: &[u8]) -> Void {
  ...
  let bucket = store.index.bucket(store.index.bucket_index_for_key(key));
...
#[compio::test]
async fn rc_entry_tag_survives_checkpoint_roundtrip() -> Void {
  let dir = tempdir()?;
  ...
  let key = pick_nonzero_tag_key(TAG_MARK);
...
/// 其余键不受影响
#[compio::test]
async fn broken_rc_chain_slot_sanitized_on_roundtrip() -> Void {
  let dir = tempdir()?;
  let ckpt_dir = dir.path().join("checkpoints");
