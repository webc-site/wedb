...
#[test]
fn commit_flush_failure_surfaces_device_error_and_recovers() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    ...
    let addr_a = hlog.append(b"a", b"value-a", 0, false)?;
    ...
    let addr_b = hlog.append(b"b", b"value-b", addr_a, false)?;
...
#[test]
fn recovery_with_failing_device_fails_fast_and_stays_recoverable() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    ...
    // 阶段 1：健康设备写入并全量落盘
    {
      ...
      addr_a = hlog.append(b"k0", b"payload-0", 0, false)?;
      let _ = hlog.append(b"k1", b"payload-1", addr_a, false)?;
      hlog.flush_all().await?;
      hlog.sync().await?;
...
#[test]
fn recovery_on_mismatched_sector_size_fails() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    ...
    // 按旧扇区几何写入并全量落盘
    {
      ...
      let epoch = Arc::new(LightEpoch::new(16));
      let hlog = HybridLog::new(config.clone(), device, epoch)?;
      addr = hlog.append(b"k", b"sector-data", 0, false)?;
      hlog.flush_all().await?;
      hlog.sync().await?;
      tail = hlog.tail_address();
    }
