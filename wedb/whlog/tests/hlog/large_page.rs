...
#[test]
fn large_page_inline_one_mib_value_roundtrip() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    ...
    let addr_big = hlog.append(b"big:1mib", &big, 0, false)?;
    ...
    let addr_small = hlog.append(b"s", b"tiny", addr_big, false)?;
...
/// 大值负载的等价收窄）
#[test]
fn large_page_value_survives_recovery() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    ...
    // 阶段 1：写入 3MB 大值并全量落盘
    {
      ...
      addr_big = hlog.append(b"big:3mib", &big, 0, false)?;
    ...
    // 阶段 2：从快照恢复，大值逐字节一致
    {
      ...
      // 恢复后无缝续写新记录
      let next = hlog.append(b"after", b"resume", addr_big, false)?;
...
#[test]
fn large_page_rejects_over_page_record_but_admits_near_full_page() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let device = Arc::new(SegmentedDevice::single_file(
    ...
    let addr_max = hlog.append(b"max:page", &max_val, 0, false)?;
    assert_eq!(hlog.config.page_id(addr_max), 0);

    // 下一记录触发跨页 Pad + 新页写入（大页位面算术 page_bits = 24 下语义不变）
    let addr_next = hlog.append(b"n", b"next", addr_max, false)?;
