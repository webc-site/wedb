...
use whlog::{
  DEFAULT_INITIAL_ADDRESS, Error, HybridLog, HybridLogConfig, RevivifyArgs, SECTOR_ALIGNMENT,
};
...
fn test_inplace_lifecycle() -> Void {
  ...
  rt.block_on(async {
    ...
    let key = b"life";
    let (addr, _) = hlog.append(key, b"1234567890", 0, false)?;
    assert_eq!(addr, DEFAULT_INITIAL_ADDRESS);

    // 槽位扩至 40 字节：记录对齐逻辑尺寸 32（16+4+10），富余 8 字节（一个整词）
    // 不足以容纳 Pad 头 → 吸纳为松弛填充（无切出块，返回 None）
    assert_eq!(
      hlog.revivify_record_at(&RevivifyArgs {
        addr,
        slot_size: 40,
        key,
        val: b"1234567890",
    ...
    // 不足一个 Pad 头 → 吸纳为松弛填充，保留原位复活容量；无切出块）
    assert_eq!(
      hlog.revivify_record_at(&RevivifyArgs {
        addr,
        slot_size: 32,
...
/// 测试 19: 复活槽位松弛吸纳（富余 ≤ MAX_FILLER_BYTES）与分裂切出块回传（富余超限）
#[test]
fn test_revivify_record_at_with_pad() -> Void {
  ...
  rt.block_on(async {
    ...
    // 甲臂：R1 为 16+3+21=40 字节槽位；复活写入新记录 16+3+5=24，富余 16 字节
    // 在填充表示上限（2040）内 → 全数吸纳为记录内松弛填充、无切出块
    // （对标 C# RecordDataHeader.SetFiller / InitializeForRevivification 松弛留存语义）
    let val21 = vec![b'x'; 21];
    let (addr1, _) = hlog.append(b"old", &val21, 0, false)?;
    assert_eq!(
      hlog.revivify_record_at(&RevivifyArgs {
        addr: addr1,
        slot_size: 40,
      ...
      None,
      "富余在表示上限内不得报出切出块（旧 Pad 切分形态已废，杜绝残片脱钩）"
    );
    let out = hlog.read_record(addr1).await?;
    assert_eq!(out.key()?, b"new");
    assert_eq!(out.value()?, b"value");
    assert_eq!(out.header()?.filler_bytes(), 16, "富余全数转为松弛填充");
    assert_eq!(out.header()?.physical_size(), 40, "物理尺寸精确覆盖整个槽位");

    // 乙臂：R3 为 4096 字节整槽（16+1+4077=4094 对齐恰 4096）；复活写入 24 字节记录后
    // 富余 4072 > 2040 → 按 C# ComputeFillerWordsOrSplit → SplitOverflowingFiller
    // （RecordDataHeader.cs:462/:509）分裂：记录内保留 64 词 = 512 字节填充，
    // 切出 3560 字节剩余块回传调用层归池（对标 TryTransferToFreeList）
    let (addr2, _) = hlog.append(b"b", b"vvv", 0, false)?;
    let big_val = vec![b'G'; 4077];
    let (addr3, _) = hlog.append(b"g", &big_val, 0, false)?;
    // 追加探针记录紧随 4096 槽位之后，验证扫描精确越过保留段与切出块
    let (addr4, _) = hlog.append(b"after", b"!", 0, false)?;
    assert_eq!(
      hlog.revivify_record_at(&RevivifyArgs {
        addr: addr3,
        slot_size: 4096,
      ...
      Some((addr3 + 536, 3560)),
      "富余超上限须分裂并按 C# SplitOverflowingFiller 坐标报出切出块（24 记录 + 512 保留 = 536 起，余 3560）"
    );
    let out = hlog.read_record(addr3).await?;
    assert_eq!(
      out.header()?.filler_bytes(),
      512,
      "分裂后记录内保留 64 词填充（对位 RecordSplitRetainFillerWords）"
    );
    assert_eq!(out.header()?.physical_size(), 536, "记录物理 = 逻辑 24 + 保留 512");

    // 扫描必须按整词物理尺寸精确越过保留段与切出块（不跳页），完整读出全部记录
    ...
    assert_eq!(
      ...
      vec![
        ...
        (addr2, b"b".to_vec()),
        (addr3, b"new".to_vec()),
        (addr4, b"after".to_vec())
      ],
      "切出块与松弛填充必须按物理尺寸精确越过，不得吞并同页后续记录"
    );
    ...
    // 尾部追加位置不受复活复用影响（复用旧槽位不推进 tail；探针记录对齐逻辑尺寸 24）
    assert_eq!(hlog.tail_address(), addr4 + 24);

    info!("复活槽位松弛吸纳与分裂切出块回传测试通过");
    aok::Result::<()>::Ok(())
  })?;
...
fn test_shift_begin_address_and_truncate() -> Void {
  ...
  rt.block_on(async {
    ...
    let (addr1, _) = hlog.append(b"k1", &big, 0, false)?;
    let (addr2, _) = hlog.append(b"k2", &big, 0, false)?;
    let (addr3, _) = hlog.append(b"k3", &big, 0, false)?;
    ...
    hlog.raise_delete_floor(new_begin);
...
fn test_grow_record_in_place() -> Void {
  ...
  rt.block_on(async {
    ...
    let (addr1, _) = hlog.append(key, &[b'x'; 20], 0, false)?;
    let (addr2, _) = hlog.append(b"nb", b"neighbor", 0, false)?;
...
fn test_grow_record_in_place_zero_len_and_capacity_boundaries() -> Void {
  ...
  rt.block_on(async {
    ...
    let key = b"zero";
    let (addr, _) = hlog.append(key, &[b't'; 8], 0, false)?;
    let (addr_next, _) = hlog.append(b"nb0", b"after-zero", 0, false)?;
    ...
    //    使贴边记录恰落页首、槽位尾紧顶页界
    let (pad_addr, _) = hlog.append(b"p", &vec![b'P'; PAGE - HEADER_SIZE - 1], 0, false)?;
    assert_eq!(pad_addr % PAGE as u64, 0, "整页记录必被推换新页落页首");
    let edge_key = b"edge";
    let edge_val_len = PAGE - HEADER_SIZE - edge_key.len();
    let (edge_addr, _) = hlog.append(edge_key, &vec![b'Z'; edge_val_len], 0, false)?;
    assert_eq!(edge_addr, pad_addr + PAGE as u64);
    // 下一条记录必然再换页落在新页首，成为「越界写」探针
    let (after_page_addr, _) = hlog.append(b"nb1", b"after-page", 0, false)?;
    assert_eq!(
      after_page_addr,
...
///
/// 验证原位增长时的最小写证据：原位改长且只拷新字节、旧数据
/// 一律不动，并与旧实现形态（整值另落一条新记录）进行对照。
///
...
fn test_grow_record_in_place_frame_byte_diff() -> Void {
  ...
  rt.block_on(async {
    ...
    let (addr, _) = hlog.append(key, &[b'x'; 20], 0, false)?;
    let (nb_addr, _) = hlog.append(b"nb4", b"neighbor4", 0, false)?;
    hlog.revivify_record_at(&RevivifyArgs {
      addr,
      slot_size: 40,
      key,
    ...
    let old_key = b"olde";
    let (old_addr, _) = hlog.append(old_key, &[b'y'; 20], 0, false)?;
    hlog.revivify_record_at(&RevivifyArgs {
      addr: old_addr,
      slot_size: 40,
      key: old_key,
