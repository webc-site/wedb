//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.hlog/LogTests.cs + LogScanTests.cs（追加与扫描）
use core::{
  hint::spin_loop,
...
fn test_append_and_memory_read() -> Void {
  ...
  rt.block_on(async {
    ...
    let (addr2, _) = hlog.append(k2, v2, addr1, false)?;
    assert!(addr2 > addr1);
    ...
    assert!(hlog.tail_address() > addr2);
...
fn test_page_turn_and_padding() -> Void {
  ...
  rt.block_on(async {
    ...
    let (overflow_addr, _) = hlog.append(overflow_key, &overflow_val, 0, false)?;
...
fn test_scan_multipage_and_pull_iterator() -> Void {
  ...
  rt.block_on(async {
    ...
    for i in 0..30 {
      ...
      let (addr, _) = hlog.append(key.as_bytes(), &val, 0, false)?;
...
fn test_scan_hybrid_disk_and_memory() -> Void {
  ...
  rt.block_on(async {
    ...
    hlog.append(fill_key, &fill_val, 0, false)?;
...
fn test_subheader_fragment_pad() -> Void {
  ...
  rt.block_on(async {
    ...
    let v = vec![7u8; 4000];
    let (addr1, _) = hlog.append(b"0123456789", &v, 0, false)?;
    assert_eq!(addr1, DEFAULT_INITIAL_ADDRESS);

    let (addr2, _) = hlog.append(b"frag", b"tail", 0, false)?;
...
/// 在途预留槽位物理形态相同的零洞；生产者线程经两级原子标志握手（ready 就位 +
/// go 放行，spin_loop 忙等绝不睡眠——mpsc 阻塞唤醒的微秒到毫秒级延迟会与扫描器
/// 自旋预算赛跑），在扫描闭包读到前驱记录 k0 后以 append encode_at 同款无锁裸
/// 指针路径补写该记录。零洞被触达的时序无论落在哪一侧均有确定收敛路径：
/// 扫描器先到则走 ZERO_HEADER_SPIN_BUDGET 有界自旋重试；生产者编码恰落在
/// parse 判零与复读之间（撕裂窗口）则走扫描器 pad_seen 单址复核重试——
/// 两条路径都要求原址读出中间记录，且同页后续记录不漏。
#[test]
fn test_scan_inflight_zero_header_respin() -> Void {
  ...
  rt.block_on(async {
    ...
    let (addr2, _) = hlog.append(b"k2", b"v2", 0, false)?;
    ...
    // 两级原子标志握手：ready 就位（生产者已进入 spin_loop 观察循环，剩余路径
    // 仅 load go → 写入 → 结束）→ go 放行（扫描闭包读到零洞前驱 k0 时置位）。
    // 生产者忙等不睡眠，放行后补写延迟压缩在纳秒级，确保「扫描器先到走自旋
    // 重试」与「撕裂走 pad_seen 复核」两条路径都在确定预算内收敛
    let (ready_tx, ready_rx) = mpsc::channel::<()>();
    let encode_flag = Arc::new(AtomicBool::new(false));
    let producer = {
      let hlog = Arc::clone(&hlog);
      let encode_flag = Arc::clone(&encode_flag);
      thread::spawn(move || {
        let _ = ready_tx.send(());
        while !encode_flag.load(Ordering::Acquire) {
          spin_loop();
        }
        let slot = hlog.buffer.page_idx(page1);
        // SAFETY: 测试单扫描线程与生产者线程互斥于补写窗口，写入区间
        // [off1, off1 + rec1_len) 为已预留槽位，无其他并发访问者；
        // encode_to_slice 以 key → val → Release fence → 头最后发布写序落笔，
        // 扫描器 parse 的原子字读见新头即键值数据必已可见
        let ptr = unsafe { hlog.buffer.raw_page_ptr_mut(slot) };
        let dst = unsafe { slice::from_raw_parts_mut(ptr.add(off1), rec1_len) };
        let _ = encode_to_slice(dst, 0, b"k1", b"v1", false, false);
      })
    };
    let _ = ready_rx.recv();
    ...
    hlog
      .scan(0, hlog.tail_address(), |addr, rec| {
        scanned.push((addr, rec.key().to_vec()));
        if addr == addr0 {
          encode_flag.store(true, Ordering::Release);
        }
        Ok(true)
      })
      .await?;
    ...
    assert_eq!(
      ...
      "在途零头自旋重试/pad_seen 复核后必须原址读出 k1，且同页后续记录不得漏扫"
    );
...
fn test_append_address_overflow_defense() -> Void {
  ...
  let config = HybridLogConfig::new(64 * 1024, 16, 0.5)?;
  let hlog = HybridLog::new(config, device, epoch)?;
  let tail = ADDRESS_MASK - 10;
  let curr_page = tail >> hlog.config.page_bits();
  // 经生产写页号链路（append 换页时的 seal_page 清零+标定）标定当前页槽位
  hlog.buffer.seal_page(curr_page);
...
/// SCAN 游标有效性校验（对标 C# SpanByteScanIterator.SnapCursorToLogicalAddress
/// 的「游标须落在记录起始字节」判据）：记录起始地址通过，记录中间地址（含
/// 8 字节对齐的中间地址）与越尾地址一律拒绝
#[test]
fn test_validate_cursor() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let dir = tempdir()?;
    let db_path = dir.path().join("hlog_validate_cursor.db");
    let device = Arc::new(SegmentedDevice::single_file(&db_path)?);
    let epoch = Arc::new(LightEpoch::new(16));
    ...
    // 首记录 16 头 + 4 字节键值对齐至 24 字节，a1+8 为对齐的记录中间地址
    let (a1, _) = hlog.append(b"k1", b"v1", 0, false)?;
    let (a2, _) = hlog.append(b"key2", b"payload2", a1, false)?;
    let (a3, _) = hlog.append(b"k3", b"v3", a2, false)?;

    // 记录起始字节全部有效
    for addr in [a1, a2, a3] {
      assert!(hlog.validate_cursor(addr).await?);
    }
    // 记录中间地址（不对齐 / 对齐）均无效
    assert!(!hlog.validate_cursor(a1 + 1).await?);
    assert!(!hlog.validate_cursor(a1 + 8).await?);
    assert!(!hlog.validate_cursor(a2 + 1).await?);
    // 越尾地址无效
    assert!(!hlog.validate_cursor(hlog.tail_address()).await?);
    assert!(!hlog.validate_cursor(hlog.tail_address() + 4096).await?);
