...
#[compio::test]
async fn scan_surfaces_error_and_resumes_when_device_read_fails() -> Void {
  let dir = tempdir()?;
  ...
  assert!(
    matches!(err, Error::Device(WdevError::Io(_))),
    "页读失败必须显式传播: {err:?}"
  );
...
/// 显式暴露 + 零静默丢失）
#[compio::test]
async fn scan_delivers_strictly_ordered_records_when_page_read_fails() -> Void {
