...
#[test]
fn test_flush_records_in_range_visits_all_appended() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
...
#[test]
fn test_flush_records_in_range_mutates_in_place() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (_dir, hlog) = build_hlog()?;
    let addr = hlog.append(b"stub", b"dirty!", 0, false)?;
...
#[test]
fn test_flush_records_in_range_early_stop_and_skip_unloaded() -> Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (_dir, hlog) = build_hlog()?;
    let val = vec![b'x'; 400];
...
/// 只读走查内核判据（对标 ReadCacheEvict 面）：零头与 pad 终止页链；
/// sealed 记录照常步进不截断（跳过与否归调用方闭包裁决）；空键记录可步进
#[test]
fn test_for_each_record_in_page_predicate() -> Void {
  ...
  assert_eq!(from_pad, 0, "pad 头必须终止页内走查");
  OK
}
