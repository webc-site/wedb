//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.hlog/LogRecoverReadOnlyTests.cs + LogTests.cs（恢复不变量）
use std::sync::Arc;
...
use whlog::{
  AddressSnapshot, DEFAULT_INITIAL_ADDRESS, Error, HybridLog, HybridLogConfig, RecordOutput,
  SECTOR_ALIGNMENT,
};
...
fn test_recovery_preload_and_resume_append() -> Void {
  ...
  rt.block_on(async {
    ...
    // 阶段 1: 写入数据并落盘
    {
      ...
      addr0 = hlog.append(b"k0", b"v0_before_crash", 0, false)?.0;
      // 填满第 0 页跨入第 1 页
      let pad_val = vec![b'P'; 4000];
      hlog.append(b"pad", &pad_val, 0, false)?;

      addr1 = hlog.append(b"k1", b"v1_page1", addr0, false)?.0;
    ...
    // 阶段 2: 恢复 HybridLog 并校验预热数据与继续追加
    {
      ...
      let (addr2, _) = hlog.append(b"k2", b"v2_after_recovery", addr1, false)?;
...
fn test_recover_scrubs_non_durable_prefix() -> Void {
  ...
  rt.block_on(async {
    ...
    let (addr1, _) = hlog.append(b"durable", &big, 0, false)?;
    let (addr2, _) = hlog.append(b"volatile", &big, 0, false)?; // 第 1 页
    hlog.flush_all().await?;
    hlog.sync().await?;
    ...
    // 从 tail 无缝续写新记录并回读
    let (addr3, _) = recovered.append(b"resumed", b"v3", addr2, false)?;
...
/// 测试 25: 磁盘冷读大记录——探针不足时按记录物理尺寸精确二次读并裁剪输出缓冲
#[test]
fn test_cold_read_large_record_exact_trim() -> Void {
  ...
  rt.block_on(async {
    ...
    let (addr_big, _) = hlog.append(big_key, &big_val, 0, false)?;

    // 同页小记录：与大记录同居一页
    let small_key = b"s:1";
    let small_val = b"tiny";
    let (addr_small, _) = hlog.append(small_key, small_val, addr_big, false)?;
    ...
    // 大记录冷读：探针段解析出头后按物理尺寸精确二次读，输出缓冲按物理尺寸精确裁剪，内容逐字节一致
  ...
  rt.block_on(async {
    ...
    let tail_before;
    {
      ...
      for i in 0..5u8 {
        ...
        let (addr, _) = hlog.append(&key, &big, 0, false)?;
    ...
    // 从 tail 无缝续写
    let (next, _) = hlog.append(b"m5", &[b'N'; 100], 0, false)?;
