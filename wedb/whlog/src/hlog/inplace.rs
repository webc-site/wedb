...
use wdev::Device;
use wrecord::{HEADER_SIZE, RDH_WORD_OFFSET, RecordHeader, RecordMut};
...
impl<D: Device> HybridLog<D> {
  ...
  /// 尝试在可变区原位增长并读-改-写记录的值（严格对标
  /// libs/server/Storage/Functions/MainStore/RMWMethods.cs:InPlaceUpdater 的
  /// APPEND 分支 :799-834 与 SETRANGE 分支 :734-763）
  ///
  ...
  pub fn try_seal_record(&self, addr: u64, invalidate: bool) -> bool {
    ...
    if let Ok(mut rec_mut) = RecordMut::from_slice_mut(&mut guard[offset..]) {
  ...
  pub fn try_revivify_in_chain(
    ...
    self.with_mutable_record(addr, expected_key, false, |rec_mut| {
      ...
      if !rec_mut.is_tombstone() || rec_mut.slack_for_val_len(new_val.len()).is_none() {
        return Ok(false);
      }
