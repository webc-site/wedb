...
/// 三级退避一步：转调 wbase::backoff 阶段机的异步动作真源并步进轮数
///
/// # 与 C# 的等价性（刻意差异）
...
/// 等待语义等价（周期性复查谓词），阻塞性相反（绝不挂起线程）。阶段阈值与阶段动作
/// 均为 wbase::backoff 单一真源（32 自旋 / 1024 让核 / 50μs 睡眠，见
/// `BackoffStage::wait_async`），与 wepoch drain 等待（epoch.rs 的 Backoff::snooze
/// 循环）同构，杜绝 whlog 内私有阈值/动作分叉。
#[inline]
async fn backoff(bo: &mut Backoff) {
  bo.stage().wait_async(sleep).await;
  bo.advance();
}
...
impl<D: Device> HybridLog<D> {
  ...
  /// [Self::wait_epoch_condition] 单一纪元屏障内核，杜绝第二套等待循环。
  ...
  pub async fn shift_begin_address(&self, new_begin: u64) -> Result<()> {
    ...
    // ShiftBeginAddress 的「先推进 begin → 纪元排空 → TruncateUntilAddress」次序）：
    // begin 仅推进自身，head/safe_head 的连带推进已由 shift_head_address 以
    // 纪元安全方式完成（"连带内联强推 safe_head"会绕过纪元排空语义、令旧页上
    // epoch 保护的读者失去保护，故不采用）。纪元排空屏障作为参数注入内核，
    // 严格落于 begin 推进之后、设备物理删段之前——begin 挪线后新读者经
    // is_on_disk 永远采不到待截断区间，而挪线前已入场的在途磁盘读者由屏障
    // 内的封口 action 钉在其入场纪元上，截断因此严格 happens-after 其 I/O 完成。
    self
      .device
      .truncate_begin_until(
        &self.addresses.begin_address,
        new_begin,
        self.wait_safe_read_only_drained(new_begin),
      )
      .await
      .map_err(Error::from)
  }
  ...
  fn unpin_self(&self) -> usize {
    ...
    self.epoch.refresh_thread_protected_entries(tid, current);
    exited
  }
