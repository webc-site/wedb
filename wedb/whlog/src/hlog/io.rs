...
impl<D: Device> HybridLog<D> {
  ...
  pub async fn read_disk_record(&self, addr: u64) -> Result<RecordOutput> {
    ...
    let slot = (page_id as usize) & DISK_READ_CACHE_MASK;
    {
        ...
        && *p == page_id
      {
        ...
        // 单点解析：磁盘缓存页字节已定稿，与内存路径共用 parse_record_from_slice
        //（原子载入与普通载入等价，论证见该函数文档），只取物理尺寸
        let physical_size =
          parse_record_from_slice(page, offset_in_page, addr, page_size)?.physical_size();
    ...
    // 单点解析同上：短读页按 min(len, page_size) 保守校验
    let physical_size =
      parse_record_from_slice(&page, offset_in_page, addr, page_size)?.physical_size();
  ...
  pub async fn flush_pages_range(&self, start_page: u64, end_page: u64) -> Result<()> {
    ...
    'copy: for (i, p) in (actual_start_page..=actual_end_page).enumerate() {
      ...
      // 新页号），随后锁内读到的将是新页字节，staging 按旧页落盘即污染设备已
      // 持久化前缀。页清空/标定均持写锁，读锁持有期间页号与数据稳定，锁内复验
