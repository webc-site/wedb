...
impl<D: Device> HybridLog<D> {
  ...
  pub async fn flush_pages_range(&self, start_page: u64, end_page: u64) -> Result<()> {
    ...
    if until_addr <= flushed_until {
      return Ok(());
    }
    let flushed_page = self.config.page_id(flushed_until);
    let start_page = start_page.max(flushed_page);
    if start_page > end_page {
      return Ok(());
    }
    self
  ...
  async fn flush_sealed_page_range(
    ...
    // 编码全部定稿（契约见本方法文档「崩溃一致性契约」）。coalesce 吸收的陈旧区间
    ...
    if let Err(err) = fill_res {
      ...
      if current_flushed >= merged_range.until_address {
        return Ok(());
      }
  ...
  fn clamp_flush_range(&self, range: PageFlushRange) -> PageFlushRange {
    ...
    let floor = self
      .config
      .page_start_address(self.config.page_id(flushed_until));
