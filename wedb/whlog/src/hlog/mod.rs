...
use wbase::{AlignedBuf, addr::ADDRESS_MASK};
use wdev::Device;
use wepoch::LightEpoch;
use wrecord::{HEADER_SIZE, RecordHeader, RecordRef, encode_to_slice, record_size};
...
pub struct HybridLog<D: Device> {
  ...
  disk_read_cache: Mutex<[DiskPageSlot; DISK_READ_CACHE_SLOTS]>,
...
impl<D: Device> HybridLog<D> {
  ...
  fn assemble(
    ...
    Self {
      ...
      disk_read_cache: Mutex::new([const { None }; DISK_READ_CACHE_SLOTS]),
      last_probe_page: AtomicU64::new(0),
      pending_flush: PendingFlushList::new(),
      flush_event: event_listener::Event::new(),
    }
  }
  ...
  ///      连带强推 / checkpoint 的 `shift_read_only_to_tail`）不取页锁——复验通过到 RDH
