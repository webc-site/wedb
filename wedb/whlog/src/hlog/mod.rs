...
use wrecord::{
  HEADER_SIZE, RecordHeader, RecordRef, encode_to_slice, publish_extent_header, record_size,
};
...
/// 在途编码窗口挂起门（仅测试用注入面，一处定义，同 wbftree `SCAN_FAIL_INJECT` 纪律）
///
/// 布防后，本实例下一次 `encode_at` 在**在途 extent 头发布之后、键值落笔
/// 之前**挂起，直至测试 [`Self::release`] 放行——确定性复现「生产者已 CAS 独占槽位、
/// 整段编码期被长时间抢占」的在途窗口。该窗口在真实并发下窄于扫描器自旋预算
/// （`ZERO_HEADER_SPIN_BUDGET`），任何睡眠/忙等猜测都无法稳定命中，故须显式挂起点。
///
/// 生产路径开销：每次编码一条 `Relaxed` 载入恒假，无跨核争用、无锁、无分配；
/// 门随实例存活、逐实例隔离（并发测试互不劫持），布防仅由集成测试触发。
#[cfg(debug_assertions)]
pub struct EncodeStall {
  /// 布防标记（生产者挂起前消费即清，一次性）
  armed: AtomicBool,
  /// 已挂起标记（测试据此确认在途窗口就位）
  parked: AtomicBool,
...
#[cfg(debug_assertions)]
impl EncodeStall {
  /// 布防：使本实例下一次 `encode_at` 挂在在途窗口（幂等）
  pub fn arm(&self) {
  ...
  /// [`HybridLog::encode_at`] 首拍后的挂入点：未布防直返，布防则睡眠至放行
...
impl<D: Device> HybridLog<D> {
  ...
  pub(crate) fn validate_append_args(&self, p: &RecParams<'_>) -> Result<usize> {
    ...
    let rec_size = record_size(p.key.len(), p.val.len());
  ...
  ///
  /// 落笔次序：先以 [wrecord::publish_extent_header] 单条对齐 store 发布只含槽位尺寸的
  /// Pad 形态最小头，再编码键值与完整头（RecordInfo 字 → Release 屏障 → RDH 字）。
  /// 「已 CAS 预占、尚未编码」的在途窗口对扫描器因此恒为可按 `physical_size` 精确步度
