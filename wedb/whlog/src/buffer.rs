...
// SAFETY: raw_pages 包含的裸指针指向 pages 中长期持有的 AlignedBuf 内存，
// 其并发读写受上层页状态机（page_ids 槽位纪元及页级锁/不可变日志区间约束）保护，
// 只要满足 HybridLog 内部同步协议，跨线程共享与传递是安全的。
...
impl CircularPageBuffer {
  ...
  /// 根据逻辑地址计算所在逻辑页号（页号换算单点，与 [`crate::HybridLogConfig::page_id`] 同口径，
  /// 供分配器与读缓存共用，杜绝调用侧各自抄一份右移算式）
  #[inline]
  pub const fn page_of_address(&self, addr: u64) -> u64 {
    addr >> self.page_bits
  }

  /// 根据逻辑页号计算该页起始逻辑地址（[`Self::page_of_address`] 的反向换算，与
  /// [`crate::HybridLogConfig::page_start_address`] 同口径）
  #[inline]
  ...
  pub const fn slot_for_address(&self, addr: u64) -> usize {
    self.page_idx(self.page_of_address(addr))
  }
  ...
  /// 尝试无锁直读指定逻辑页的切片数据（严格对标 libs/storage/Tsavorite/cs/src/core/Allocator/AllocatorBase.cs 纯指针无锁直读）
  ///
  ...
  /// 获取槽位物理页的可变首地址裸指针（严格对标 libs/storage/Tsavorite/cs/src/core/Allocator/AllocatorBase.cs 写入物理切片）
  ///
  /// # Safety
