...
use wbase::{
  AlignedBuf,
  backoff::{Backoff, BackoffStage, YIELD_LIMIT},
};
use wdev::Device;
...
/// 供 [AddressManager::is_mutable_snapshot] 贯穿使用，消除逐记录的重复 Acquire 加载。
  ...
  pub async fn next_ref<R>(
    ...
    while self.curr_addr < effective_end {
      ...
      let bytes = if curr_addr < head {
        ...
        if !AddressManager::is_mutable_snapshot(curr_addr, head, self.safe_read_only, tail)
        ...
        Err(Error::PadRecord(_)) => {
          ...
          if !matches!(bytes, PageBytes::Disk(_)) && is_zero_header(&bytes, offset) {
            ...
            while backoff.step_count() < ZERO_HEADER_SPIN_BUDGET && is_zero_header(&bytes, offset) {
              // 预算上限即睡眠阶段边界：Spin/Yield 两阶段按 wbase 阶段机推进，
              // Sleep 分支被预算严格排除（兜底防御性让核，见预算常量文档）
              match backoff.stage() {
