use itoa::Buffer;
use wbase::{align::DEFAULT_SECTOR_SIZE, cfg::DEFAULT_HLOG_PAGE_SIZE};
use zmij::Buffer as ZmijBuffer;

use crate::error::{Error, Result};

/// 默认单页容量（64KB，微型嵌入式/测试基线；whlog 为常驻整页分配模型，
/// 页容量直接乘进环形缓冲内存，故库级默认保守取小页）
pub const DEFAULT_PAGE_SIZE: usize = 64 * 1024;

/// 生产服务器默认单页容量（16MB，对标 libs/server/Servers/ServerOptions.cs:46
/// PageSize = "16m"）：单页可内联承载 C# DefaultMaxInlineValueSize = 1MB 的
/// 大值记录（libs/storage/Tsavorite/cs/src/core/Index/Common/KVSettings.cs:159）
pub const DEFAULT_SERVER_PAGE_SIZE: usize = DEFAULT_HLOG_PAGE_SIZE;
...
/// 扇区基准对齐大小
pub const SECTOR_ALIGNMENT: usize = DEFAULT_SECTOR_SIZE;
...
impl HybridLogConfig {
  ...
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Index/Common/LogAddress.cs:GetPageOfAddress
  /// （C# 共享静态工具，本函数即其唯一 rust 位点，六个 C# 声明位在此并挂——
  /// libs/storage/Tsavorite/cs/src/core/Allocator/ObjectAllocator.cs:GetPageOfAddress 与
  /// libs/storage/Tsavorite/cs/src/core/Allocator/SpanByteAllocator.cs:GetPageOfAddress 为其
  /// 一行委托，libs/storage/Tsavorite/cs/src/core/Allocator/IAllocator.cs:GetPageOfAddress 为
  /// 接口声明，libs/storage/Tsavorite/cs/src/core/Allocator/TsavoriteLogAllocator.cs:GetPageOfAddress
  /// 与测试侧假分配器
  /// libs/storage/Tsavorite/cs/test/test.hlog/ScanIteratorEpochFailureTests.cs:GetPageOfAddress
  /// 为同式内联右移而非委托。C# 静态工具首句先经
  /// `AbsoluteAddress` 剥地址域类型位，rust 日志位点为纯 u64（读缓存独立成件
  /// `wkv/src/read_cache`，位域剥除口径在 `wbase::addr`），直接右移即同一语义）
  ...
  /// 根据逻辑页号计算该页起始逻辑地址
  /// libs/storage/Tsavorite/cs/src/core/Allocator/AllocatorBase.cs:GetLogicalAddressOfStartOfPage
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Index/Common/LogAddress.cs:GetLogicalAddressOfStartOfPage
  /// （C# 两处同源一行算术：静态工具收 bits 形参、AllocatorBase 实例包装 baked 本日志
  /// `LogPageSizeBits`；rust 位数由本 config 承载，故两处锚点同挂此单点）
  #[inline]
  ...
  pub const fn calculate_read_only_address(&self, head: u64, tail: u64) -> u64 {
    ...
    if aligned_ro <= head {
      ...
      let is_at_page_records_start = if head_page == 0 {
        (head & self.page_mask()) <= DEFAULT_INITIAL_ADDRESS
      } else {
        (head & self.page_mask()) == 0
      };
      if is_at_page_records_start || head_page == tail_page {
        head
      } else {
