...
use crate::{aof::DatabaseAof, cache_size_tracker::CacheSizeTracker};

/// WATCH 版本表默认桶数（libs/server/GarnetDatabase.cs:DefaultVersionMapSize）
pub const DEFAULT_VERSION_MAP_SIZE: u64 = 1 << 16;

/// 逻辑数据库
pub struct GarnetDatabase<D: Device, A: DatabaseAof<D> = ()> {
  /// 数据库编号
  pub id: i64,
  /// 存储引擎句柄（多库共享，前缀隔离）
...
impl<D: Device, A: DatabaseAof<D>> GarnetDatabase<D, A> {
  /// 创建逻辑数据库
  pub fn new(
    id: i64,
  ...
  /// 基于具体 AOF 构建逻辑数据库（打通数据库与 AOF 无缝链接）
