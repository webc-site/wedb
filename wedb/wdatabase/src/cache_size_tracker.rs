...
mod tests {
  ...
  #[test]
  fn test_cache_size_tracker_lifecycle() {
    let tracker = CacheSizeTracker::new();
    assert_eq!(tracker.heap_bytes(), 0);
    assert!(!tracker.is_stopped());

    tracker.add_heap_size(1024);
    assert_eq!(tracker.heap_bytes(), 1024);
    tracker.add_heap_size(-256);
    assert_eq!(tracker.heap_bytes(), 768);

    tracker.add_read_cache_heap_size(512);
