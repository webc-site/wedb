...
impl<D: Device> DatabaseManagerBase<D> {
  ...
  pub async fn take_checkpoint_helper_async<A: DatabaseAof<D>>(
    &self,
    db: &GarnetDatabase<D, A>,
    entry_ms: u64,
  ) -> wkv::Result<bool> {
  ...
  pub async fn execute_hash_collect<A: DatabaseAof<D>>(
    &self,
    db: &GarnetDatabase<D, A>,
  ) -> wkv::Result<usize> {
  ...
  pub async fn execute_sorted_set_collect<A: DatabaseAof<D>>(
    &self,
    db: &GarnetDatabase<D, A>,
  ) -> wkv::Result<usize> {
  ...
  pub async fn initiate_checkpoint_async<A: DatabaseAof<D>>(
    &self,
    db: &GarnetDatabase<D, A>,
  ) -> wkv::Result<bool> {
    self.take_database_checkpoint_async(db).await
  ...
  pub async fn store_expired_key_deletion_scan<A: DatabaseAof<D>>(
    &self,
    db: &GarnetDatabase<D, A>,
  ) -> wkv::Result<(u64, u64)> {
    db.store
...
async fn execute_collect<D: Device, A: DatabaseAof<D>>(
  db: &GarnetDatabase<D, A>,
  tag: u8,
) -> wkv::Result<(u8, usize)> {
  let prefix = SessionPrefixBuf::new(0, db.id.max(0) as u64);
