...
impl<D: Device, A: DatabaseAof<D>> MultiDatabaseManager<D, A> {
  ...
  pub fn copy_databases(&self) -> GxHashMap<i64, Arc<GarnetDatabase<D, A>>> {
    self
      .databases
      .pin()
      .iter()
  ...
  pub fn update_last_save_data(&self, db: &GarnetDatabase<D, A>, now_ms: u64) {
  ...
  pub async fn await_commit(&self, db_id: i64) -> wkv::Result<bool> {
    self.wait_for_commit_to_aof_async(db_id).await
  }
