...
use super::{
  aof::DatabaseAof, garnet_database::GarnetDatabase, i_database_manager::IDatabaseManager,
  multi_database_manager::MultiDatabaseManager, single_database_manager::SingleDatabaseManager,
};

/// 数据库管理器（单库 / 多库二选一，静态分发）
pub enum DatabaseManager<D: Device, A: DatabaseAof<D> = ()> {
  /// 单库模式
  Single(SingleDatabaseManager<D, A>),
  /// 多库模式
  Multi(MultiDatabaseManager<D, A>),
}

impl<D: Device, A: DatabaseAof<D>> DatabaseManager<D, A> {
  /// 库数量
  ...
  pub fn try_default_database(&self) -> Option<Arc<GarnetDatabase<D, A>>> {
    match self {
...
impl DatabaseManagerFactory {
  /// 数据库管理器构造
  fn build<D: Device, A: DatabaseAof<D>>(
    multi: bool,
    store: Arc<WedbStore<D>>,
    checkpoint_root: PathBuf,
    default_db: Arc<GarnetDatabase<D, A>>,
  ) -> DatabaseManager<D, A> {
    if multi {
...
impl DatabaseManagerFactory {
  ...
  pub fn create_database_manager<D: Device, A: DatabaseAof<D>>(
    multi: bool,
    store: Arc<WedbStore<D>>,
    checkpoint_root: PathBuf,
    default_db: Arc<GarnetDatabase<D, A>>,
