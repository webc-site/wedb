...
use crate::functions_state::FunctionsState;

/// 单库管理器
pub struct SingleDatabaseManager<D: Device, A: DatabaseAof<D> = ()> {
  /// 共享基座（检查点管理器等）
  pub base: DatabaseManagerBase<D>,
  /// 唯一数据库（DB 0）
  pub db: Arc<GarnetDatabase<D, A>>,
}

impl<D: Device, A: DatabaseAof<D>> SingleDatabaseManager<D, A> {
  /// 创建单库管理器
  pub fn new(checkpoint_dir: PathBuf, db: Arc<GarnetDatabase<D, A>>) -> Self {
    Self {
  ...
  pub fn try_get_or_add_database(&self) -> wkv::Result<(Arc<GarnetDatabase<D, A>>, bool)> {
  ...
  pub async fn wait_for_commit_to_aof(&self) -> wkv::Result<bool> {
  ...
  /// 异步等待单库 AOF 提交完成（1:1 对标 C# SingleDatabaseManager.WaitForCommitToAofAsync）
  #[inline]
  pub async fn wait_for_commit_to_aof_async(&self) -> wkv::Result<bool> {
    self.wait_for_commit_to_aof().await
  }
  ...
  pub fn get_databases_snapshot(&self) -> Vec<Arc<GarnetDatabase<D, A>>> {
...
impl<D: Device, A: DatabaseAof<D>> IDatabaseManager<D> for SingleDatabaseManager<D, A> {
  type A = A;
  ...
  /// 满足 IDatabaseManager trait 接口规范，单库实现始终操作默认 db，保留 _db_id
  fn try_get_database(&self, _db_id: i64) -> Option<Arc<GarnetDatabase<D, A>>> {
  ...
  /// 满足 IDatabaseManager trait 接口规范，单库实现始终操作默认 db，保留 _db_id
  async fn wait_for_commit_to_aof_async(&self, _db_id: i64) -> wkv::Result<bool> {
    SingleDatabaseManager::wait_for_commit_to_aof(self).await
  }
