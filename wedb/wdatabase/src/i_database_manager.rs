...
use super::{aof::DatabaseAof, functions_state::FunctionsState, garnet_database::GarnetDatabase};
...
/// 获取或新建数据库返回类型
pub type GetOrAddResult<D, A> = wkv::Result<(Arc<GarnetDatabase<D, A>>, bool)>;

/// 数据库管理接口
pub trait IDatabaseManager<D: Device>: Send + Sync {
  type A: DatabaseAof<D>;

  /// 取库（不存在则新建），返回 (库, 是否新建)
  ///
  /// libs/server/Databases/IDatabaseManager.cs:TryGetOrAddDatabase
  fn try_get_or_add_database(&self, db_id: i64)
  -> impl Future<Output = GetOrAddResult<D, Self::A>>;

  /// 取库（不新建）
  ///
  /// libs/server/Databases/IDatabaseManager.cs:TryGetDatabase
  fn try_get_database(&self, db_id: i64) -> Option<Arc<GarnetDatabase<D, Self::A>>>;
  ...
  fn commit_to_aof_async(&self, db_id: i64) -> impl Future<Output = wkv::Result<()>>;

  /// 等待 AOF 提交完成（事件驱动无锁等待）
  ///
  /// libs/server/Databases/IDatabaseManager.cs:WaitForCommitToAofAsync
  fn wait_for_commit_to_aof_async(&self, db_id: i64) -> impl Future<Output = wkv::Result<bool>>;
  ...
  fn get_databases_snapshot(&self) -> Vec<Arc<GarnetDatabase<D, Self::A>>>;
