...
impl GarnetObjectSerializer {
  ...
  ///
  /// 直接从字节切片反序列化（零中转分配）
  pub fn deserialize_from_slice(slice: &[u8]) -> io::Result<Option<GarnetObjectValue>> {
    let Some((&first, payload)) = slice.split_first() else {
  ...
  pub fn deserialize_internal(reader: &mut impl Read) -> io::Result<Option<GarnetObjectValue>> {
    ...
    Self::deserialize_from_slice(&payload)
  }
