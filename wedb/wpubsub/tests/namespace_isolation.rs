...
/// 推送帧剥离失败安全丢弃（task/ing/wpubsub-auth-namespace-leak.md 兜底验收；
/// 源票 wpubsub-auth-namespace-leak 已随 2026-09-22 历史重置灭失，验收即本测试）：
/// 会话换绑 ns 前旧租户投递的残留消息（邮箱内为他 ns 隔离键）在 drain 时
/// 整帧丢弃，裸隔离键与跨租户消息体严禁外发；本域消息剥离正常不误伤
#[test]
fn foreign_ns_residual_frames_are_dropped() {
  use wpubsub::subscriber::PubSubSink;

  let broker = Arc::new(SubscribeBroker::new());
  let mut wire = PubSubSession::new(broker);
  let mut h = Host::new(3, 1);
  assert!(h.network_subscribe(&mut wire, false, &[b"chan"]));
  h.take();

  // 模拟旧租户（ns2）残留投递：三类消息帧直入邮箱，隔离键均非本域前缀
  let mailbox = wire.mailbox().unwrap();
  mailbox.publish(b"2:chan", b"leak-channel");
  mailbox.pattern_publish(b"2:p*", b"2:chan", b"leak-pattern");
  mailbox.shard_publish(b"2:schan", b"leak-shard");
  // 本域正常消息混排：唯一应外发的帧
  mailbox.publish(b"1:chan", b"ok");

  h.drain_pubsub_frames(&mut wire);
  let out = h.take();
  assert_eq!(
    out, "*3\r\n$7\r\nmessage\r\n$4\r\nchan\r\n$2\r\nok\r\n",
    "仅本域帧外发，他 ns 残留帧（通道/模式/分片）整帧丢弃: {out}"
  );
  assert!(!out.contains("2:"), "裸隔离键前缀严禁外发: {out}");
}
