...
impl<S: PubSubSink> SubscribeBroker<S> {
  ...
  /// 单消费者流式无锁出队广播，零中间分配（O(1) 辅助空间），逐条释放负载内存。
  pub fn consume_pending(&self) -> usize {
    ...
    while let Some((key, value)) = self.pending_queue.pop() {
      total_notified += self.broadcast(&key, &value);
      if self.is_idle() {
        self.clear();
        break;
      }
  ...
  pub fn publish(&self, key: &[u8], value: &[u8]) {
    ...
    let _ = self.pending_queue.push((key.into(), value.into()));
  ...
  ///
  /// C# 后台消费任务 `StartAsync` 中 `iterator.ConsumeAllAsync` 的等待面
  /// （扫描器在无新日志时挂起，Enqueue 后被信号唤醒）；宿主后台消费
  /// 任务以 `wait_pending → consume_pending` 循环对译同一驱动模型。
...
fn decode_payload_slices(payload: &[u8]) -> Option<(&[u8], &[u8])> {
  let (key_len_bytes, rest) = payload.split_first_chunk::<4>()?;
  let key_len = usize::try_from(i32::from_le_bytes(*key_len_bytes)).ok()?;
  let (key, rest) = rest.split_at_checked(key_len)?;

  let (val_len_bytes, rest) = rest.split_first_chunk::<4>()?;
  let val_len = usize::try_from(i32::from_le_bytes(*val_len_bytes)).ok()?;
  let (value, _) = rest.split_at_checked(val_len)?;
...
mod tests {
  ...
  #[test]
  fn decode_payload_slices_edge_cases() {
    // 空负载
    assert_eq!(decode_payload_slices(b""), None);
    // 不足 key_len 头部（< 4 字节）
    assert_eq!(decode_payload_slices(&[1, 2, 3]), None);

    // key_len 溢出剩余长度
    let mut bad_key_len = Vec::new();
    bad_key_len.extend_from_slice(&10i32.to_le_bytes());
    bad_key_len.extend_from_slice(b"abc");
    assert_eq!(decode_payload_slices(&bad_key_len), None);

    // 负 key_len
    let mut neg_key_len = Vec::new();
    neg_key_len.extend_from_slice(&(-1i32).to_le_bytes());
    assert_eq!(decode_payload_slices(&neg_key_len), None);

    // val_len 溢出剩余长度
    let mut bad_val = Vec::new();
    bad_val.extend_from_slice(&1i32.to_le_bytes());
    bad_val.push(b'k');
    bad_val.extend_from_slice(&5i32.to_le_bytes());
    bad_val.extend_from_slice(b"xy");
    assert_eq!(decode_payload_slices(&bad_val), None);

    // 合法负载携带额外尾部字节
    let mut valid_with_tail = Vec::new();
    valid_with_tail.extend_from_slice(&1i32.to_le_bytes());
    valid_with_tail.push(b'k');
    valid_with_tail.extend_from_slice(&1i32.to_le_bytes());
    valid_with_tail.push(b'v');
    valid_with_tail.extend_from_slice(b"extra");
    let (k, v) = decode_payload_slices(&valid_with_tail).unwrap();
    assert_eq!(k, b"k");
    assert_eq!(v, b"v");
  }
}
