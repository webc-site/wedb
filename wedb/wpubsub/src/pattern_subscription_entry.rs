...
pub type PatternSubscriberSet<S = Arc<crate::PubSubMailbox>> = ConcurrentMap<u64, S>;

/// 模式订阅条目：一个 glob 模式及订阅它的会话集合
///
/// 托管面以"模式 -> 条目"键化并发表承接 C# 的条目集合，模式字段与键
/// 同源（C# 条目即携模式副本，此处键为去重面、字段为广播面）。
pub struct PatternSubscriptionEntry<S = Arc<crate::PubSubMailbox>> {
...
impl PatternSubscriptionEntry<Arc<crate::PubSubMailbox>> {
...
impl<S> PatternSubscriptionEntry<S> {
  ...
  pub fn equals<O>(&self, other: &PatternSubscriptionEntry<O>) -> bool {
