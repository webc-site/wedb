...
impl CustomCommandManager {
  ...
  pub fn is_custom_command_registered(&self, cmd_name: &str) -> bool {
    !cmd_name.is_empty() && self.custom_command_names.contains(&cmd_name.to_lowercase())
  }
  ...
  /// 保留 _id 形参以匹配 CustomCommandManager.GetCustomGarnetObjectType 签名规范
  pub fn get_custom_garnet_object_type(&self, _id: u8) -> GarnetObjectType {
