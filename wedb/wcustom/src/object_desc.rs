...
//!
//! 自研依据: 编译期静态对象描述（transpile 契约：删除运行时动态注册，静态枚举分发）
...
/// COSCAN 成员扫描执行体签名（C# `GarnetObjectBase.Scan` 抽象成员扫描段的
/// 静态承接：`Scan(long start, out List<byte[]> items, out long cursor, int count,
/// byte* pattern, int patternLength, bool isNoValue)`）
///
/// 入参：信封载荷（剥 1B 类型标签）、起始游标、单轮上限（已钳
/// OBJECT_SCAN_COUNT_LIMIT）、MATCH 模式、NOVALUES 旗标；返回 `(成员序列,
/// 出页游标)` 或错误文案（由组帧内核落 RESP 错误帧，如 JSON 域
/// NotImplementedException 的错误帧裁量）。C# items 元素可为 null（RESP null
/// 帧），本仓扩展对象契约不产出 null 成员，收窄为非 null 成员序列。
pub type CustomScanMembersFn =
  fn(&[u8], i64, i64, &[u8], bool) -> Result<(Vec<Vec<u8>>, i64), &'static [u8]>;
...
pub struct CustomObjectEntry {
  ...
  /// COSCAN 成员扫描执行体（每对象单点，见 [`CustomScanMembersFn`]）
  ///
  /// 对标 C# 抽象 Scan 的每对象实现：RoaringBitmapObject.cs 上游刻意留空
  ///（恒空收集 + 游标 0，注释与实现矛盾以实现为准）、GarnetJsonObject.cs:132
  /// NotImplementedException（rust 裁量错误帧）。COSCAN 域收口（仅扩展标签
  /// 域命中才到本执行体）由 server 层分派单点保证
  pub scan_members: CustomScanMembersFn,
}
