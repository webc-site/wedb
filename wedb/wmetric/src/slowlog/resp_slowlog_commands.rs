...
impl RespSlowlogCommands {
  ...
  pub fn handle_slow_log(ctx: &SlowLogContext<'_>, slow_log_start_time: &mut u64) {
    ...
    let elapsed = ctx.ticks_stopwatch.saturating_sub(*slow_log_start_time);
    ...
    if ctx.cmd != RespCommand::Invalid && elapsed > ctx.slow_log_threshold {
      let entry = SlowLogEntry {
        ...
        // 不引实时域时间戳）
        timestamp: (ctx.ticks_stopwatch / TICKS_PER_SECOND as u64) as i32,
        command: ctx.cmd,
    ...
    // 推进起始时间戳，跟踪批次内的下一命令。
    *slow_log_start_time = ctx.ticks_stopwatch;
  }
}
