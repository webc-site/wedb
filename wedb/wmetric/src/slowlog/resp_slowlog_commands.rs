...
use wbase::{convert::stopwatch::TICKS_PER_MICROSECOND, num::strict_i32, time::TICKS_PER_SECOND};
...
///
/// 按值消费：`handle_slow_log` 取走 `arguments` 所有权直接 move 进
/// `SlowLogEntry`（对标 C# RespSlowlogCommands.cs:171 `entry.Arguments = args`
/// 一次分配直赋值），杜绝借用语义下对参数快照的二次深拷贝；Arc 胶囊使
/// `get_entries` 读出面克隆降为引用计数浅拷贝（C# struct 按引用共享同位）。
...
impl RespSlowlogCommands {
  ...
  pub fn network_slow_log_get(
    ...
    for entry in &entries {
      ...
      output.write_resp_int(entry.id);
      output.write_resp_int(i64::from(entry.timestamp));
      output.write_resp_int(i64::from(entry.duration));

      // C# :84/:96 `entry.Command.ToString()`：枚举成员名 SCREAMING 词形
      //（RespCommand.cs BLPOP/GET），取 strum 单点 to_cs_name（'static 零分配，
      // 对齐 pubsub NotAllowed 错误帧同源词形）
      let command_name = entry.command.to_cs_name();
