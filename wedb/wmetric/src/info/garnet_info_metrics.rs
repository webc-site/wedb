...
use wbase::time::{now_nanos, now_secs};
...
pub trait InfoProvider {
  ...
  /// 订阅邮箱溢出丢弃数聚合（rust 域扩展，C# 直写模型无丢帧语义故无此
  /// 指标；None = 数据源未装配，clients 段不输出该项）
...
impl GarnetInfoMetrics {
  ...
  fn populate_server_info(&mut self, provider: &impl InfoProvider) {
    ...
    let uptime_secs = now_secs() as i64 - facts.startup_timestamp_unix_secs;
  ...
  fn populate_memory_info(&mut self, provider: &impl InfoProvider) {
    ...
    self.memory_info = Some(vec![
      MetricsItem::from_usize("system_page_size", page_size()),
  ...
  /// STORE 族行表预分配（对齐 C# `new MetricsItem[storeWrapper.MaxDatabaseId + 1][]`
  /// 与 GetDatabasesSnapshot() 共用 databases 映射的自洽性）：行数 = 快照
  /// 最大库 id + 1（空快照 0 行），与写入下标出自同一份快照，越界在类型上
  /// 不可构造；未填行保留空 Vec（对应 C# 的 null 项，渲染只出段头）。
  ...
  fn populate_store_stats(&mut self, provider: &impl InfoProvider) {
    ...
    let mut store_info = Self::store_rows(&databases);
    for db in &databases {
  ...
  ///
  /// connected_clients 之外追加 rust 域扩展项 pubsub_dropped（订阅邮箱溢出
  /// 丢弃聚合，C# 无此指标；数据源未装配时省略，见 InfoProvider::
  /// pubsub_dropped）。
  fn populate_clients_info(&mut self, provider: &impl InfoProvider) {
    ...
    let mut items = vec![MetricsItem::new(
      "connected_clients",
    ...
    if let Some(dropped) = provider.pubsub_dropped() {
      items.push(MetricsItem::new("pubsub_dropped", dropped.to_string()));
    }
    self.clients_info = Some(items);
  }
