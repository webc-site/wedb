...
impl LatencyMetricsType {
  ...
  pub fn from_name(name: &[u8]) -> Option<Self> {
    if name.eq_ignore_ascii_case(b"NET_RS_LAT") {
      Some(Self::NetRsLat)
    } else if name.eq_ignore_ascii_case(b"PENDING_LAT") {
      Some(Self::PendingLat)
    } else if name.eq_ignore_ascii_case(b"TX_PROC_LAT") {
      Some(Self::TxProcLat)
    } else if name.eq_ignore_ascii_case(b"NET_RS_BYTES") {
      Some(Self::NetRsBytes)
    } else if name.eq_ignore_ascii_case(b"NET_RS_OPS") {
      Some(Self::NetRsOps)
    } else if name.eq_ignore_ascii_case(b"NET_RS_LAT_ADMIN") {
      Some(Self::NetRsLatAdmin)
    } else {
      None
    }
  }
