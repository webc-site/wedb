...
mod tests {
  ...
  fn test_session_lifecycle_and_switching() {
    ...
    let session = GarnetLatencyMetricsSession::new(
    ...
    assert_eq!(
      snapshot[LatencyMetricsType::NetRsLat.idx()].latency[0].len(),
      1
    );

    // 2. stop_and_switch (相同命令)
    ...
    let snapshot = session.metrics_snapshot().unwrap();
    assert_eq!(
      snapshot[LatencyMetricsType::NetRsLatAdmin.idx()].latency[0].len(),
      1
    );
    ...
    let snapshot = session.metrics_snapshot().unwrap();
    assert_eq!(
      snapshot[LatencyMetricsType::NetRsBytes.idx()].latency[0].len(),
      1
    );
    ...
    assert_eq!(session.prior_version(), 0);

    // 6. reset_all (重置 prior_version 0)
    session.reset_all();
    let snapshot = session.metrics_snapshot().unwrap();
    assert_eq!(
      snapshot[LatencyMetricsType::NetRsLat.idx()].latency[0].len(),
