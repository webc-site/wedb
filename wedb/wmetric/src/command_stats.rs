//! 在 garnet 中的相对路径: libs/server/Metrics/CommandInfo? 对标 C# 命令统计（test/standalone/Garnet.test/RespCommandStatsTests.cs）
...
impl CommandStats {
  ...
  pub const ENTRY_COUNT: usize = LAST_VALID_COMMAND as u16 as usize + 1;
  ...
  pub fn increment_calls(&mut self, cmd: RespCommand) {
    self.increment_calls_by(cmd, 1);
  }

  /// 指定命令的调用计数增加指定数量。
  #[inline]
  pub fn increment_calls_by(&mut self, cmd: RespCommand, count: u64) {
