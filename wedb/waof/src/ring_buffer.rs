...
impl RingBuffer {
  ...
  pub fn new(capacity: usize, align: usize) -> Result<Self> {
    if !wbase::is_valid_sector_size(align) {
      return Err(wbase::Error::InvalidAlignment(align, wbase::MIN_SECTOR_SIZE).into());
    }
    if capacity == 0 || !capacity.is_multiple_of(align) {
      return Err(Error::Mem(wbase::Error::InvalidAlignment(capacity, align)));
    }
    let layout = Layout::from_size_align(capacity, align).map_err(wbase::Error::from)?;
    let raw = unsafe { alloc_zeroed(layout) };
    let ptr = NonNull::new(raw).ok_or(wbase::Error::AllocFailed(layout))?;
