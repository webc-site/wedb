//! AOF 条目类型、负载形状判定与头布局公共常量
//! （对标 libs/server/AOF/AofEntryType.cs:AofEntryType / AofEntryTypeExtensions）
//!
//! 在 garnet 中的相对路径: libs/server/Resp/AofEntryType? 对标 AOF 条目类型枚举（本仓 enum u8 形态）
...
pub enum AofEntryType {
  ...
  /// FLUSH DB。
  FlushDb = 0x61,
  /// FLUSH 命名空间（rust 多租户共享单 AOF 扩展变体：整 ns 虚拟换号清库，
  /// 载荷携带 ns 维度；C# 每实例单租户无此形态）
  FlushNs = 0x62,
  /// 统一存储 upsert 字符串。
...
impl AofEntryType {
  ...
  /// 分块写入器与读取器依赖此判定保持一致。
  #[inline]
  pub const fn has_chunk_value(self) -> bool {
    matches!(
      self,
      Self::StoreUpsert
        | Self::ObjectStoreUpsert
  ...
  #[inline]
  pub const fn has_chunk_input(self) -> bool {
    matches!(
      self,
      Self::StoreUpsert
        | Self::StoreRMW
  ...
  #[inline]
  pub const fn has_chunk_object_value(self) -> bool {
    matches!(
      self,
