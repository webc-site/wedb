...
/// 复制流推流回调端口（对标 C# TsavoriteLog 迭代器驱动的 AOF 消费端，
/// 见 libs/cluster/Server/Replication/PrimaryOps/AofOperations/AofSyncTask.cs:Consume）
///
/// 参数为（记录起始逻辑地址, 完整记录帧：8 字节记录头 + 负载）。帧成功入队后、
/// 在途槽位释放前同栈触发——推流序与 AOF 地址序原子一致，并发写入下从侧
/// 应用序与主侧重启重放序永不发散。回调必须无阻塞。
pub struct ReplicationSinkFn {
  ptr: *const (),
  call: unsafe fn(*const (), u64, &[u8]),
  drop: unsafe fn(*const ()),
  clone: unsafe fn(*const ()) -> *const (),
}

unsafe impl Send for ReplicationSinkFn {}
unsafe impl Sync for ReplicationSinkFn {}

impl ReplicationSinkFn {
  pub fn new<F>(f: F) -> Self
  where
    F: Fn(u64, &[u8]) + Send + Sync + 'static,
  {
    unsafe fn call_fn<F: Fn(u64, &[u8])>(ptr: *const (), addr: u64, frame: &[u8]) {
  ...
  #[inline]
  pub fn call(&self, addr: u64, frame: &[u8]) {
    unsafe { (self.call)(self.ptr, addr, frame) }
  }
}

impl Drop for ReplicationSinkFn {
  fn drop(&mut self) {
    if !self.ptr.is_null() {
...
impl Clone for ReplicationSinkFn {
  fn clone(&self) -> Self {
    Self {
...
impl<F> From<Arc<F>> for ReplicationSinkFn
where
  F: Fn(u64, &[u8]) + Send + Sync + 'static,
{
...
/// 栈上推流帧缓冲上限（小于等于此尺寸免堆分配）
const SMALL_FRAME_STACK_BUF_SIZE: usize = 1024;

/// 自旋等待在途槽位让出前的最大自旋次数
const INFLIGHT_SLOT_SPIN_LIMIT: u32 = 32;
...
impl<D: Device> WalLog<D> {
  ...
  pub fn set_replication_sink(&self, sink: impl Into<ReplicationSinkFn>) -> bool {
    self.inner.replication_sink.set(sink.into()).is_ok()
  }
  ...
  fn acquire_inflight_slot(&self) -> (usize, u64) {
    ...
    loop {
      ...
      spins += 1;
      if spins < INFLIGHT_SLOT_SPIN_LIMIT {
        spin_loop();
      } else {
        yield_now();
      }
  ...
  fn reserve_address(
    ...
    loop {
      ...
      let start_aligned = wbase::align_down(flushed, sector_size);
  ...
  async fn flush_and_sync_range(&self, flushed: u64, safe_tail: u64) -> Result<u64> {
    ...
    let sector_size = self.device.sector_size() as u64;
    let start_aligned = wbase::align_down(flushed, sector_size);
    let end_aligned = wbase::align_up(safe_tail, sector_size);
