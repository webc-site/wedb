//! WAL 入队流水线（对标 libs/storage/Tsavorite/cs/src/core/TsavoriteLog/
//! TsavoriteLog.cs:Enqueue 族）：预校验 → 注册在途槽位 → CAS 预占地址 →
//! 分部件写环形缓冲 → 推流唤醒 → 释放槽位。
...
impl<D: Device> WalLog<D> {
  ...
  ///（对标 C# TsavoriteLog.Chunked.cs:EnqueueChunkedSpan 的
  /// BeginInflightEnqueue 单在途操作）。
  ///
  ...
  /// 依序写入全部帧 → 推流唤醒 → 释放槽位（单点骨架，单帧与分块多帧共用）
  ///
    ...
    I: IntoIterator<Item = (WalFrameHeader, &'p [&'p [u8]])>,
  {
    ...
    // 4. 推流唤醒信号：全部帧已写入环形缓冲（safe_tail 语义保证信号被消费时
