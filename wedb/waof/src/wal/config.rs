//! 自研依据: WAL 配置单点（C# AOF 由 TsavoriteLog 承载，配置面 libs/host/ServerSettingsManager.cs）
use wbase::cfg::DEFAULT_HLOG_PAGE_SIZE;

/// 默认并发写入在途追踪槽位容量
const DEFAULT_INFLIGHT_SLOTS: usize = 256;

/// 默认内存环形写缓冲区容量（16MB）
const DEFAULT_BUFFER_SIZE: usize = DEFAULT_HLOG_PAGE_SIZE;
