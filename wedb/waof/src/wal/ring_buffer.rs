...
impl RingBuffer {
  ...
  pub(crate) fn decode_frame(&self, addr: u64, read_limit: u64) -> MemFrame {
    ...
    // 全零头（填充/崩溃残缺尾）或负载超限伪头：与 C# Scan 的 GetLength 后
    // 零长/非法长度守卫同位
    if header.is_zero() || entry_len > self.capacity {
      return MemFrame::Invalid;
    }
    let next_addr = addr + (RECORD_HEADER_LEN as u64) + (entry_len as u64);
    if next_addr > read_limit {
      return MemFrame::Invalid;
    }
  ...
  pub fn read_frame_vec(
    ...
    let cap = self.capacity;
    let ring_off = self.ring_offset(payload_offset);
    let raw = self.ptr.as_ptr();
    let dest = vec.as_mut_ptr();
    // SAFETY: total_len = RECORD_HEADER_LEN + payload_len，vec 分配了对应容量；
    // dest 前 8 字节就地写入 header 序列化字节；
    // payload_dest 偏移 RECORD_HEADER_LEN，直接从环形内存单次拷贝（或分两段回绕拷贝），
    ...
    unsafe {
      ...
      let payload_dest = dest.add(RECORD_HEADER_LEN);
      if ring_off + payload_len <= cap {
        copy_nonoverlapping(raw.add(ring_off), payload_dest, payload_len);
      } else {
        let part1 = cap - ring_off;
        let part2 = payload_len - part1;
        copy_nonoverlapping(raw.add(ring_off), payload_dest, part1);
        copy_nonoverlapping(raw, payload_dest.add(part1), part2);
      }
  ...
  /// 一次分配头+负载缓冲，消除每次 reconstruct_frame 的二次分配重拷）
  #[inline]
