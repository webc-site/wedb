...
impl<D: Device> WalLog<D> {
  ...
  ///   绝不静默清空位点伪装成空日志；
  /// - 收尾位点安全收敛（对标 C# AllocatorBase.cs:RecoveryReset）：tail/flushed
  ///   统一收敛至提交上界（最后 commit 帧尾），提交帧后的未提交孤儿与撕裂残包
  ///   经 [`Device::erase_tail_after`] 物理截断收缩 + 删孤儿段，杜绝重放复活
  ///   与残留复观测（恢复幂等）；
  ...
  pub async fn recover(&self) -> Result<u64> {
    ...
    loop {
      if !disk_win.covers(cur, RECORD_HEADER_LEN) {
        ...
        {
          Ok(buf) => buf,
          // 可恢复链正常终止于 EOF（末尾残缺头不足 8 字节）；actual > 0 表示
          // 尾后仍留有非零残包（不足一帧头的撕裂写），同样计入保守截尾观测，
          // 供下方物理擦尾判据承接——静默放行会让撕裂残留逃过清理，破坏二次
          // 恢复幂等
          Err(Error::Device(DeviceError::UnexpectedEof { actual, .. })) => {
            if actual > 0 {
              self
                .note_recover_truncation(cur, "末尾残缺帧头（不足 8 字节）")
                .await;
            }
            break;
          }
    ...
    // 撕裂尾与未提交孤儿段的物理清理（对标 C# AllocatorBase.cs:RecoveryReset
    // 收敛位点后 ClearPage 清尾的盘面对位，盘面单点下沉
    // [`Device::erase_tail_after`]，本处不起第二套截断臂）：扫描终点越过提交
    // 上界即存在「CRC 完好但无 commit 帧见证」的孤儿记录，或截尾观测点亮即
    // 存在撕裂坏写——两者都自提交上界起物理收缩 + 删孤儿段，保证二次恢复
    // 幂等（残尾不会再被 has_nonzero_after 复现观测）。纯扇区填充零（观测
    // 判为崩溃常态）按 C# 页面填充保留口径不清理
    //
    // 擦尾先于位点写：物理失败即上抛快速失败，内存位点保持擦除前原状
    if committed < cur || self.recover_truncated_at.load(Ordering::Acquire) != 0 {
      self.device.erase_tail_after(committed).await?;
    }
    ...
    if let Some((meta, _)) = last_commit {
      ...
      self
        .last_commit_cookie
        .store(meta.cookie, Ordering::Release);
      self.last_commit_begin.store(meta.begin, Ordering::Release);
    }
    self.last_commit_frame.store(committed, Ordering::Release);

    // 位点安全收敛（对标 RecoveryReset 把 tailAddress/flushedUntilAddress 统一
    // 收敛至提交点）：tail/flushed 恒取提交上界而非扫描终点 cur——cur 含崩溃前
    // 已落盘但未获 commit 帧见证的在途记录，取 cur 会让 wnode 重放驱动
    // （until = tail）复活脏写并使新 enqueue 继承幽灵尾；无 commit 帧回退形态
    // 下 committed == cur，本收敛恒为幂等空操作
    self.tail_address.store(committed, Ordering::Release);
    self
