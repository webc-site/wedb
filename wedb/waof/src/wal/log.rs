...
pub(crate) fn log_begin_address<D: Device>(device: &D) -> u64 {
  ...
  let seg_size = device.segment_size();
  start_seg * seg_size
}
...
pub struct WalLogInner<D: Device> {
  ...
  /// 已写出的最后 commit 元数据帧的 cookie
  pub last_commit_cookie: AtomicI64,
  /// 已写出的最后 commit 元数据帧的 begin 快照
  pub last_commit_begin: AtomicU64,
...
impl<D: Device> WalLog<D> {
  ...
  pub fn new(device: Arc<D>, config: WalConfig) -> Result<Self> {
    ...
    Ok(Self {
      inner: Arc::new(WalLogInner {
        ...
        last_commit_cookie: AtomicI64::new(commit::NO_COOKIE),
        last_commit_begin: AtomicU64::new(begin_addr),
  ...
  /// 副本回放提交元数据对齐：把本地日志的提交边界原子推进至主端 commit 帧位
  /// （副本本地不写 commit 帧，本地提交上界唯一推进源即本口——主端帧经推流
  /// 保真落盘后由回放链拦截对齐）
  ///
  /// 在 garnet 中的相对路径:libs/storage/Tsavorite/cs/src/core/TsavoriteLog/
  /// TsavoriteLog.cs:UnsafeCommitMetadataOnly（:758，经
  /// libs/cluster/Server/Replication/ReplicaOps/AOFReplay/ReplicaReplayDriver.cs
  /// :ConsumeDirect 的 payloadLength < 0 分支调用）
  ///
  /// 语义分解：
  /// - 刷盘并推进 flushed/committed 至 `until_address`（帧尾，即帧位置承载的
  ///   UntilAddress 口径，见 [`super::commit`] 模块文档）。目标恒钳
  ///   [`Self::safe_tail_address`]（在途写入者压低的已定稿下界，与
  ///   flush.rs:WalCommitStep 的单点钳制同不变式——刷盘与提交推进绝不越过
  ///   未定稿字节）；`until` 已在刷盘水位之下时内置早退，幂等
  /// - `last_commit_frame` 单调推进至帧尾（本地镜像流的帧游标与主端对齐；
  ///   truncate 的 fetch_min 钳制与升主后 WalCommitStep 的级联去重均以它为准）
  /// - cookie 单调回填（对标 C# UpdateCommittedState :2732 的
  ///   `MonotonicUpdate(persistedCommitNum)`；[`commit::NO_COOKIE`] =
  ///   i64::MIN 恒不回退真实序列号），begin 快照直赋（对标
  ///   `CommittedBeginAddress = recoveryInfo.BeginAddress`）
  /// - `begin_address` 按 `meta.begin` 单调推进（对标 C# 「First update the
  ///   begin address」的纯逻辑栅栏：主端已在帧写时点物理截断至该位点，本地
  ///   其下记录已应用且永不再需；物理删段仍由 [`Self::truncate`] 的
  ///   truncate_begin_until 内核承担，本口不做设备 I/O 删段）
  ///
  /// 持有 [`WalLogInner::commit_lock`]：与 commit_to Leader / commit_flush_only
  /// / truncate / reset 全量串行，对位 C# `lock (ongoingCommitRequests)` 的
  /// 提交元数据互斥
  pub async fn unsafe_commit_metadata_only(
    ...
    let guard = self.commit_lock.lock().await;
    // 提交推进恒以刷盘为先（committed ≤ flushed 不变式：wait_for_commit 的
    // 「已提交 = 已持久」承诺不得出现内存超前于盘面）
    let target = until_address.min(self.safe_tail_address());
    ...
    self
      .recovered_cookie
      .fetch_max(meta.cookie, Ordering::AcqRel);
    self
      .recovered_committed_begin
      .fetch_max(meta.begin, Ordering::AcqRel);
    // meta.begin ≤ 帧位 ≤ until：钳 target 封住钳刷盘引入的窄窗，保
    // begin ≤ committed 不变式恒成立（正常流 target == until，钳制为空操作）
    self
      .begin_address
      .fetch_max(meta.begin.min(target), Ordering::AcqRel);
    drop(guard);
    Ok(())
  }

  /// 创建指定范围的 WAL 记录扫描迭代器
  /// libs/storage/Tsavorite/cs/src/core/Allocator/TsavoriteLogAllocatorImpl.cs:Scan
  ...
  pub async fn reset(&self) -> Result<()> {
    ...
    self.last_commit_begin.store(begin_addr, Ordering::Release);
    self
      .pending_cookie
      .store(commit::NO_COOKIE, Ordering::Release);
  ...
  /// 是否有元数据变更需要提交（对标 C# ShouldCommitMetadata: beginAddress > CommittedBeginAddress || info.Cookie != null）
  #[inline]
  pub fn should_commit_metadata(&self) -> bool {
    let pending_cookie = self.pending_cookie.load(Ordering::Acquire);
    let last_cookie = self.last_commit_cookie.load(Ordering::Acquire);
    let begin = self.begin_address.load(Ordering::Acquire);
    let last_begin = self.last_commit_begin.load(Ordering::Acquire);
    (pending_cookie != commit::NO_COOKIE && pending_cookie != last_cookie) || begin > last_begin
  }
