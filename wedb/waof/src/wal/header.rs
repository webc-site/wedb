...
//!
//! 自研依据: WAL 记录头编解码（对标 C# AofHeader 元数据形态 libs/server/Storage/AofHeader? 本仓 bitcode 序化）
...
#[cfg(test)]
mod tests {
  use super::{EMPTY_PAYLOAD_CRC, RECORD_HEADER_LEN, WalFrameHeader};
  ...
  fn test_record_header_roundtrip() {
    ...
    let empty_payload_header = WalFrameHeader::for_payload(&[]);
