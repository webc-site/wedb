...
impl<D: Device> WalScanIterator<D> {
  ...
  /// TsavoriteLog 的 Scan 单趟解码），本入口只保留落盘回退终止语义
  ...
  async fn read_record_from_device(
    ...
    let payload = if let Some(p) = self.disk_win.payload(record_addr, &header, rel_off) {
      header.verify(p)?;
      p.to_vec()
    } else {
    ...
    Ok(Some((header, payload, next_addr)))
  }
