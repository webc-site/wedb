...
async fn collect_iter<D: wdev::Device>(
  mut iter: WalScanIterator<D>,
) -> aok::Result<Vec<WalRecord>> {
...
async fn collect_data<D: wdev::Device>(
  mut iter: WalScanIterator<D>,
) -> aok::Result<Vec<WalRecord>> {
  let mut records = Vec::new();
  while let Some(rec) = iter.next().await? {
...
/// 测试 WalFrameHeader 的编解码与校验和
#[test]
fn test_record_header() -> Void {
  assert_eq!(RECORD_HEADER_LEN, 8);

  // 1. 空负载测试：携带非零哨兵 CRC，保证记录头绝不与扇区填充零混淆
  let empty_hdr = WalFrameHeader::for_payload(&[]);
  assert_eq!(empty_hdr.entry_len, 0);
  assert_ne!(empty_hdr.crc32, 0);
  assert!(!empty_hdr.is_zero());
  empty_hdr.verify(&[])?;
  assert!(matches!(
    WalFrameHeader::new(0, 0).verify(&[]),
    Err(Error::ChecksumMismatch { .. })
  ));

  // 2. 正常负载编解码测试
  let payload = b"Hello, Wedb Wal Engine!";
  let header = WalFrameHeader::for_payload(payload);
  assert_eq!(header.entry_len, payload.len() as u32);
  header.verify(payload)?;

  let bytes = header.to_bytes();
  let decoded = WalFrameHeader::decode(&bytes)?;
  assert_eq!(header, decoded);
  assert_eq!(WalFrameHeader::decode_opt(&bytes), Some(header));
  assert_eq!(WalFrameHeader::decode_opt(&bytes[..7]), None);
  assert!(matches!(
    WalFrameHeader::decode(&bytes[..7]),
  ...
  info!("WalFrameHeader 编解码与校验和测试通过");
  OK
}

/// 验证 WalFrameHeader 在各种极端破坏、位反转、截断及溢出情况下的鲁棒性
#[test]
fn test_record_header_corruption_and_boundary_checks() -> Void {
  ...
  assert!(WalFrameHeader::decode(&[0u8; 7]).is_err());
  assert!(WalFrameHeader::decode(&[]).is_err());
  ...
  // 4. 全零头 is_zero 判定
  let zero_hdr = WalFrameHeader::new(0, 0);
  assert!(zero_hdr.is_zero());
  assert_eq!(zero_hdr.payload_len(), 0);

  info!("WalFrameHeader 极端校验与位反转测试通过");
  OK
}
...
#[compio::test]
async fn test_wal_end_to_end_smoke() -> Void {
  let dir = tempdir()?;
  ...
  // 1. 初始化并写入 30 条记录（跨越 2 个段）
  {
    ...
    // 验证提交前可以从内存中完整扫描
    let mem_iter = wal.scan(wal.begin_address(), wal.tail_address());
    let mem_records = collect_iter(mem_iter).await?;
    ...
    // 验证提交后 scan_committed 准确读取（+1 为随批写出的 commit 元数据帧）
    let disk_iter = wal.scan_committed();
    let disk_records = collect_iter(disk_iter).await?;
    assert_eq!(disk_records.len(), 31);
  }
  ...
  // 2. 模拟系统崩溃与重启：重新通过 WalLog::open 恢复
  {
    ...
    let recovered_records = collect_iter(iter).await?;
    assert_eq!(recovered_records.len(), 31);
    ...
    // [append_addr, new_tail) 含数据记录与随批写出的 commit 元数据帧
    let data_records = collect_data(wal.scan(append_addr, new_tail)).await?;
    assert_eq!(data_records.len(), 1);
    assert_eq!(data_records[0].payload, append_payload);
  }
