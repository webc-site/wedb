use aok::{OK, Void};
use log::info;
use waof::{RECORD_HEADER_LEN, WalFrameHeader};
...
#[compio::test]
async fn test_scan_basic_default() -> Void {
  ...
  while let Some(record) = iter.next().await? {
    // commit 元数据帧随批写出，非数据记录不计入数据链断言
    if !support::is_data_record(&record) {
      continue;
    }
    if read_count > 0 {
      assert_eq!(
        record.address, prev_next_addr,
        "数据帧地址须衔接（跳过 commit 帧）"
      );
    }
...
#[compio::test]
async fn test_scan_no_default_subrange() -> Void {
  ...
  let iter = wal.scan(start_addr, end_addr);
  let sub_records = support::collect_iter(iter).await?;
...
#[compio::test]
async fn test_scan_uncommitted_memory_and_commit() -> Void {
  ...
  // 验证 scan 可以直接从内存环形缓冲区读取到未提交记录
  let uncommitted_iter = wal.scan(wal.begin_address(), wal.tail_address());
  let mem_records = support::collect_iter(uncommitted_iter).await?;
  assert_eq!(mem_records.len(), 5);

  // 提交落盘
  wal.commit().await?;

  // 提交后 scan_committed() 即可读取到全部 5 条记录（commit 元数据帧自滤）
  let disk_data = support::collect_data(wal.scan_committed()).await?;
  assert_eq!(disk_data.len(), 5);
...
#[compio::test]
async fn test_scan_behind_begin_address_graceful_jump() -> Void {
...
#[compio::test]
async fn test_scan_graceful_stop_after_physical_truncate() -> Void {
...
#[compio::test]
async fn test_slow_reader_eviction_fallback_to_disk() -> Void {
...
#[compio::test]
async fn test_scan_memory_clobber_falls_back_to_disk() -> Void {
  ...
  let bad_hdr = WalFrameHeader::new(1000, 0xDEADBEEF);
  ...
  for (i, &expected_addr) in addrs.iter().enumerate().take(COUNT) {
    ...
    assert!(support::is_data_record(&rec), "尾批后仅剩 commit 元数据帧");
...
#[compio::test]
async fn test_scan_buffered_large_records() -> Void {
...
#[compio::test]
async fn test_scan_disk_prefetch_flushed_boundary() -> Void {
...
#[compio::test]
async fn test_scan_memory_window_sync() -> Void {
  ...
  // 窗口内全量：与 enqueue 数一致且负载逐条对应（直构 WalRecord，零二次分配）
  let mut rec_count = 0;
  assert!(wal.scan_memory_records(0, u64::MAX, |rec| {
    if waof::is_commit_frame(&rec.payload) {
      return true;
    }
    assert_eq!(rec.payload, make_payload(64, rec_count as u8));
    assert!(rec.next_address > rec.address);
    rec_count += 1;
    true
  }));
  assert_eq!(rec_count, 30);

  // 回调提前终止
  let mut count = 0;
  assert!(wal.scan_memory_records(0, u64::MAX, |_| {
    count += 1;
    count < 5
  }));
  assert_eq!(count, 5);
