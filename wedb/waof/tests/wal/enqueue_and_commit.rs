...
use waof::{COMMIT_FRAME_TOTAL_LEN, Error, RECORD_HEADER_LEN, WalConfig, WalFrameHeader, WalLog};
use wdev::SegmentedDevice;

use super::support::{
  self, ShortWriteDevice, WalFixture, make_pattern_payload, make_payload, reopen_single_file,
};

/// 对标 C# LogTests.cs::EnqueueAndWaitForCommitAsync
/// 异步写入、提交落盘与已提交位点无锁等待。
#[compio::test]
async fn test_enqueue_and_wait_for_commit() -> Void {
  ...
  // commit 元数据帧随批尾写出：提交位点与尾地址均覆盖帧尾
  expected_tail += COMMIT_FRAME_TOTAL_LEN;
  ...
  // 验证追加写入并等待提交持久化
  let extra_payload = b"enqueue_and_wait_for_commit_record";
  let extra_addr = wal.enqueue(extra_payload)?;
  assert_eq!(extra_addr, expected_tail);
  let extra_end = extra_addr + (RECORD_HEADER_LEN + extra_payload.len()) as u64;
  wal.wait_for_commit(extra_end).await?;
  expected_tail += (RECORD_HEADER_LEN + extra_payload.len()) as u64;
  // commit 帧再次随批写出
  expected_tail += COMMIT_FRAME_TOTAL_LEN;
  assert_eq!(wal.committed_until_address(), expected_tail);

  let data_records = support::collect_data(wal.scan_committed()).await?;
  assert_eq!(data_records.len(), 101);

  info!("Enqueue 与 wait_for_commit 异步写入与提交测试通过");
  OK
}
...
#[compio::test]
async fn test_try_enqueue_buffer_full_and_commit_cycle() -> Void {
  ...
  // commit 元数据帧随批写入，非数据记录，不计入
  let all_records = support::collect_data(wal.scan(0, wal.tail_address())).await?;
...
#[compio::test]
async fn test_enqueue_payload_limits_and_config_guards() -> Void {
  let buf_size = 8 * 1024;
...
/// 对标 C# libs/storage/Tsavorite/cs/test/test.hlog/LogFastCommitTests.cs:CommitRecordBoundedGrowthTest
...
fn test_commit_record_bounded_growth_concurrent_enqueue() -> Void {
  ...
  rt.block_on(async {
    ...
    // 扫描全部记录
    // 并发提交各自随批写出 commit 元数据帧，非数据记录，不计入
    let records = support::collect_data(wal.scan(0, final_tail)).await?;
...
fn test_fast_commit_concurrent_waiters_barrier() -> Void {
  ...
  rt.block_on(async {
    ...
    // 扫描校验（commit 元数据帧自滤）
    let data_records = support::collect_data(wal.scan_committed()).await?;
    assert_eq!(data_records.len(), thread_count * ops_per_thread);
...
#[compio::test]
async fn test_commit_short_write_guard() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn test_enqueue_raw_frame_fidelity() -> Void {
  ...
  for i in 0..5u32 {
    ...
    let mut frame = WalFrameHeader::for_payload(&raw_payload)
      .to_bytes()
      .to_vec();
  ...
  // 数据帧在先、commit 元数据帧随提交批尾写出，对齐断言仅看数据帧
  let records = support::collect_data(wal.scan(0, wal.tail_address())).await?;
  ...
  let committed_records = support::collect_data(wal2.scan_committed()).await?;
  ...
  let iter3 = replica.scan(0, replica.tail_address());
  let replayed = support::collect_iter(iter3).await?;
  ...
  let mut torn = WalFrameHeader::for_payload(b"abc").to_bytes().to_vec();
  ...
  let mut oversized =
    WalFrameHeader::new(oversized_len as u32 - RECORD_HEADER_LEN as u32, 0xDEAD_BEEF)
      .to_bytes()
      .to_vec();
