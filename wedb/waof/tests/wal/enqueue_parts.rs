//! 自研依据: WAL 多部件入队字节等价（C# 对应 TsavoriteLog 追加语义 test.hlog/LogTests.cs）
...
async fn assert_scan_frames_identical(
  wal_a: &WalLog<SegmentedDevice>,
  wal_b: &WalLog<SegmentedDevice>,
  from: u64,
) -> Void {
  let records_a = support::collect_iter(wal_a.scan(from, wal_a.tail_address())).await?;
  let records_b = support::collect_iter(wal_b.scan(from, wal_b.tail_address())).await?;
...
#[compio::test]
async fn test_enqueue_parts_byte_equivalence() -> Void {
  let dir = tempdir()?;
  ...
  assert_scan_frames_identical(&wal_whole, &wal_parts, 0).await?;
...
#[compio::test]
async fn test_enqueue_parts_ring_wrap_boundary() -> Void {
  ...
  assert_scan_frames_identical(&wal_whole, &wal_parts, addr_parts).await?;

  let records = support::collect_iter(wal_parts.scan(addr_parts, wal_parts.tail_address())).await?;
  for rec in &records {
...
/// 独立入队逐字节一致（单线程对照）
#[compio::test]
async fn test_enqueue_frames_contiguous() -> Void {
...
#[compio::test]
async fn test_enqueue_frames_oversize_rejected_atomically() -> Void {
  let buf_size = 8 * 1024;
...
#[compio::test]
async fn test_enqueue_frames_group_contiguity_under_concurrency() -> Void {
