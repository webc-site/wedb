...
- `WalLog<D>`: open (open-and-recover), enqueue (lock-free CAS address reservation with in-flight slot registration), commit (batch flush up to safe_tail), enqueue_raw (write a pre-formatted frame verbatim — replica-faithful persistence, cf. C# UnsafeTryEnqueueRaw), wait_for_commit, scan / scan_committed, total_size, recover, truncate, reset
...
- `WalLog<D>`：open（打开并自动 recover）、enqueue（无锁 CAS 预占地址并注册在途槽位）、commit（批量刷盘至 safe_tail）、enqueue_raw（原样写入完整记录帧——复制从节点保真落盘，对标 C# UnsafeTryEnqueueRaw）、wait_for_commit、scan / scan_committed、total_size、recover、truncate、reset
