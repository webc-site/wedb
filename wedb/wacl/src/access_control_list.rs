...
use wresp::RespAclCategories;

use super::{AclPassword, UserHandle, acl_exception::AclError, acl_parser::AclParser, user::User};
...
impl Default for AccessControlList {
  fn default() -> Self {
    Self::new("", None).expect("failed to create default AccessControlList")
  }
}
...
impl AccessControlList {
  ...
  fn import<R: BufRead>(&self, mut input: R, configuration_file: &str) -> Result<(), AclError> {
    ...
    loop {
      ...
      if let Err(exception) = AclParser::parse_acl_rule(trimmed, Some(self)) {
