...
//!
//! 在 garnet 中的相对路径: libs/server/Auth/? 对标 C# 口令哈希（libs/common/HashUtils? ACL 密码域）
...
use wbase::hex::{hex_decode, hex_encode_32};
...
impl AclPassword {
  ...
  ///
  /// C# byte.Parse(HexNumber) 携 AllowLeadingWhite/AllowTrailingWhite，
  /// 二字切片「 0」形 C# 可收、本侧精确查表拒收系有意分叉（deviations
  /// §134，.NET 解析器怪癖族第三形），严禁改用带样式数字解析对齐
...
impl fmt::Display for AclPassword {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    let buf = hex_encode_32(&self.password_hash);
    // 缓冲区全为 ASCII 16 进制码表字符，保证为有效 UTF-8
