...
impl User {
  ...
  /// 别名：从存储载荷解码用户
  #[inline]
  pub fn decode(bytes: &[u8]) -> Result<Arc<Self>, AclError> {
  ...
  pub fn add_category(&mut self, category: RespAclCategories) -> Result<(), AclError> {
    ...
    // 全允许档为无操作
    if self.enabled_commands.is_all() {
      return Ok(());
    }

    // ALL 即整体换为全允许档（对标 C# 的 CommandPermissionSet(all: true) 分支）
    if category == RespAclCategories::ALL {
      self.enabled_commands = CommandPermissionSet::all();
      return Ok(());
    }
  ...
  pub fn remove_category(&mut self, category: RespAclCategories) -> Result<(), AclError> {
    ...
    // 从全拒绝档移除是无操作
    if self.enabled_commands.is_none() {
      return Ok(());
    }
  ...
  pub fn add_custom_command(&mut self, custom_name: &str) -> Result<(), AclError> {
    ...
    // 无操作快路：已允许
    if self.enabled_commands.is_all()
      || (self.enabled_commands.custom_allowed().contains(&normalized)
        && !self.enabled_commands.custom_denied().contains(&normalized))
    {
      return Ok(());
    }
  ...
  /// 口令集合副本
  ///
  /// libs/server/ACL/User.cs:Passwords
  pub fn passwords(&self) -> HashSet<AclPassword> {
    self.copy_password_hashes()
  }
...
mod tests {
  ...
  fn test_user_serialization_passwords_and_custom() {
    ...
    assert!(restored.can_access_custom_command(RespCommand::Customrawstringcmd, "json.set"));
  }
}
