...
use wresp::{
  RespAclCategories, RespCommand,
  catalog::{CmdEntry, children_of, commands_for_category, try_get_resp_command_info},
};

use super::{
  AclPassword, acl_exception::AclError, acl_parser::AclParser,
...
impl User {
  ...
  pub fn validate_password(&self, password: &AclPassword) -> bool {
    ...
    let hashes = self.password_hashes.lock();
    hashes
      .iter()
