...
use wresp::{RespAclCategories, RespCommand, catalog};

use super::{
  AclPassword, UserHandle, access_control_list::AccessControlList, acl_exception::AclError,
  user::User,
};
...
impl AclParser {
  ...
  pub fn parse_acl_rule(
    ...
    // 词法切分与完整性初检（至少 user <username> <op1>）
    let mut tokens = input.split_whitespace();
    let first = tokens.next();
    let username = tokens.next();
    let op1 = tokens.next();
    let (Some(first), Some(username), Some(op1)) = (first, username, op1) else {
    ...
    Self::apply_acl_op_to_user(&user, op1)?;
    for op in tokens {
