...
//!
//! 每节点一张实例（C# 类注释 "An instance per garnet server"，服务级经
//! DatabaseManagerBase.VersionMap => DefaultDatabase.VersionMap 单点暴露）：由引擎宿主
//! 持有，同时注入存储写钩子与各会话事务管理器，多库共用同一张表；按键哈希分桶记录
//! "被监视键"的写入版本，存储会话写入口在完成实际写入后调用
