...
use wbase::entry_type::REPLAY_TASK_ACCESS_VECTOR_BYTES;
...
impl<L: TxnAofLog> TransactionManager<L> {
  ...
  /// 向键集合登记键（自动去重；分字段借用安全）
  #[inline]
  pub fn push_txn_key(txn_keys: &mut Vec<Box<[u8]>>, key: &[u8]) {
    if !txn_keys.iter().any(|k| k.as_ref() == key) {
      txn_keys.push(key.into());
    }
  }

  /// 登记事务涉及的键（自动去重；对标 C# txnKeysParseState）
  #[inline]
