...
/// 进程级共享锁表
static GLOBAL_LOCK_TABLE: LazyLock<TxnLockTable> = LazyLock::new(TxnLockTable::new);
...
pub struct TxnKeyEntries {
  ...
  pub phase: i32,
  /// 已持有的锁守卫（内联 4 槽位，覆盖绝大多数事务，消除堆分配）
...
impl TxnKeyEntries {
  ...
  fn lock_plan(&mut self) -> SmallVec<[LockPlanSlot; 4]> {
    ...
    for entry in &self.keys {
      ...
      plan.push(LockPlanSlot { stripe, exclusive });
    }
    plan
  }
  ...
  fn test_lock_plan_stripes(&mut self) -> Vec<usize> {
    self.lock_plan().iter().map(|slot| slot.stripe).collect()
  }
  ...
  pub fn lock_all_keys(&mut self) {
    ...
    if !plan.is_empty() {
      let lock_table = &*GLOBAL_LOCK_TABLE;
      self.held_locks.extend(
        plan
          .into_iter()
          .map(|slot| lock_table.lock_stripe(slot.stripe, slot.exclusive)),
      );
    ...
    if !plan.is_empty() {
      ...
      for slot in plan {
        let remaining = if lock_timeout > Duration::ZERO {
          lock_timeout.saturating_sub(start.elapsed())
        } else {
          Duration::ZERO
        };
        match lock_table.try_lock_stripe_for(slot.stripe, slot.exclusive, remaining) {
