...
fn save_key_entry_marks_perform_writes_on_exclusive() {
  let mut txn = manager();
  txn.save_key_entry_to_lock(root().as_slice(), b"a", LockType::Shared);
  assert!(!txn.perform_writes);
  txn.save_key_entry_to_lock(root().as_slice(), b"b", LockType::Exclusive);
  assert!(txn.perform_writes);
  ...
  txn.save_key_entry_to_lock(root().as_slice(), b"c", LockType::Shared);
