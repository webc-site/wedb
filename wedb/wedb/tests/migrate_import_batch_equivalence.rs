...
//!    expire_at 头部 coarse 粗化落库值逐字节恒等」「毫秒换算 ticks 恒 16 对齐」
//!    两处折叠论断。
...
#[test]
fn migrate_import_batch_equals_per_record_and_reference() {
...
#[test]
fn migrate_import_replace_false_skips_existing_batch_and_per_record() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
