...
use wedb::server::replication::{
  AofBackpressureGate, aof_replication_pump::AofReplicationPump, aof_sync_driver::AofSyncDriver,
...
fn throttle_loop_publishes_idle_watermark() {
  Runtime::new().unwrap().block_on(async {
    ...
    let store = Arc::new(AofSyncDriverStore::new(1));
    store.attach_backpressure(Some(AofBackpressureGate::Watermark {
      watermark: Arc::clone(&watermark),
      publish_delta: budget / 8,
    }));
    let driver = Arc::new(AofSyncDriver::new(
      "primary-1".to_string(),
      "replica-1".to_string(),
    ...
    assert!(store.try_add_replication_driver(driver.clone(), false));
    assert!(any_stalled(), "副本 attach 后水位收紧应停滞");
    ...
    assert!(!any_stalled(), "水位 9000 追平尾 9000 应放行");
    ...
    assert!(any_stalled(), "水位仍 9000：尾差 4300 > 4096 停滞");
