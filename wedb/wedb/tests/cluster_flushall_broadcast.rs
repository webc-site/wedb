...
/// 换号后新写入正常且旧数据不复活
#[compio::test]
async fn flushall_broadcast_flushes_all_primaries() -> Void {
...
#[compio::test]
async fn flushall_broadcast_unreachable_primary_errors() -> Void {
  let ca = ClusterProvider::new();
  let na = start_flush_node(Arc::clone(&ca), cluster_decorate(Arc::clone(&ca)))?;
  let cb = ClusterProvider::new();
  let nb = start_flush_node(Arc::clone(&cb), cluster_decorate(Arc::clone(&cb)))?;
...
#[compio::test]
async fn flushall_ns_receiver_guards() -> Void {
  let ca = ClusterProvider::new();
  let na = start_flush_node(Arc::clone(&ca), cluster_decorate(Arc::clone(&ca)))?;
...
#[compio::test]
async fn flushall_single_primary_ok_without_peers() -> Void {
  let ca = ClusterProvider::new();
  let na = start_flush_node(Arc::clone(&ca), cluster_decorate(Arc::clone(&ca)))?;
