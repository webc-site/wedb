...
//! 装普通帧 + RI 树装 RangeIndexStream 分块帧 + 向量集经 CLUSTER RESERVE 预留
//! 后装索引/元素帧）→ 真 socket GarnetServer 副本集群会话导入 → 副本端
//! RI.EXISTS/RI.GET 数据在、字符串值与 TTL 保留、向量集索引与元素齐。
