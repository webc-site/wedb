...
fn test_single_key_slot_verify_replica_reads() -> Void {
  ...
  assert!(matches!(res_readonly, ClusterSlotVerificationState::Ok));
  ...
  match res_default {
    ClusterSlotVerificationState::Moved { port, .. } => assert_eq!(port, 7000),
    _ => panic!("Expected Moved to primary for non-readonly session"),
  }
  ...
  match res_write {
    ClusterSlotVerificationState::Moved { port, .. } => assert_eq!(port, 7000),
    _ => panic!("Expected Moved to primary for write on replica"),
  }

  // 4. 内部写会话（AOF 重放）-> 副本本地放行写
  let res_internal = single_key_slot_verify(
    &config,
    slot,
    false,
    SlotVerifySessionState {
  ...
  assert!(matches!(res_internal, ClusterSlotVerificationState::Ok));
