...
use wconn::record::{
  BatchItem, CONTINUATION_FLAG, MigrateVal, MigrationDomainContext, MigrationFrame,
  MigrationRecord, MigrationRecordKind, encode_frame_payload, encode_migration_payload,
...
/// 发送侧守卫：probe_unsupported_keys 分类准确——string 键、Hash/Set/List/
/// ZSet 对象信封键、wbftree 页存储集合键（RangeIndex 元记录与升阶分层集合，
/// 均走带外分块流通道）都可迁移；RangeIndex 信封 / 未知信封内层标签（数据
/// 损坏面）显式「暂不支持」。run_keys_migration_driver 对含暂不支持键的请求
/// 在触达远端前整体拒绝并列明清单，源端键权零变更（不发帧不删除不交权）
#[test]
fn migrate_driver_rejects_object_keys_without_losing_ownership() {
  ...
  rt.block_on(async {
    ...
    // 升阶分层集合键（Hash 升阶入驻 Meta 域元记录，携原集合类型）：本票新
    // 契约下经带外流通道迁移，不再列入预检拒绝清单
    {
...

    {
      ...
      storage
        ...
        .await
        .unwrap();
      // RangeIndex 信封（内层标签 5，损坏数据面：RI 应落 Meta 域，不该在信封）
      storage
      ...
      assert_eq!(
        ...
        "升阶键经 TieredTree 走带外通道不再列入预检拒绝清单，仅未知信封与
         RangeIndex 信封（数据损坏面）显式暂不支持"
      );

      // 纯 string + 可迁移信封 + 升阶键清单预检放行（回归：不误拦）
      assert!(
        probe_unsupported_keys(
          &storage,
          None,
          &[
            b"mig:str".to_vec(),
    ...
    assert!(
      !msg.contains("mig:tiered"),
      "升阶键走带外通道不再进拒绝清单: {msg}"
    );

    // 源端键权零变更：RI 键与升阶键仍在（未发帧、未删除、未交权），string 键未删
    {
      let session = store.new_session().unwrap();
      let batch = session.enter_batch();
      let storage = StorageSession::new_readonly(batch);
    ...
    assert!(
      store
        .new_session()
        .unwrap()
        .load_collection_stub(b"mig:tiered")
        .await
        .unwrap()
        .is_some(),
      "升阶键必须保留源端（分层态元记录未动）"
...
/// replace=T string 经对象信封清退安全覆盖；未知 kind、已分配扩展 kind 的
/// 非法帧体、导槽链上的跨域扩展帧三类违约各自显式拒绝且不落库（文案按
/// kind 分派归位，见用例内次序与归属注释）；纯 string 写入回归不受影响
#[test]
fn cluster_migrate_recv_replace_object_semantics() {
  ...
  //
  // 次序指锚：C# 接收端 CLUSTER MIGRATE 面（ClusterSession 分部
  // libs/cluster/Session/RespClusterMigrateCommands.cs 内 NetworkClusterMigrate
  // 的 Process 局部函数）先读 kind 字节再分派
  // （`var kind = (MigrationRecordSpanType)(*payloadPtr)`），未落入
  // VectorSetElement / SerializedRangeIndexStream / LogRecord 任一臂即走
  // else 抛 InvalidOperationException("Unexpected MigrationRecordSpanType")——
  // 未知 kind 的拒绝只由 kind 本身决定，与帧体内容无关。本仓同一口径：
  // wconn/src/record.rs:parse_migration_payload 先按 kind match 分派，
  // other 臂回 Unsupported migration record kind，帧体校验一律在各 kind
  // 臂内后置（落域上下文帧体长度检查只在 kind=7 臂内，不抢占未知 kind 判定）。
  //
  // 样本取值指锚：kind=7/8 自 agy 轮 7 起为本仓多租户扩展帧占用（落域上下文
  // / DbMeta 映射，C# 单租户单库无此面，见 record.rs 模块文档），已非未知
  // kind。未知样本恒取永不分配的 kind：0（C# 帧类型枚举
  // libs/client/ClientSession/GarnetClientSessionIncremental.cs 内
  // MigrationRecordSpanType.Invalid，显式非记录类型）与 0x80（已分配区间
  // 1..=8 之外），二者不随后续新增帧类型失效。
  //
  // 文案归属（两种违约各归其主，无抢占）：未知 kind 的「Unsupported
  // migration record kind」只由 kind 分发的 other 臂产出；「落域上下文帧体
  // 长度不足」只在 kind=7 臂内对其定长帧体后置校验——与 C# 各臂自读自校验
  // 同构（ChunkedLogRecord 臂自读 chunk 长度头、其余臂先
  // GetSerializedRecordSpan 读体后才是 kind 判定臂）。故接收端「缺域上下文
  // 类」文案永远不可能抢在未知 kind 判定之前：能走到该文案的必先是一个已
  // 分配 kind。dev 基线本用例的红正是样本 kind 过期（7 已分配）而非解码
  // 次序回归，生产码无需改动。（口径差异只在收紧方向：C# 未识别 kind 且
  // 帧体长度头不合法时先读体失败即 return，连应答都不发；本仓 kind 判定
  // 提到读体之前，违约恒有显式应答——与本文件
  // migrate_driver_silent_target_times_out_instead_of_hanging 同面的
  // 「接收端不许静默」要求一致）
  set_slot(&obj_key, SlotState::Importing);
  // 单条记录帧非法载荷构造一处定义：仅 kind 字节可变，未知 kind 与已知
  // 扩展 kind 携非法帧体两种违约共用
  let bad_payload = |kind: u8| {
    let mut v = Vec::new();
    v.extend_from_slice(&1u32.to_le_bytes()); // recordCount = 1
    v.push(kind); // 待判 kind（未知 / 已知扩展两类用法）
    v.extend_from_slice(&(obj_key.len() as u32).to_le_bytes());
    v.extend_from_slice(obj_key.as_bytes());
    v.extend_from_slice(&1u32.to_le_bytes()); // 帧体携值 x：若被静默当 string 写则可见
    v.extend_from_slice(b"x");
    v.extend_from_slice(&0i64.to_le_bytes());
    v
  };
  for kind in [0u8, 0x80] {
    let frame = migrate_recv_frame(b"F", &bad_payload(kind));
    let out = drive(&rt, &mut consumer, &frame);
    let msg = String::from_utf8_lossy(&out);
    assert!(
      msg.starts_with("-ERR Invalid migration payload:")
        && msg.contains(&format!("Unsupported migration record kind {kind}")),
      "非法 kind {kind} 应显式拒绝: {out:?}"
    );
  }

  // 上段次序裁决的反证（dev 基线红现场的原样样本）：同一帧体（键 8B + 值
  // 1B + TTL 8B + 两段长度头 = 25B，短于落域上下文定长 32B）换到已分配
  // kind=7 上，拒绝文案即变为该臂的帧体长度校验——证明两条违约的分支点始
  // 终在 kind 分发一处，帧体校验按臂后置。钉死此口径，后续重构若把域校验
  // 上提为全局前置门（会连带改写上方未知 kind 的应答）即在此处显式失败。
  const FRAME_HEAD_LEN: usize = 4 + 1; // recordCount(4B) + kind(1B)
  let short_ctx_payload = bad_payload(MigrationRecordKind::DomainContext as u8);
  assert!(
    short_ctx_payload.len() - FRAME_HEAD_LEN < MigrationDomainContext::BODY_LEN,
    "样本帧体须短于落域上下文定长，否则本断言退化为别的臂文案"
  );
  let out = drive(
    &rt,
    &mut consumer,
    &migrate_recv_frame(b"F", &short_ctx_payload),
  );
  let msg = String::from_utf8_lossy(&out);
  assert!(
    msg.starts_with("-ERR Invalid migration payload:")
      && msg.contains("落域上下文帧体长度不足 (domain context)"),
    "已分配 kind=7 携短帧体应落本臂帧体校验: {out:?}"
  );

  // 上述三类被拒载荷（未知 kind 0/0x80 与 kind=7 短帧体）的帧体值绝未落库：
  // 目标键仍是前一步 replace=T 迁入的 string
  set_slot(&obj_key, SlotState::Stable);
  assert_eq!(
    drive(
      &rt,
    ...
    b"$21\r\nmigrated_string_value\r\n",
    "未知 kind 不得静默当 string 写入"
  );

  // 扩展帧在导槽链上同样显式拒绝（frame_import.rs accept_domain_frames=false
  // 臂）：CLUSTER MIGRATE 头声明槽位一次性门禁后导入，落域上下文帧会把写入
  // 换到声明槽之外未门禁的域，故整批拒绝而非放行换域（仅无盘全量同步承接面
  // 放行，见 cluster_session/replication.rs:cluster_sync_slow）
  set_slot(&obj_key, SlotState::Importing);
  let ctx_frame = MigrationDomainContext {
    vns: 0,
    vdb: 0,
    ns: 0,
    db: 0,
  ...
  .encode_frame();
  let domain_payload = encode_frame_payload(&[ctx_frame.to_vec()]);
  let out = drive(
    &rt,
    &mut consumer,
    &migrate_recv_frame(b"F", &domain_payload),
  );
  assert!(
    String::from_utf8_lossy(&out).starts_with("-ERR Unexpected domain frame in migration payload"),
    "导槽链应显式拒绝落域上下文帧: {out:?}"
  );
}

/// 目标端 is_importing_slot 接收门语义保留：槽位非 IMPORTING 时
/// CLUSTER MIGRATE 拒收记录并回显式 ERR 判败（C#
/// RespClusterMigrateCommands.cs 置 migrateState 吞错后仍回 +OK，
/// rust 显式 ERR 系修复型分叉，登记条回指 §160）
#[test]
...
/// 假目标端逐帧原文留档（向量集帧载荷含剥域用户键，供用例按键名检索
/// 「发没发」——跨用例并发追加，断言侧只用各用例独有键名过滤，互不污染）
static TARGET_FRAMES: Mutex<Vec<Vec<u8>>> = Mutex::new(Vec::new());

/// 合成 RESERVE 应答的上下文 id 发号器（与真实目标端预留域不重叠即可）
static RESERVE_CTX_ID: AtomicU64 = AtomicU64::new(900_000);

/// CLUSTER RESERVE VECTOR_SET_CONTEXTS 合成应答：按请求数即时产出上下文字符串
/// 数组（脚本 +OK 非合法预留应答，向量集收尾需真实 id 完成重映射）
...
/// 每帧前 3 参记入 seen 供用例断言帧序；CLUSTER RESERVE 合成就地应答不耗脚本
        ...
        loop {
          ...
          while let Some((frame_len, args)) = try_parse_frame_args(&acc) {
            ...
            let head = args
              ...
              .collect::<Vec<_>>()
              .join(" ");
            let raw = acc[..frame_len].to_vec();
            acc.drain(..frame_len);
            TARGET_FRAMES.lock().push(raw);
            seen.lock().push(head);
...
/// wait_dur 三态哨兵映射与 C# `TimeSpan.FromMilliseconds(_timeout)` 基线
/// 一致（r116-migrate1 票一回归）：>0 限时、0 立即超时 Some(ZERO)（快失败
/// 档，不得折成定值）、-1 免超时 None（对标 Timeout.InfiniteTimeSpan，
/// 不得封顶）；其余负值命令解析期拒收、不达本函数
#[test]
fn migrate_wait_dur_sentinel_tri_state() {
  assert_eq!(wait_dur(200), Some(Duration::from_millis(200)));
  assert_eq!(wait_dur(15_000), Some(Duration::from_millis(15_000)));
  assert_eq!(
    wait_dur(0),
    Some(Duration::ZERO),
    "0 档必须映射为立即超时，旧实现折成 15s 定值与本档互斥"
  );
  assert_eq!(
    wait_dur(-1),
    None,
    "-1 档必须映射为免超时（无限等待），旧实现 15s 封顶人为掐断长迁移"
  );
}

/// 0 档（基线快失败语义）：目标端即便逐帧即刻 +OK，迁移也必须首次停等即
/// 判超时快失败（旧实现将 timeout<=0 折成 15s 定值，此脚本下会整链成功，
/// 恰与本档相反）；源端键保留、任务移除
#[test]
fn migrate_zero_timeout_fails_fast_even_with_eager_target() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let store = migrate_store("mt_zero.db");
    let k1 = local_slot_key("mt_z");
    {
      let session = store.new_session().unwrap();
      let batch = session.enter_batch();
      let storage = StorageSession::new_readonly(batch);
    ...
    let seen = Arc::new(Mutex::new(Vec::new()));
    // 全量应答脚本：限时档下足以整链成功；0 档必须不等应答即刻判超时
    ...
    assert!(
      ...
      "0 档必须立即失败，不得挂定值窗: {elapsed:?}"
    );
    // 未确认传输：源端键保留；finally 移除任务
    assert_eq!(read_str(&store, k1.as_bytes()).await, Some(b"v1".to_vec()));
    assert_eq!(
...
/// 正常值档（限时上限）：200ms 限时内应答照常放行、整链成功删键（限时
/// 上限不误伤快目标；同档超时面回归见
/// migrate_driver_silent_target_times_out_instead_of_hanging）
#[test]
fn migrate_positive_timeout_passes_target_within_limit() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let store = migrate_store("mt_pos200.db");
    let k1 = local_slot_key("mt_p");
    {
      let session = store.new_session().unwrap();
      let batch = session.enter_batch();
      let storage = StorageSession::new_readonly(batch);
    ...
    let count = run_keys_migration_driver(
      Arc::clone(&cp),
      Arc::clone(&store),
      migrate_spec(port_of(&addr), 200),
      &slot_set1(),
      &[k1.clone().into_bytes()],
    )
    .await
    .unwrap();
    assert_eq!(count, 1, "200ms 限时内应答应放行完成");
    assert_eq!(read_str(&store, k1.as_bytes()).await, None, "键应迁移删除");
  });
}

/// -1 免超时档（对标 C# Timeout.InfiniteTimeSpan）：目标端批次后挂起，
/// 驱动停等不自发超时（旧实现 15s 封顶误判 poison、触发远端 STABLE 回退
/// 编排人为掐断长迁移）；外部 dispose 触发会话取消令牌后，在途停等即时
/// 收敛（recover → FAIL → 本端槽位回退 → 任务移除），取消联动不丢
#[test]
fn migrate_infinite_timeout_never_self_times_out_and_dispose_converges() {
  use wedb::server::migration::migrate_state::MigrateState;
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let store = migrate_store("mt_inf.db");
    let slot = SLOT0;
    let k1 = key_in_slot("mt_inf_", slot);
    {
      let session = store.new_session().unwrap();
      let batch = session.enter_batch();
      let storage = StorageSession::new_readonly(batch);
    ...
    let seen = Arc::new(Mutex::new(Vec::new()));
    // 脚本：连接1 握手×2 + IMPORTING 应答，批次帧起静默（目标挂起）
    let addr = scripted_migrate_target(vec![vec![b"+OK\r\n"; 3]], Arc::clone(&seen)).await;
    let spec = MigrateTaskSpec {
      transfer_option: TransferOption::Slots,
      ..migrate_spec(port_of(&addr), -1)
    };
    let slots: HashSet<i32> = [slot as i32].into_iter().collect();
    let session = try_add_slots_migration_task(&cp, spec.clone(), &slots).unwrap();
    let handle = spawn(run_slots_migration_task(
      Arc::clone(&store),
      spec,
      Arc::clone(&session),
    ));

    // 推进至批次停等：线上可见非空 CLUSTER MIGRATE 批次帧
    let mut reached = false;
    for _ in 0..20_000 {
      if seen.lock().iter().any(|f| f.starts_with("CLUSTER MIGRATE")) {
        reached = true;
        break;
      }
      yield_now().await;
    }
    assert!(reached, "驱动应下发批次帧进入停等: {:?}", seen.lock());

    // 免超时档不自发超时：静默目标下远超取消轮询节拍仍停在途
    sleep(Duration::from_millis(400)).await;
    assert!(
      !handle.is_finished(),
      "-1 免超时档挂起目标不得自发超时收敛（旧实现 15s 封顶即于此判败回滚）"
    );

    // dispose 触发取消令牌：在途停等即时收敛
    session.dispose();
    let res = timeout(Duration::from_secs(5), handle)
      .await
      .expect("dispose 后免超时停等必须在 5s 内收敛，取消联动不得丢失")
      .expect("驱动任务不得 panic/取消");
    let err = res.expect_err("挂起目标下的迁移必须以失败收口");
    assert!(
      format!("{err}").contains("取消"),
      "收敛错误应为会话取消: {err:?}"
    );
    assert_eq!(
      *session.status.read(),
    ...
    // SLOTS 链 recover 回退本端槽位、任务移除、源端键保留（未确认传输不删）
    let m = cp.cluster_manager().unwrap();
...
fn migrate_driver_sentinel_failure_fails_explicitly() {
  ...
  rt.block_on(async {
    ...
    let seen = Arc::new(Mutex::new(Vec::new()));
    // 脚本：IMPORTING +OK、批次 +OK、哨兵 -ERR、STABLE +OK
    let addr = scripted_migrate_target(
      vec![vec![
        b"+OK\r\n",
        b"+OK\r\n",
...
// ---------------------------------------------------------------------------
// M3 源端生产链（对标 C# ClusterManager.cs / MigrationManager.cs）：SLOTS 驱动全链、
// MIGRATE 命令入口（KEYS 同步 / SLOTS 后台）与解析错误路径
// ---------------------------------------------------------------------------

use wedb::server::migration::migrate_driver::{
  RevivPauseGuard, keys::wait_dur, run_slots_migration_task, try_add_slots_migration_task,
};
...
fn migrate_driver_epoch_quiescence_fences_match_csharp() {
  ...
  rt.block_on(async {
    ...
    let mut spec = migrate_spec(port_of(&addr), 5000);
...
fn slots_migration_registers_unsupported_keys_explicitly() {
  ...
  rt.block_on(async {
    ...
    let slots: HashSet<i32> = [slot as i32].into_iter().collect();
    let session = try_add_slots_migration_task(&cp, spec.clone(), &slots).unwrap();
    let res = run_slots_migration_task(Arc::clone(&store), spec, Arc::clone(&session)).await;
    assert!(
      res.is_err(),
      "槽内含有暂不支持/损坏信封键时槽位迁移必须显式报错中止"
    );

    // string 键未删，RangeIndex 信封键保留源端（源端键权零变更）
    assert_eq!(read_str(&store, k1.as_bytes()).await, Some(b"v1".to_vec()));
    {
      let session = store.new_session().unwrap();
      let batch = session.enter_batch();
...
/// MIGRATE 命令单键形态（Redis `MIGRATE host port key db timeout`）未置
/// MIGRATING → NOTMIGRATING 前置校验拒绝（库级定槽 doc/zh/db.md 4.1：单键
/// 与 KEYS 臂同口径做会话槽位两查并收录 key_slots——漏收录则载荷头槽位列表
/// 为空串，目标端判空拒收 Slot out of range、本端 MIGRATING 门控因空集
/// `.all` 恒真空转）。源端零副作用：键保留、远端未触达、任务未注册
#[test]
fn migrate_command_single_key_without_migrating_state_rejected() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let (mut consumer, store) = migrate_consumer(&cp);
    let k1 = local_slot_key("mc_sk");
    assert_eq!(
      drive(
        &rt,
        &mut consumer,
    ...
    assert_eq!(
      drive(&rt, &mut consumer, &frame),
      b"-ERR slot state not set to MIGRATING state\r\n",
      "单键形态槽位未置 MIGRATING 应回 NOTMIGRATING 文案"
    );
...
/// MIGRATE 命令单键形态端到端（真 TCP → 真目标端消费，模式对标
/// migrate_vector_set_keys_e2e）：手工前置 MIGRATING 后单键迁移 →
/// 载荷头显式携带会话槽位「0」、目标端真实导入落库、源端键删除、
/// 属主不移交（KEYS 链不发 NODE，收口归运维 CLUSTER SETSLOT，对标 C#
/// MigrateKeysAsync 不动槽位）
#[test]
fn migrate_command_single_key_variant_e2e_delivers_slot_header() -> aok::Void {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    // ── 源端装配（node_1 持全槽，SLOT0 为会话库槽位）──
    let cp = two_primary_provider();
    let (mut consumer, store) = migrate_consumer(&cp);
    let k1 = local_slot_key("mc_sk_e2e");
    assert_eq!(
      drive(
        &rt,
        &mut consumer,
    ...
    // ── 目标端装配（node_2 视角：本地持全槽，SLOT0 置 IMPORTING）──
    let target_cp = ClusterProvider::new();
    let target_store = migrate_store("sk_target.db");
    ...
    let mut target_consumer = RespSessionConsumer::with_cluster(
      ...
      Arc::new(StoreGarnetApi::new(target_store.new_session().unwrap())),
    );
    target_consumer
      .attach_transaction_components(Arc::new(WatchVersionMap::new(64)), TxnLockTable::new());

    // ── 桥任务：TCP → 目标端消费（CLUSTER MIGRATE 真处理并留档头槽集；
    // 握手/SETSLOT 直答 +OK，IMPORTING 态已由配置预先排布）──
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap().to_string();
    let port = port_of(&addr);
    retarget_remote_port(&cp, port);
    let slot_lists: Arc<Mutex<Vec<Vec<u8>>>> = Arc::new(Mutex::new(Vec::new()));
    let captured = Arc::clone(&slot_lists);
    spawn(async move {
      ...
      loop {
        ...
        while let Some((_len, args)) = bridge_try_parse(&mut acc) {
          let is_migrate = args.len() >= 2
            && args[0].eq_ignore_ascii_case(b"CLUSTER")
            && args[1].eq_ignore_ascii_case(b"MIGRATE");
          // 头格式 CLUSTER MIGRATE <src> <replace> <slot-list> <payload>：
          // 留档第 5 参槽集供槽位头断言
          if is_migrate {
            captured.lock().push(args[4].clone());
          }
          let frame = bridge_frame(&args);
          let reply: Vec<u8> = if is_migrate {
    ...
    // ── 手工前置 MIGRATING + 单键 MIGRATE ──
    cp.cluster_manager()
      .unwrap()
      .try_prepare_slot_for_migration(SLOT0 as usize, 0x0000_0000_0000_0000_0000_0000_0000_DE12)?;
    let port_text = port.to_string();
    let frame = resp_frame(&[
      b"MIGRATE",
      b"127.0.0.1",
    ...
    assert_eq!(
      drive(&rt, &mut consumer, &frame),
      b"+OK\r\n",
      "单键形态迁移应 +OK（目标端真实接收导入）"
    );

    // 载荷头槽位列表 = 会话槽位「0」（批次帧 + 哨兵帧均携带，非空串）
    {
      let lists = slot_lists.lock();
      assert_eq!(
        lists.len(),
        2,
        "应恰发批次 + 哨兵两帧 CLUSTER MIGRATE: {lists:?}"
      );
      assert!(
        lists.iter().all(|l| l.as_slice() == SLOT0_LIST),
        "载荷头槽集应为会话槽位列表，实得 {lists:?}"
      );
    }
    ...
    assert_eq!(read_str(&store, k1.as_bytes()).await, None, "键应迁移删除");
    assert_eq!(
      read_str(&target_store, k1.as_bytes()).await,
      Some(b"v1".to_vec()),
      "目标端应真实导入键值"
    );

    // 属主不移交：槽保持 Migrating 且归属本端，任务已移除
    let m = cp.cluster_manager().unwrap();
    assert_eq!(
      m.current_config.read().get_state(SLOT0),
      SlotState::Migrating,
      "单键 KEYS 链完成不得移交槽属主"
    );
    assert_eq!(
      m.current_config.read().get_worker_id_from_slot(SLOT0),
      LOCAL_WORKER_ID,
      "槽属主必须仍是本端"
    );
    assert_eq!(
...
/// MIGRATE timeout 哨兵拒收档（r116-migrate1 票一）：-1 与 2 之外的负值
/// （基线 C# 在首个 WaitAsync 抛 ArgumentOutOfRangeException 致迁移运行期
/// 失败）收敛到解析期显式 ERR——零注册、零远端触达；-1/0 两合法哨兵不被
/// 误伤（语义回归见 migrate_wait_dur_sentinel_tri_state 与
/// migrate_zero_timeout_fails_fast_even_with_eager_target）
#[test]
fn migrate_negative_timeout_rejected_at_parse() {
  let rt = Runtime::new().unwrap();
  let cp = two_primary_provider();
  let (mut consumer, _store) = migrate_consumer(&cp);
  ...
  let out = drive(
    &rt,
    &mut consumer,
    &resp_frame(&[b"MIGRATE", b"127.0.0.1", b"7001", b"", b"0", b"-2"]),
  );
  assert!(
    out.starts_with(b"-ERR MIGRATE timeout is invalid"),
    "其余负值应解析期显式拒收: {out:?}"
  );
  let out = drive(
    &rt,
...
fn chunk_payload(chunk: &[u8], more: bool) -> Vec<u8> {
  ...
  payload.push(MigrationRecordKind::Chunked as u8);
...
fn vector_manager_for(_store: &Arc<WedbStore<SegmentedDevice>>) -> Arc<VectorManager> {
...
fn vector_set_discovery_for_slots() -> aok::Void {
  ...
  rt.block_on(async {
    ...
    // 直调 VectorManager 建索引/增元素/导出触达存储回调，不经命令面绑定段，
    // 按新契约自持同步段绑定会话（与生产 RESP 臂同形，参照
    // wnode/tests/vector_store_rmw_pure_read.rs）
    ...
    // 单线程测试体无并发删除者：守卫仅护建索引窗口，try_add 内部自走锁
    // 协议读，提前落守卫即不构成跨 await 持锁
    drop(_lock);
    ...
    // 导出面：元素 + 原生向量 + 属性全集
    let elements = vm.export_migration_elements(&index_value).await;
...
fn migrate_vector_set_keys_e2e() -> aok::Void {
  ...
  rt.block_on(async {
    ...
    // 迁移后源/目标端向量面断言同为直调臂：各按所属存储自持同步段绑定会话
    let src_bind = store.new_session().unwrap();
    let _src_domain = ActiveVectorSessionGuard::bind(&src_bind);
    // 源端键消失（登记表摘除 + 清理登记）
    assert!(
      source_vm
    ...
    drop(_src_domain);
    drop(src_bind);
    let tgt_bind = target_store.new_session().unwrap();
    let _tgt_domain = ActiveVectorSessionGuard::bind(&tgt_bind);
...
fn slots_migration_copy_keeps_source_keys() {
  ...
  rt.block_on(async {
    ...
    let set_key = key_in_slot("st_cpset", slot);
    {
      let session = store.new_session().unwrap();
      let batch = session.enter_batch();
      let storage = StorageSession::new_readonly(batch);
      storage.upsert_string(k1.as_bytes(), b"v1").await.unwrap();
      // Hash 对象信封键（内层标签 0x03）与 Set 对象信封键（0x02）均可迁移
      storage
        .upsert_tag(obj_key.as_bytes(), KeyTag::ObjectEnvelope, b"\x03p")
        .await
        .unwrap();
      storage
        .upsert_tag(set_key.as_bytes(), KeyTag::ObjectEnvelope, b"\x02s")
        .await
        .unwrap();
    }
    ...
    assert_eq!(
      migrated, 3,
      "copy 仅计数确认传输键（string + 对象信封），传输完成计 3"
    );
...
// ---------------------------------------------------------------------------
// r19-migrate 审查票收口：MIGRATE 空键显式拒绝（发现一）、SLOTS 删除环失败
// 收敛（发现二）、接收端 TTL 副作用判错（发现三）
// ---------------------------------------------------------------------------

/// MIGRATE 空键两形态显式拒绝非 +OK（对标 Redis migrateCommand 的 NOKEY）：
/// 键参数空串且无 KEYS/SLOTS 选项（C# NONE 形态）、KEYS 选项零后继键。C# 该
/// 形态 slots=null 直调 TryAddMigrationTask，TryAddMigrateSession 的
/// new MigrateSession 位于 try 块之外（MigrateSessionTaskStore.cs:94/:107），
/// 构造器 GetRanges（MigrateSession.cs:230）对 null _sslots 取 Count 抛 NRE
/// 异常逃逸——绝非 +OK
#[test]
fn migrate_empty_key_forms_rejected_not_ok() {
  let rt = Runtime::new().unwrap();
  let cp = two_primary_provider();
  let (mut consumer, _store) = migrate_consumer(&cp);

  // 形态一：NONE（键参数空串、无任何选项）——不触槽位检查直达拒绝
  let out = drive(
    &rt,
    &mut consumer,
    &resp_frame(&[b"MIGRATE", b"127.0.0.1", b"7001", b"", b"0", b"5000"]),
  );
  assert_eq!(out, b"-NOKEY\r\n", "NONE 形态必须显式拒绝: {out:?}");

  // 形态二：KEYS 零后继键——先置会话槽 MIGRATING 过 IsLocal/IsMigrating 前置门
  let m = cp.cluster_manager().unwrap();
  m.current_config.write().slot_map[SLOT0 as usize] = HashSlot {
    worker_id: LOCAL_WORKER_ID as u16,
...
/// zcode-r157c-migrate 案二帧锁：MIGRATE KEYS 形驱动失败臂（远端批次拒绝）
/// 真协议帧应答逐字节钉 `-IOERR Migrate keys failed\r\n`——rust 单源常量
/// 无句号规范形（`wresp::cmd_strings::cluster::ERR_IOERR`，注册失败臂与
/// 本臂两消费点同引）。C# 该两臂文案自相矛盾：注册失败臂无句号
/// （MigrateCommand.cs:352 经 CmdStrings.cs:80），KEYS 驱动失败臂内联带尾
/// 句号（MigrationDriver.cs:106），系上游一手形，本仓取规范形已登
/// doc/zh/deviations.md 册尾条，严禁按 C# 回改内联散写。失败恢复侧行为与
/// 驱动级族一致：批次未确认源端键保留、recover STABLE 发出、本端
/// MIGRATING 保持（KEYS 链不回退）
#[test]
fn migrate_command_keys_driver_failure_returns_ioerr_frame() {
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let (mut consumer, store) = migrate_consumer(&cp);
    let k1 = local_slot_key("mc_io");
    assert_eq!(
      drive(
        &rt,
        &mut consumer,
    ...
    // 手工前置：键所在槽置 MIGRATING 过解析期门（与成功形夹具同族）
    let m = cp.cluster_manager().unwrap();
    m.try_prepare_slot_for_migration(SLOT0 as usize, 0x0000_0000_0000_0000_0000_0000_0000_DE12)
      .unwrap();
    ...
    assert_eq!(
      out, b"-IOERR Migrate keys failed\r\n",
      "KEYS 驱动失败臂必须回单源无句号规范帧: {out:?}"
    );

    // 失败恢复零副作用：源端键保留、recover STABLE 已发、本端 MIGRATING 保持
    assert_eq!(
      read_str(&store, k1.as_bytes()).await,
      Some(b"v1".to_vec()),
      "批次未确认不得删除源端键"
...
/// zcode-r26-migratetrans 发现一与发现三回归：
/// 1. SLOTS COPY 迁移槽内键数 >64 时，排除集不阻断游标推进，槽尾键完整迁出不截断；
/// 2. 迁移成功后 MigrateSession status 置 MigrateState::Success 终态闭环；
/// 3. MigrateState 变体与协议槽位字串映射一致。
#[test]
fn slots_migration_copy_over_64_keys_does_not_truncate_and_sets_success() {
  use wedb::server::migration::migrate_state::MigrateState;

  // 映射断言
  assert_eq!(MigrateState::Import.as_slot_state_str(), "IMPORTING");
  assert_eq!(MigrateState::Stable.as_slot_state_str(), "STABLE");
  assert_eq!(MigrateState::Node.as_slot_state_str(), "NODE");

  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let store = migrate_store("st_copy_70.db");
    let slot = SLOT0;

    // 写入 70 个键（超过 MAX_MIGRATION_BATCH_COUNT=64）
    {
      let session = store.new_session().unwrap();
      let batch = session.enter_batch();
      let storage = StorageSession::new_readonly(batch);
      for i in 0..70 {
        let key = format!("large_slot_key_{i:03}");
        storage.upsert_string(key.as_bytes(), b"val").await.unwrap();
      }
    }
    ...
    // 初始状态必须为 Pending
    assert_eq!(*session.status.read(), MigrateState::Pending);

    let migrated = run_slots_migration_task(Arc::clone(&store), spec, Arc::clone(&session))
      .await
      .unwrap();

    // 修复前：第一轮取 64 键加进 untouchable，第二轮截断退出只迁 64 键；
    // 修复后：第二轮继续扫描槽尾 6 键，70 键全部迁出！
    assert_eq!(migrated, 70, "超过 64 键时必须完整迁出全部 70 键");

    // 终态闭环：成功收尾后状态必须推进为 Success
    assert_eq!(
      *session.status.read(),
      MigrateState::Success,
      "迁移成功必须推进为 MigrateState::Success"
    );

    // COPY 模式：源端 70 键全部保留且可读
    for i in 0..70 {
      let key = format!("large_slot_key_{i:03}");
      assert_eq!(
        read_str(&store, key.as_bytes()).await,
        Some(b"val".to_vec())
      );
    }
...
// ---------------------------------------------------------------------------
// wedb-slots-migration-vector-set-ns-overreach（r118-triage-dbi1）：SLOTS 向量集
// 收尾判据收窄至「仅默认域 (0,0)」门禁口径——弃 logic_domain_of 逻辑反查，
// 与默认域碰撞同槽的非默认租户域跳过保源并 log::warn 点名留痕（对位 SYNC
// 快照侧「静默跳过必须留痕」纪律），冷域（路由快照未装载）静默漏发臂随
// 反查面拆除消失
// ---------------------------------------------------------------------------
...
/// 留痕检索走 wtest_base 进程级捕获（ctor 双面 logger）：本文件曾以
/// set_boxed_logger 自装捕获，但 wtest_base ctor 先行装配后恒失败且吞错，
/// LOGS 恒空致两测出生红（2026-09-25）
fn warn_logged_since(mark: u64, needle: &str) -> bool {
  wtest_base::log_capture_records_since(mark)
    .iter()
    .any(|(lvl, msg)| *lvl == Level::Warn && msg.contains(needle))
}

/// 字节帧载荷检索（RESP bulk 原文含用户键字节）
fn frame_has(frame: &[u8], needle: &[u8]) -> bool {
  frame.windows(needle.len()).any(|w| w == needle)
}

/// 直调 VectorManager 建向量集（登记表 + 内存索引 + 单元素，建表口径对位
/// vector_set_discovery_for_slots；调用侧须已按目标域绑定 ActiveVectorSessionGuard）
async fn put_vector_set(vm: &VectorManager, prefix: &[u8], key: &[u8]) {
  let params = CreateIndexParams {
    hash_slot: SLOT0,
    dims: 2,
    reduce_dims: 0,
  ...
  assert!(
    matches!(
      vm.try_add(prefix, key, &index_value, &args).await,
      Ok(VectorManagerResult::OK)
    ),
    "夹具向量集建集应成功"
  );
...
/// 碰撞租户域夹具：(ns, db) ≠ (0,0) 且与默认域同槽（slot_of 满射必存，
/// 编译期锁定本二进制取用的具体对）
const COLLIDE_NS: u64 = 1;
const COLLIDE_DB: u64 = 3038;
const _: () = assert!(slot_of(COLLIDE_NS, COLLIDE_DB) == slot_of(0, 0));

/// SLOTS 向量集收尾收窄回归（方案 a：根域物理号 (0,0) 直判）：
/// 默认域自有集正常迁出删源，与默认域碰撞同槽的租户域集跳过保源——
/// 修复前判据走 logic_domain_of 反查现算槽位，碰撞域集被整集发往目标
/// 节点默认域并删源（隔离破口），修复后零帧触达目标且 warn 点名域号
#[test]
fn slots_migration_vector_set_collide_domain_skipped_with_warn() {
  let log_mark = wtest_base::log_capture_mark();
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let store = migrate_store("st_vs_domain.db");
    let vm = vector_manager_for(&store);
    cp.set_store(Arc::clone(&store));
    cp.set_vector_manager(Arc::clone(&vm));

    // 默认域 (0,0) 自有集
    let def_sess = store.new_session().unwrap();
    let _def_domain = ActiveVectorSessionGuard::bind(&def_sess);
    put_vector_set(&vm, SessionPrefixBuf::ROOT.as_slice(), b"vsx:defk").await;

    // 碰撞租户域集：set_context(1, 3038) 解析出的物理域必非 (0,0)
    let col_sess = store.new_session().unwrap();
    assert!(
      col_sess.set_context(COLLIDE_NS, COLLIDE_DB),
      "碰撞域上下文应物化"
    );
    let (cvns, cvdb) = col_sess.virtual_domain();
    assert_ne!((cvns, cvdb), (0, 0), "碰撞域物理号应异于根域");
    let col_prefix = col_sess.session_prefix();
    {
      let _col_domain = ActiveVectorSessionGuard::bind(&col_sess);
      put_vector_set(&vm, col_prefix.as_slice(), b"vsx:colk").await;
    }
    ...
    let slots: HashSet<i32> = [i32::from(SLOT0)].into_iter().collect();
    let session = try_add_slots_migration_task(&cp, spec.clone(), &slots).unwrap();
    let migrated = run_slots_migration_task(Arc::clone(&store), spec, Arc::clone(&session))
      .await
      .unwrap();
    // 仅默认域集计入迁出数（修复前碰撞域集连带计数为 2）
    assert_eq!(migrated, 1, "向量集收尾只应迁出默认域自有集");

    // 目标触达面：默认域集有帧（落目标默认域为既有声明口径），碰撞域集零帧
    //（目标端 ns0/db0 无幻形集的充分前置——帧口径恒为剥域用户键）
    let frames = TARGET_FRAMES.lock();
    assert!(
      frames.iter().any(|f| frame_has(f, b"vsx:defk")),
      "默认域自有集应照常迁出（收窄不误杀），实收帧 {:?}",
      seen.lock()
    );
    assert!(
      !frames.iter().any(|f| frame_has(f, b"vsx:colk")),
      "碰撞租户域集不得发出任何迁移帧（修复前整集错迁目标默认域）"
    );
    drop(frames);

    // 源端收口：默认域集登记摘除，碰撞域集存活
    assert!(
      vm.read_migrated_index(SessionPrefixBuf::ROOT.as_slice(), b"vsx:defk")
        .is_none(),
      "默认域自有集迁移后源端应消失"
    );
    assert!(
      vm.read_migrated_index(col_prefix.as_slice(), b"vsx:colk")
        .is_some(),
      "碰撞域集必须存活源端（修复前被删源且不可恢复）"
    );

    // warn 留痕点名碰撞域物理号（「静默跳过必须留痕」纪律，对位 SYNC 侧形态）
    let needle = format!("vns={cvns} vdb={cvdb}");
    assert!(
      warn_logged_since(log_mark, "SLOTS 迁移向量集登记域为非默认域") && {
        let logs = wtest_base::log_capture_records_since(log_mark);
        logs.iter().any(|(_, msg)| msg.contains(&needle))
      },
      "跳过碰撞域应 warn 点名 {needle}，实收 {:?}",
      wtest_base::log_capture_records_since(log_mark)
    );
  });
}

/// 冷域漏发留痕回归：登记表物理域无逻辑映射（路由快照未装载形态，
/// 修复前 logic_domain_of 返 None 走 is_some_and(false) 无声剔除、零留痕），
/// 修复后判据纯物理直判，该域集同样跳过保源且 warn 点名可观测
#[test]
fn slots_migration_vector_set_unmapped_domain_warns_not_silent() {
  let log_mark = wtest_base::log_capture_mark();
  let rt = Runtime::new().unwrap();
  rt.block_on(async {
    let cp = two_primary_provider();
    let store = migrate_store("st_vs_unmap.db");
    let vm = vector_manager_for(&store);
    cp.set_store(Arc::clone(&store));
    cp.set_vector_manager(Arc::clone(&vm));

    // 未在册物理域 (63, 63)：直设虚域建集，复刻冷租户路由快照未装载态
    let cold_sess = store.new_session().unwrap();
    // 直设携逻辑域=物理对 (63, 63)：未在册死域 version_domain_of 即孤域替身
    cold_sess.set_virtual_context(63, 63, 63, 63);
    let cold_prefix = SessionPrefixBuf::new(63, 63);
    let _cold_domain = ActiveVectorSessionGuard::bind(&cold_sess);
    put_vector_set(&vm, cold_prefix.as_slice(), b"vsx:cold").await;
    ...
    let migrated = run_slots_migration_task(Arc::clone(&store), spec, Arc::clone(&session))
      .await
      .unwrap();
    assert_eq!(migrated, 0, "未在册域集不迁入发送清单");

    assert!(
      !TARGET_FRAMES
        .lock()
        .iter()
        .any(|f| frame_has(f, b"vsx:cold")),
      "未在册域集不得发出迁移帧"
    );
    assert!(
      vm.read_migrated_index(cold_prefix.as_slice(), b"vsx:cold")
        .is_some(),
      "未在册域集应存活源端"
    );
    assert!(
      warn_logged_since(log_mark, "SLOTS 迁移向量集登记域为非默认域") && {
        let logs = wtest_base::log_capture_records_since(log_mark);
        logs.iter().any(|(_, msg)| msg.contains("vns=63 vdb=63"))
      },
      "冷域跳过不得静默，应 warn 点名 vns=63 vdb=63，实收 {:?}",
      wtest_base::log_capture_records_since(log_mark)
    );
  });
}
