...
#[compio::test]
async fn tiered_sync_diskless_replicates_promoted_collections() {
  ...
  let (h_meta, mut h_stub) = target_sess
  ...
  }
  {
    let tree = target_sess
      .acquire_tree_read(b"t:hash", &mut h_stub)
      .await
      .unwrap();
  ...
  // 2) Set 升阶键在副本端齐备
  let (s_meta, mut s_stub) = target_sess
  ...
  {
    let tree = target_sess
      .acquire_tree_read(b"t:set", &mut s_stub)
      .await
      .unwrap();
...
#[compio::test]
async fn tiered_migration_frame_import_and_stream_interrupt() {
  ...
  // 校验发布成果
  let (rmeta, mut rstub) = target_sess
    .load_collection_stub(b"m:hash")
    .await
    .unwrap()
  ...
  {
    let tree = target_sess
      .acquire_tree_read(b"m:hash", &mut rstub)
      .await
      .unwrap();
...
#[compio::test]
async fn tiered_source_delete_cleans_meta_and_stub() {
...
#[compio::test]
async fn tiered_dead_or_malformed_meta_record_is_gone_not_tiered() {
...
/// 同时代表两条链携带升阶集合的传输面。
#[compio::test]
async fn tiered_migration_real_chain_imports_and_empties_source() {
  let source = open_node("mig_src");
  let target = open_node("mig_dst");
  let target_provider = provider_with_role(
    &target,
    REPLICA_ID,
    7102,
    NodeRole::Primary,
  ...
  // 3) 目的端逐键成员与值字节全等 + 键级 TTL 回填
  {
    ...
    {
      let tree = sess.acquire_tree_read(b"m:tier", &mut stub).await.unwrap();
