//! APPENDLOG 拒收断流端到端测试（next/net.md P1-1 / P2-6）
//!
...
fn memory_wire_divergent_appendlog_disconnects() -> Void {
  ...
  // init 帧：+OK 应答，通道保持健康
  assert!(wire.append_log("primary_1", 0, -1, -1, -1, b"").is_ok());
  assert!(wire.is_connected());

  // divergent 记录帧：会话拒收置致命 → sink 投递判失败 → 通道断连
  assert!(
    wire
      .append_log("primary_1", 0, 64, 9000, 9064, b"xyz")
      .is_err()
  );
...
#[test]
fn primary_wire_disconnects_on_stray_error_reply() -> Void {
  Runtime::new().unwrap().block_on(async {
    ...
    wire.append_log("primary_1", 0, 64, 64, 128, b"record")?;
    assert!(
