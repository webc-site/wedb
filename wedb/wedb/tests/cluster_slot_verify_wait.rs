...
use wedb::server::{
  ...
  slot_verify::{IterativeSlotVerifyCache, SlotVerifiedState, SlotVerifySessionState},
...
fn transmitting_write_waits_then_proceeds() {
  Runtime::new().unwrap().block_on(async {
    ...
    let _ = adv.await;
    assert!(!memo.exhausted.load(Ordering::Acquire), "迁移推进后不应超时");
...
fn deleting_read_waits_until_key_removed() {
  Runtime::new().unwrap().block_on(async {
    ...
    let _ = adv.await;
    assert!(!memo.exhausted.load(Ordering::Acquire), "删除完成后不应超时");
...
fn wait_for_stable_slot_waits_until_stable() {
  Runtime::new().unwrap().block_on(async {
    ...
    cm.wait_key_gate(req, memo.clone()).await;
    let _ = adv.await;
    assert!(!memo.exhausted.load(Ordering::Acquire), "槽位稳定后不应超时");
...
/// 用例 7：node-timeout=0（无限哨兵）+ TRANSMITTING 拦写：迭代门评立即
/// Pending，零同步自旋（修复前同步 loop 在 0 哨兵 deadline=u64::MAX 下
/// 永不脱出，且 thread::yield_now 自旋饿死同线程迁移驱动 → 活锁）
#[test]
fn iterative_gate_pending_immediately_under_zero_timeout() {
  Runtime::new().unwrap().block_on(async {
    ...
    // 写命令 TRANSMITTING 拦截：立即 Pending（若仍在同步自旋，本断言在
    // 无限 deadline 下永远执行不到）
    let mut cache = IterativeSlotVerifyCache::default();
    let started = Instant::now();
    let verdict = cm.iterative_slot_verify(&mut cache, key, slot, false, SESSION_DEFAULT, None);
    assert!(
      started.elapsed() < Duration::from_millis(500),
      "迭代门评不得同步自旋"
    );
    assert!(matches!(verdict, IterativeGate::Pending));
    // Pending 非终态：缓存不步进
    ...
    let mut cache = IterativeSlotVerifyCache::default();
    assert!(matches!(
      cm.iterative_slot_verify(&mut cache, key, slot, true, SESSION_DEFAULT, None),
      IterativeGate::Done(true)
    ));
  });
}

/// 用例 8：迭代门评 Pending → 会话登记 wait_key_gate 等待体（同一超时/
/// memo 口径）→ 迁移推进后驱动 → 重驱门评放行（同线程不死锁闭环）
#[test]
fn iterative_pending_parks_waiter_then_serves_after_advance() {
  Runtime::new().unwrap().block_on(async {
    ...
    // Prepare 段逐键迭代校验（写命令被拦）：false + 登记等待体
    assert!(!cs.network_iterative_slot_verify(key, false, false, slot));
    let slow = cs.take_pending_slow().expect("Pending 应登记等待体");
    ...
    // 重驱门评（memo 沿用，未超时）：可访问 + 键存在 → 放行
    assert!(cs.network_iterative_slot_verify(key, false, false, slot));
  });
}

/// 用例 9：迭代等待超时闭环——迁移不推进时等待体置位 exhausted，重驱
/// 门评压制等待点按超时终评（MIGRATING → ASK），不无限挂起
#[test]
fn iterative_wait_timeout_forces_ask_on_re_drive() {
  Runtime::new().unwrap().block_on(async {
    ...
    // 重驱门评：exhausted 压制等待点 → NotOperable → ASK 终评落缓存
    assert!(!cs.network_iterative_slot_verify(key, false, false, slot));
    let mut out = Vec::new();
    cs.write_cached_slot_verification_message(&mut out);
    assert!(
      out.starts_with(b"-ASK "),
      "超时重驱应按 ASK 终评，实得 {:?}",
