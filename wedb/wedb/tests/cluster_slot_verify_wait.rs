...
use wedb::server::cluster_manager_slot_gate::KeyGateParams;
...
fn iterative_gate_pending_immediately_under_zero_timeout() {
  Runtime::new().unwrap().block_on(async {
    ...
    let started = Instant::now();
    let verdict = cm.iterative_slot_verify(
      &mut cache,
      &KeyGateParams {
        key,
        slot,
...
fn iterative_pending_parks_waiter_then_serves_after_advance() {
  Runtime::new().unwrap().block_on(async {
    ...
    assert!(!cs.network_iterative_slot_verify(key, false, 0, false, slot));
