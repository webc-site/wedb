...
/// 分段设备装配变体（段大小显式）：物理删段断言面——单文件设备 truncate
/// 只推进逻辑位点、无段文件可删，物理回收观测须分段形态
fn setup_replica_segmented(max_lag_bytes: i32, segment_size: u64) -> ReplicaFixture {
  setup_replica_impl(
    max_lag_bytes,
    1,
...
/// 装配本体（`segment_size` None = 单文件设备）
fn setup_replica_impl(
  max_lag_bytes: i32,
  replay_task_count: i32,
  replica_sync_timeout_secs: u64,
  ...
  *provider.cluster_manager.write() = Some(cm);
  ...
  let device = match segment_size {
    Some(seg) => Arc::new(
      SegmentedDevice::new(dir.path().join("replica.wal"), seg, DEFAULT_SECTOR_SIZE)
        .expect("device"),
    ),
    None => Arc::new(SegmentedDevice::single_file(dir.path().join("replica.wal")).expect("device")),
  };
...
#[compio::test]
async fn background_replay_applies_records_and_catches_up() -> aok::Void {
  ...
  // 断链处置：dispose 仅终结会话自持代的驱动仓（排空驱动、停背景重放并
  // 关闭该代容器，对标 C# ClusterSession.Dispose 的自持引用 Dispose）；
  // 重连须先由 recovery 面换代（reset = 旧实例 dispose + new 新实例，
  // 对标 C# ResetReplicaReplayDriverStore），init 帧在新一代重注册成功
  fixture.session.dispose();
  assert!(
    !driver.background_replay_started(),
    "背景重放任务随断链处置终止"
  );
  assert!(!driver.is_active(), "旧代驱动随自持仓处置转非活跃");
  // 旧连接迟到的二次 dispose：命中已处置旧实例，幂等空转不误伤新代
  fixture.session.dispose();
...
#[compio::test]
async fn throttle_primary_blocks_until_caught_up() -> aok::Void {
...
#[compio::test]
async fn advance_time_pulse_applies_when_caught_up() -> aok::Void {
...
#[compio::test]
async fn advance_time_pulse_advances_every_virtual_sublog() -> aok::Void {
  let fixture = setup_replica_with(-1, 2, DEFAULT_SYNC_TIMEOUT_SECS);
  let rcm = fixture
    .aof
    .read_consistency_manager()
...
#[compio::test]
async fn replica_consistent_read_times_out_when_watermark_lags() -> aok::Void {
...
#[compio::test]
async fn primary_role_consistent_read_passes_without_wait() -> aok::Void {
  let fixture = setup_replica_with(-1, 2, MIN_SYNC_TIMEOUT_SECS);
  let rcm = fixture
    .aof
    .read_consistency_manager()
...
/// 源日志（主端形态）随批 commit 帧产出：入队锚记录 + 置 cookie + 提交
///（主端 WalCommitStep 随批写帧的唯一真身路径），先截断推进 begin 使帧内
/// begin 快照非零，再扫描取出最后一条 commit 元数据帧负载与 begin 快照值
///（pump 保真推流的帧负载口径；负载 24B = MAGIC + begin + cookie）
async fn source_commit_frame(cookie: i64) -> (Vec<u8>, u64) {
  let dir = tempfile::tempdir().expect("源日志目录");
  let device = Arc::new(SegmentedDevice::single_file(dir.path().join("src.log")).expect("device"));
  let wal = Arc::new(WalLog::new(device, WalConfig::default()).expect("wal"));

  // 先落一批并截断，令后续 commit 帧携带非零 begin 快照（主端截断后写帧的
  // 常态形态，快照值可观测）
  wal.enqueue(b"anchor-before-truncate").expect("入队");
  let begin_snapshot = wal.enqueue(b"anchor-at-new-begin").expect("入队");
  wal.set_pending_cookie(1);
  wal.commit().await.expect("主端首次提交");
  wal
    .truncate(begin_snapshot)
    .await
    .expect("源端截断推进 begin");
  assert_eq!(wal.begin_address(), begin_snapshot);

  // 随批 commit 帧（目标 cookie）写出
  wal.enqueue(b"commit-anchor-record").expect("入队");
  wal.set_pending_cookie(cookie);
  wal.commit().await.expect("主端提交（随批帧写出）");

  let mut payload = None;
  wal.scan_memory_records(0, wal.tail_address(), |rec| {
    if waof::is_commit_frame(&rec.payload) {
      payload = Some(rec.payload.to_vec());
    }
    true
  });
  (
    payload.expect("主端随批 commit 帧在场"),
    wal.begin_address(),
  )
}

/// 副本回放拦截主端 commit 元数据帧并对齐本地提交边界（危害链回归）
///
/// 对标 Garnet C#：ReplicaReplayDriver.cs:ConsumeDirect 的 payloadLength < 0
/// 分支（TsavoriteLogRecoveryInfo.Initialize + physicalSublog.
/// UnsafeCommitMetadataOnly，TsavoriteLog.cs:758）。
///
/// 修复前红相：回放链对 commit 帧仅跳过，副本本地 committed_until_address
/// 恒滞留构造初值 0——truncate 的 safe_until 恒 0（物理删段永久失效、段文件
/// 无限积压）、get_committed_replication_offset 观测恒零值（故障转移把陈旧
/// 零位点写入 replication.toml）、scan_committed 恒空区间。
///
/// 编排：主端形态源日志产出 8 条数据帧（值 700B，帧流跨越段 0/1 边界）与
/// 一条随批 commit 帧（cookie = 42，begin 快照非零）→ 会话逐帧保真落盘 →
/// 背景重放拦截 commit 帧对齐本地提交边界 → 断言提交上界/cookie/begin 快照
/// 三面同步前移，数据条目照常应用进存储，truncate 不再被 0 钳死、旧段文件
/// 物理删除。
#[compio::test]
async fn replay_commit_frame_aligns_local_committed_boundary() -> aok::Void {
  let fixture = setup_replica_segmented(-1, 4096);

  // 主端数据帧逐帧推流（首帧带初始化握手）
  let value = vec![b'v'; 700];
  for i in 0..8 {
    let key = format!("ck{i}").into_bytes();
  ...
  // 主端随批 commit 帧推流（帧尾即副本应对齐的提交上界）
  let (commit_payload, begin_snapshot) = source_commit_frame(42).await;
  let frame = record_frame(&commit_payload);
  let current = fixture.wal.tail_address() as i64;
  let commit_next = current + frame.len() as i64;
  let outcome = fixture
    .session
    .process_append_log(PRIMARY_ID, 0, current, current, commit_next, &frame)
    .expect("commit 帧推流");
  assert_eq!(outcome, AppendLogOutcome::Record);

  // applied 位点越过 commit 帧尾（对齐发生在位点发布之前，自此断言无竞态）
  assert!(
    wait_for(
      || fixture.rm.get_replication_offset(0) >= commit_next,
      Duration::from_secs(5),
    )
    .await,
    "applied 位点越过 commit 帧"
  );

  // 核心断言：副本本地提交边界三面前移对齐主端（修复前恒滞留 0 / NO_COOKIE）
  assert_eq!(
    fixture.wal.committed_until_address() as i64,
    commit_next,
    "提交上界对齐主端帧位"
  );
  assert_eq!(fixture.wal.recovered_cookie(), 42, "cookie 回填");
  let sublog = fixture.aof.log().get_sub_log(0);
  assert_eq!(
    sublog.recovered_cookie(),
    Some(42),
    "子日志 cookie 标量同步"
  );
  assert_eq!(
    sublog.committed_begin_address(),
    begin_snapshot as i64,
    "committed_begin 对齐主端帧内快照"
  );
  assert_eq!(
    fixture.wal.begin_address(),
    begin_snapshot,
    "begin 逻辑栅栏推进至主端快照"
  );

  // 数据条目照常应用进存储（commit 帧拦截不碍数据回放）
  assert_eq!(
    read_string(&fixture.store, b"ck0").await.as_deref(),
    Some(value.as_slice()),
    "首条数据帧已应用"
  );
  assert_eq!(
    read_string(&fixture.store, b"ck7").await.as_deref(),
    Some(value.as_slice()),
    "末条数据帧已应用"
  );

  // truncate 物理删段生效（修复前 safe_until 恒 0，段文件永存）
  fixture
    .wal
    .truncate(commit_next as u64)
    .await
    .expect("truncate");
  assert_eq!(fixture.wal.begin_address(), commit_next as u64);
  assert!(
    !fixture.wal.device().segment_path(0).exists(),
    "段 0 已物理删除"
  );
  assert!(
    !fixture.wal.device().segment_path(1).exists(),
    "段 1 已物理删除"
  );
  assert!(fixture.wal.device().segment_path(2).exists(), "段 2 保留");
  OK
}

struct ReplaySessionProvider(ClusterReplicationSession<SegmentedDevice>);

impl SessionProviderFace for ReplaySessionProvider {
...
/// AOF 帧 CRC 失配对接 divergent 断流重同步收敛通路：
/// 1. 副本接收并落盘携带错误 CRC32 的记录帧；
/// 2. 背景重放扫描校验失败检出 ChecksumMismatch；
/// 3. 重放任务终止，调用 reset_replica_replay_driver_store 触发 divergent 收敛：
///    - 驱动仓被 dispose，活跃流判定复位（has_active_replication_stream 为 false）；
///    - 后续记录帧被拒收并产生致命断流信号（take_fatal_disconnect 为 true）；
///    - 破除在同步假象，确保 ensure_replication 具备重同步触发条件。
#[compio::test]
async fn crc_mismatch_triggers_divergent_disconnect_and_resync() -> aok::Void {
  let mut fixture = setup_replica(-1);
  let valid_entry = source_entries(b"valid_key", b"valid_val");
  let corrupt_entry = source_entries(b"corrupt_key", b"corrupt_val");

  // 1. 初始化并推流首条合法记录，验证应用链与流状态正常
  let next_valid = attach_and_push(&fixture, &valid_entry);
  assert!(
    wait_for(
      || fixture.rm.get_replication_offset(0) >= next_valid,
      Duration::from_secs(5),
    )
    .await,
    "合法记录正常回放追平"
  ...
  // 2. 构造并推流 CRC 失配帧（wal 保真落盘，不阻断网络推流入队）
  let corrupt_frame = {
    let header = WalFrameHeader::new(corrupt_entry.len() as u32, 0xDEAD_BEEF);
    let mut frame = header.to_bytes().to_vec();
    frame.extend_from_slice(&corrupt_entry);
    frame
  };
  let current = fixture.wal.tail_address() as i64;
  let next_corrupt = current + corrupt_frame.len() as i64;
  let outcome = fixture
    .session
    .process_append_log(
      PRIMARY_ID,
  ...
  // 3. 背景重放任务扫描到失配帧，触发 divergent 收敛：
  //    reset_replica_replay_driver_store 使活跃复制流状态即刻复位
  assert!(
    wait_for(
      || !fixture.rm.has_active_replication_stream(),
      Duration::from_secs(5),
    ...
    .await,
    "CRC 失配后驱动仓必须被处置，has_active_replication_stream 转为 false"
  );
  ...
  let driver = fixture
    ...
    .current_replica_replay_driver_store()
    .get_replay_driver(0);
  assert!(driver.is_none(), "新一代驱动仓为空，旧代驱动已注销");

  // 5. 损坏记录未被应用进存储
  assert_eq!(
    read_string(&fixture.store, b"corrupt_key").await,
    None,
    "损坏记录不得应用进存储"
  );

  // 6. 主端后续推流必须命中 divergent 拒收（避免损坏流继续写入本地 WAL）
  let subsequent_entry = source_entries(b"sub_key", b"sub_val");
  let subsequent_frame = record_frame(&subsequent_entry);
  let sub_current = fixture.wal.tail_address() as i64;
  let sub_next = sub_current + subsequent_frame.len() as i64;
  let err = fixture
    .session
    .process_append_log(
      PRIMARY_ID,
  ...
  assert_eq!(
    err.kind(),
    ErrorKind::NotFound,
    "后续帧因驱动仓已处置（驱动缺席）必须返回 NotFound 拒收错误"
  );

  // 7. 会话必须登记致命断连信号（使网络泵主动断开 TCP 连接）
  assert!(
    fixture.session.take_fatal_disconnect(),
    "会话必须登记致命断连信号以关闭网络连接"
  );

  // 8. 活跃流已复位，具备重同步触发条件（确保 ensure_replication 触发 resync）
  assert!(
    !fixture.rm.has_active_replication_stream(),
    "破除在同步假象，具备重同步触发条件"
  );
...
/// CRC 失配帧导致真 TCP 连接断流：
/// GarnetServer 托管副本会话，主端经 TcpSessionWire 发送包含 CRC 损坏的帧，
/// 随后发送下一帧时副本因驱动仓已处置（divergent 收敛）断开 TCP 连接，
/// 主端感知断连（!wire.is_connected()），触发重同步收敛。
#[test]
fn primary_wire_crc_mismatch_disconnects() -> aok::Void {
  let fixture = setup_replica(-1);
  let server = GarnetServer::new(
    &["127.0.0.1:0".to_string()],
    65536,
    100,
    Arc::new(ReplaySessionProvider(fixture.session.clone())),
  )?;
  ...
  Runtime::new()?.block_on(async {
    ...
    .await?;
    assert!(wire.is_connected(), "建连后通道健康");

    // 推流合法记录，验证复制正常运行
    let valid_entry = source_entries(b"wire_k1", b"wire_v1");
    let valid_frame = record_frame(&valid_entry);
    let cur = fixture.wal.tail_address() as i64;
    let nxt = cur + valid_frame.len() as i64;
    wire.append_log(PRIMARY_ID, 0, cur, cur, nxt, &valid_frame)?;
    assert!(
      wait_for(
        || fixture.rm.get_replication_offset(0) >= nxt,
        Duration::from_secs(5),
      )
      .await,
      "合法记录回放追平"
    ...
    // 发送 CRC 损坏的记录帧
    let corrupt_entry = source_entries(b"wire_k2", b"wire_v2");
    let corrupt_frame = {
      let header = WalFrameHeader::new(corrupt_entry.len() as u32, 0xBAAD_F00D);
      let mut f = header.to_bytes().to_vec();
      f.extend_from_slice(&corrupt_entry);
      f
    };
    let cur2 = fixture.wal.tail_address() as i64;
    let nxt2 = cur2 + corrupt_frame.len() as i64;
    wire.append_log(PRIMARY_ID, 0, cur2, cur2, nxt2, &corrupt_frame)?;

    // 等待重放任务检出 CRC 错误并处置驱动仓
    assert!(
      wait_for(
        || !fixture.rm.has_active_replication_stream(),
        Duration::from_secs(5),
    ...
    // 主端发送下一帧，副本拒收并主动断流
    let sub_entry = source_entries(b"wire_k3", b"wire_v3");
    let sub_frame = record_frame(&sub_entry);
    let cur3 = fixture.wal.tail_address() as i64;
    let nxt3 = cur3 + sub_frame.len() as i64;
    let _ = wire.append_log(PRIMARY_ID, 0, cur3, cur3, nxt3, &sub_frame);

    // 主端感知连接断开
    assert!(
      wait_for(|| !wire.is_connected(), Duration::from_secs(5)).await,
      "CRC 失配后主端通道必须转断连态（触发重同步）"
    );
