...
use std::{net::hostname, path::Path, sync::Arc};
...
use wnode::{ClusterSessionFace, ClusterSlotVerificationInput, SlotVerifyGate};
...
fn empty_announce_falls_back_to_os_hostname_once() {
  ...
  // 与 std::net::hostname() 单源探测一致
  if let Ok(raw) = hostname() {
    assert_eq!(host, raw.to_string_lossy(), "回退值应等于 OS 主机名");
  }
}
...
fn moved_redirect_emits_announced_hostname() {
  ...
  *a_cm.current_config.write() = merged;
  ...
  // 迭代校验族随过程域清退，重定向改由活路多键门入口 network_multi_key_slot_verify
  // 判定并直写 output（对标 C# NetworkMultiKeySlotVerify 有应答臂渲染单点）
  ...
  assert!(
    matches!(
      cs.network_multi_key_slot_verify(&input, &[b"key"], &mut out),
