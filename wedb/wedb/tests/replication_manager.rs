...
use wedb::server::{
  replication::{
    CLUSTER_SUBDIR, REPLICATION_STATE_FILE,
...
use wnode::database::checkpoint_version;
...
/// recover=true 且 replication.toml 非空 → replid 历史跨重启保留（修复前
/// 生产装配无持久化目录，主端重启丢历史、副本被迫全量重同步）；
/// recover=false → 初始化新历史并覆盖旧文件（对标 InitializeReplicationHistory
/// 尾段 FlushConfig）；空目录 recover=true → 新历史落盘 replication.toml
#[test]
fn test_with_options_replication_history_gate() {
  let dir = tempfile::tempdir().expect("tempdir");
  let config_dir = dir.path().join(CLUSTER_SUBDIR);
  ...
  // 分支 3：空目录 + recover=true → 初始化新历史并落盘 replication.toml
  ...
  assert!(
    empty_dir.join(REPLICATION_STATE_FILE).exists(),
    "初始化须落盘 replication.toml"
  );
}
...
fn test_concurrent_history_flush_keeps_single_complete_version() {
  ...
  let path = config_dir.join(REPLICATION_STATE_FILE);
...
/// 主库 AOF 截断下界预检：同历史同版本（skipLocalMainStoreCheckpoint 为真）
/// 但主库截断线/日志起点越过从库尾位点时，增量接续的物理区间已不存在，
/// 须降级 FullResync（C# 把同一断层的暴露面放在挂载期 DataLossCheck /
/// TryAddReplicationDriver 拒绝，从库只剩节流重连死循环；rust 协商期预检）
#[test]
fn test_disk_resync_strategy_truncated_aof_falls_back_to_full() {
  let committed = AofAddress::create(2, 1000);

  // 基线：无截断（truncated_until=0、primary_begin=200 < rep_tail）→ Partial
  let mgr = ReplicationManager::with_options(2, None, false);
  let entry = disk_primary_entry(&mgr, 10, 500);
  let primary_begin = AofAddress::create(2, 200);
  let sync_meta = SyncMetadata {
  ...
  // 1. 边界相等：截断线 == 从库尾位点，[tail, tail) 为空区间，仍可增量接续
  //    （update_truncated_until 单调取大，场景按 800 → 900 单调推进编排）
  mgr
    .aof_sync_driver_store
    .update_truncated_until(&AofAddress::create(2, 800));
  assert!(matches!(
    negotiate(&sync_meta),
    ResyncStrategy::PartialResync { .. }
  ));

  // 2. 主库 SafeTruncateAof 推进截断线越过从库尾位点 → 增量区间物理丢失 →
  //    FullResync（修复前此处误判 Partial，挂载期 try_add_replication_driver
  //    以 start_address < truncated 拒绝注册，从库节流重连死循环）
  mgr
    .aof_sync_driver_store
    .update_truncated_until(&AofAddress::create(2, 900));
  assert!(matches!(
    negotiate(&sync_meta),
    ResyncStrategy::FullResync { .. }
  ));

  // 3. FastAofTruncate 模式豁免（口径同 ckpt_begin 判定：允许 unsafe attach 直推）
  let negotiate_fast =
    |m: &SyncMetadata| mgr.disk_resync_strategy(m, &committed, &primary_begin, true);
  assert!(matches!(
    negotiate_fast(&sync_meta),
    ResyncStrategy::PartialResync { .. }
  ));

  // 4. 物理日志起点维度：primary_begin 越过从库尾位点（截断线未推进）同判 Full
  let mgr2 = ReplicationManager::with_options(2, None, false);
  let entry2 = disk_primary_entry(&mgr2, 10, 500);
  let high_begin = AofAddress::create(2, 900);
  let negotiate_high =
    |m: &SyncMetadata| mgr2.disk_resync_strategy(m, &committed, &high_begin, false);
  let sync_meta2 = SyncMetadata {
    checkpoint_entry: Some(entry2),
    origin_node_id: 0x2E82,
    current_primary_repl_id: mgr2.primary_repl_id(),
    ..sync_meta.clone()
  };
  assert!(matches!(
    negotiate_high(&sync_meta2),
    ResyncStrategy::FullResync { .. }
  ));
}

/// 无盘臂判据：测试 [`ReplicationManager::diskless_resync_strategy`] 的
/// NeedToFullSync 三条件（历史、store 版本 !=、副本 AOF 尾位点越界；第 4 条
/// 回放量门限在 rust 配置面缺席），方向是 != 而非存在性判据
/// （C# NeedToFullSync 锚由 diskless_resync_strategy 单点持有，测试不重复挂锚）
#[test]
...
fn test_get_latest_checkpoint_from_disk_info() {
  ...
  let mut meta = CheckpointMeta {
    ...
    checkpoint_aof_address: Some(vec![0x4000]),
...
/// 盘上最新检查点条目组装（对标 C#
/// ReplicationCheckpointManagement.cs:GetLatestCheckpointEntryFromDisk——
/// 副本重连上报（assembly::recover_replication）经此读盘取数：重启副本
/// 内存仓为空也可凭盘上快照上报，主端判同检查点历史走 PartialResync）：
/// version 由 token 派生、hlog/index token 同 token、covered 取快照 AOF 边界、
/// repl id 取当前主复制 ID；initialize_checkpoint_store 以同一组装口径
/// seed 内存仓；未注入目录 / 空目录 / 损坏元数据返回 None
#[test]
fn test_get_latest_checkpoint_entry_from_disk() {
  let mgr = ReplicationManager::new();

  // 未注入目录：无盘上快照
  assert!(mgr.get_latest_checkpoint_entry_from_disk().is_none());
  ...
  write(dir.path().join(meta_filename(token)), meta.encode()).expect("write meta");

  let entry = mgr
    .get_latest_checkpoint_entry_from_disk()
    .expect("disk entry");
  assert_eq!(entry.metadata.store_version, checkpoint_version(token));
  assert_eq!(entry.metadata.store_hlog_token, token);
  assert_eq!(entry.metadata.store_index_token, token);
  assert_eq!(
    entry.metadata.store_checkpoint_covered_aof_address,
    AofAddress::create(mgr.sublog_count() as i32, 0x4000)
  );
  assert_eq!(
    entry.metadata.store_primary_repl_id,
    Some(mgr.primary_repl_id())
  );

  // 同一组装口径接通内存仓（C# Initialize：seed 后内存仓可取到同 token 条目）
  assert!(mgr.initialize_checkpoint_store());
  let seeded = mgr
    .checkpoint_store
    .read()
    .latest_entry()
    .expect("seeded entry");
  assert_eq!(seeded.metadata.store_hlog_token, token);

  // 元数据损坏：解码失败返回 None（attach 上报随之折算空条目）
  let corrupt = tempfile::tempdir().expect("tempdir");
  mgr.set_checkpoint_dir(corrupt.path().to_path_buf());
  write(corrupt.path().join(meta_filename(1)), b"junk").expect("write junk");
  assert!(mgr.get_latest_checkpoint_entry_from_disk().is_none());
