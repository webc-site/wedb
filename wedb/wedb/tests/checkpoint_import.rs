...
use std::{fs, num::NonZeroUsize, ops::Deref, path::PathBuf, sync::Arc, time::Duration};

use cluster_cc::cluster_consumer;
use common::{provider_with_role, replica_host};
...
/// 独立存储节点（公共核 db+wal 加本地检查点目录薄包装）
struct NodeStorage {
  base: common::NodeStorage,
...
/// 副本角色 provider（公共核装配 + 检查点目录接线；不调攒批窗 setter，
/// 保本文件 provider 原生形态——diskless_sync_delay=None）
fn replica_provider(node: &NodeStorage) -> Arc<ClusterProvider> {
  let provider = provider_with_role(
    node,
    REPLICA_ID,
    7001,
    NodeRole::Replica,
...
/// 主端角色 provider（全槽自持 + 检查点目录接线，同上不经攒批窗 setter）
...
#[compio::test]
async fn checkpoint_import_end_to_end_over_socket() {
  // ===== 主端：建库 + 快照 + 复制域登记 + 推流资产
  let primary = open_node("primary");
  ...
  let assets = Arc::new(primary_assets(&primary, &rm_p));
...
#[compio::test]
async fn snapshot_data_client_requires_connection() {
