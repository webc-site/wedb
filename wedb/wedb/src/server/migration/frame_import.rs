    ...
    if matches!(
      ...
      match frame {
        MigrationFrame::DomainContext(ctx) => {
          session.set_virtual_context(ctx.vns, ctx.vdb);
    ...
    let should_write = replace || {
      ...
      let alive = match probe_alive_with_prefix(&storage.batch, prefix, record.key()) {
        ...
        Ok(None) | Err(_) => storage
          .probe_alive_domain_async_with_prefix(prefix, record.key())
          .await
          .ok()
          .flatten()
          .is_some(),
    ...
    if should_write {
      let write_res = match &record {
        ...
        MigrationRecord::Env { key, env, .. } => {
          ...
          // 源键 TTL 由下方 TTL 回填（清退失败罕见磁盘候选，与回填同款
          // 容错忽略）
      ...
      if expire_unix_ms > 0 {
        ...
        // 毫秒换算产物恒为 16 对齐，与 expire_at 头部 coarse 粗化落库值
        // 逐字节恒等）
