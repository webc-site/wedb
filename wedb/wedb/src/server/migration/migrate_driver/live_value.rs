...
/// 迁移探针读窗留钩（一次性，[`wkv::TEST_COLD_WINDOW_HOOK`] 同族）：
/// [`read_live_value`] 单键探针内值读物化之后、TTL 读之前回调，供
/// 「换号族介入两独立 await 之间」定向用例定序注入 FLUSHDB/SWAPDB/纯换代
/// （工单 wedb-migrate-cross-generation-ttl-ghost 测试锁 1/2/3）。生产路径
/// 钩槽恒 None，仅一次无争锁读判空；业务代码禁止触碰
#[doc(hidden)]
pub static TEST_LIVE_VALUE_READ_HOOK: Mutex<Option<Box<dyn FnOnce() + Send>>> = Mutex::new(None);
...
/// 登记表域判定。
///
/// 跨代 TTL 幽灵防护（deviations §96 既定加固路 / §118 主案）：C# Reader 为
/// 整记录原子读（expiration 随 RecordDataHeader 同读，无两读窗），rust 值读
/// 与 TTL 读系两独立 await，换代（FLUSHDB/SWAPDB 的 bump_generation）落两读
/// 之间时逐 op 现解析的会话前缀会把 TTL 读改指新代域，拼出「值在旧代、TTL
/// 在新代」异代帧——本件因此为**单探针窗**：每键探针头部单次捕获
/// `session_prefix()`，string/信封两臂值读与 TTL 读并读该前缀
/// （[`wkv::StoreSession::raw_ttl_with_prefix`]），前缀捕获一次即换代无感，
/// 值与 TTL 恒同代同域。TieredTree/向量臂 TTL 随流元携载不经本窗，不入改动面。
pub async fn read_live_value<D: Device>(
  storage: &StorageSession<'_, D>,
  vm: Option<&VectorManager>,
  key: &[u8],
) -> Result<LiveValue> {
  // 探针窗首单次捕获会话物理前缀（值读与 TTL 读共用，见函数头跨代防护注）
  let prefix = storage.batch.session_prefix();
  let prefix = prefix.as_slice();
  // 1. string 域（命中即活值；过期键域内不可见）
  if let Some(val) = storage
    .read_tag_with_prefix(prefix, key, KeyTag::String, |v| v.to_vec())
    .await?
  {
    fire_live_value_read_hook();
    return Ok(LiveValue::Migratable(
      MigrateVal::Str(val),
      expire_unix_ms(storage, prefix, key).await?,
    ));
  }
  // 2. 对象信封域：闭包只读判标签，仅合规类型物化整值
  if let Some((tag, env_opt)) = storage
    .read_tag_with_prefix(prefix, key, KeyTag::ObjectEnvelope, |raw| {
...
/// 探针读窗留钩消费点（一次性 take，[`read_live_value`] 两臂值读物化后、
/// TTL 读前各挂一处；生产恒 None 一次判空零负担）
#[inline]
fn fire_live_value_read_hook() {
  if let Some(hook) = TEST_LIVE_VALUE_READ_HOOK.lock().take() {
    hook();
  }
}
...
///
/// 显式前缀读取（[`wkv::StoreSession::raw_ttl_with_prefix`]）：与值读共用
/// 探针窗头单次捕获的会话物理前缀，换代对本探针无感，杜绝两读异代拼接
/// （跨代 TTL 幽灵主案修，见 [`read_live_value`] 函数头注与 deviations §118）
async fn expire_unix_ms<D: Device>(
  storage: &StorageSession<'_, D>,
  prefix: &[u8],
  key: &[u8],
) -> Result<i64> {
  Ok(
    match storage.batch.raw_ttl_with_prefix(prefix, key).await? {
