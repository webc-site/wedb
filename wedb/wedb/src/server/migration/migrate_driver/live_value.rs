...
pub async fn probe_live_key_kind<D: Device>(
  ...
  // 3. 集合元记录域（RangeIndex 单树专属物理域）
  if storage
    .read_tag_with(key, KeyTag::Meta, |_| ())
    .await?
    .is_some()
  {
    return Ok(LiveKeyKind::RangeIndex);
  }
...
pub async fn read_live_value<D: Device>(
  ...
  if storage
    ...
    .is_some()
  {
    return Ok(LiveValue::RangeIndex);
  }
...
/// 预检键清单，返回其中「暂不支持迁移」的键（未知信封
/// 类型）。供迁移入口在触达远端前对混合键请求整体拒绝并列明清单——绝不
/// 静默跳键。向量集键不再列入拒绝清单（带外通道已打通），由调用方收集
/// 后走向量集迁移。
