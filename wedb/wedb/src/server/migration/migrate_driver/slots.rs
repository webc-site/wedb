...
/// 复活挂起计数守卫（RAII，仅冻结/恢复挂起计数，无纪元排空）
///
/// 迁移搬迁窗口的生产路径必须走 [`WedbStore::pause_revivification`]（挂起计数 +
/// 纪元排空强同步合一，对标 Tsavorite.cs:PauseRevivification 完整协议）；本守卫
/// 保留给无并发覆写窗口的纯计数配对场景（wreviv pause 契约「由调用方按需组合」）。
///
...
impl<'a, D: Device> RevivPauseGuard<'a, D> {
  /// 构造守卫并暂停存储复活分配（仅挂起计数，不等待在途写者排空）
  pub fn new(store: &'a WedbStore<D>) -> Self {
    log::info!("维护窗口：暂停存储复活分配（仅挂起计数）");
...
async fn execute_slots_migration(
  ...
  // 强同步暂停复活（对标 MigrationDriver.cs:139 BeginAsyncMigrationTaskAsync 首动作
  // PauseRevivification(_timeout, _cts.Token)）：挂起计数冻结新复活分配，纪元推进 +
  // 排空等待保证返回时先前进入临界区的在途原位复活写者已全数退出，扫描读取期间
  // 可变区/不可变区无并发原位改写；C# 取消令牌投影为 compio Future 取消（守卫先于
  // 等待构造，栈展开即恢复计数），finally 的 ResumeRevivification 投影为守卫 Drop
  // dur 三态直传（0 → Some(ZERO) 快失败、-1 → None 无限等待，对标 C#
  // PauseRevivification(_timeout) 携同一 TimeSpan）
  let dur = wait_dur(spec.timeout);
  let _reviv_guard = store.pause_revivification(dur).await;
  ...
  for slot in sorted_slots {
    ...
    loop {
      ...
      let work = storage
        .get_keys_in_slot_excluding(slot as u16, MAX_MIGRATION_BATCH_COUNT, |k| {
          untouchable.contains(k)
        })
        .await?;
      if work.is_empty() {
        break;
      ...
      if !normal_work.is_empty() {
        ...
        // 返值承判：rust 有界化后静止未达成即判败 recover，§95 迁移族收口口径）
        if !session
          .cluster_provider
        ...
        if !spec.copy_option {
          ...
          // MigrateSessionSlots.cs:247；返值承判：未达成即判败，杜绝在途写在
          // 删除之后才持久化成源端复活键，§95 迁移族收口口径）
          if !session
            .cluster_provider
            .bump_and_wait_for_epoch_transition_async()
            .await
          ...
          // 删除失败键并入 untouchable（与 stuck_keys 同形收敛）：键已传输
          // 目标端但源端删除失败时键权保留源端，重扫剔除防「重传-删除-再失败」
          // 死循环。C# DeleteKeys 的 `_ = DELETE(key)` 同吞删除失败，但
          // ScanStoreTaskAsync 的地址游标 `cursor = current`（
          // MigrateSessionSlots.cs:254）只进不回，失败键成游标外孤儿、任务
          // 照常收敛；本驱动以槽头重扫替代地址游标，缺此登记则每轮重取失败
          // 键死循环。单轮整批失败即持续存储故障（磁盘满等），判败 recover
          // 交运维介入而非空转
          let mut delete_failed = 0usize;
          for key in &transferred {
            if let Err(err) = storage.delete_string(key).await {
              log::error!(
                "槽 {slot} 键 {} 源端删除失败，保留源端: {err}",
          ...
          if !transferred.is_empty() && delete_failed == transferred.len() {
            let err = Error::InvalidArgument("SLOTS 删除环整批失败".into());
            recover_and_fail!(
              &client,
              session,
              &ranges,
  ...
  // 判据与 cluster_session/mod.rs「键扫描与落域只覆盖默认域 (ns=0, db=0)」
  // 门禁同口径收窄：登记表复合键解出的物理域恒为根域物理号 (0,0) 者收录
  // （根域映射恒 (0,0) 见 wkv/src/vdb/manager.rs new/reset），弃
  // logic_domain_of 逻辑反查 + slot_of∈slot_set 判据——slot_of 为满射，
  // 与默认域碰撞同槽的非默认租户域会被整集错迁目标节点默认域并删源
  //（隔离破口）；反查前置契约（冷域须 load_routes_of_vns 回建）亦随
  // 反查面拆除自然消失，冷域静默漏发臂不复存在
  let slot_set: BTreeSet<i32> = session.get_slots().iter().copied().collect();
  let vector_sets = vm
    .as_deref()
    .map(|vm| {
      vm.get_vector_set_keys_for_slots_with(&slot_set, |vns, vdb| {
        if vns == 0 && vdb == 0 {
          return Some(slot_of(0, 0));
        }
        // 非默认登记域（含与默认域碰撞同槽的逻辑域）：跳过保源不迁，
        // 并按 SYNC 快照侧「静默跳过必须留痕」纪律 log::warn 点名域号
        //（形态对位 replication_snapshot_iterator 覆盖清单缺席臂）
        log::warn!("SLOTS 迁移向量集登记域为非默认域，跳过保源: vns={vns} vdb={vdb}");
        None
      })
    })
    .unwrap_or_default();
