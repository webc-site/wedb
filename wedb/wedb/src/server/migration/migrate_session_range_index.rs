...
pub async fn migrate_range_index_keys_async<D: Device>(
  ...
  // 返值承判 Err 上抛交调用侧既有 recover 臂收敛，§95 迁移族收口口径）
  if !session
    .cluster_provider
    .bump_and_wait_for_epoch_transition_async()
    .await
  {
    let err = Error::InvalidArgument("迁移纪元转换等待失败".into());
    migrate_activity.on_error(&err.to_string());
    session.sketch.clear();
  ...
  if !spec.copy_option {
    ...
    // DELETING 收口只删不释（C# 对位 MigrateOperation.DeleteRangeIndex
    // 只删；本链零登记 claim，未持有即释会误删并发 RENAME/换入窗的同源键
    // claim，持有者纪律见 wkv migration.rs rename_range_index 释放配对表锚；
    // 持 claim 键经 DEL 闸回 MigrationBusy 即上抛判败，不销毁他人在搬运的树）
    for key in range_index_keys {
