...
use wbase::{hash_slot::CLUSTER_SLOT_COUNT, map::HashSet};
...
impl MigrateSessionTaskStore {
  ...
  pub fn new() -> Self {
    Self {
      state: RwLock::new(StoreState {
        disposed: false,
        sessions: vec![None; CLUSTER_SLOT_COUNT],
      }),
    }
  }
  ...
  pub fn try_add_migrate_session(
    ...
    if slots.iter().any(|&slot| {
      slot < 0 || slot as usize >= CLUSTER_SLOT_COUNT || state.sessions[slot as usize].is_some()
    }) {
      return None;
    }
  ...
  pub fn try_remove_node(&self, target_node_id: u128) -> bool {
    let mut state = self.state.write();
    if state.disposed {
      return false;
    }
  ...
  pub fn can_access_key(&self, key: &[u8], slot: i32, read_only: bool) -> bool {
    if slot < 0 || slot as usize >= CLUSTER_SLOT_COUNT {
      return true;
    }
    let state = self.state.read();
    if state.disposed {
