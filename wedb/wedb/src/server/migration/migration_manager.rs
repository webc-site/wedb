...
impl MigrationManager {
  ...
  pub fn new(cluster_provider: Arc<ClusterProvider>) -> Self {
    ...
    Self {
      ...
      send_buffer_size,
      network_pool: LimitedFixedBufferPool::new(send_buffer_size, DEFAULT_MAX_ENTRIES_PER_LEVEL),
    }
  }
  ...
  /// libs/cluster/Server/Migration/MigrationManager.cs:GetNetworkPool
  ///
  /// 迁移网络缓冲池读出面：迁移会话建连（MigrateSession.GetGarnetClient
  /// 同源）注入同池，跨连接复用
