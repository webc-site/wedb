...
/// 迭代式单键门评参数（归组消除 too_many_arguments）
pub struct KeyGateParams<'a> {
  pub key: &'a [u8],
  pub slot: u16,
  pub read_only: bool,
  pub session: SlotVerifySessionState,
  pub wait_for_stable: bool,
  pub memo: Option<&'a SlotWaitMemo>,
}
...
impl ClusterManager {
  ...
  /// `wait_for_stable` 对标 C# waitForStableSlot（SingleKeyReadWriteSlotVerify
  /// 的 WaitForSlotToStabalize 循环在迭代门的投影；事务 Prepare 段调用方
  /// 恒 false，形参贯通接口契约）
  fn evaluate_iterative_key_gate(
    ...
    let force = memo.is_some_and(SlotWaitMemo::exhausted);
    // 无盘全量同步扫描键门：被门键写命令单次快评即 Pending（读命令放行；
    // 同步面不得等待，交宿主登记等待体挂起重驱）；超时强制按 TRYAGAIN
    // 终评（语义同单键门评）
    if self.scan_gate_blocks(&[key], slot, read_only) {
      return if force {
        Some(ClusterSlotVerificationState::new(
          SlotVerifiedState::TryAgain,
          slot,
        ))
      } else {
    ...
    // waitForStableSlot 门（谓词同单键门评 evaluate_single_key：向量集写
    // 命令要求槽位先脱离 IMPORTING/MIGRATING）；同步面单次快评不等待，
    // 未稳定即 Pending 交宿主挂起重驱；超时强制按状态机自然降级
    //（MIGRATING → ASK / IMPORTING → CLUSTERDOWN）
    if wait_for_stable
      && !force
      && matches!(
        config.get_state(slot),
        SlotState::Importing | SlotState::Migrating
      )
    {
      return None;
    }
  ...
  pub fn iterative_slot_verify(
    ...
    match self.evaluate_iterative_key_gate(
      p.key,
      p.slot,
      p.read_only,
      p.session,
