...
use crate::server::{
  ...
  cluster_manager_slot_gate::KeyGateParams,
  slot_verify::{
...
impl ClusterSession {
  ...
  /// [`Self::write_cached_slot_verification_message`] 落线。`session_asking`
  /// 为 u8 剩余计数，入口按 > 0 折布尔（对标 C# :39 的
  /// `SingleKeySlotVerify(..., SessionAsking > 0, ...)` 边界转换）；
  /// `wait_for_stable_slot` 对标 C# waitForStableSlot 形参贯通（事务调用方
  /// 恒 false）
  ///
  ...
  pub fn network_iterative_slot_verify(
    ...
    let session = self.slot_verify_session_state(session_asking > 0);
    ...
    let verdict = {
      ...
      cm.iterative_slot_verify(
        ...
        &KeyGateParams {
          ...
          read_only,
          session,
          wait_for_stable: wait_for_stable_slot,
          memo: memo.as_deref(),
        },
      )
    };
