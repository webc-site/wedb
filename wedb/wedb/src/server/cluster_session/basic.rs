...
use wresp::{
  cluster_cmd_strings::{
    RESP_ERR_GENERIC_CANNOT_FORGET_MY_PRIMARY, RESP_ERR_GENERIC_CANNOT_FORGET_MYSELF,
    RESP_ERR_GENERIC_CONFIG_EPOCH_ASSIGNMENT, RESP_ERR_GENERIC_CONFIG_UPDATE,
    RESP_ERR_RESET_WITH_KEYS_ASSIGNED,
  },
  cmd_strings::{
...
use super::{
  ClusterSession, ERR_CLUSTER_NOT_INITIALIZED, cluster_sub_name, run_readonly_storage_slow,
};
use crate::{
  error::Error,
  server::{
...
async fn cluster_reset_slow(
  ...
  run_readonly_storage_slow!(store, storage, out, {
    ...
    if !soft && storage.delete_all_user_keys().await.is_err() {
      log::warn!("CLUSTER RESET HARD flush failed");
    }
    match resp {
      Ok(()) => out.extend_from_slice(RESP_OK),
      Err(Error::ResetWithKeysAssigned) => out.write_resp_error(RESP_ERR_RESET_WITH_KEYS_ASSIGNED),
...
async fn cluster_forget_slow(
  ...
  match manager.try_remove_worker(node_id, expiry_seconds).await {
    ...
    Err(Error::CannotForgetMyself) => out.write_resp_error(RESP_ERR_GENERIC_CANNOT_FORGET_MYSELF),
    Err(Error::CannotForgetPrimary) => {
      out.write_resp_error(RESP_ERR_GENERIC_CANNOT_FORGET_MY_PRIMARY)
    }
    Err(Error::NodeNotFound(id)) => {
      out.write_resp_error(&format!("ERR I don't know about node {id}."))
    }
...
impl ClusterSession {
  ...
  pub(super) fn network_cluster_bumpepoch(
    ...
    if m.try_bump_cluster_epoch() {
      ...
      output.write_resp_error(RESP_ERR_GENERIC_CONFIG_UPDATE);
    }
    true
  }
  ...
  pub(super) fn network_cluster_meet(
    ...
    if let Some(gm) = self.cluster_provider.gossip_manager() {
      spawn(async move {
        if let Err(e) = gm.try_meet_async(&ip, port as i32, true).await {
  ...
  pub(super) fn network_cluster_setconfigepoch(
    ...
    if m.current_config().num_workers() > 1 {
      output.write_resp_error(RESP_ERR_GENERIC_CONFIG_EPOCH_ASSIGNMENT);
      return true;
    }
  ...
  ///
  /// C# 收端不区分 PUBLISH/SPUBLISH（一律 Publish 入队）——其订阅图仅一张，
  /// SSUBSCRIBE 复用普通频道图故无害；rust 三表分离（shard_subscriptions 独立），
  /// SPUBLISH 须入分片域（publish_shard），否则落地普通图：分片订阅者收不到、
  /// 同名频道普通/模式订阅者串台。分派只在接收端此一处，发送侧分片定向见
  /// ClusterManager::try_cluster_publish_async
  pub(super) fn network_cluster_publish(
    ...
    if cmd == RespCommand::ClusterSpublish {
      broker.publish_shard(args[0], args[1]);
    } else {
      broker.publish(args[0], args[1]);
    }
    // C# 无应答写出
    true
  }
