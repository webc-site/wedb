...
use wkv::WedbStore;
use wnode::{
  StorageSession, rangeindex::RangeIndexMigrationReceiveState, resp::slow_path::SlowWait,
};
use wresp::{
  cluster_cmd_strings::{
    RESP_ERR_GENERIC_REPLICATION_AOF_TURNEDOFF, RESP_ERR_GENERIC_VALUE_IS_NOT_BOOLEAN,
    RESP_ERR_MULTI_LOG_DISABLED,
  },
  cmd_strings::{
...
/// CLUSTER SYNC 慢路径壳（无盘全量同步数据导入；帧导入实体收归
...
impl ClusterSession {
  ...
  pub(super) fn network_cluster_replicate(
    ...
    if self.cluster_provider.try_aof().is_none() {
      output.write_resp_error(RESP_ERR_GENERIC_REPLICATION_AOF_TURNEDOFF);
      return true;
    }
  ...
  pub(super) fn network_cluster_flushall_ns(&self, args: &[&[u8]], output: &mut Vec<u8>) -> bool {
    if args.len() != 3 {
      abort_with_wrong_number_of_arguments(
        output,
        cluster_sub_name(RespCommand::ClusterFlushallNs),
      );
      return true;
    }
  ...
  pub(super) fn network_cluster_mlog_key_time(&self, args: &[&[u8]], output: &mut Vec<u8>) -> bool {
    ...
    let Some(aof) = self
      ...
      output.write_resp_error(RESP_ERR_MULTI_LOG_DISABLED);
      return true;
    };
    let is_primary = self
      .cluster_manager()
  ...
  /// （C# `TryBeginDiskbasedSyncAsync` 尾部的局部函数体——TryGetSession 取会话 →
  /// `SendCheckpointAsync` → finally TryRemove 摘除登记，并以 errorMessage 回 RESP；
  /// rust 会话为装配期单例 `assets.sync_session`（无会话仓与逐次登记/摘除），本处即该
  /// 后台体的位点：端点反查失败即回错、同步体经 `pending_slow` 慢路径承载、结果收敛为
  ...
  /// 检查点接收命令执行闭环（上下文解析 -> blocking_wait 单点收割 -> 装配应答）
    ...
    Fut: Future<Output = Result<(), String>>,
  {
    ...
    let Ok(ctx) = self.cluster_provider.checkpoint_import_ctx() else {
      output.write_resp_error(ERR_CLUSTER_NOT_INITIALIZED);
      return true;
    };
    match blocking_wait(f(rm, ctx, token, file_type)) {
  ...
  pub(super) fn network_cluster_snapshot_data(&self, args: &[&[u8]], output: &mut Vec<u8>) -> bool {
    if args.len() != 4 {
      abort_with_wrong_number_of_arguments(
        output,
        cluster_sub_name(RespCommand::ClusterSnapshotData),
      );
      return true;
    }
  ...
  pub(super) fn network_cluster_send_checkpoint_metadata(
    ...
    if args.len() != 3 {
      abort_with_wrong_number_of_arguments(
        output,
        cluster_sub_name(RespCommand::ClusterSendCkptMetadata),
      );
      return true;
    }
    let data = args[2];
  ...
  pub(super) fn network_cluster_send_checkpoint_file_segment(
    ...
    if args.len() != 5 {
      abort_with_wrong_number_of_arguments(
        output,
        cluster_sub_name(RespCommand::ClusterSendCkptFileSegment),
      );
      return true;
    }
    let Some(start_address) = strict_i64(args[2]) else {
  ...
  pub(super) fn network_cluster_begin_replica_recover(
    ...
    if args.len() != 6 {
      abort_with_wrong_number_of_arguments(
        output,
        cluster_sub_name(RespCommand::ClusterBeginReplicaRecover),
      );
      return true;
    }
    let Some(rm) = self.cluster_provider.replication_manager() else {
    ...
    // C# TryGetBool 的 T/F 形态
    let recover_store_from_token = match args[0] {
      b"T" => true,
      b"F" => false,
      _ => {
        output.write_resp_error(RESP_ERR_GENERIC_VALUE_IS_NOT_BOOLEAN);
        return true;
      }
    };
  ...
  pub(super) fn network_cluster_attach_sync(&self, args: &[&[u8]], output: &mut Vec<u8>) -> bool {
    if args.len() != 1 {
      abort_with_wrong_number_of_arguments(
        output,
        cluster_sub_name(RespCommand::ClusterAttachSync),
      );
      return true;
    }
  ...
  pub(super) fn network_cluster_sync(&self, args: &[&[u8]], output: &mut Vec<u8>) -> bool {
    if args.len() != 2 {
      abort_with_wrong_number_of_arguments(output, cluster_sub_name(RespCommand::ClusterSync));
      return true;
    }
