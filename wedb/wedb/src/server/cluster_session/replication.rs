...
///
/// 全借用零拷贝形态（对标 C# NetworkClusterSync 以
/// parseState.GetArgSliceByRef(1) 裸指针同步走查接收缓冲，无整包克隆）：
/// `payload` 直接借用接收缓冲切片，由调用方经 [`blocking_wait`] 内联驱动，
/// 帧导入全链视图挂接收缓冲（同文件检查点接收命令族 execute_checkpoint_recv
/// 同款先例）；应答直写 `out`
...
impl ClusterSession {
  ...
  ///
  /// 载荷零拷贝（N16）：借用接收缓冲切片内联驱动导入（C# 网络线程同步
  /// 走查 `GetArgSliceByRef(1)` 裸指针的同构，无 `to_vec` 整包克隆，亦无
  /// pending_slow 挂起）；经 [`blocking_wait`] 收割（wbase 生产同步收割
  /// 单点，挂起窗口让位本运行时其他任务与 I/O driver），应答直写
  /// `output`——与同文件检查点接收命令族（execute_checkpoint_recv /
  /// network_cluster_snapshot_data 等）同款内联形态
  pub(super) fn network_cluster_sync(&self, args: &[&[u8]], output: &mut Vec<u8>) -> bool {
    ...
    blocking_wait(cluster_sync_slow(
      &self.cluster_provider,
      &store,
      &self.chunk_reassembler,
      &ri_receive_state,
      args[1],
      output,
    ));
