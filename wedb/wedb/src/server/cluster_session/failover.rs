...
use wresp::{
  cluster_cmd_strings::{
    RESP_ERR_GENERIC_CANNOT_FAILOVER_FROM_NON_MASTER, RESP_ERR_GENERIC_UNKNOWN_ENDPOINT,
  },
  cmd_strings::{
...
impl ClusterSession {
  ...
  pub(super) fn network_failover(&self, args: &[&[u8]], output: &mut Vec<u8>) -> bool {
    ...
    if !is_primary {
      output.write_resp_error(RESP_ERR_GENERIC_CANNOT_FAILOVER_FROM_NON_MASTER);
      return true;
    }
    ...
    if let Some(replica_address) = replica_address {
      ...
      let Some(replica_id) =
        ...
        output.write_resp_error(RESP_ERR_GENERIC_UNKNOWN_ENDPOINT);
        return true;
      };
