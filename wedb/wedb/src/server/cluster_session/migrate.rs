...
use wbase::{hash_slot::CLUSTER_SLOT_COUNT, hex::hex_u128, num::strict_i32};
...
use wresp::{
  cluster_cmd_strings::{
    RESP_ERR_GENERIC_SLOT_OUT_OFF_RANGE, RESP_ERR_GENERIC_UNKNOWN_ENDPOINT, RESP_ERR_IOERR,
  },
  cmd_strings::{
...
impl MigrateParseErr {
  ...
  fn err_text(&self, target_address: &str, target_port: i32) -> String {
    match *self {
      Self::UnknownTarget => RESP_ERR_GENERIC_UNKNOWN_ENDPOINT.to_string(),
...
impl ClusterSession {
  ...
  ///    会话槽位的 IsLocal + IsMigratingSlot 两查。
      ...
      TransferOption::Slots => {
        match try_add_slots_migration_task(&self.cluster_provider, spec.clone(), &slots) {
          ...
          Err(err) => {
            ...
            output.write_resp_error(RESP_ERR_IOERR);
          }
        }
        true
      }
        ...
        *self.pending_slow.lock() = Some(SlowWait::new(async move {
          ...
          match run_keys_migration_driver(provider, store, spec, &key_slots, &keys).await {
            ...
            Err(err) => {
              ...
              out.write_resp_error(RESP_ERR_IOERR);
            }
          }
          out
        }));
