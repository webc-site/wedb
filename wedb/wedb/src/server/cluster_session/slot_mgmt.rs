...
use wresp::{
  cluster_cmd_strings::{
    RESP_ERR_GENERIC_SLOT_OUT_OFF_RANGE, RESP_ERR_GENERIC_SLOT_STATE, RESP_ERR_INVALID_SLOT,
    write_slot_duplicate_error, write_slot_range_error,
  },
  cmd_strings::{
...
async fn get_keys_in_slot_slow(
  ...
  let res = for_each_db_in_slot(&store, slot, async |storage| {
    ...
    keys.extend(
      storage
        .get_keys_in_slot(slot, key_count - keys.len())
        .await?,
    );
    Ok(())
  })
  ...
  match res {
    Ok(()) => {
      ...
      out.write_resp_array_len(keys.len());
      for k in &keys {
...
pub(super) enum SlotParseError {
  /// 非整数参数（C# `!TryGetInt` → CmdStrings.RESP_ERR_INVALID_SLOT）
  NotInteger,
  /// 槽位数值越界 [0, 16383]
  OutOfRange,
...
impl SlotParseError {
  ...
  pub fn write_to(self, output: &mut Vec<u8>) {
    match self {
      Self::NotInteger => output.write_resp_error(RESP_ERR_INVALID_SLOT),
      Self::OutOfRange => output.write_resp_error(RESP_ERR_GENERIC_SLOT_OUT_OFF_RANGE),
...
impl ClusterSession {
  ...
  /// `slot_end = slot_start` 收敛，其倒挂态天然不可达）
  ...
  pub(super) fn network_cluster_set_slot(
    ...
    let Some(slot) = strict_i64(args[0]) else {
      output.write_resp_error(RESP_ERR_INVALID_SLOT);
      return true;
    };
    ...
    if ClusterConfig::out_of_range(slot) {
      output.write_resp_error(RESP_ERR_GENERIC_SLOT_OUT_OFF_RANGE);
      return true;
    }
  ...
  pub(super) fn network_cluster_set_slots_range(
    ...
    let slot_state = match parse_slot_state(args[0]) {
      Some(s) if !matches!(s, SlotState::Invalid | SlotState::Offline | SlotState::Fail) => s,
      _ => {
        output.write_resp_error(RESP_ERR_GENERIC_SLOT_STATE);
        return true;
      }
    };
