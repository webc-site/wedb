...
async fn count_keys_in_slot_slow(store: Arc<WedbStore<SegmentedDevice>>, slot: u16) -> Vec<u8> {
  ...
  match for_each_db_in_slot(&store, slot, true, async |storage| {
...
async fn get_keys_in_slot_slow(
  ...
  let res = for_each_db_in_slot(&store, slot, true, async |storage| {
...
async fn del_keys_in_slots_slow(
  ...
  let mut ok = true;
  for slot in &slots {
    let res = for_each_db_in_slot(&store, *slot, false, async |storage| {
...
impl ClusterSession {
  ...
  pub(super) fn network_cluster_add_slots(
    ...
    reject_wrong_arity!(!valid, cmd, output);
  ...
  pub(super) fn network_cluster_set_slot(
    ...
    reject_wrong_arity!(args.len() < 2 || args.len() > 3, cmd, output);
    ...
    // xor 门以 args[2] 在场性判定（非 hex 解析结果），对齐 C# GetString 零校验透传
    if (slot_state == SlotState::Stable) == args.get(2).is_some() {
    ...
    // 协议入口：32 字符 hex 节点 id 解析为内部 u128 身份；拒收门本体保留
    // （§13/r119 A6 收紧不回退），门序=首个错误帧对齐 C# :458-466（r123 裁）
    ...
    match result {
      ...
      Err(e) => output.write_resp_error(&cluster_err_text(e, RespScope::SlotState)),
    }
    true
  }
  ...
  pub(super) fn network_cluster_set_slots_range(
    ...
    reject_wrong_arity!(args.len() < 3, cmd, output);
    ...
    match Self::try_parse_slots(&args[if stable { 1 } else { 2 }..], true) {
      ...
      Ok(slots) => {
        // 协议入口：32 字符 hex 节点 id 解析为内部 u128 身份；拒收门本体保留
        // （§13/r119 A6 收紧不回退），门序=首个错误帧对齐 C# :552-558（r123 裁）
  ...
  pub(super) fn network_cluster_count_keys_in_slot(
    ...
    reject_wrong_arity!(args.len() != 1, cmd, output);
    // 槽号解析：C# 此处为 TryGetInt（int32 档），超 i32 溢出判 invalid slot，007 因死参放行；
    // rust 用 strict_i64 严格文法收口（007 拒收判 invalid slot，见 doc/zh/deviations.md §32）
    let Some(slot) = strict_i64(args[0]) else {
      output.write_resp_error(ERR_INVALID_SLOT);
      return true;
    };
