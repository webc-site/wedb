...
use wbase::{hex::hex_str_u128, supervise::supervise_task};
...
use crate::{
  ...
  server::{
    ...
    replication::{
      assembly::{release_attach_recovery, try_replicate_sync_async},
...
const RESYNC_TASK: &str = "replica_resync";
/// 同上（重启首接臂）
const START_ATTACH_TASK: &str = "replica_start_attach";
...
impl ClusterProvider {
  ...
  ///    AllowRoleChange）。
  ///
  /// 启动阻塞臂口径：C# ReplicationManager.cs:604-605 的 `Start` 启动臂
  /// 已由 `start_replication_attach` 承接。本函数受第 1 步（轮询频率 0
  /// 即禁用，默认 0）与第 3 步（要求 `active_remote_node_id` 即已存在来自
  /// 主端的活跃会话）双门控，专注覆盖「会话断链后的重连」。
  ///
  ...
  pub fn ensure_replication(self: &Arc<Self>, active_remote_node_id: Option<u128>) {
    ...
    let poll_frequency = if let Some(cfg) = self.runtime_config.read().as_ref() {
      cfg.get_int(ServerConfigType::ClusterReplicationReestablishmentTimeout) as i64
    } else {
      self
    ...
    spawn(async move {
      // 任务体经 wbase supervise_task 顶层监督（单点一次成型）：panic 臂补跑
      // allow_role_change——对标 C# EnsureReplication 尾段 prevent → Task.Run →
      // finally allow 的 finally 必达语义，杜绝 panic 后角色门永久封锁
      let run = async {
        log::info!(
      ...
      if supervise_task(RESYNC_TASK, run).await.is_err() {
        provider.allow_role_change();
      }
    })
    .detach();
  ...
  pub fn start_replication_attach(&self) {
    ...
    if role == NodeRole::Replica && self.recover() {
      ...
      spawn(async move {
        // 任务体经 wbase supervise_task 顶层监督（单点一次成型）：panic 臂补跑
        // finally 释放段（启动臂 UpgradeLock:false → 释放到 NoRecovery）——
        // 对标 C# BeginRecovery 后 finally EndRecovery 必达，杜绝恢复锁永锁
        // 使副本此后永久无法 attach（每次 error "could not acquire recovery
        // lock"）
        let run = async {
          if let Err(e) = try_replicate_sync_async(&attach_provider, opts).await {
            log::error!("An error occurred at ReplicationManager.Start: {e}");
          }
        };
        if supervise_task(START_ATTACH_TASK, run).await.is_err() {
          release_attach_recovery(&attach_provider, opts.upgrade_lock);
        }
      })
      .detach();
