...
impl NodeConnection {
  ...
  /// 建连等待面以 gossip_delay 为上界（对标 C# gc.ReconnectAsync().WaitAsync(
  /// clusterProvider.clusterManager.gossipDelay, cts.Token)）：到点放弃等待即返回，
  /// 不回退 initialized 单次语义、不引入取消令牌——被放弃的建连等待自然终结，
  /// 调用方以 is_connected 判定走摘除分支，防网络分区期失联节点以
  /// cluster_node_timeout（60 秒）逐个串行钉死 gossip 主循环。facade 内层
  /// timeout_ms 界（对齐 C# ConnectSendSocketAsync 语义）保留不动
  pub async fn initialize_async(&self, gossip_delay: Duration) {
    ...
    let _ = timeout(gossip_delay, self.client.connect_async()).await;
  }

  /// libs/cluster/Server/Gossip/GarnetServerNode.cs:TryMeetAsync
  pub async fn try_meet_async(
    &self,
    config_bytes: &[u8],
    ...
    self.initialize_async(gossip_delay).await;
  ...
  pub async fn try_flushall_ns_async(
    &self,
    ns: u64,
    origin_hex: &str,
    epoch: i64,
  ...
  /// 3. 执行无响应异步写出
  pub async fn try_cluster_publish_async(
    &self,
    is_spublish: bool,
    channel: &[u8],
    message: &[u8],
