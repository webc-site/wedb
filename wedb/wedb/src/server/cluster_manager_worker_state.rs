  ...
  pub async fn try_reset(
    ...
    loop {
      ...
      for (ns, db) in storage.batch.store.vdb.list_logic_dbs() {
        let tmp_session = storage.batch.store.new_session()?;
        tmp_session.set_context(ns, db);
        let tmp_batch = tmp_session.enter_batch();
        let tmp_storage = StorageSession::new_readonly(tmp_batch);
        if tmp_storage.has_keys_in_slots(&slots).await? {
          has_keys = true;
          break;
        }
      }
      ...
      let mut current = self.current_config.write();
      // 取代 C# CAS 失败重试臂：流式迭代器逐项比对，避免 Vec<usize> 堆分配
      if current
        .slot_list_iter(LOCAL_WORKER_ID as u16)
        .map(|s| s as u16)
      ...
      // 原地复位：保留 64KB slot_map 与 local worker 的 address / port / hostname
      current.reset(new_node_id, config_epoch);
      break;
    }
  ...
  pub async fn try_add_replica_async(
  ...
  ) -> Result<()> {
    {
      ...
      if !force && current.local_node_role() != NodeRole::Primary {
        let primary_id = current
          .local_node_primary_id()
          .map(hex_str_u128)
          .unwrap_or_default();
        return Err(Error::AlreadyReplica(primary_id));
      }
      if !force && current.has_assigned_slots(LOCAL_WORKER_ID as u16) {
        return Err(Error::PrimaryHasAssignedSlots);
      }
      ...
      if current.get_node_role_from_node_id(node_id) != NodeRole::Primary {
        return Err(Error::ReplicateTargetNotPrimary(hex_str_u128(node_id)));
      }
    }
...
// 副本接入错误 → C# TryAddReplicaAsync 各校验臂错误文案的映射已收编至
// wedb::error::cluster_err_text 副本域臂（票 zcode-r135c 案三：会话应答与
// 副本同步发起端共用的唯一映射入口，本域第二函数不再手抄同族句）
