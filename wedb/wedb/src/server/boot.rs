...
use wconf::{ServerArgs, ServerConfigType};
#[cfg(feature = "tls")]
...
pub fn run_cluster_server(
  ...
  bootstrap.run_async(|args, cluster| async move {
    ...
    cluster.update_cluster_auth(
      node.cluster_username.clone(),
      node.cluster_password.clone(),
    );
    ...
    if args.cluster_node_timeout_ms != DEFAULT_CLUSTER_NODE_TIMEOUT_MS {
      let secs = (args.cluster_node_timeout_ms / 1000) as i32;
