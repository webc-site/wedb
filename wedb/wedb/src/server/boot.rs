...
/// 集群数据文件名（node dir 内）
pub const DATA_FILE: &str = "wedb-cluster.db";
...
pub fn run_cluster_server(
  ...
  bootstrap.run_async(|args, cluster| async move {
    ...
    // 报错退出。存储分片拓扑（GarnetLog sharded 分支）不受此门影响，
    // 多子日志复制打通另立需求）
    ...
    // InitLocal 与周期刷盘拉起）：端点取 protected-mode 回退后的首个 TCP
    // 绑定（C# GetClusterEndpoint 同语义）；置于复制域恢复之前、端点 accept
    // 与 gossip 启动之前（须在 compio 运行时内，本回调即运行时内执行）
    let announce_addr = node.bind.as_deref().unwrap_or(if node.protected_mode {
      wconf::DEFAULT_BIND
    } else {
      wconf::DEFAULT_BIND_ANY
    });
    cluster
