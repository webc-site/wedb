...
use wresp::{
  cluster_cmd_strings::{
    RESP_ERR_MALFORMED_APPENDLOG_FRAME, RESP_ERR_PROTOCOL_ERROR_PREFIX,
...
impl<D: Device> ClusterReplicationSession<D> {
  ...
  fn process_primary_stream(
    ...
    // 重放资产）保 enqueued——落盘面直推，掉电窗口与收敛路径登记见
...
impl<D: Device> MessageConsumerFace for ClusterReplicationSession<D> {
    ...
    loop {
      let parsed = match parse_resp_frame(&self.recv_buffer[self.read_head..]) {
        ...
        Err(e) => {
          ...
          resp_buf.extend_from_slice(RESP_ERR_PROTOCOL_ERROR_PREFIX);
      ...
      {
        resp_buf.extend_from_slice(RESP_ERR_UNEXPECTED_CLUSTER_CMD);
        continue;
      }
      if items.len() > 8 {
        resp_buf.extend_from_slice(RESP_ERR_MALFORMED_APPENDLOG_FRAME);
        continue;
      }
  ...
  fn dispose(&mut self) {
    // 断流处置：活跃复制流存在时清空重放驱动（对标 C# ClusterSession.Dispose
    // 的 `if (IsReplicating) replicaReplayDriverStore?.Dispose()`——C# 驱动
    // 仓库随会话消亡，新会话自建；rust 仓库挂 rm 共享，故以 reset 承接同款
    // 语义：清驱动 + 停背景重放任务，容器保持开放，重连时 init 帧重注册；
    // 彻底关闭仅归节点停机面 rm.dispose）
    if self.rm.has_active_replication_stream() {
      self.rm.replica_replay_driver_store.reset();
    }
  }
}
