...
//!    发生，其驱动挂点三处：`cluster_provider.rs` `ensure_replication` 断链
//!    重连后台体、`cluster_session/replica_of.rs` REPLICAOF、
//!    `cluster_session/replication.rs` CLUSTER REPLICATE（C# 第四处
//!    `ReplicationManager.Start` 启动阻塞臂在 rust 无对应挂点，且
//!    `ensure_replication` 受轮询频率与活跃会话双门控亦不承接——口径差异记于
//!    task/done/replica-diskless-sync-initiator.md，挂点归
//!    task/ing/replicaof-diskbased-sync-initiate.md）。
...
use crate::{
  ...
  server::{
    ...
    replication::{
      ...
      recovery_status::RecoveryStatus, replica_wire::REPL_ATTACH_TIMEOUT,
...
async fn replica_diskless_attach(
  ...
  // 4. 连接主节点（用后即弃，对标 C# gcs 构造 / finally Dispose）
  ...
  //    ExecuteClusterAttachSync；attach 级限时取 REPL_ATTACH_TIMEOUT(60s)，
