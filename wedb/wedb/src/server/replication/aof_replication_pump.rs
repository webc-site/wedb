...
use super::aof_sync_driver::AofSyncDriverStore;
...
impl AofReplicationPump {
  ...
  /// 3. 空闲收尾：空闲防抖窗口超时单次触发，落地残存水位（对标 C# idle == true
  ///    语义），解冻闸门内停滞的追加方，随后立刻切回深度休眠。窗口周期每轮
  ///    现取 replica-sync-delay 槽位（C# `BulkConsumeAllAsync(consumer,
  ///    runtimeConfig.GetInt(REPLICA_SYNC_DELAY))` 的每轮现取对译，CONFIG SET
  ///    即时生效）；无配置句柄形态回落兜底常量。
  pub fn start_throttle_loop(&self, runtime_config: Option<Arc<RuntimeServerConfig>>) {
    ...
    spawn(async move {
      while running.load(Ordering::Acquire) {
        ...
        // 步骤 3：空闲收尾防抖窗口（窗口周期每轮现取槽位，热更即时随动）
        loop {
          if !running.load(Ordering::Acquire) {
            return;
          }
          let idle_delay =
            super::replica_replay_task::current_sync_delay(runtime_config.as_deref());
          match timeout(idle_delay, rx.recv()).await {
            ...
            Ok(Err(_)) => return,
            Err(_) => {
              // 超过窗口无新追加：进入空闲期，单次执行收尾发布（对标 C# idle == true）
              store.throttle_all();
...
/// 推流单飞闸守卫：持有期标记驱动在泵，Drop 释放（扫描 I/O 错误经 `?`
/// 上抛的早退路径同样不漏放，下轮泵可重新接管）
...
/// 终止本副本本轮（对标 C# Consume 异常上抛 → RunAsync finally
/// TryRemove(this)：死副本驱动不再留册，AOF 截断线与背压闸门不被失联位点
/// 钉制）。有进展的副本刷新背压水位（对标 C# 迭代间隙的 Throttle）。游标
