...
impl DisklessSyncSession {
  ...
  async fn try_begin_aof_sync(
    ...
    // TryAddReplicationDriver(ref syncFromAddress)）：此处为「原地原子置换」，
    // try_add_replication_driver 在 write 锁内原子校验并原地覆盖预锁的钉线驱动并 dispose 旧实例
    //（同 remote_node_id 覆盖，对标 C# TryAddReplicationDriver 原地替换与互斥，消除先摘后挂空档）；
    ...
    if !rm
      ...
      .try_add_replication_driver(driver.clone(), provider.allow_data_loss())
    {
