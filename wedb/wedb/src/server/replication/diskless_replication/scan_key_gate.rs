...
//! 永久发散。键门分两相栅写：
//!
//! - `Blocking`（注册 → 锚点）：**全部键写命令**挂起等待（覆盖域清单须在窗口
//!   内枚举完成方能按域装载未读键集，枚举前无从按槽裁决——新域在枚举完成后
//!   始终缺席覆盖清单，其数据恰在锚后经 AOF 续推，全阻窗口放行后不损失）；
//!   换号族（FLUSHDB/FLUSHNS/SWAPDB）无用户键可校验、不经本门（门谓词唯挂
//!   键位判定核，`is_data_command` 排除表使其在分派第一跳摘出）——其窗内
//!   域重绑由快照读值域钉承接为执行面禁换绑（见 [`super::replication_snapshot_iterator`]
//!   模块头「域钉」）；纪元静止等待排空在途写后取锚，锚点即「键写静止点」
//!   ——锚前键写记录效果必然全部已含于其后的扫描读值；
...
/// 门相：锚前全阻（全部键写命令挂起、换号族由读值域钉承接，纪元排空后取锚）
