...
impl ReplicationSyncManager {
  ...
  async fn stream_sync(
    ...
    // .cs:56-61：先于流式迭代取锚、逐会话赋同值）：锚前记录的效果已含于
    // 快照，AOF 推流恰从锚起续推（授予位点 = 锚），[协商位点, 锚] 窗口记录
    // 不再重放——否则集合域的增量语义记录（ObjectStoreRMW 的 ReplayInput
    // 增量载荷，重放非幂等）在副本双重应用翻倍；主存字符串侧写侧恒只产
    // 终值 StoreUpsert / StoreDelete 与绝对毫秒 Pexpireat / Persist 条目
    //（StoreRMW 的 INCR/APPEND 类增量重放臂已整族删除，见
    // wnode/src/aof/aof_processor.rs:store_rmw），锚前重放对它们天然收敛。
    // 同批副本共享同锚即修掉旧逐副本各起一遍路径逐调用独立取锚造成授予
    // 位点互异的问题
    let snapshot_anchor = AofAddress::create(sublog_count, wal.tail_address() as i64);
    ...
    run_snapshot_fanout(provider, local_node_id, full_sessions).await
  }
}
