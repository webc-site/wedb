...
//! 活键，向量段按覆盖域展开 db_slots；快照链不重放锚前 AOF，副本域映射
//! 体系经 DbMeta 映射帧（[`wkv::DbMetaRecord`] 单点编码，接收端
...
impl SnapshotIteratorManager {
  ...
  async fn run(
    ...
    // 原语；返值忽略：C# 无限自旋，rust 有界放行）
    let _ = provider.bump_and_wait_for_epoch_transition_async().await;
    ...
    // ===== 全量活跃域枚举（Blocking 相下无并发写可完成：域清单与逐域键集
    // 即锚点时刻的完整快照态）=====
