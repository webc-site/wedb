...
impl ReplicaSyncSession {
  ...
  async fn send_checkpoint_and_recover(
    ...
    // 复制网络缓冲池同池注入，对标 C# ReplicaSyncSession.cs:99 SendCheckpointAsync
    // gcs 构造的 replicationManager.GetNetworkPool 形参；出站 TLS 单源透传）
    let mut client = GarnetClient::with_auth(
      replica_endpoint.to_string(),
      provider.cluster_username(),
      provider.cluster_password(),
    );
    client.set_network_pool(Some(self.rm.network_pool()));
    apply_tls!(client, provider);
    if let Err(e) = client.connect_async().await {
      let err_msg = format!("Failed connecting to replica for checkpoint send: {e}");
      log::warn!("{err_msg}");
      return Err(err_msg);
    }
    if !client.is_connected() {
      return Err("Failed connecting to replica for checkpoint send (not connected)".to_string());
    }
  ...
  async fn transmit_checkpoint(
    ...
    let begin_aof_addr =
      AofAddress::create(self.rm.sublog_count() as i32, wal.begin_address() as i64);
    self
      .rm
      .data_loss_check(provider.allow_data_loss(), &sync_from, &begin_aof_addr)?;
    log::info!(
...
mod tests {
  ...
  fn open_test_wal(dir: &tempfile::TempDir) -> Arc<WalLog<SegmentedDevice>> {
    let device = Arc::new(
      SegmentedDevice::single_file(dir.path().join("test.wal")).expect("create wal device"),
    );
