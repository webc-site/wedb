...
impl ReplicaSyncSession {
  ...
  pub async fn initiate_replica_sync<D: Device + 'static>(
    &self,
    ...
    let wire = TcpSessionWire::connect(replica_endpoint, local_node_id, 0, None, None)
      .await
    ...
    // 4. 推流泵挂载（入队信号唤醒增量拉取）+ 存量补扫（attach 前的记录）
