...
///（登记见 task/done/replica-offset-semantics.md）。
#[derive(Debug)]
...
impl ReplicaReplayDriver {
  ...
  ///
  /// 单任务折叠形态：直推读一致时间（C# 多任务形态经页闸栏广播的
  /// stagedPulseSequenceNumber 已折叠，登记见 replica_replay_task 模块头）
  fn apply_pulse(&self, sequence_number: i64) {
    let Some(assets) = self.assets.as_ref() else {
      return;
    };
    if let Some(rcm) = assets.aof.read_consistency_manager() {
      rcm.advance_virtual_sublog_time(
        assets
          .aof
          .get_virtual_sublog_idx(self.physical_sublog_idx, 0),
        sequence_number,
      );
    }
  }
