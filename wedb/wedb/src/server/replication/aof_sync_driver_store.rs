...
impl AofBackpressureFace for AofBackpressureGate {
  fn publish_shipped_address(&self, physical_sublog_idx: usize, min_shipped: i64) {
    match self {
      ...
      Self::Watermark { watermark, .. } => watermark.store(min_shipped, Ordering::Release),
...
/// libs/cluster/Server/Replication/PrimaryOps/AofOperations/AofSyncDriverStore.cs:ReplicaRoleInfo
///
...
pub struct AofSyncDriverStore {
  ...
  /// 物理日志句柄（C# 经 clusterProvider.storeWrapper.appendOnlyFile?.Log
  /// 反查；Rust 依赖方向反转，集群装配期注入，见 attach_log）
  log: RwLock<Option<Arc<GarnetLog>>>,
}
...
impl AofSyncDriverStore {
  ...
  ///
  /// libs/cluster/Server/Replication/PrimaryOps/AofOperations/AofSyncDriverStore.cs:MinAofAddressFromActiveSyncTasks
  pub fn min_aof_address_from_active_sync_tasks(&self) -> AofAddress {
    let mut min_addr = AofAddress::create(self.sublog_count.try_into().unwrap(), i64::MAX);
  ...
  ///
  /// 记账后经注入的物理日志（[`Self::attach_log`]）执行 C# 尾部同款截断：
  /// `Log.TruncateUntil(TruncatedUntil); Log.Commit();`——截断地址取单调
  /// 记账水位（只进不退，受在册副本最小位点收敛），锁外执行物理平移与
  /// 提交（与 C# `_lock.WriteUnlock()` 后截断的并发纪律一致；begin 平移
  /// 为原子推进，与 commit pipeline 无锁冲突）。FastAofTruncate 分支
  /// （UnsafeShiftBeginAddress）的驱动器是 C# 构造期注册的尾页平移回调，
  /// Rust 无该注册面且配置默认关闭，故不接。
...
mod tests {
  ...
  use wnode::{GarnetLog, InMemorySublog, Sublog};

  use super::*;

  /// 内存双子日志拓扑（物理截断接线测试用）
  fn mem_log_of(sublogs: usize) -> GarnetLog {
    let options = wconf::RuntimeServerOptions {
      aof_physical_sublog_count: sublogs as i32,
      ..RuntimeServerOptions::default()
    };
    let backends: Vec<Arc<Sublog>> = (0..sublogs)
      .map(|_| Arc::new(Sublog::Mem(InMemorySublog::new())))
      .collect();
    GarnetLog::new(&options, backends, None)
  }
  ...
  fn test_driver_store_lifecycle_and_safe_truncate() {
    ...
    assert_eq!(store.safe_truncate_aof(&prune_addr).get(0), Some(50));

    let truncate_beyond = AofAddress::create(2, 150);
    assert_eq!(store.safe_truncate_aof(&truncate_beyond).get(0), Some(100));
  ...
  fn safe_truncate_aof_drives_physical_begin_shift() {
    ...
    use wnode::aof::garnet_log::RecordShape;

    let log = Arc::new(mem_log_of(2));
    let store = AofSyncDriverStore::new(2);
    store.attach_log(Some(Arc::clone(&log)));

    // 双子日志各写一条记录推进物理尾地址（begin 保持初始 1；
    // 按目标物理子日志反选键，规避哈希路由集中）
    for i in 0..2 {
      ...
      log.enqueue(&record);
      assert!(log.get_tail_address(i) > 1, "子日志 {i} 应有记录");
      assert_eq!(log.get_begin_address(i), 1);
    }

    // 无副本：检查点覆盖地址 50 → 物理 begin 直达 50
    let safe = store.safe_truncate_aof(&AofAddress::create(2, 50));
    ...
    assert!(store.try_add_replication_driver(d1, false));
    let safe = store.safe_truncate_aof(&AofAddress::create(2, 150));
    ...
    let safe = bare.safe_truncate_aof(&AofAddress::create(1, 32));
  ...
  fn test_backpressure_gate_wiring() {
    ...
    let gate = AofBackpressureGate::Log(log.clone());
    let store = AofSyncDriverStore::new(2);
    store.attach_backpressure(Some(gate));

    let last_for_sublog = |sublog: usize| {
      log
        .read()
        .iter()
        .rev()
    ...
    assert_eq!(last_for_sublog(0), Some(i64::MAX), "无副本时门控应放行");
    ...
    assert_eq!(last_for_sublog(0), Some(0), "副本 attach 后水位收紧");
    ...
    assert_eq!(last_for_sublog(0), Some(9_500), "水位推进到 9500");

    // detach：TryRemove 尾部重报应写 MAX 释放门控
    assert!(store.try_remove("r1"));
    assert_eq!(last_for_sublog(0), Some(i64::MAX), "副本移除后门控应放行");
    ...
    assert!(store.try_add_replication_driver(d2, false));
    store.reset();
    assert_eq!(last_for_sublog(0), Some(i64::MAX), "reset 后门控应放行");
  }
}
