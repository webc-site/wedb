use std::{
  fmt::Write as _,
  fs::{File, create_dir_all, read, remove_file, rename},
  io::{self, Write as _},
  net::hostname,
  path::{Path, PathBuf},
  sync::{
...
use wbase::{
  hex::{generate_hex_id, hex_str_u128},
...
pub use crate::server::cluster_manager_slot_gate::{GateVerdict, SlotVerifyRequest, SlotWaitMemo};
use crate::{
  error::{Error, Result},
  server::{
...
/// 监督快照里的集群配置周期刷盘任务名（[`ClusterManager::start_flush_task`] 的
/// 监督归组名，随 INFO bg_task_health 出；对标 C# ClusterManager.cs 构造尾
/// Task.Run(FlushTaskAsync) 的 try-finally 终止留痕契约）
const FLUSH_TASK: &str = "cluster_flush";

/// 生成 40 字符十六进制随机串（对标 C# Generator.CreateHexId(40)）；
/// 仅供复制 replid（主从复制身份，Redis 协议惯例 40 hex）使用，
/// 集群节点 ID 已收敛 u128，走 [`create_node_id`]。
/// 单一真源收敛于 [`generate_hex_id`]
#[inline]
...
/// OS 主机名探测（C# Format.GetHostName() 的 rust 等价，使用标准库 `std::net::hostname()`）：
/// 探测失败或返回值非 UTF-8 时回退空串（与「未宣告」同义，下游端点/反查臂据此退化为 IP）。
/// pub(crate)：announce 宣告 IP 解析的机器名等价门共用本单源
...
/// libs/cluster/Server/ClusterUtils.cs:WriteInto 的 rust 形态（落盘原语）
///
/// 安全原子写链（对标 [`crate::server::replication::replication_history::ReplicationHistory::flush_to_file`] 的发布持久化口径）：
/// 同目录随机 .tmp、write_all + sync_all 数据屏障、rename 原子替换、[`wdev::sync_dir`] 目录屏障
pub fn write_into(path: &Path, bytes: &[u8]) -> io::Result<()> {
  let parent = path
    .parent()
  ...
  // 随机后缀临时文件（同目录保证 rename 原子，防并发互踩）
  let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("nodes");
  let tmp_path = parent.join(format!(".{file_name}.{:016x}.tmp", fastrand::u64(..)));
...
pub struct ClusterManager {
  ...
  /// 停写让渡态标志：try_stop_writes 置位（主端把槽位划归副本、自身降级
  /// 复制源），try_restore_stop_writes / try_reset_replica 清位。让渡态的
  /// 唯一真实判据——「复制源在场」与常态副本（从属正常主节点）在配置层
  /// 完全同形，无法区分让渡态主节点与被动副本，误判即成单命令窃取他节点
  /// 全部槽位的通道
  stop_writes_delegated: AtomicBool,
...
impl ClusterManager {
  ...
  pub fn new(cluster_provider: Arc<ClusterProvider>) -> Self {
    ...
    Self {
      ...
      stop_writes_delegated: AtomicBool::new(false),
  ...
  pub(crate) fn start_flush_task(self: &Arc<Self>, period: Duration) {
    ...
    // bg_task_health 出）；Err 臂复位幂等启动位 flush_running（upgrade 失败即
    // 宿主已亡无需复位），恢复 start_flush_task 可重拉能力——对标 C#
    // ClusterManager.cs 构造尾 Task.Run(FlushTaskAsync) 的 try-finally 终止收口
    // （supervise.rs 模块头 Err 臂复位纪律）
    spawn(async move {
      if supervise_task(
        FLUSH_TASK,
        ClusterManager::flush_task_async(manager.clone(), period),
      )
      .await
      .is_err()
        && let Some(cm) = manager.upgrade()
      {
        cm.flush_running.store(false, Ordering::Release);
      }
    })
    .detach();
  }
  ...
  ///
  /// gossip 主循环启动/重拉唯一入口（幂等门在 GossipManager::start 的
  /// is_running CAS）：装配期经 Start，运行期由 MEET 发起
  /// （network_cluster_meet）与入站 gossip 合并（cluster_gossip_slow）两个
  /// 事件安全点重拉——主循环 panic 终局臂复位拆池后随下一事件复活
  ...
  pub async fn try_cluster_publish_async(&self, cmd: RespCommand, channel: &[u8], message: &[u8]) {
    ...
    // 建连等待界与 gossip 主循环节拍同源（对标 C# InitializeAsync 内
    // WaitAsync(gossipDelay)，live 读 CONFIG SET gossip-delay 即时生效）
    let gossip_delay = Duration::from_millis(self.cluster_provider.gossip_delay_ms());
    ...
    for (node_id, endpoint) in node_entries {
      ...
      spawn(async move {
        conn
          .try_cluster_publish_async(is_spublish, &ch, &msg, gossip_delay)
          .await;
      })
      .detach();
    }
  ...
  pub async fn flushall_broadcast_async(&self, ns: u64) -> Result<()> {
    ...
    let outcomes = join_all(conns.into_iter().map(|(node_id, conn)| async move {
      match wait_async(
        wait,
        conn.try_flushall_ns_async(ns, origin_hex, epoch, gossip_delay),
      )
      .await
      {
        Some(r) => r,
  ...
  ///
  /// 落盘口单一互斥（对齐 C# `lock(this)` 与同仓 [`crate::server::replication::replication_manager::ReplicationHistoryManager::flush_config`]）：
  /// 先占 [`Self::flush_disk_lock`] 再读当前配置快照并写盘，保证内存版本序与落盘序严格一致
  ...
  /// （ClusterManager.cs:235/:244-245，C# 全库无按槽位覆盖推导 cluster_state
  /// 的判据，非本仓计数器缺口）；真实 gossip 计数经 GossipStats 单点在
  /// INFO stats 段暴露（对位 ClusterProvider.cs:GetGossipStats）
  ...
  ///
  /// 复位即让渡态解除（副本回发 stop-writes 复位 / REPLICAOF NO ONE 升主
  /// 清理两路入口）：槽位已归接管者，让渡赎回标志一并清位
  pub fn try_reset_replica(&self) {
    self.stop_writes_delegated.store(false, Ordering::Release);
    {
      let mut current = self.current_config.write();
      current
  ...
  pub fn try_stop_writes(&self, replica_id: u128) {
    ...
    // 让渡态在配置落地后置位：此后 try_restore_stop_writes 才对本端生效
    self.stop_writes_delegated.store(true, Ordering::Release);
    self.flush_config();
  }

  /// 当前是否处于停写让渡态（主节点发起 failover 期间）
  #[inline]
  pub fn is_stop_writes_delegated(&self) -> bool {
    self.stop_writes_delegated.load(Ordering::Acquire)
  }

  /// TryStopWrites 的反向赎回（C# 上游缺失，PR #1670 立论：主节点让渡槽位
  /// 但从节点未接管会导致槽位无主的非法状态）：主节点 failover 失败或被
  /// abort 时，把让渡给副本的槽位原子收回 LOCAL_WORKER_ID，恢复 PRIMARY
  /// 角色并清空复制源，自增纪元后刷盘。判据化赎回：唯一判据是
  /// try_stop_writes 置位的真实让渡标志（swap 清位，单次赎回幂等）——
  /// 「复制源在场」与常态副本（从属正常主节点）在配置层完全同形，不可
  /// 作为让渡判据；裸 PRIMARY、被动副本与未让渡主节点一律拒绝，杜绝
  /// 抢夺他节点槽位
  pub fn try_restore_stop_writes(&self) -> bool {
    if !self.stop_writes_delegated.swap(false, Ordering::AcqRel) {
      return false;
    }
    let mut restored = false;
    {
      let mut current = self.current_config.write();
      if let Some(primary_id) = current.local_node_primary_id() {
        let worker_id = current.get_worker_id_from_node_id(primary_id);
        let slots = current.get_slot_list(worker_id);
        current
          .assign_slots(&slots, LOCAL_WORKER_ID as u16, SlotState::Stable)
          .make_replica_of(None)
          .set_local_worker_role(NodeRole::Primary)
          .bump_local_node_config_epoch();
        restored = true;
  ...
  ///（C# 局部函数 `expiry < DateTimeOffset.UtcNow.Ticks` 的过期判据；rust
  /// 以秒级时间戳同判，内联于本循环条件）
