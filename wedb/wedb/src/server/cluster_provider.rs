...
/// 副本重连发起钩子（函数指针，零成本抽象与零堆分配）
pub type RecoverReplicationHook = fn(&Arc<ClusterProvider>, &str);
...
impl ClusterProvider {
  ...
  pub fn ensure_replication(self: &Arc<Self>, active_remote_node_id: Option<&str>) {
    ...
    let hook = *self.recover_replication_hook.read();
  ...
  pub fn checkpoint_version_shift_hooks(
    self: &Arc<Self>,
  ) -> (
    impl Fn(i64, i64) + Send + Sync + 'static,
    impl Fn(i64, i64) + Send + Sync + 'static,
  ) {
    let start_provider = Arc::clone(self);
    let start = move |old: i64, new: i64| {
