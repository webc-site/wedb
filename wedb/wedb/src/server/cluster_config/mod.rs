...
impl ClusterConfig {
  ...
  pub fn get_local_node_replica_endpoints(&self) -> Vec<SocketAddr> {
    ...
    for worker in self.workers.iter().skip(2) {
      if worker.replica_of_node_id == Some(local_id)
  ...
  pub fn remove_worker(&self, nodeid: u128) -> Self {
    ...
    for slot in new_slot_map.iter_mut() {
      ...
      if state == SlotState::Stable && wid == worker_id {
        ...
        if self.workers[wid].nodeid == Some(nodeid) {
          slot.worker_id = RESERVED_WORKER_ID as u16;
          slot.state = SlotState::Offline;
        }
...
impl ClusterConfig {
  ...
  pub fn merge_slot_map(&mut self, sender_config: &ClusterConfig) -> bool {
    ...
    for i in 0..CLUSTER_SLOT_COUNT {
      ...
      if sender_slot_map[i].worker_id as usize != LOCAL_WORKER_ID && sender_is_primary {
        let current_owner_node_id = self.workers.get(current_owner_id).and_then(|w| w.nodeid);
        if current_owner_node_id.is_some() && current_owner_node_id == sender_node_id {
      ...
      if sender_is_primary {
        ...
        let owner_is_sender = sender_node_id.is_some_and(|sid| {
          self
            .workers
            .get(current_owner_id)
            .is_some_and(|w| w.nodeid == Some(sid))
        });
        if !owner_is_sender {
          continue;
        }
  ...
  pub fn merge(
    ...
    worker_ban_list: &HashMap<u128, i64>,
  ) -> Option<Self> {
    ...
    for worker in sender_config.workers.iter().skip(1) {
      ...
      if local_id == Some(sid) || worker_ban_list.contains_key(&sid) {
        continue;
      }
...
impl ClusterConfig {
  ...
  fn collect_worker_endpoints(&self, primary_only: bool) -> Vec<(u128, SocketAddr)> {
    self
      ...
      .filter_map(|worker| {
        let id = worker.nodeid?;
        let ip = worker.address.parse().ok()?;
        Some((id, SocketAddr::new(ip, worker.port as u16)))
      })
      .collect()
  }
  ...
  pub fn get_node_ids_for_shard(&self) -> Vec<(u128, SocketAddr)> {
    ...
    self
      ...
      .filter_map(|worker| {
        ...
        if is_match
          && let Some(nid) = worker.nodeid
          && let Ok(ip) = worker.address.parse()
        {
          Some((nid, SocketAddr::new(ip, worker.port as u16)))
        } else {
          None
        }
      })
  ...
  pub fn get_slot_list(&self, worker_id: u16) -> Vec<usize> {
    self
      ...
      .iter()
      .enumerate()
      .filter_map(|(i, slot)| (slot.eff_worker_id() == worker_id).then_some(i))
      .collect()
  }
