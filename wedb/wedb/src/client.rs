...
use wconn::{Error as WconnError, client::GarnetClient as ConnClient};
#[cfg(feature = "tls")]
...
impl GarnetClient {
  ...
  pub async fn execute_cluster_fail_replication_offset_async(&self, offset: &AofAddress) -> String {
    ...
    let resp = client
      .execute_for_bytes_result_async(&[b"CLUSTER", b"FAILREPLICATIONOFFSET", &payload])
      .await;
    match resp {
      Ok(v) => from_utf8(&v).unwrap_or_default().to_string(),
      Err(_) => {
