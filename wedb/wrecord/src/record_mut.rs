...
use crate::{
  ...
  header::{
    ...
    bits::{MODIFIED_BIT, SEALED_BIT, TOMBSTONE_BIT},
  },
...
/// - 值内容先落笔，随后 [Self::publish_val_layout] 以**单次原子 store** 发布 RDH 字
///   （全量改写 [Self::write_val_with_slack] 与零复制改长
///   [Self::resize_val_with_slack] 两写口共用本内核）——
...
///
/// 头位段只读访问面经 `Deref<Target = RecordHeader>` 直达记录头，
/// 本视图不写转发代理（写侧一律走本视图的原子 RMW / 单字发布原语）。
#[derive(Debug)]
...
impl<'a> RecordMut<'a> {
  ...
  /// 获取值区完整可变切片（逻辑值 + 槽位松弛富余，长度即 [RecordHeader::val_capacity]）
  ///
  /// 原位增长写口的落笔面：新字节直接写在旧值尾部或间隙之上，旧数据零复制
  /// （对位 C# `logRecord.ValueSpan.Slice(originalLength)` 的可写口径）。容量视图
  /// 刻意只在写侧 [Self] 上开放——超出现逻辑值长的字节在布局发布前对读者不可见，
  /// 改长必须经 [Self::resize_val_with_slack]，绝不允许旁生第二处裸改头。
  #[inline]
  pub fn value_capacity_mut(&mut self) -> &mut [u8] {
    let key_end = self.key_end();
    // 安全性保证：构造期 parse_header_phys 已把 slice 截断至物理槽位，
    // 物理槽位恒为 头 + 键 + 值 + 松弛填充
    unsafe { self.slice.get_unchecked_mut(key_end..) }
  }

  /// 布局发布内核：值字节已落笔后，以单次 RDH 原子字 store 同时发布新 val_len 与
  /// 重排的松弛填充，再置位 MODIFIED（复活路径单次 CAS 同字清墓碑）
  ///
  /// 全仓唯一的记录长度发布点（对标 C# RecordDataHeader.Initialize 单 8 字节原子写
  /// 协议）——一切改长写口一律收敛到本内核，不设第二套。
  #[inline]
  fn publish_val_layout(
    &mut self,
    new_val_len: usize,
    remaining_slack: usize,
    clear_tombstone: bool,
  ) {
    // 富余空间精确折算为 8 字节词粒度松弛填充（记录对齐不变式下差值恒为词整数倍），
    // 更新缓存的 RDH 位段
    self.header.set_val_len(new_val_len as u32);
    self.header.set_filler_words((remaining_slack >> 3) as u8);

    // RDH 原子字单次发布完整新布局（filler + key_len + val_len 同字原子可见）
    self.publish_rdh();

    // RecordInfo 字原子置位 MODIFIED 标记对齐 Tsavorite InPlaceWriter；
    // 复活路径原子清除墓碑位（单次 CAS 同步完成置位与清零，消除双重总线同步与瞬态窗口，
    // 保证读者观察到非墓碑时新布局与新值必已发布）
    if clear_tombstone {
      let mut word = self.atomic_info_word().load(Ordering::Relaxed);
      loop {
        let new_word = (word | MODIFIED_BIT) & !(TOMBSTONE_BIT | SEALED_BIT);
      ...
      self.header.prev_address =
        (self.header.prev_address | MODIFIED_BIT) & !(TOMBSTONE_BIT | SEALED_BIT);
    } else {
    ...
    trace!(
      "原位更新记录值(动态松弛): key_len={key_len}, val_len={new_val_len}, filler_bytes={remaining_slack}, clear_tombstone={clear_tombstone}"
    );
  }

  /// 基于 FillerWords 与动态松弛写入新值的公共实现（严格对标 libs/storage/Tsavorite/cs/src/core/Allocator/LogRecord.cs:TrySetPinnedValueSpan & InternalRMW 原位写入语义）
  ///
  /// - 非复活路径（`clear_tombstone == false`）拦截墓碑记录，与 [RecordHeader::can_update_with_slack] 查询语义一致；
  /// - 新长度经 [RecordHeader::slack_for_val_len] 单点容量门校验（对齐后不超过槽位
  ///   物理占用，腾出的显式松弛可被 8 位 FillerWords 完整表达）；
  /// - 原位覆写值内容，富余空间折算为 8 字节词粒度的松弛填充；
  /// - `clear_tombstone` 为 true 时同步原子清除墓碑位（链内原地复活）；
  /// - 发布协议：值内容先落笔 → [Self::publish_val_layout] 单次 RDH 原子字 store
  ///   发布新布局（filler + val_len 同字）→ RecordInfo 字原子置位 MODIFIED（复活路径
  ///   原子清墓碑），严格对标 C# RecordDataHeader.Initialize 单 8 字节原子写发布协议。
  #[inline]
  fn write_val_with_slack<V: ValSrc + ?Sized>(
    &mut self,
    new_val: &V,
    clear_tombstone: bool,
  ) -> Result<()> {
    ...
    let new_val_len = new_val.val_len();
    let physical_size = self.header.physical_size();
    let Some(remaining_slack) = self.header.slack_for_val_len(new_val_len) else {
    ...
    let key_end = self.key_end();
    let new_val_end = key_end + new_val_len;

    // 1. 写入新值内容（先于布局发布，Release store 保证读者观察到新布局时值字节就绪）；
    //    分段值源（[ValSrc]）在此单点直写落笔，消除中间整值缓冲
    // SAFETY: 前置容量门已校验 new_aligned <= physical_size == self.slice.len()，且 new_val_end <= new_aligned
    new_val.write_val(unsafe { self.slice.get_unchecked_mut(key_end..new_val_end) });

    // 2. 值已就位：单次 RDH 原子字发布新长度与新松弛，并置位 MODIFIED/清墓碑
    self.publish_val_layout(new_val_len, remaining_slack, clear_tombstone);
  ...
  #[inline]
  pub fn update_value_with_slack<V: ValSrc + ?Sized>(&mut self, new_val: &V) -> Result<()> {
    self.write_val_with_slack(new_val, false)
  }

  /// 零复制原位改长（严格对标 libs/server/Storage/Functions/MainStore/RMWMethods.cs
  /// :InPlaceUpdater 的 `TrySetContentLengths(totalLength)` 臂——SETRANGE 分支 :745 /
  /// APPEND 分支 :813，旧数据一律不动）
  ///
  /// 值字节已由 [Self::value_capacity_mut] 视图就地落笔，本口只重排松弛填充并以
  /// 同一次 RDH 原子字 store 发布新 val_len（复用 [Self::publish_val_layout]，与
  /// 全量写口 [Self::write_val_with_slack] 同一发布内核，绝无第二处 set_val_len）。
  /// 容量门同为 [RecordHeader::slack_for_val_len] 单点：槽位富余不足即回
  /// [Error::ValueLengthMismatch]——已落笔的富余字节尚未发布，
  /// 读者不可见，调用方可安全降级尾部追加。墓碑记录一律拒绝（复活须走
  /// [Self::revivify_with_slack]）。
  #[inline]
  pub fn resize_val_with_slack(&mut self, new_val_len: usize) -> Result<()> {
    ...
    self.publish_val_layout(new_val_len, remaining_slack, false);
    Ok(())
  }

  /// 链内原地复活专用方法：单次覆写原子完成新值填入、松弛吸纳与清除墓碑标记
  #[inline]
  pub fn revivify_with_slack<V: ValSrc + ?Sized>(&mut self, new_val: &V) -> Result<()> {
...
/// PreviousAddress 等）唯一定义于 RecordInfo 自身，写侧视图直接取 recordInfo 实例访问，
/// 绝无第二层转发面。本视图据此撤除逐个手写的零参数代理，改由 Deref 承载，
/// `rec.is_tombstone()` / `rec.val_capacity()` 等写法不变而定义只剩头一处。
///
/// 刻意**不实现 DerefMut**：位段不得经只引用旁路改写（并发下绕过原子发布协议），
/// 一切变更必须走本视图的原子 RMW 与单字 store 写侧接口。
