...
use wrecord::{
  Error, HEADER_SIZE, RECORD_ALIGNMENT, RecordHeader, RecordMut, RecordRef, encode_to_slice,
...
fn test_variable_length_zero_copy_consistency() -> Void {
  ...
  for (i, (k, v, addr, is_tombstone)) in test_cases.iter().enumerate() {
    // 对齐逻辑尺寸（含隐式对齐填充，8 字节整数倍）
    let expected_record_size = record_size(k.len(), v.len());
    assert_eq!(expected_record_size % RECORD_ALIGNMENT, 0);
    assert!(expected_record_size >= HEADER_SIZE + k.len() + v.len());
    ...
    let written = encode_to_slice(&mut slice_buf, *addr, k, v, *is_tombstone, false)?;
...
/// - Key 非空 (17B)，Value 为空 (0B)：读取正确；超出物理槽位容量的写入被拦截
#[test]
fn test_empty_key_value_boundary() -> Void {
  ...
  rec_mut.update_value_with_slack(b"")?;
  assert_eq!(rec_mut.value(), b"");

  // 空值记录槽位仅 16 字节头，9 字节新值对齐后需 24 字节，超容拦截且不触碰数据区
  let err_empty = rec_mut.update_value_with_slack(b"non_empty");
  assert_eq!(
    err_empty,
  ...
  assert_eq!(empty_key_encoded.len(), record_size(0, 128));
  ...
  assert_eq!(empty_val_encoded.len(), record_size(key_payload.len(), 0));
...
/// - 验证连续多条变长记录在连续页切片中经 from_slice + physical_size 逐条无缝推进
#[test]
fn test_streaming_split_and_page_scan() -> Void {
  ...
  // 1. 逐条 from_slice 推进的流式切分（物理槽位尺寸由 RDH 原子字推导）
  let mut read_slice = page_buf.as_slice();
  let mut parsed_count = 0;

  while !read_slice.is_empty() {
    let rec = RecordRef::from_slice(read_slice)?;
    ...
    read_slice = &read_slice[rec.physical_size()..];
    parsed_count += 1;
  }
...
/// 记录对齐不变式下的非对齐缓冲行为测试
/// 对标 C# Tsavorite Constants.kRecordAlignment = 8（RDH 单 8 字节原子字发布的前提）：
/// - RecordHeader 采用小端字节序方法，不依赖裸指针直接强转，纯格式层编解码
///   （encode_to_slice / from_slice / 快速探针）对 1..=7 字节非对齐偏移均安全无崩溃；
/// - RecordMut 的原子发布协议要求记录头 8 字节对齐（debug 构建断言拦截）——
///   whlog 页内记录由 RECORD_ALIGNMENT 对齐分配保证恒对齐，非对齐偏移仅在
///   独立测试缓冲中出现，原位原子修改属调用方契约违约，不在本测试范围。
#[test]
fn test_unaligned_memory_access_safety() -> Void {
  info!("开始测试: 非对齐缓冲纯格式层编解码安全性");
  ...
  for unaligned_shift in 1..=7 {
    ...
    let written = encode_to_slice(target_slice, prev_addr, key, val, false, false)?;
