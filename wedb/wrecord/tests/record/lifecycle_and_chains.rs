...
/// APPEND 分支 :799-834 / SETRANGE 分支 :734-763：C# 只以
/// `TrySetContentLengths(totalLength)` 改长度、旧值字节一律不动，新字节另落
/// `ValueSpan.Slice(originalLength)`；本口 `value_capacity_mut` +
/// `resize_val_with_slack` 即该臂的 rust 直译。
///
...
/// 一律不动）与 CopyUpdater 臂 :1040/:1402（整帧另落一条新记录）。两条臂的物理写面
