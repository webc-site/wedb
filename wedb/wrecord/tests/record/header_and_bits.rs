...
fn test_tombstone_lifecycle_and_address_preservation() -> Void {
  ...
  // RecordHeader 直接设置与清空墓碑，以及清除磁盘镜像易失位 (ClearBitsForDiskImages)
  {
    ...
    hdr.set_sealed(true);
    assert!(hdr.is_sealed());
    hdr.clear_bits_for_disk_images();
    assert!(!hdr.is_sealed());
    assert!(hdr.is_tombstone());
    hdr.set_tombstone(false);
    assert!(!hdr.is_tombstone());
...
/// 验证 TrySeal, TryResetModifiedAtomic, TryUpdateAddress, SetInvalidAtomic, WriteInfo 等
#[test]
fn test_record_info_cas_and_lifecycle() -> Void {
  ...
  // 1. InitializeForNewRecord / WriteInfo
  let mut h = RecordHeader::initialize_for_new_record();
  assert!(h.is_sealed());
  assert!(h.is_closed());
  assert!(!h.is_tombstone());
  assert_eq!(h.address(), 0);

  h.write_info(true, 0x1234_5678);
  assert!(h.is_sealed());
  assert!(h.is_in_new_version());
  assert_eq!(h.address(), 0x1234_5678);

  h.unseal_and_validate();
  assert!(!h.is_sealed());
  assert!(!h.is_closed());

  h.seal_and_invalidate();
  assert!(h.is_sealed());
  assert!(h.is_closed());
  ...
  h.set_in_place_updated(true);
  assert!(h.is_in_place_updated());
  assert!(h.is_modified());
  h.set_in_place_updated(false);
  assert!(!h.is_in_place_updated());
  assert!(!h.is_modified());
  ...
  assert_eq!(rec_mut.previous_address(), 0x6000);
  assert!(rec_mut.is_valid());
  assert!(!rec_mut.is_invalid());
  assert!(!rec_mut.skip_on_scan());

  // 9. InitialValid 与 Valid/Invalid/SkipOnScan 1:1 对标验证
  let init_valid = RecordHeader::INITIAL_VALID;
  assert_eq!(init_valid.address(), 1);
  assert_eq!(init_valid.previous_address(), 1);
  assert!(init_valid.is_valid());
  assert!(!init_valid.is_invalid());
  assert!(!init_valid.skip_on_scan());
  assert_eq!(RecordHeader::initial_valid(), init_valid);

  let mut set_addr_hdr = RecordHeader::default();
  set_addr_hdr.set_previous_address(0x7000)?;
  assert_eq!(set_addr_hdr.previous_address(), 0x7000);
