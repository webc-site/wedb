//! 运行时配置管理与更新行为集成测试
//!
//! 自研依据: 运行时服务端配置装配（C# 对应 ServerSettingsManager 装配面）

use std::time::Duration;

use wbase::cfg::LogCompactionType;
use wconf::{
  ConfigError, ConfigKind, ConfigReconcile, RuntimeServerConfig, RuntimeServerOptions,
  ServerConfigType, runtime_server_config::NAME_LOOKUP,
};
...
fn time_span_non_positive_means_infinite() {
  ...
  // 0 = 无限超时（下界 0，负值被 CONFIG SET 拒绝，发出 0 哨兵调停消息）。
  assert_eq!(
    config.try_set(ServerConfigType::ClusterNodeTimeout, "0"),
    Ok(Some(ConfigReconcile::ClusterNodeTimeout { ms: 0 }))
  );
  assert_eq!(
    config.get_time_span(ServerConfigType::ClusterNodeTimeout),
    None
  ...
  // repl-attach-timeout: 缺省 60s 回归；CONFIG SET repl-attach-timeout 0 同效（槽位 min=0 合法接受，非正即 None 无限）；正数透传
  assert_eq!(
    config.get_time_span(ServerConfigType::ReplAttachTimeout),
    Some(Duration::from_secs(60))
  );
  assert_eq!(
    config.try_set(ServerConfigType::ReplAttachTimeout, "0"),
    Ok(None)
  );
  assert_eq!(
    config.get_time_span(ServerConfigType::ReplAttachTimeout),
  ...
  assert_eq!(
    config.try_set(ServerConfigType::ReplAttachTimeout, "120"),
    Ok(None)
  );
  assert_eq!(
    config.get_time_span(ServerConfigType::ReplAttachTimeout),
  ...
  // RuntimeServerOptions 配置 0 与负数：经 seconds_from_time_span 归 0，get_time_span 均映射为 None 无限
  let config_zero = RuntimeServerConfig::new(RuntimeServerOptions {
    replica_attach_timeout_secs: 0,
    ..Default::default()
  });
  assert_eq!(
    config_zero.get_time_span(ServerConfigType::ReplAttachTimeout),
    None
  );
  let config_neg = RuntimeServerConfig::new(RuntimeServerOptions {
    replica_attach_timeout_secs: -1,
    ..Default::default()
  });
  assert_eq!(
    config_neg.get_time_span(ServerConfigType::ReplAttachTimeout),
    None
  );
}
...
fn commit_freq_lifecycle_and_invariants() {
  ...
  // -1 为合法手动提交档，发出 CommitTask { commit_frequency_ms: -1 } 调停消息
  assert_eq!(
    config.try_set(ServerConfigType::AofCommitFreq, "-1"),
    Ok(Some(ConfigReconcile::CommitTask {
      commit_frequency_ms: -1
    }))
  );
  assert_eq!(config.get_int(ServerConfigType::AofCommitFreq), -1);
...
#[test]
fn cluster_node_timeout_zero_emits_reconcile() {
  let config = RuntimeServerConfig::with_defaults();
  assert_eq!(
    config.try_set(ServerConfigType::ClusterNodeTimeout, "0"),
    Ok(Some(ConfigReconcile::ClusterNodeTimeout { ms: 0 }))
  );
  assert_eq!(config.get_int(ServerConfigType::ClusterNodeTimeout), 0);

  // 负值越界拒绝（对标 C# min = 0）
  assert!(matches!(
    config.try_set(ServerConfigType::ClusterNodeTimeout, "-5"),
    Err(ConfigError::OutOfRange { .. })
  ));
  ...
  assert_eq!(
    config.try_set(ServerConfigType::ClusterNodeTimeout, "5"),
    Ok(Some(ConfigReconcile::ClusterNodeTimeout { ms: 5000 }))
  );
  assert_eq!(config.get_int(ServerConfigType::ClusterNodeTimeout), 5);
}
