...
use wconf::{
  DEFAULT_BIND, DEFAULT_DIR, DEFAULT_PORT, DEFAULT_RESP_VERSION, LogCompactionType, NodeArgs,
...
fn default_configuration_options_coverage() {
  ...
  assert_eq!(o.replay_drift_check_freq, 0);
  ...
  // TLS 面默认单向（入站不请求客户端证书、无 issuer 钉根，对标 C#
  // Options.cs:333 ClientCertificateRequired 的 bool? null 缺省）
...
fn test_tls_client_cert_required_knobs() {
  ...
  // 配置文件同路生效（nested_text 漏斗）
  let path = temp_dir().join("wedb-wconf-mtls-knob.nt");
