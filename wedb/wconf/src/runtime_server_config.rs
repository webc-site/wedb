...
impl RuntimeServerConfig {
  ...
  /// 以 RESP bulk string 格式将当前配置值直接写入输出流（零中间 String 分配）。
  pub fn write_resp_value(&self, type_: ServerConfigType, output: &mut Vec<u8>) {
    let meta = &META[type_ as usize];
    if meta.read_only {
      match type_ {
        ServerConfigType::Timeout => output.write_resp_bulk_string(b"0"),
        ServerConfigType::Save => output.write_resp_bulk_string(b""),
        ServerConfigType::AppendOnly => output.write_resp_bulk_string(if self.options.enable_aof {
          b"yes"
        } else {
          b"no"
        }),
        ServerConfigType::Databases => {
          let mut buf = itoa::Buffer::new();
          output.write_resp_bulk_string(buf.format(self.options.max_databases).as_bytes());
        }
        ServerConfigType::Dir => {
          output.write_resp_bulk_string(self.options.checkpoint_base_directory.as_bytes())
        }
        ServerConfigType::Logdir => {
          output.write_resp_bulk_string(self.options.log_dir.as_deref().unwrap_or("").as_bytes())
        }
        ServerConfigType::UnixSocket => output.write_resp_bulk_string(
          self
            .options
            .unix_socket_path
            .as_deref()
        ...
        ServerConfigType::ClusterEnabled => {
          output.write_resp_bulk_string(if self.options.enable_cluster {
            b"yes"
          } else {
            b"no"
          })
        }
        ServerConfigType::AofMemory => output.write_resp_bulk_string(
          self
            .options
            .aof_memory_size
            .as_deref()
        ...
        ServerConfigType::AofPageSize => output.write_resp_bulk_string(
          self
            .options
            .aof_page_size
            .as_deref()
        ...
        ServerConfigType::AofSegmentSize => output.write_resp_bulk_string(
          self
            .options
            .aof_segment_size
            .as_deref()
        ...
        ServerConfigType::AofPhysicalSublogCount => {
          ...
          output.write_resp_bulk_string(
            buf
              .format(self.options.aof_physical_sublog_count)
              .as_bytes(),
          );
        }
        ServerConfigType::AofReplayTaskCount => {
          let mut buf = itoa::Buffer::new();
          output.write_resp_bulk_string(buf.format(self.options.aof_replay_task_count).as_bytes());
        }
        ServerConfigType::AofCommitWait => {
          output.write_resp_bulk_string(if self.options.wait_for_commit {
            b"yes"
          } else {
            b"no"
          })
        }
        ServerConfigType::AofSizeLimit => output.write_resp_bulk_string(
          self
            .options
            .aof_size_limit
            .as_deref()
        ...
        ServerConfigType::FastAofTruncate => {
          output.write_resp_bulk_string(if self.options.fast_aof_truncate {
            b"yes"
          } else {
            b"no"
          })
        ...
        ServerConfigType::AofNullDevice => {
          output.write_resp_bulk_string(if self.options.use_aof_null_device {
            b"yes"
          } else {
            b"no"
          })
        }
        _ => output.write_resp_bulk_string(b""),
      }
      return;
    }
    ...
    match meta.kind & ConfigKind::STORAGE_MASK {
      ConfigKind::INT32 => {
        let mut buf = itoa::Buffer::new();
        output.write_resp_bulk_string(buf.format(raw as i32).as_bytes());
      }
      ConfigKind::INT64 => {
        let mut buf = itoa::Buffer::new();
        output.write_resp_bulk_string(buf.format(raw).as_bytes());
      }
      ConfigKind::BOOL => {
        output.write_resp_bulk_string(if raw != 0 { b"yes" } else { b"no" });
      }
      ConfigKind::ENUM => {
        if let Some(name) = meta.enum_type.and_then(|e| e.name_of(raw)) {
