...
///
/// libs/server/Config/RuntimeServerConfig.cs:Set
/// libs/server/Config/RuntimeServerConfig.cs:SetReadOnly
///（两个局部构建函数为条目铸造壳：C# 在 `BuildMeta` 内逐条调用，rust 以
/// `ConfigMeta::runtime` / `ConfigMeta::read_only` 两个构造子一一对应，
/// 条目铸造壳不再各占一个函数符号）
///
...
pub static META: [ConfigMeta; RuntimeServerConfig::TABLE_SIZE] = [
  ...
  // 22: ExpiredObjectCollectionFreq（0 = 禁用，缺省 0；副作用：兼分层键后台
  // 降阶评估轮唯一生产宿主任务的启动门，置 0 时降阶评估一并不跑，
  // doc/zh/collection.md 3.3 与 doc/zh/deviations.md §120）
  ConfigMeta::runtime(
...
/// 派生，键名不再二次硬编码（对位 C# `BuildNameLookup` 建表期由 Meta 派生的行为）。
/// 纯编译期常量，零运行时分配；`cluster-timeout` 兼容别名是全表唯一字面量。
///
/// libs/server/Config/RuntimeServerConfig.cs:BuildNameLookup
const NAME_LOOKUP_ALIAS: (&[u8], ServerConfigType) =
  (b"cluster-timeout", ServerConfigType::ClusterNodeTimeout);

const fn build_name_lookup() -> [(&'static [u8], ServerConfigType); RUNTIME_TYPES.len() + 1] {
  let mut tbl = [(NAME_LOOKUP_ALIAS.0, NAME_LOOKUP_ALIAS.1); RUNTIME_TYPES.len() + 1];
  let mut i = 0;
  while i < RUNTIME_TYPES.len() {
    let type_ = RUNTIME_TYPES[i];
    tbl[i] = (META[type_ as usize].name.as_bytes(), type_);
    i += 1;
  }
  tbl[RUNTIME_TYPES.len()] = (NAME_LOOKUP_ALIAS.0, NAME_LOOKUP_ALIAS.1);
  tbl
}

pub static NAME_LOOKUP: [(&[u8], ServerConfigType); RUNTIME_TYPES.len() + 1] = build_name_lookup();
...
impl RuntimeServerConfig {
  ...
  /// `Timeout.InfiniteTimeSpan`，RuntimeServerConfig.cs:308-321）：向消费面下发
  /// `ClusterNodeTimeout { ms: 0 }`（0 为无限超时哨兵，置入 ClusterProvider
  /// 原子槽，使 gossip / failover 消费面随动关闭计时器）。
  fn apply_cluster_node_timeout_update(
    ...
    if new_value <= 0 {
      return Ok(Some(ConfigReconcile::ClusterNodeTimeout { ms: 0 }));
    }
  ...
  /// 副作用随动：该收集任务是分层键后台降阶评估轮（wnode
  /// `tiered_demote_round`）唯一生产宿主，重启 / 停止本任务即同步开启 /
  /// 关闭冷分层键降阶评估（doc/zh/deviations.md §120）。
  ...
  #[inline]
  pub const fn seconds_from_time_span(ts_secs: i64) -> i64 {
    if ts_secs > 0 { ts_secs } else { 0 }
  }
  ...
  /// 首次构造 [`Self::new`] 时由 [`Self::validate_meta`] 对全部登记条目一次性
  /// 调用（对标 C# 建表期的静态校验），错误语义与 C# 的 InvalidOperationException
  /// 对齐。
  ...
  /// 一次性遍历静态元数据表，对全部 runtime 条目执行建表期校验。
  ///
  /// 对标 C# 在 `RuntimeServerConfig` 构造期对每项调用 `EnsureValidKind`
  /// /`EnsureSupportedEnum`（非法即抛 `InvalidOperationException`）。此处借
  /// [`Once`] 保证进程内仅校验一次，声明非法直接 `expect` panic。
  fn validate_meta() {
    ...
    VALIDATED.call_once(|| {
      for meta in META.iter() {
        ...
        Self::ensure_valid_kind(meta.kind, meta.time_unit)
          .expect("配置元数据 storage 类别或 duration 时间单位声明非法");
        if (meta.kind & ConfigKind::ENUM) != ConfigKind::NONE {
          Self::ensure_supported_enum(meta.enum_type).expect("ENUM 配置项未声明受支持的枚举类别");
        }
      }
    });
  }
