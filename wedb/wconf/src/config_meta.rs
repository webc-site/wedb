...
use crate::{
  ...
  runtime_server_config::RuntimeServerConfig, runtime_server_options::RuntimeServerOptions,
};
...
pub enum ConfigReconcile {
  ...
  /// rust 特有投影消息（C# 无对应调停：Gossip.cs:25 / GarnetServerNode.cs:90 /
  /// FailoverManager.cs:24 每轮 `GetTimeSpan/GetInt(CLUSTER_NODE_TIMEOUT)` 现取；
  /// rust 消费面统一读 ClusterProvider 原子槽，由本消息把刚落槽的
  /// cluster-node-timeout 投影进去，两侧超时随动同效）。`ms` 为秒槽新值
  /// 换算的毫秒数，仅承载**有限正间隔**：非正值经 `get_time_span` 归一为
  /// 无限超时（不挂计时器），不产本消息。
  ClusterNodeTimeout { ms: u64 },

  /// rust 特有投影消息（C# 无对应调停：DatabaseManagerBase.DoCompactionAsync
  /// 每轮 `GetInt(COMPACTION_MAX_SEGMENTS)` 现取；rust 引擎读 GcConfig 快照，
  /// 由本消息把刚落槽的 compaction-max-segments 投影进引擎 GcConfig，
  /// GcManager 每轮重读达成同效）。`max_segments` 为刚写入槽位的新值。
  CompactionMaxSegments { max_segments: i64 },
