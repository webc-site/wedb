...
impl NodeArgs {
  ...
  /// options.IsValid 的 IntRangeValidation 漏斗末端校验 + GarnetServer.cs:508
  ...
  ///
  /// C# GarnetServer.cs:519「CommitFrequencyMs < 0（手动提交）不可配
  /// WaitForCommit」在 rust 无对位：`aof_commit_ms` 为 Option<u8>，
  /// 手动态（-1）无 CLI / 配置文件编码通路，该组合无从出现。
