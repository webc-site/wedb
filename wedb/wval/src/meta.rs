...
pub enum StorageEncoding {
  ...
  #[default]
  Compact = 0,
  /// whlog 打平子键编码（离散 SubKey 散入主日志库，meta 记录仅 32B）
  Flattened = 1,
  /// BfTree 树算子编码（独立持久化树文件，meta 记录为 MetaValue + RangeIndexStub/ListStub 存根）
  FlattenedTree = 2,
}
...
impl StorageEncoding {
  ...
  /// 是否为打平存储（whlog 打平子键或 BfTree 树算子，统一秒删生命周期口径）
  #[inline(always)]
  pub const fn is_flattened(self) -> bool {
    matches!(self, Self::Flattened | Self::FlattenedTree)
  }
}
...
impl MetaValue {
  ...
  #[inline(always)]
  pub const fn new(key_id: u64, collection_type: GarnetObjectType, version: u64, size: u64) -> Self {
    Self {
      key_id,
      collection_type,
      reserved: [0u8; 7],
...
///
/// 供 [SubKeyCodec] 的头编解码复用，消除雷同数组字面量
#[inline(always)]
