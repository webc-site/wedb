...
pub enum KeyTag {
  ...
  /// 虚拟数据库元数据记录 (0x0E)
  DbMeta = 0x0E,
  /// 向量集合登记旁路记录 (0x0F)：key = 会话前缀 + 本标签 + [1B 子标签][负载]
  ///
  /// 子标签由本文件强类型枚举 [`VectorRegistrySubTag`] 单点定义与编解码
  /// （wnode 写透/回建侧与 wkv 紧缩豁免臂共用）：Index (0x01) = 索引登记，
  /// 负载 = 集合用户键，值 = Index 字节；Metadata (0x02) = 上下文元数据，
  /// 负载 = i32 LE 元数据下标，值 = ContextMetadata 字节，为全局单例恒驻
  /// 根域前缀 (0,0)，生死由登记表回收通道自治、豁免于业务死域判定（见
  /// wkv `compact.rs` is_deleted 豁免臂）。对标 C# VectorManager.RecordType
  /// 索引记录与 MetadataNamespace 元数据记录——两者在 C# 驻 Tsavorite 主存
  /// 随检查点持久（恢复趟 GarnetRecordTriggers.cs:OnRecoverySnapshotRead
  /// 逐记录回建登记的数据源）；rust 登记表驻内存，经该标签写透存储域达成
  /// 同等持久性
  VectorRegistry = 0x0F,
}
...
impl KeyTag {
  ...
  pub const fn as_str(self) -> &'static str {
    match self {
      ...
      Self::Acl => "Acl",
      Self::DbMeta => "DbMeta",
      Self::VectorRegistry => "VectorRegistry",
    }
  }
...
/// [`KeyTag::VectorRegistry`] 旁路记录子标签（对标 C# Tsavorite 记录头的
/// RecordType / Namespace 判别位；rust 侧登记记录自带 1B 子标签，一处定义
/// 强类型枚举，杜绝裸 const u8 散落）。
///
/// 编码单点在 wnode `resp::vector::vector_registry_recovery`（写透与回建
/// 共用）；wkv 紧缩豁免臂（compact.rs is_deleted）经本枚举判别 Metadata
/// 恒驻根域住户，两侧共用同一强类型真值源。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
pub enum VectorRegistrySubTag {
  /// 索引登记：负载 = 集合用户键，值 = Index 字节。
  Index = 0x01,
  /// 上下文元数据：负载 = i32 LE 元数据下标，值 = ContextMetadata 字节。
  /// 元数据为全局单例，恒落根域前缀 (0,0)。
  Metadata = 0x02,
}

impl VectorRegistrySubTag {
  /// 从 1 字节整数解析子标签（未定义值即 None，扫描侧跳过）。
  #[inline]
  pub const fn from_u8(v: u8) -> Option<Self> {
    match v {
      0x01 => Some(Self::Index),
...
pub enum GarnetObjectType {
  ...
  RangeIndex = 5,
  /// C# 原生保留的泛型扫描哨兵（0xfb）：COSCAN/CUSTOMOBJECTSCAN 经
  /// header.type == All 下传，仅自定义对象 `CustomObjectBase.Operate`
  /// 接受并转对象 Scan（libs/server/Custom/CustomObjectBase.cs:77-89）；
  /// 内置对象类型不匹配一律 WrongType（HashObject.cs:225-231 /
  /// SetObject.cs:126-135 同型严格检查）——并非「通配任意对象类型」
  #[strum(serialize = "all")]
  All = 0xfb,
}
...
impl GarnetObjectType {
  ...
  /// 分层域判据：可驻留 KeyTag::Meta 复合元记录 `[MetaValue][RangeIndexStub]`
  /// 的集合类型（const fn）
  ///
  /// RI.CREATE 显式索引与自适应升阶后的通用集合（Hash/Set/ZSet/List）共用
  /// Meta 域与同一存根形态（见 doc/zh/collection.md 与 wkv session/collection.rs
  /// 域文档：内存态通用对象只驻信封域，绝无独立 Meta 记录）。存根治愈内核
  /// （wkv range_index 的 OnFlush 置位 / RIPROMOTE / RIRESTORE）以此判定目标
  /// 记录，不得收窄为 `== RangeIndex`——那会把升阶集合元记录整体排除出治愈
  /// 链，升阶树刷盘置位、摘树恢复、句柄重绑全部静默失效
  #[inline(always)]
  pub const fn is_tiered_collection(self) -> bool {
    matches!(
      self,
      Self::RangeIndex | Self::Hash | Self::Set | Self::SortedSet | Self::List
    )
  }
}
