...
pub enum KeyTag {
  ...
  ///
  /// 子标签 0x01 = 索引登记：负载 = 集合用户键，值 = Index 字节；子标签 0x02 =
  /// 上下文元数据：负载 = i32 LE 元数据下标，值 = ContextMetadata 字节。对标
  /// C# VectorManager.RecordType 索引记录与 METADATA_NAMESPACE 元数据记录——
  /// 两者在 C# 驻 Tsavorite 主存随检查点持久（恢复趟
  /// GarnetRecordTriggers.cs:OnRecoverySnapshotRead 逐记录回建登记的数据源）；
  /// rust 登记表驻内存，经该标签写透存储域达成同等持久性
  VectorRegistry = 0x0F,
}
