//! 命名空间/DB 会话前缀与标签物理键编解码器
//!
//! 物理键编码必须字节稳定、前缀封闭且保序（KV 键空间按字典序扫描与范围裁剪），
//! bitcode 非保序格式故不适用，属 C# 逐字节对标 + 物理键域双重理由的手写布局。
...
use wbase::{
  ...
  simd::fast_key_eq,
  varint::{
    VARINT_1B_FIRST_BYTE_LIMIT, VARINT_1B_MAX, VARINT_LEN_LUT, VarintError, decode_u64,
    encode_u64_to_array, varint_len,
  },
};
...
impl SessionPrefixBuf {
  ...
  /// 根域 (0, 0) 会话前缀（varint(0) 两字节 0x00；wkv 根域两态数值恒等
  /// 的物理形态，测试与非会话面缺省域单点引用，杜绝散落魔数）
  pub const ROOT: Self = Self {
    buf: [0u8; MAX_SESSION_PREFIX_LEN],
    len: 2,
  };
...
// 物理键内部存储排布表示（62 字节栈容量上限）
pub type KeyBufRepr = StackHeapBuf<STACK_KEY_CAP>;
...
impl TaggedKeyBuf {
  ...
  pub const fn from_stack(buf: [u8; STACK_KEY_CAP], len: u8) -> Self {
    Self {
      inner: KeyBufRepr::from_stack(buf, len),
    }
  }
  ...
  pub const fn from_heap(vec: Vec<u8>) -> Self {
    Self {
      inner: KeyBufRepr::from_heap(vec),
    }
  }
  ...
  /// 原位替换指定偏移的键标签字节（栈/堆双态零拷贝）
  ///
  /// 与 [`NamespaceDbCodec::replace_tag_at`] 配套：同键多标签形态重复探查时，
  /// 一次拷贝构造探查缓冲后原位轮换标签字节，消除多次全键复制
  ///
  /// # Panics
  /// `tag_offset` 越界（不在有效键长范围内）时 panic
  #[inline]
  pub fn set_tag_at(&mut self, tag_offset: usize, new_tag: KeyTag) {
    assert!(tag_offset < self.len(), "tag_offset 越界");
    self.inner.as_mut_slice()[tag_offset] = new_tag.as_u8();
  }
}
...
impl NamespaceDbCodec {
  ...
  ///
  /// OPPV 真值单点在 [`wbase::varint::varint_len`]
  #[inline(always)]
  pub const fn session_prefix_len(ns: u64, db: u64) -> usize {
    varint_len(ns) + varint_len(db)
  }
  ...
  ///
  /// OPPV 真值单点在 [`wbase::varint::encode_u64_to_array`]
  #[inline]
  pub const fn encode_session_prefix_to_array(
    ...
    let (ns_buf, ns_len) = encode_u64_to_array(ns);
    let (db_buf, db_len) = encode_u64_to_array(db);
    let mut buf = [0u8; MAX_SESSION_PREFIX_LEN];
    let mut i = 0;
    while i < ns_len {
      buf[i] = ns_buf[i];
  ...
  #[inline(always)]
  const fn varint_len_from_byte(first: u8) -> Option<usize> {
  ...
  ///
  /// [`wbase::varint::decode_u64`] 的 wval 错误面适配（VarintError → [`Error`]）。
  /// pub 供复合登记键剥离单点（VectorManager 登记表）解码前缀域值
  #[inline]
