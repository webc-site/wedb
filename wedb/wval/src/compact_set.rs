//! 紧凑连续内存无序集合（字典序物理排列）容器与零拷贝编解码器
//!
//! 对标 garnet/libs/server/Objects/Set/SetObject.cs（C# 为 HashSet，无序）；
//! 本布局改按成员字典序物理排列，以支持二分查找与有序合并，语义仍为集合。
//!
//! 不用 bitcode 而手写逐字节布局的理由：热路径要求零拷贝切片迭代与
//! 原位插入/删除（免全量重编码），属性能关键路径例外。
...
impl CompactSetCodec {
  ...
  pub fn parse_entry(slice: &[u8], offset: usize) -> Result<(usize, &[u8])> {
    ...
    let Some(m_end) = m_start.checked_add(m_len) else {
      return Err(Error::BufferTooShort {
        expected: usize::MAX,
        actual: slice.len(),
      });
  ...
  pub fn insert(buf: &mut Vec<u8>, member: &[u8]) -> Result<bool> {
    ...
    if insert_pos == old_len {
      ...
      unsafe {
        ...
        ptr::copy_nonoverlapping(len_bytes.as_ptr(), p.add(insert_pos), COMPACT_SET_LEN_SIZE);
