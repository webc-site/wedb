...
use log::info;
use wval::{
  GarnetObjectType, CompactHash, CompactHashCodec, CompactSet, CompactSetCodec, CompactZSet,
...
fn test_compact_zset_count_range_and_streaming_iter() -> Void {
  ...
  // 全区间 O(1) 短路与开闭区间 / NaN 边界
  assert_eq!(zset.count_range(f64::NEG_INFINITY, f64::INFINITY), 5);
  assert_eq!(zset.count_score_range(20.0, false, 40.0, false), 1); // 仅 m30
  assert_eq!(zset.count_score_range(f64::NAN, true, 40.0, true), 0);
  assert_eq!(zset.count_score_range(20.0, true, f64::NAN, true), 0);
...
fn test_compact_zset_batch_encode_dedup_and_order() -> Void {
  ...
  // 空 batch 仅含计数前缀
  let empty = CompactZSetCodec::encode(Vec::<(f64, &[u8], Option<i64>)>::new())?;
  assert_eq!(empty.len(), 2);
  assert_eq!(CompactZSetCodec::count(&empty)?, 0);

  // 条目级过期批量编码往返（对标 C# SortedSetObject ExpirationBitMask 序列化）
  let exp_entries = vec![
    (5.0, b"a".as_slice(), Some(1_700_000_000_000i64)),
    (1.0, b"b".as_slice(), None),
    (5.0, b"a".as_slice(), Some(1_800_000_000_000i64)), // 后写覆盖过期时间
  ];
  ...
  assert_eq!(entries[0].expire_at_ticks, None);
  assert_eq!(entries[1].member, b"a");
  assert_eq!(entries[1].expire_at_ticks, Some(1_800_000_000_000i64));
...
fn test_compact_zset_member_expiration() -> Void {
  ...
  let entries: Vec<ZSetEntryRef> = zset.iter_members().collect();
  assert_eq!(entries[0].expire_at_ticks, Some(now - 1));
  assert_eq!(entries[1].expire_at_ticks, Some(now + 100));
  assert_eq!(entries[2].expire_at_ticks, None);
