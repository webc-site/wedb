...
fn test_key_tag_roundtrip_and_properties() -> Void {
  ...
  // 非法 tag 校验（0x0c 已分配给 ObjectEnvelope，0x0d 起空闲）
  assert_eq!(KeyTag::from_u8(0x0d), None);
  assert_eq!(KeyTag::from_repr(0x0d), None);
  assert_eq!(KeyTag::from_u8(0xff), None);
  // 对象信封为用户可见逻辑键
  assert!(KeyTag::ObjectEnvelope.is_user_visible());
  assert!(KeyTag::ObjectEnvelope.is_object_envelope());
  assert!(!KeyTag::String.is_object_envelope());
