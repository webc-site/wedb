...
use log::info;
use wval::{
  COMPACT_META_VALUE_SIZE, GarnetObjectType, CompactMetaValue, Error, KeyTag, META_VALUE_SIZE,
...
fn test_meta_value_roundtrip_and_serialization() -> Void {
  ...
  // 严格解码面：已知三值逐一命中、未知值 None
  assert_eq!(
    StorageEncoding::try_from_u8(0),
    Some(StorageEncoding::Compact)
  );
  assert_eq!(
    StorageEncoding::try_from_u8(1),
    Some(StorageEncoding::Flattened)
  );
  assert_eq!(
