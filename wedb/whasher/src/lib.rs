//! 高性能哈希原语与流式校验和库（全量基于 AES 硬件向量加速的 `gxhash` 后端）
//!
//! # 与 `wbase::hash` 的职能分工
...
//! - [`fast_hash_i64`] ← `libs/server/AOF/GarnetLog.cs 的 HASH 位面`
//!   （`(long)HashBytes(key)` 的 i64 位面重解释单点：AOF 分片/一致读/事务锁序
//!   跨层同键同哈希，各域一律转引本原语）
...
/// （C# 向量域 `VectorManager.Locking.cs` 与 RangeIndex 域 `RangeIndexManager.*.cs` 均经
/// GetKeyHash → `GarnetKeyComparer` → `SpanByteComparer.StaticGetHashCode64` 单源至此）；
/// 向量键条带锁与 RI 条带锁两域键哈希一处引用，迁移 Sketch 槽位等场景同此承担。
/// 注意：Redis 逐位兼容的 HyperLogLog 必须使用
...
pub fn fast_hash(bytes: &[u8]) -> u64 {
  // 标量单值重载（对 8 字节输入即等价 C# 单 long 混洗）：
  // libs/storage/Tsavorite/cs/src/core/Utilities/Utility.cs:GetHashCode
...
/// 键哈希 i64 位面单点原语（[`fast_hash`] 的有符号视角；除此之外全仓禁止
/// 再写 `fast_hash(..) as i64`）
///
/// C# 对位:libs/server/AOF/GarnetLog.cs 的 HASH 位面重解释(正式锚点由 wnode aof 域 GarnetLog::hash 单点持有)
///
/// C# `public static long HASH(byte[] key)` 即 `(long)HashBytes(key)` 的位面
/// 重解释：AOF 分片（GarnetLog）、一致读（ConsistentReadContext.GetKeyHash）、
/// 事务锁序（UnifiedTransactionalContext.GetKeyHash）三面同键同哈希。rust 侧
/// 跨层（wnode aof / wkv 一致读 / wtxn 事务）必须单点同源，杜绝各处独立
/// `as i64` 漂移；`as i64` 为 100% 安全的补码位面重解释，无值域损失。
#[inline(always)]
pub fn fast_hash_i64(bytes: &[u8]) -> i64 {
  fast_hash(bytes) as i64
}
...
/// 归属域 scoped 键哈希单点构造（键级桶闩会合域全仓唯一口径）
///
/// `fast_hash_with_seed(key, fast_hash(prefix))`：以会话**物理**前缀
///（`[NsVarint][DbVarint]`，真值源 wkv `StoreSession::session_prefix`）的
/// 固定种子哈希为种子混入裸键体，不同 (ns, db) 同名键落位正交，零堆分配。
/// 对标 C# 每库独立 Tsavorite 实例与锁表的库内天然隔离
///（`libs/server/GarnetDatabase.cs` / `MultiDatabaseManager`），rust 单物理
/// 引擎共享全服一张索引锁面（doc/zh/db.md 前缀刚性隔离）下键身份自带归属
/// 维度的等价承接。
///
/// 全仓键级锁面/会合域寻桶只准经本函数构槽，消费方（禁第二处手写拼接）：
/// - `wtxn::TxnKeyEntryComparison::scoped_key_hash`（事务键锁登记，i64 位面）；
/// - `wkv` rmw 读改写窗口（`rmw_window.rs` 单键/多键计划）与 TTL 键闩
///   （`ttl.rs` 经 `HashIndex::try_lock_key_hash_exclusive` 同源哈希寻桶）。
#[inline(always)]
pub fn scoped_hash(prefix: &[u8], key: &[u8]) -> u64 {
  fast_hash_with_seed(key, fast_hash(prefix))
}
