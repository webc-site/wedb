如需定义错误（不需要就算了），用 thiserror 在 src/error.rs 中定义，并定义 Result，在 lib.rs 中 `use error::{Result, Error};`，避免使用字符串作为错误
合理划分模块，暴露接口要低耦合，高内聚
避免手写，尽可能复用库
...
如果需要一个并发读写的字典, 用工作区的 `wbase::map`（本 crate 只做哈希与校验和原语，不封装容器）
