...
//!
//! 自研依据: JSONPath 执行器（C# 对应 JSONPath 执行语义）
...
///
/// C# 原用例文档即单元素数组 [{"b":...}]（过滤器走 QueryFilter 数组遍历元素臂）；
/// .NET RegexMatchTimeoutException 超时机制在 rust regex（NFA 无回溯爆炸）无对位，
/// 超时断言臂不移植，此处仅对标单命中产出形态。
#[test]
...
fn evaluate_wildcard_scan() {
  // 对标 C# EvaluateWildcardScan：根为对象数组的 $..*，逐节点先序 DFS 匹配
  let json = r#"[
    { "Name": 1 },
    { "Name": 2 }
  ]"#;
  ...
  assert_eq!(matches[0]["Name"].as_i64(), Some(1));
  assert_eq!(matches[1].as_i64(), Some(1));
  assert_eq!(matches[2]["Name"].as_i64(), Some(2));
  assert_eq!(matches[3].as_i64(), Some(2));
}
...
fn evaluate_wildcard_scan_nest_results() {
  // 对标 C# EvaluateWildcardScanNestResults：三层嵌套的 $..*，
  // 断言根数组元素与递归元素全部匹配且顺序为先序 DFS
  let json = r#"[
    { "Name": 1 },
    { "Name": 2 },
    { "Name": { "Name": [3] } }
  ...
  assert_eq!(matches[2]["Name"].as_i64(), Some(2));
  assert_eq!(matches[3].as_i64(), Some(2));
  assert!(matches[4]["Name"].is_object());
  assert!(matches[5]["Name"].is_array());
  assert!(matches[6].is_array());
  assert_eq!(matches[7].as_i64(), Some(3));
}

/// test/standalone/Garnet.test.extensions/GarnetJSON/JSONPath/JsonPathExecuteTests.cs:EvaluateWildcardScan
#[test]
fn scan_wildcard_array_root() {
  // 根为纯数组标量 [1,2,3]，$..* 必须命中所有元素
  let json = r#"[1, 2, 3]"#;
  let val: Value = sonic_rs::from_str(json).unwrap();
  let path = JsonPath::parse("$..*").unwrap();
  let matches = path.evaluate(&val);
  ...
  assert_eq!(matches[1].as_i64(), Some(2));
...
///
/// C# 原断言双路径：$..book[?(@.price <= $['expensive'])] 回 2、
/// $.store..[?(@.price > $.expensive)] 回 3（过滤器上下文皆为数组，
/// 谓词内 $ 引用按文档根求值）。
#[test]
fn root_in_filter_with_root_object() {
  ...
  let val: Value = sonic_rs::from_str(json).unwrap();
  let result = JsonPath::parse("$..book[?(@.price <= $['expensive'])]").unwrap();
  assert_eq!(result.evaluate(&val).len(), 2);
  let result = JsonPath::parse("$.store..[?(@.price > $.expensive)]").unwrap();
  assert_eq!(result.evaluate(&val).len(), 3);
}

/// test/standalone/Garnet.test.extensions/GarnetJSON/JSONPath/JsonPathExecuteTests.cs:RootInFilterWithInitializers
///
/// C# 原断言：$.dateObjectsArray[?(@.date == $.referenceDate)] 回 2
/// （数组上下文过滤器，谓词根引用取 referenceDate）。
#[test]
fn root_in_filter_with_initializers() {
  ...
  let val: Value = sonic_rs::from_str(json).unwrap();
  let path = JsonPath::parse("$.dateObjectsArray[?(@.date == $.referenceDate)]").unwrap();
  assert_eq!(path.evaluate(&val).len(), 2);
}
