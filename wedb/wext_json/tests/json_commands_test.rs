...
//!
//! 自研依据: JSON 命令端到端（C# 对应 modules/JSON 模块测试）

use std::str::from_utf8;

use sonic_rs::JsonValueTrait;
use wext_json::{GarnetJsonObject, RESP_NEW_OBJECT_AT_ROOT, RESP_WRONG_STATIC_PATH, SetResult};
...
/// 对标 C# JsonCommandsTest.JsonSetPathOperations（1:1 对位映射由同文件
/// json_set_path_operations 单点持有，本用例为衍生扩展）
/// 的 Add new field 语义扩至根数组与引号属性路径：修复
/// C# GarnetJsonObject.GetParentPath / GetPropertyName
/// 只回看 '.'、']' 造成 `$[N]`、`$['k']`、`$["k"]` 落入 ("$", 0) 兜底，
/// 属性名残留 `$` 前缀导致写入错误字面量的断链
#[test]
fn json_set_root_array_and_quoted_path() {
  for (path, new_val, initial, expected) in [
    // 根数组尾插（Evaluate 空，走 GetParentPath 兜底）
    ("$[2]", "99", "[1,2]", "[1,2,99]"),
    ("$[1]", "99", "[1]", "[1,99]"),
    ("$[0]", "99", "[]", "[99]"),
    // 根对象引号属性：单引号 / 双引号
    ...
    let res = obj
      .set(path.as_bytes(), new_val.as_bytes(), ExistOptions::None)
      .unwrap();
    assert_eq!(res, SetResult::Success, "path={path}");
    let mut out = Vec::new();
    obj.try_get(&[], &mut out, None, None, None, 2).unwrap();
    assert_resp_json_eq(&out, expected);
  }
...
/// 单元级锁定 GarnetJsonObject::get_parent_path / get_property_name 的
/// 分隔符回溯与 '[]'/引号剥离，覆盖根直插、点分、方括号、混合形态
#[test]
fn get_parent_path_and_property_name_split() {
  use wext_json::GarnetJsonObject;
  for (path, expected_parent, expected_prop) in [
    ("$[2]", "$", "2"),
    ("$[0]", "$", "0"),
    ("$['foo']", "$", "foo"),
    ("$[\"bar\"]", "$", "bar"),
    ...
    let (parent, offset) = GarnetJsonObject::get_parent_path(path);
    let prop = GarnetJsonObject::get_property_name(path, offset);
    assert_eq!(parent, expected_parent, "path={path} parent");
    assert_eq!(prop, expected_prop, "path={path} prop");
  }
}
...
/// 对标 C# JsonPathExecuteTests.cs:EvaluateWildcardScan 的写侧
/// —— JSON.SET $..* 命中根数组元素并按先序替换全部标量叶子
#[test]
fn json_set_wildcard_scan_hits_array_elements() {
  let mut obj = GarnetJsonObject::create();
  obj.set(b"$", br#"[1, 2, 3]"#, ExistOptions::None).unwrap();
  let res = obj.set(b"$..*", br#""x""#, ExistOptions::None).unwrap();
  assert_eq!(res, SetResult::Success);
  let mut out = Vec::new();
  obj.try_get(&[b"$"], &mut out, None, None, None, 2).unwrap();
  assert_resp_json_eq(&out, "[[\"x\",\"x\",\"x\"]]");
}

/// 对标 C# JsonPathExecuteTests.cs:EvaluateWildcardScanNestResults 的删侧
/// —— JSON.DEL $..* 命中根数组元素并清空对象内嵌套数组
#[test]
fn json_del_wildcard_scan_hits_array_elements() {
  let mut obj = GarnetJsonObject::create();
  obj
    .set(
      b"$",
    ...
    .unwrap();
  let n = obj.del(Some(b"$..*"));
  // 顶层对象清空 2 个字段：a 与 b（数组/对象作为直接子节点先被 obj.clear() 吞掉）
  assert_eq!(n, 2);
  let mut out = Vec::new();
  obj.try_get(&[], &mut out, None, None, None, 2).unwrap();
  assert_resp_json_eq(&out, "{}");
}

/// JSON.DEL $..* 直接命中根数组：清空全部元素
#[test]
fn json_del_wildcard_scan_array_root() {
  let mut obj = GarnetJsonObject::create();
  obj
    .set(b"$", br#"[10, 20, 30]"#, ExistOptions::None)
    .unwrap();
  let n = obj.del(Some(b"$..*"));
  assert_eq!(n, 3);
  let mut out = Vec::new();
