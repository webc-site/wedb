...
use crate::{
  EpochAccessor, EpochEntry, Error, Participant, ProtectedScope, Result,
  tls::{
...
/// 测试钩子读侧（对位 C# 独立分部文件
/// libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.TestHooks.cs：五口全 `internal`，
/// 文件头明示「Read-only views of LightEpoch's internal state, used only by the unit tests
/// in Tsavorite.test.epoch」，即「独立文件门 + internal 可见性门」双门）。
///
