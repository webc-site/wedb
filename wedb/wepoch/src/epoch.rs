...
impl LightEpoch {
  ...
  ///
  /// 用于 `wepoch::wait_condition_async` 与 `help_drain` 中解除本线程对旧纪元的自钉
  #[inline]
...
/// Display 共用：将元素以逗号连接写入列表，空迭代器输出占位文本
fn join<T>(
  f: &mut fmt::Formatter<'_>,
  mut items: impl Iterator<Item = T>,
  render: impl Fn(&mut fmt::Formatter<'_>, &T) -> fmt::Result,
  none: &str,
) -> fmt::Result {
...
impl fmt::Display for LightEpoch {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    join(
      f,
      self.entries.iter().filter(|e| e.thread_id() != 0),
      |f, e| write!(f, "tid={} epoch={}", e.thread_id(), e.local_current_epoch()),
      "none",
    )?;
    writeln!(f, "]")?;
    write!(f, "DrainList: [")?;
    join(
      ...
      self
        .drain_list
        .iter()
        .map(|d| d.epoch.load(Ordering::Relaxed))
        .filter(|&ep| ep != DRAIN_ENTRY_FREE),
      |f, ep| write!(f, "epoch={}", ep),
      "none",
    )?;
    write!(f, "]")
  }
