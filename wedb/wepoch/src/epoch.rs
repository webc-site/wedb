...
use std::{
  ...
  sync::{
    Arc,
    atomic::{AtomicI64, AtomicU32, AtomicU64, AtomicUsize, Ordering, fence},
  },
...
use crate::{
  EpochAccessor, EpochEntry, Error, MAX_USER_WORDS, Participant, ProtectedScope, Result,
  tls::{
...
pub struct LightEpoch {
  ...
  /// 用户字已占用槽位掩码（CAS 原子分配与回收，对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:userWordMask）
...
impl LightEpoch {
  ...
  pub fn new(max_threads: usize) -> Self {
    ...
    let entries: Box<[EpochEntry]> = repeat_with(EpochEntry::new).take(max_threads).collect();
  ...
  /// 若当前线程尚未受保护则进入并返回 true，否则返回 false
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:ResumeIfNotProtected
  pub fn resume_if_not_protected(self: &Arc<Self>) -> bool {
    if self.this_instance_protected() {
      false
    } else {
      self.resume();
  ...
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:SuspendResume
  pub fn suspend_resume(self: &Arc<Self>) {
    self.suspend();
    self.resume();
  }
  ...
  /// 别名：进入受纪元保护的作用域，返回 RAII 守卫（委托 [`Self::protected_scope`]）
  #[inline]
  pub fn enter_epoch<'a>(self: &'a Arc<Self>) -> ProtectedScope<'a> {
    self.protected_scope()
  }

  /// 进入受纪元保护的作用域，返回 RAII 守卫（委托 [`Self::protected_scope`]）
  #[inline]
  pub fn enter<'a>(self: &'a Arc<Self>) -> ProtectedScope<'a> {
    self.protected_scope()
  }

  /// 刷新本线程（当前调用线程）在本实例以任一机制（TLS 作用域或 `Participant` 会话）
  /// 持有的全部保护条目至当前纪元，并在有就绪延迟动作时协助收割
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:ProtectAndDrain
  /// 刷新语义：长期不刷新的持有者会拖延全局回收进度。C# 单一保护机制下刷新本线程
  /// 唯一条目即完整覆盖；Rust 双轨机制下同一线程可能同时持有多条保护条目，必须
  /// 全量刷新——漏看任一旧纪元条目将自钉 safe_to_reclaim 推进，drain_list 耗尽后
  /// [`Self::bump_current_epoch_action`] 的注册路径永久自旋（活锁）。
  ///
  /// 安全性契约与 C# ProtectAndDrain 一致：仅属主线程可刷新自身条目，且调用方须保证
  /// 刷新窗口内不再以旧纪元访问数据（wedb 调用点均在单次屏障调用内闭环消费）。
  /// 仅 TLS 单槽刷新用 [`Self::protect_and_drain`]，仅单个 Participant 条目刷新用
  /// [`Participant::refresh`]。
  #[inline]
  pub fn refresh_participant(&self) {
    self.help_drain();
  }
  ...
  ///
  /// 用于 wait_epoch_condition、unpin_self 与 help_drain 中解除本线程对旧纪元的自钉
  #[inline]
  ...
  fn help_drain(&self) {
    ...
    let tid = current_thread_id();
    self.refresh_thread_protected_entries(tid, current);
    self.drain();
  }
  ...
  pub fn bump_and_wait(&self, target_epoch: u64) {
    ...
    // 活锁防御：若当前调用线程自身名下持有 ≤ target_epoch 的保护纪元（如调用方误在保护区内触发 bump_and_wait），
    // 自动刷新至最新 current_epoch，彻底消除 release 构建下因自身钉住目标纪元导致的永久自旋活锁
    let tid = current_thread_id();
    let cur = self.current_epoch.load(Ordering::Acquire);
    for entry in self.entries.iter() {
      if entry.is_protected() && entry.thread_id() == tid && entry.protected_epoch() <= target_epoch
      {
  ...
  /// 获取延迟清理列表总容量（对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.TestHooks.cs:TestHookDrainListCapacity）
  #[inline]
  ...
  /// 获取支持的最大用户字槽位数量（对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:MaxUserWords）
  #[inline]
  pub fn test_hook_max_user_words(&self) -> usize {
    MAX_USER_WORDS
  }

  /// 分配一个全局用户字槽位并将其初始化为 initial_value
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:AllocateUserWord
  ...
  /// 释放先前分配的用户字槽位
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:ReleaseUserWord
  ...
  /// 获取当前线程对应用户字的原子引用
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:ThisThreadUserWord
  ...
  /// 扫描所有活跃条目并返回指定用户字的最小值
  ///
  /// 对照 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:GetMinUserWord
...
impl Default for LightEpoch {
  fn default() -> Self {
    Self::new(Self::default_table_size())
  }
}
...
/// 严格对标 libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs:ToString
impl fmt::Display for LightEpoch {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    writeln!(
      f,
      "CurrentEpoch: {}, SafeToReclaimEpoch: {}",
    ...
    for e in self.entries.iter() {
      ...
      if tid != 0 {
        ...
        write!(f, "tid={} epoch={}", tid, e.local_current_epoch())?;
        first = false;
      }
    }
    if first {
    ...
    write!(f, "DrainList: [")?;
    let mut first = true;
    for d in self.drain_list.iter() {
      let ep = d.epoch.load(Ordering::Relaxed);
      if ep != DRAIN_ENTRY_FREE {
        if !first {
          write!(f, ", ")?;
        }
        write!(f, "epoch={}", ep)?;
...
impl EpochAccessor for Arc<LightEpoch> {
  #[inline]
