...
use crate::{EpochEntry, LightEpoch};
  ...
  fn drop(&mut self) {
      ...
      .chain(self.overflow.iter_mut())
    {
      if state.active_entry != 0
        && let Some(epoch) = state.epoch.as_ref().and_then(Weak::upgrade)
        && state.active_entry <= epoch.entries.len()
      {
        epoch.entries[state.active_entry - 1].reset();
        need_fence = true;
      }
...
impl LocalEpochEntries {
  ...
  where
    F: FnOnce() -> Weak<LightEpoch>,
  {
    if let Some(item) = self.find_mut(instance_id) {
      item.active_entry = entry;
      if entry != 0 {
        item.cached_slot = entry;
        if item.epoch.as_ref().is_none_or(|e| e.strong_count() == 0) {
          item.epoch = Some(get_weak());
        }
      }
      return;
    }
    ...
    if self.count < MAX_LOCAL_ENTRIES {
      ...
      if self.overflow.len() >= MAX_OVERFLOW_STATES {
        self.overflow.retain(|s| {
          s.active_entry != 0 || s.epoch.as_ref().is_some_and(|w| w.strong_count() > 0)
        });
      }
  ...
  let resolved = THREAD_LOCAL_ENTRIES.with(|cell| {
    ...
    if entry != 0 {
      entries
        ...
        .and_then(|s| s.epoch.as_ref())
        .and_then(Weak::upgrade)
        .filter(|epoch| entry <= epoch.entries.len())
        .map(|epoch| ptr::from_ref(&epoch.entries[entry - 1]))
    } else {
      None
    }
  });
