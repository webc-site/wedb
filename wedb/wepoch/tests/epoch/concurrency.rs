//! 高并发竞争、槽位耗尽与多实例压力测试
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs（Suspend/Resume/Drain 语义）+ test/EpochProtectedVersionScheme.cs
...
fn concurrent_readers_and_epoch_advance_stress() -> Void {
  ...
  // 主线程推进纪元并检测安全回收
  for _ in 0..ITERATIONS {
    let curr = epoch.bump_current_epoch();
    assert!(curr >= 2);
  }
