...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/src/core/Epochs/LightEpoch.cs（保护域/槽位）
...
use aok::{OK, Void};
use log::info;
use wepoch::{EpochEntry, EpochSuspendGuard, Error, LightEpoch, ProtectedScope};
...
/// try_suspend 幂等性与正确的布尔返回值
///（对标 libs/storage/Tsavorite/cs/test/test.epoch/ProtectionTests.cs:ResumeIfNotProtectedIsIdempotent
/// 的幂等语义；阻塞让出形态 ResumeIfNotProtected 随 compio 单线程模型未移植）
#[test]
fn try_suspend_is_idempotent() -> Void {
  info!("验证 TrySuspend 幂等性");
...
fn participant_capacity_and_slot_recycling() -> Void {
  ...
  // EpochEntry 严格 64 字节 Cacheline 对齐（防伪共享的结构底线）
  assert_eq!(size_of::<EpochEntry>(), 64);
...
/// 验证挂起守卫 Drop 后按原深度恢复保护区
///
/// 对标 libs/storage/Tsavorite/cs/src/core/Allocator/AllocatorBase.cs 长 I/O 临界区的
/// UnsafeSuspendThread/ResumeThread 配对：挂起窗口内解除保护，窗口结束须回到挂起前的
/// 重入深度与保护态，否则外层保护区被静默拆掉，后续重试窗口处于无保护态。
#[test]
fn suspend_guard_restores_reentrant_depth() -> Void {
  info!("验证 EpochSuspendGuard Drop 后恢复挂起前的重入深度与保护态");
  ...
  let g2 = p.enter();
  let g3 = p.enter();
  assert_eq!(p.reentrant_count(), 3);
  assert!(p.is_protected());

  {
    let _guard = EpochSuspendGuard::new(&p);
    assert_eq!(p.reentrant_count(), 0);
    assert!(!p.is_protected());
  }
