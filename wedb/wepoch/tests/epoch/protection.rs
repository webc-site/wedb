...
use aok::{OK, Void};
use log::info;
use wepoch::{EpochAccessor, EpochEntry, Error, LightEpoch, ProtectedScope};
...
fn deref_ergonomics() -> Void {
  ...
  // 验证 ProtectedScope 的 Deref<Target = LightEpoch> 与 enter_epoch 别名
  {
    let scope = epoch.enter_epoch();
  ...
  // 验证 EpochGuard 的 Deref<Target = Participant> 与 enter_epoch 别名
  let p = epoch.register()?;
  let word_idx = epoch.allocate_user_word(42)?;
  {
    let guard = p.enter_epoch();
...
/// 验证 EpochAccessor trait 以及 Display / local_current_epoch 1:1 对标实现
#[test]
fn epoch_accessor_and_display() -> Void {
  info!("验证 EpochAccessor trait 与 Display 格式化输出");

  let default_epoch = LightEpoch::default();
  assert!(default_epoch.entry_count() >= 128);
  assert_eq!(
    default_epoch.entry_count(),
    LightEpoch::default_table_size()
  );

  let epoch = Arc::new(LightEpoch::new(8));

  #[inline]
  fn verify_accessor<T: EpochAccessor + ?Sized>(accessor: &T, is_protected: impl Fn() -> bool) {
    assert!(!accessor.try_suspend());
    accessor.resume();
    assert!(is_protected());
    assert!(accessor.try_suspend());
    assert!(!is_protected());
  ...
  // 验证 LightEpoch 的 EpochAccessor
  verify_accessor(&epoch, || epoch.this_instance_protected());

  // 验证 Participant 的 EpochAccessor
  let p = epoch.register()?;
  verify_accessor(&p, || p.is_protected());

  // 验证 Entry local_current_epoch 与 Display
  let guard = p.enter();
  let entry = &epoch.entries[p.entry_idx()];
  assert_eq!(entry.local_current_epoch(), epoch.current_epoch());
  let entry_str = format!("{entry}");
  assert!(entry_str.contains("lce = 1"));
  assert!(entry_str.contains("tid = "));

  // 验证 LightEpoch Display 格式化
  let epoch_str = format!("{epoch}");
  assert!(epoch_str.contains("CurrentEpoch: 1, SafeToReclaimEpoch: 0"));
  assert!(epoch_str.contains("Threads: ["));
  assert!(epoch_str.contains("DrainList: [none]"));
