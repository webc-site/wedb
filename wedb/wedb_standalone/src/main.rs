...
fn main() -> wnode::Result<()> {
  ...
  // + flush_into（挂上正式日志面并回灌先行缓冲存量）
  let node = &args.node;
  let network_connection_limit = node.network_connection_limit;
  LoggingBuilder::from_node(node).flush_into(&pre_parse);
  ...
  ServerBootstrap::new(args)
    // 连接上限装配（C# GarnetServer.cs:294；-1 = 不限）
    .network_connection_limit(i64::from(network_connection_limit))
