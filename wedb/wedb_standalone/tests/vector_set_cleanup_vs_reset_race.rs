...
fn drop_vector_set_while_resetting_store() {
  ...
  // 生产拓扑的三条常驻清理协程（对标 C# VectorManager 常驻任务；spawn 须在
  // compio runtime 内执行，故随 block_on 驱动）
  ...
  rt.block_on(async {
    ...
    // 向量集元记录（56B 索引存根）在 VectorManager 键登记表承载，
    // 即 DEL 路径 request_deletion 的载荷
    let value = vm
      .read_stored_index(key)
      .expect("向量集索引记录应在登记表在位");
