...
use waof::{AofHeader, WalConfig, WalLog};
use wbase::{
  convert::TICKS_PER_MILLISECOND, entry_type::AofEntryType, hash_slot::hash_slot as cluster_slot,
};
...
use wnode::{
  ...
  storage::session::{
    ...
    objectstore::{
      common::RmwOutcome, sorted_set_geo_ops::GeoCenter, sorted_set_ops::ZSetRemoveRange,
    },
    storage_session::StorageSession,
    unifiedstore::advanced_ops::UnifiedRMWOp,
  },
};
use wresp::SortedSetAggregateType as ZSetAggregate;
...
fn test_string_and_bitmap_and_hll() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (_dir, store) = open_test_store("str.db")?;
...
fn test_object_stores() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (_dir, store) = open_test_store("obj.db")?;
    let session = store.new_session()?;
    let ss = storage_session(&session);
...
fn test_multi_database_manager() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (dir, store) = open_test_store("multi.db")?;
    let manager = DatabaseManagerFactory::create_database_manager(
      true,
      Arc::clone(&store),
      dir.path().to_path_buf(),
...
fn test_multi_saved_database_ids_error_semantics() -> aok::Void {
  ...
  use wdatabase::MultiDatabaseManager;

  let rt = Runtime::new()?;
  rt.block_on(async {
    let (dir, store) = open_test_store("ids.db")?;

    // 1. 根目录不存在：良性空集，recover_checkpoint_async 静默成功且不注册任何库
    let manager =
      MultiDatabaseManager::<_, ()>::new(Arc::clone(&store), dir.path().join("checkpoints"));
    ...
    let manager = MultiDatabaseManager::<_, ()>::new(Arc::clone(&store), blocked);
...
#[test]
fn test_scan_watch_bitfield_semantics() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
...
fn test_review_rr1_csharp_semantics() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    let (_dir, store) = open_test_store("rr1.db")?;
    let session = store.new_session()?;
    let ss = storage_session(&session);
...
#[test]
fn test_missing_key_and_wrongtype_contract() -> aok::Void {
  use wnode::storage::session::objectstore::sorted_set_geo_ops::GeoCmd;

  let rt = Runtime::new()?;
  rt.block_on(async {
    let (_dir, store) = open_test_store("rr3.db")?;
    let session = store.new_session()?;
    let ss = storage_session(&session);
    ...
    assert_eq!(
      ss.hash_collect(b"h_absent").await?,
      (GarnetStatus::NotFound, 0)
    );
    ...
    assert_eq!(ss.hash_collect(b"hlive").await?, (GarnetStatus::Ok, 1));
...
fn test_storage_session_advanced_contract_apis() -> aok::Void {
  ...
  rt.block_on(async {
    ...
    // 3. mset_conditional & delete_main_store
    let k1: &[u8] = b"mset_1";
    let k2: &[u8] = b"mset_2";
    let v1: &[u8] = b"10";
    let v2: &[u8] = b"20";
