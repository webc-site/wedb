...
use wnode::storage::{
  session::storage_session::StorageSession, storage_scripting_api::StorageScriptingApi,
};
...
#[test]
fn complex_lua_test_1() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    ...
    let args = eval_args(script, &[b"key1", b"key2"], &[b"value", b"1", b"60000"]);
    assert!(harness.eval(&args, &mut out));
    assert_eq!(out, b"$8\r\nkey1key2\r\n");

    let args = eval_args(script, &[b"key1"], &[b"value", b"1", b"60000"]);
    assert!(harness.eval(&args, &mut out));
    assert_eq!(out, b"$4\r\nkey1\r\n");

    let args = eval_args(script, &[], &[b"value", b"1", b"60000"]);
    assert!(harness.eval(&args, &mut out));
    assert_eq!(out, b"$0\r\n\r\n");
...
#[test]
fn complex_lua_test_2() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    ...
    let args = eval_args(script, &[b"key1", b"key2"], &[b"foo"]);
    assert!(harness.eval(&args, &mut out));
    assert_eq!(out, b"$2\r\nOK\r\n");

    let script2 = "local values = redis.call('mget', unpack(KEYS))\nfor i, _ in ipairs(KEYS) do\n  if values[i] ~= ARGV[1] then\n    return false\n  end\nend\nredis.call('del', unpack(KEYS))\nreturn redis.status_reply('OK')";
    let args2 = eval_args(script2, &[b"key3"], &[b"foo"]);
    assert!(harness.eval(&args2, &mut out));
    assert_eq!(out, b"$-1\r\n");

    let args3 = eval_args(script2, &[b"key1", b"key2"], &[b"foo"]);
    assert!(harness.eval(&args3, &mut out));
    assert_eq!(out, b"$2\r\nOK\r\n");
...
#[test]
fn complex_lua_test_3() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    ...
    let args = eval_args(script, &[b"key1", b"key2"], &[]);
    assert!(harness.eval(&args, &mut out));
    assert_eq!(out, b"*2\r\n$-1\r\n$-1\r\n");
...
#[test]
fn script_exists_errors() -> aok::Void {
  let rt = Runtime::new()?;
  rt.block_on(async {
    ...
    let empty_args: Vec<Vec<u8>> = vec![];
    assert!(harness.script_exists(&empty_args, &mut out));
    assert_eq!(
      out,
