...
fn object_encoding_types_and_expiration() {
  with_batch(|s, batch| {
    ...
    s.network_set(&[b"str_key", b"hello"], batch, &mut out)
      .unwrap();
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[b"str_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$3\r\nraw\r\n");
    ...
    s.hash_set(&[b"hash_key", b"f1", b"v1"], batch, &mut out)
      .unwrap();
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[b"hash_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$9\r\nhashtable\r\n");
    ...
    s.list_push(&[b"list_key", b"e1"], batch, &mut out, true)
      .unwrap();
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[b"list_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$9\r\nquicklist\r\n");
    ...
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[b"set_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$9\r\nhashtable\r\n");
    ...
    s.sorted_set_add(&[b"zset_key", b"1.0", b"m1"], batch, &mut out)
      .unwrap();
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[b"zset_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$8\r\nskiplist\r\n");
    ...
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[b"exp_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$-1\r\n");
    ...
    s.network_expire(ExpireCmd::Expireat, &[b"exp_key2", b"1"], batch, &mut out)
      .unwrap();
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[b"exp_key2"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$-1\r\n");

    // 9. REFCOUNT: 存在键 -> :1, 不存在键 -> nil
    out.clear();
    s.network_object(ObjectSubCmd::Refcount, &[b"str_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b":1\r\n");
    out.clear();
    s.network_object(
    ...
    out.clear();
    s.network_object(ObjectSubCmd::Idletime, &[b"str_key"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b":0\r\n");
    out.clear();
    s.network_object(
    ...
    // 11. FREQ: 恒不支持报错
    out.clear();
    s.network_object(ObjectSubCmd::Freq, &[b"str_key"], batch, &mut out)
      .unwrap();
    ...
    // 13. 参数数量错误
    out.clear();
    s.network_object(ObjectSubCmd::Encoding, &[], batch, &mut out)
      .unwrap();
    assert_eq!(
      out,
...
fn async_command_resp2_and_resp3() {
  with_batch(|s, batch| {
    ...
    // RESP3 下：ASYNC ON -> +OK，并且 use_async 置为 true
    out.clear();
    s.network_async(&[b"ON"], batch, &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");
    assert!(s.use_async);

    // RESP3 下：ASYNC OFF -> +OK，并且 use_async 置为 false
    out.clear();
    s.network_async(&[b"OFF"], batch, &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");
    assert!(!s.use_async);

    // RESP3 下：ASYNC BARRIER -> +OK
    out.clear();
    s.network_async(&[b"BARRIER"], batch, &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");
