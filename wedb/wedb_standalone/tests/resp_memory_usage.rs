...
fn memory_usage_string_grows_with_value() {
  with_batch(|s, batch| {
    let mut out = Vec::new();
    s.network_set(&[b"mk", b"abc"], batch, &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");

    let mut out = Vec::new();
    s.network_memory_usage(&[b"mk"], batch, &mut out).unwrap();
    assert_eq!(out[0], b':');
    ...
    let mut out = Vec::new();
    s.network_set(&[b"mk", &long_val], batch, &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");
...
fn memory_usage_collection_objects_positive() {
  with_batch(|s, batch| {
    ...
    let mut out = Vec::new();
    s.network_memory_usage(&[b"hk"], batch, &mut out).unwrap();
    assert_eq!(out[0], b':');
    ...
    let mut out = Vec::new();
    s.network_memory_usage(&[b"zk"], batch, &mut out).unwrap();
    assert_eq!(out[0], b':');
...
fn memory_usage_missing_key_returns_null() {
  with_batch(|s, batch| {
    let mut out = Vec::new();
    s.network_memory_usage(&[b"missing"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"$-1\r\n");
  });
}
...
fn memory_usage_samples_option_accepted() {
  with_batch(|s, batch| {
    let mut out = Vec::new();
    s.network_set(&[b"sk", b"v"], batch, &mut out).unwrap();
    assert_eq!(out, b"+OK\r\n");

    let mut out = Vec::new();
    s.network_memory_usage(&[b"sk", b"SAMPLES", b"5"], batch, &mut out)
      .unwrap();
    assert_eq!(out[0], b':');
    ...
    // 语法错误：负采样数与未知选项
    let mut out = Vec::new();
    s.network_memory_usage(&[b"sk", b"SAMPLES", b"-1"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"-ERR syntax error\r\n");

    let mut out = Vec::new();
    s.network_memory_usage(&[b"sk", b"BOGUS", b"5"], batch, &mut out)
      .unwrap();
    assert_eq!(out, b"-ERR syntax error\r\n");
  });
}
