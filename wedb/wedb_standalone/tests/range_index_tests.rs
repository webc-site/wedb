...
use wbftree::{RangeIndexManager as Engine, RangeIndexStub, StorageBackendType, TreeTuning};
...
fn ri_disk_file_cleanup_on_delete_test() {
  let dir = tempdir().unwrap();
  let engine = Engine::new(dir.path().join("ri"), dir.path().join("cpr")).unwrap();
  let p1 = Engine::base32_prefix_of(b"k1");
  let p2 = Engine::base32_prefix_of(b"k2");
...
fn ri_len_basic_test() {
  ...
  rt.block_on(async {
    ...
    out.clear();
    resp
      .network_rilen(&[b"myindex"], Some(ri.as_ref()), &session, &mut out)
      .await
      .unwrap();
    assert_eq!(out, b":0\r\n");
