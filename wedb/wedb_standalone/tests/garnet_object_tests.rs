...
const ZSET_TAG: u8 = GarnetObjectType::SortedSet.as_u8();
  ...
  rt.block_on(async {
    ...
    // 1. LocalWrite & Checkpoint
    {
      ...
      CheckpointManager::new()
        .create_checkpoint(&store, &cp_dir, CheckpointType::FoldOver)
        .await?;
    }
    ...
    // 2. Recover & LocalRead
    {
      let device = Arc::new(SegmentedDevice::single_file(&db_path)?);
      let store =
        Arc::new(CheckpointManager::<SegmentedDevice>::recover_latest(&cp_dir, device).await?);
      let session = store.new_session()?;
