...
use wreviv::{BEST_FIT_SCAN_ALL, DEFAULT_BIN_SIZES, FreeRecordBin, FreeRecordPool};

use super::support::{ADDRESS_INCREMENT, total_active};
...
fn thread_contention_on_single_record() -> Void {
  ...
  for _ in 0..num_threads {
    ...
    handles.push(thread::spawn(move || {
      ...
      for _ in 0..num_iterations {
        if let Some((addr, _)) = pool.take(small_size, min_address, min_address) {
  ...
  let in_pool = pool.take(small_size, min_address, min_address).is_some();
    ...
    for t in 0..num_take_threads {
      ...
      take_handles.push(thread::spawn(move || {
        ...
        while total_taken.load(Ordering::Acquire) < max_records {
          if let Some((addr, _)) = pool.take(add_record_size, 0, 0) {
...
fn exact_match_contention_fallback() -> Void {
  ...
  for _ in 0..4 {
    ...
    handles.push(thread::spawn(move || {
      barrier.wait();
      if let Some((addr, size)) = pool.take(40, 0, 0) {
...
fn concurrent_put_take_purge_stress() -> Void {
  ...
  // 消费者线程（探测尺寸轮转 32/96/160：take 检索窗受控至多探相邻一档（对标 C#
  // NumberOfBinsToSearch 语义），单一 32B 需求覆盖不到 128/256B 桶槽位，
  // 三档窗口 [32,64]+[128,256]+[256,512] 轮转清池；病态级联溢出越窗的槽位
  // 由池面静止判定（收工后连续 STABLE_SPINS 次活跃数不变即死局收敛）与空闲
  // 自旋上限双重兜底退出，压测不变量（无重复、地址不越下界）不受影响）
  const PROBE_SIZES: [u32; 3] = [32, 96, 160];
  const MAX_IDLE_SPINS: usize = 100_000;
  /// 收工后池面静止判定的连续观测次数：越窗死局提前收敛（替代 10 万次空转兜底）
  const STABLE_SPINS: usize = 16;
  /// 活跃数哨兵：必不等于任何真实 active，保证重置后首观测走「变更」分支
  const ACTIVE_UNSET: usize = usize::MAX;
  let taken_records = Arc::new(Mutex::new(Vec::new()));
  for _ in 0..num_consumers {
    ...
    handles.push(thread::spawn(move || {
      ...
      let mut last_active = ACTIVE_UNSET;
      while !producers_done.load(Ordering::Acquire)
        || (total_active(&pool) > 0 && idle_spins < MAX_IDLE_SPINS)
      {
        let cur_min = min_addr.load(Ordering::Acquire);
        let probe = PROBE_SIZES[tick % PROBE_SIZES.len()];
        tick += 1;
        if let Some((addr, _size)) = pool.take(probe, cur_min, cur_min) {
          idle_spins = 0;
          stable_spins = 0;
          last_active = ACTIVE_UNSET;
          assert!(
            addr >= cur_min,
          ...
          idle_spins += 1;
          // 收工后池面静止判定：done 后无新 put，活跃数单调不增（purge 只减
          // 不增，观测变小仅重置判定）——连续 STABLE_SPINS 次观测不变即死局
          // （越窗槽位永远取不到），提前收敛退出；MAX_IDLE_SPINS 仍兜底池面
          // 持续抖动的病态
          if producers_done.load(Ordering::Acquire) {
            let active = total_active(&pool);
            if active == last_active {
              stable_spins += 1;
              if stable_spins >= STABLE_SPINS {
                break;
              }
            } else {
              stable_spins = 0;
...
fn active_count_invariant_under_contention() -> Void {
  ...
  let bin = Arc::new(FreeRecordBin::with_scan_limit(64, 16, BEST_FIT_SCAN_ALL));
  assert_eq!(bin.len(), 0);
  assert!(bin.is_empty());
  ...
  for _ in 0..threads {
    ...
    take_handles.push(thread::spawn(move || {
      barrier.wait();
      for _ in 0..iterations {
        let _ = bin.take_best_fit(48, ADDRESS_INCREMENT, ADDRESS_INCREMENT);
