...
//! - min_eligible_floor_skips_without_purge (双下界分档：申请下界仅跳过不清退)
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recordops/RevivificationTests.cs（清理与上限）
...
use log::info;
use wreviv::{
  BEST_FIT_SCAN_ALL, Error, FreeRecord, FreeRecordBin, FreeRecordPool, SetStatus, USE_FIRST_FIT,
};

use super::support::total_active;
...
fn unelide_capacity_overflow() -> Void {
  ...
  assert_eq!(total_active(&pool), capacity);
  ...
  // 取出 1 个槽位腾出空间
  let taken = pool.take(64, 0x1000, 0x1000);
  assert!(taken.is_some());
  assert_eq!(total_active(&pool), capacity - 1);
...
fn purge_below_multi_bin_bulk() -> Void {
  ...
  let pool = FreeRecordPool::with_bin_sizes_and_scan_limit(&[64, 128, 256], 8, BEST_FIT_SCAN_ALL)?;
  ...
  assert_eq!(total_active(&pool), 6);
  assert_eq!(pool.stats().drop_count, 0);
  ...
  assert_eq!(pool.stats().drop_count, 3);
  assert_eq!(total_active(&pool), 3);

  // 热区记录仍可正常复活
  let take1 = pool.take(100, 0x3500, 0x3500);
  assert_eq!(take1, Some((0x4000, 120)));

  let take2 = pool.take(180, 0x3500, 0x3500);
  assert_eq!(take2, Some((0x5000, 200)));

  let take3 = pool.take(210, 0x3500, 0x3500);
  assert_eq!(take3, Some((0x6000, 220)));

  assert!(pool.is_empty());
  assert_eq!(pool.stats().hit_count, 3);
...
fn put_replaces_expired_slot() -> Void {
  ...
  let pool = FreeRecordPool::with_bin_sizes_and_scan_limit(&[64], 1, BEST_FIT_SCAN_ALL)?;
  assert!(pool.put(0x1000, 64, 0x1000));
  assert!(pool.put(0x3000, 64, 0x2000));
  assert_eq!(pool.stats().put_count, 2);
  assert_eq!(pool.stats().drop_count, 1, "被覆盖的过期槽位应计入丢弃统计");
  assert_eq!(total_active(&pool), 1);

  // 4. 取出 B，A 不可复活
  assert_eq!(pool.take(64, 0x2000, 0x2000), Some((0x3000, 64)));
  assert!(pool.is_empty());
...
fn best_fit_scan_limit_clamping() -> Void {
  ...
  let (res1, _) = bin_limit1.take_best_fit(35, 0, 0);
  assert_eq!(res1, Some((0x1000, 40)));
  let (res2, _) = bin_limit1.take_best_fit(35, 0, 0);
...
fn boundary_parameter_defense() -> Void {
  ...
  assert_eq!(pool.take(0, 100, 100), None);
  assert_eq!(pool.take(65536, 100, 100), None);
  assert_eq!(pool.take(u32::MAX, 100, 100), None);
  ...
  // 8. 成功存入与取出最大整词对齐内联尺寸（末桶 65528；65535 非 8 倍数已出阶梯）
  let max_aligned =
    FreeRecord::MAX_INLINE_SIZE - FreeRecord::MAX_INLINE_SIZE % FreeRecord::RECORD_ALIGNMENT;
  assert_eq!(max_aligned, 65528);
  assert!(pool.put(FreeRecord::ADDRESS_MASK, max_aligned, 0));
  let taken = pool.take(max_aligned, 0, 0);
  assert_eq!(taken, Some((FreeRecord::ADDRESS_MASK, max_aligned)));
  // 出阶梯尺寸拒收：put 落空计 drop、take 直接 None
  assert!(!pool.put(FreeRecord::ADDRESS_MASK, FreeRecord::MAX_INLINE_SIZE, 0));
  assert_eq!(pool.take(FreeRecord::MAX_INLINE_SIZE, 0, 0), None);

  // 9. FreeRecordPool 构建参数校验防御
  assert!(matches!(
    FreeRecordPool::with_bin_sizes_and_scan_limit(&[], 16, BEST_FIT_SCAN_ALL),
    Err(Error::EmptyBinSizes)
  ));
  assert!(matches!(
    FreeRecordPool::with_bin_sizes_and_scan_limit(&[64], 0, BEST_FIT_SCAN_ALL),
    Err(Error::InvalidCapacity)
  ));
  assert!(matches!(
    FreeRecordPool::with_bin_sizes_and_scan_limit(&[64, 32], 16, BEST_FIT_SCAN_ALL),
    Err(Error::UnsortedBinSizes)
  ));
  assert!(matches!(
    FreeRecordPool::with_bin_sizes_and_scan_limit(&[64, 64], 16, BEST_FIT_SCAN_ALL),
    Err(Error::UnsortedBinSizes)
  ));
  assert!(matches!(
    FreeRecordPool::with_bin_sizes_and_scan_limit(&[64, 70_000], 16, BEST_FIT_SCAN_ALL),
...
fn non_monotonic_min_address_safety() -> Void {
  ...
  let pool = FreeRecordPool::with_bin_sizes_and_scan_limit(&[64, 128], 8, BEST_FIT_SCAN_ALL)?;
  ...
  assert_eq!(pool.take(40, 0x1000, 0x1000), Some((0x3000, 40)));
  assert_eq!(total_active(&pool), 1);
  ...
  assert_eq!(pool.stats().put_count, 3);
  assert_eq!(pool.stats().take_count, 2);
  assert_eq!(pool.stats().hit_count, 1);
  assert_eq!(pool.stats().drop_count, 2);
  assert_eq!(pool.stats().failed_takes(), 1);

  OK
}

/// 验证 min_eligible_addr（本次申请下界）仅跳过让位、绝不误淘汰
/// 对标 C# FreeRecordPool.cs:TryTake（候选槽位低于 minAddress 仅返回 false 放弃本
/// 槽位，绝不就地 CAS 淘汰清零）与 BlockAllocate.cs:59-62（链首抬升 minRevivAddress）
///
/// 场景：复活池同时存在「高于全局水位但低于本次链首下界」的低址槽位与高址槽位：
/// 1. Best-Fit 与 First-Fit 两条取出路径均只跳过低址槽位（不计 purged、不清零、
///    active_count 与 drop_count 不动），取走高址合格槽位；
/// 2. 全部槽位低于下界时 take 落空返回 None，池内槽位原封不动；
/// 3. 同一低址槽位以 eligible = 水位正常申请即可照常取出（他链仍可用，杜绝误杀）；
/// 4. 仅低于 min_reviv_addr 全局水位的槽位才被就地清零淘汰并计入 purged/drop。
#[test]
fn min_eligible_floor_skips_without_purge() -> Void {
  info!("> min_eligible_floor_skips_without_purge [双下界分档处置]");

  // 1. Best-Fit 路径：低址槽跳过、高址槽取出
  let pool = FreeRecordPool::with_bin_sizes_and_scan_limit(&[64], 8, BEST_FIT_SCAN_ALL)?;
  assert!(pool.put(0x1000, 64, 0));
  assert!(pool.put(0x2000, 64, 0));
  assert_eq!(total_active(&pool), 2);

  // 本次下界 0x1800（如链首 + 1）：0x1000 低于下界仅跳过，0x2000 合格取出
  assert_eq!(pool.take(64, 0x800, 0x1800), Some((0x2000, 64)));
  assert_eq!(pool.stats().drop_count, 0, "跳过档绝不产生淘汰计数");
  assert_eq!(total_active(&pool), 1);
  assert!(
    pool.bins[0]
      .slots
      .iter()
      .any(|s| !s.is_empty() && s.address() == 0x1000),
    "低于本次下界但高于水位的槽位必须原样留池，他链仍可用"
  );

  // 2. 全部槽位低于下界：落空返回 None，槽位与计数原封不动
  assert_eq!(pool.take(64, 0x800, 0x2000), None);
  assert_eq!(total_active(&pool), 1);
  assert_eq!(pool.stats().drop_count, 0);

  // 3. 同水位正常申请（eligible = min_reviv）：低址槽位照常取出，未被误杀
  assert_eq!(pool.take(64, 0x800, 0x800), Some((0x1000, 64)));
  assert!(pool.is_empty());

  // 4. First-Fit 路径同构：低址跳过并计入 purged 的只有水位之下的槽位
  let bin = FreeRecordBin::with_scan_limit(64, 8, USE_FIRST_FIT);
  assert_eq!(bin.put(0x1000, 64, 0), SetStatus::InsertedEmpty);
  assert_eq!(bin.put(0x2000, 64, 0), SetStatus::InsertedEmpty);
  assert_eq!(bin.put(0x3000, 64, 0), SetStatus::InsertedEmpty);
  // 水位 0x2500 以下（0x1000、0x2000）就地清零淘汰；0x3000 合格取出
  let (res, purged) = bin.take_first_fit(64, 0x2500, 0x2800);
  assert_eq!(res, Some((0x3000, 64)));
  assert_eq!(purged, 2, "仅全局水位之下的槽位被清零淘汰");
  assert!(bin.is_empty());

  // 5. Best-Fit 混合档：水位与下界之间仅跳过（purged 只数水位之下的）
  let bin2 = FreeRecordBin::with_scan_limit(64, 8, BEST_FIT_SCAN_ALL);
  assert_eq!(bin2.put(0x1000, 64, 0), SetStatus::InsertedEmpty);
  assert_eq!(bin2.put(0x2000, 64, 0), SetStatus::InsertedEmpty);
  assert_eq!(bin2.put(0x3000, 64, 0), SetStatus::InsertedEmpty);
  // 水位 0x800 下无淘汰；下界 0x2800 令 0x1000、0x2000 仅跳过，0x3000 取出
  let (res2, purged2) = bin2.take_best_fit(64, 0x800, 0x2800);
  assert_eq!(res2, Some((0x3000, 64)));
  assert_eq!(purged2, 0, "水位之上、下界之下的槽位严禁清零");
  assert_eq!(bin2.len(), 2, "跳过的两枚槽位必须仍在池");
