...
//! - cross_bin_search_capped (受控向上跨桶探查上限)
//! - bin_ladder_word_alignment (全桶阶梯 8 字节整词对齐)
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.recordops/RevivificationTests.cs（Best-Fit 分桶）

use aok::{OK, Void};
use log::info;
use wreviv::{
  DEFAULT_BIN_SIZES, FreeRecord, FreeRecordBin, FreeRecordPool, SetStatus, USE_FIRST_FIT,
};

use super::support::{
  ADDRESS_INCREMENT, RECORD_ALIGNMENT, TAKE_RECORD_SIZE, create_single_bin_pool,
  populate_best_fit_records, total_active,
};
...
fn simple_allocation() -> Void {
  ...
  let taken = pool.take(TAKE_RECORD_SIZE, min_addr, min_addr);
...
fn first_fit_allocation_sequence() -> Void {
  ...
  for i in 1..=6 {
    let (res, _) = bin.take_best_fit(TAKE_RECORD_SIZE, min_addr, min_addr);
  ...
  let (res_none, _) = bin.take_best_fit(TAKE_RECORD_SIZE, min_addr, min_addr);
...
fn min_address_take_invalidation() -> Void {
  ...
  let taken = pool.take(64, 0x3000, 0x3000);
  assert_eq!(taken, None, "已滑入失效区的记录不可被复活");
  assert_eq!(total_active(&pool), 0);
  assert!(
    pool.stats().drop_count >= 1,
    "过期槽位应就地淘汰清零并计入 drop"
  );
...
fn bin_selection_partition_point() -> Void {
  ...
  // 最大分桶边界（末桶整词对齐上限 65528；65529~65535 出阶梯拒收，
  // 杜绝以非 8 倍数尺寸切出 Pad 破坏整词对齐不变式）
  assert_eq!(pool.find_bin_index(32769), Some(12));
  assert_eq!(pool.find_bin_index(65528), Some(12));
  assert_eq!(pool.find_bin_index(65529), None);
  assert_eq!(pool.find_bin_index(65535), None);
...
/// 验证受控向上跨桶探查上限（对标 C# FreeRecordPool.cs:TryTake 的 NumberOfBinsToSearch
/// 逐桶受控探查，RevivificationSettings.cs:48；本实现固化为至多探相邻一档）
///
/// 修复前 take 硬编码 `get_unchecked(start_bin..)` 无限制向上检索至末桶，
/// 微小记录会掠夺 32KB/64KB 大槽位造成大空间耗竭。本用例确定性断言：
/// 1. 当期桶落空至多命中紧邻上一档桶（24B 需求取到 64B 桶的 40B 槽位）；
/// 2. 更远档位（128B 桶、65528B 末桶）槽位在当期+相邻档皆空时绝不可触达；
/// 3. take 命中槽位尺寸上界受窗约束（≤ 相邻档桶上限）。
#[test]
fn cross_bin_search_capped_at_one_next_bin() -> Void {
  info!("> cross_bin_search_capped_at_one_next_bin [受控跨桶探查上限]");

  let pool = FreeRecordPool::new(true);

  // 24B 需求的检索窗 = [32B 桶(idx1), 64B 桶(idx2)] 两档
  assert!(pool.put(0x1000, 40, 0)); // 40B → 64B 桶（窗内，应命中）
  assert!(pool.put(0x2000, 100, 0)); // 100B → 128B 桶（窗外）
  assert!(pool.put(0x3000, 65528, 0)); // 末桶 65528B 大槽位（窗外）

  let taken = pool.take(24, 0, 0);
  assert_eq!(
    taken,
    Some((0x1000, 40)),
    "至多探相邻一档：须命中 64B 桶槽位"
  );
  let (_, size) = taken.unwrap();
  assert!(size <= 64, "跨桶命中槽位尺寸不得越过相邻档桶上限 64");

  // 当期与相邻档皆空 → 24B 需求落空回退；128B 桶与末桶大槽位原封不动
  assert_eq!(
    pool.take(24, 0, 0),
    None,
    "跨两档以上不得检索，微小记录严禁掠夺大槽位"
  );
  assert_eq!(total_active(&pool), 2);

  // 各槽位只能被检索窗覆盖其档位的需要取走
  assert_eq!(pool.take(100, 0, 0), Some((0x2000, 100))); // 窗 = [128, 256]
  assert_eq!(pool.take(32769, 0, 0), Some((0x3000, 65528))); // 窗 = [末桶]

  assert!(pool.is_empty());
  OK
}

/// 验证默认分桶阶梯 8 字节整词对齐与末桶对齐上限（65535 模 8 余 7 已修正为 65528）
///
/// 非对齐末桶尺寸会容许切出非整词跨度的 Pad，令后续扫描跳步失准并引发未对齐
/// 内存访问风险；本用例锁死阶梯对齐不变式与 wreviv/wrecord 对齐常量同值互锁。
#[test]
fn bin_ladder_word_alignment_and_max_bin() -> Void {
  info!("> bin_ladder_word_alignment_and_max_bin [桶阶梯整词对齐不变式]");

  // 全桶尺寸 8 字节整词对齐，末桶恰为对齐化最大内联尺寸 65528
  for &s in &DEFAULT_BIN_SIZES {
    assert_eq!(
      s % RECORD_ALIGNMENT,
      0,
      "分桶尺寸 {s} 违背 8 字节整词对齐不变式"
    );
  }
  assert_eq!(*DEFAULT_BIN_SIZES.last().unwrap(), 65528);
  assert_eq!(
    FreeRecord::MAX_INLINE_SIZE - FreeRecord::MAX_INLINE_SIZE % FreeRecord::RECORD_ALIGNMENT,
    65528
  );

  // 对位互锁：wreviv 槽位对齐常量与 wrecord 生产单点同值
  assert_eq!(
    FreeRecord::RECORD_ALIGNMENT,
    wrecord::RECORD_ALIGNMENT as u32,
    "wreviv 与 wrecord 的记录对齐常量必须恒等"
  );

  let pool = FreeRecordPool::new(true);
  assert_eq!(pool.bins.last().unwrap().max_size, 65528);
  // 65528 整词尺寸入末桶可存取；非对齐出阶梯尺寸拒收
  assert!(pool.put(0x1000, 65528, 0));
  assert_eq!(pool.take(65528, 0, 0), Some((0x1000, 65528)));
  assert!(
    !pool.put(0x2000, 65535, 0),
    "65535 非整词对齐，已出阶梯必须拒收"
  );
  assert_eq!(pool.take(65535, 0, 0), None);
