...
/// 记录对齐字节数，派生自生产单点 `wrecord::RECORD_ALIGNMENT`（对位 Constants.cs:kRecordAlignment 的锚注在单点处）
pub const RECORD_ALIGNMENT: u32 = wrecord::RECORD_ALIGNMENT as u32;
...
/// 汇总当前所有分桶中活跃槽位总数（O(B)，直读各分桶活跃计数）
///
/// 替代池层查询便利方法：对标 C# RevivificationTestUtils 直读内部结构的夹具形状。
pub fn total_active(pool: &FreeRecordPool) -> usize {
  pool.bins.iter().map(|b| b.len()).sum()
}
