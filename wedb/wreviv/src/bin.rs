...
impl FreeRecordBin {
  ...
  ///
  /// 双下界语义与 [`Self::take_first_fit`] 完全一致：
  ...
  pub fn take_best_fit_with_limit(
    ...
    if scan_limit == USE_FIRST_FIT {
      return self.take_first_fit(required_size, min_reviv_addr, min_eligible_addr);
    }
    ...
    loop {
      ...
      if let Some(idx) = best_idx {
        ...
        if local_scan_limit <= 1 {
          let (res, p) = self.take_first_fit(required_size, min_reviv_addr, min_eligible_addr);
          return (res, purged + p);
        }
      } else {
        // 无可用候选
