...
impl FreeRecordBin {
  ...
  ///
  /// 与 C# 差异：C# `FreeRecord.TryTake` 遇低于 minAddress 的过期槽位仅跳过不清零
  /// （仅 Best-Fit 路径的 `TryPeek` 就地清零）；本实现两条取出路径统一对过期槽位
  /// 就地 CAS 清零并递减活跃计数——本实现以 active_count 取代 C# CheckEmptyWorker
  /// 的 isEmpty 标志，计数与真实非空槽位严格一致是空桶快路径正确性的前提。
  #[inline]
