...
/// 受控向上跨桶探查数（对标 C# `RevivificationSettings.NumberOfBinsToSearch`，
/// RevivificationSettings.cs:48，构造时固化、非调用参数；C# 默认 0 仅检索当期桶，
/// C# TryTake 开启后也只是逐桶受控探查相邻桶，FreeRecordPool.cs:556-560。
/// wedb 固化为 1：当期桶落空时至多再探紧邻上一档桶，杜绝微小记录掠夺
/// 32KB/64KB 大槽位，同时保留一档命中率缓冲；不对外暴露配置开关）
...
impl FreeRecordPool {
  ...
  pub fn take(
    ...
    // SAFETY: start_bin < self.bins.len()（来自 find_bin_index），end 已对 bins.len() 取
    // min，区间切片端点必定在界内；受控跨桶窗上界 start_bin + 1 + BINS_TO_SEARCH
    let search_bins = unsafe {
