...
pub(crate) struct ExpiryLedger {
  ...
  pub times: Option<HashMap<Vec<u8>, i64>>,
  /// 过期最小堆（与 times 同生命周期）
...
impl ExpiryLedger {
  ...
  pub fn initialize(&mut self, heap: &mut i64) {
    if self.times.is_none() {
      self.times = Some(HashMap::with_hasher(GxBuildHasher::default()));
  ...
  pub fn insert(&mut self, heap: &mut i64, key: Vec<u8>, expiration: i64) {
  ...
  /// （SortedSetObject.cs 同名方法同体，双份收敛于此）
  pub fn pop_expired(
    &mut self,
    heap: &mut i64,
    now: i64,
  ...
  ///
  /// libs/server/Objects/Hash/HashObject.cs:SetExpiration 过期段
  /// （SortedSetObject.cs 同段同体，双份收敛于此）。成员存在性判定与
  ...
  pub fn set_expiration(
    ...
    if current.is_some() {
      if let Some(slot) = self.times.as_mut().unwrap().get_mut(key) {
        *slot = expiration;
      }
      // 字典项槽位已计，仅补堆项两槽
      *heap += SLOT * 2;
