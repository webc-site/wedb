...
pub trait IGarnetObject: Send + Sync + Debug {
  ...
  /// 在对象上执行操作
  ///
  /// C# 契约双声明收口一处：接口 IGarnetObject.Operate 与抽象基类
  /// GarnetObjectBase.Operate（派生类覆写各自持本类型 Operate 锚，见
  /// hash/set/list/sorted_set_object 各 operate），rust trait 单声明替代继承树
  /// 的两层重声明
  ///
  /// libs/server/Objects/Types/IGarnetObject.cs:Operate
  /// libs/server/Objects/Types/GarnetObjectBase.cs:Operate
  fn operate(
    &mut self,
    sub_id: u16,
    args: &[&[u8]],
...
/// IGarnetObject 同构转发臂单点展开：四内存对象的类型标签、operate /
/// serialize_to_vec / heap_memory_size / is_empty 转发与 raw-len 计数六臂
/// 逐字面同构（各自具体类型持有同形固有方法），抽宏消重；异构臂
/// export_entries（升阶导出语义各异）以闭包参注入（宏卫生下 `self`
/// 不能经 expr 参数外带，改由宏内回灌 receiver）
macro_rules! impl_garnet_object {
  ($ty:ty, $variant:expr, $cnt_field:ident, $export:expr) => {
    impl IGarnetObject for $ty {
      #[inline]
      fn obj_type(&self) -> GarnetObjectType {
        $variant
      }
...
impl_garnet_object!(
  ...
  |o: &HashObject| {
    ...
    o.hash
      .iter()
      .filter(|(k, _)| !o.ledger.is_expired_at(k, now))
      .map(|(k, v)| (k.to_vec(), encode_member(v, o.ledger.get_time(k))))
      .collect()
  }
);

impl_garnet_object!(SetObject, GarnetObjectType::Set, set, |o: &SetObject| {
  o.set
    .iter()
...
impl_garnet_object!(
  ...
  |o: &ListObject| {
    ...
    //
    // 记录形态经 member_ttl 单点 codec 落 1B 形态旗标（List 无成员级 TTL，
    // 恒 `None` = 裸载荷形态），与树内写漏斗 `tree_put`、Hash/ZSet 导出臂
    // 同帧：导出若裸 clone 载荷，同一棵树即混排两形态，读侧 `decode_member`
    // 会把首字节 `0x00` 的元素剥 1B、把首字节 `0x01` 且长 ≥9 的元素剥 9B
    // 并将载荷字节读成假刻度（污染 `meta.next_expiry`），二进制载荷逐轮
    // 「物化→重灌」持续衰减（见 tests/tiered_list_export_member_codec.rs）
    .map(|(idx, val)| {
      (
        (LIST_SEQ_BASE + idx as u128).to_be_bytes().to_vec(),
...
// raw len 只读口径（升阶判定输入）：含成员级 TTL 的对外计数走具体类型
// `purge_expired_len`（堆序剔除过期后直读），与本口径一分二，禁止混用
impl_garnet_object!(
  SortedSetObject,
  ...
  |o: &SortedSetObject| {
    ...
    o.sorted_set_dict
      ...
      .map(|(k, score)| {
        (
          k.to_vec(),
          encode_member(&score.to_be_bytes(), o.ledger.get_time(k)),
        )
      })
      .collect()
  }
