...
use crate::{
  list::list_object::ListOperation, set::set_object::SetOperation, types::RespInputHeader,
...
impl GarnetObject {
  ...
  pub fn need_to_create(header: &RespInputHeader) -> bool {
    // C# 直接 cast 类型字节后 switch；Rust 经 from_u8 单点分派（未知名目一律需建）
    match GarnetObjectType::from_u8(header.data[0]) {
      Some(GarnetObjectType::SortedSet) => !matches!(
        header.data[1],
        Z_REM
          | Z_POPMAX
          | Z_POPMIN
      ...
      Some(GarnetObjectType::List) => !matches!(
        header.data[1],
        L_POP | R_POP | L_RANGE | L_INDEX | L_TRIM | L_REM | L_INSERT | L_PUSHX | R_PUSHX
      ),
      Some(GarnetObjectType::Set) => !matches!(header.data[1], S_CARD | S_MEMBERS | S_REM | S_POP),
      Some(GarnetObjectType::Hash) => !matches!(header.data[1], H_EXPIRE | H_COLLECT),
      _ => true,
    }
  }
}
