...
pub struct SortedSetObject {
  ...
  ///
  /// libs/server/Objects/SortedSet/SortedSetObject.cs:expirationTimes
...
impl SortedSetObject {
  ...
  pub(crate) fn serialize_wire(&self) -> (u32, Vec<u8>) {
    ...
    for (m, s) in &self.sorted_set_dict {
      if !has_expirations || !self.is_expired_at(m, now) {
  ...
  pub fn update_expiration_size(&mut self, add: bool, include_pq: bool) {
    ...
    // C#: IntPtr.Size + sizeof(long) + DictionaryEntryOverhead
    //     (+ IntPtr.Size + sizeof(long) + PriorityQueueEntryOverhead if includePQ)，口径塌缩
    ...
    if add {
      ...
      debug_assert!(self.heap_memory_size >= CONTAINER_BASE * 2);
    }
  }
