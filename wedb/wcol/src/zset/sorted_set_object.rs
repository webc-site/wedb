...
///
/// member 为共享句柄 [`Arc`]（对位 C# byte[] 引用共享：有序视图、散列与过期
/// 账本指向同一份数组，一处分配多处持有，插入/装载路径零字节复制）
...
impl Ord for SortedSetEntry {
  ...
  fn cmp(&self, other: &Self) -> Ordering {
    ...
    SortedSetComparer::compare(
      (&self.score, self.member.as_ref()),
      (&other.score, other.member.as_ref()),
    )
  }
}
...
pub struct SortedSetObject {
  ...
  /// member → score 散列（键与有序视图、过期账本共享同一 [`Arc`] 句柄，
  /// 对位 C# sortedSetDict 与 sortedSet/expirationTimes 的 byte[] 引用共享）
  pub sorted_set_dict: HashMap<Arc<[u8]>, f64>,
...
impl SortedSetObject {
  ...
  pub fn deserialize_from_slice(slice: &[u8]) -> io::Result<Self> {
    ...
    for (member, score) in wire.entries {
      obj.load_insert(Arc::from(member), score);
    }
    ...
    if let Some(expirations) = wire.expirations {
      for (member, expiration) in expirations {
        if expiration < now {
          ...
          if obj.remove_member(member.as_slice()).is_some() {
            obj.mutated_by_ttl = true;
          }
        } else if obj.ledger.get_time(member.as_slice()).is_none() {
          // expirations 判重守卫：重复条目经 ExpiryLedger::insert 二次满额入账
          // 而字典项仅覆盖，净虚标两槽且堆内滞留陈旧项（与 load_insert 的 entries
          // 守卫同形，跳过保持首条刻度）；句柄经字典内同一 Arc 传入账本
          if let Some(handle) = obj.dict_handle(member.as_slice()) {
    ...
    if let Some(old_score) = self.sorted_set_dict.get_mut(member) {
      if *old_score != score {
        ...
        *old_score = score;
        ...
        if let Some(m) = self.dict_handle(member) {
  ...
  pub fn rem(&mut self, member: &[u8]) -> Option<f64> {
    let old_score = self.remove_member(member)?;
    self
      .ledger
  ...
  /// 形参——成员后恒附带分值文本，且「截断比较」以 `items.len() == count * 2`
  /// 判定保留（含 count=0 时首个未命中条目即停的上游怪癖）。1:1 宣称在
  /// 极值点失真：C# int32 unchecked 侧 COUNT=-2147483648 翻倍回绕为 0，
  /// 退化为「首条未命中即停」空页爬行（不属负值全量遍历族），rust 按
  /// i64 加宽永不回绕、不复刻该形态（登记见 doc/zh/deviations.md 第 20 条
  /// d)，严禁按 C# 回改为 i32 截断回绕）；分值
  /// 统一经 [`wresp::ext::RespVecExt::write_resp_double_bulk_string`] 文本化
  /// （非有限值输出 "inf"/"-inf"；不对齐 C# ZSCAN 侧 Utf8Formatter "Infinity" 词形，
  /// 与 ZRANGE 同形收敛，见 doc/zh/deviations.md §80；订正旧注失真：C#
  /// Utf8Formatter 恒成功、其 null 项为不可达死臂）。
  /// 尾段游标收敛则经单点 [`scan_converge_cursor`] 以 `>=` 归零，系对上游相等
  /// 判定死锁死角的刻意修复（与内存态 hash、分层态同口径；偏离登记见
  /// doc/zh/deviations.md）。
  ///
  /// emit 回调借成员切片 + 原始分值直写出帧（返回发出条目计数，恒 2：
  /// 成员 + 分值，替代 C# out List 的引用收集——集合内字节零拷贝，
  /// 无逐条目堆物化），游标由本内核返回
  pub fn scan(
    &self,
    start: i64,
    ...
    pattern: &[u8],
    _is_no_value: bool,
    mut emit: impl FnMut(&[u8], f64) -> usize,
  ) -> i64 {
    ...
    for (member, score) in self.sorted_set_dict.iter() {
      ...
      if pattern.is_empty() || glob_match(pattern, member) {
        emitted += emit(member, *score) as i64;
      }
      ...
      // 上游怪癖一并保留）。极值点 1:1 失真已登记：C# int32 侧
      // COUNT=-2147483648 翻倍回绕为 0 致首条未命中即停，rust i64 加宽不
      // 复刻该回绕（登记见 doc/zh/deviations.md 第 20 条 d)，勿按 C# 回改）
      if emitted == count * 2 {
        break;
      }
    }
    ...
    scan_converge_cursor(
      cursor,
      expired_keys_count,
      self.sorted_set_dict.len() as i64,
    )
  }
  ...
  ///
  /// member 取共享句柄：账本内字典与堆只增引用计数，不复制成员字节
  pub fn insert_expiration(&mut self, member: Arc<[u8]>, expiration: i64) {
    self
      .ledger
  ...
  pub fn delete_expired_items(&mut self) {
    ...
    ledger.pop_expired(heap_memory_size, now_ticks(), EXPIRY_FLOOR, |key, heap| {
      // remove_entry 拿回字典内的共享句柄原值移入有序视图，零复制
      if let Some((m, score)) = sorted_set_dict.remove_entry(key) {
        sorted_set.remove(&SortedSetEntry { score, member: m });
  ...
  pub fn set_expiration(
    ...
    // 过期账本收主容器内的共享句柄，不另行分配（成员字节全容器单份）
    let Some(handle) = self.dict_handle(key) else {
  ...
  /// 取回字典内存活的共享句柄克隆（对位 C# 同一 byte[] 引用传入账本）
  #[inline]
  fn dict_handle(&self, key: &[u8]) -> Option<Arc<[u8]>> {
    self
      .sorted_set_dict
      .get_key_value(key)
      .map(|(k, _)| k.clone())
  ...
  /// 装载/还原用双索引插入（重复成员跳过保持首条）：有序视图与散列共享同一
  /// [`Arc`] 句柄、先记账再双插，杜绝畸形载荷下双索引失步与重复累加虚标
  fn load_insert(&mut self, member: Arc<[u8]>, score: f64) {
    if self.sorted_set_dict.contains_key(member.as_ref()) {
      return;
    }
    self.update_size(&member, true);
    ...
    self.sorted_set_dict.insert(member, score);
  }

  /// 双索引剔除核心：从散列摘出成员并同步移除有序视图条目、回收记账，
  /// 返回被剔成员原分值（成员不存在返回 None）；账本剔除由各调用点自理
  fn remove_member(&mut self, key: &[u8]) -> Option<f64> {
    let (m, score) = self.sorted_set_dict.remove_entry(key)?;
...
/// 条目内存记账单点（`SortedSetObject::update_size` 的内部实现，与到期剔除闭包共用；
/// C# UpdateSize 锚由宿主方法单点持有，本函数不重复挂锚）：
/// 数据实计 + 四槽（散列条目槽 + 有序视图树节点槽 + 双份分值槽 SLOT*2）
///
/// member 字节全容器仅一份（双索引与过期账本共享同一 [`Arc`] 句柄），此处
/// 计一份即足额，对位 C# byte[] 引用共享下的 UpdateSize 单份口径；过期账本
/// 侧按 C# UpdateExpirationSize 只计槽位不计字节，无重复
#[inline]
fn account_entry(heap: &mut i64, item: &[u8], add: bool) {
  // C#: RoundUp(len,IntPtr.Size) + ByteArrayOverhead + 2*sizeof(double)
  //     + SortedSetEntryOverhead + DictionaryEntryOverhead(=168)，rust 口径按构成项
  //     逐项塌缩为 SLOT*4(=64)，值不等：散列条目槽 ↔ DictionaryEntryOverhead、
  //     BTreeSet 树节点槽 ↔ SortedSetEntryOverhead、双份分值槽 SLOT*2 ↔
  //     2*sizeof(double) + member 句柄槽（双容器各持一份 f64 + Arc 胖句柄，
  //     字节实体已由 round_up_ptr 单份实计）
  let memory_size = round_up_ptr(item.len()) as i64 + SLOT * 4;
