...
impl SortedSetObject {
      ...
      while curr_idx < count {
        ...
        if equals_ignore_case(token, b"BYSCORE") {
          ...
          let (Some(offset), Some(count_limit)) = (
            strict_i32(arg(input, curr_idx)).map(|v| v as i64),
            strict_i32(arg(input, curr_idx + 1)).map(|v| v as i64),
          ) else {
  ...
  pub(crate) fn sorted_set_remove_range_by_rank(
    ...
    let (Some(start), Some(stop)) = (
      strict_i32(arg(input, 0)).map(|v| v as i64),
      strict_i32(arg(input, 1)).map(|v| v as i64),
    ) else {
      output.write_error(RESP_ERR_GENERIC_VALUE_IS_NOT_INTEGER.as_bytes());
      return;
    };
  ...
  pub(crate) fn get_elements_in_range_by_score(
    ...
    if query.rem {
      // 借用遍历：remove_entry 返回自有 member，直接复用构造 BTreeSet 条目
      for (score, member) in scored_elements.iter() {
        if let Some((m, _)) = self.sorted_set_dict.remove_entry(member) {
          self.sorted_set.remove(&SortedSetEntry {
            score: *score,
            member: m,
          });
