...
impl SortedSetObject {
  ...
  pub(crate) fn sorted_set_remove(&mut self, args: &[&[u8]], output: &mut ObjectOutput<'_>) {
    ...
    for &value in args {
      ...
      self
        .ledger
        .remove_expiration(&mut self.heap_memory_size, EXPIRY_FLOOR, value);
  ...
  pub(crate) fn sorted_set_remove_range_by_rank(
    ...
    for item in doomed {
      ...
      self
        .ledger
        .remove_expiration(&mut self.heap_memory_size, EXPIRY_FLOOR, &item.member);
    }
  ...
  pub(crate) fn sorted_set_random_member(
    ...
    let indexes = pick_k_random_indexes(
  ...
  pub fn pop_min_or_max(&mut self, pop_max_score_element: bool) -> Option<(f64, Vec<u8>)> {
    ...
    self
      .ledger
      .remove_expiration(&mut self.heap_memory_size, EXPIRY_FLOOR, &element.member);
