...
//! [计数头][bitcode 载荷]`——内层标签区分子类型，计数头小端整数直读计数，
//! 用户字符串值内容任意、互不干扰。计数头两态：哈希/有序集合（成员级 TTL
//! 仅此两类）恒带 `[4B count LE][8B 到期水位 LE]` 12B 头，水位为序列化时
//! 存活挂期成员的最早到期刻度（读侧计数 O(1) 的水位门单源，与分层态
//! `MetaValue.next_expiry` 同构，见 [`expiry_watermark_of_blob`]）；集合/列表
//! 无水位域，恒 `[4B count LE]` 4B 头。
//!
...
//! 边界收窄为 u8）。
//!
//! 自研依据: 信封对象载荷零拷贝（doc/zh/collection.md）
...
pub trait GarnetObjectPayload: Sized + Default {
  ...
  ///
  /// C# 公有读入口 Deserialize(out IGarnetObject)（按类型标签实例化）在 rust
  /// 为类型直读：标签校验收口 [`obj_decode`] / [`obj_decode_custom`]，本方法
  /// 承接标签后的载荷物化，两段合流即 C# Deserialize → DeserializeInternal 链
  ///
  /// 在 garnet 中的相对路径:libs/server/Objects/Types/GarnetObjectSerializer.cs:Deserialize
  /// 在 garnet 中的相对路径:libs/server/Objects/Types/GarnetObjectSerializer.cs:DeserializeInternal
...
/// 无成员级 TTL 类型（Set/List）信封计数头长：`[4B count LE]`
pub const COUNT_BLOB_HEADER: usize = 4;

/// 携成员级 TTL 类型（Hash/SortedSet）信封头长：`[4B count LE][8B 到期水位 LE]`
pub const WATERMARKED_BLOB_HEADER: usize = 12;

/// 无存活挂期恒水位（`now_ticks()` 永不越线，读侧计数恒 O(1) 快道）
pub const NO_EXPIRY_WATERMARK: i64 = i64::MAX;

/// 从对象信封载荷计数头中直读元素数量（O(1) 零反序列化）
///
/// 水位旗标等头部扩展不影响本读取：计数恒居载荷前 4B（哈希/有序集合的
/// 8B 水位紧随其后，见 [`expiry_watermark_of_blob`]）
#[inline]
...
/// 从对象信封载荷头部直读到期水位（O(1) 零反序列化）
///
/// 仅哈希/有序集合信封带 8B 水位域（序列化写时固化的存活挂期最早到期刻度，
/// 无存活挂期恒 [`NO_EXPIRY_WATERMARK`]）；其余类型无水位域，恒快道。
/// 消费点为 HLEN/ZCARD 等计数读探针的水位门——`now <= 水位` 期间头部计数
/// 恒精确（写时已剔除到期项，且水位内不可能再有成员到期）；越线即字段级
/// TTL 已有成员到期、头部计数失真，读侧降级物化矫正臂（堆序惰性剔除 +
/// 升格写回一次矫正，collection.md §6.3「计数前先走堆序惰性剔除」的信封态
/// 承接，与分层态 `now <= next_expiry` 判定同构）。`None` 仅载荷不足
/// 12B（畸形），由调用方显式判 Corrupt 拒载——与装载族单一 fail-fast 漏斗
/// 收敛，严禁静默兜底出假计数
#[inline]
pub fn expiry_watermark_of_blob(tag: GarnetObjectType, payload: &[u8]) -> Option<i64> {
  match tag {
    GarnetObjectType::Hash | GarnetObjectType::SortedSet => {
      let b: &[u8; 8] = payload
        .get(COUNT_BLOB_HEADER..WATERMARKED_BLOB_HEADER)?
        .try_into()
        .ok()?;
      Some(i64::from_le_bytes(*b))
    }
    _ => Some(NO_EXPIRY_WATERMARK),
  }
}
