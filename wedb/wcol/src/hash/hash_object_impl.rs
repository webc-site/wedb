...
use wbase::{
  ...
  heap::round_up_ptr,
  num::{strict_f64, strict_i64, try_parse_f64},
  time::now_ticks,
};
use wresp::{
  cmd_strings::{
...
/// 解析 i64（调用 `wbase::num::strict_i64`）
...
impl HashObject {
  ...
  pub(crate) fn hash_get_all(&mut self, output: &mut ObjectOutput<'_>, resp_protocol_version: u8) {
    ...
    let is_expirable = self.ledger.has_items();
  ...
  pub(crate) fn hash_increment_float(&mut self, args: &[&[u8]], output: &mut ObjectOutput<'_>) {
    ...
    match self.hash.get(key).cloned() {
      ...
      Some(hash_value) => {
        let Some(result) = strict_f64(&hash_value, true) else {
          RespWriter::new_ref(output.payload)
    ...
    for &field in args {
      ...
      if result >= 0 {
        if is_timestamp && is_milliseconds {
          ...
          result = milliseconds_from_diff_ticks(result, now_ticks());
        } else if !is_timestamp && !is_milliseconds {
          result = seconds_from_diff_ticks(result, now_ticks());
        }
      }
