...
fn read_list_position_input(
  ...
  while curr_token_idx < input.parse_state.count {
    ...
    if sb_param == b"RANK" || sb_param == b"rank" {
      let Some(rank) = strict_i32(arg(input, curr_token_idx)) else {
      ...
      let Some(count) = strict_i32(arg(input, curr_token_idx)) else {
        return Err(RESP_ERR_GENERIC_VALUE_IS_NOT_INTEGER.as_bytes());
      };
      curr_token_idx += 1;
      ...
      let Some(maxlen) = strict_i32(arg(input, curr_token_idx)) else {
        return Err(RESP_ERR_GENERIC_VALUE_IS_NOT_INTEGER.as_bytes());
      };
      curr_token_idx += 1;
