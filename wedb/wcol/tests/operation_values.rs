...
//!
//! 自研依据: 操作值向量解析（C# 对应 libs/server/Storage/Session/ObjectStore/*Ops.cs 的输入解析面）
...
fn object_sub_op_values_are_stable() {
  ...
  assert_eq!(HASH_OPS_GOLDEN.len(), 19, "HashOperation 黄金计数漂移");
  assert_eq!(
...
fn object_sub_op_bands_are_dense_and_bounded() {
  assert_dense_and_bounded("HashOperation", 19, |v| {
    // 9 原为 Hlen，全链死码清理后空置；判别值只追加不重排，9 留空防后序落盘值漂移
    v == 9 || HashOperation::from_repr(v).is_some()
  });
