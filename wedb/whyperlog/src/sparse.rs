...
impl HyperLogLog {
  ...
  /// init_sparse 落形后的初始 current 基座（稀疏头 + 128B 零段）
  ///
  /// libs/server/Resp/HyperLogLog/HyperLogLog.cs:InitSparse
  #[inline]
  pub fn sparse_init_current(&self) -> usize {
    self.sparse_header_size + self.sparse_zero_ranges
  }

  /// 稀疏写容纳唯一前置谓词：分配长度能否容纳批量稀疏更新的物理写峰值
  ///
  /// 稀疏流每次寄存器更新的物理写峰为零段拆分后缀右移 `end + blen - 1`
  /// （`UpdateSparseReg` 尾移，blen ≤ 3），即每单元最坏 +2B 且峰值字节恰落在
  /// 新流尾之后一格——容纳充要条件 `current + 2*count < alloc`（strict-<，
  /// 代数恒等于 1B 峰值裕度）。C# 命令路径恒以 count=1 逐元素驱动存储算子
  /// （HyperLogLogCommands.cs:34 Slice(i,1)），该裕度结构性恒成立；rust
  /// PFADD/PFMERGE 批量折叠以 count=N 调用同式分配点，一切稀疏出形必须过
  /// 本谓词显式前置校验（task zcode-r151c-pfconv 案一，形为唯一法定形见
  /// doc/zh/deviations.md §135）——三分配点（hll_init_payload 建键上探、
  /// hll_add_payload 扩容复检、hll_merge_payload 合并复检）与原位臂
  /// [`Self::can_grow_in_place`]、原位合并 [`Self::try_merge`] 同源单判据。
  ///
  /// libs/server/Resp/HyperLogLog/HyperLogLog.cs:CanGrowInPlace
  #[inline]
  pub fn sparse_fits(&self, current: usize, count: usize, alloc: usize) -> bool {
    current + (SPARSE_MAX_BYTES_PER_INSERT * count) < alloc
  }

  /// 现有分配能否原位容纳 count 个新元素（谓词 [`Self::sparse_fits`] 的切片视角壳）
  ///
  /// libs/server/Resp/HyperLogLog/HyperLogLog.cs:CanGrowInPlace
  #[inline]
  pub fn can_grow_in_place(&self, value: &[u8], value_len: usize, count: usize) -> bool {
    self.sparse_fits(self.sparse_current_size_in_bytes(value), count, value_len)
  }

  /// 增长后的新载荷长度：稀疏按需扩张（超上限稠密化），稠密不变
  ///
  /// C# 逐元素 count=1 使尾移写峰 1B 裕度恒成立，本式无须设防；rust 批量
  /// 折叠以 count=N 消费本出形，roundup128(2N)==2N 之形恰缺裕度——调用方
  /// 须以唯一容纳谓词 [`Self::sparse_fits`] 复检、不足升稠密
  ///（task zcode-r151c-pfconv 案一，形登记见 doc/zh/deviations.md §135）
  ///
  ...
  ///
  /// C# 逐元素合并粒度下裕度恒成立，rust 批量折叠须以唯一容纳谓词
  /// [`Self::sparse_fits`] 对本出形及等长原位形复检（与 add 轨同式同界；
  /// 冷臂形冲突面归案二登记，task zcode-r151c-pfconv，
  /// 见 doc/zh/deviations.md §135）
  ///
