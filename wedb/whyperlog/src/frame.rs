...
impl HyperLogLog {
  ...
  pub fn is_valid_hyll(&self, ptr: &[u8]) -> bool {
    self.is_valid_hyll_len(ptr, ptr.len())
  }

  /// 魔数 + 长度校验（重载 IsValidHYLL(byte*, int)）
  #[inline]
  pub fn is_valid_hyll_len(&self, ptr: &[u8], length: usize) -> bool {
    let Some(slice) = ptr.get(..length) else {
      return false;
    };
    Self::is_hyll(slice) && self.is_valid_hll_length(slice, length)
  }
  ...
  pub fn is_hyll(ptr: &[u8]) -> bool {
    ptr.get(4..8) == Some(&HYLL_MAGIC.to_le_bytes())
  }
