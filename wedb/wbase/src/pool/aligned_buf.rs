...
impl Clone for AlignedBuf {
  fn clone(&self) -> Self {
    ...
    // SAFETY: self.cap != 0（上方分支已处理零容量）且 cap/align 沿用原分配的合法布局，非零对齐为 2 的幂
    let layout = unsafe { Layout::from_size_align_unchecked(self.cap, self.align) };
    // SAFETY: layout 合法（见上行），返回空仅表示分配失败，交由 `handle_alloc_error` 处置
    let raw = unsafe { alloc_zeroed(layout) };
    let Some(ptr) = NonNull::new(raw) else {
      handle_alloc_error(layout);
    };
    if self.len > 0 {
      // SAFETY: self.len <= self.cap，源与目的均为各自 cap 字节已分配堆块，深拷贝区间不重叠
      unsafe {
