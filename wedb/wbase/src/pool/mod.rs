...
pub use limited::{
  DEFAULT_BUFFER_SIZE, DEFAULT_MAX_POOL_SIZE, LimitedFixedBufferPool, PooledRefBuffer,
};
...
impl BufferPool {
  ...
  /// 单线程本地保留字节上限 (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:ThreadLocalByteCap)
  #[inline]
  #[must_use]
  pub fn thread_local_byte_cap(&self) -> usize {
    ...
    if cls < self.first_large_class {
      ...
      let (cached_opt, inbox) = TLS_POOLS
        .try_with(|mgr| {
          ...
          // 1a. 本地私有栈 (0 锁、0 原子操作)
          if let Some(node) = entry.pop_local(cls) {
  ...
  /// 记录一次绕过池缓存的直配 (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:RecordBypassAlloc 与 RecordAlloc：超界与关闭态路径)
  #[inline]
