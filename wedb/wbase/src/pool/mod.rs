...
impl BufferPool {
  ...
  /// 注册线程本地跨线程收件箱弱引用 (对标 C# `poolShardRegistry.Add`)
  ...
  /// 动态调整单线程本地保留字节上限 (对标 C# `ThreadLocalByteCap`)
  #[inline]
  pub fn set_thread_local_byte_cap(&self, cap: usize) {
    self
      .thread_local_byte_cap
      .store(cap.max(MIN_THREAD_LOCAL_BYTES), Relaxed);
  }

  /// 当前调用线程在当前池中保留的本地缓存总字节数 (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:CallerLocalBytes)
  #[must_use]
  pub fn caller_local_bytes(&self) -> usize {
    TLS_POOLS
      .try_with(|mgr| mgr.borrow().find(self.pool_id).map_or(0, |e| e.local_bytes))
      .unwrap_or(0)
  }

  /// 当前调用线程在指定 class 保留的本地缓存字节数 (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:CallerLocalBytesForClass)
  #[must_use]
  pub fn caller_local_bytes_for_class(&self, cls: usize) -> usize {
    if cls >= NUM_CLASSES {
      return 0;
    }
    TLS_POOLS
      .try_with(|mgr| {
        mgr
          .borrow()
          .find(self.pool_id)
          .map_or(0, |e| e.class_bytes[cls])
      })
      .unwrap_or(0)
  }

  /// 排空全局条带仓库中所有缓存的缓冲区并释放预算许可 (不关闭池)
  pub fn clean_depot(&self) {
    self.depot.drain(|cls, cap| {
