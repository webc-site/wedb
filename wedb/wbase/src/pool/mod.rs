...
/// 单 class 缓存总量 = 条带数 × 容量 = 64，本地栈上限由 per-thread 字节封顶约束)
pub const DEPOT_STRIPE_CAP: usize = 8;

/// 指定 class 的扇区容量 (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:ClassCapacitySectors 与 TestClassCapacitySectors，1:1 算术)
///
...
///
/// 测试口径重载（同式）：
/// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:TestClassCapacitySectors
#[inline]
#[must_use]
pub const fn class_capacity_sectors(cls: usize) -> usize {
  if cls < TINY_EXACT_CLASSES {
    ...
    // 越界 class 的移位量钳制到位宽后交给 checked_shl 统一饱和为 usize::MAX
    let shift = if exp > usize::BITS as usize {
      usize::BITS as usize
    } else {
      exp
    };
    match base.checked_shl(shift as u32) {
      Some(cap) => cap,
      None => usize::MAX,
    }
  }
...
pub const fn class_capacity_bytes(cls: usize, sector_size: usize) -> usize {
  if cls < NUM_CLASSES {
    CLASS_CAPACITIES_SECTORS[cls].saturating_mul(sector_size)
  } else {
    usize::MAX
  }
}

/// 按扇区数选择 size class；超出可池化范围返回 None (走 bypass，对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:ClassOfSectors 与 TestClassOfSectors)
///
/// 测试口径重载（同式）：
/// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:TestClassOfSectors
#[inline]
#[must_use]
...
pub struct BufferPool {
  ...
  sector_size: usize,
  sector_shift: u32,
  small_budget: CachePadded64<Budget>,
  large_budget: CachePadded64<Budget>,
  first_large_class: usize,
  thread_local_byte_cap: AtomicUsize,
  inbox_registry: Mutex<Vec<Weak<CrossThreadInbox>>>,
  depot: Depot,
...
impl BufferPool {
  ...
  /// 以指定小/大预算创建缓冲池 (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:InitOriginReturn 与 AcquireSlot)
  ///
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:AcquireSlot
  pub fn with_budgets(
    sector_size: usize,
    ...
    for budget in [small_budget_bytes, large_budget_bytes] {
      if budget < 0 {
        return Err(Error::InvalidBudget(budget));
      }
    }
    ...
    let expected_threads = available_parallelism()
      .map(|n| (n.get() * 2).min(64))
      .unwrap_or(2)
      .max(1);
    let thread_local_byte_cap =
      ((small_budget_bytes as usize) / expected_threads).max(MIN_THREAD_LOCAL_BYTES);

    Ok(Arc::new(Self {
      pool_id,
      sector_size,
      sector_shift,
      small_budget: CachePadded64::new(Budget::new(small_budget_bytes)),
      large_budget: CachePadded64::new(Budget::new(large_budget_bytes)),
      first_large_class: (0..NUM_CLASSES)
        .find(|&c| class_capacity_bytes(c, sector_size) > LARGE_TIER_MIN_BYTES)
        .unwrap_or(NUM_CLASSES),
      thread_local_byte_cap: AtomicUsize::new(thread_local_byte_cap),
  ...
  pub(crate) fn register_inbox(&self, inbox: &Arc<CrossThreadInbox>) {
    let mut reg = self.inbox_registry.lock();
    if reg.len() >= 32 {
      reg.retain(|w| w.strong_count() > 0);
    }
    reg.push(Arc::downgrade(inbox));
  }

  /// 获取统计指标快照（容量配错诊断入口，对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:PrintOriginReturn 与 SnapshotAllocs 与 SnapshotReuses）
  ///
  ...
  ///
  /// C# 快照/调试输出的统一落点：
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:SnapshotAllocs
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:SnapshotReuses
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.cs:ToString
  ...
  /// 显式关闭并释放缓冲池中所有缓存的缓冲区 (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:FreeOriginReturn 与 ReleaseSlot 与 DropChain)
  ///
  /// C# 池关闭/许可释放/链丢弃的统一落点：
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:ReleaseSlot
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:Close
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:ReleaseChainPermits
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:DropChain
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.cs:Free
  ///
  /// 观测口径：不仅回收调用线程可见的部分（全局 Depot + 本线程 TLS 私有栈），
  /// 还会通过跨线程收件箱注册表原子封闭并清空所有活跃异线程的在途收件箱并释放许可；
  /// 他线程 TLS 中的空闲缓冲随各线程退出（TLS 析构）或后续借还路径探测到池关闭
  /// 时释放，预算许可同步归还，`reserved_bytes` 最终一致归零
  pub fn free(&self) {
    if self.is_closed.swap(true, AcqRel) {
      return;
    }
    // 1. 密封并排空所有注册的跨线程收件箱，杜绝在途跨线程缓冲在异线程泄漏 (对标 C# FreeOriginReturn 遍历 poolShardRegistry 并 SealAndDrainShard)
    let inboxes: Vec<_> = {
      let mut reg = self.inbox_registry.lock();
    ...
    for inbox in inboxes {
      for cls in 0..NUM_CLASSES {
        let chain = inbox.seal_and_drain(cls);
        if !chain.is_null() && chain != SEALED {
          // node 出作用域经 CachedBuf::drop 释放内存，先归还预算许可
          for node in ChainIter::new(chain) {
            self.budget_for(cls).release(node.cap as i64);
          }
        }
      }
    }

    // 2. 清空全局 Depot 并释放配额 (条带在锁内原子关闭，此后一切推入必然失败并就地释放)
    self.depot.clear(|cls, cap| {
      self.budget_for(cls).release(cap as i64);
    });
    // 3. 清空当前线程 TLS 中属于本池的缓存
    self.drain_tls_self();
  }
  ...
  ///
  /// libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.cs:Get
  ///
  ...
  pub fn get_with_policy(
    ...
    let Some(cls) = class_of_sectors(required >> self.sector_shift) else {
      ...
      return self.bypass_alloc(required);
    };
    ...
    //    大容量 class (>= first_large_class)：全局条带共享，不占本地栈与收件箱
    let (cached_opt, inbox) = if cls < self.first_large_class {
      ...
      TLS_POOLS
        .try_with(|mgr| {
          ...
          // 1a. 本地私有栈 (0 锁、0 原子操作)
          let mut cached = entry.pop_local(cls);
          if cached.is_none() {
            // 1b. 批量收割跨线程收件箱：单次 CAS 整链获取，按本地字节上限拼接，超出部分溢出 Depot (对标 C# SpliceIntoLocal)
            let chain = entry.inbox.claim(cls);
            if !chain.is_null() {
              let mut iter = ChainIter::new(chain);
              cached = iter.next();
              for node in iter {
                if entry.local_bytes.saturating_add(node.cap)
                  <= self.thread_local_byte_cap.load(Relaxed)
                {
                  entry.push_local(cls, node);
                } else {
    ...
    // 2. 全局条带仓库 (8-way 分片工作窃取) 仍未命中：系统新分配并预留预算
    if let Some(node) = cached_opt.or_else(|| self.depot.pop(cls, tid)) {
      return Ok(self.reuse_cached(node, cls, required_bytes, clear_on_return, tid, inbox));
    }
    self.issue_new(cls, required_bytes, clear_on_return, tid, inbox)
  }

  /// 绕过池缓存的精确容量直配并计入 BypassAllocs (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:RecordBypassAlloc 与 RecordAlloc：超界与关闭态路径)
  #[inline]
  fn bypass_alloc(&self, required: usize) -> Result<AlignedBuf> {
    self.bypass_alloc_count.fetch_add(1, Relaxed);
    self.bypass_alloc_bytes.fetch_add(required as u64, Relaxed);
    AlignedBuf::new(required, self.sector_size)
  }
  ...
  fn reuse_cached(
    ...
    AlignedBuf::from_cached(
      node,
      self.clone(),
      BufMeta::issued(cls, cacheable, clear_on_return, required, tid, inbox),
    )
  }
  ...
  /// 字节超标时通过 TryMakeRoom 逐出最超额 class，仍无法容纳则溢出转移至全局条带仓库
  /// (对标 libs/storage/Tsavorite/cs/src/core/Utilities/BufferPool.OriginReturn.cs:PushLocal)
  fn return_owner(&self, cls: usize, node: CachedBuf, tid: u64) {
    let mut hold = Some(node);
    let mut victims = Vec::new();
    let _ = TLS_POOLS.try_with(|mgr| {
      let mut mgr = mgr.borrow_mut();
      let Some(entry) = mgr.find_mut(self.pool_id) else {
        return;
      };
      // SAFETY: hold 在进入闭包前刚初始化为 Some(node)
      let node = unsafe { hold.take().unwrap_unchecked() };
      let bytes = node.cap;
      let cap = self.thread_local_byte_cap.load(Relaxed);
      if entry.local_bytes.saturating_add(bytes) <= cap {
        entry.push_local(cls, node);
      } else {
        let (made_room, spilled) = entry.try_make_room(cls, bytes, cap, self.first_large_class);
        victims = spilled;
        if made_room {
          entry.push_local(cls, node);
        } else {
    ...
    for (v_cls, v_buf) in victims {
      self.spill_to_depot(v_cls, v_buf, tid);
    }
