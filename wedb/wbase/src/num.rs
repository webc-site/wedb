...
/// [`try_parse`] 的 i64 特化版（对应 C# NumUtils 的 `out long` 重载）
#[inline]
...
/// 严格整数文法单点（C# RespReadUtils.TryReadInt32Safe / TryReadInt64Safe
/// allowLeadingZeros:false + 整段消费的扫描段）：可选 `+`/`-` 号，首数字
/// '0' 且仍有后续数字即拒绝（"0"/"-0" 合法，"007" 非法），非数字字节与超
/// u64 幅值拒绝；返回（十进制幅值, 是否负号），值域门另由调用方按目标宽度判定
#[inline]
...
pub enum DbIndexError {
  /// 非纯数字、空串、符号后无数字、前导零、尾随垃圾或超出 int32 值域
  /// （C# TryGetInt 失败档）
  NotInteger,
...
/// 不得放宽到 u64：
/// - [`strict_i32`] 失败（非纯数字、空串、符号后无数字、前导零、尾随垃圾、
///   超 int32 值域；`-2147483648` 是 C# 合法 int，`-2147483649` 起非法）
...
mod tests {
  ...
  fn test_parse_db_index() {
    ...
    // 非整数档：非法文法与超 int32 值域同档（C# TryGetInt 失败）
