...
///
/// libs/storage/Tsavorite/cs/src/core/Utilities/Utility.cs:PreviousPowerOf2
///
/// 全仓向下取幂的唯一实现点，`usize` 口径按 `prev_power_of2(n as u64) as usize` 转调：
/// - `wkv::config::StoreConfig::from_memory_budget_with_keys` 内存预算推导
///   （页容量 / 索引桶数 / 环形缓冲页数）；
/// - `wnode::service::apply_hlog_overrides` 节点参数 hlog 段的页数推导。
#[inline]
#[must_use]
...
/// 判断数值是否按指定字节对齐
/// libs/storage/Tsavorite/cs/src/core/Utilities/Utility.cs:IsAligned
#[inline(always)]
...
/// libs/storage/Tsavorite/cs/src/core/Utilities/Utility.cs:RoundDown
#[inline(always)]
...
///
/// libs/storage/Tsavorite/cs/src/core/Utilities/Utility.cs:RoundUp
#[inline(always)]
#[must_use]
...
/// 刷盘物理区间的扇区界圆整：起点向下、终点对齐到 `sector` 整数倍
///
/// 全仓刷盘/腾窗物理边界的唯一圆整点（对位 C#
/// libs/storage/Tsavorite/cs/src/core/Allocator/AllocatorBase.cs 内 WriteInlinePageAsync
/// 的 `aligned_start / aligned_end` RoundDown/RoundUp 往返计算，whlog 与 waof 共用；
/// 该 C# 方法的 1:1 刷盘内核锚点登记在 `wdev::Device::flush_range_aligned`，此处不复挂）；
/// 调用方不再各自内联 align_down/align_up 组合。
#[inline(always)]
#[must_use]
pub const fn sector_bounds(from: u64, to: u64, sector: u64) -> (u64, u64) {
  (align_down(from, sector), align_up(to, sector))
}
