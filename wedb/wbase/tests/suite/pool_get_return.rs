...
/// 本地保留字节上限硬约束：当多缓冲归还触发 try_make_room 时，caller_local_bytes 严格不超过 cap
#[test]
fn thread_local_retention_byte_cap_strictly_enforced() -> Void {
  info!("验证 TryMakeRoom 腾挪逻辑：本地缓存总字节数恒不超过 thread_local_byte_cap");
  ...
  for _ in 0..100 {
    ...
    buffers.push(pool.get(8192)?);
    buffers.push(pool.get(16384)?);
  }
  // 逐一释放归还入池
  drop(buffers);

  let local_bytes = pool.caller_local_bytes();
  assert!(
    local_bytes <= cap,
    "本地保留字节数 {local_bytes} 严禁超过 thread_local_byte_cap {cap}"
  );
