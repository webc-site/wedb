...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/SectorAlignedBufferPoolTests.cs（GetReturnsZeroedBufferAndReuses 等）

use aok::{OK, Void};
use log::info;
use wbase::{
  align::{DEFAULT_SECTOR_SIZE, MIN_SECTOR_SIZE},
  pool::BufferPool,
};
...
/// 0 字节请求返回空缓冲区
#[test]
fn zero_byte_request_is_safe() -> Void {
  info!("验证 get(0) 空缓冲语义");
  ...
  // 空缓冲语义可重复获取（AlignedBuf 无 Clone：深拷贝臂无法以签名平滑表达
  // 分配失败，票 zcode-r55-alloc）：再次 get(0) 同样空且对齐元数据完好
  let again = pool.get(0)?;
  assert_eq!(again.capacity(), 0);
  assert!(again.is_aligned_to(again.align()));
  drop((empty, again));
