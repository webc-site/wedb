...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/SectorAlignedBufferPoolTests.cs（对齐缓冲零化与复用）
...
use log::info;
use wbase::{
  align::{DEFAULT_SECTOR_SIZE, MIN_SECTOR_SIZE, SectorRange, is_aligned},
  error::Error,
  pool::AlignedBuf,
};
...
fn aligned_pointer_matches_expected_alignment() -> Void {
  ...
  for sector_size in [MIN_SECTOR_SIZE, DEFAULT_SECTOR_SIZE, 8192] {
    for size in [1, 511, 512, 1000, 4096, 7777] {
      ...
      assert!(buf.is_aligned_to(buf.align()));
...
fn new_rejects_invalid_alignment_and_supports_zero_capacity() -> Void {
  ...
  assert!(empty.is_aligned_to(empty.align()));
...
fn set_len_clear_and_len_bound_checks() -> Void {
  ...
  let mut buf = AlignedBuf::new(DEFAULT_SECTOR_SIZE, DEFAULT_SECTOR_SIZE)?;
...
/// from_slice 拷贝创建、Deref/DerefMut 读写与显式重建副本独立性
#[test]
fn from_slice_deref_mut_and_copy_independence() -> Void {
  info!("验证 from_slice 数据拷贝、DerefMut 原地修改与显式重建副本互不影响");
  ...
  // 显式重建为独立副本（AlignedBuf 无 Clone：深拷贝臂无法以签名平滑表达
  // 分配失败，票 zcode-r55-alloc）：修改副本不影响原缓冲区
  let mut sec_buf = AlignedBuf::new(DEFAULT_SECTOR_SIZE, DEFAULT_SECTOR_SIZE)?;
  sec_buf.as_allocated_slice_mut()[0..4].copy_from_slice(b"TEST");
  sec_buf.set_len(4)?;
  let mut copied = AlignedBuf::from_slice(sec_buf.as_slice(), DEFAULT_SECTOR_SIZE)?;
  assert_eq!(copied, sec_buf);
  copied[0] = b'B';
  assert_eq!(&copied[..], b"BEST");
  ...
  let copied = AlignedBuf::from_slice(zbuf.as_slice(), DEFAULT_SECTOR_SIZE)?;
  assert_eq!(copied.len(), DEFAULT_SECTOR_SIZE);
  assert_eq!(copied.capacity(), DEFAULT_SECTOR_SIZE);
  assert_eq!(copied[0], 0xAA);
  assert_eq!(copied[DEFAULT_SECTOR_SIZE - 1], 0xBB);
  assert_eq!(copied, zbuf);
...
fn concurrent_readers_share_immutable_buffer() -> Void {
  ...
  for _ in 0..8 {
    ...
    handles.push(thread::spawn(move || {
      ...
      assert!(s.is_aligned_to(s.align()));
    }));
  }
  for h in handles {
