...
/// [`RespWriter::write_error`]、[`RespWriter::write_error_with_prefix`] 与 RESP2 的
/// [`Resp2::write_bulk_error`]（bulk error 在 RESP2 退化为简单错误，语义同上）
...
pub trait RespProtocol: Copy + Default + Send + Sync + 'static {
  ...
  /// 写 null bulk string 应答
  fn write_null_bulk_string(buf: &mut Vec<u8>);
  ...
  /// 写 attribute 长度头
  fn write_attribute_len(buf: &mut Vec<u8>, len: usize);
  ...
  /// 写大整数（BigNumber）
  fn write_bignum(buf: &mut Vec<u8>, bignum: &[u8]);

  /// 写 Bulk Error
  fn write_bulk_error(buf: &mut Vec<u8>, err: &[u8]);
...
impl RespProtocol for Resp2 {
  ...
  #[inline(always)]
  fn write_bignum(buf: &mut Vec<u8>, bignum: &[u8]) {
    write_bulk_string_to(buf, bignum);
  }

  #[inline(always)]
  fn write_bulk_error(buf: &mut Vec<u8>, err: &[u8]) {
    // RESP2 无 bulk error，退化为简单错误帧：与错误应答共用唯一成帧点（自动清洗）
    write_error_frame_to(buf, b"", err);
  }
...
impl RespProtocol for Resp3 {
  ...
  #[inline(always)]
  fn write_bignum(buf: &mut Vec<u8>, bignum: &[u8]) {
    buf.reserve(1 + bignum.len() + 2);
    buf.push(b'(');
    buf.extend_from_slice(bignum);
    buf.extend_from_slice(b"\r\n");
  }
  ...
  fn write_bulk_error(buf: &mut Vec<u8>, err: &[u8]) {
    ...
    let len_str = buf_int.format(err.len()).as_bytes();
    buf.reserve(1 + len_str.len() + 2 + err.len() + 2);
    buf.push(b'!');
...
impl<B: RespBuffer, P: RespProtocol> RespWriter<B, P> {
  ...
  /// 写 null bulk string 应答：根据 P 静态分派（RESP3 `_\r\n`，RESP2 `$-1\r\n`）
  #[inline(always)]
  pub fn write_null_bulk_string(&mut self) {
    P::write_null_bulk_string(self.buf_mut());
  }
  ...
  /// 写大整数（RESP3 为 `(<bignum>\r\n`，RESP2 降级为 bulk string）
  #[inline(always)]
  pub fn write_bignum(&mut self, bignum: &[u8]) {
    P::write_bignum(self.buf_mut(), bignum);
  }

  /// 写 Bulk Error（RESP3 为 `!<len>\r\n<err>\r\n`，RESP2 为 `-<err>\r\n`）
  ///
  /// RESP2 分支即简单错误帧，自动经唯一成帧点清洗；RESP3 分支为长度前缀帧，
  /// 客户端按 `<len>` 取正文，正文内 CRLF 不构成第二帧，故原样写出。
  #[inline(always)]
  pub fn write_bulk_error(&mut self, err: &[u8]) {
    P::write_bulk_error(self.buf_mut(), err);
  }
