...
//!
//! 在 garnet 中的相对路径: libs/server/Resp/CommandsInfo.cs（COMMAND INFO）
...
pub(crate) fn acl_category_descriptions(cats: RespAclCategories) -> Vec<&'static str> {
  RespAclCategories::CATEGORIES
    .iter()
    .filter(|(_, cat)| cats.contains(*cat))
    .map(|(desc, _)| *desc)
    .collect()
}
...
pub(super) fn build_tables(imported: &[RespCommandsInfo]) -> Option<RespCommandsTables> {
  ...
  let mut acl_command_info: HashMap<u32, Vec<RespCommandsInfo>> = HashMap::default();
  ...
  let fast_basic: Vec<Option<RespCommandsInfo>> = (0..fast_len)
    .map(|i| {
      flattened
        .get(&((i + FIRST_DATA_COMMAND as usize) as u16))
        .cloned()
    })
    .collect();
...
/// 产出位集中的每个单类别位（逐位隔离，驱动 AclCommandInfo 类别→命令索引构建）
///
...
pub fn try_get_commandsfor_acl_category(
  ...
  Some(tables.acl_command_info.get(&acl.bits())?.iter().collect())
}
...
pub fn try_fast_get_resp_command_info(cmd: RespCommand) -> Option<&'static RespCommandsInfo> {
  let tables = super::tables()?;
  // 同前：cmd 低于区间下界时减法溢出即 panic（与基线口径一致）
  let offset = cmd as usize - FIRST_DATA_COMMAND as usize;
  tables.fast_basic.get(offset).and_then(|slot| slot.as_ref())
}
...
mod tests {
  ...
  fn tables_initialize_and_lookup() {
    ...
    // 计数快照：清理 MODULE 和 REGISTERCS、以及动态注册自定义命令哨兵
    //（CustomTxn / CustomRawStringCmd / CustomProcedure，仅余静态清单复用的
    // CustomObjCmd）后为 259 根命令（其中 1 个内部命令，258 个外部命令；含本仓
    // 自定义扩展 RI.COUNT 与 SUNSUBSCRIBE）
