...
pub use simplified::{
  ...
  SimpleRespKeySpecFindKeys, populate_simple_command_info, try_get_simple_key_spec,
};
...
mod tests {
  ...
  /// 目录全量锚定：262 根 + 94 子 = 356 条（含 4 条内部根命令与两条
  /// SECONDARYOF 历史别名条目，逐条对照内嵌 JSON；相对 C# 356 条：删
  /// module/module|loadcs/registercs 三条模块加载命令、增
  ...
  fn catalog_size_and_aliases() {
    ...
    assert_eq!(entries().iter().filter(|e| e.parent.is_none()).count(), 262);
