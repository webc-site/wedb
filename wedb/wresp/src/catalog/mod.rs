...
//! 静态 `RespCommandsInfo` 取命令元数据（名称 / 父子关系 / 分类归属）；rust
//! 侧以 [`CMD_ENTRIES`] 静态表承接（数据自
//! garnet/libs/resources/RespCommandsInfo.json 生成），全部查询为无锁单遍
//! 扫描——目录仅 356 条且 ACL 规则修改是管理频度操作。
...
use data::CMD_ENTRIES;
pub use data::CmdEntry;
...
pub fn commands_for_category(acl: RespAclCategories) -> impl Iterator<Item = &'static CmdEntry> {
  CMD_ENTRIES.iter().filter(move |e| e.cats.intersects(acl))
}
