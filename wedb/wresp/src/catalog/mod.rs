...
//! 的 false 语义。
//!
//! 在 garnet 中的相对路径: libs/server/Resp/CommandsInfo? 对标 C# 命令目录
...
pub use commands_info::{
  ...
  try_get_resp_commands_info, try_get_resp_commands_info_count, try_get_resp_commands_info_ordered,
...
impl RespAclCategories {
  /// C# 枚举成员名 → 类别位（声明序，单一数据源）
  pub const CATEGORIES: [(&'static str, Self); 24] = [
    ("admin", Self::ADMIN),
    ("bitmap", Self::BITMAP),
...
/// JSON 文本 → 完整元数据索引纯函数面（不落 CATALOG 静态；集成测试以合成
/// JSON 对拍 C# 数据源形态，锁 Command 缺键值缺省语义与跨父重名熔断判据）
///
/// libs/server/Resp/RespCommandsInfo.cs:TryInitializeRespCommandsInfo 纯函数形态
