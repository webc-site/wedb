...
//! [`wresp::catalog::SimpleRespKeySpec`]（对标
//! libs/server/SessionParseStateExtensions.cs），此处不设第二套提取实现。
//!
//! 在 garnet 中的相对路径: libs/server/Resp/CommandKeys? 对标 C# RespCommand KeyArgumentsSpec
...
impl KeySpecificationFlags {
  ...
  fn from_wire_name_single(name: &str) -> Option<Self> {
    ...
    ALL_FLAGS.iter().find_map(|(flag, wire)| {
      let mut it_a = s.bytes().filter(|&c| c != b'_');
      let mut it_b = wire.bytes().filter(|&c| c != b'_');
      let matches = loop {
