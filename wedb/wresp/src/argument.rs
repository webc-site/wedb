...
use crate::{IRespSerializable, RespBuffer, RespProtocol, RespWriter};
...
impl RespCommandArgumentFlags {
  ...
  pub fn from_member_name(name: &str) -> Option<Self> {
    ...
    for part in name.split(',') {
      ...
      let bit = if trimmed.eq_ignore_ascii_case("NONE") {
        Self::NONE
...
/// libs/server/Resp/RespCommandArgument.cs:ArgumentBase
///
...
mod tests {
  ...
  fn key_argument_resp_format() {
    ...
    let mut w = crate::RespWriter::<Vec<u8>, crate::Resp3>::new_p();
