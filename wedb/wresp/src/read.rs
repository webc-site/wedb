...
/// C# RespParsingException.ThrowIntegerOverflow 的
/// `Encoding.ASCII.GetString(buffer, length)` 载荷：溢出数字串恒为 ASCII 数字，
/// 仅在违例路径分配文本供上层回显。该 C# 符号的违例臂本体单点对位在
/// wnode `violation_integer_overflow`，本函数只承担其中 GetString 一拍的
/// 文本化，不持符号锚
#[cold]
  ...
  *value = 0;
  ...
  while !read_head.is_empty() {
    ...
    if (*value == 1844674407370955161 && next_digit > 5) || (*value > 1844674407370955161) {
      // C# ThrowIntegerOverflow(ptr, readHead - ptr)：回显已读数字串（不含触发位）
      return Err(Error::IntegerOverflow {
        digits: ascii_digits(&(*ptr)[..i]),
      });
    }
...
///
/// 全仓 RESP 定长帧头（`$len\r\n` / `*count\r\n` / `%` / `~` / `=` 同形头）
/// 解码单点：仅 `-` 前导负号被识别（C# :362 取 `*readHead == '-'`），`+` 前导
/// 落 `digitsRead == 0` 判 UnexpectedToken（C# :404）——C# 的 `TryReadSign`
/// （接受 `+`）仅用于整数值解析（:165/:265），帧头从无正号容度。
/// 三态语义：`Err` = 协议违例（须断连）、`Ok(false)` = 字节未到齐（须等待）
#[inline]
pub fn try_read_signed_length_header(
  ...
  *length = -1;
  ...
  if value > i32::MAX as u64 && (!negative || value > (i32::MAX as u64) + 1) {
    // C# ThrowIntegerOverflow(readHead - digitsRead, digitsRead)：回显数字串
    return Err(Error::IntegerOverflow {
      digits: ascii_digits(&digits_at[..digits_read]),
    });
  }
  ...
  *length = if negative {
  ...
  if &read_head[0..2] != b"\r\n" {
    // C# RespReadUtils.cs:379 掷 readHead+2 处字节、批尾越 end 读邻位，rust 刻意取首个不符字节
    let unexpected = if read_head[0] != b'\r' {
      read_head[0]
    } else {
      read_head[1]
    };
...
pub fn try_read_span_with_length_header<'a>(
  ...
  // 与 [`try_slice_with_length_header`] 同一实现（行为恒等）：原前置
  // `ptr.len() < 3` 检查已被内层 `try_read_unsigned_length_header` 的同型检查覆盖
  try_slice_with_length_header(result, ptr)
}
...
pub enum ReplyError<'a> {
  /// `-ERR...\r\n` 错误应答（携带剥离 '-' 头与 CRLF 后的错误文本）
  ErrorReply(&'a [u8]),
  /// 其余不可识别形态（协议错误）
  Malformed,
}
...
/// 错误帧文本提取单点（`-TEXT\r\n` → `TEXT`；整包回包帧必完整，读失败归
/// Malformed）
fn error_reply_of(reply: &[u8]) -> ReplyError<'_> {
  let mut span: &[u8] = &[];
  let mut ptr = reply;
  match try_read_error_as_span(&mut span, &mut ptr) {
    Ok(true) => ReplyError::ErrorReply(span),
    _ => ReplyError::Malformed,
  }
}
...
///
/// `$len\r\n<payload>\r\n` → `Ok(Some(载荷))`；`$-1\r\n` → `Ok(None)`；
/// `-...\r\n` → `Err(ReplyError::ErrorReply(错误文本))`；其余 → `Err(ReplyError::Malformed)`
///
/// 头解码复用 [`try_read_ptr_with_signed_length_header`] 单点（本函数消费方为
/// 脚本 GET/SET 整包回包，帧必完整，故 `Ok(false)` 与违例同归 Malformed）
pub fn parse_bulk_reply(reply: &[u8]) -> result::Result<Option<&[u8]>, ReplyError<'_>> {
  match reply.first() {
    Some(b'$') => {
      let mut payload = None;
      let mut ptr = reply;
      if !try_read_ptr_with_signed_length_header(&mut payload, &mut ptr)
        .map_err(|_| ReplyError::Malformed)?
      {
        return Err(ReplyError::Malformed);
      }
      Ok(payload)
    }
    Some(b'-') => Err(error_reply_of(reply)),
...
///
/// `+...\r\n` → `Ok(())`；`-...\r\n` → `Err(ReplyError::ErrorReply(错误文本))`；
/// 其余 → `Err(ReplyError::Malformed)`
pub fn parse_simple_reply(reply: &[u8]) -> result::Result<(), ReplyError<'_>> {
  match reply.first() {
    Some(b'+') => Ok(()),
    Some(b'-') => Err(error_reply_of(reply)),
    _ => Err(ReplyError::Malformed),
