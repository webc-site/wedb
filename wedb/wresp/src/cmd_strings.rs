//! 在 garnet 中的相对路径: libs/server/Resp/CmdStrings.cs
...
use crate::{
  ext::{is_resp3, sanitize_error_str},
...
/// MIGRATE 空键形态显式拒绝（rust 相对 C# 的扩展文案，对标 Redis
/// src/cluster.c migrateCommand 同形态 addReplyError(c,"NOKEY")：键参数为
/// 空串且无 KEYS 选项、或 KEYS 后零键——C# 该形态走 MigrateSession 构造器
/// NRE 异常逃逸，无对应文案常量）
pub const NOKEY: &str = "NOKEY";
...
/// 登记见 doc/zh/deviations.md 条目 35）
...
/// rust 特有污染拒绝臂（对位 C# CmdStrings.cs:318 无符号裸文案经 TryWriteError
/// 单点成帧形态）：文案不含前导 `-` 与行尾 CRLF，负号由唯一成帧点
/// `resp_memory_writer::write_error_frame_to` 补加
pub const RESP_ERR_DEVICE_CONTAMINATED_REFUSING_CHECKPOINT: &str =
  "ERR device is contaminated by a failed checkpoint receive, refusing checkpoint";
/// rust 特有污染拒绝臂（FLUSH 族，同 [`RESP_ERR_DEVICE_CONTAMINATED_REFUSING_CHECKPOINT`]
/// 无符号裸文案单源形态）
pub const RESP_ERR_DEVICE_CONTAMINATED_REFUSING_FLUSH: &str =
  "ERR device is contaminated by a failed checkpoint receive, refusing flush";
/// rust 特有租户门禁拒绝臂（FLUSHALL UNSAFETRUNCATELOG 非 0 租户拦截，
/// 同无符号裸文案单源形态，负号经唯一成帧点补加）
pub const RESP_ERR_FLUSH_TRUNCATE_LOG_NS0: &str =
...
/// 槽位非全部由本地节点掌管、或任一槽位处于 MIGRATING/IMPORTING 迁移窗口时
/// 拦截，故偏离 C# 原文案（"not allowed in cluster mode"）改为归属判定语义
...
/// （rust 自有文案；C# CmdStrings 无对应异步要求错误常量，拓扑专有面。
/// 登记见 doc/zh/deviations.md 条目 35）
...
/// 对象命令未知子操作码的防御帧文案（rust 自有优雅帧，跨 hash/set/list/sortedset
/// 四对象域复用）
///
/// C# 无同名错误帧文案：四对象 Operate 的 switch default 臂各抛 GarnetException
/// "Unsupported operation {op} in {Type}Object.Operate" 走异常通道掐断连接——
/// libs/server/Objects/Hash/HashObject.cs:298、List/ListObject.cs:185、
/// Set/SetObject.cs:167、SortedSet/SortedSetObject.cs:449。登记见
/// doc/zh/deviations.md 条目 34
pub const RESP_ERR_GENERIC_UNSUPPORTED_OPERATION: &str = "ERR unsupported operation";
/// libs/server/Resp/CmdStrings.cs:GenericErrInvalidClientAttr（CLIENT SETINFO
/// 属性名校验，{0} = LIB-NAME|LIB-VER）
pub const GENERIC_ERR_INVALID_CLIENT_ATTR: &str =
  "ERR {0} cannot contain spaces, newlines or special characters.";
...
pub const GENERIC_ERR_UNKNOWN_SUBCOMMAND_OR_WRONG_NUM_ARGS: &str =
...
/// libs/server/Resp/CmdStrings.cs:GenericErrAtLeastOneKey（ZINTER/ZINTERCARD/
/// ZINTERSTORE/ZUNION/ZUNIONSTORE 族 numkeys < 1 校验，{0} = 命令名）
pub const GENERIC_ERR_AT_LEAST_ONE_KEY: &str =
...
/// [`GENERIC_ERR_SHOULD_BE_GREATER_THAN_ZERO`] 单源内联常量（client-id 专配，杜绝运行期 replace 堆分配）
pub const RESP_ERR_CLIENT_ID_GREATER_THAN_ZERO: &str = "ERR client-id should be greater than 0";
...
/// [`GENERIC_PARAM_SHOULD_BE_GREATER_THAN_ZERO`] 单源内联常量（numkeys 专配，杜绝运行期 replace 堆分配）
pub const RESP_ERR_PARAM_NUMKEYS_GREATER_THAN_ZERO: &str =
  "ERR Parameter `numkeys` should be greater than 0";
/// [`GENERIC_PARAM_SHOULD_BE_GREATER_THAN_ZERO`] 单源内联常量（count 专配，杜绝运行期 replace 堆分配）
pub const RESP_ERR_PARAM_COUNT_GREATER_THAN_ZERO: &str =
  "ERR Parameter `count` should be greater than 0";
/// libs/server/Resp/CmdStrings.cs:GenericErrMandatoryMissing（对象 TTL 族 {0} =
/// FIELDS|MEMBERS，C# 调用点逐命令传词元，不为每个词元复制常量）
pub const GENERIC_ERR_MANDATORY_MISSING: &str =
  "Mandatory argument {0} is missing or not at the right position";
/// libs/server/Resp/CmdStrings.cs:GenericErrMustMatchNoOfArgs（对象 TTL 族
/// {0} = numFields|numMembers）
pub const GENERIC_ERR_MUST_MATCH_NO_OF_ARGS: &str =
  "The `{0}` parameter must match the number of arguments";
...
pub const GENERIC_SYNTAX_ERROR_OPTION: &str = "ERR Syntax error in {0} option '{1}'";
...
/// 事务内禁 HELLO 认证/换租（wedb 自有面，对标 SELECT_IN_TXN_UNSUPPORTED 同族契约，登记见 doc/zh/deviations.md §58）
pub const RESP_ERR_HELLO_IN_TXN_UNSUPPORTED: &str =
  "ERR HELLO is currently unsupported inside a transaction.";
/// EXEC 重放窗禁 ACL 族执行（wedb 自有面：ACL 规则读写经存储点查须停车，
/// 事务窗零停泊红线拒之，文案分支不与 HELLO 共帧；排队面与 C# 同构照常
/// +QUEUED，登记见 doc/zh/deviations.md §58d）
pub const RESP_ERR_ACL_IN_TXN_UNSUPPORTED: &str =
  "ERR ACL is currently unsupported inside a transaction.";
...
/// libs/server/Resp/CmdStrings.cs:GenericErrIndexSizeAutoGrow
pub const GENERIC_ERR_INDEX_SIZE_AUTO_GROW: &str =
  "ERR Cannot adjust index size when auto-grow task is running (option: '{0}')";
...
/// LPOS 的 RANK 选项（libs/server/Resp/CmdStrings.cs:RANK）
pub const RANK: &[u8] = b"RANK";
/// LPOS 的 MAXLEN 选项（libs/server/Resp/CmdStrings.cs:MAXLEN）
pub const MAXLEN: &[u8] = b"MAXLEN";
...
#[inline]
pub fn abort_with_args(output: &mut Vec<u8>, template: &str, args: &[&str]) {
  const PLACEHOLDERS: [&str; 2] = ["{0}", "{1}"];
  let mut message = template.to_string();
  for (i, &arg) in args.iter().enumerate() {
    let clean = sanitize_error_str(arg, MAX_PARAM_NAME_LEN);
    if let Some(&ph) = PLACEHOLDERS.get(i) {
      message = message.replace(ph, clean);
    }
  }
  write_error_raw(output, &message);
}

/// 以 [`GENERIC_ERR_WRONG_NUM_ARGS`] 回填命令名并写出错误应答（对标 C#
/// `AbortWithErrorMessage(string.Format(CmdStrings.GenericErrWrongNumArgs, 命令名))`：
/// 组文案一次堆分配后落唯一成帧点，本函数不自拼协议字节）
#[inline]
pub fn abort_with_wrong_number_of_arguments(output: &mut Vec<u8>, cmd_name: &str) {
  abort_with_args(output, GENERIC_ERR_WRONG_NUM_ARGS, &[cmd_name]);
}

/// 以 [`GENERIC_ERR_AT_LEAST_ONE_KEY`]（CmdStrings.cs:GenericErrAtLeastOneKey）
/// 回填命令名并写出错误应答（对标 C#
/// libs/server/Resp/Objects/SortedSetCommands.cs:1068 等调用点
/// `AbortWithErrorMessage(CmdStrings.GenericErrAtLeastOneKey, nameof(RespCommand.*))`：
/// ZINTER/ZINTERCARD/ZINTERSTORE/ZUNION/ZUNIONSTORE 五命令归一，
/// 组文案一次堆分配后落唯一成帧点）
#[inline]
pub fn abort_with_at_least_one_key(output: &mut Vec<u8>, cmd_name: &str) {
  abort_with_args(output, GENERIC_ERR_AT_LEAST_ONE_KEY, &[cmd_name]);
}

/// 以 [`GENERIC_ERR_INVALID_CLIENT_ATTR`]（CmdStrings.cs:GenericErrInvalidClientAttr）
/// 回填属性名并写出错误应答
#[inline]
pub fn abort_with_invalid_client_attr(output: &mut Vec<u8>, attr: &str) {
  abort_with_args(output, GENERIC_ERR_INVALID_CLIENT_ATTR, &[attr]);
}

/// 以 [`GENERIC_PUBSUB_COMMAND_DISABLED`] 模板回填命令名并写出错误应答
#[inline]
pub fn abort_with_pubsub_command_disabled(output: &mut Vec<u8>, cmd_name: &str) {
  abort_with_args(output, GENERIC_PUBSUB_COMMAND_DISABLED, &[cmd_name]);
}

/// 以 [`GENERIC_ERR_UNSUPPORTED_OPTION`] 回填选项名并写出错误应答（经唯一成帧点）
#[inline]
pub fn abort_with_unsupported_option(output: &mut Vec<u8>, option: &str) {
  abort_with_args(output, GENERIC_ERR_UNSUPPORTED_OPTION, &[option]);
}

/// 以 [`GENERIC_SYNTAX_ERROR_OPTION`]（CmdStrings.cs:GenericSyntaxErrorOption）回填
/// 命令名与选项名并写出错误应答（经唯一成帧点，实参各过 MAX_PARAM_NAME_LEN 清洗帽）
#[inline]
pub fn abort_with_syntax_error_option(output: &mut Vec<u8>, cmd_name: &str, option: &str) {
  abort_with_args(output, GENERIC_SYNTAX_ERROR_OPTION, &[cmd_name, option]);
}

/// 以 [`GENERIC_ERR_UNKNOWN_SUB_COMMAND`] 回填子命令与命令名写出错误应答
/// （经唯一成帧点）
#[inline]
pub fn abort_with_unknown_subcommand(output: &mut Vec<u8>, sub_command: &str, cmd_name: &str) {
  abort_with_args(
    ...
    &[sub_command, cmd_name],
  );
}

/// 以 [`GENERIC_ERR_UNKNOWN_SUBCOMMAND_OR_WRONG_NUM_ARGS`] 回填写出未知子命令或
/// 错误参数数量应答（对标 C#
/// ObjectStoreUtils.cs:AbortWithWrongNumberOfArgumentsOrUnknownSubcommand，经唯一成帧点）
#[inline]
pub fn abort_with_unknown_subcommand_or_wrong_num_args(
  ...
  abort_with_args(
    output,
    GENERIC_ERR_UNKNOWN_SUBCOMMAND_OR_WRONG_NUM_ARGS,
    &[sub_command, cmd_name],
  );
}
...
mod tests {
  ...
  /// 模板回填族与集群拼帧族收编到唯一成帧点后，帧字节与收编前逐位一致；
  /// 实参 CRLF 经 [`sanitize_error_str`] 逐参截断（成帧点清洗对其幂等）
  #[test]
...
pub mod cluster {
  ...
  // [`crate::cmd_strings`] 单源，此处不重复。错误帧一律经
  // [`super::write_error_raw`] 落 wresp 唯一成帧点（对标 C# 侧
  // `$"MOVED {slot} {endpoint}:{port}"` 组串后 TryWriteError 的单一通路），
  // 本模块不自拼 `-` 与 `\r\n`。

  use wbase::ascii_sanitize;

  use super::write_error_raw;

  /// libs/cluster/CmdStrings.cs:RESP_ERR_CLUSTERDOWN
  pub const ERR_CLUSTERDOWN: &str = "CLUSTERDOWN Hash slot not served";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_TRYAGAIN
  pub const ERR_TRYAGAIN: &str = "TRYAGAIN Multiple keys request during rehashing of slot";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_INVALID_SLOT
  pub const ERR_INVALID_SLOT: &str = "ERR Invalid or out of range slot";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_SLOT_OUT_OFF_RANGE
  pub const ERR_GENERIC_SLOT_OUT_OFF_RANGE: &str = "ERR Slot out of range";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_CONFIG_EPOCH_ASSIGNMENT
  pub const ERR_GENERIC_CONFIG_EPOCH_ASSIGNMENT: &str =
  ...
  pub const ERR_GENERIC_CONFIG_UPDATE: &str = "ERR Updating the config epoch";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_RESET_WITH_KEYS_ASSIGNED
  pub const ERR_RESET_WITH_KEYS_ASSIGNED: &str =
  ...
  pub const ERR_GENERIC_CANNOT_FORGET_MYSELF: &str = "ERR I tried hard but I can't forget myself";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_CANNOT_FORGET_MY_PRIMARY
  pub const ERR_GENERIC_CANNOT_FORGET_MY_PRIMARY: &str = "ERR Can't forget my primary";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_CANNOT_FAILOVER_FROM_NON_MASTER
  pub const ERR_GENERIC_CANNOT_FAILOVER_FROM_NON_MASTER: &str =
    "ERR Cannot failover a non-master node";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_UNKNOWN_ENDPOINT
  pub const ERR_GENERIC_UNKNOWN_ENDPOINT: &str = "ERR Unknown endpoint";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_SLOT_STATE
  pub const ERR_GENERIC_SLOT_STATE: &str = "ERR Invalid slot state";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_MULTI_LOG_DISABLED
  pub const ERR_MULTI_LOG_DISABLED: &str = "ERR Multi-log disabled";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_MIGRATE_TO_MYSELF
  /// （CLUSTER SETSLOT MIGRATE/IMPORTING 指向自身，槽位域文案；与
  /// [`ERR_GENERIC_CANNOT_REPLICATE_SELF`] 同变体两域异文，语境分流见
  /// wedb::error::cluster_err_text）
  pub const ERR_GENERIC_MIGRATE_TO_MYSELF: &str = "ERR Can't MIGRATE to myself";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_CANNOT_REPLICATE_SELF
  /// （REPLICATE/REPLICAOF 指向自身，副本域文案）
  pub const ERR_GENERIC_CANNOT_REPLICATE_SELF: &str = "ERR Can't replicate myself";
  /// 「未知节点」同族句前缀（C# ClusterManagerSlotState.cs 六处、
  /// ClusterManagerWorkerState.cs 两处与 ReplicaOfCommand.cs 内联同族模板的
  /// 单源前缀；带参句以常量加 format 组串，句尾句号有无随各消费点对位
  /// C# 分支实测原样，词项对拍归 r28）
  pub const ERR_UNKNOWN_NODE_PREFIX: &str = "ERR I don't know about node ";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_IOERR
  pub const ERR_IOERR: &str = "IOERR Migrate keys failed";
  /// libs/cluster/CmdStrings.cs:RESP_ERR_GENERIC_VALUE_IS_NOT_BOOLEAN
  pub const ERR_GENERIC_VALUE_IS_NOT_BOOLEAN: &str = "ERR value is not a boolean.";
  ...
  pub const ERR_GENERIC_REPLICATION_AOF_TURNEDOFF: &str = "ERR Replication unaivalable because AOF is switched off, please restart replica with --aof option";
  /// libs/cluster/CmdStrings.cs:87 GenericErrInvalidPort（CLUSTER MEET 非法端口
  /// 错误文案模板；{0} 由消费点以端口字面原始字节的 ASCII 折叠形回填，对标 C#
  /// string.Format(模板, parseState.GetString(1)) → Encoding.ASCII 逐字节折 '?'）
  pub const GENERIC_ERR_INVALID_PORT: &str = "ERR Invalid TCP base port specified: {0}";
  ...
  pub const ERR_MALFORMED_APPENDLOG_FRAME: &str = "ERR malformed APPENDLOG frame";
  ...
  pub const ERR_PROTOCOL_ERROR_PREFIX: &str = "ERR Protocol Error: ";
  ...
  /// 动态实参组串后落唯一成帧点，对标 C# `$"ERR Slot {slot} ..."` → TryWriteError）
  #[inline]
  pub fn write_slot_duplicate_error(output: &mut Vec<u8>, slot: i64) {
    write_error_raw(output, &format!("ERR Slot {slot} specified multiple times"));
  }

  /// 写出区间倒挂错误帧 `-ERR Invalid range <start> > <end>!\r\n`
  /// （文案取自 C# ClusterCommands 的 TryParseSlots 同名报错，判定序与错误映射定义
  /// 在 wedb cluster_session 的 try_parse_slots，此处只承载其错误帧单点写出）
  #[inline]
  pub fn write_slot_range_error(output: &mut Vec<u8>, start: i64, end: i64) {
    write_error_raw(output, &format!("ERR Invalid range {start} > {end}!"));
  }

  /// 库级分片迁移域限制文案模板（本仓多库定槽专有，C# 单库无此面；{0} = 被拒槽位）
  ///
  /// 迁移驱动侧（键扫描会话与迁移帧落域）目前只覆盖默认域 `(ns=0, db=0)` 的槽位，
  /// MIGRATE 臂与 CLUSTER SETSLOT/SETSLOTSRANGE 的 MIGRATING 臂共用本单源文案，
  /// 调用点不组串（判据单点见 `wedb::server::cluster_session::migration_slot_supported`）
  pub const GENERIC_ERR_MIGRATION_SLOT_DOMAIN: &str =
    "ERR migration of slot {0} is not supported: only the default database slot can be migrated";

  /// 渲染库级分片迁移域限制文案（MIGRATE 解析期与其它首错聚合共用，
  /// 单点模板回填；调用方经 `write_resp_error` 落唯一成帧点）
  #[inline]
  #[must_use]
  pub fn migration_slot_domain_error(slot: i64) -> String {
    GENERIC_ERR_MIGRATION_SLOT_DOMAIN.replace("{0}", &slot.to_string())
  }

  /// 写出 MOVED/ASK 重定向错误行 `-<kind> <slot> <endpoint>:<port>\r\n`
  /// （对标 C# RespClusterSlotVerify.cs 的 `$"MOVED {slot} {endpoint}:{port}"` 组串
  /// 经 `Encoding.ASCII.GetBytes` 收口后落 TryWriteError 单一通路，rust 组帧落
  /// wresp 唯一成帧点）：端点实参在入帧前经 `wbase::ascii_sanitize` 单机制逐字节
  /// 折 '?'（对标 C# 组串处的 ASCII 编码折叠；值域收拢恒 ASCII，非逐字节等形，
  /// 折叠机制单源见 `wbase/src/ascii.rs`、分叉登记见 deviations.md），杜绝非
  /// ASCII 宣告主机名以 UTF-8 原字节外泄至重定向帧
  #[inline]
  pub fn write_redirect_error(
    ...
    let endpoint = ascii_sanitize(endpoint.as_bytes());
    write_error_raw(output, &format!("{kind} {slot} {endpoint}:{port}"));
  }
}

pub use cluster::ERR_PROTOCOL_ERROR_PREFIX;
