...
pub fn abort_with_wrong_number_of_arguments(output: &mut Vec<u8>, cmd_name: &str) {
  ...
  let message = GENERIC_ERR_WRONG_NUM_ARGS.replace("{0}", clean_name);
...
pub fn abort_with_at_least_one_key(output: &mut Vec<u8>, cmd_name: &str) {
  let clean_name = sanitize_error_str(cmd_name, MAX_PARAM_NAME_LEN);
  let message = GENERIC_ERR_AT_LEAST_ONE_KEY.replace("{0}", clean_name);
  write_error_raw(output, &message);
}

/// 以 [`GENERIC_ERR_INVALID_CLIENT_ATTR`]（CmdStrings.cs:GenericErrInvalidClientAttr）
/// 回填属性名并写出错误应答（对标 C#
/// libs/server/Resp/ClientCommands.cs:NetworkCLIENTSETINFO 的
/// `AbortWithErrorMessage(CmdStrings.GenericErrInvalidClientAttr, parseState.GetString(0))`，
/// 组文案一次堆分配后落唯一成帧点）
#[inline]
pub fn abort_with_invalid_client_attr(output: &mut Vec<u8>, attr: &str) {
  let clean_attr = sanitize_error_str(attr, MAX_PARAM_NAME_LEN);
  let message = GENERIC_ERR_INVALID_CLIENT_ATTR.replace("{0}", clean_attr);
  write_error_raw(output, &message);
}
...
pub fn abort_with_unsupported_option(output: &mut Vec<u8>, option: &str) {
  ...
  let message = GENERIC_ERR_UNSUPPORTED_OPTION.replace("{0}", clean_opt);
  write_error_raw(output, &message);
}
...
pub fn abort_with_syntax_error_option(output: &mut Vec<u8>, cmd_name: &str, option: &str) {
  ...
  let clean_opt = sanitize_error_str(option, MAX_PARAM_NAME_LEN);
  let message = GENERIC_SYNTAX_ERROR_OPTION
    .replace("{0}", clean_cmd)
    .replace("{1}", clean_opt);
  write_error_raw(output, &message);
}
...
pub fn abort_with_unknown_subcommand(output: &mut Vec<u8>, sub_command: &str, cmd_name: &str) {
  ...
  let clean_cmd = sanitize_error_str(cmd_name, MAX_PARAM_NAME_LEN);
  let message = GENERIC_ERR_UNKNOWN_SUB_COMMAND
    .replace("{0}", clean_sub)
    .replace("{1}", clean_cmd);
  write_error_raw(output, &message);
}
...
pub fn abort_with_unknown_subcommand_or_wrong_num_args(
  ...
  let clean_sub = sanitize_error_str(sub_command, MAX_PARAM_NAME_LEN);
  let clean_cmd = sanitize_error_str(cmd_name, MAX_PARAM_NAME_LEN);
  let message = GENERIC_ERR_UNKNOWN_SUBCOMMAND_OR_WRONG_NUM_ARGS
    .replace("{0}", clean_sub)
    .replace("{1}", clean_cmd);
  write_error_raw(output, &message);
}
...
pub mod cluster {
  ...
  /// 后经 TryWriteError 单一通路，落 wresp 唯一成帧点）
  #[inline]
