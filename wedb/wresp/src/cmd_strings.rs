...
pub use crate::{MAX_ERROR_MSG_LEN, RespVecExt, sanitize_error_str};
...
/// libs/server/Resp/CmdStrings.cs:GenericErrCommandDisallowedWithOption (MODULE 实例化)
pub const RESP_ERR_MODULE_DISALLOWED: &str = "ERR MODULE command not allowed. If the enable-module-command option is set to \"local\", you can run it from a local connection, otherwise you need to set this option in the configuration file, and then restart the server.";
/// libs/server/Resp/CmdStrings.cs:GenericErrCommandDisallowedWithOption (REGISTERCS 实例化)
pub const RESP_ERR_REGISTERCS_DISALLOWED: &str = "ERR REGISTERCS command not allowed. If the enable-module-command option is set to \"local\", you can run it from a local connection, otherwise you need to set this option in the configuration file, and then restart the server.";
...
pub fn write_error_raw(output: &mut Vec<u8>, msg: &str) {
  let clean_msg = sanitize_error_str(msg, MAX_ERROR_MSG_LEN);
  output.push(b'-');
  output.extend_from_slice(clean_msg.as_bytes());
...
mod tests {
  ...
  #[test]
  fn at_least_one_key_formats() {
    assert_eq!(
      RESP_ERR_GENERIC_AT_LEAST_ONE_KEY.replace("{0}", "ZINTERSTORE"),
