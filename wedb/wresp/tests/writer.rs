//! 在 garnet 中的相对路径: libs/common/RespWriteUtils.cs + test/standalone/Garnet.test/Resp/RespReadUtilsTests.cs（RESP 写出）
use wresp::{
...
fn test_protocol_aware_lengths() {
  ...
  assert_eq!(w.out, b"*0\r\n");
  w.out.clear();
  // COMMAND DOCS 失败臂（对标 C# WriteEmptyMap 唯一服务端消费点）实走本口：
  // RESP2 `*0\r\n`、RESP3 `%0\r\n`
  w.write_map_length(0);
  assert_eq!(w.out, b"*0\r\n");
...
fn test_generic_serializable_and_writer_features() {
  use wresp::resp_memory_writer::{RespBuffer, RespProtocol};
...
/// `write_error`、带前缀入口 `write_error_with_prefix` 全部同源于
/// CRLF 切断 + 长度帽，一请求恒出一帧。
#[test]
