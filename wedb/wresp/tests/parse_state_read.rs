...
//!
//! 在 garnet 中的相对路径: libs/server/Resp/RespSession? 对标 C# SessionParseState 读取
...
/// 发现二锁用例：try_read_signed_length_header 遇到终止符次位不符，精准回显首个不符字节 'X'
#[test]
fn try_read_signed_length_header_bad_terminator_returns_first_unexpected_token() {
  use wresp::read::try_read_signed_length_header;

  // 数组头：*3\rX\r\n
  let mut length = 0;
  let mut ptr = &b"*3\rX\r\n"[..];
  assert_eq!(
    try_read_signed_length_header(&mut length, &mut ptr, b'*'),
    Err(Error::UnexpectedToken(b'X'))
  );
