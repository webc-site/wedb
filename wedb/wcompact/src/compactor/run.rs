...
struct CandidateRecord {
  addr: u64,
  /// 死亡判定（墓碑、自定义过滤或 TTL 过期，不止墓碑一种形态）
  is_dead: bool,
}
...
impl<S: CompactStore> LogCompactor<S> {
    ...
    F: FnMut(&[u8], &[u8]) -> bool,
  {
    ...
    while let Some((curr_addr, rec)) = cursor.pull().await? {
      ...
      // 墓碑短路在前，自定义谓词仅对非墓碑记录生效（对标 C# `!Tombstone && !IsDeleted`）
      if self
    ...
    // 游标推进至页末 PadRecord 时自动对齐至下一页首（段文件物理截断与页末跳过必要保障），绝不越过只读区快照
    let cursor_addr = cursor.cursor();
    if cursor_addr <= read_only_addr {
      actual_until = cursor_addr;
    }
    actual_until = actual_until.min(read_only_addr);
