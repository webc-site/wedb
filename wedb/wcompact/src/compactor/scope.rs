...
///    "已完成"的删除：结合不变量「DEL 墓碑地址大于该集合全部历史 meta 记录地址」，
///    死亡地址 ≤ actual_until 时该 key_id 的 meta 记录已随本次截断整体退役，
///    截断后日志中不再有该 key_id 的任何存活 meta 记录。
/// 2. 若之后（异常情况下）紧缩器再遇到该 key_id 的记录，get_key_id_meta 返回 None
///    ⇒ 按保守方向处理——错删方向是保守的（宁可泄漏一条元数据，绝不丢数据）。
...
impl MetaDeathScope {
  ...
  pub(super) fn observe<S: CompactStore>(
    ...
    let Some(meta) = store.parse_meta_value(val) else {
      return;
    };
    ...
    // 单调回放版本水位：仅接受比当前已知更新的版本，保证紧缩收尾的死条目
    // 回收能看到时间线上已发生过的集合创建、删除与版本演进
    if store
