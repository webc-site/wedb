...
#[derive(Error, Debug)]
pub enum Error {
  /// 紧缩目标地址超出安全只读区边界（内核自保硬拒，对标 C#
  /// "Can compact only until Log.SafeReadOnlyAddress"，TsavoriteCompaction.cs:35-36/:72-73）
  #[error("紧缩目标地址 {until_address:#x} 超出安全只读区边界 {safe_read_only_address:#x}")]
  UntilAddressOutOfRange {
    until_address: u64,
    safe_read_only_address: u64,
  },
