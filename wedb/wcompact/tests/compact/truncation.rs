...
//!
//! 在 garnet 中的相对路径: libs/storage/Tsavorite/cs/test/test.hlog/SpanByteLogCompactionTests.cs（紧缩后段截断）
...
#[compio::test]
async fn until_mid_record_aligns_to_record_end() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn record_crossing_read_only_falls_back_to_start() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn zero_retry_budget_retains_and_rolls_back_truncation() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn lazy_compaction_window_bounded_by_budget() -> Void {
  let dir = tempdir()?;
...
#[compio::test]
async fn compaction_does_not_overextend_past_until_address() -> Void {
  let dir = tempdir()?;
