...
- `CompactSession`: enter_epoch / append_record / read_ttl_expiry
...
Inline tests in src/compactor/mod.rs and src/compactor/scope.rs cover CompactionStats defaults / is_empty and MetaDeathScope death registration. The crate-level tests/compact suite (lookup_mode / scan_mode / concurrency / truncation), built on a minimal whlog + windex fixture with deterministic contention injection, covers the Lookup / Scan strategies, the four death channels (user predicate / tombstone / TTL expiry / orphan TTL), CAS contention fallback, truncation boundary semantics, and the lazy budget constraint. End-to-end engine coverage continues in wkv/tests/compact (basic / lazy_compaction / concurrency_and_collision / spanbyte_compaction / more_log_compaction).
...
src/compactor/mod.rs 与 src/compactor/scope.rs 内联测试覆盖：CompactionStats 默认值 / is_empty 与 MetaDeathScope 死亡登记。crate 级 tests/compact 集成套件（lookup_mode / scan_mode / concurrency / truncation）基于 whlog + windex 最小宿主 fixture（含确定性竞争注入），覆盖 Lookup / Scan 双策略、四通道死亡判定（用户谓词 / 墓碑 / TTL 过期 / 孤儿 TTL）、CAS 竞争回退、截断边界语义与 lazy 预算约束。引擎端到端覆盖续于 wkv/tests/compact（basic / lazy_compaction / concurrency_and_collision / spanbyte_compaction / more_log_compaction）。
