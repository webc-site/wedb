...
pub use ycsb::{WorkloadParams, YcsbHarness, pattern_value};
pub use zipf::ZipfConstants;
...
/// wbftree 读缓冲统一长度：引擎要求 out_buf 容量 >= cb_max_record_size 才走 read_into
/// 直读快路径（ops.rs 判 >= max_record_size），而 lifecycle 以 max+1 设定
/// cb_max_record_size(4097)，故取 +1；低于该长度（含 4096/512）均触发 fallback 双拷贝中转
pub const READ_BUF_LEN: usize = TUNE_BENCH.max_record_size + 1;
