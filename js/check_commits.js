...
async function loadFixedCommits() {
  ...
  try {
    ...
    if (err.code === "ENOENT") {
      console.log(`首次运行：工作产物 fixed.yml 尚未建立（${FIXED_YML_PATH}），去重通道暂无记录。`);
    } else {
...
async function main() {
  ...
  if (allCommits.length === 0) {
    console.error("语料未采到（garnet/ 缺失或 git log 失败），判定不可作数");
    process.exit(1);
  }
  ...
  console.log(`已在 fixed.yml 记录:   ${fixedCount} (来源: ${FIXED_YML_PATH})`);
