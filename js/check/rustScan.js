...
export const rsWalk = async (dir_path) => {
...
export const CS_REF_REGEX =
  ...
  csPathNormalize = (raw_cs_path) => {
    if (typeof raw_cs_path !== "string") return "";
    let cs_path = raw_cs_path.trim().replaceAll("\\", "/");
    if (cs_path.startsWith("garnet/")) {
      cs_path = cs_path.slice(7);
    } else {
      const idx = cs_path.indexOf("/garnet/");
      if (idx !== -1) {
        cs_path = cs_path.slice(idx + 8);
      }
    }
    cs_path = cs_path.replace(/^\.?\//, "");
    if (!cs_path.endsWith(".cs")) cs_path += ".cs";
    return cs_path;
  };
