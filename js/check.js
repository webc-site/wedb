...
const BASE_DIR = resolve(import.meta.dirname, ".."),
  ...
  MISS_DIR = join(ROOT_DIR, "js/check/miss");

// 运行期收口开关：ignore 档案的整删 / 就地回写 / 空目录连删是破坏性写入，
// 默认一律只读（可淘汰项仅打建议清单，不落盘），只有显式 --prune-ignore
// 或 CHECK_PRUNE_IGNORE=1 才执行既有收口动作。参数解析风格沿用 check_commits.js。
const PRUNE_IGNORE = process.argv.slice(2).includes("--prune-ignore") ||
  process.env.CHECK_PRUNE_IGNORE === "1";
...
// 默认只读：规则遍历只做映射对账，不删档、不回写；「可淘汰」以建议清单形式
// 走 diag.deleted_li（suggest 标记）打印。仅 prune=true（--prune-ignore /
// CHECK_PRUNE_IGNORE=1）时执行整删 / 回写 / 空目录连删的既有收口动作。
// 例外：内容为空 / 仅空白文件的 rm 按既有语义默认执行（无信息面，selftest 第 3 节锁定）。
const ignoreLoadAndPrune = async (doc_file_fn_map, fn_map, test_map, ignore_dir = IGNORE_DIR, prune = PRUNE_IGNORE) => {
  ...
  for (const yml_path of yml_file_li) {
    ...
    if (new_rule_li.length === 0) {
      // 既有收口动作「条目全部已在文档注释中登记即整文件淘汰」：仅显式开关下执行；
      // 只读模式沿用同一 why 文案，作为建议清单（suggest）打印，不 rm。
      if (prune) {
        await rm(yml_path, { force: true });
        diag.deleted_li.push({ path: yml_path, why: "条目已全部文档化，整文件忽略自动淘汰" });
        has_deleted_files = true;
      } else {
        diag.deleted_li.push({ path: yml_path, suggest: true, why: "条目已全部文档化，整文件忽略自动淘汰" });
      }
    } else if (file_modified) {
      if (prune) {
        await Bun.write(yml_path, yaml.stringify(new_rule_li));
      } else {
        diag.deleted_li.push({ path: yml_path, suggest: true, why: "部分条目已全部文档化，可裁剪收口" });
      }
    }
  }

  if (prune && has_deleted_files) {
    await emptyDirClean(ignore_dir);
  }

  // zcode-r139c-gateaudit 案五：原此处的 root ignore（js/check/ignore.yml）分支为
  // 零接线死代码——该文件从未存在（git 全历史零记录），global_ignore_set 恒空，
  // 且其裸名字面匹配语义与 ignore/ 语料双轨并存。连同消费点一并删除。

  return [file_ignore_map, diag];
};
...
// suggest 标记 = 只读模式的建议清单（未落盘）；无标记 = 已实际执行的淘汰。
const corpusDeletedLines = (diag) =>
  diag.deleted_li.map(({ path, why, suggest }) =>
    (suggest ? "# 可淘汰 ignore（默认只读，未落盘；--prune-ignore 收口）: " : "# 已自动淘汰 ignore: ") +
    path + "（" + why + "）");
...
const check = async () => {
  const [fn_map, test_map, cs_health] = await garnetScan(GARNET_DIR),
    ...
    [file_ignore_map, corpus_diag] = await ignoreLoadAndPrune(doc_file_fn_map, fn_map, test_map),
...
export { ignoreLoadAndPrune, corpusFailLines, corpusDeletedLines, csDegradedLines, isDocumentedAnchor };
