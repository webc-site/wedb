...
// ignore 收口默认只读，整删/回写仅在显式 prune 下发生（第 8 节）。
...
import { ignoreLoadAndPrune, corpusFailLines, corpusDeletedLines, csDegradedLines, isDocumentedAnchor } from "./check.js";
...
const run = async () => {
  ...
  console.log("# 7 miss 判定口径（词元面不作为已文档化判据）");
  // 炸点原形：isDocumented 曾兜一层 doc_set——任意 .rs 注释里出现过同名裸词
  // （叙述性提名、跨路径同名锚）就把该 C# 符号永久豁免出「实现缺失」段。
  const rs_dir = join(tmpdir(), "wedb-check-selftest-rs-" + process.pid);
  await mkdir(join(rs_dir, "src"), { recursive: true });
  await writeFile(
    join(rs_dir, "src", "sample.rs"),
    "// BareNarrative 只在叙述里被提名，实现缺失必须报出\n" +
    "fn plain() {}\n\n" +
    "/// 在 garnet 中的相对路径: anchored/Sample.cs::AnchoredFn\n" +
    "fn anchored() {}\n"
  );

  const [rs_doc_set, rs_doc_map] = await rustScan(rs_dir);

  expect(
    "词元面确实采集了叙述裸词（旧口径遮蔽源真实存在）",
    rs_doc_set.has("BareNarrative"),
    [...rs_doc_set].join(",")
  );
  expect(
    "规范锚进入按路径精确映射",
    rs_doc_map.get("anchored/Sample.cs")?.has("AnchoredFn") === true,
    JSON.stringify([...rs_doc_map.entries()].map(([k, v]) => [k, [...v]]))
  );
  expect(
    "同路径规范锚不误报",
    isDocumentedAnchor(rs_doc_map, "anchored/Sample.cs", "AnchoredFn")
  );
  expect(
    "同路径裸词提名不算已文档化（改前靠 doc_set 兜底漏报）",
    !isDocumentedAnchor(rs_doc_map, "anchored/Sample.cs", "BareNarrative")
  );
  expect(
    "跨路径同名锚不越权（path 精确映射）",
    !isDocumentedAnchor(rs_doc_map, "other/Place.cs", "AnchoredFn")
  );

  await rm(rs_dir, { recursive: true, force: true });

  console.log("# 8 剪枝开关：默认只读不删不回写，显式 prune 才收口（同一 ignoreLoadAndPrune 机制）");
  // 触发形态照第 3 节纪律：全部已文档化的整文件条目（改前默认会被整删）+
  // 混合条目与函数列表条目（改前默认被就地回写），外加一份空子目录锁 emptyDirClean。
  const PRUNE_DIR = join(tmpdir(), "wedb-check-selftest-prune-" + process.pid),
    WHOLE_YML = "- 文件:\n    - libs/common/Documented.cs\n  理由: 自检 fixture，条目全部已文档化\n",
    TRIM_YML = "- 文件:\n    - libs/common/Documented.cs\n    - libs/common/Undocumented.cs\n" +
      "  理由: 自检 fixture，混合条目\n" +
      "- 文件:\n    libs/common/Partial.cs:\n      - Done\n      - Todo\n  理由: 自检 fixture，函数列表条目\n",
    prune_fn_map = {
      "libs/common/Documented.cs": ["Foo"],
      "libs/common/Undocumented.cs": ["Bar"],
      "libs/common/Partial.cs": ["Done", "Todo"]
    },
    prune_doc_map = new Map([
      ["libs/common/Documented.cs", new Set(["Foo"])],
      ["libs/common/Partial.cs", new Set(["Done"])]
    ]);

  await mkdir(join(PRUNE_DIR, "sub"), { recursive: true });
  await writeFile(join(PRUNE_DIR, "sub", "whole.yml"), WHOLE_YML);
  const trim_path = join(PRUNE_DIR, "trim.yml");
  await writeFile(trim_path, TRIM_YML);
  const dir_gone = async (p) => { try { await readdir(p); return false; } catch { return true; } };

  // 不带 prune 调用（默认值口径）：只报不删不回写，忽略项照常进 map
  const [ro_map, ro_diag] = await ignoreLoadAndPrune(prune_doc_map, prune_fn_map, {}, PRUNE_DIR);
  const ro_rel = (p) => relative(PRUNE_DIR, p),
    ro_suggest_whole = ro_diag.deleted_li.find(
      (d) => ro_rel(d.path) === "sub/whole.yml" && d.why === "条目已全部文档化，整文件忽略自动淘汰");
  expect(
    "只读：全已文档化整文件不删（改前默认 rm）",
    await exists(join(PRUNE_DIR, "sub", "whole.yml")) &&
      ro_suggest_whole?.suggest === true &&
      await Bun.file(trim_path).text() === TRIM_YML &&
      !(await dir_gone(join(PRUNE_DIR, "sub"))),
    JSON.stringify(ro_diag.deleted_li.map((d) => [ro_rel(d.path), !!d.suggest, d.why]))
  );
  expect(
    "只读：可裁剪条目进建议清单且不回写（沿用 deleted_li 通道）",
    ro_diag.deleted_li.some(
      (d) => ro_rel(d.path) === "trim.yml" && d.suggest === true && d.why === "部分条目已全部文档化，可裁剪收口") &&
      corpusDeletedLines(ro_diag).some((l) => l.startsWith("# 可淘汰 ignore") && l.includes("trim.yml")) &&
      corpusDeletedLines(ro_diag).every((l) => !l.startsWith("# 已自动淘汰")),
    JSON.stringify(corpusDeletedLines(ro_diag))
  );
  expect(
    "只读：file_ignore_map 既有语义不变（未文档化整文件仍为 true，函数列表仍余未文档化名）",
    ro_map.get("libs/common/Undocumented.cs") === true &&
      ro_map.get("libs/common/Partial.cs") instanceof Set &&
      ro_map.get("libs/common/Partial.cs").has("Todo") &&
      !ro_map.has("libs/common/Documented.cs"),
    JSON.stringify([...ro_map.entries()].map(([k, v]) => [k, v === true ? true : [...v]]))
  );

  // 显式 prune=true：与改前默认行为逐字节一致（整删 + 回写 + 空目录连删）
  const [, pr_diag] = await ignoreLoadAndPrune(prune_doc_map, prune_fn_map, {}, PRUNE_DIR, true),
    expected_trim = yaml.stringify([
      { "文件": ["libs/common/Undocumented.cs"], "理由": "自检 fixture，混合条目" },
      { "文件": [{ "libs/common/Partial.cs": ["Todo"] }], "理由": "自检 fixture，函数列表条目" }
    ]);
  expect(
    "prune：整删 + 空目录连删 + 就地回写与改前逐字节一致",
    !(await exists(join(PRUNE_DIR, "sub", "whole.yml"))) && await dir_gone(join(PRUNE_DIR, "sub")) &&
      await Bun.file(trim_path).text() === expected_trim,
    "actual=" + JSON.stringify(await Bun.file(trim_path).text().catch(() => null))
  );
  expect(
    "prune：淘汰汇报仍走「已自动淘汰」原文案（无 suggest 标记）",
    pr_diag.deleted_li.length === 1 && !pr_diag.deleted_li[0].suggest &&
      corpusDeletedLines(pr_diag)[0].startsWith("# 已自动淘汰 ignore"),
    JSON.stringify(corpusDeletedLines(pr_diag))
  );

  await rm(PRUNE_DIR, { recursive: true, force: true });
};
