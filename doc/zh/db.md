...
KeyTag：1 字节类型标签（String 0x00、Meta 0x01、Ttl 0x09、Vector 0x0A、Etag 0x0B、ObjectEnvelope 0x0C、Acl 0x0D、DbMeta 0x0E）
Payload：用户原始键或子键
...
集群拓扑门禁：
单机模式直接允许执行。
集群模式校验两库所属槽位是否均由当前本地节点掌管。
若分属不同物理节点（或本节点不持有任一所属槽位），直接拦截报错
...
协调者通过集群内部总线协议端口向全网所有活跃主节点广播控制帧：
ClusterMsgFlushAll { ns: u64, origin_node_id: [u8; 20], epoch: u64 }
各主节点接收广播后，在本地原子替换该租户 virtual_ns_id 并入队待回收。
全网租户多库分布式微秒级清空。
协调者收集广播确认后向客户端响应 +OK。
